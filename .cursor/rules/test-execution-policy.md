...
Тесты при этом были написаны по канону, с «арбитром» в комментарии у каждого. Причина структурная: арбитра
выбирает автор теста ПОСЛЕ написания теста, поэтому арбитр подтверждает ровно то, что автор и так покрыл.

**Механика (не «впредь буду», а роли и гейт):**

1. **Воркер пишет тесты и не подтверждает их годность.** Его собственные арбитры остаются пояснением для
   читателя, доказательством они не являются.
2. **Аудитор составляет список поломок ПО ПЛАНУ и решениям владельца, НЕ ЧИТАЯ тестов**, вносит их в продуктовый
   код и прогоняет лично. Слепота списка — суть механизма; смена автора теста её не заменяет, она лишь переносит
   ту же слепоту на аудитора, и мерить его работу становится некому.
3. **Роль запуска — `auditor-live`.** Обычный `auditor` уходит в песочницу read-only и физически не может внести
   поломку: так первый аудит уровня 2 выдал предсказания по чтению кода вместо прогонов.
4. **Критерий приёмки — «убиты ВСЕ названные планом поломки», процентного порога нет.** Google на 6000 инженеров
   и каждом диффе гейтит бинарно (diff-only, один мутант на строку, подавление тривиальных строк), а не
   процентом: процент набивается дешёвыми мутантами и не говорит, что именно не проверено.
5. **Тест, не покрасневший ни на одной чужой поломке, считается отсутствующим** — независимо от того, зелёный
   он или нет.

**Дорогой — только ПЕРВЫЙ аудит. Повторный — механика (владелец, 01.08):** «первый слепой аудит — да.
Повторно — только механика по уже готовым тестам/поломкам, и там агент попроще, может и сам воркер делать,
там нет смысла новые сессии запускать».

- **Круг 1** — слепой аудит: сильная модель, список поломок составляется заново по плану, не читая тестов.
  Это исследование, и оно стоит дорого один раз.
- **Круг 2 и дальше** — **прогон УЖЕ СОСТАВЛЕННОГО списка** по починенному коду: дешёвая модель, а если
  список зафиксирован в отчёте построчно — его повторяет сам воркер в конце своей же починки. Новую сессию
  под это не заводить.
- Это **не** возврат к самосертификации: список поломок воркеру не принадлежит, он составлен слепо и
  зафиксирован до починки. Воркер только исполняет его и показывает вывод.
- **Новый слепой аудит нужен только там, где появилась НОВАЯ поверхность** — новый модуль, новая ветка
  решения, новый гейт. На «те же тесты после правки» его не тратить.
...
7. **свой слепой список поломок составлен по плану ДО чтения тестов и прогнан лично** (см. раздел выше);
