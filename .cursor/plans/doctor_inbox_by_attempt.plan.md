...
- [x] Несколько `test_results` с одним `attempt_id` в инбоксе — **одна** группа, **одна** ссылка.
- [x] Бейдж «К проверке» отражает **число неоценённых результатов** (как сейчас по смыслу нагрузки на врача).
- [x] Новый unit-тест группировки (проект `fast`); typecheck зелёный.
- [x] Короткая запись в выбранном doc-файле; **`pnpm run ci`** зелёный.
