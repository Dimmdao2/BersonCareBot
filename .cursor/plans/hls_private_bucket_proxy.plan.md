...
todos:
  ...
  - id: docs-retention
    ...
    status: pending
  - id: verify-smoke
    content: "Чеклист ручного smoke (DevTools); при наличии — расширить webapp integration тесты маршрута"
    status: pending
isProject: true
---
...
- [ ] Реализовать S3 GetObject streaming + Range + типизированные ошибки.
- [ ] Unit-тесты маппинга ошибок (мок SDK или обёртка).
- Локальная проверка: `pnpm --dir apps/webapp test` для затронутых файлов.

**Фаза B — Прокси-роут**

- [ ] `hlsDeliveryProxy` + route GET (и опционально HEAD).
- [ ] Интеграционные тесты роута с моком S3 / существующим паттерном из playback tests.
- [ ] Плейлисты: чтение буфером + rewrite.

**Фаза C — Контракт playback**

- [ ] Изменить `resolveMediaPlaybackPayload`; обновить типы и `playback` tests.
- [ ] Smoke: один медиа-файл с ready HLS в dev.

**Фаза D — Наблюдаемость**

- [ ] Миграция + `recordHlsProxyError` только на error path.
- [ ] System health probe + UI accordion.
- [ ] Retention route + строка в `api.md`.

**Фаза E — Документация и CI**

- [ ] Обновить все docs из §12.
- [ ] `pnpm install --frozen-lockfile && pnpm run ci` перед merge (политика репозитория).
