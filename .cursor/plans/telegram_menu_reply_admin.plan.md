...
- [apps/integrator/src/content/telegram/user/menu.json](apps/integrator/src/content/telegram/user/menu.json) — опционально (паритет с reply); если не трогать — инлайн «main» останется с тремя кнопками до отдельной задачи.
...
| E | При необходимости добавить `questions.mark_all_answered` в `excludeActions` у сценария admin reply message, если матчинг по тексту/callback может пересечься (оценка по факту после реализации). | Нет ложных срабатываний |
...
| [scripts.json](apps/integrator/src/content/telegram/user/scripts.json) | Все `message.replyKeyboard.show` с тройкой `menu.diary` / `menu.more` заменить на ту же пару кнопок |
...
**Инлайн [menu.json](apps/integrator/src/content/telegram/user/menu.json):** в Definition of Done явно: либо обновить в этом PR, либо завести backlog «паритет reply vs inline main».
...
1. У админа в меню команд Telegram нет `start` и `show_my_id`; остальные админские команды на месте.
2. В сообщении со списком неотвеченных (TG и MAX, т.к. один builder) есть кнопка «пометить все»; callback обрабатывается; в БД помечаются ответы по правилу п. 2.2; есть unit-тесты на mapIn + executeAction.
3. Reply-меню и welcome-keyboard после линка — две кнопки; `rg` не находит старую тройку в user `scripts.json` и в указанном блоке `executeAction` (исключение: осознанно не тронутый `menu.json`).
4. `TELEGRAM_REPLY_MENU_ACTIONS` согласован с фактическими reply-кнопками; `MESSAGE_MENU_ACTIONS_NEED_PHONE` не «очищали» от `diary`/`menu.more`.
5. Прогон целевых тестов/линта integrator по затронутым файлам; перед пушем — полный `pnpm run ci` по правилам репозитория.

---

## Ссылки на исходный черновик

Копия ранее сгенерированного плана: `~/.cursor/plans/telegram_menu_updates_c6bf2302.plan.md` — **канон для исполнения**: этот файл в репозитории.
