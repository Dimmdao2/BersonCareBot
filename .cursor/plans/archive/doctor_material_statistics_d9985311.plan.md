...
**Продуктовое решение (зафиксировать в docs):** врач видит **те же платформенные** агрегаты, что оператор в админке по вкладке «Статистика» — не фильтр «только мои пациенты». Если позже понадобится сужение — отдельный порт с фильтром по `doctor_id` / клинике.
...
- Фильтр статистики по врачу/клинике — новый порт и политика доступа.
