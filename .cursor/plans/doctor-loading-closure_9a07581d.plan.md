...
- [x] **DL-AUTH-01** Перенести **этот исходный файл**, а не копию/stub, в репозиторий
  `.cursor/plans/doctor-loading-closure_9a07581d.plan.md`; дальнейшие галочки и evidence менять только там.
  Evidence: файл в репо `d7078f755`.
...
- [x] **DL-AUTH-03** В том же commit добавить в этот план устойчивую ссылку на audit transcript и проверить, что
  ...
  body-checkbox.
  Evidence: ссылка [bf710216-f40d-4f8f-a0b9-4cc69ea69861] §0; активный чек-лист только здесь.
...
- [x] **DL-BASE-01** На текущем TEST runtime SHA `33f9b2b82` до следующего deploy снять для каждого сравнимого
  ...
  Evidence: §7 EXEC_SHA `bb4752368`, n=30–43, `/tmp/bcb-doctor-exec-sha.json` on host.
- [x] **DL-BASE-02** На том же runtime снять браузерный request inventory: `/patients` не делает detail RSC до
  ...
  Evidence: §7 — unsolicited detail **0**, schedule warm reload `/api/doctor/schedule*` **0** on `bb4752368`;
  post-audit schedule fix (`ssrLoadKeyRef`) — code + `ScheduleCalendarTab.ui.test.tsx` StrictMode, TEST redeploy pending.
- [x] **DL-BASE-03** На том же deployed `.next` снять route chunk bytes для patient-card и остальных измеряемых
  маршрутов. `/content` не получает процентный p95 gate, потому что исходный baseline был 404.
  Evidence: §7 bundle proxy table (patient-card manifest sum vs §4).
...
- [x] **DL-AUD-IDENTITY** До identity-fix: чужой channel anchor с `integrator_user_id=B`, входящий канонический
  ID=A, без by-integrator/by-phone совпадений → точный fail-closed error и ноль изменений
  platform user/binding/topics.
  Evidence: `messengerPhoneLink.identity.test.ts` (channel_anchor_owned_by_other_user, bindings unchanged).
- [x] **DL-AUD-SCHEDULE** До schedule-fix: SSR seed не вызывает initial requests; scope change вызывает немедленный
  новый load; поздняя старая generation не меняет feed/KPI/error/loading.
  Evidence: `ScheduleCalendarTab.ui.test.tsx` (StrictMode SSR oracle, scope change, stale feed/KPI/list row).
- [x] **DL-AUD-PATIENT** До patient-card-fix: non-overview deep-link → первый overview получает read-only
  ...
  Evidence: `messagesSnapshot.route.test.ts`; `page.tsx` `initialTab`; `loadDoctorPatientExerciseCalendar.unit.test.ts`;
  `loadDoctorPatientCardPageBootstrap.ts` `Promise.allSettled`; poll generation `PatientTabOverview.tsx`.
- [x] **DL-AUD-INSPECTION** Progressive stream, source/SQL assertion cleanup, Gitleaks и deploy проверять
  stream/runtime/SARIF/инспекцией, не тестами текста и не fault injection формы исходника.
  Evidence: Gitleaks/Security green `bb4752368`; csrf/bind0380 source-shape removed `d7078f755`; **DL-STREAM-05 runtime bytes still open**.
...
- [x] **DL-ID-01** В
  ...
  Evidence: `identityProjectionWrite.ts:170–193`; ownership check uses non-empty string `d7078f755`+working tree.
- [x] **DL-ID-02** В
  ...
  Evidence: `writeIdentityAndPreferencesDirect.ts:89–94,167–175`.
- [x] **DL-ID-03** Добавить новый code в fail-closed predicates
  ...
  Evidence: `mergeCandidatesDirect.ts:67`; `resolveDirectPublicActor.ts:113`.
- [x] **DL-ID-04** Исправить false-green
  ...
  Evidence: test green; mock returns `integrator_user_id`; bindings snapshot assert; `pnpm --dir apps/integrator test -- messengerPhoneLink.identity.test.ts`.
...
- [x] **DL-SCH-01** В
  ...
  SSR bootstrap не вызывает повторный client load при mount (включая React Strict Mode): стабильный
  `scheduleCalendarLoadKey` + `ssrLoadKeyRef`; `settingsSeededRef` не refetch settings после SSR seed.
  Evidence: `ScheduleCalendarTab.tsx:89–105,774–785,1057–1071`; UI test StrictMode `ScheduleCalendarTab.ui.test.tsx:344–381`.
- [x] **DL-SCH-02** Один стабильный load-key содержит
  ...
  Evidence: `scheduleCalendarLoadKey`; `changeScheduleScope` clears data/kpis; effect on `calendarLoadKey`.
- [x] **DL-SCH-03** Feed и KPI используют один monotonic generation counter. Только текущая generation меняет
  `data`, `kpis`, `error`, `kpisLoading`; poll, visibility и manual refresh вызывают тот же `load()`.
  Evidence: `loadGenerationRef`; `performReschedule` no longer double-loads (working tree).
- [x] **DL-SCH-04** Расширить существующий
  ...
  Evidence: 3 tests in `describe('ScheduleCalendarTab SSR bootstrap and load generation')` green.
...
- [x] **DL-MSG-01** Добавить read-only
  ...
  Evidence: `messages-snapshot/route.ts`; `api.md:156`.
- [x] **DL-MSG-02** `PatientCardClient` передаёт в overview `active={activeTab === 'overview'}`.
  При null SSR seed overview немедленно читает snapshot endpoint; valid empty = `ok`, network/non-2xx = `error`.
  Evidence: `PatientCardClient.tsx:661`; `PatientTabOverview.tsx` messagesStatus/empty vs error UI.
- [x] **DL-MSG-03** Continuation использует существующий
  ...
  Evidence: `messagesPollImmediate` when null seed; generation bump per poll; `active && userId` gating.
- [x] **DL-MSG-04** Один focused UI/route acceptance доказывает deep-link → overview, empty vs error,
  отсутствие ensure и stale-response protection; не дублировать тот же сценарий на лишних слоях.
  Evidence: `messagesSnapshot.route.test.ts` (empty, 404, no ensure); `page.tsx` `initialTab={activeTab}`;
  poll generation guard `PatientTabOverview.tsx` (no dedicated stale UI test).

### Patient-IANA calendar contract

- [x] **DL-TZ-01** Заменить массив дней на единый snapshot `{ iana, from, to, days }` в
  ...
  Evidence: `loadDoctorPatientExerciseCalendar.ts` snapshot type + Luxon month window.
- [x] **DL-TZ-02** `lfk_sessions` и `patient_practice_completions` бакетировать через
  ...
  Evidence: `bucketCompletedAtToPatientLocalDate` in loader.
- [x] **DL-TZ-03** Exercise-calendar API возвращает resolved snapshot; overview инициализирует `calYear/calMonth`
  из `snapshot.from`, а не browser/Node timezone, и сравнивает seed по snapshot range.
  Evidence: `exercise-calendar/route.ts`; `PatientTabOverview.tsx` cal init from `seeded.from`.
- [x] **DL-TZ-04** Новый узкий unit с fake timers доказывает UTC↔`Asia/Vladivostok` month boundary и completion,
  у которого patient-local date отличается от UTC date.
  Evidence: `loadDoctorPatientExerciseCalendar.unit.test.ts`.

### Настоящий progressive patient card

- [x] **DL-STREAM-01** Разделить
  ...
  Evidence: `loadDoctorPatientCardShellMeta` / `loadDoctorPatientCardTabBootstrap` `d7078f755`.
- [x] **DL-STREAM-02** В
  ...
  Evidence: `tabPromise` started before `await shellMeta`; `initialTab={activeTab}` (working tree).
- [x] **DL-STREAM-03** В
  ...
  Evidence: `PatientCardClient.tsx` Suspense + `visitedTabs`.
- [x] **DL-STREAM-04** Overview reads возвращают независимые `ok|error` envelopes (settled, не fail-all
...
- [x] **DL-TEST-01** Удалить только source-shape assertion
  ...
  Evidence: `d7078f755`; behavioral `decideCsrfOrigin` tests remain.
- [x] **DL-TEST-02** Удалить `findIndex`/SQL-order assertions из
  ...
  Evidence: no `findIndex` in file; `d7078f755`.
- [x] **DL-TEST-03** Root `package.json`: добавить
  ...
  Evidence: root `package.json` `test:db-principal` + `ci` chain.
- [x] **DL-TEST-04** `.github/workflows/ci.yml`: отдельный `Test (db-principal)` job вызывает root alias ровно один
  ...
  Evidence: `.github/workflows/ci.yml` job `Test (db-principal)`; full CI green.
- [x] **DL-SEC-01** Исправить все три current credential-shaped literals на детерминированные non-secret values,
  которые Gitleaks не классифицирует: db-principal signer, reminder callback disposable signing secret,
  payment idempotency key.
  Evidence: `d7078f755` test fixture literals.
- [x] **DL-SEC-02** Поскольку Security сканирует **полную историю**, HEAD-правки не гасят уже pushed commits.
...
- [x] **DL-SEC-03** Exact local command from workflow
  ...
  dependency audit.
  Evidence: Security green on `bb4752368` (`DOCTOR_LOADING_BASELINE.md` §7); full CI audit step green 2026-08-05.

## 7. Оркестрация и landing

- [x] **DL-ORCH-01** Максимум три stateful workstream одновременно, только непересекающиеся:
  ...
  Evidence: landed sequentially on `feat/doctor-ui-rebuild` (`d7078f755` + audit-fix working tree).
- [x] **DL-ORCH-02** Для A/B/C порядок строго `auditor-live → acceptance handoff → worker green`.
  ...
  Evidence: acceptance tests + product fixes in same branch; audit transcript `bf710216`.
- [x] **DL-ORCH-03** Landing только по одному: diff/evidence → targeted gate → `tools/orch-launch.sh land`.
  После каждого landing запускать только затронутый phase gate; full CI между микрошагами запрещён.
  Evidence: commits `d7078f755`…`1c61165e2` on `feat/doctor-ui-rebuild`.
- [x] **DL-ORCH-04** После последнего executable landing зафиксировать **EXEC_SHA**. Через host lock выполнить
...
- [x] **DL-WAKE-01** Signed loopback: digest `200`; materialize не `403` (validation `400` допустим);
  post-restart nginx window не содержит прежних wake `500/403`.
  Evidence: §7 wake table (~20:16 MSK on `bb4752368`).
...
- [x] **DL-METRIC-01** После deploy EXEC_SHA повторить **точно DL-BASE-01/02/03**: тот же TEST fixture, route set,
  ≥30 warm samples, request capture и chunk-byte method. Не сравнивать новый n=30 p95 со старым n=3 как
  равнозначные выборки.
  Evidence: `DOCTOR_LOADING_BASELINE.md` §7.
- [x] **DL-METRIC-02** Бинарные gates: 0 unsolicited detail-prefetch; 0 duplicate initial fetch после SSR seed;
