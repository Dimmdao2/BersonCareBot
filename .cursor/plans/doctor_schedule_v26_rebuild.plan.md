...
### Исправлено оркестратором

- **firstVisitInPeriod** в `pgDoctorCanonicalAppointments` — «первичная» = первая запись пациента
  вообще (строгий порядок `(start_at, id)`), а не «до начала окна». Коммит `1743b445`.

### Добавлено субагентом

- **route-тест nearest-free-window** —
  `apps/webapp/src/app/api/doctor/schedule/nearest-free-window/route.test.ts`
  (10 тестов: 401/403 gate; 400 невалидный UUID; happy-path window+args; деградация при throw
  и при отсутствии deps.bookingScheduling → `{ ok:true, window:null }`, не 500).
- **Типизация fcViews** — `ScheduleCalendarTab.tsx`: возвращаемый тип useMemo изменён с
  `Record<string, object>` на `NonNullable<FullCalendarOptions["views"]>` (импорт
  `CalendarOptions as FullCalendarOptions` из `@fullcalendar/core`); `as any` на JSX-пропе `views`
  убран полностью. tsc зелёный.

### Явно отложено (deferred, не блокеры)

- [ ] Полный `pnpm run ci` перед merge.
- [ ] Независимая верификация применения миграций 0116/0117 на dev/целевых окружениях.
- [ ] Runtime/визуальная проверка на 127.0.0.1:5200 (виды «3 дня» / «График работы» / «Лента»).
- [ ] inMemory KPI/booking-scheduling = zero-stub (нет датасета) — by design, не баг.
