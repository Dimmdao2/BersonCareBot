...
6. держать целевую изоляцию каталогов: `src/domain`, `src/db`, `src/orchestrator`, `src/ports`, `src/integrations`.
...
Нужно:
- убрать "размазанные" сценарии по `content`/`integrations`/`dispatchers`/`usecases`;
- выделить orchestration-слой в `src/orchestrator`;
- вынести интерфейсы в `src/ports`;
- оставить в `src/integrations` только адаптеры (вход/выход и SDK вызовы).

Критерий завершения:
- путь обработки соответствует канону:
  `incoming -> middleware -> router -> orchestrator -> domain -> db/dispatch`.
...
- Главный незакрытый риск: структурная "смешанность" сценариев по папкам, мешающая полной изоляции (`orchestrator/ports` еще не доведены до целевого состояния).
