...
- [x] Watermark включается только по явному флагу/политике и не затрагивает базовый pipeline без флага.
- [x] Зафиксирована и согласована политика PII для watermark (или явно выбран вариант без PII).
- [x] Рост времени/стоимости транскодинга измерен и признан приемлемым для выбранного scope.

**Реализация (phase-10):** только глобальный флаг `video_watermark_enabled`; текст watermark — **`id <media UUID>`** (non-PII по выбранной политике). Детали: [`PHASE_10_WATERMARK_POLICY.md`](../PHASE_10_WATERMARK_POLICY.md).
