...
- [ ] `video_default_delivery=auto` (или `hls`) включен после прохождения gate и подтвержденного fallback.
- [ ] Доля fallback на MP4 контролируемая и не указывает на массовую деградацию HLS.
- [ ] Rollback до `video_default_delivery=mp4` проверен и документирован как быстрая операция.
