...
**Вывод:** инициатива по **коду и репозиторным** артефактам в состоянии приёмки. **Major FIND-P08-1** закрыт в scope репозитория: шаблоны SQL, логи, Safari-чеклист. Подпись на конкретной среде (staging/prod) — **ops/QA**.
...
| ID / источник | Суть | Статус после global fix |
|---------------|------|-------------------------|
...
### Minor / INFO — явный defer (не блокируют merge)
...
| systemd unit + строка в SERVER CONVENTIONS для media-worker | **DEFER (ops)** — имя unit не подтверждено host audit; канон: `deploy/HOST_DEPLOY_README.md` до фиксации в conventions |
