...
- **В репозитории BersonCareBot** отдельное дерево: **`<репо>/.cursor/plans/`** — см. [`.cursor/plans/archive/README.md`](../.cursor/plans/archive/README.md). Пример закрытого трека: **Integrator → Drizzle** (2026-05-15, архив **2026-06-04**: `.cursor/plans/archive/integrator_drizzle_*.plan.md` + [`docs/INTEGRATOR_DRIZZLE_MIGRATION/LOG.md`](INTEGRATOR_DRIZZLE_MIGRATION/LOG.md)); закрытое по методологии — в **`<репо>/.cursor/plans/archive/`**.
