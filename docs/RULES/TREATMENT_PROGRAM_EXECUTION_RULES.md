...
- **Обсуждение по пунктам программы (закрыта 2026-05-30):** `docs/archive/2026-05-initiatives/PROGRAM_ITEM_DISCUSSION_INITIATIVE/LOG.md` — thread per stage item, feature flags, patient media submission; plan — `.cursor/plans/archive/program_item_discussion_070c3846.plan.md` (`status: completed`).
...
- Миграции: `drizzle-kit generate` → применение на БД **только санкционированным прогонщиком**: `bash deploy/host/migrate-dev.sh --execute` (DEV), `bash deploy/host/deploy-test.sh <branch>` (TEST), `pnpm --dir apps/webapp run migrate` (локально, по `DATABASE_URL`). `drizzle-kit migrate`, `db:migrate:drizzle` и `db:seed-drizzle-meta` **сняты и отказывают**: первый применял по водяному знаку журнала, последний писал строки «применено» вообще без выполнения SQL. Не ручной SQL для новых сущностей. Исключение: seed data. Канон: [`AGENTS.md`](../../AGENTS.md), «Миграции после baseline B0».
- Активная миграционная цепочка начинается с B0 и продолжается только файлами из `apps/webapp/db/drizzle-migrations/`; historical replay, legacy/bootstrap, disposable и A→B0 execution paths отсутствуют из active checkout.
