...
Command executed from the candidate checkout: `node /tmp/reclaim-audit-probe.mjs` (exit success). The exact candidate-pinned one-time script remains reproducible from audit commit `ed222b9cd`; it is deliberately absent from the active tree because it is neither a permanent suite test nor a CI input. Its baseline assertions were all true, and every injected class made its named oracle false; an escaped mutation exited unsuccessfully.
