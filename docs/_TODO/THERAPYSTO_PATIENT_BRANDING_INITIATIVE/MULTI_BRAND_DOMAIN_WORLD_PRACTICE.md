...
- **Passkey — самое жёсткое.** Ключ привязан к RP ID (домену, для которого он создан). Спецификация WebAuthn
  ...
  **Пять доменов — это потолок.** Значит passkey **физически не масштабируется** на много клинических доменов. Это
  независимое техническое подтверждение решения владельца от 22.08 убрать passkey из докторского приложения и не
  заводить его на пациентских поверхностях: даже если бы захотели — на шестой клинике механизм бы кончился.
