...
- 🔴 **Работа этапа `A` НЕ ВЛИВАЕТСЯ в `feat/doctor-ui-rebuild` и не приземляется** до отдельной команды
  владельца. Живёт на `wt/therapysto-stage-a-20260822`. Причина: владелец даёт оба домена сразу, и
  переезд идёт одним куском, а не промежуточными состояниями в общей ветке.
