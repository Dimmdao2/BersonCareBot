...
- [x] `F2` Выключить OAuth-вход на staff/admin surface значением в матрице механик. Ничего не удалять: UI,
  ...
  **Закрыт 24.08.2026:** resolver-backed `isAuthMechanicEnabled` — единый server gate для OAuth start/callback; `authDeliveryGate.unit.test.ts`, `oauthAppleToggle.route.test.ts` и `callback/apple/route.test.ts` зелёные, а удаление OAuth gate даёт red.
- [x] `F2b` Passkey НЕ удалять. Подключить его к матрице механик как переключаемую опцию, по умолчанию
  ...
  **Закрыт 24.08.2026:** matrix default `staff.passkey=false`; все passkey entrance routes идут через тот же gate. `independentAuthMethodToggle.route.test.ts` зелёный и краснеет при снятой проверке входа.
