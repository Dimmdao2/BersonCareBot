...
**Дата:** 2026-08-21. **Актуализировано:** 2026-09-07 по текущему `feat/doctor-ui-rebuild` и решениям владельца о доменах.

**Текущее состояние.** Код Therapysto/Therapygo и patient-branding сведён в `feat`, но branded Host-путь не
работает ни на DEV, ни на TEST, ни на новом пробном PROD. `proxy.ts` принимает тестовый `TenantSurfaceLookup`
вторым аргументом, тогда как Next передаёт туда `NextFetchEvent`, после чего production runtime использует
`NO_TENANT_SURFACE`. Поэтому проверки с вручную собранным lookup не доказывают живой request path; `B1`, `B3`
и `B4a` ниже вновь открыты. Существующая однохостовая схема `test.bersoncare.ru` продолжает работать только
благодаря transitional fallback; значения `therapysto.ru`, `admin.therapysto.ru`, `therapygo.ru`,
`*.therapygo.ru` и домены клиник в runtime/proxy/TLS не активированы.
...
`TPB-20`/`TPB-21`, а также общий с Track D живой provider-gate телефона/кода/доставки `TPB-12`. Остальная кодовая
...
| `Q2` | Второй фактор у специалистов | **Решение обновлено владельцем 07.09.2026.** На первом запуске обычный вход — email + пароль без обязательного кода из письма. Если специалист добровольно подключил TOTP, после пароля требуется его код; обязательность 2FA для клиники/роли решается позже. Подтверждение email при регистрации и восстановление доступа — служебные письма, а не второй фактор каждого входа. | Существующий добровольный TOTP сохраняется. Старое обещание обязательного email-кода удалено как заменённое более поздним решением. |
...
- **🔴 СКВОЗНОЕ ПРАВИЛО: механики не вырезаем, их выключают.** Владелец, 22.08.2026: «я же тебе сказал уже,
  ...
  работает».
- **Значения первого запуска для докторов (владелец, 07.09.2026):** email + пароль; обязательного email-кода
  после пароля нет. Добровольно подключённый TOTP проверяется после пароля. Будущая обязательность 2FA для
  клиники/роли не решена и не включается автоматически.
...
- [ ] `TPB-21` Обычный staff-вход не требует кода из письма после верного пароля. Если пользователь сам подключил
  TOTP, существующий factor-step обязателен; обязательность TOTP по клинике/роли не вводится. Служебная email-
  доставка подтверждения регистрации и восстановления доступа работает независимо от выключенного passwordless
  email-code login. Доказательство: behavior-тесты трёх путей — password-only без TOTP, password→TOTP при
  подключённом факторе, signup/recovery email при выключенном email-code login.
- [ ] `TPB-22` Patient login на `therapygo.ru`, `<slug>.therapygo.ru` и собственном домене клиники называется
  ...
  запуске доступны email и подтверждение телефона через бота; OAuth/passkey и остальные сохранённые механики
