...
**`W3` ЗАКРЫТ вариантом (а).** Пациент клиники приходит на **наш** поддомен вида `<клиника>.<patient-domain>`
(например `personcare.therapygo.ru`). Собственный домен клиники — отложенная платная опция, не часть базового
пути.
