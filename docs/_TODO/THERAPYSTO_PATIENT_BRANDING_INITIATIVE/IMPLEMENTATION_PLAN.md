...
- **Собственный домен клиники — self-service-функция, решение владельца 07.09.2026.** Ручной выпуск сертификата
  ...
  2. на основном домене уже есть сайт — система сама добавляет `app.` и получает `app.clinic.ru`; кабинет
     принимает любой из двух эквивалентных вариантов: `A app → <стабильный IP edge>` **или**
...
- **Кабинет надо изменить, одной правки текста недостаточно.** Нынешнее одиночное поле сохраняет введённый
  ...
  сервере вычисляет exact hostname, показывает допустимую A-инструкцию для корня либо альтернативы A/CNAME
  для `app.`-поддомена и состояния
...
`A @ → <стабильный IP edge>`; для сохранения существующего сайта — вычисленный нами `app.<домен>` и любой
из двух поддерживаемых вариантов: `A app → <стабильный IP edge>` либо
`CNAME app → <канонический domain target платформы>`. Проверка принимает успех любого одного варианта.
Сертификат выпускает и продлевает автоматика платформы.
...
`app_tenant_service` должна получать отказ. На TEST branded Berson Care patient custom domain —
`app.bersoncare.ru`; `test.bersoncare.ru` из активного TEST-контура исключён.
...
- [ ] `B7` Выпустить и продлевать сертификат, который явно содержит `therapygo.ru` и `*.therapygo.ru`;
  ...
  * Сертификат сейчас ИМЕННОЙ список без wildcard: `therapysto.ru`, `admin.therapysto.ru`,
    ...
    `turn.therapysto.ru`, `www.therapygo.ru`, `www.therapysto.ru` (один `certbot`-сертификат
    `therapysto`, HTTP-01, `authenticator = nginx`). Запись DNS `*.therapygo.ru` при этом УЖЕ
    смотрит на `135.106.187.95` — проверено, произвольное имя резолвится.
  * Следствие, которое владелец увидел сам 11.09: любой поддомен, которого нет в этом списке,
    отдаёт браузеру ошибку сертификата ДО того, как приложение успевает ответить хоть что-нибудь.
    Дословно: «открывается страница, на которой мне пишет, что там какой-то отсутствующий
    сертификат… HTTPS не установлен». Это же закрывает дорогу любой НОВОЙ клинике: её
  ...
  * Расхождение в документации: `docs/_TODO/CUSTOM_DOMAIN_TLS_EDGE_RUNBOOK_2026-09-07.md` и
    ...
    `/opt/therapysto/env/caddy.prod`. Класть креды по адресу из runbook — значит получить
    `missing …/caddy.prod` на cutover.
- [ ] `B7a` **Адрес клиники в кабинете — поддомен, а не слаг после домена** (владелец 12.09:
...
5. ~~форма собственного домена и выпуск TLS~~ — **ЗАКРЫТ 10.09.2026:** если сайта нет, приложение может жить
   прямо на введённом корневом домене через `A @`; если сайт есть, клиника вводит базовый домен, а `app.`
   добавляет система и принимает на выбор `A app` либо `CNAME app`. Для клиента достаточно одной рабочей
   DNS-записи любого из этих двух типов.
