...
- **Ничего уже написанного не удаляется.** Владелец: «я просто не хочу удалять то что уже сделано».
  Выключенная механика остаётся в коде и включается настройкой, когда понадобится.
...
- [ ] `TPB-19` Механики входа настраиваются в админке матрицей «поверхность × механика», раздельно для докторов и
  ...
  ⚠️ ОТКРЫТО (`OG-5`): прежняя редакция требовала, чтобы OAuth на staff-поверхности нельзя было включить в
  принципе. Это противоречит модели «все механики включаются в админке». Развилка владельцу: OAuth для докторов
  — (а) вообще отсутствует в списке механик, или (б) присутствует, но выключен по умолчанию. Safe default до
  ответа: (б) — выключен, потому что это единообразно с passkey и ничего не удаляет.
