...
- [ ] **C4. Неспешная диагностика первого открытия TEST.** По наблюдению владельца 24.08.2026 сам TEST-сервер
      не открылся при первом заходе и показал production-style Server Components error (`digest 2128085231`).
      Это не переход по кнопке поддержки и не дефект helpdesk. Позднее сопоставить время/digest с серверным
      error tracking и логами, воспроизвести начальную загрузку и устранить фактическую причину без догадки по
      клиентскому generic-тексту.
