...
Пункт считается закрытым только при: (1) галочка в этом файле; (2) зелёный **full CI** (не только typecheck);
(3) **живая проверка** на ТЕСТ-стенде. «Audit PASS» и «зелёные моканые тесты» сами по себе — НЕ «готово».
Прод не трогаем: вся работа на `test`/dev, миграции — только на тест-БД.
...
- **Phase 0**: A1 — критический путь (делать первым); C2/F1/D3 идут параллельно ему.
...
## Приёмка в середине
После Phase 1 — выкатить накопленное на ТЕСТ (code-only) и передать владельцу на клик-through ДО старта Phase 3.
Не полировать A4, пока Phase 0-1 не приняты живьём.
