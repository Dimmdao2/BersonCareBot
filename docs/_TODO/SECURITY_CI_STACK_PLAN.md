...
**Режимы (решение владельца 2026-07-19):**
- **Пассивный (baseline)** — только наблюдение, ничего не отправляет/не меняет. Безопасен, в т.ч. по проду.
...
| Прод (после 1-го прод-деплоя) | **только пассивный baseline** | публичный, достаём напрямую (VPN не при чём) | еженедельно + после каждого релиза |
...
- [ ] **Прод — ТОЛЬКО пассивный baseline**, добавляется в скоуп после первого прод-деплоя. Активный по проду не запускать никогда.
...
## Границы / правила
- Ничего разрушительного против **прода** (память `prod-is-untouchable-hard-rule`). По проду ZAP — **только пассивный baseline**; active-scan — исключительно тест/эфемерная копия.
