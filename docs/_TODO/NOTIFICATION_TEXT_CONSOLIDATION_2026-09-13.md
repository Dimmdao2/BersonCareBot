...
- [x] Замер (исправлен после четвёртого аудита — первое число было неверным). Команда:
      `git diff 27b8a0038 HEAD -- apps/webapp/src | grep -cE "^-.*(клиник|Клиник)"` → **100**
      изменённых строк. Прежняя цифра «62» была получена узким грепом по строковым литералам в
      кавычках, ДО того как нашлись тексты прямо в JSX, и в план попала не перемеренной.
      Слово «доктор» в текстах для человека не встречалось ни разу — только в именах файлов и
      компонентов (проверено на родительском дереве, 0 попаданий).
...
- [x] НЕ трогал лендинг (`components/landing/*` — **6 упоминаний в 4 файлах**: `HeroSection` нет,
      `PracticePathsSection.tsx:26,63`, `PricingTeaserSection.tsx:23,35`, `FinalCta.tsx:30`,
      `LandingHeader.tsx:166`; цифра «5 мест» в первой редакции была неверна). Это продающий текст, обращённый к покупателю, а не интерфейс
...
- [x] Общая карта `errorCodeText.ts` (введена в шестом проходе) дособрана: на неё переведены
      ...
      и создание карточки клиента в двух разных местах (`invalid_email`, `email_conflict`).
      Загрузка медиа на общую карту НЕ переведена — там правились сами тексты
      (`invalid_body`/`expected_multipart` вели на «Данные введены неверно», хотя человек ничего
      не вводил, а выбрал файл). Первая редакция этого пункта утверждала обратное; неправда
      названа четвёртым аудитом.
...
## Восьмой проход — закрытие FAIL четвёртого адверсарного аудита (13.09)

Независимый адверсарный аудит (Opus, не автор) вернул **FAIL, 4 блокирующих**. Все четыре
подтверждены мной самостоятельно перед правкой и закрыты.

- [x] **B1 — перечисление адресов через слаг.** `specialist-signup/start` отвечал занятому адресу
      успехом ДО проверки занятости публичного адреса, а свободному — доходил до неё и мог отдать
      `409 slug_unavailable`. Пара «занятый слаг + чужая почта» давала двоичный ответ на вопрос
      «есть ли тут аккаунт». Занятость адреса теперь проверяется ДО ветки про аккаунт, обе ветки
      отвечают одинаково.
- [x] **B2 — перечисление через двойную отправку.** Свободный адрес на втором отправлении внутри
      минуты получал `429` от кулдауна письма, занятый — тот же нейтральный `200`, потому что по
      нему кода не создаётся и кулдаун не тратится. Введён общий потолок «один старт на адрес в
      минуту» (`auth.signup_start`) в самом начале ОБОИХ маршрутов регистрации, до любой проверки
      аккаунта: вторая отправка теперь одинакова для занятого и свободного адреса.
- [x] **B3 — ссылка в письме вела не туда.** Письмо обещает «восстановите пароль по ссылке», а
      ссылка `?intent=specialist` открывала форму РЕГИСТРАЦИИ — ту самую, которая человеку только
      что отказала. Заведён настоящий вход: `/app/doctor/login?recover=1` открывает экран
      email+пароль, где стоит «Забыли пароль?». Код при переходе не запрашивается — отправку
      начинает сам человек.
- [x] **B4 — опечатка в заголовке экрана.** Моя же правка превратила «для клиник» в «для
      организик». Исправлено на «для организаций». Ни tsc, ни eslint, ни гейт такого не видят —
      поймал только человек-аудитор.
- [x] **N10 (не блокирующее, но того же класса).** При сбое почты пациентская регистрация отвечала
      свободному адресу `503 email_send_failed`, а занятому — `200`. Свободная ветка теперь так же
      превращает отказ доставки и блокировку в нейтральный успех. Цена известна и принята: в час
      сбоя почты человек видит «код отправлен» и кода не получает — ровно как на обычном входе.
- [x] **N1 — гейт закрывал одно написание из десятка.** Аудитор написал десять обходов и показал,
      что ловится один. Правило переписано на общий разбор «утекает ли ключ»: имя, шаблонная
      строка, склейка через `+`; тернарник перестал требовать именно `in` и понимает
      `СЛОВАРЬ[код] !== undefined ? … : …` и `СЛОВАРЬ[код] ? … : …`, а ключом может быть свойство
      объекта, а не только переменная. Ужесточённое правило СРАЗУ нашло живую утечку, которой не
      было в скоупе: `quota.unit in QUOTA_UNIT_LABELS ? … : quota.unit` в консоли организаций —
      неизвестная единица печаталась человеку как есть («лимит 5 seats»). Исправлено.
      Фикстур стало 50 красных против 45. Два обхода НЕ закрыты и записаны в честные ограничения
      заголовка пунктами 9-10 (чтение по словарю, поднятое строкой выше; ключ с применённым
      методом или прочитанный под другим именем).
- [x] **N2/N3 — регрессия формулировки.** Панель календаря врача до сведения карт показывала на
      `email_conflict` текст «Этот email уже занят другим пациентом. Укажите другой», а после —
      общий «Этот email уже используется другим аккаунтом.» без следующего шага. Следующий шаг
      вернулся в ОБЩИЙ текст (а не отдельной картой панели), ключ-сирота удалён.
- [x] **N4** — посимвольный дубль «Этот email уже используется другим аккаунтом» в
      `email/confirm/route.ts` сведён в ключ. **N6** — околодубль `commonNoAccessToAction` удалён,
      `forbidden` ведёт на `authForbidden`, у которого есть следующий шаг.
- [x] **N7 — слаги.** «Slug организации» → «Публичный адрес», «Slug: <код>» → «Адрес: <код>».
- [x] **N8** — вернул следующий шаг трём текстам, которые его потеряли при вычистке слова
      «клиника». **N9** — с экрана убрано имя таблицы `saas_billing_invoices`.
- [x] **N12** — исправлены два неверных числа в этом файле (см. выше, оба перемерены командой).
- [x] **N13** — суточному потолку писем добавлена подчистка строк.

### Девятый проход — машинные слова на экранах (вопросы аудита 4-5)

Аудитор вынес их как вопросы владельцу, владелец 13.09 ответил: «правь то что нашёл аудитор» —
в части текстов. Все ниже подпадают под его собственный критерий «человек не должен читать
машинное слово».

- [x] Диалог объединения учётных записей: «Дубликат станет алиасом (**merged_into_id**), его
      **strong id** будут очищены», «первые 4 **hex-символа UUID**» → человеческие формулировки
      про карточки и «знаки кода». Сами коды на экране ОСТАВЛЕНЫ: там они и есть содержимое —
      врач выбирает, какие две карточки сливает, и вводит начало кода как подтверждение.
      Подписи рядом с ними перестали быть жаргоном: «Целевой (канон)» → «Основная карточка»,
      «станет алиасом» → «станет ссылкой на основную».
- [x] `AuthProvidersSection` — «ID приложения (client_id / APP_ID)» → «ID приложения».
- [x] `AdminAuditLogSection` — подсказка поля «UUID или произвольный id» → «код объекта или любой
      его идентификатор».
- [x] `VideoSystemSettingsSection` — «Burn-in **UUID** в кадре; дольше **ffmpeg**. Уже готовые
      **HLS** не меняются» → «В кадр впечатывается код записи. Обработка идёт дольше; уже готовые
      видео не меняются».
- [x] «Slug» в подписях полей → «Адрес» (`SectionForm`, `PatientHomeCreateSectionInlineDialog`,
      `SectionSlugRenameDialog`) — прямое указание владельца про слаги.
- [x] «отдельному клиническому и тарифному **слайсу**» → «подключаются отдельно и зависят от
      тарифа». «Слайс» — внутреннее процессное слово, на экране ему делать нечего.

Регистрация специалиста из находок аудита НЕ бралась: владелец 13.09 — «кроме регистрации
специалиста, это не твоя работа». Блокирующие B1-B3 по ней закрыты РАНЬШЕ этого указания и лежат
в `80b6a526b`; дальше эта зона не трогалась.

### Тесты на формулировки удалены (владелец, 13.09)

Дословно: «НИКАКИХ ТЕСТОВ НА ТЕКСТ!!!!!!» и «тесты формулировок удалил правильно».

Раньше в этой работе я подгонял в трёх чужих тестах ожидаемые строки под новые тексты — 5 строк.
Это и была та самая привязка теста к формулировке, только чужая. Теперь вместо подгонки:

- [x] `shared/ui/auth/staffSecurityErrorText.unit.test.ts` удалён целиком — все его утверждения
      были про формулировки.
- [x] `api/admin/booking-engine/form-fields/route.route.test.ts` — проверка фразы заменена на
      проверку контракта: код ответа 400 и машинный код `invalid_body`.
- [x] `modules/auth/oauthAppleToggle.route.test.ts` — то же: `toEqual` с фразой заменён на
      `toMatchObject` без неё.

Замер по репозиторию: привязок к русскому тексту в тестах остаётся **76 в 32 файлах**
(`grep -rhoE "(toBe|toContain|toMatch)\(\s*'[^']*[А-Яа-я]{4}[^']*'" --include=*.test.ts*`).
Сам их не трогал — это отдельная работа, жду слова владельца.
...
### Дополнение 14.09 — дыра в стороже и правило в каноне

Владелец спросил, показывается ли человеку понятный текст и берётся ли он из общего файла, а не
размазан по коду. Проверка нашла две вещи.

- [x] **Сторож пропускал тернарник в ответе маршрута.** Для всплывающих сообщений он давно обходил
      все ветки `??` и тернарника, а для поля `message:` в ответе маршрута проверял только литерал,
      стоящий значением напрямую. Маршрут, выбиравший фразу тернарником, проходил чисто. Живых
      случаев было **три** (смена пароля, настройки админа, проверка доставки в клинике), и в смене
      пароля показывалась вторая формулировка того, что в словаре уже есть.
      Сделано: ветки обходятся теми же правилами, три маршрута читают из словаря, в самопроверку
      сторожа добавлены случаи тернарника и `??`. Гейт, самопроверка, typecheck и точечный ESLint —
      PASS. Коммит `b8257f46a`.
- [x] **Правила не было в каноне.** `AGENTS.md` не упоминал ни словарь, ни карту кодов — правило
      жило в шапке самого словаря и в стороже, то есть его видел только тот, кто уже туда попал.
      Добавлен раздел **§21a** и строка в маску путей.

Замер, оставшийся открытым: `message:` с тернарником или `??`, содержащим русский литерал, вне
ответов маршрутов — **21 место** (в основном компоненты: `AuthFlowV2.tsx`, `EmailAccountPanel.tsx`,
`PhoneMessengerAuthFlow.tsx`). Правило G4 покрывает только ответы маршрутов, и я его намеренно не
расширял на произвольные свойства `message` — там есть законные динамические значения. Это вопрос
владельцу, а не самостоятельная работа.
