...
- [ ] Живая проверка на TEST владельцем: у `TEST VERT YOUTUBE` и `TEST VK HORIZ` появилось превью.
      **Для VK не наступит, пока в `system_settings` TEST не появится `vk_video_service_token`** — до этого
      VK-строка честно доходит до `failed` с причиной `vk_video_service_token_missing`, а не висит в
      «готовится».
