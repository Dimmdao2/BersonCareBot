...
**Как берётся картинка.**
- **YouTube** — публичный oEmbed (`youtube.com/oembed?url=…&format=json`) отдаёт `thumbnail_url`; ключей и
  регистрации не нужно. Обложка доступна и по идентификатору ролика.
- **VK** — обложка отдаётся их API по идентификатору видео и, в отличие от YouTube, скорее всего потребует
  сервисный токен. ⚠️ Это НЕ проверено живьём; первый шаг работы — разведка их API, а не обещание срока.
...
- [ ] Разведка VK: как именно отдаётся обложка, нужен ли токен, что делать при приватном/удалённом ролике.
      Результат — замер с командой и ответом, а не предположение.
- [ ] Одна дверь «получить превью» на оба кабинета, знающая три источника: наш файл, внешний хост, отсутствие.
- [ ] Картинка сохраняется в наше хранилище; запросов браузера пациента к чужому хосту нет (проверить сетевой
      панелью на живом стенде, а не чтением кода).
- [ ] Плейсхолдер и состояния совпадают с остальным медиа; «Превью не создаётся» остаётся только там, где
