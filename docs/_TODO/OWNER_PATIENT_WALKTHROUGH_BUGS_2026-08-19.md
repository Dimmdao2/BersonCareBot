...
### Чек-лист (ведущий сверяет сам; DONE исполнителя ≠ DONE плана)

- [x] Установить причину замером на живой базе, а не по чтению кода. Диагноз подтвердился по симптому и по
      тексту отказа, механизм уточнён: роль у диспетчера была ВЕРНАЯ (`app_operational_media_worker`), не
      работала ПОЛИТИКА — единственная разрешающая политика этой роли на `media_transcode_jobs` собрана из
      арендаторских веток, обе зовут `app.current_org_id()`, а та роль воркера не принимает и поднимает
      42501. Отказ приходит только когда в очереди есть строка, поэтому пустая очередь молчала.
      A/B на `bcb_webapp_dev` тем же боевым маршрутом:
      [`docs/REPORTS/MEDIA_WORKER_QUEUE_ROOT_2026-08-19.md`](../REPORTS/MEDIA_WORKER_QUEUE_ROOT_2026-08-19.md).
- [x] Закрыть той же формой, что у соседей: фильтр стены на ПОСТАНОВКЕ (он уже стоял), разбор очереди —
      за объявленным корнем. Миграция `0050`, три корня от `app_seam_patient_lfk_media_owner`, коммит
      `4fe5d179f`.
- [x] Прав рабочим ролям не добавлять. Не добавлено ни одной табличной привилегии; мёртвые прямые гранты
      `app_operational_media_worker` на `media_transcode_jobs` СНЯТЫ, `generate-cli --check` побайтно чист.
- [x] Тест на ПОВЕДЕНИЕ, не на текст кода. `apps/webapp/src/infra/repos/pgMediaWorkerControl.unit.test.ts`,
      8 проверок; три названные поломки внесены руками — тест краснел на каждой (таблица в отчёте).
- [x] Живой прогон на DEV: работа поставлена боевым маршрутом, взята, доведена до конца — файл
      `ready/pending` стал `ready/ready` с плейлистом, постером и длительностью, работа — `done`.
      Проверены и `retry`, и `failed`, и отказ чужому замку.
- [x] Стена арендаторов на этом пути: работа чужой организации воркеру НЕ выдана и отбита в `failed`
      с `organization_invariant_violation`.
- [x] Второй симптом проверен, ответ раздваивается: файл СО строкой очереди лечится сам (доказано), файл
      БЕЗ строки очереди остаётся с заглушкой навсегда — `reconcile` не видит статус `pending`
      (`candidatesScanned=0`). Это решение владельца, см. «НЕ СДЕЛАНО» п.2 отчёта.
- [ ] **Выкатить на TEST.** Не сделано: миграция применена только к DEV. Пока её нет на TEST, видео там
      не пересобирается — отказ идёт в журнале прямо сейчас.
- [ ] **Живая приёмка владельцем на TEST:** приложить видео к комментарию и увидеть, что оно собралось.
