...
- [ ] Тест на поведение: снятая дверь краснит тест. Инъекции перечислены ниже («Инъекции 28.08»), но
      на момент этого коммита ещё НЕ прогнаны — галочка ставится результатом прогона, не намерением.
...
Список инъекций, которыми проверяется, что тесты держат дверь, а не форму кода. Каждая — снятие
одной двери с последующим откатом правки:

1. Убрать hosted-ветку из `catalogMediaLadderLookup` (ссылка перестаёт резолвиться) →
   `catalogMediaLadderLookup.unit.test.ts` краснит на «отдаёт НАШ адрес обложки» и «без строки обложки
   говорит „превью не создаётся"».
2. Убрать вызов `enqueueHostedVideoCover` из записи упражнения → `pgLfkExercisesHostedCover.unit.test.ts`
   краснит: заказа обложки не появляется.
3. Убрать hosted-ветку из воркера → `mediaPreviewWorker.unit.test.ts` краснит: обложка не качается и
   `terminal` не переводит строку в `skipped`.
4. Вернуть внешний URL в `previewSmUrl` мапперов → `hostedVideoSlot.ui.test.tsx` краснит: в слоте
   появляется `<img>` с чужим хостом.
