...
**Текущее состояние 01.09.2026:** исходные четыре дефекта и поздние системные хвосты закрыты кодом и живой
проверкой. Единственный внешний остаток этого файла — живая проверка VK-превью после предоставления владельцем
`vk_video_service_token`; он ведётся taskdb-карточкой `#1092`. Исторические замеры ниже не переоткрывают работу.
...
- [x] **Выкатить на TEST.** Код `1fc682380` развёрнут штатным `deploy-test.sh`: transcript
      `/var/log/bersoncarebot/deploy-test/deploy-test.20260828T061212Z.TqQA1G.log`, итог
      `PASS branch=feat/doctor-ui-rebuild head=1fc682380a2`.
- [x] **Живая приёмка на TEST штатными owner-учётками:** пациент `kinesiospace@gmail.com` приложил
      односекундное MP4 к реальному пункту программы; обработка и attach завершились за `3670 ms`, playback
      вернул `200`, `delivery=mp4` и наш poster, врач `dimmdao@yandex.ru` увидел то же media-сообщение.
      Сообщение и файл удалены штатными API; в журналах после прохода нет `500`, `42501` или permission denied.
      Artifact: `runs/test-interactive-acceptance/out/video-discussion-live-2026-08-28T06-24-25.728Z.json`
      (runtime-only, не коммитится).

      На первом проходе 28.08 найден отдельный достижимый дефект коротких роликов: FFmpeg мог завершиться с
      кодом `0`, не создав `poster.jpg` на отметке `1s`, и attach ждал до таймаута. Общий helper теперь
      проверяет сам файл и повторяет извлечение с `0s` для обоих transcode-путей; первая failed job делает
      system-health `degraded`, а не `ok`. Исправление `1fc682380`, независимый аудит — `PASS`, `0 MUST FIX`.
...
- [x] «время по стенным часам» (напоминание в 9:00) при смене пояса устройства НЕ переносить молча —
      спросить один раз (§34 п.3).
      → **ЗАКРЫТО решением владельца 20.08: «не спрашивать - согласен».** Замер, на котором стоит решение:
      молча ничего и не переносится — `reminder_rules.timezone` хранит пояс правила, планировщик считает по
      нему, `UPDATE reminder_rules` пояс не трогает. Значит стенные часы переезд человека не двигают и
      спрашивать не о чем. Вместо вопроса — сверка при заходе: сохранённый пояс догоняет устройство, при
      совпадении записи в базу нет вовсе (находка аудита закрыта, `3474b3a84`). Канон §34 дополнен.
...
## Замер TEST ДО выкатки починки диспетчера (20.08, 00:35) — точка отсчёта для приёмки

Снято до приземления `wt/media-worker-root-20260819`, чтобы после выкатки сравнивать не на словах.

| что | значение |
|---|---|
| `media_transcode_jobs`, `pending` | **2** (обе висят: диспетчер не может их взять) |
| `media_transcode_jobs`, `done` | 149 — все ДО ввода порт-контекста |
| `media_files` `ready/ready` | 149 |
| `media_files` `ready/pending` | **2** — те самые «вечная заглушка, а по клику играет» |
| `media_files` `pending` без `video_processing_status` | **2** — пара `video/quicktime`, диспетчер их не подберёт и после починки (находка, не работа) |
| `user_notification_topic_channels` | 315 строк — ошибка 3 закрыта |
| `GET /` · `GET /book` | 200 за 0.033s · 200 за 0.016s |

**Признак приёмки после выкатки:** обе `pending`-работы уходят в `done`, оба файла `ready/pending`
становятся `ready/ready`, и в журнале PostgreSQL нет ни одного `42501` по `media_transcode_jobs`.

## Пятая ошибка живого прохода: программа не сохраняется при добавлении упражнения (20.08, 00:16)

Владелец на TEST: «ошибка сохранения программы при добавлении новых упражнений». В журнале PostgreSQL —
ровно то же место, дважды подряд (00:16:01.130 и 00:16:09.938, `bcb_test_webapp_staff@bersoncarebot_test`):
...
**Замер прав (TEST и DEV одинаково):**

| роль | привилегии на `treatment_program_instance_stage_items` |
|---|---|
| `app_object_owner` | DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE |
| `app_staff` | **DELETE, SELECT** — INSERT нет |
| `app_patient` | SELECT |

То же самое у соседней `treatment_program_instance_stages`. То есть сотруднику объявлено читать и удалять
задания программы, но не создавать их, а код создаёт их прямым `insert` из webapp-пула.

**Это НЕ регрессия сегодняшней выкатки.** Последняя строка в таблице создана **15.08 18:37**, всего строк 644.
Писать перестали не сегодня — сегодня строгий гейт сделал молчаливый дефект громким. Тот же класс, что
диспетчер очереди пересборки: писателя без своей двери держали живым слабые права.

**Решение владельца по порядку приземления миграций (20.08), дословно:** «так откати миграции, переименуй и
...
Владелец на живом TEST, увидев работающее добавление ролика ссылкой: «видео теперь можно добавлять сразу из
ютуба/вк ссылкой - ШИКАРНО!!! осталось сделать чтобы делалось превью, это реально?» → «ставь».

**Что уже есть.** `apps/webapp/src/shared/lib/hostingEmbedUrls.ts` разбирает ссылку и знает провайдера
(`youtube | rutube | vk | vimeo`), приводит её к каноническому виду — идентификатор ролика достаётся уже
сегодня.

**Чего нет.** Конвейер превью написан под НАШИ файлы: кадр берётся из видео, которое сам конвейер и
перекодировал. У `hosted_video` файла нет, конвейер до него не доходит, экран честно пишет «Превью не
создаётся» (замер: два упражнения на TEST, `TEST VERT YOUTUBE` и `TEST VK HORIZ`).

**Как берётся картинка — проверено живыми запросами 27.08, не предположение.**
- **YouTube** — публичный oEmbed (`youtube.com/oembed?url=…&format=json`) отдаёт `thumbnail_url`; ключей и
  регистрации не нужно.
  ```
  $ curl -s "https://www.youtube.com/oembed?url=https://www.youtube.com/watch?v=dQw4w9WgXcQ&format=json"
  {"thumbnail_url":"https://i.ytimg.com/vi/dQw4w9WgXcQ/hqdefault.jpg","thumbnail_width":480, ...}
  ```
  На несуществующий/приватный/удалённый ролик YouTube отвечает **`HTTP 400`** (проверено на заведомо
  несуществующем id), а не 5xx/429 — это постоянная ошибка, ретраить нечего.
- **VK** — сервисный токен ОБЯЗАТЕЛЕН, подтверждено запросом:
  ```
  $ curl -s "https://api.vk.com/method/video.get?owner_id=-22822305&videos=-22822305_456239017&v=5.199"
  {"error":{"error_code":5,"error_msg":"User authorization failed","descr":"token required", ...}}
  ```
  Обход без API (прочитать `og:image` со страницы ролика) тоже не работает: `vk.com`/`vkvideo.ru` без
  авторизованной сессии редиректят на `login.vk.ru`. Нужен НОВЫЙ сервисный токен VK API с правом `video` —
  это **не** `vk_community_access_token` (Callback API бота сообщества для сообщений, к `video.get`
  отношения не имеет). Получение токена — действие владельца (регистрация VK-приложения); класть его
  некуда, кроме `system_settings` (scope `admin`, AGENTS.md §2), и место под него уже заведено —
  ключ `vk_video_service_token`.

**Обязательное требование: картинку скачиваем один раз и кладём в НАШЕ хранилище.** Показывать ссылкой на
чужой хост нельзя — иначе браузер пациента ходит на YouTube и VK при каждом открытии программы, и они видят,
кто и когда смотрит упражнение. Дальше превью живёт по общим правилам: те же размеры, тот же путь, тот же
плейсхолдер «готовится».

**Делается вместе с Р2** (`DEEP_CODE_AUDIT_PLAN.md`, реестр раздвоенных дверей): одна дверь «получить превью»,
знающая конвертацию, миниатюру и внешний хост. Появился третий вид медиа — если вести его отдельной веткой в
двух копиях экрана (доктор и пациент), разъедется втройне.

### Чек-лист (ведущий сверяет сам; DONE исполнителя ≠ DONE плана)

- [x] Разведка VK: как именно отдаётся обложка, нужен ли токен, что делать при приватном/удалённом ролике.
      Результат — замер с командой и ответом, а не предположение. Обе команды и ответы — в «Как берётся
      картинка» выше.
- [x] Одна дверь «получить превью» на оба кабинета, знающая три источника: наш файл, внешний хост, отсутствие.
      `catalogMediaLadderLookup` теперь принимает URL медиа, а не разобранный id, и сам решает, что это:
      `/api/media/{uuid}` — своя строка, каноническая ссылка хостинга — служебная строка обложки, ничего —
      `skipped`. Разбор id, который каждый вызывающий делал сам, удалён из `pgRecommendations`,
      `pgTreatmentProgram`, `pgTreatmentProgramItemSnapshot`; `pgLfkExercises` лишился двух собственных
      `LEFT JOIN media_files` и ходит той же дверью.
- [x] Картинка сохраняется в наше хранилище. Служебная строка `media_files`
      (`usage_purpose = 'hosted_video_preview'`, новая колонка `hosted_video_source_url`) заводится
      транзакционно при сохранении упражнения; воркер превью (тот же самый, не новый крон) качает обложку,
      перекодирует существующим `imageStandardRendition` и раскладывает по обычным `preview_sm/md_key`.
      Наружу отдаётся только `/api/media/{id}/preview/{size}`.
      Сетевую панель на живом стенде — см. последний пункт, он не закрыт.
- [x] Плейсхолдер и состояния совпадают с остальным медиа; «Превью не создаётся» остаётся только там, где
      обложки действительно нет (приватный ролик, неизвестный провайдер). Ссылка идёт по той же лестнице
      `MediaThumb`/`getMediaThumbPhase`, что файл: `pending` — обложка качается, `ready` — наша картинка,
      `failed` — попытки исчерпаны, `skipped` — обложки не будет.
- [x] Тест на поведение: снятая дверь краснит тест; прогнано инъекциями, см. «Инъекции 28.08» ниже.
- [x] Живая проверка YouTube на TEST под штатными owner-ролями врача и пациента: врач создал упражнение со
      ссылкой, минутный worker сохранил обложку в наше S3, точный `/api/media/{id}/preview/sm` вернул `200`,
      врач увидел её в архивном списке, пациент — после временного назначения в действующей программе.
      В обоих браузерах внешних `youtube`/`ytimg`/`vk` картинок нет; повторный пациентский проход завершился
      без ошибок консоли и сети. Временная ступень удалена, упражнение снова архивировано. TEST развёрнут на
      `206be5478`.
- [ ] Живая проверка VK на TEST: нужен `vk_video_service_token`. Пока токена нет, строка честно исчерпывает
      retry и переходит в `failed` с причиной `vk_video_service_token_missing`, а не висит в «готовится».
      Получение сервисного токена VK остаётся внешним owner-шагом; код и хранение секрета готовы.

### Что построено 28.08 (ветка `wt/systemic-hosted-preview-impl-20260827`)

**Хранилище — не новая таблица, а тот же lifecycle.** Скачанная обложка — это наш файл, и она обязана
проходить те же двери превью/статуса/очистки/хранилища, что остальные наши файлы. Поэтому вместо отдельной
`hosted_video_previews` расширен `media_files`:

- `usage_purpose` получил второе значение `hosted_video_preview` (CHECK расширен, не заменён);
- добавлена nullable `hosted_video_source_url` — канонический URL ролика (`hostingEmbedUrls.ts#canonicalUrl`,
  тот же текст, что в `lfk_exercise_media.media_url`), а не generic JSON;
- CHECK `media_files_hosted_video_preview_check` связывает их намертво: ссылка есть ровно на строках-обложках
  и такая строка всегда принадлежит клинике. Иначе обычная загрузка могла бы получить source URL и попасть в
  hosted-ветку воркера;
- частичный `UNIQUE INDEX uq_media_files_hosted_video_preview_source (organization_id,
  hosted_video_source_url) WHERE usage_purpose = 'hosted_video_preview'` — он же ключ дедупа для `ON
  CONFLICT` при сохранении и он же индекс поиска для двери чтения.

Миграция `20260827T205400_hosted_video_covers_live_in_our_storage.sql`: четыре statement'а от
`app_object_owner`, ни одного `GRANT`/`POLICY`; права новой колонки объявлены только в
`deploy/postgres/privileges/declaration.ts` (`app_patient` получил `SELECT` — это публичный адрес ролика,
который пациент и так видит) и приехали генератором.

**Один поток, без второго крона.** Сохранение упражнения со ссылкой в той же транзакции делает `INSERT …
ON CONFLICT DO UPDATE` служебной строки (в сеть внутри транзакции никто не ходит). Строка рождается
`preview_status = 'pending'` без `s3_key`; очередь воркера превью расширена так, чтобы такие строки видеть.
Повторное сохранение той же ссылки переиспользует строку, а сдавшуюся (`failed`) возвращает в очередь с
обнулённым счётчиком. Удаление или замена медиа упражнения общую обложку НЕ удаляет — её может использовать
другое упражнение клиники; осиротевшую убирает обычный retention/purge.

**Кто ходит в сеть.** Ровно один серверный модуль `apps/webapp/src/shared/lib/hostedVideoThumbnail.ts`,
рядом с разбором ссылки и не в UI. YouTube — публичный oEmbed; VK — `video.get` с `vk_video_service_token`
из `system_settings`. Адрес картинки принимается только с allowlist-хостов провайдера, проверяется
фактический origin ПОСЛЕ редиректов, MIME и потолок размера (8 МБ, тело читается кусками и обрывается).
Произвольный URL из чужого JSON не запрашивается никогда, и наружу уходят байты, а не адрес.

**Разряды ошибок.** `private/deleted/unsupported provider` → terminal → `skipped` («превью не создаётся»).
`timeout/429/5xx/сеть/нет токена VK` → retryable через уже существующий bounded-счётчик попыток
(`MAX_PREVIEW_ATTEMPTS = 5`) → после лимита `failed`. Вечного `pending` не остаётся ни в одном случае.

**Служебные строки в медиатеке врача не появляются:** список библиотеки фильтруется одним общим списком
`MEDIA_LIBRARY_HIDDEN_USAGE_PURPOSES` (`mediaSqlPredicates.ts`), а не вторым перечислением по месту.

### Инъекции 28.08 — чем именно краснеют тесты

Прогнано 28.08 на коммите `90a83bd84`; каждая правка после прогона откачена (`git checkout --`),
дерево после всех четырёх — чистое. База до инъекций: 5 файлов, 60 тестов, все зелёные.

1. Убрать hosted-ветку из `catalogMediaLadderLookup` (ссылка перестаёт резолвиться) →
   `catalogMediaLadderLookup.unit.test.ts` **5 failed / 6 passed**: краснеет «отдаёт НАШ адрес обложки»,
   «без строки обложки говорит „превью не создаётся"» и соседние hosted-случаи.
2. Убрать вызов `enqueueHostedVideoCover` из записи упражнения (обе точки, create и update) →
   `pgLfkExercisesHostedCover.unit.test.ts` **3 failed / 3 passed**: заказа обложки не появляется.
3. Убрать hosted-ветку из воркера превью → `mediaPreviewWorker.unit.test.ts` **3 failed / 5 passed**:
   обложка не качается и `terminal` не переводит строку в `skipped`.
4. Вернуть внешний URL провайдера в `previewSmUrl` обоих мапперов → `hostedVideoSlot.ui.test.tsx`
   **2 failed / 10 passed**: в слоте появляется `<img>` с чужим хостом.

Что это доказывает: тесты держат именно дверь (обложка заказывается, качается, отдаётся нашим
адресом), а не текст кода — снятие любой из четырёх дверей ловится, причём тем тестом, который эту
дверь описывает.
...
- [x] Строка, которую отправитель не может взять из-за отсутствия конфигурации канала, обязана
      засчитывать попытку и умирать по `max_attempts`, а не переноситься вечно с нулём попыток.
      Закрыто системным Track D: claim увеличивает `attempt_count`, лимит переводит строку в `dead`;
      контракт закреплён `apps/integrator/src/infra/runtime/worker/outgoingDeliveryWorker.finalize.test.ts`.
- [x] Отсутствие конфигурации канала на стенде — это отдельный сигнал оператору («канал не настроен»),
      а не молчаливый вечный перенос.
      Закрыто системным Track D: отказ провайдера материализуется через
      `apps/integrator/src/infra/operatorIncident/reportOperatorFailure.ts` и не маскируется пустой аудиторией.
- [x] Разобрать 99 отложенных строк на TEST: часть от 25.06 — мусор прошлых прогонов, ему место в `dead`.
      Исторические строки очищены в ходе Track D retention/reconcile; текущая очередь и её возраст проверены
      живьём на TEST, текущий источник — `docs/_TODO/DB_PRIVILEGE_LAYER_REBUILD/PLAN.md`.
...
- [x] Позднее owner-решение заменило предложенный здесь источник: среда определяется явным `TEST` в env,
      а не `APP_BASE_URL` или `NODE_ENV`; текущий контракт — `apps/integrator/src/config/env.ts`.
      Историческая формулировка ниже не исполняется:
...
- [x] Инверсия защиты реализована поздним owner-решением без перенаправления: DEV подавляет реальную доставку,
      TEST пропускает только адресатов из `TEST_ACCOUNT_*`, PROD не фильтруется; одна дверь —
      `apps/integrator/src/shared/testDeliverySafety.ts`.
- [x] Fail-closed проверка конфигурации TEST-адресатов выполняется общей дверью доставки и покрыта
      `apps/integrator/src/shared/testDeliverySafety.test.ts`; отдельный `DEV_DELIVERY_REDIRECT` не является
      источником безопасности.
- [x] Остальные потребители разделяют режим сборки и среду доставки по актуальному env-контракту; старый
      список «~10 мест» был исследовательским замером до Track D и не переоткрывает отдельный sweep.
