...
Черновик Ч1.2-r4 — ревизия после адверсарного критика №4 + решения владельца 08.08. Реализует
...
PG16 на `:5432` (SERVER CONVENTIONS.md:124; `migrate-all.sh:85,:92` пиновит оба таргета к
...
1. **roles** — все канонические роли; kind — закрытая грамматика `terminal | capability | owner |
   ...
   декларация закрывает одной строкой. Оба — оставить-и-объявить: решения §I Р5 и Р9.
...
5. **migrate** — `pnpm migrate` под временной скобкой элевации (`ALTER ROLE $DBROLE BYPASSRLS`
   ...
   **Маркер-скобка живёт в `scripts/migrate-all.sh`** — дефолтной точке migrate (корневой
   ...
   ведут ПЯТЬ поименованных обходов** (перепись по репо): корневые `migrate:webapp`,
   `db:migrate`, `db:migrate:prod` (`package.json:82-84` — прямые per-app migrate),
   `migrate:legacy` (`apps/webapp/package.json:27`) и `db:migrate:drizzle`
   (`apps/webapp/package.json:62` — сырой `drizzle-kit migrate`, третья реализация мигратора);
   судьбу каждого решает Ф2 — перевод на migrate-all.sh либо удаление скрипта.
   **Честное следствие обхода** (прежнее «бьётся о fail-closed reject» было неверно):
   ...
   до следующего полного прогона цепочки. Чтобы обход не жил незамеченным: шаг 6 пишет в
   `app_control.privileges_watermark` максимум `created_at` журнала drizzle на момент
   генерации; §F красен, если журнал ушёл вперёд водяного знака. Одна строка в УЖЕ существующей
   схеме стены `app_control`; машинерия оправдана — превращает «закрывающие гейты не отбежали»
   из остаточного риска в детектируемый красный; сверх evidence/12, приёмка — Ф3/Ф4. Сегодня
...
6. **генерат** — в слоте нынешней цепочки оверлеев, тем же админ-каналом. Полное переприменение
   ...
   уходят ЗДЕСЬ; тут же пишется watermark (шаг 5). Соответствует предписанию Liquibase «гранты
...
декларации §A.3), `ALTER FUNCTION … OWNER` (владение — статьи генерата),
...
| журнал vs watermark | `max(created_at)` из `drizzle.__drizzle_migrations` против `app_control.privileges_watermark` | §B шаг 5 (обход мимо цепочки); сверх evidence/12, приёмка Ф3/Ф4 |
...
один: `scripts/verify-db-privileges-conformance.mjs`.
...
3. **CI-гейты:** (а) новая миграция содержит статью из красного списка §B (GRANT/REVOKE/политики/
   роли/CREATE SCHEMA/ALTER FUNCTION OWNER/SECURITY DEFINER вне definerExceptions/
   CREATE MATERIALIZED VIEW) → красный; (б) `generated/privileges.<db>.sql` разошёлся с
   декларацией → красный.
...
### Блок 1 — развилки владельца (осталась одна)
...
3. **`platform_users` RLS — сейчас** — инженерное решение 08.08: единственная стена на 278 строк
   ...
   нет; декларация несёт `rls: 'force'` (образец §A), реальность приводит Ф6.
4. **7 ячеек утечки — сузить `app_platform_settings`** — инженерное решение 08.08: 5/7 —
   ...
   (образец c5a:1293-1355), не табличный SELECT.
...
## Changelog Ч1.2-r3 → Ч1.2-r4 (по находкам критика №4 + решения владельца 08.08)

- **BLOCKER-1** → §A, §F, §G №8, §H.1, §I: юрисдикция = ДОСТУП к управляемым базам, не
  существование на кластере; перепись кластера снята заново своими командами (§A: 11 баз, 45
  не-pg ролей); кластерная сверка §F двусторонняя по доступу; свип №8; остаточный риск чужого
  SUPERUSER/BYPASSRLS принят в §I; Ф2 тегирует все 45 ролей; `bcb_webapp_prod` — старая копия
  прода (владелец: «копия явно»), вне контура, чистка не заказана.
- **MAJOR-1** → §B: владение — статьи генерата (`ALTER TABLE/FUNCTION … OWNER TO`, прецедент
  c5a:43) — дрейф владения получил конструируемый красный→зелёный; в красный список добавлены
  `CREATE FUNCTION … SECURITY DEFINER` (вырез — сигнатуры definerExceptions, сканер сверяется с
  декларацией) и `ALTER FUNCTION … OWNER`.
- **MAJOR-2** → §B шаг 5: пять поименованных обходов migrate-all.sh (`package.json:82-84`;
  `apps/webapp/package.json:27,:62` — вкл. третью реализацию `drizzle-kit migrate`); ложное
  «бьётся о fail-closed reject» заменено честным следствием (grantless-таблица, шаги 4/6/7 не
  бегут); добавлен `app_control.privileges_watermark` (шаг 6 пишет, §F сверяет) — машинерия
  оправдана явно.
- **m-1…m-7** → грамматика `rls: 'force'|'on'|'off'` (§A.4, образец); множество схем — обе
  стороны §F + `CREATE SCHEMA` в красный список; рендер свипа в момент исполнения (§G); kind
  `service` — дом `app_migration_phase` (§A п.1); цитата `deploy-test.sh:83-86,:173-176`
  проверена по файлу; `dev-c5:63-64`/`dev-c9:81-82` → roles-install/env-маппинг;
  `DROP/ALTER POLICY` в красный список.
- **Развилки** → §I: Блок 1 — ОДНА развилка (приёмка Ч1.3); Блок 2 — 10 принятых решений
  (Р1-Р2 — владельца 08.08: dev в контуре, свой генератор; Р3-Р10 — инженерные); свип №9
  (FK между org-таблицами, каскады в обход RLS — из Q4 критика, прогон в Ф3); Q3 (PUBLIC
  CONNECT в чужие базы) — одна строка во «Вне рамок».
