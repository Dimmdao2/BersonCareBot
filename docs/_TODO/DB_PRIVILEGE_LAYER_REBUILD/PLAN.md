...
история замен остаётся в Git, но не действует одновременно с новым планом.
...
- [x] **Приглашение пациента в портал на DEV исправлено 31.08.** Предыдущий живой проход не покрывал это
  ...
  колонки Drizzle-схемы, а контракт-тест падает при появлении новой неклассифицированной колонки. Канонический
  DEV reconcile завершён; каталог показал `28/28`, и тот же живой doctor POST вернул `200` вместо `42501/500`.
...
- [ ] **Анонимная публичная запись — owner gate #805.** Живьём пройти полный public-booking путь без кабинетной сессии и доказать,
  что узкие roots дают только публичный каталог и создание записи, не tenant-данные.
  Каталог и страницы анонимной поверхности зелёные, но завершение пути требует одноразовый код на реальном
  TEST-контакте владельца по обязательному правилу «код или вход»; читать код из БД вместо доказательства нельзя.
- [ ] **Реальная доставка — owner gate #1085.** Existing owner должен пройти messenger contact proof и код
  входа; запись на приём — реально доставить подтверждение и напоминание. Operator digest закрыт живьём
  29.08: тик материализовал e-mail и первая попытка доставки завершилась успешно. Настройка времени сводки
  также закрыта живьём: global admin изменил `digestTime`, перезагрузка подтвердила новое значение, затем
  исходное значение было восстановлено без ошибок запросов. Отдельного «переноса cron» здесь нет: scheduler
  будит digest каждый час, а `digestTime` выбирает час материализации. Открыты только contact proof и
  подтверждение/напоминание записи; все три требуют реальный результат owner-канала и не являются автономными
  TEST-шагами.
- [x] **Повторы в operator digest свёрнуты по виду сигнала.** Письмо TEST от 29.08 показало одну и ту же
  строку за каждый цикл инцидента. Теперь в сводке одна строка на вид и счётчик повторов; поведение защищает
  `apps/webapp/src/modules/operator-health/buildOperatorHealthDigest.unit.test.ts`, fault injection счётчика покрасил тест.
- [x] **Возраст просроченной очереди начинается с момента due-time.** Письмо TEST в 14:00 с «13 ч» оказалось
  ложной тревогой: запланированная в полночь строка впервые стала готова к отправке только в 14:00, но прежний
  агрегат считал возраст от создания намерения. Миграция `20260829T125604…` сохраняет совместимый wire-key и
  возвращает в нём самый ранний `next_retry_at` среди реально готовых строк; parser теперь считает именно
...
`20260829T031736_resolve_provider_incidents_after_delivery.sql` создаёт одну `SECURITY DEFINER`-функцию и не
меняет таблицы, колонки, роли, grants или policies. Тело исполняется владельцем
`app_seam_telemetry_operator_owner`, вызывается только узкой delivery-worker ролью через integrator port-context
с purpose `health.outbound-provider.recover` и принимает один фиксированный канал доставки. Для исполнения нужны
`SELECT`/`UPDATE` только колонок идентификатора, направления, канала, времени закрытия и полей alert-claim в
операторском журнале; ровно эта поверхность внесена в единую декларацию. Ничего отсутствующего в декларации после
разбора не осталось. `migrate-dev.sh --preflight` скомпилировал две pending-миграции под настоящими владельцами и
откатил транзакцию; последующий `migrate-dev.sh --execute` применил их и завершил declaration reconcile/catalog
audit с `PASS`.
...
- [ ] **PROD — только отдельным разрешённым этапом #996.** Подготовить одну атомарную `A → B0` миграцию и rollback;
...
- [x] Разовая инвентаризация и очистка разросшихся DB-журналов выполнена в `bca1d376a`; подробный run-evidence
  удалён из рабочего дерева и остаётся в истории Git.
...
- [x] Исторический `AUDIT_LOG.md` свёрнут после завершения технической части инициативы и удалён из рабочего
  дерева; текущие остатки ведутся только верхним чек-листом этого плана и taskdb, а старый журнал доступен в
  истории Git.
...
- [x] **A-1 (ориг. C1) Отдельные владельцы definer-функций вместо общего bypass, плюс исполняемая декларация.**
  Закрыто текущей архитектурой: функции распределены по узким seam-owner ролям, требования строятся из одной
  декларации, а каталожный census на именованной TEST не нашёл отсутствующих или частичных требований.
- [x] **A-2 (ориг. C2) Отдельная поверхность публичного чтения.** Публичная запись использует только узкие
  public-booking doors и проекции; неиспользуемая diagnostic/config-reader роль удалена из активной схемы.
  Полный живой anonymous journey остаётся отдельным runtime-gate выше и не меняет состояние архитектурного
  пункта.
- **A-4 (ориг. C3) Физическая перестройка `platform_users` вынесена из Track D.** Текущий seam уже разделяет
  actor/identity и medical-subject ссылки; большой identity/contact redesign остаётся отдельной
  post-production задачей `#1086`, как зафиксировано в актуальном состоянии плана выше.
...
- **Историческая ссылка:** публичная запись без входа перенесена в единственный текущий owner gate #805.
  Владелец 17.08: «надо пройти». Открыть ссылку на TEST как посторонний без учётной записи, создать
...
### Проход публичной записи анонимом на DEV, 12.09.2026 — и почему владельческий гейт #805 сегодня не проходится

Проход сделан на DEV (`127.0.0.1:5200`, HEAD ветки `feat/doctor-ui-rebuild`), потому что TEST держит
сборку `2ca8f89642da` от 11.09 13:29 и публичная страница клиники там отдаёт 404 — на 292 коммита
старше. Перевыкатка TEST — общее действие четырёх сессий, само собой не запускалось.

**Пройдено анонимом, без единой куки:**

| Шаг | Адрес | Итог |
| --- | --- | --- |
| карточка клиники | `/tochka-zdorovya` | 200 |
| вход в запись | `/tochka-zdorovya/booking` | 200, филиал ведёт на `/book/service?...` |
| старый адрес | `/book/tochka-zdorovya` | 308 → `/tochka-zdorovya/booking` |
| выбор услуги | `/book/service?cityCode=moscow&orgSlug=…&branchId=…` | 200, ровно две АКТИВНЫЕ услуги; выключенная `be74d30a…` не показана |
| выбор времени | `/book/slot?type=in_person&…` | 200 |
| слоты | `GET /api/booking/public/slots` | 200, чистые `startAt/endAt`, ни одного идентификатора специалиста |
| подтверждение | `/book/confirm?…&date=…&slot=…` | 200 |

**Стена арендатора держит и стоит ПЕРВОЙ.** Четыре разных причины отказа на двери слотов дают
побайтно одинаковый ответ `503 {"ok":false,"error":"slots_unavailable"}` (отличается только
`correlationId`): услуга выключена, услуги не существует, филиала не существует, слаг чужой.
На двери создания записи те же три подмены дают одинаковый `400 ambiguous_booking_tenant`, причём
ДО любой работы с каналом доставки — перебрать клиники или услуги по форме ответа нечем.

**Чужих данных в анонимной выдаче нет.** Все UUID на трёх экранах — только `branchId` и `serviceId`
той же клиники, которые аноним и выбирал; идентификаторов людей, записей и организации в разметке
нет.

- [ ] **Владельческий гейт #805 «создать записи» СЕГОДНЯ НЕ ПРОХОДИТСЯ, и не из-за сборки TEST.**
      После решения A-3 («всегда просить код или вход») единственный канал доказательства владения
      контактом для АНОНИМА — SMS: `publicBookingVerification.ts` жёстко шлёт `deliveryChannel: 'sms'`,
      а ветка `proofMethod: 'email'` в `api/booking/public/create` отвечает `403 identity_not_verified`
      всем, кто не вошёл. SMS выключен и на DEV, и на TEST (`auth_sms_enabled = false`,
      `auth_surface_patient_sms_enabled = false` в обеих базах) — дверь честно отдаёт
      `403 auth_channel_disabled` на полностью верном запросе. То есть посторонний человек не может
      создать запись ни на DEV, ни на TEST, сколько бы раз TEST ни перевыкатывали.

**Развилка владельцу (деньги или продукт, поэтому не решается сама):** либо включить SMS-провайдера
на TEST, либо разрешить анониму подтверждать контакт КОДОМ НА E-MAIL — второе укладывается в его же
формулировку «всегда просить код или вход» и не требует провайдера, но это изменение продукта, а не
настройки. До ответа гейт #805 остаётся открытым по причине, не зависящей от кода.
