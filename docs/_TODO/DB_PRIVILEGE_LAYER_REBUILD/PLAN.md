...
- **Четвёртый поток остановлен на FAIL и был потерян.** Коррекция B0 — 14 коммитов, 258 файлов,
  ...
  Закрывается в `wt/b0-findings-20260817`; до зелёного повторного аудита ветка не вливается.
- **DEV-стек стоял.** С 12:42 (SIGTERM) до вечера 17.08 не работало ничего: ни webapp на `5200`, ни
  integrator, ни планировщик, ни воркеры. Поэтому единственная известная живая ошибка — 265 отказов `500` и
  139 отказов `400` на `POST /api/integrator/patient-reminders/materialize-wake` — относится к состоянию ДО
  фиксов `ff2c19445` и `8da88d798`, и по логу проверить, ушла ли она, невозможно. Стек поднят заново,
  `/api/health` отвечает `200`; повторная живая проверка этого маршрута — обязательный шаг перед любым
  утверждением о зелёном DEV.
- **Разовый ремонт двух осиротевших правил напоминаний не сделан.** Скрипт
  `reconcile-dev-patient-reminder-orphans.ts` асимметричен: `--execute` идёт через транзакцию порта, а
  сухой прогон читает мимо неё и потому отвергается стенами — предпросмотр невозможен, а исполнение прошло бы.
  Правится в `wt/reconcile-orphan-dryrun-20260817`. Сам ремонт запускается только после зелёной правки.
