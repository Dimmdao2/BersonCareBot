...
25. **Пациентское видео — короткая инструкция, а не запись приёма.** Владелец 17.08: «никто не хранит запись
    часового приёма, хранить только инструкцию для пациента как делать упражнение». Видео с приёма и
    видео-упражнение — одно и то же.
    **УСТАРЕЛО/ЗАМЕНЕНО 2026-08-19:** лимит «5–7 минут» снят.
    **Владелец 2026-08-19:** файл упражнения — не длиннее **10 минут**; длиннее — только ссылка YouTube /
    RuTube / VK Video / Vimeo в iframe. Файл CMS — не длиннее **20 минут**. Лимит по длительности и по
    размеру проверяется на загрузке, а не только описан. Канон исполнения: `GLOBAL_ADMIN_UI_INITIATIVE`
    STAGE_01 §«Лимиты видео». Уточняет объём `IS-I2-03` и снимает худший случай из расчёта дискового scratch.
    Реализовано 2026-08-20: общий media-service gate закрывает обычный, bulk, personal editor-batch
    и CMS `video_url`/Markdown/legacy HTML; evidence — `GLOBAL_ADMIN_UI_INITIATIVE/LOG.md`
    §«Follow-up fix GA-L-01 / GA-L-02».
27. **Продукт — не медицинское ПО, а заметки врачей и тренеров** (владелец 17.08, дословно: «У НАС НЕ МЕДИЦИНСКОЕ
    ПО А ЗАМЕТКИ ДЛЯ ВРАЧЕЙ И ТРЕНЕРОВ — ПРОСТО УДОБНЫЙ БЛОКНОТ»). Следствия, которые обязаны быть разведены:
    - обязательного срока хранения медицинской документации у нас **нет**; сколько живёт заметка, видео или файл —
      продуктовое решение владельца, а не норма. Все прежние рассуждения про сроки Минздрава сняты;
    - защита данных **не смягчается**: сведения о состоянии здоровья остаются специальной категорией ПДн по ст. 10
      152-ФЗ независимо от того, медицинская это организация или блокнот. УЗ-3, шифрование, RLS и разделение
      хранилищ остаются в силе без изменений.
26. **Пациентское видео по сроку не удаляется, а помечается архивным.** Физический перенос в холодный класс
    хранения отложен: при нынешних объёмах он экономит десятки рублей в месяц и добавляет движущуюся часть.
    Условие включения — `DEFERRED_INFRA_TRIGGERS.md` D-6. Сейчас достаточно, чтобы модель данных различала
    «активное» и «архивное» состояние — тогда позднее перемещение станет заменой пути хранения, а не переделкой.
    Уточняет `IS-I3-12`.
...
- [ ] **IS-I2-08. Versioning/lifecycle safety.** Разделение зафиксировано владельцем 17.08: на **рабочем** бакете
      ...
      этот путь обязан быть доказан drill'ом `IS-I3-09` (мягкое удаление с суточной отсрочкой решено, но отнесено
      на после запуска — [`POST_PRODUCTION_FILE_DELETION_AND_PURGE.md`](POST_PRODUCTION_FILE_DELETION_AND_PURGE.md)); в квоте клиники версий не существует, поэтому вопрос
...
- [ ] **IS-I4-07. Разделение секретов по потребителям.** ПЕРЕНЕСЕНО 17.08 из `NIGHT_PLAN_2026-07-26.md` B-3
      (owner-источник — находка B1 в `SECURITY_AUDIT_2026-07-25/FINDINGS_AND_OPTIONS.md`, HIGH/open). Каждый
      сервис читает только свои секреты: разные файлы, разные owners/modes, выдача через systemd `LoadCredential`
      (не общий `EnvironmentFile`, содержимое которого видно в `/proc/<pid>/environ` любому процессу того же
      пользователя). Состояние 17.08: `/opt/env/bersoncarebot/webapp.test` — один файл `root:deploy 0640` с
      `DB_PRINCIPAL_SIGNING_SECRET`, `SESSION_COOKIE_SECRET`, `INTEGRATOR_SHARED_SECRET`, `INTERNAL_JOB_SECRET`,
      `S3_SECRET_KEY`, приложение работает под `deploy`. Одна компрометация процесса отдаёт все пять плюс DSN.
      Хостовая часть на PROD — `PRE_PRODUCTION_TODO.md` §5.3.
- [ ] **IS-I4-08. Key id у подписывающих ключей — ротация без массового разлогина.** ПЕРЕНЕСЕНО 17.08 из
      `NIGHT_PLAN_2026-07-26.md` B-4 (owner-источник — находка B2 того же аудита, MEDIUM/open). `SESSION_COOKIE_SECRET`
      и `DB_PRINCIPAL_SIGNING_SECRET` — одиночные значения без `kid` и без окна «принимаем предыдущий»:
      `decodeSessionCookie` сверяет ровно один HMAC (`apps/webapp/src/modules/auth/sessionCookie.ts:126`), поэтому
      смена ключа мгновенно обнуляет все живые сессии и контексты. Требуемая форма: подписываем текущим, проверяем
      против текущего + N предыдущих. Схема уже принята в этом репозитории для приглашений (keyring с активным
      write-ключом и сохранёнными read-ключами, `SAAS_PRODUCT_UX_INITIATIVE/ENTRY_AND_INVITE_JOURNEYS.md:158`) —
      переносится, а не изобретается. Разовый принудительный разлогин при cutover уже одобрен владельцем
      (`PRE_PRODUCTION_TODO.md` §4) и этот пункт не заменяет.
- [ ] **IS-I4-09. Токен бота хранится в базе открытым текстом.** ПЕРЕНЕСЕНО 17.08 из `NIGHT_PLAN_2026-07-26.md`
      E-2 (owner-источник — находка B3 того же аудита, MEDIUM/open). `system_settings.telegram_bot_token` держит
      живой токен как есть. Пометка `secret_envelope` в реестре настроек означает только скрытие значения в
      интерфейсе и в журнале действий: сам конверт — `{ value }` без шифрования
      (`apps/integrator/src/infra/db/publicSystemSettings.ts:29`). Кто читает таблицу настроек — владеет ботом.
      Владелец ротировал токен 17.08 (`/revoke` в BotFather); ротация меняет значение, но не способ хранения, и
      новый токен лёг туда же. Ранее пункт был связан с изъятием бота из RU-сборки; решение владельца 26.07
      «ничего не вырезаем, делаем настраиваемым» это снимает — значит хранение надо чинить, а не ждать удаления.
