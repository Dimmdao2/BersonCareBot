...
21. **Диск второго сервера шифруется, но разблокируется вручную.** Круговая зависимость с tang недопустима, а
    ожидание человека для этой машины безопасно — она не обслуживает пациентов. Обязательное следствие: недоступность
    приёмника архива WAL дольше согласованного порога поднимает алерт, иначе тихо ломается RPO (`IS-I3-06`).
22. **Break-glass принят в минимальном виде:** вход root по SSH выключен, работа под именованным пользователем по
