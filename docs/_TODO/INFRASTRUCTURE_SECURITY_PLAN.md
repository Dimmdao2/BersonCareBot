...
- [ ] **IS-I0-03. Disk layout.** Выбрать один вариант: LUKS2 root с проверенным unlock/recovery либо минимальный root
      плюс LUKS2 data volume, содержащий PostgreSQL/WAL/temp, env, logs, backup staging и swap. Для открытого root
      обязательна машинная проверка отсутствия ПДн и секретов.
...
- [ ] **IS-I0-07. Independent review.** Crypto/key design и итоговая матрица инфраструктурных мер проходят
      независимую technical/security оценку; regulatory sufficiency подтверждает профильный специалист, не агент.
...
- [ ] **IS-I2-03. Streaming/multipart.** Large object не собирается целиком в RAM или plaintext temp. Выбран и
      проверен browser-side streaming либо controlled upload service; abort/expiry удаляет multipart parts и
      временный key material.
...
- [ ] **IS-I2-10. Selected field encryption decision.** После data inventory отдельно решается, нужны ли envelope/
      field encryption для конкретных DB fields и restricted settings против dump/DB-reader threat. Массовое
      «зашифровать все поля» и смешение с RLS/grants запрещены; решение получает собственный migration/key proof.
...
- [ ] **IS-I3-04. Independent Russian copy.** Настроены offsite copy, retention и integrity check в отдельном
      российском failure domain; копируются DB backups, encrypted media ciphertext и необходимые manifests.
- [ ] **IS-I3-05. PITR decision.** Из утверждённого RPO выбран один механизм (`pgBackRest`/WAL/PITR либо обоснованное
      отсутствие), и существует один документированный restore path.
