...
25. **Пациентское видео — короткая инструкция, а не запись приёма.** Владелец 17.08: «никто не хранит запись
    ...
    видео-упражнение — одно и то же, лимит 5–7 минут. Лимит по длительности и по размеру проверяется на загрузке,
    а не только описан. Уточняет объём `IS-I2-03` и снимает худший случай из расчёта дискового scratch.
