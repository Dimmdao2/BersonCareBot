...
- [ ] Имя бота получать по токену, а не спрашивать (владелец 16.09: «не понимаю почему нельзя его просто получать
      по токену не спрашивая… при сохранении требовать ввода всех значений и проверять корректность имени по
      токену»). Сейчас `telegram_login_bot_username` и `botPublicId` клиники вводятся руками, ничем не сверяются с
      токеном, и расхождение делает канал тихо неработающим: `app.is_telegram_login_configured()` смотрит только на
      непустое имя. Внешний вызов Telegram принадлежит integrator (webapp в мессенджеры не ходит) — значит
      `getMe` идёт через его `/api/bersoncare/*` поверхность, а токен через сеть не передаётся.
