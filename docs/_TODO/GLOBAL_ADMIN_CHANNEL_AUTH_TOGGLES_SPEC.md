...
- [ ] Реализовать отдельный global-admin блок настройки auth-policy рядом с существующими переключателями способов
      входа: разрешённые OAuth/мессенджеры/SMS, упорядоченные пути доставки кода, password-only policy и клиентская
      2FA. Клиентский UI и серверные маршруты должны применять одну и ту же сохранённую policy fail-closed.
