...
- [x] Удалить оставшиеся Telegram/MAX mini-app entry points из diary-путей, сохранив bot-сценарии дневника. — diary graph: `node /home/dev/brain/tools/code-search.mjs 'diary actions bot scripts keyboard URL entry point' --repo bcb -k 50`, `node /home/dev/brain/tools/code-search.mjs 'diary web app mini app Telegram MAX' --repo bcb --semantic -k 50`, exact `rg -n -i 'diary' apps/integrator/src --glob '*.{ts,json}'`; единственный code-result — canonical browser URL в `apps/integrator/src/kernel/domain/reminders/buildPatientReminderDeepLink.ts`. В `content`, `executor`, `reminders` и `integrations` отдельного diary action/script/helper entry point нет. Поведение проверяет `apps/integrator/src/kernel/domain/executor/executeActionDiaryReminderMiniAppRemoval.unit.test.ts`.
