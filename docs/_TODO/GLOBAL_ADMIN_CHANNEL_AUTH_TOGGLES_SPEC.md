...
- [x] **Этап 2 — два подтверждённых бага текущего fallback.** Phone→email разрешён только когда введённый телефон
      ...
      typecheck — PASS.
#### Решение владельца 16.09.2026 — ОТМЕНЯЕТ фиксированный порядок «телефон → email»

Дословно:

> «по телефону можно отправлять только в ботов, то есть в макс или телеграм. По имейл - надо ввести имейл»
>
> «если я выбираю "другой способ ввода" - я ввожу данные другого канала. и эти попытки полностью самостоятельны -
> мы же не должны создавать сессию до успешного входа. то есть если я случайно ошибся в номере и такой аккаунт уже
> есть, а я думаю что это проблема с телеграм, я хочу поменять способ входа - но не могу ввести правильный имейл.
> Я вспомнил почему сейчас так как сейчас но это надо исправить»
>
> «2 - бред и вред» (о показе маскированного адреса `d***@gmail.com` вместо ввода своего)
>
> «3 это про показ куда ушло - тоже надо делать правильно»

Что это отменяет: решение 30.07 «если смс путь не включен и пуша у человека нет — то даже при вводе телефона
отправляется на имейл» больше не действует для двери входа ПО НОМЕРУ. Причина, названная владельцем на живом
входе: адрес брался у аккаунта, найденного по ВВЕДЁННОМУ номеру, и при опечатке код уходил постороннему, а
человек ждал письмо, которого ему никто не слал, и сменить способ входа уже не мог. Маскированная подсказка
`d***@g***.ru`, допущенная выше по этому же плану, тоже снята: она показывает чужой адрес и не даёт ввести свой.
Почтовый вход остаётся полноценным способом — отдельной дверью, где адрес вводит сам человек.

- [x] Дверь входа по номеру доставляет только в привязанные к номеру каналы (боты Telegram/Max, SMS): явный
      `deliveryChannel: 'email'` отклоняется до поиска аккаунта, почта выпала из автоподбора, а в списке «другой
      способ» её место занял переход на почтовую дверь. —
      `apps/webapp/src/app/api/auth/phone/start/route.ts` +
      `apps/webapp/src/shared/ui/patient/auth/otpDoor.ts` +
      `phoneStartFallback.route.test.ts` (10/10) + `otpDoor.unit.test.ts` (7/7), коммит `f1a5c7f0d`.
- [x] Экран кода не утверждает доставку, которой могло не быть: при входе по номеру формулировка условная и
      называет место, где код искать; привязка своего номера вошедшим человеком утверждает отправку прямо. —
      тот же коммит; причина — владелец 15.09 полчаса ждал код при пустом `telegram_login_bot_username`.
- [x] Имя бота получать по токену, а не спрашивать. Уточнение владельца 16.09 после первой версии: «имя,
      вписанное руками — убрать, сразу получать и показывать там как нередактируемое. вся остальная механика
      лишняя — сверка, кнопка и тд». Поле имени в настройках входа и у телеграм-бота клиники стало показом:
...
- [ ] **Вопрос владельцу (16.09.2026, ждёт ответа):** остаётся ли SMS способом входа ПО НОМЕРУ. Дословно владелец
      сказал «по телефону можно отправлять только в ботов, то есть в макс или телеграм» — буквально это убирает и
      SMS, и аудит судил по этой фразе. Но другой двери у SMS нет: убрать её здесь значит выключить SMS-вход
      целиком, а названная владельцем причина (нельзя ввести данные другого канала) к SMS не относится — код
      уходит на тот самый введённый номер. Сейчас `auth_sms_enabled = false`, на контурах разницы нет. Два
      acceptance-теста аудитора стоят `.skip` в `phoneStartFallback.route.test.ts` с этим гейтом: ответ
      «выпиливаем» — снять `.skip`; ответ «остаётся» — удалить оба.
- [ ] **Находка аудита без пункта плана (16.09.2026):** экран пишет «Код отправлен в мессенджер, привязанный к
      вашему номеру» и неизвестному номеру, для которого доставки не было. Текст дословно назначил владелец
      16.09, а честная формулировка ломает анти-энумерацию (OWASP ASVS 6.3.8) — ответ обязан быть одинаков для
      существующего и несуществующего аккаунта. Не чинилось: это развилка владельца, а не дефект.
- [ ] Имя для Login Widget по-прежнему ОДНО на платформу (`telegram_login_bot_username`, registry `global`), а код
      ...
      указывать на платформенного бота, пока код приходит от клиникового. Это и есть «левый бот» из жалобы 16.09 в
      его исходном виде; решается тем, что имя для поверхности берётся у того бота, который ей доставляет.
