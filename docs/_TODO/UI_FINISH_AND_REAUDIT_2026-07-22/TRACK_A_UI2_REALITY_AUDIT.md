...
is blocked at live DEV by the full patient-booking enrichment/request-principal query: canonical migrate/runtime
closure and direct capability checks pass, but the restarted page repeats the same RSC failure (`0` booking markers,
`6` query-error markers); no second correction loop was started. Row 6 still lacks its sanctioned public split PNG.
