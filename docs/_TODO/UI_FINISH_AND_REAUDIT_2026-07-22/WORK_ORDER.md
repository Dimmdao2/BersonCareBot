...
10. **D15a** схема идентичности → **D25/D26/D27/D28/D29** по схеме → **D15b** пошаговый перенос.
    ...
    исторически интегратор сшивал людей по телефону, ошибка = склеенные чужие карточки. D15a поэтому оставался
    перед переносом; сегодня generic webhook уже не пишет идентичность, а D15b/D17 открыты только до общего
    положительного existing-owner provider-gate.
...
- **Р-D10a (владелец, 31.07), действует → D10a.** «со мной было только решено „оставить одну из трёх таблиц“ —
  ...
  **Уточнение владельца 24.08.2026, заменяет более широкое агентское толкование журнала:** задача доставки
  живёт одной строкой в `public.outgoing_delivery_queue`; планировщик делает её готовой, воркер переводит по
  статусам, при ошибке назначает retry, при успехе ставит `sent`. Отдельная строка в
  `public.notification_delivery_attempts` допустима только для реального неуспешного обращения к внешнему
  provider. Ошибка чтения БД/настройки/подготовки до provider, успех и локальный пропуск не являются попыткой
  доставки и в этот журнал не пишутся.
...
- **Р-E2 (владелец, 31.07), действует.** Кнопка «отписаться от темы» — в каждом письме рассылки и в каждом
  ...
  отдельный экран знакомства. **Развилку закрыл ведущий 22.08.2026 (владелец её не принимал):** служебные
  сообщения (код входа, подтверждение почты, напоминание о приёме, ответ на действие человека) кнопку не
  получают; отписка относится только к рассылкам — человек на служебные не подписывался, и «отписаться» от
  них означало бы не получить вход. Это же мировая норма: отписка обязательна у добровольных тем и запрещена
  у транзакционных. Владелец может переиграть. → чек-лист Track E, пункт E2.
...
- [x] **A4. Первый запуск и график «Сегодня», owner-report 25.08.2026.** Владелец: «При первом входе: все еще
      вижу хинт “нет доступа к этому разделу”. Плюс пустой график записей — то ли не правильно подключили то ли
      доступа к данным нет. Логи можно смотреть». Требуемый результат: запуск старого установленного
      пациентского PWA при активной докторской сессии возвращает в кабинет врача без ложного предупреждения;
      намеренный переход на глубокую страницу чужого кабинета по-прежнему предупреждает; недельный график
      показывает неотменённые записи из фактического календарного фида TEST. **Живой TEST 29.08:** докторская
      `/app/doctor` открылась с непустым недельным графиком (`Записей на неделе: 11`); старый пациентский PWA URL
      при активной докторской сессии дал `307` в `/app/doctor` без `app_access_denied=1`, то есть ложный хинт не
      создаётся.
...
- [x] **Б2 — регистрация второй клиники как это сделал бы живой человек.** ✅ **ЗАКРЫТО ЖИВЫМ ПРОГОНОМ НА TEST 23.08.2026, после выкатки `a62e9f254`.** Оба блокера, записанные ниже, на TEST отсутствуют. Прогон целиком, ничего не подкручивая руками: `specialist-signup/start` → `200` (`challengeId ed760c83…`); код взят из очереди доставки (`outgoing_delivery_queue`, `kind=auth_email_otp`, `status=sent` — то есть письмо реально ушло, а не «показалось»); `specialist-signup/confirm` → **`200`** `{"ok":true,"organizationId":"db283b7a…","specialistId":"48a749c9…","membershipId":"3fd09012…"}` — ни `500 pre_session`, ни `503 provisioning_pending` больше нет. Дальше вход этим же человеком: `email-password/login` → `200` `{"role":"doctor","redirectTo":"/app/doctor"}`, кабинет открывается и в шапке стоит **своя** клиника «Клиника Проба 2308», страницы `/app/doctor` и `/app/doctor/patients` живые, ответов `4xx/5xx` — ноль. ⚠️ Мелочь рядом, не блокер: `/app/doctor/clinic/settings` уводит на `/app/settings?tab=organization`, и в шапке там вместо названия клиники стоит «Therapysto Therapysto» — задвоение бренда на странице настроек. Прежний текст пункта с двумя блокерами сохранён ниже как история измерения.
      Прежняя формулировка: **Б2 — регистрация второй клиники как это сделал бы живой человек.** Владелец 22.08 дословно: «просто
...
**Актуальное состояние 28.08.2026.** Основная кодовая и миграционная часть Track D сведена в `feat`, но финальный
runtime-census опроверг прежнюю запись «остался только provider-gate»: в пяти живых путях ещё используется
retired `integrator_user_id`. Поэтому Track D остаётся `doing` и включает как новую доработку этих путей, так и
последующий объединённый эксплуатационный проход:

- вход из Telegram/MAX всё ещё подписывает и разрешает старый numeric id, а fallback способен создать аккаунт
  из мессенджера; нужно оставить только существующего webapp-пользователя, подтверждение телефона и canonical
  channel binding;
- создание, чтение и статистика напоминаний всё ещё местами зависят от старого id вместо canonical UUID;
- кнопки напоминаний Telegram/MAX проверяют владельца через старый id вместо channel binding + UUID;
- ссылка из рассылки врача возвращает пользователя в старый numeric-id auth-path;
- после перевода этих путей нужно удалить публичный retired-id runtime-контракт и доказать отсутствие живых
  читателей; внутренний ключ principal самого integrator не является публичной identity пользователя.

После этого общий живой provider-gate проверяет, что:

- existing owner проходит телефон → подтверждение self-owned contact в Telegram/MAX → код → webapp session →
  authenticated profile bind; обычный webhook без попытки webapp не создаёт аккаунт;
- обычная запись на приём рождает подтверждение и напоминание, которые действительно забирает резидентный
  integrator и доставляет разрешённому TEST-получателю;
- scheduler реально доставляет operator digest и после смены `digestTime` переносит следующий запуск.

До общего provider-gate открыты runtime-перевод пяти путей, `D15b/2`, `D15b/6`, формально зависящий от него
`D15b/7`, `D17`, `D25` и `D30`. `D30` завершён по коду и ждёт живого recipient/digest proof. Подробная ранняя
хроника сохранена ниже только как evidence.
...
- [ ] **D15b — перенос идентичности, отдельными шагами. CURRENT 28.08:** вся разработка, physical cutover,
      удаление исполняемых хвостов retired ID, actor/subject и независимые проверки завершены. Открыт один
      owner-operated messenger proof, общий для D15b/2, D15b/6, D15b/7 и D25; после него закрывается вся группа.
      Шаги обязательны по отдельности: утверждённая схема →
      ...
      **ОБЪЁМ, ИЗМЕНЁННЫЙ 20.08: шаги 1–6 ниже ПЛЮС D15b/7a — не отложено.** Владелец 03.08 исключал
      ...
      - [ ] **D15b/2 — D25: интегратор перестаёт писать идентичность.** **CURRENT 28.08:** продуктовый код
            готов: generic webhook больше не создаёт пользователя/контакт, `user.phone.link` удалён, неизвестная
            привязка возвращает `null`. Остался только положительный existing-owner TEST-проход
            `webapp start → messenger-owned contact proof → code → session`; без него нельзя закрыть реальное
            получение входа. Отрицательная часть проверена на named TEST; история исправлений остаётся в Git и
            evidence D25.
      ...
      - [ ] **D15b/6 — контакты в `user_contacts`.** **CURRENT 28.08:** physical cutover завершён:
            `user_contacts` — канонический источник, обратный mirror и legacy contact columns удалены, код и
            права находятся на TEST. Остался тот же existing-owner live gate входа/Telegram-MAX bind, включая
            authenticated `profile_bind` без создания второго аккаунта. Ниже — историческая цепочка миграции.
            В исходной переписи было 36 читателей `infra`. Не перенос, а сборка из ЧЕТЫРЁХ
      ...
      - [ ] **D15b/7 — разделение actor/subject.** **CURRENT 28.08:** реализация, композиционный аудит и
            rollback-only проверки DEV/TEST завершены; новой разработки не осталось. Пункт формально ждёт
            закрытия live gate `D15b/6`, после которого можно закрыть последовательность целиком. Ниже —
            историческая постановка и evidence. ⛔ **«OWNER-DEFER 03.08» СНЯТО 20.08 — в работе, не отложено.** Владелец,
            ...
            **143 SCOPED**-таблицы; patient-subject subset — **47 FK / 47 таблиц**. Классификация по трём группам
            ...
            текущим не являются.
            **Фактическое состояние D15b/7a на 24.08:** Ш1–Ш9 сведены и применены; композиционный аудит
            `docs/_TODO/runs/integrator-cleanup/D15B7A_COMPOSED_AUDIT_2026-08-23.md` дал PASS по полному kill-set
            KS-01…KS-20. Все семь rollback-only identity/clinical-wall проб затем выполнены на именованной TEST:
            первый проход дал 37/39 из-за двух ложных ожиданий постоянной истории `admin_audit_log`; тест
            исправлен коммитом `5a6bc4523`, повтор только затронутого набора дал 4/4. Итого по strong reuse —
            все 39 утверждений проходят, изменения TEST полностью откатываются. Галочка остаётся `[ ]` только
            из-за заданного владельцем порядка: D15b/7a закрывается после живого existing-owner TEST-гейта
            D15b/6 (webapp login → подтверждение телефона самим мессенджером → bot code → session →
            authenticated `profile_bind`). Код, аудит и DB-проверка D15b/7a завершены; новой разработки по Ш1–Ш9
            не осталось.
...
- [ ] **D17 — узкая роль в базе.** **CURRENT 28.08:** узкая роль, владельцы швов и все необходимые
      delivery roots исправлены; независимый DB-proof подтверждает доступ узкой роли и отказ широкой без
      раскрытия медицинских данных. Остался один живой сценарий: новая запись на приём должна породить и реально
      доставить подтверждение и напоминание через резидентный integrator. Исправленные TEST-отказы и их причины
      остаются историческим evidence в `runs/briefs/D17_INTEGRATOR_LOST_EXECUTE_BRIEF_2026-08-23.md` и
      `runs/briefs/D17_SEAM_OWNER_PRIVILEGES_BRIEF_2026-08-23.md`; они не описывают текущее состояние.
      ...
      **ТЕКУЩИЙ DB/runtime-гейт 24.08 — PASS, галочку не ставит без owner journey.** После восстановления
      TEST канонический rollback-only D17 proof исправлен так, чтобы брать артефакт прав именно целевой
      именованной базы (`1320a15d9`), и прошёл на `bersoncarebot_test` `1/1`: узкая роль читает шесть
      ключей доставки только своей клиники, чужая клиника и широкая роль получают `42501`, прямых прав на
      медицинские отношения нет, а реальный корень materialization создаёт одну pending-строку напоминания;
      вся транзакция откатилась. Остаётся обычная запись владельца через приложение с наблюдением
      подтверждения и напоминания в резидентной доставке.
- [x] **D19 — перепроверить правило и схему ПОСЛЕ реализации.** ✅ **ЗАКРЫТО 23.08.2026:** текущая
      `apps/webapp/ARCHITECTURE.md` синхронизирована с декларацией после D17: отдельные логины/роли,
      `app_integrator_tenant_service`, восемь FORCE RLS relations, отсутствие широкого/прямого табличного доступа
      к медицинскому канону при сохранении ограниченных named-root writers. Замер и команды —
      `8b3983fc1` (`D19_ARCHITECTURE_RECONCILE_2026-08-23.md`) и
      `docs/_TODO/runs/integrator-cleanup/D19_ARCHITECTURE_REVERIFY_2026-08-22.md`. Когда D11–D18 закрыты, вернуться к
      ...
      приложений вместо пакета. ⛔ **УСТАРЕЛО 23.08.2026:** известная находка 20.08 про текст «same
      `DATABASE_URL` and DB role for both services» исправлена в `apps/webapp/ARCHITECTURE.md`; актуальный текст
      перечисляет разные логины и точную узкую роль интегратора.
...
- [ ] **D25 — идентичность: интегратору остаётся только доставка входа.** **CURRENT 28.08:** запрещённое
      создание пустого аккаунта generic webhook'ом устранено и отрицательно проверено на TEST. Остался тот же
      положительный existing-owner `webapp → messenger proof → code → session` gate, что у `D15b/2` и
      `D15b/6`; отдельной кодовой работы нет. Решение — **Р-D25** (§2.3). Это же
      ...
      token-bound webapp→bot→webapp проверка. Отрицательная проверка generic webhook пройдена на именованном
      TEST rollback-only: неизвестные Telegram/MAX ids не создают строки, известная привязка продолжает
      разрешаться; исправленный oracle и зелёный `push:checked` — `aa943d474`.
...
- [ ] **D30 — разворот архитектуры запуска по расписанию.** **CURRENT 28.08:** единая очередь, один
      резидентный worker+scheduler, снятие старых cron/Bearer triggers и write-time lifecycle напоминаний
      специалиста реализованы. Остался эксплуатационный TEST-proof: реальная доставка получателю, запись
      operator digest и перенос следующего запуска после смены `digestTime`. Ниже — историческая последовательность
      промежуточных проходов. Решение и гейты — **Р-D30** (§2.3).
      ...
      **CORRECTION 29.08:** живой digest tick действительно доходил до назначенного часа, но падал на прямом
      чтении закрытого журнала админки; следующий холостой `not_slot` затем затирал failure успехом. На named DEV
      пять чтений сведены в один объявленный digest-window root, а `not_slot` больше не переписывает статус.
      Код и DEV проходят, итоговая галочка всё ещё ждёт той же реальной TEST-доставки и смены `digestTime`.
      **Ш9 закрыт 21.08.2026** (один резидентный процесс, один замок, один цикл). **(1) закрыт и принят
      ...
      (Ш3) — ✅ **ПРОЙДЕН 23.08.2026 ведущим на живом TEST** после выкатки `7c30a778f`. Прежний FAIL 21.08
      (`42501 permission denied for table specialist_tasks`) не воспроизводится. Замер (учётка врача
      владельца, канал — почта, включена переключателем в `/app/account?tab=notifications`):
      **create** → в `public.outgoing_delivery_queue` появляется строка `specialist_task_reminder|email|pending`
      с `next_retry_at` = времени напоминания и `event_id` вида `specialist-task:<id>:<remindAt>:email`;
      **update** времени напоминания → прежняя строка становится `dead`, рядом новая `pending` на новое время;
      **complete** → обе строки `dead`, живых доставок не остаётся; **delete** → задача исчезает
      (`count=0`), осиротевших `pending` нет. Тестовые задачи за собой удалены. ⚠️ Попутная находка,
      НЕ автофиксится: пока у темы НЕТ явной строки канала, интерфейс рисует почту включённой, а
      резолвер берёт глобальный запас (`web_push`+`telegram`) и почту НЕ выбирает — доставки не будет,
      пока человек не переключит тумблер дважды. Строка в списке находок обхода; (3) снятие
...
- [ ] **E2 — кнопка «отписаться от темы». CURRENT 28.08.** _Код приземлён 22.08 (`7e2943fc9`) плюс имя темы
      (`5d6d37327`, land `94bb29ce8`); форма проверена по коду и тестами. Старое объяснение через
      `DEV_DELIVERY_REDIRECT` больше не действует: реальные отправки на TEST ограничиваются env-режимом и
      owner TEST-аккаунтами. Галочка ждёт одну целевую рассылку owner-аккаунту по реально настроенному каналу и
      нажатие ссылки/кнопки «отписаться от темы». Тема едет внутри подписанного
      маркера, экран называет тему и ведёт в настройки_ — «готово» это план + зелёный full CI + живой прогон, а не отчёт исполнителя._ По решению **Р-E2** (§2.5): в каждом письме рассылки и рассылочном
      сообщении бота, плюс быстрый доступ к настройке на экране рассылок; служебные сообщения кнопку не получают.
      _Доказательство: `modules/doctor-broadcasts/service.ts` применяет master-тему одним send-time gate для
      preview/execute и всех каналов; `deliveryJobs.ts` и `fanOutBroadcastEmail.ts` добавляют topic CTA;
      `/api/public/notifications/unsubscribe` принимает HMAC-подписанный бессессионный маркер и всегда отвечает
      одинаково; shortcut переиспользует `DoctorPageHeader.info`. Поведенческие проверки и fault injection —
      `docs/_TODO/runs/integrator-cleanup/E2_UNSUBSCRIBE_TOPIC_2026-08-22.md`._
      **CORRECTION 29.08:** первая живая owner-рассылка дошла до реального provider-attempt и исчерпала ретраи,
      но её вид рассылки не открывал операторский инцидент. Общая dead-delivery ветка расширена на рассылки;
      повторная TEST-рассылка после выкатки должна одновременно доказать доставку и ссылку отписки.
