...
- [ ] **D16 — один цикл вместо трёх.**
      ...
      **Что сделать до сведения:** отдельная работа-исследование — как устроены резидентные процессы и планирование
      ...
      как решение с обоснованием, и только потом сводятся циклы. **Сведение НЕ сделано** и до этого исследования не
      начинается. После переноса планировщика (D5–D7, D14) и сноса транспорта (D10) у воркера должен остаться один вечный цикл — доставка. Аудит 30.07: арифметика «ровно один» сегодня не сходится, потому что циклов и точек планирования больше, чем три; перед закрытием пункта пересчитать их поимённо и назвать каждый, который остаётся, с причиной. Целевое состояние — один цикл — очередь исходящей доставки с попытками, отступами и «мёртвой полкой». Очередь
