...
- [ ] **D18 — вычистить ВЕСЬ остаток сырого SQL, оба приложения (решение владельца 30.07: «по ходу плана надо вычистить
      ...
      **Что считается законным и НЕ трогается:** файлы миграций и деплойные SQL-скрипты; вызовы через параметризованную
      обёртку drizzle (`sql` tagged template, `.execute()`), где параметры связаны, а не склеены строкой — образец
      правильного вызова, дверь жизненного цикла; сам загрузчик миграций и низкоуровневый клиент пула.

      **Что чистится:** всё остальное — сырые `.query(...)` с текстом запроса в коде приложений.

      **Замер на 30.07, чтобы пункт был проверяемым:** в интеграторе 22 файла содержат сырые вызовы (крупнейшие —
      загрузчик миграций 13, `writeIdentityAndPreferencesDirect` 9, `writeSupportQuestionsDirect` 6, клиент пула 6,
      телеметрия изоляции 5); в вебаппе 20 файлов, при том что через законную обёртку drizzle работают 79. Первым шагом —
      перепись поимённо с классификацией «миграция / законная обёртка / чистить», потому что числа выше включают и
      законные случаи.

      **Порядок:** перепись → чистка того, что уезжает вместе с D11–D16 (часть файлов просто исчезнет) → остаток
      переводится на drizzle → механическая проверка, что нового сырого SQL не появляется. Семь файлов `directPublic`
      владелец 30.07 забрал себе отдельным воркстримом — они входят в перепись, но не в наш объём работ.

      **Проверка закрытия:** поиск по обоим приложениям не находит сырых вызовов вне разрешённого списка, и это
      закреплено падающим тестом или гейтом, иначе правило снова протухнет.
