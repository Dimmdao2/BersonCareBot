...
Для этапов из многих шагов обязательны разделы `docs/ORCHESTRATION_BINDINGS.md` «Универсальный режим исполнения
многоэтапного плана» и «Урок 2026-07-22».
...
### 5.4 Пункты D20–D35 — решения владельца 31.07
...
- [ ] **D21 — напоминания о самопомощи как часть общей системы, а не своя ветка.** Владелец 31.07: «У нас есть
      ...
      **Решения владельца 31.07 (после исследования `runs/integrator-cleanup/D21_SNOOZE_RESEARCH.md`):**
      ...
      · **Причины пропуска убрать** — четыре пресета и свободный текст удаляются, остаётся факт пропуска.
        «Надо будет написать, что больно — напишет врачу. В остальном это просто лишнее раздражающее действие.»
        ⚠️ Это ОТМЕНЯЕТ рекомендацию исследования (оно предлагало пресеты сохранить).
...
- [x] **D31 (часть 1/2) — Instagram удалить.** Владелец 31.07 по развилке №4 карты: «делать API для VK, инсту
      удалять». Вырезано `apps/integrator/src/integrations/instagram/*` (дескриптор + плейсхолдеры), запись из
...
- [ ] **D32 — неподдержанный тип сообщения получает ответ, а не тишину.** Владелец 31.07 по развилке №10:
      «Какое голосовое? У нас нет такого механизма пока в вебапп. Если ты про бота — можно ответить, что пока
      не поддерживается». Сегодня белые списки типов (`config/appSettings.ts`) молча роняют сообщение: пациент
      прислал голосовое — не дошло, объяснения нет. Правило: тип вне белого списка → человеку уходит ответ
      «пока не поддерживается», сообщение не пересылается.
...
- [x] **D34 — убрать фолбэк дедупликации событий записи в памяти, порт идемпотентности сделать обязательным.**
      Развилка №2 карты (`runs/integrator-cleanup/D20_FORKS_RESEARCH.md` §2): предпосылка «дедуп в памяти»
      опровергнута — в проде дедупликация уже идёт через Postgres-порт, in-memory путь был неиспользуемым
      фолбэком, тихой тропой к повторному напоминанию после рестарта, если сборка не передала порт.
      Инженерное решение, не вопрос владельцу. **ЗАКРЫТО** (`a077732b1`):
...
- [ ] **D35 — политика отказа доставки: закрыть служебный ответ, а не оставлять его только в логе.** Решение
      владельца: «доставляем в любом случае» — опоздавший служебный ответ лучше тишины. Принятая политика:
      постоянный отказ — пометка канала без тревоги; временный — обычная лестница ретраев, исчерпание попыток
      → инцидент; служебный ответ (подтверждение, что вопрос принят) — своя КОРОТКАЯ лестница отдельно от общей
      `[60, 300, 900, 3600]` (сегодня по общей лестнице «вопрос принят» может прийти через час); подтверждение
      нажатия не ретраится вовсе. **Строить ничего не нужно** — инциденты и durable-очередь уже работают на
      соседних путях, работа в том, чтобы подключить их там, где их нет. Источник дословной формулировки:
      `TEST_SUITE_AUDIT_2026-07-29.md` §«Оба вопроса, которые я объявил открытыми» (О7).
