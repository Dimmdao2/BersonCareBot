...
- [ ] **D10a — один журнал доставки и одна очередь.** Решение — **Р-D10a** (§2.3): журнал попыток —
      ...
      **Следствие для порядка работ:** снести `integrator.delivery_attempt_logs` сейчас — значит молча
      ...
      ⚠️ Развилка инженерная, решает ведущий, владельца не касается: либо ослабить требование очереди в
      `app.record_operator_delivery_attempt` (источник становится необязательным, org/topic берутся из
      аргументов), либо провести operator-alert и booking-ICS через `outgoing_delivery_queue`. Второе честнее
      по единому циклу доставки (D16), но трогает живой путь писем пациентам — поэтому делается ПОСЛЕ D10 и
      отдельным пакетом с живой проверкой.
