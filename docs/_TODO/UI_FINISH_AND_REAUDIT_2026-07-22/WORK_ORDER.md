...
- [ ] **D37 — постановка повторов доставки идёт мимо общей очереди в двух местах.**
      ...
      `QueuePort` (`infra/adapters/jobQueuePort.ts`), через которую ходят обработчики доставки. Мимо неё
      ходят двое, каждый со своим построением payload и своей политикой повторов:
      `integrations/bersoncare/bookingLifecycleRoute.ts` (свои `maxAttempts`/`backoffSeconds`) и
      `infra/db/writePort.ts` (свой payload `message.retry.enqueue`). Следствие по конструкции: политика
      повторов живёт в трёх местах, правка общей до двух обходов не доедет. Воспроизводимого бага нет —
      это следствие, а не наблюдение. Чинится переводом обоих вызовов на `QueuePort`, без новых гейтов
...
- [ ] **D20 — карта интегратора и настоящие тесты по модулям.** Решение и порядок — **Р-D20** (§2.3): сначала
      ...
      каждому «дано → когда → тогда»), потом тесты под карту, а не под нынешний код. Уровень 0 закрыт
      (`D20_LEVEL0_TESTS_REPORT.md`, +41 тест), дальше уровни 1–4 по разделу карты «Порядок написания тестов».
...
- [ ] **D32 — неподдержанный тип сообщения получает ответ, а не тишину.** Решение — **Р-D32** (§2.3). Сегодня
      белые списки типов (`config/appSettings.ts`) молча роняют сообщение: пациент прислал голосовое — не
      дошло, объяснения нет. Правило: тип вне белого списка → человеку уходит ответ «пока не поддерживается»,
...
- [ ] **D22 — таксономия категорий уведомлений уезжает в настройки клиники.** Решение — **Р-D22** (§2.3).
