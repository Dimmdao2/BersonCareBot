...
- [ ] **D18 — один порт к базе внутри интегратора.** Сегодня доступов два: сырой `DbPort.query` с текстом запроса (вся
      ...
      (правило `.cursor/rules/clean-architecture-module-isolation.mdc`, раздел наверху). Аудит 30.07: сырой SQL живёт и ЗА пределами семьи `directPublic` — область пункта считать по всему `apps/integrator`, а не по одной папке. Переписывание семи файлов `directPublic` владелец 30.07 забрал себе отдельным воркстримом («значит я отдам это переписывать всё») — здесь
