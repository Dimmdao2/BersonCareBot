...
1. **D18a** — храповик на сырой SQL (ставится до любой правки, чтобы долг не рос).
2. **D11** ✅ дневник и ЛФК — удалено.
3. **D12** мёртвые ветки исполнителя · **D12b** перепись достижимых сценариев.
...
9. **D18c** — перепись остатка сырого SQL и снятие храповика до нуля.
...
- [ ] **D18 — вычистить ВЕСЬ остаток сырого SQL, оба приложения (решение владельца 30.07: «по ходу плана надо вычистить
      ...
      **Порядок — по решению владельца 30.07:** перепись НЕ делается заранее («думаю это имеет смысл делать после
      ...
      - [ ] **D18a — храповик, ставится ПЕРВЫМ.** Механическая проверка фиксирует нынешний список файлов с сырыми
            вызовами как замороженный долг и **падает на любом новом** сыром вызове вне разрешённого списка. Долг не
            растёт, пока идёт работа. Приём в репозитории уже применялся — так заморожен счётчик неклассифицированных
            ручек в матрице авторизации. Без этого пункта мы не заметим сырой SQL, который добавят по ходу этапов.
      ...
      - [ ] **D18c — перепись остатка и снятие храповика, ПОСЛЕДНИМ.** Когда вырезано всё, что вырезается, и переведено
            ...
            остатка, снижение храповика до нуля. Пункт закрывается тем, что проверка перестаёт находить исключения, а не
            отчётом.
