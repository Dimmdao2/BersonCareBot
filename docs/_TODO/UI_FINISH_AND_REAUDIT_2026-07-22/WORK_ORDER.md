...
**Актуальное состояние 27.08.2026, HEAD `484056ae5`.** Кодовая и миграционная часть Track D сведена в `feat`,
задеплоена на TEST и повторно проверена по ролям после поздних исправлений прав. Старые описания конкретных
`42501`, DEV-обходов и webhook-создания аккаунта ниже — история найденных и исправленных дефектов, а не текущая
поломка.

Track D остаётся `doing` из-за одного объединённого эксплуатационного остатка, а не из-за новой разработки:
...
До этого общего живого provider-gate открыты только `D15b/2`, `D15b/6`, формально зависящий от него `D15b/7`,
`D17`, `D25` и `D30`. Для первых пяти новой продуктовой реализации не осталось; `D30` также завершён по коду и
ждёт живого recipient/digest proof. Подробная ранняя хроника сохранена ниже только как evidence.
