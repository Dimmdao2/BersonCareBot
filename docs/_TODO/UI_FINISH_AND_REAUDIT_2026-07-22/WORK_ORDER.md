...
- [ ] **D31 — VK как настоящий канал, Instagram удалить.** Владелец 31.07 по развилке №4 карты: «делать API для
      VK, инсту удалять». Сегодня оба — только дескрипторы-заглушки без доставки (`integrations/vk/*`,
      `integrations/instagram/*`). Instagram вырезается как мёртвое; VK доводится до настоящей доставки по
      образцу MAX (адаптер доставки, разбор входящих, контент сценариев, деградация канала).
...
- [ ] **D33 — мёртвый провайдер бэкфила удалить.** Развилка №9 карты снята фактом: скрипта
      ...
      поиском). Удаляется независимо от судьбы самого бэкфила.
