...
> **Р-D26**). **Прогресс без ложного `[x]`:** D15b/5–6, D18b touched-path slices 1–4, D10a journal partial.
...
- [ ] **D26 — слияние аккаунтов переписывается как ИНСТРУМЕНТ ПОДДЕРЖКИ, нынешний мерж вырезается.** Решение —
      **Р-D26** (§2.3). **OWNER-DEFER pre-TEST 05.08:** не блокирует TEST deploy; `[ ]` до отдельного workstream
      после D15b. Автослияние остаётся только для аккаунта БЕЗ какой-либо медицинской истории (визиты,
