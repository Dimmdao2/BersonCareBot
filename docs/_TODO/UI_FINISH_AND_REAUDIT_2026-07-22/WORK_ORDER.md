...
- [ ] **D27 — вход по номеру: настройка «отправлять код для входа сюда» и экран подтверждения.**
      ...
      **CURRENT PARTIAL 03.08:** public `email-otp/start` не раскрывает наличие email по provider failure или
      быстрому unknown-path: valid non-rate-limited known/unknown и provider success/failure получают нейтральный
      `200` одной схемы с server-enforced minimum response floor; provider failure остаётся только в server log без
      email/OTP. Durable delivery queue, preference/default и полный channel screen ещё впереди; D27 остаётся открыт.
