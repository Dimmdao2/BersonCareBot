...
**Актуальное состояние 29.08.2026.** Runtime-перевод пяти путей, удаление публичного retired-id контракта,
единая declaration/generator схема прав, расписание, retention и связный TEST-проход завершены. Messenger webhook
сам аккаунт не создаёт: он подтверждает self-owned телефон существующего webapp-пользователя и создаёт canonical
channel binding. Track D остаётся `doing` только до общего живого provider-gate, который проверяет, что:
...
- scheduler реально доставляет operator digest и после смены `digestTime` переносит следующий запуск. Сам digest
  уже доставлен на TEST 29.08; открыта только проверка переноса следующего запуска после смены времени.

До общего provider-gate открыты `D15b/2`, `D15b/6`, формально зависящий от него `D15b/7`, `D17`, `D25` и
операционная часть `D30`. Подробная ранняя хроника сохранена ниже только как evidence.
