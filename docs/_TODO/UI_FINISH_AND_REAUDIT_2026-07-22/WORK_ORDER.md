...
- **Р-D26 (владелец, 31.07), действует → D26.** «Если мед история была — то мерж решается только
  ...
  §5.2a.
  **Р-D26b (владелец, 20.08), действует, ЗАМЕНЯЕТ отсрочку ниже.** «как раз пора» — снимает
  `OWNER-DEFER pre-TEST 05.08` (не блокирует TEST deploy, `[ ]` до workstream после D15b): D26 переходит в
  работу сейчас, не ждёт закрытия D15b. Сам объём и правила Р-D26 не меняются — меняется только момент
  старта.
...
> **OWNER-DEFER:** D15b/7 (псевдоним — решение после D15b/6, владелец 03.08). D26 (merge → инструмент поддержки,
> **Р-D26**) — отсрочка СНЯТА 20.08 (**Р-D26b**, «как раз пора»), в работе, не ждёт D15b.
...
- [ ] **D26 — слияние аккаунтов переписывается как ИНСТРУМЕНТ ПОДДЕРЖКИ, нынешний мерж вырезается.** Решение —
      **Р-D26** (§2.3). ⚠️ **В РАБОТЕ с 20.08 (Р-D26b) — прежний `OWNER-DEFER pre-TEST 05.08` снят владельцем**
      («как раз пора»), не ждём закрытия D15b. Автослияние остаётся только для аккаунта БЕЗ какой-либо медицинской истории (визиты,
