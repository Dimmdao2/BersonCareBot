...
- [ ] **D26 — слияние аккаунтов переписывается как ИНСТРУМЕНТ ПОДДЕРЖКИ, нынешний мерж вырезается.** Решение —
      ...
      **Статус НЕ «пропущено»:** явно отложено владельцем 04.08 (`docs/OWNER_DECISIONS.md`: «D26 — не надо делать
      сейчас»), а не забыто. Живой код сегодня (`packages/platform-merge/src/messengerPhonePublicBind.ts`) всё ещё
      сам сливает аккаунты по телефону без техподдержки — это тот самый интегратор-путь, который D26 должен
      вырезать, и он ждёт именно этого пункта, не отдельного нового.
