...
| --- | --- | --- |
| D4: «нужен ли санкционированный CI-раннер и креды для RLS-теста» | Вопроса нет. Тест проверяет, что **функция реально пишет в базу** (появился после живого дефекта), а не «правильно ли настроены права» — такие владелец запрещает. Место прогона задано §10b: именованная `bcb_webapp_dev`/`bersoncarebot_test`, никогда прод и никогда одноразовая база. Fixture-dependent test был удалён 21.08.2026 owner-решением о запрете отдельного fixture-наполнения; не переписан, и no replacement fixture machinery. Live named DEV/TEST verification, if needed, is a lead one-off using existing accounts/data. | D4 (RLS-проверка upsertReminderRuleDirect) |
