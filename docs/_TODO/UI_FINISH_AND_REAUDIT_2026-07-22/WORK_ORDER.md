...
- [ ] **D36 — тесты интегратора, блок И** (передано из #1081 целиком). ВЕДЁТСЯ В D20 (этот файл, пункт «D20 —
      карта интегратора и настоящие тесты по модулям») и `AGENTS.md` §10a. Исходная методология И1-И3
      («уровни 0-2, причём уровень 0 переписывается против ПЛАНА; уровень 2 закрывается числом, а не следующим
      кругом; стоп по целевому состоянию модуля, а не по номеру уровня») дословно взята из #1081 и уровнями
      как активное поручение больше не действует — владелец 01.08: «твоя задача — не писать тесты, а писать
      код… тесты нужны только для того, для чего созданы изначально — безопасность и поведение»; действующее
      правило — implementation-first по §10a, тест заводится только под названный дорогой и молчаливый отказ
      или security-поломку, никогда под форму/число/уровень исходника. Контракт шва вебапп → интегратор (И4)
      измерен и поведенчески проверен `docs/REPORTS/D39_DELIVERY_SEAM_CENSUS_2026-08-20.md`: восемь швов
      закрыты route-тестом «тот же signed request дважды → один dispatch/write, другой idempotency key —
      второй», один (`manual integrator merge`) закрыт как мёртвый код, один (transactional email) — owner-решение
      «не дедуплицировать» (20.08). Самостоятельного скоупа сверх D20/§10a/D39 у этого пункта не остаётся;
      отдельная работа по нему не заводится.
...
- [ ] **D20 — карта интегратора и настоящие тесты по модулям.** Решение и порядок — **Р-D20** (§2.3): сначала
      ...
      режиме `legacy-guc/locked`; живые DEV и TEST работают в `port-context`
      (`TRACK_D_ORCHESTRATION_HANDOFF_2026-08-20.md`), где эта ветка не исполняется. Текущая декларация прав это
