...
- [ ] **D26 — слияние аккаунтов переписывается как ИНСТРУМЕНТ ПОДДЕРЖКИ, нынешний мерж вырезается.** Решение —
      **Р-D26** (§2.3). Автослияние остаётся только для аккаунта БЕЗ какой-либо медицинской истории (визиты,
      ...
      ⚠️ **Формулировка правила устарела на день 20.08.** Владелец в тот же день уточнил цепочкой правок правило
      конфликта — `IDENTITY_AND_MERGE_SCHEME.md` §5.2b «Финальное правило — блок только при КОНФЛИКТЕ»: блокирует
      только когда мед-данные есть с ОБЕИХ сторон одновременно, а не любая история на одной стороне; при
      отсутствии конфликта история переносится автоматически. Цитата выше («только для аккаунта БЕЗ истории»)
      — старее и строже; при исполнении D26 читать §5.2b, не эту строку.
      **Статус — В РАБОТЕ, не «отложено» и не «не начато» (поправка 20.08, найдено независимым аудитом).** 04.08
      было решено «не сейчас» (`docs/OWNER_DECISIONS.md`), но параллельный ход (`wt/d26-support-merge-20260820`,
      ведущий Opus, сегодня 12:57–15:21) уже начал именно D26: слепой аудит `bdc1f11c8` нашёл два дефекта,
      находка 1 отклонена владельцем (не дефект, уточнение внесено в §5.2), гейт доведён до финального правила
      владельца — `e6f605883`, `assertAutomaticMergeHasNoMedicalHistory` теперь блокирует автослияние ТОЛЬКО при
      конфликте мед-данных на обеих сторонах (переписка не блокирует никогда), лог инициатора мержа — `19501de2f`.
      **Находка 2 аудита остаётся открытой (handoff воркеру, не закрыт):** обратный support-перенос не переносит
      `clinical_visit` и ряд clinical/anamnesis/discussion/clinic-link таблиц — «перенести все данные в любом
      направлении» (§5.8) не выполнено полностью. Это и есть реальный остаток D26, не «мерж по телефону без
      гейта вовсе» — та часть уже сделана. Полный протокол — очередь `NIGHT_WAVE_AUDIT_QUEUE_2026-07-28.md`,
      записи вокруг `bdc1f11c8`/`39a8d0d41`/`e6f605883`/`19501de2f`. Чекбокс не закрывать до находки 2.
