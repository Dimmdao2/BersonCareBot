...
- [ ] **D13 — константы уведомлений о записи.** В маршруте жизненного цикла записи зашиты смещения «за 24 часа» и «за 2
      часа» и тексты напоминаний строками в коде — при том, что в вебаппе УЖЕ есть настройки
      `doctor_appointment_reminder_enabled`, `doctor_appointment_reminder_offsets_minutes` (на клинику) и модуль шаблонов
      уведомлений. То есть это вторая, конкурирующая реализация, которая перекрывает настроенную врачом. Решение
      владельца 30.07: «вырезать нещадно». Источником становятся настройки и шаблоны вебаппа.
...
- [ ] **D15 — идентичность: исследование ПЕРЕД работой.** Интегратор сам создаёт и сливает людей, решает доверие к
      телефону, ставит зачисления и предпочтения по умолчанию. Это одновременно главная утечка домена и главный блокер
      узкой роли. Решение владельца 30.07: «надо запустить сначала сильное исследование командой». Только после его
      результатов — план переноса с миграцией и идемпотентностью. Неверное слияние смешивает разных людей и их
      медицинские данные — цена ошибки выше, чем у всех остальных пунктов вместе.
- [ ] **D16 — один цикл вместо трёх.** После переноса планировщика (D5–D7, D14) и сноса транспорта (D10) у воркера
      остаётся ровно один вечный цикл — очередь исходящей доставки с попытками, отступами и «мёртвой полкой». Очередь
...
- [ ] **D18 — один порт к базе внутри интегратора.** Сегодня доступов два: сырой `DbPort.query` с текстом запроса (вся
      семья `directPublic`, семь файлов) и drizzle-сессия `getIntegratorDrizzleSession` (восемь потребителей в
      `infra/db/repos/`). Целевое состояние — один путь: drizzle-слой своего приложения, сырого SQL для нового кода нет
      (правило `.cursor/rules/clean-architecture-module-isolation.mdc`, раздел наверху). Переписывание семи файлов
      `directPublic` владелец 30.07 забрал себе отдельным воркстримом («значит я отдам это переписывать всё») — здесь
      фиксируется только целевое состояние и проверка: после D11–D16 в интеграторе не должно остаться второго способа
      ходить в базу. Без этого пункта D17 не проверить: узкую роль нельзя выдать, пока непонятно, кто и как ходит.
...
- [ ] **D17 — узкая роль в базе, последним.** Отдельная роль для интегратора: право писать только то, что нужно приёму и
      ...
      что у вебаппа, поэтому никакая изоляция в коде ничего не значит. Выдавать роль можно только после D11–D16: пока
      интегратор пишет канон, узкая роль уронит живые пути. Проверка — деплой ассертит точный набор прав, несовпадение
      валит выкатку.
