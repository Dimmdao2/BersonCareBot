...
- [ ] **D18 — вычистить весь остаток сырого SQL в обоих приложениях.** Решение и объём — **Р-D18** (§2.3).
      ...
      - [ ] **D18c — перепись остатка и снятие списка исключений, последним.** Когда вырезано всё, что вырезается:
            ...
            **CURRENT 02.08:** `directPublic` уже переведён на общий порт (`018043646`). Точный текущий запуск
            ...
            здесь — legacy integration tests, а главный production-хвост integrator —
            `infra/db/repos/projectionHealthCore.ts` и его CLI-контракт. Старые числа выше оставлены как история
            проходов и не являются текущим остатком.
