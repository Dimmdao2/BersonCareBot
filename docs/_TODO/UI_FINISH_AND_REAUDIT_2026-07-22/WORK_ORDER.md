...
**ВЫБОР ИСПОЛНИТЕЛЯ — ПО КАНОНУ, НЕ ПО ВКУСУ ЛИДА.** Владелец 30.07: «у тебя же есть полный список решения когда и
какого агента выбрать». Есть: `/home/dev/brain/docs/MODEL_TIERS.md`, таблица `Job → модель/effort`, источник истины в
коде — `lib/agent-runner/model-policy.mjs`. Лид называет ТИП РАБОТЫ, а модель и effort выбирает карта; своих таблиц в
этом плане не держим, чтобы знание о цене жило в одном месте. Запускать через `ORCH_JOB=<job> tools/orch-launch.sh …`.

| Работа по этой ветке | job |
| --- | --- |
| перепись, инвентаризация, поиск | `explorer` (terra/low) |
| вырезание веток, названных поимённо; конверсии по доказанному образцу; массовые правки с командной приёмкой | `mechanic` (luna/low) |
| обычная правка кода по этапу | `worker` (terra/medium) |
| правка с нетривиальной логикой (перенос владения доменом, потребитель настроек) | `worker-hard` (terra/high) |
| обычный независимый аудит этапа | `reviewer` (terra/high) |
| аудит опасного: идентичность, узкая роль, снос транспорта | `reviewer-critical` (sol/xhigh) |

Отсюда же ответ про `xhigh`: он не «на всякий случай», но и не запрещён — он положен ровно одному job’у,
`reviewer-critical`, и берётся только на трёх дорогих этапах. Оркестрация по канону всегда Opus, независимо от
провайдера — это не выбор лида.
