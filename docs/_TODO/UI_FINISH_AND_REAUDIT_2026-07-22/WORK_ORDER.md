...
- [ ] **D15a — идентичность: сильное исследование командой ПЕРЕД работой** (решение владельца 30.07).
      ...
      он требует «утверждённую схему», а схему утверждает владелец — семь продуктовых развилок сведены ему одним
      листом в `runs/owner-questions.md` и ждут ответа. Ключевой вывод исследования: интегратор СЕГОДНЯ владеет
...
- [ ] **D16 — один цикл вместо трёх.**
      ...
      циклу вердикт с причиной и ссылкой на блокирующий пункт. **Сведение НЕ сделано:** упирается в D10 (ждёт ответа
      владельца про `outgoing_delivery_queue`) и в открытый вопрос формулировки — один цикл на весь интегратор или
      один цикл ДОСТАВКИ внутри воркера (у интегратора два резидентных процесса, вопрос владельцу записан). После переноса планировщика (D5–D7, D14) и сноса транспорта (D10) у воркера должен остаться один вечный цикл — доставка. Аудит 30.07: арифметика «ровно один» сегодня не сходится, потому что циклов и точек планирования больше, чем три; перед закрытием пункта пересчитать их поимённо и назвать каждый, который остаётся, с причиной. Целевое состояние — один цикл — очередь исходящей доставки с попытками, отступами и «мёртвой полкой». Очередь
