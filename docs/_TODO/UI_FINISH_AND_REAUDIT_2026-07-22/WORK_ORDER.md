...
сохраняется здесь только как указатель, чекбокса у пункта нет. Исходная методология И1-И3 («уровни 0-2, причём
уровень 0 переписывается против ПЛАНА; уровень 2 закрывается числом, а не следующим кругом; стоп по целевому
состоянию модуля, а не по номеру уровня») дословно взята из #1081 и уровнями как активное поручение больше не
действует — владелец 01.08: «твоя задача — не писать тесты, а писать код… тесты нужны только для того, для
чего созданы изначально — безопасность и поведение»; действующее правило — implementation-first по §10a, тест
заводится только под названный дорогой и молчаливый отказ или security-поломку, никогда под форму/число/уровень
исходника. Контракт шва вебапп → интегратор (И4) измерен `docs/REPORTS/D39_DELIVERY_SEAM_CENSUS_2026-08-20.md`:
