...
**ТИРЫ ИСПОЛНИТЕЛЕЙ ПО ЭТОЙ ВЕТКЕ (решение владельца 30.07: «можно гнать кодекс на все основные задачи, только не
борщи с xhigh там где не надо — у нас лимит не резиновый»).**

- **Дефолт — `gpt-5.6-terra`, medium/high.** Механическое и повторяемое: удаления, переносы файлов, конверсии на drizzle,
  перепись, правка контента, тесты по образцу.
- **`gpt-5.6-sol`, high — только где нужно СУЖДЕНИЕ:** идентичность (D15), выдача узкой роли (D17), снос транспорта
  (D10), разбор спорных случаев переписи, закрывающие аудиты опасных этапов.
- **`xhigh` не берём вовсе**, пока не назову причину вслух в брифе. Длинная связная задача — не повод; повод — только
  реальная потеря нити на длинном контексте.
- **Второй аудитор другой моделью (claude-opus-5) — не по умолчанию**, а на трёх этапах, где ошибка дорогая:
  идентичность, узкая роль, снос транспорта. На остальном достаточно одного независимого аудитора.
- **`gpt-5.6-luna`, low — берём смелее, но по признаку, а не по ощущению «просто».** Луна годится там, где приёмка
  МЕХАНИЧЕСКАЯ и проверяется командой: поиск вернул ноль, тайпчек зелёный, тесты зелёные, число совпало. Примеры по
  этой ветке: подсчёты и поиски, вырезание веток, которые перепись уже назвала поимённо с перехватчиком, массовая
  конверсия вызовов на drizzle по уже доказанному образцу, вытаскивание списков из отчётов, форматирование.
- **Луна НЕ берётся, если ошибка тихая.** То есть там, где надо ДОКАЗАТЬ, что путь мёртв, решить, что именно относится
  к механике, написать поведенческий тест или проверить его снятием защиты. Пример из этой же работы: удаление блока
  дневника выглядело механическим, а требовало решений — какие шаблоны и сценарии считать частью блока, что оставить
  соседям, куда деть общие резолверы. Это была работа sol, и правильно.
- Практическое правило: **если приёмку можно записать одной командой — луна; если приёмка начинается со слова
  «убедиться» — терра и выше.**
