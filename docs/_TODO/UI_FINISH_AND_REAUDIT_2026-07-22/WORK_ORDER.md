...
**Интегратор (Track D), текущее состояние 01.09.2026.** Кодовая и DB-часть вычистки завершена на DEV и TEST:
интегратор принимает события и доставляет сообщения через узкие именованные двери, не создаёт аккаунты и не
пишет продуктовые данные обходным путём; права формируются единой декларацией и генератором. Открыты только
живые проверки с участием владельца: подтверждение телефона и вход через мессенджер, реальная доставка
подтверждения и напоминания о записи, а также отдельный PROD/domain cutover. Текущая правда Track D —
`docs/_TODO/DB_PRIVILEGE_LAYER_REBUILD/PLAN.md`; нижняя подробная хроника этого файла не создаёт новых задач.
...
⚠️ **Старшинство 01.09:** по DB-привилегиям, ролям, RLS и Track D старшая инстанция — последние решения
владельца в `docs/OWNER_DECISIONS.md` и текущий статус в
`docs/_TODO/DB_PRIVILEGE_LAYER_REBUILD/PLAN.md`. Несовместимая старая проза ниже — только история; она не
возвращает закрытые работы. Одноразовые брифы, сырые логи и промежуточные аудиты удалены из рабочего дерева и
...
- **Р-TD2 (владелец, 2026-07-23), действует.** Единая PostgreSQL-цель не держит HTTP как внутренний транспорт
  ...
  Ответы владельца 31.07 дословно — `история Git: OWNER_QUOTE_2026-07-31_IDENTITY.md`.
  Пересказывать их вместо ссылки нельзя.
- **Р-TD3 (taskdb/агентское), действует.** Текущее состояние всего Track D ведёт одна карточка `#1085` и
  `docs/_TODO/DB_PRIVILEGE_LAYER_REBUILD/PLAN.md`; старые карточки `#987`, `#984` и `#959` — исторические срезы,
  а не параллельные источники работы. HTTP-транспортная
  часть Stability E3 (`#980`) поглощена Track D и отдельно не исполняется: транспорт, назначенный к удалению,
  не оптимизируют и не расширяют. Историческая развилка D1 заархивирована; текущую реализованную схему и остаток
  работы определяет только `DB_PRIVILEGE_LAYER_REBUILD/PLAN.md`.
...
9. **D10a + D16 + D30** — одна работа: единая очередь доставки, один цикл доставки, разворот запуска по
   расписанию. Порядок шагов внутри — `история Git: D30_SCHEDULER_REVERSAL_PLAN.md` (Ш0…Ш9).
...
- **Р-D21 (владелец, 31.07, после исследования `история Git: D21_SNOOZE_RESEARCH.md`), действует →
...
- **Р-D30 (владелец, 31.07 и 20.08), действует → D30, D16.** Разделяются две оси: владение решением (какие
  ...
  **Два условия владельца («необходимы», 31.07) — гейты, а не пожелания:**
  ...
  `история Git: D30_SCHEDULER_REVERSAL_PLAN.md`; статус каждого шага закрывается только фактическим
...
- **Р-D34 (агентское), действует → D34.** Развилка №2 карты (`история Git: D20_FORKS_RESEARCH.md`
...
- [x] **Б2 — регистрация второй клиники как это сделал бы живой человек.** ✅ **ЗАКРЫТО ЖИВЫМ ПРОГОНОМ НА TEST 23.08.2026, после выкатки `a62e9f254`.** Оба блокера, записанные ниже, на TEST отсутствуют. Прогон целиком, ничего не подкручивая руками: `specialist-signup/start` → `200` (`challengeId ed760c83…`); код взят из очереди доставки (`outgoing_delivery_queue`, `kind=auth_email_otp`, `status=sent` — то есть письмо реально ушло, а не «показалось»); `specialist-signup/confirm` → **`200`** `{"ok":true,"organizationId":"db283b7a…","specialistId":"48a749c9…","membershipId":"3fd09012…"}` — ни `500 pre_session`, ни `503 provisioning_pending` больше нет. Дальше вход этим же человеком: `email-password/login` → `200` `{"role":"doctor","redirectTo":"/app/doctor"}`, кабинет открывается и в шапке стоит **своя** клиника «Клиника Проба 2308», страницы `/app/doctor` и `/app/doctor/patients` живые, ответов `4xx/5xx` — ноль. ⚠️ Мелочь рядом, не блокер: `/app/doctor/clinic/settings` уводит на `/app/settings?tab=organization`, и в шапке там вместо названия клиники стоит «Therapysto Therapysto» — задвоение бренда на странице настроек. Прежний текст пункта с двумя блокерами сохранён ниже как история измерения.
      ...
      **Проверено, что за 200 стоит настоящая клиника, а не пустая строка:** организация
      ...
      (`история Git: SIGNUP_RESEND_DEAD_PATH_2026-08-22.md`), приземлено, но на TEST
...
**Актуальное состояние 01.09.2026.** Runtime-перевод пяти путей, удаление публичного retired-id контракта,
единая declaration/generator схема прав, расписание, retention, digest/digestTime и связный TEST-проход завершены. Messenger webhook
...
- operator digest и смена `digestTime` доказаны живьём на TEST 29.08 и больше не являются хвостом Track D.

Открыты только два owner-operated результата: `D25` — messenger proof/код/session без второго аккаунта;
`D17` — реальное получение подтверждения и напоминания обычной записи. Завершённые `D15b` и `D30` ниже не
возобновлять по старым промежуточным FAIL; подробная ранняя хроника — только evidence.
...
- [x] **D3 — беседы и сообщения поддержки.** ✅ **ЗАКРЫТО 31.07** (`c124a2d78`) переносом владения: открытие
      ...
      **Хвост, не закрыт:** сверка двух строк бесед с `organization_id IS NULL` (нужна перед любым ужесточением
      ...
      `история Git: D3_SUPPORT_OWNERSHIP_REPORT.md`.
...
- [x] **D10 — снос транспорта проекции, после всех производителей.** ✅ **ЗАКРЫТО 21.08.** Снесены fanout/outbox,
      ...
      **Р-D10** (§2.3), сохранены.
      ...
      `история Git: D10_TRANSPORT_REMOVAL_STAGE1_2026-08-20.md`,
      ...
      Не блокер D10 и не второй активный пункт D10 — см. `история Git: DB_PRIVILEGE_LAYER_REBUILD/AUDIT_LOG.md` (запись про
...
- [x] **D12b — перепись ДОСТИЖИМЫХ сценариев исполнителя.** ✅ **ЗАКРЫТО 31.07** (`43eff0ac8`),
      `история Git: D12B_REACHABLE_CENSUS.md`: 105 достижимых сценариев — 30 «канал/доставка»,
...
- [x] **D15a — идентичность: схема переноса, утверждённая владельцем.**
      🟡 Исследование готово 31.07 (`1c6a25546`) — `история Git: D15A_IDENTITY_RESEARCH.md`.
      🟡 Схема с ответов владельца готова — `история Git: IDENTITY_AND_MERGE_SCHEME.md`, проверена
...
- [x] **D15b — перенос идентичности, отдельными шагами. CURRENT 01.09:** вся разработка, physical cutover,
      удаление исполняемых хвостов retired ID, actor/subject и независимые проверки завершены. Живой
      owner-operated messenger proof учитывается один раз в D25 и не переоткрывает завершённые шаги D15b.
      ...
      **ОБЪЁМ, ИЗМЕНЁННЫЙ 20.08: шаги 1–6 ниже ПЛЮС D15b/7a — не отложено.** Владелец 03.08 исключал
      ...
      Обоснование порядка и замеры — `история Git: IDENTITY_DB_SPLIT_RESEARCH_2026-08-03.md` §6 и
      `IDENTITY_AND_MERGE_SCHEME.md` §2b.
      - [x] **D15b/1 — живой замер перед работой.** ✅ 03.08, `ff359cff6`, land `ee59d00e6`; документ
            `история Git: D15B1_IDENTITY_CENSUS_2026-08-03.md`. ⚠️ Перепись ОПРОВЕРГЛА три числа,
      ...
      - [x] **D15b/2 — D25: интегратор перестаёт писать идентичность.** **CURRENT 01.09:** продуктовый код
            готов: generic webhook больше не создаёт пользователя/контакт, `user.phone.link` удалён, неизвестная
            привязка возвращает `null`; отрицательная часть проверена на named TEST. Положительный existing-owner
            проход `webapp start → messenger-owned contact proof → code → session` учитывается только в D25.
      - [x] **D15b/3 — один порт идентичности в вебаппе.** ✅ 03.08, ветка `wt/d15b3-identity-port`,
            отчёт `история Git: D15B3_IDENTITY_PORT_2026-08-03.md`. Новый модуль
      ...
      - [x] **D15b/4 — RLS на `platform_users`.** Делается ПОСЛЕ 2 и 3: пока таблицу пишут 27 мест из двух
            ...
            `pgAdminPlatformUserStats.ts` — `история Git: D15B4_PLATFORM_USERS_RLS_REPORT.md`.
      ...
      - [x] **D15b/6 — контакты в `user_contacts`.** **CURRENT 01.09:** physical cutover завершён:
            `user_contacts` — канонический источник, обратный mirror и legacy contact columns удалены, код и
            права находятся на TEST. Existing-owner live gate входа/Telegram-MAX bind учитывается только в D25.
            Ниже — историческая цепочка миграции.
            ...
            — `история Git: DB_PRIVILEGE_LAYER_REBUILD/evidence/42-d15b6-canonical-contacts-cutover.md`). Галочка
      ...
      - [x] **D15b/7 — разделение actor/subject.** **CURRENT 01.09:** реализация, композиционный аудит и
            rollback-only проверки DEV/TEST завершены; новой разработки и отдельного live gate не осталось. Ниже —
            ...
            **143 SCOPED**-таблицы; patient-subject subset — **47 FK / 47 таблиц**. Классификация по трём группам
            и спорные случаи — в документе переписи (`история Git: D15B1_IDENTITY_CENSUS_2026-08-03.md`);
            ...
            **Фактическое состояние D15b/7a на 24.08:** Ш1–Ш9 сведены и применены; композиционный аудит
            `история Git: D15B7A_COMPOSED_AUDIT_2026-08-23.md` дал PASS по полному kill-set
...
- [ ] **D17 — узкая роль в базе.** Код, декларация прав, владельцы швов и живой DB-proof на DEV/TEST завершены. Остаётся только owner-operated TEST-проверка: создать запись на приём и получить реальное подтверждение и напоминание через резидентный integrator. Текущее техническое доказательство ведётся в `docs/_TODO/DB_PRIVILEGE_LAYER_REBUILD/PLAN.md`; промежуточная хроника удалена из рабочего дерева и остаётся в истории Git.
- [x] **D19 — перепроверить правило и схему ПОСЛЕ реализации.** ✅ **ЗАКРЫТО 23.08.2026:** текущая
      ...
      `история Git: D19_ARCHITECTURE_REVERIFY_2026-08-22.md`. Когда D11–D18 закрыты, вернуться к
...
- [x] **D20 — карта интегратора и настоящие тесты по модулям.** Решение и порядок — **Р-D20** (§2.3): сначала
      ...
      `история Git: D20_INTEGRATOR_MAP.md` (279 модулей, по каждому «дано → когда → тогда»), включая
      ...
      **Текущий итог (21.08.2026):** конкретные найденные behavioral/security-разрывы уровней 0–4 закрыты
      ...
      - D4 (RLS-проверка `upsertReminderRuleDirect`) — тест `writeReminderRulesDirect.rls.integration.test.ts`
        ...
        census — `история Git: LIVE_DEV_TEST_FIXTURES_RETIREMENT_2026-08-21.md` (Pass 6).
...
- [x] **D31 (часть 2/2) — VK как настоящий канал.** Та же развилка №4: Callback API webhook с confirmation и
      ...
      availability are wired. Evidence: `история Git: D31_VK_CHANNEL_REPORT_2026-08-21.md`.
...
- [x] **D35 — политика отказа доставки: закрыть служебный ответ, а не оставлять его только в логе.** Решение и
      ...
      Бриф `история Git: D35_DELIVERY_FAILURE_POLICY_BRIEF.md` живёт в клоне `fmtcut`, до слияния в
...
- [x] **D23 — старая ветка ответа администратора в поддержке** (`handleConversationAdminReply`, оставлена D3).
      ...
      **Разбор 01.08 (исполнитель, подтверждено по коду): дубликатов НЕТ, удалять нечего.** Функция уже
      ...
      без привязанной учётки — уходят. Учёт «отвеченности» остаётся дырой D4 и закрывается там. Отчёт: `история Git: worker-d23-admin-reply.log`.
...
- [ ] **D25 — идентичность: интегратору остаётся только доставка входа.** Кодовая часть завершена: webhook не создаёт аккаунты, канонические контакты перенесены, старый integrator-ID и дублирующие хранилища удалены. Остаётся только owner-operated TEST-проверка `webapp → подтверждение телефона средствами мессенджера → код → сессия/профиль` на существующей учётной записи; бот не создаёт нового пользователя.
- [x] **D26 — слияние аккаунтов переписывается как ИНСТРУМЕНТ ПОДДЕРЖКИ, нынешний мерж вырезается.** Решение —
      **Р-D26** (§2.3). Правило конфликта — `IDENTITY_AND_MERGE_SCHEME.md` §5.2b (финальное, 20.08): блокирует
      ...
      `история Git: D26_HISTORY_TRANSFER_FIX_2026-08-20.md`.
...
- [x] **D27 — вход по номеру: настройка «отправлять код для входа сюда» и экран подтверждения.**
      ...
      **CURRENT PARTIAL 03.08 (D27-B1):** прежний бриф D27-B1 прямо запрещал делать провенанс-дефолт «первый
      ...
      бриф `история Git: D27_CHANNEL_ORDER_RECONCILE_BRIEF_2026-08-03.md` §F5. Тесты:
...
- [x] **D29 — ФИО только кириллицей, автоподстановка имени отменяется.** Решение — **Р-D29** (§2.3). Это не
      ...
      бриф `история Git: FIO_REJECT_MESSAGE_BRIEF_2026-08-04.md`.
- [x] **D30 — разворот архитектуры запуска по расписанию.** **CURRENT 01.09:** единая очередь, один
      резидентный worker+scheduler, снятие старых cron/Bearer triggers и write-time lifecycle напоминаний
      специалиста реализованы. Operator digest и изменение `digestTime` доказаны живьём на TEST 29.08.
      Подтверждение/напоминание обычной записи учитывается только в D17. Ниже — историческая последовательность
      промежуточных проходов; она не задаёт новые задачи. Решение — **Р-D30** (§2.3).
      ...
      `история Git: D30_SCHEDULER_REVERSAL_PLAN.md`; его старые технические предпосылки не имеют authority.
      **CORRECTION 29.08:** живой digest tick действительно доходил до назначенного часа, но падал на прямом
      ...
      Код, DEV и итоговая TEST-проверка digest/digestTime проходят; прежнее ожидание снято.
...
- [ ] **E2 — кнопка «отписаться от темы». CURRENT 28.08.** _Код приземлён 22.08 (`7e2943fc9`) плюс имя темы
      ...
      `история Git: E2_UNSUBSCRIBE_TOPIC_2026-08-22.md`._
