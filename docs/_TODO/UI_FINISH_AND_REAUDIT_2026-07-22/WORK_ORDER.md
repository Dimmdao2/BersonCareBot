...
- [ ] **D25 — идентичность: интегратору остаётся только доставка входа.** Решение — **Р-D25** (§2.3). Это же
      ...
      до D17.
      ⚠️ **ЧАСТИЧНО 03.08–05.08:** D15b/2 свёл запись к общей реализации
      `packages/platform-merge/src/identityProjectionWrite.ts` (`5137e8c68`, land `2c1cd63fb`), но не удалил
      два достижимых writer-пути интегратора: `user.upsert` и `user.phone.link`; D25 по коду ещё не закрыт.
