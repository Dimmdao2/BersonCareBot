...
- [ ] **D10 — снос транспорта проекции, после всех производителей.** Только после точной переписи «нулевой
      ...
      **Перепроверено ведущим своей рукой:** webapp `tsc --noEmit` EXIT=0; webapp vitest по ВСЕЙ затронутой
      ...
      🔴 **ЗАМЕР 21.08 ведущим на живых базах — сноса таблицы СЕЙЧАС БЫТЬ НЕ МОЖЕТ, и вот почему.**
         На TEST развёрнут код ДО D10 (`/opt/projects/bersoncarebot-test`, вершина `4693847510d`; в нём живы
         `projectionWorker.ts`, `projectionFanout.ts`, `projectionOutbox.ts`), и он **продолжает писать**:
         94 строки за сутки, последняя 20.08 19:00. То есть «ноль производителей» верно для репозитория и
         НЕВЕРНО для работающего TEST. Порядок жёсткий: сначала выкатить D10-код на TEST, и только потом DROP.
         Перепись строк (её надо знать до сноса, чтобы не потерять неприменённое):
         DEV — 3759 `done` + 9 `cancelled`, ноль `pending`/`dead`, последняя 25.07;
         TEST — 4668 `done` + 9 `cancelled`, ноль `pending`/`dead`. Неприменённых событий нет ни там, ни там,
         поэтому сама таблица истории не держит — сносится как транспортный остаток, переносить нечего.
