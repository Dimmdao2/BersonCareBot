...
- [ ] **D15b — перенос идентичности, отдельными шагами.** Шаги обязательны по отдельности: утверждённая схема →
      ...
      **ОБЪЁМ, ИЗМЕНЁННЫЙ 20.08: шаги 1–6 ниже ПЛЮС D15b/7a — не отложено.** Владелец 03.08 исключал
      ...
      - [ ] **D15b/2 — D25: интегратор перестаёт писать идентичность.** **CURRENT 27.08:** продуктовый код
      ...
      - [ ] **D15b/6 — контакты в `user_contacts`.** **CURRENT 27.08:** physical cutover завершён:
      ...
      - [ ] **D15b/7 — разделение actor/subject.** **CURRENT 27.08:** реализация, композиционный аудит и
...
- [ ] **D17 — узкая роль в базе.** **CURRENT 27.08:** узкая роль, владельцы швов и все необходимые
...
- [ ] **D25 — идентичность: интегратору остаётся только доставка входа.** **CURRENT 27.08:** запрещённое
...
- [ ] **D30 — разворот архитектуры запуска по расписанию.** **CURRENT 27.08:** единая очередь, один
...
- [ ] **E2 — кнопка «отписаться от темы».** _Код приземлён 22.08 (`7e2943fc9`) плюс имя темы в тексте
      (`5d6d37327`, land `94bb29ce8`), галочка ждёт живой
      проверки на TEST. ⛔ **Живая проверка 23.08 УПЁРЛАСЬ в конфигурацию стенда, а не в код:** рассылка
      по почте отказывает с `clinic_delivery_channel_not_configured:clinic_smtp_outbound` — у клиники на
      TEST не подключён почтовый канал, а рассылки ходят только подключённым каналом клиники. Через
      Telegram проверка технически возможна, но `DEV_DELIVERY_REDIRECT=1` сведёт все 45 адресатов на
      телефон владельца одним залпом, поэтому ведущий её не запускал. **Решение владельца:** либо
      подключить почтовый канал клиники на TEST (нужны учётные данные SMTP), либо разрешить залп в
      Telegram. Форма отписки при этом проверена по коду и тестами: тема едет внутри подписанного
