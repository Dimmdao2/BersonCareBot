...
- [ ] **D18 — вычистить ВЕСЬ остаток сырого SQL, оба приложения (решение владельца 30.07: «по ходу плана надо вычистить
      ...
      **Порядок — по решению владельца 30.07:** перепись НЕ делается заранее («думаю это имеет смысл делать после
      ...
      - [x] **D18a — запрет на НОВЫЙ сырой SQL.** Механическая проверка запоминает нынешний список файлов с сырыми
            ...
            **DONE 2026-07-30:** `scripts/check-no-new-raw-sql.mjs`, подключённый к root и webapp lint, фиксирует manifest 17 integrator + 30 webapp файлов; временный `apps/integrator/src/rawSqlD18aFixture.ts:2` упал как новый SQL, а удаление живой manifest-строки — как тот же запрещённый путь. Доказательство: `pnpm lint`, `pnpm --filter @bersoncare/integrator lint`.
