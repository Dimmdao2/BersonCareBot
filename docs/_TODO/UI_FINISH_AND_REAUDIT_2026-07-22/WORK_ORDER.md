...
- [ ] **D20 — карта интегратора и настоящие тесты по модулям.** Решение и порядок — **Р-D20** (§2.3): сначала
      ...
      🟡 УСТАРЕЛО/ЗАМЕНЕНО → ~~**Настоящий дефект ОТКРЫТ:** путь ретрая входит в `enqueueDirectPublicWriteRetry`
      под принципалом `app_patient`, которому там не место. Лечится сменой принципала в коде, НЕ декларацией прав.~~
      ✅ **Перепроверено 21.08 против текущего кода — дефекта нет, живой принципал уже `app_integrator_request`.**
      `enqueueDirectPublicWriteRetry` (`apps/integrator/src/infra/db/repos/directPublicWriteRetry.ts:44`) не
      называет свой `functionIdentity`, поэтому голый `integrator`-принципал (`createDbIntegratorPrincipal`,
      `packages/db-principal/src/index.ts:435`), которым его оборачивает `runWithIntegratorPrincipal`, резолвится
      порт-контекстом (`integratorPortContextPrincipal`, `apps/integrator/src/infra/db/portContextRuntime.ts:188`,
      `defaultCapability = 'request'` на строке 198) в `targetRole = 'app_integrator_request'` — единственная
      роль, которую валидатор `assertPrincipal` (`packages/db-principal/src/portContext.ts:341-358`) разрешает
      для `contextClass: 'integrator'` с заданным `integratorUserId`/`organizationId`. Мапинг `integrator → app_patient`
      (`dbRuntimeRoleForPrincipal`, `packages/db-principal/src/index.ts:1108-1128`) существует только в старом
      режиме `legacy-guc/locked`; живые DEV и TEST работают в `port-context` (`docs/ARCHITECTURE/SERVER
      CONVENTIONS.md`, раздел «Webapp dev env: подтвержденные ключи» — `DB_PRINCIPAL_CONTEXT_MODE=port-context`
      для DEV, TEST webapp там же зафиксирован работающим в том же режиме), где эта ветка не исполняется. Текущая декларация прав это
      подтверждает буквально: единственный писатель `integrator.direct_public_write_retries` —
      `app_integrator_request` (`INSERT` на `operation, organization_id, idempotency_key, payload`); гранта
      `app_patient` там нет вовсе (`deploy/postgres/privileges/declaration.ts:6794-6804`) — ровно то, что и
      требует FORCE RLS-политика `rev10_context_gate_*`, упомянутая абзацем выше. D6 закрыт правильно и остаётся
      закрытым; открывать по нему новый скоуп не нужно.
