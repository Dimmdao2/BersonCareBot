...
> зелёные» сами по себе ≠ готово. Готово = галочки ниже + зелёный full CI + **живой скрин всех затронутых экранов** владельцу.
>
...
> отдельный **HTML-конверт письма** (layout с плейсхолдерами, правится как HTML-код) — это часть N1B, **не Tiptap**.
> Канон конверта: [`TOOLING_AND_PACKAGES_DECISIONS.md`](../ARCHITECTURE/TOOLING_AND_PACKAGES_DECISIONS.md)
> §«HTML-конверт транзакционных писем». `#931` и N1B-конверт — независимы и не пересекаются.
