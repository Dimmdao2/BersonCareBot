...
> **ПОПРАВКА 22.08 по итогам исполнения Ш1–Ш2 (замер, не мнение).** Слово «миграция» в Ш1 и Ш2 ниже
> НЕВЕРНО, и это не стилистика: `ALTER TABLE … ADD CONSTRAINT … PRIMARY KEY` строит индекс, то есть требует
> ОДНОВРЕМЕННО владения таблицей и `CREATE` на схеме `app_ext`. Эти права принадлежат разным ролям, членства
> между ними нет: владелец таблицы `app_seam_identity_lookup_owner` получает `42501 permission denied for
> schema app_ext` (на схеме у него только `USAGE`), `app_object_owner` (единственный с `UC`) — `must be owner
> of table`. Выдать недостающее право миграцией запрещено (AGENTS.md §1), а объявить семенному владельцу
> `CREATE` на `app_ext` — значит разрешить ему плодить в шве любые объекты. Оба шага исполнены идемпотентными
> DDL в `deploy/postgres/port-context/contract.sql` — авторитете рождения объектов `app_ext`, который сам
> снимает стену рождения на время своего прохода; приезжают шагом reconcile. Ни одна миграция репозитория
> никогда не создавала и не меняла объект `app_ext`. Шаги Ш3, Ш5, Ш7 и Ш9 ниже читать с той же поправкой:
> тела функций `app_ext.*` тоже живут в `contract.sql` и переписываются там, а не миграцией — reconcile
> проигрывает этот файл ПОСЛЕ наката миграций и перетёр бы любой `CREATE OR REPLACE` из миграции.
...
> **ДОБАВЛЕНО К Ш2 22.08: одна правка тела обязательна вместе с ключом, иначе Ш2 ломает вход.**
> `app_ext.resolve_variant_a_identity` вставляет с `ON CONFLICT (physical_user_id) DO NOTHING`, а
> `ON CONFLICT (<колонки>)` — это ВЫВОД ИНДЕКСА: после смены ключа спецификация, называющая один
> `physical_user_id`, не совпадает ни с одним индексом, и первое же разрешение НОВОГО человека умирает
> `42P10 there is no unique or exclusion constraint matching the ON CONFLICT specification`. Воспроизведено
> на именованной DEV в откаченной транзакции; тот же класс поломки, что убил почтовую дверь входа 22.08
> (`20260822T090000_the_email_contact_door_names_its_real_index`). Арбитр переведён на
> `(physical_user_id, ref_kind)` в `contract.sql`; вид не присваивается вставкой — он приходит из DEFAULT
> колонки, а вывод индекса от списка колонок INSERT не зависит. Сигнатура, владелец, `regprocedure` и
> возвращаемое значение прежние — это НЕ Ш3.
