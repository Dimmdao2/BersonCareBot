...
### Условие 1. Планировщик исполняет, но не решает

Обещанием «мы проследим» это условие не держится: история интегратора ровно так и произошла. **Действующая
приёмка — поведение живого планировщика/очереди и названные по правилу `AGENTS.md` §10a дорогие молчаливые
отказы** (двойная отправка, потерянное сообщение, решение бизнес-события — срок, текст, канал, ветвление по
тарифу — принятое внутри интегратора вместо вебаппа), доказанные живым прогоном (dev/TEST): строка задания,
записанная вебаппом, доходит до планировщика уже с готовым решением, и планировщик её не пересчитывает.
Форма исходника, число просканированных файлов или наличие/отсутствие импорта приёмкой не являются.

`schedulerDecisionGuard.ts` (Ш0.1, раздел «Шаги» ниже) — уже реализованный в коде анализатор по дереву
разбора TypeScript с самотестами; он существует как факт этого прохода и остаётся в дереве как защита от
отката (§10a, ступень 3), но не как критерий, которым проверяется исполнение этого условия дальше по плану.
...
4. **Политика перезапуска — операционный инвариант «ровно один живой executor на среду».** Проверяется живым
   свидетельством, а не текстом репозитория: `systemctl is-active`/`status` на целевом хосте показывает ровно
   один запущенный unit планировщика; захват `SCHEDULER_LOCK_KEY` подтверждён держателем через `pg_locks`
   (`objid`/`pg_backend_pid()`); за период наблюдения в `outgoing_delivery_queue` нет роста `dead`-строк или
   дублей `event_id`, которые выдали бы параллельную работу второго экземпляра. `deploySystemdSchedulerUnitGate.test.ts`
   (Ш0.5) остаётся в дереве как защита от отката над файлами репозитория, но не подменяет это живое
   доказательство — точный filename/count/текст unit-файла приёмкой не является.
...
- [ ] **Ш3. B2 — напоминания о задачах специалиста (первый и самый изолированный переезд).** Вебапп пишет
      ...
      **обязательной** в `deploy/HOST_DEPLOY_README.md` как cronport-задача и входит в пост-деплойный
      чек-лист — снятие обязано в том же коммите пройти точную последовательность: сначала именованная
      DEV/TEST gap-перепись выше и живой прогон create/update/complete/delete/доставки, затем точный
...
- [ ] **Ш5. B5b + B5c — сводка оператора и system-health-guard.** `digestTime` продолжает читаться из
      ...
      **CURRENT 21.08.2026 (D30 cron-retirement pass, code-side):** трассировка producer → scheduler → wake
      ...
      `internal/operator-health-digest/tick` и `internal/system-health-guard/tick` и их запись в
      `deploy/postgres/privileges/declaration.ts` — правка каталога БД-привилегий вне hard boundary этой
      задачи («Do not touch ... database privileges»). Их callsite-строка сегодня держится
      `deploy/postgres/privileges/port-context-callsite-catalog.test.mjs` (exact-match по тексту), но этот
      тест не authority для решения «снимать роут или нет» (`AGENTS.md` §10a — проверка формы исходника, не
      поведения) и сам по себе снятие не блокирует: он лишь синхронно правится тем же коммитом, когда роут
      уходит. Действующий порядок: после живого доказательства на TEST/PROD, что резидентных обращений к
      этим роутам больше нет (recipient probe, наблюдение за период), одним коммитом снимаются — внешний
      cron через `cronport`, мёртвый HTTP-роут и его строка в привилегийном каталоге вместе. Это не
      отдельный гейт сверх живого доказательства и не повод возвращать источник-текстовый тест как критерий
      ...
      **CURRENT 26.08.2026:** живые scheduler-tick на TEST подтверждены для
      `health.operator_health_digest.tick` и `health.system_health_guard.tick`; повторный `cronport list` не
      нашёл host cron этих задач. Старые Bearer-роуты удалены, их principal/privilege/CSRF записи сняты,
      `cronJobRegistry` теперь указывает на подписанные scheduler wake. Пункт остаётся `[ ]` до повторной
      проверки фактической постановки digest и guard-очистки после итогового объединённого TEST-деплоя.

- [x] **Ш6. B4 — снять внешний cron health-проб.** Работу продолжает делать `runScheduledOperatorHealthProbeTick`
      ...
      **CLOSED ON TEST 24.08.2026:** проверка на каноническом DEV/TEST-хосте `151.241.228.122` показала active
      `bersoncarebot-scheduler-test.service`, ровно один resident scheduler process и ноль совпадений
      `cronport list` для B4 health-probe. В `operator_health_probe_config` все существующие провайдерные пробы
      выключены (`google_calendar=false`, `max=false`, `telegram=false`), поэтому требование наблюдать движение
      `lastRunAt` каждой **включённой** пробы имеет пустое множество и включать реальный канал только ради
      доказательства нельзя. Последний сохранённый `health.outbound_probe.run` имеет `last_status=success`;
      конфигурация не менялась. Для текущего TEST-состояния снятие является доказанным no-op без внешнего cron.
...
1. ⚠️ **УСТАРЕЛО/ЗАМЕНЕНО 21.08.2026.** Список B1–B6 собран из `deploy/HOST_DEPLOY_README.md`, шаблонов
   `deploy/host/cron.d/*.template` и `cronJobRegistry.ts`. Историческая формулировка требовала «перед Ш3 —
   `cronport list` с обоих хостов от владельца или от того, у кого есть доступ» — это требование снято фактом:
   точный host-census уже выполнен на DEV/TEST-хосте (`151.241.228.122`, `hostname -> localhost`) командой
   `node /home/dev/brain/tools/cronport.mjs list` и не нашёл ни одной записи specialist-task,
   operator-health-digest, system-health-guard или operator-health-probe — только посторонние brain/backup/
   hygiene строки. Действующее требование теперь единственное и уже описано в самом Ш3 (см. выше): именованная
   DEV/TEST read-only gap-перепись и живой TEST-прогон create/update/complete/delete/доставки → повторный
   `cronport list` на текущем DEV/TEST-хосте → `cronport disable`/`remove` только НАЙДЕННОЙ записи; если записи
   нет — cleanup cron является no-op, и синхронно снимаются только реально мёртвые route/registry/docs хвосты
   там, где это уже предусмотрено шагом. `135.106.162.170` (PROD) в это требование не входит: доступа к PROD у
   меня нет и быть не должно, PROD не блокирует и не открывает Ш3 и остаётся отдельным будущим
   runtime-доказательством только после явного owner-разрешения (см. §«Развилки» выше).
