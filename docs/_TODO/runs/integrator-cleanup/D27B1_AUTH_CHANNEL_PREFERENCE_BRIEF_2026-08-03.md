...
> **ИСПРАВЛЕНО 03.08 (repo-lead).** Этот бриф в разделе «Сделать» п.1 прямо запрещал провенанс-дефолт
> «первый бот, подтвердивший номер» и требовал оставить автоматический фолбэк-лестницу
> Telegram → Max → email → SMS. Это было ошибкой: `IDENTITY_AND_MERGE_SCHEME.md` §3.1 требует ИМЕННО
> дефолт «бот, которым впервые подтвердили номер, либо почта того OAuth», без какой-либо лестницы —
> лестница прямо названа владельцем как то, что надо искоренить (§3.2, §3.3). Реализовано по §3, не по
> исходному тексту брифа ниже; см. `WORK_ORDER.md` D27 «CURRENT PARTIAL 03.08 (D27-B1)» за итоговое
> доказательство. Текст брифа ниже оставлен как есть для истории решения, не как действующая инструкция.
