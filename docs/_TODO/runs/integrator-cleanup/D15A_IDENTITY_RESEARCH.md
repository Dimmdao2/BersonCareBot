...
Это не отметка о завершении D15a/D15b и не разрешение выдавать интегратору узкую роль. Порядок неизменен:
**D15a → D15b → D17**. Продуктовые развилки, которые ставил раздел 6, владелец закрыл 31.07 — действующие
правила живут в `IDENTITY_AND_MERGE_SCHEME.md`, этот документ описывает, что интегратор делает СЕГОДНЯ и как
устроен перенос.
...
Семь продуктовых развилок (O1–O7) закрыл владелец 31.07: «семь развилок по идентичности — это не вопрос».
Правило по каждой живёт в `IDENTITY_AND_MERGE_SCHEME.md`, дословные слова — в
`OWNER_QUOTE_2026-07-31_IDENTITY.md`. O8 — инженерная, её закрывает правило плана 5.1.1.
⛔ Заново их не задавать.

| # | о чём была развилка | действующее правило |
|---|---|---|
| O1 | какой телефон из мессенджера даёт доверие | телефон, который мессенджер/OAuth однажды подтвердил, считается подтверждённым; ⛔ доверие к строке (Telegram-ссылка без подписи, MAX без подписи, признак «доверенный» без источника) искореняется — схема §1, пункт **D25** |
| O2 | когда разрешено автоматическое слияние | только когда у старого аккаунта нет НИКАКОЙ медицинской истории; иначе слияние делает только техподдержка — §5.2, §5.2a, пункт **D26** |
| O3 | переиспользованный номер и выбор целевого аккаунта | целевой — тот, из которого делается привязка (§5.3); отвязка номера снимает подтверждение (§1, пункт **D28**) |
| O4 | создаёт ли первый контакт в мессенджере зачисление | пациент появляется записью на приём или приглашением специалиста — §7 |
| O5 | существующие люди из мессенджеров без зачисления | ничего не делать — §8 |
| O6 | дефолт уведомлений после новой привязки канала | необязательные темы включены; кнопка «отписаться от темы» в рассылках — §9 |
| O7 | кто владеет `user_channel_bindings` | вебапп: «интегратору остаётся только доставка входа» — §10, пункт **D25** |
| O8 | поведение при недоступной команде идентичности вебаппа | ⛔ прямой fallback в `public` запрещён; durable-очередь с тем же ключом и повтор — правило 5.1.1 плана |
