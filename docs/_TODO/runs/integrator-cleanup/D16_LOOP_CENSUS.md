...
3. **A4a (`reminders.planDue`/`dispatchDue` для бот-каналов)** — не уходит без **D6**: чтение
   ...
   не сделана.
4. ~~**Судьба процесса `scheduler` целиком.**~~ **РЕШЕНО 20.08 (Р-D30а):** один резидентный процесс вместо
   `worker` + `scheduler`. Исполнение — шаг Ш9 в `D30_SCHEDULER_REVERSAL_PLAN.md`. Блокером быть перестало.
