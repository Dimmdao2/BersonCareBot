...
- The exact source census is 35 files / 121 top-level test declarations. The nine `RegExp.prototype.test` false
  matches are excluded. The matrix now claims only 2 exact static replacements and 10 same-consequence READY cells;
  it leaves 94 product/worker and 9 security/catalog consequences unproved and explicitly retires 6 non-product
...
- The 10 READY product consequences are not PASS until the shared live DEV audit releases the single server and the
  serialized runner records durable readbacks.
- The remaining 94 product/worker consequences require ordinary product/provider/worker state grouped in the matrix;
