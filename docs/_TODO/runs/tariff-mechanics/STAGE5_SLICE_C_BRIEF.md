# MISSION: stage 5, slice C — statistics with booking sources, proactive insights, booking prepayment
...
- **Plan:** `docs/_TODO/SAAS_FOUNDATION/TARIFF_MECHANICS_PLAN_2026-07-30.md` — items **5.5**, **5.6**, **5.8**; scope §1;
  policy §2.
...
- **5.5 `doctor_statistics`** — the specialist's statistics and analytics, and this ONE mechanic also covers booking-source
  (UTM) analytics: owner 30.07 «пусть будет в скопе аналитики для клиник». Do not create a second key for sources.
- **5.6 `proactive_insights`** — proactive prompts for the specialist.
- **5.8 `booking_prepayment`** — prepayment when booking. Cancellation rules get NO mechanic at all (owner: «правила
  отмены — нет») — if a handler mixes prepayment and cancellation settings, guard only the prepayment keys, exactly the
  way the shared settings endpoint already guards payment keys.
...
- Do not touch: the registry key list, migration `0275`, the seat chokepoint, the file write port, billing, the support
  ...
  (diaries, warmups, promo, external calendar) or slice B (clinical tests, online intake, specialist tasks).
