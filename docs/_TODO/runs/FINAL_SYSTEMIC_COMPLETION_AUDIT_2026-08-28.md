...
- **Последний зафиксированный green full `pnpm run ci`:** коммит `206be5478` (сам план это утверждает: «полный
  ...
  По критерию §9 это ровно тот случай, где полный CI даёт новый сигнал — не ритуал: пакет одновременно меняет
  общие модули, доступ к базе и системные ограничения. **Целевые гейты уже покрыли каждый коммит по отдельности**
