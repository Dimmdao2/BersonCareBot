...
## Bounded re-audit — 2026-08-02 (HEAD `5022cfa619437c8537ff8485939bcea83bf4784a`)

1. Saved acceptance suite → **PASS**: `pnpm --dir apps/webapp exec vitest run src/modules/saas-billing/service.test.ts src/modules/org-entitlements/service.test.ts src/app/api/payments/saasWebhook.route.test.ts src/app/api/admin/organizations/route.route.test.ts src/app/api/admin/commercial/route.route.test.ts src/app/api/admin/saas-billing/payments/route.route.test.ts src/app/app/admin/commercial/CommercialConstructorClient.ui.test.tsx src/app/api/clinic/billing/route.route.test.ts src/app/app/settings/BillingSection.ui.test.tsx src/app/app/settings/billingCommercialState.test.ts` → **10 files / 92 tests passed**. The additional audit test makes the settings control prove `PATCH` for the selected tariff and `DELETE` for cancellation.
2. PostgreSQL capture transaction → **PASS by inspection plus the saved replay oracle**: `captureSaasBillingPaymentSucceeded()` encloses provider-event insert, locked invoice transition, saved method, due-boundary `promotePaidInvoice()` (subscription snapshot/tariff/dates, organization projection, pending clear), and processed marker in one `getDrizzle().transaction`. Duplicate event insertion does not short-circuit the remaining transaction work, so replay completes it; a thrown write rolls the complete transaction back. Existing `service.test.ts` replay cases remain green in the suite above.
3. Clinic tariff route/UI → **PASS**: `GET` returns tariff choices/current pending boundary; `PATCH` schedules only a restrictive transition and `DELETE` cancels it through the scoped service. Scheduling preserves the active paid subscription period/snapshot, creates no provider intent, and renewal rechecks the selected target before intent creation. A self-service upgrade rejects with the stable `saas_billing_upgrade_charge_policy_unresolved` before any service-side write/provider path; no price was introduced.
4. Migration/journal → **PASS**: `git diff --quiet c23aa19aa..HEAD -- apps/webapp/db/drizzle-migrations/0307_tariff_change_paid_period_local.sql apps/webapp/db/drizzle-migrations/meta/_journal.json` → exit 0 (unchanged); `bash apps/webapp/scripts/check-drizzle-journal-sync.sh` → exit 0; journal inspection reports `0307 idx=307`, following `0306 idx=306`, with strictly increasing `when`.

Additional validation: `pnpm --dir apps/webapp typecheck` → exit 0; `pnpm --dir apps/webapp lint` → exit 0; `git diff --check` → exit 0. No migration, database, deploy, or temporary production-code fault injection was performed.

**Result: PASS. Remaining blocker:** the owner-money policy for clinic self-service upgrades is intentionally unresolved; the route refuses that transition and does not create a payment intent.
