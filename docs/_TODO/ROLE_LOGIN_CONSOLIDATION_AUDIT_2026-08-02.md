...
✅ **СДЕЛАНО 2026-08-03:** `b5f721462` восстановил отсутствовавший импорт; текущий
`apps/webapp/src/app/app/AppEntryLoginContent.tsx` больше не использует `buttonVariants`, поэтому finding
ниже закрыт удалением dev-bypass разметки. ⚠️ **ФАКТ УСТАРЕЛ 2026-08-23:** этот audit verdict относится к
снимку `3272c7006`, а не к HEAD; позднее вход дополнительно разделён на pre-session и человеческий контекст
(`apps/webapp/src/infra/db/portContextRuntime.ts`, migration `20260822T100000_…sql`).
...
~~FAIL~~ ✅ **СДЕЛАНО 2026-08-03, `b5f721462`.** Текущий файл не содержит ни usages, ни необходимости в
этом импорте; исторические строки и вывод typecheck ниже оставлены как evidence исходного дефекта.
...
~~`PASS` заблокирован одним build-breaking дефектом в `00608ed47`.~~ ✅ **СДЕЛАНО 2026-08-03:**
`b5f721462` закрыл ровно этот defect. ⚠️ **ФАКТ УСТАРЕЛ 2026-08-23:** итог ниже нельзя использовать как
вердикт для текущего входа после pre-session/human-session cutover.
