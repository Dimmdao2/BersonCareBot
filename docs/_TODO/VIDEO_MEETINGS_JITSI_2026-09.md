...
Owner-correction 08.09.2026 по live-интерфейсу: открытие страницы не создаёт и не начинает звонок само — специалист
явно нажимает «Начать звонок». Панель управления находится внизу видео; на мобильном переключение фронтальной и
...
показывается. На странице специалиста сохраняются канонические вкладки карточки пациента. App-wide плавающее видео,
системный iOS Picture-in-Picture, демонстрация экрана и дополнительные Jitsi-функции пока являются исследованными
возможностями, но не owner-authority на реализацию этой коррекции.
...
- [ ] **VM-06.** Интерфейс звонка не содержит брендинг Jitsi, техническое имя комнаты и конференционные функции.
      Нижняя панель содержит mute/unmute микрофона и камеры, завершение звонка и на мобильном отдельное прямое
      переключение фронтальной/задней камеры во время звонка; выбор устройств в настройках не заменяет эту кнопку.
...
- [ ] **UI-08.** Открытие doctor live-страницы не создаёт meeting session, не присоединяет Jitsi и не запрашивает
      камеру/микрофон. Только на doctor live-странице до нажатия вместо stage показывается «Начать звонок»; общий
      `VideoMeetingStage` и guest/patient live-страницы этой кнопки не получают и продолжают присоединяться по ссылке
      автоматически. Одно явное нажатие запускает существующий create/join path, а повторные быстрые нажатия не
      создают несколько сессий.
...
- автозапуск create/join/media при простом открытии live-страницы и duplicate create при двойном нажатии;
...
- `[live-only]` дублирование локального self-view до прихода клиента, техническое имя комнаты, верхняя панель
  управления и отсутствие прямого мобильного переключения фронтальной/задней камеры;
...
10. Живая проверка owner-correction на TEST с iPhone владельца через `awg1` и с desktop: открытие doctor live не
    создаёт сессию и не запрашивает камеру/микрофон; одно нажатие «Начать звонок» поднимает звонок, быстрое двойное —
    одну сессию; панель находится у нижнего края на обоих; мобильная кнопка прямо в звонке переключает фронтальную и
    заднюю камеры; до прихода клиента локальное изображение ровно одно; технического имени комнаты и брендинга Jitsi
    нет; канонические вкладки карточки видны и ведут туда же, что на подстранице приёма. Отдельно принудительный отказ
    `external_api.js` показывает retryable отказ, а «Повторить» действительно загружает bundle заново. Вкладки
    остаются обычной page navigation и не обещают сохранение звонка между маршрутами.
