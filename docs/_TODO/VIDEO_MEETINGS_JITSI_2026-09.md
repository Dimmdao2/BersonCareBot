...
Taskdb: **#1100**.
Статус: **выполнено и проверено на DEV/TEST. Интеграционная ветка и TEST содержат код
`5f97267db79d4404dc75c699f9d4863780fbddbd`; app deploy, Jitsi/coturn health, обычный direct-P2P звонок,
принудительный TURN fallback и серверный отказ третьему участнику прошли. Итоговое evidence:
...
- [x] **VM-06.** Интерфейс звонка не содержит брендинг Jitsi и конференционные функции; остаются видео, звук,
      mute/unmute камеры и микрофона и завершение звонка. Доказательство: blind UI audit/fix
      `docs/audit/video-meetings-full-surface-2026-09-08.md`, full CI `71ea8a3ca`.
