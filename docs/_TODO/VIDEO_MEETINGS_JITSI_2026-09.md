...
## 7a. Живая приёмка владельца 15.09.2026 — качество самого звонка

Владелец на живом звонке `berson.therapygo.ru/live` (Safari desktop + телефон): «во-первых, очень криво выглядит
у клиента; во-вторых, при копировании приглашения не появляется сообщение об этом; непонятно, что значит
„приглашение поставлено в очередь“ у специалиста при открытии страницы созвона и почему эта надпись не пропадает;
и можно ли у специалиста картинку me поудачнее расположить, и называть надо людей нормально там тоже».

Это приёмка уже принятого MVP: звонок соединяется, но выглядит и читается плохо. Отдельного нового скоупа здесь
нет, это доведение пунктов UI-01/UI-02/UI-10 и ACC-05/ACC-07 до состояния, в котором владелец их принимает.

- [x] **LIVE-01.** Клиентский экран звонка занимает весь экран. Причина дефекта измерена, а не угадана: гостевая
      и пациентская страницы давали контейнеру только `min-height`, а провайдер растягивает свой iframe на 100%
      высоты контейнера — без настоящей высоты звонок сжимался в полосу ~320px сверху чёрной страницы.
      Доказательство: живой звонок двух контекстов на `:5200` против TEST-Jitsi, замер iframe гостя
      1440x900 при окне 1440x900 и врача 760x817, снимок `/home/dev/shots-vm/guest2.png`; `0f1713a87`.
- [x] **LIVE-02.** Копирование гостевой ссылки даёт человеку видимое подтверждение («Ссылка скопирована»),
      как в остальных местах кабинета; текст берётся из общего словаря, а не пишется на месте.
      Доказательство: живой клик по кнопке на свежей встрече, снимок `/home/dev/shots-vm/fresh-copied.png`;
      ключ `commonLinkCopied`, `0f1713a87`.
- [x] **LIVE-03.** Вместо «Приглашение поставлено в очередь» специалист видит, что именно произошло с точки
      зрения человека: ссылка отправлена клиенту / уже отправлялась / отправить не удалось — скопируйте вручную.
      Сообщение живёт как подтверждение действия и само уходит, а не висит на экране всё время звонка.
      Доказательство: на живой свежей встрече видно «Ссылка на звонок отправлена клиенту» (тот же снимок),
      ключи `doctorVideoInviteSent`/`AlreadySent`/`NotSent`, таймер 12 с; `0f1713a87`.
- [x] **LIVE-04.** Участников зовут по именам: специалиста — его именем из карточки клиники, клиента — его ФИО.
      Имя выдаёт приложение внутри подписанного join-material, а не браузер участника, поэтому подменить его с
      клиента нельзя. Интерфейс звонка — русский, поэтому собственная плитка больше не подписана «me».
      Доказательство и ограничение: первая попытка читала имена под организационным принципалом и молча не
      работала — живой JWT приезжал без `name`, в логе «Missing declared webapp port capability:
      tenant_service» (этот класс ходит к данным только через именованные корни). Исправлено сменой
      принципала (`3c1a980f1`): имя специалиста читается кабинетным staff-принципалом, имя клиента —
      пациентской дверью `getCurrentPatientFio`. Замер после правки: JWT врача `"name":"Дмитрий Берсон"`,
      JWT гостя `"name":"Клиент"`; на экране клиента большой участник подписан «Дмитрий Берсон».
      По ГОСТЕВОЙ ссылке участник подписан словом «Гость»: сессии человека там нет вовсе и законного
      чтения его ПДн не существует. Настоящее ФИО потребовало бы новой колонки у встречи, правки
      definer-двери обмена (объявленное свойство которой — «не отдавать данные пациента») и объявления
      прав. Владелец 15.09.2026 эту работу закрыл: «давай просто Гость тогда писать… не надо
      заморачиваться».
- [x] **LIVE-05.** Собственная плитка специалиста расположена так, что не выглядит случайной. Отдельной
      настройки «self-view в углу» провайдер не даёт (в конфиге есть только `filmstrip.*` и полное
      `disableSelfView`), поэтому плитка осталась штатной плёнкой — но после LIVE-01 и LIVE-04 она стала
      подписанной миниатюрой в углу полноэкранного звонка вместо безымянного «me» на сжатой полосе; видно
      на `/home/dev/shots-vm/doctor2.png` и `guest2.png`. Если владельцу этого мало, следующий шаг —
      собственная раскладка поверх провайдера, а не его настройка.
