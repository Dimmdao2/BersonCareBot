...
- [ ] **R3 — пациентские прямые записи в обход именованной функции.**
  ...
  `app/app/patient/profile/DiaryDataPurgeSection.tsx:116`). Тот же класс, меньше поверхность:
  `pgLfkDiary.ts:437-456` (`updateLfkComplexExerciseLocalCommentForUser`) — безусловный сырой
  `UPDATE lfk_complex_exercises`. У `app_patient` на этих отношениях по каталогу DEV только `SELECT` либо
  вообще ничего, а все соседние записи того же модуля идут через `app.*` SECURITY DEFINER. Поведенческого
...
- [ ] **S1. Колоночный `INSERT`-грант выводится из типизированной схемы, а не пишется руками.**
  ...
  *Конструкция:* в `relation-access.ts` для роли с колоночным `INSERT` хранится ссылка на Drizzle-экспорт;
  генератор раскрывает `getTableColumns()` в полный список, вычитая `generatedAlwaysAs`. Новая колонка едет в
  грант той же миграцией, что и в схему. Расширяется ровно `INSERT`; `SELECT/UPDATE/DELETE` не трогаются.
  Второй ручной авторитет не заводится — обе стороны выводятся из уже существующих (декларация + метаданные ORM).
  *Оракул приёмки:* `pnpm test:db-privileges` и `generate-cli.mjs --check` побайтово; отдельно — счётчик
  «сколько пар (роль, таблица) с колоночным `INSERT` и живым `.insert()`-callsite», напечатанный самим гейтом;
  это и есть настоящее число вместо оценок 31/30/78.
  ...
  *Аудитор (§24.4):* смешанный этап. Полнота грантов — **повторяемое поведение**, первый независимый аудитор
  обязан принести **падающий** тест на исходной реализации (сегодня он обязан краснеть минимум на
  `be_patient_package_items`) и один раз показать fault injection: убрать колонку из декларации → красный,
  вернуть → зелёный. Замена рукописного списка на производный — **разовое действие**, проверяется чтением diff
  и генератором, тест на отсутствие списка не пишется.
- [ ] **S2. Один принципал на шве платежей.**
  ...
  *Конструкция:* перевести `pgPayments.ts`, `pgPatientPayments.ts`, `pgPaymentCaptureUnitOfWork.ts` на
  именованные корни — ровно так, как это уже сделано для delivery-targets и public-booking; либо, если
  реляционный доступ здесь обоснован, объявить недостающую возможность порта явно. Выбор из двух — решение
  исполнителя по месту, но не «оба». Заодно снять неверный комментарий `pgSaasBilling.ts:295`.
...
- [ ] **S3. Пациентские записи — только через именованную дверь.**
  ...
  *Семьи действий:* очистка дневника целиком; локальный комментарий к упражнению.
  *Конструкция:* сначала **falsification** — один rollback-only прогон обоих путей под настоящим
  `bcb_dev_webapp_patient`, чтобы установить `42501` против скрытого обхода. Только по результату:
  заменить оба сырых стейтмента на `app.*` SECURITY DEFINER-функции с `app.require_accepted_context(...)`
  первым оператором и объявить их в декларации — та же форма, что у всех соседей в
  `pgLfkDiary.ts`/`pgSymptomDiary.ts`. Новый шаблон не изобретается.
  *Оракул приёмки:* `patient-diary-purge` rollback-only proof под пациентским логином; закрывает оба пути.
...
- [ ] **S4. Одна безопасная дверь ошибки на API и на React, плюс механический гейт от рецидива.**
  ...
  *Конструкция:* не новый механизм, а обязательное принятие уже существующего и правильно спроектированного
  ...
  в серверный лог, а пользователю уходит безопасный идентификатор корреляции. Убрать
...
### Owner-gates этой волны (одним листом, до старта зависимых этапов)

- S2: перевести платежи на именованные корни (рекомендация, совпадает с уже принятой формой для
  delivery-targets/public-booking) **или** объявить webapp-возможность `tenant_service` с `purpose: 'relation'`.
  Safe-default при молчании — именованные корни.
- S4: формат безопасного идентификатора для пользователя (короткий correlation id в ответе и в серверном логе).
  Safe-default — уже существующий `error.digest` как единственное, что видит пользователь.
- S5: допустимо ли выполнять `package:sell-manual` и `payment:cash` на TEST под owner-учётками с последующей
  уборкой. Safe-default — да, с обязательным шагом восстановления, как у существующего
  `task:cleanup-acceptance`.
