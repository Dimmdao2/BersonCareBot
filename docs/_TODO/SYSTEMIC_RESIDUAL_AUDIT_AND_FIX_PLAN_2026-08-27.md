...
## Волна 03.09.2026 — системная коррекция доступа, границы ошибок и правдивой приёмки (ТЕКУЩАЯ ТОЧКА ВХОДА)

Этот раздел заменяет вывод раздела «Финальная автономная приёмка 02.09.2026» о закрытии автономной части
Track D. Прежние evidence-разделы ниже остаются историей и не переписываются, но их вывод больше не является
действующим статусом. Основание — прямое указание владельца 03.09: сначала свести все находки в один документ,
затем разобрать их и выпустить системный план коррекции по общим причинам, а не чинить отдельные поломки.

Источники этой волны: четыре read-only расследования 03.09 (`/tmp/package-privilege-audit-opus-20260903.log`,
`/tmp/doctor-actions-access-opus-20260903.log`, `/tmp/patient-admin-actions-sonnet-20260903.log`,
`/tmp/api-error-boundary-sonnet-20260903.log`) и артефакты живой приёмки 02.09 в
`runs/test-interactive-acceptance/out/`. Каждое утверждение ниже сверено по текущему дереву заново; предложение
из отчёта доказательством не считалось.

### Коррекция действующего статуса

Формулировка «Автономная часть Track D закрыта» не подтверждается процитированными в ней же артефактами:

- `flows-2026-09-02T10-04-00.034Z.json` — 9 шагов, все `pass: true`, но это **только врач** и только девять
  действий; слова `package` в файле нет ни разу.
- `role-mutations-2026-09-02T09-08-35.175Z.json` — 5 шагов, из них **два `pass: false`**:
  `doctor:working-schedule-change-restore` (`error: "lossless_schedule_row_absent"`) и `doctor:visit-create`
  (`404 Страница не найдена` на `/app/doctor/patients/9278b8ef-…`, затем таймаут ожидания кнопки «Новый визит»).
  Эти два отказа в разделе о закрытии не упомянуты.
- `crawl-2026-09-02T08-41-52.810Z.json` — doctor 72, patient 55, global_admin 21, clinic_admin 8 маршрутов,
  0 ошибок. Это **открытие страниц**, а не выполненное действие.
- **У роли clinic admin во всех артефактах 02.09 нет ни одной выполненной мутации** — только восемь открытых
  страниц.
- Утверждение «в журналах нет `42501`» верно для окна того деплоя, но продажа абонемента в этом окне не
  выполнялась ни разу, а 03.09 она дала `42501` живьём на TEST.

Действующий статус: **Track D не закрыт**. Закрыты этапы 1–6 и часть этапа 7; открыты R1–R5 ниже и правдивая
матрица приёмки.

### Принятые находки этой волны, сгруппированные по общей причине

- [ ] **R1 — Drizzle называет в `INSERT` все колонки, а колоночный грант собран из «полей бизнес-логики».**
  Человеческое последствие: врач нажимает «продать абонемент», получает ошибку, абонемент не создаётся;
  тот же класс ломает ещё 16 подтверждённых действий врача и админа клиники. Drizzle 0.45.2 печатает список
  всех колонок таблицы и подставляет `DEFAULT` незаданным; PostgreSQL требует право на каждую названную
  колонку. Проверено заново по текущему дереву (brace-matched разбор `REV10_CLINICAL_ACCESS` в
  `deploy/postgres/privileges/relation-access.ts`): 156 объявленных отношений, из них **145** имеют у
  `app_staff` колоночный `INSERT`, и у **78** из них в списке колонок нет `id`. Это верхняя граница кандидатов
  по стороне декларации — сколько из них реально достижимы `db.insert()` под `app_staff`, обязан посчитать
  механический гейт этапа S1, а не оценка агента (два отчёта 03.09 дали здесь разные числа, 31 и 30 из 87;
  ни одно из них не воспроизведено и в план не переносится).
  Поимённо подтверждено `id=MISSING` у всех таблиц, названных расследованиями:
  `be_patient_package_items`, `be_package_history_events`, `be_package_usages`, `be_payment_intents`,
  `be_subscription_packages`, `be_package_items`, `be_patient_booking_profiles`, `patient_comorbidity`,
  `organization_member_invites`, `program_item_discussion_messages`, `be_booking_form_submissions`,
  `be_cancellation_policies`, `be_reschedule_policies`, `be_availability_rules`,
  `content_section_slug_history`, `patient_home_block_items`, `patient_merge_candidates`.
  Живое доказательство отказа: TEST, `postgresql-16-main.log`, `42501 permission denied for table
  be_patient_package_items` на `insert into "be_patient_package_items" ("id", …) values (default,$1,…)`.
  Синхронная цепочка продажи не одна транзакция: родитель и позиции идут в одной, а история
  (`be_package_history_events`) и активация — в отдельных. Поэтому починка **только** позиций даст худший
  исход, чем сегодня: абонемент с позициями закоммитится, история и активация упадут, врач увидит ошибку, а в
  базе останется полусозданный абонемент. Чинить обязательно всю цепочку сразу.
  Рецидив четвёртый: `456170936`, `f9cc3a2ce`, `c07ef8b2d` — каждый раз чинили одну таблицу.
- [ ] **R2 — на шве платежей объявленный принципал не совпадает с рантайм-принципалом.**
  Человеческое последствие: врач принимает оплату (наличные, ссылка, предоплата, возврат) — действие падает
  в приложении **до базы**. `runWithDbOrganizationPrincipal` (`pgPayments.ts:158`, `pgPatientPayments.ts:53`,
  `pgPaymentCaptureUnitOfWork.ts:16,22`) даёт принципала `kind: 'organization'`; в живом режиме
  `port-context` `apps/webapp/src/infra/db/portContextRuntime.ts:334-335` отображает его на класс
  `tenant_service` и ищет возможность порта с именем `tenant_service`. Проверено по
  `deploy/postgres/privileges/declaration.ts`: единственная возможность с `contextClass: 'tenant_service'` и
  `purpose: 'relation'` объявлена на строке 2590 и принадлежит порту **integrator**, не webapp. Значит
  обычный реляционный стейтмент под организационным принципалом гарантированно бросает
  `Missing declared webapp port capability: tenant_service`.
  Два отчёта 03.09 здесь противоречили друг другу: package-аудит цитировал `dbRuntimeRoleForPrincipal`
  (`packages/db-principal/src/index.ts:1110-1120`, `organization → app_staff`), doctor-аудит — класс
  `tenant_service`. Противоречие разрешено: обе ветки существуют, но первая относится к снятому signed-режиму,
  а живой режим закреплён на `port-context`. Действует вторая. Комментарий `pgSaasBilling.ts:295`, прямо
  утверждающий «`runWithDbOrganizationPrincipal` → `SET ROLE app_staff`», описывает снятый режим и вводит в
  заблуждение.
- [ ] **R3 — пациентские прямые записи в обход именованной функции.**
  Человеческое последствие: пациент нажимает «удалить все мои дневниковые данные» — самообслуживаемое
  удаление данных либо не работает вовсе, либо никогда не проверялось под настоящим пациентским логином.
  `apps/webapp/src/infra/repos/pgDiaryPurge.ts:12-49` выполняет сырые `UPDATE lfk_complexes`,
  `UPDATE patient_lfk_assignments`, `DELETE symptom_trackings`, `DELETE lfk_complexes`; вызывается из
  `buildAppDeps.ts:1889` по `POST /api/patient/diary/purge` (живой контрол —
  `app/app/patient/profile/DiaryDataPurgeSection.tsx:116`). У `app_patient` на этих отношениях по каталогу DEV
  только `SELECT` либо вообще ничего, а все соседние пациентские записи того же модуля идут через `app.*`
  SECURITY DEFINER. Поведенческого
  прогона под `bcb_dev_webapp_patient` нет ни одного — это первое, что обязан установить S3: живой `42501`
  или скрытый обход, который трассировка не увидела.
- [ ] **R4 — нет общей безопасной двери ошибки, поэтому внутренний текст доходит до UI.**
  Человеческое последствие: врач в браузере видит `permission denied for table be_patient_package_items` —
  имя внутреннего отношения наружу; пациент и неавторизованный посетитель — тот же класс.
  `apps/webapp/src/app/api/booking-engine/patientPackagesRouteShared.ts:49-53` (`membershipErrorResponse`)
  берёт `err.message`, не находит его в `ERROR_STATUS` и отдаёт его же телом ответа со статусом 500.
  Замерено заново по текущему дереву: `route.ts` всего **454**; безопасный маппер
  `shared/http/apiResponse.ts` импортируют **14**; прямое обращение к `error.message`/`err.message` — в
  **49** `route.ts`. Второй, независимый транспорт той же утечки — React: `app/global-error.tsx:30,51` и
  `shared/ui/{doctor,patient}/SegmentRouteError.tsx:71,89` рендерят `error.message` и `error.digest`
  напрямую. Утечка не зависит от окружения — течёт одинаково в DEV/TEST/PROD; появилась в `696297cfb`
  (04.06), то есть задолго до вчерашних правок, и ими не создана.
  Компенсации в логах нет: `infra/logging/logger.ts` (`serializeError`) по умолчанию выбрасывает
  `.message`/`.stack`, оставляя `type` и pg-код, а падения 500 03.09 в `journalctl` webapp-test вообще нет —
  единственный след остался в журнале PostgreSQL. То есть сегодня внутренняя деталь уходит пользователю и при
  этом **не** сохраняется оператору — это чистая потеря, а не размен.
- [ ] **R5 — матрица живой приёмки объявляет больше, чем доказывает.**
  Человеческое последствие: разрывы R1–R3 доживают до владельца, потому что «зелёная приёмка» смешивает
  открытую страницу, unit-тест и выполненное действие в одно слово «PASS». Корреляция полная и она же
  приговор методу: **всё, что прогнано живьём, имеет полные гранты; все сломанные — из действий, которых в
  живом прогоне нет.** Ручной контракт полноты колонок в
  `deploy/postgres/privileges/relation-access.test.mjs:66-85` состоит ровно из трёх записей —
  `be_patient_packages`, `media_files`, `patient_invites`, — и проверено, что ровно у этих трёх и только у них
  `id` в гранте есть. Тест зелёный при сломанном продукте по построению: он перебирает не таблицы, а
  рукописный список. `access-census.mjs` сверяет присутствие имени таблицы, не операцию и не колонки.
  `*.devDbProof.test.mjs` в обычном CI не исполняются.

### Разобрано и НЕ принято как отдельная работа

- Расширение прав до табличного `INSERT` вместо колоночного — не требуется: S1 расширяет ровно `INSERT`, где
  список колонок выбирает ORM, а `SELECT/UPDATE/DELETE` остаются точечными.
- `F5` из patient/admin-отчёта (в `pgSaasBilling.ts` нет `assertPlatformOperationsPrincipal`) — сегодня
  недостижимо: каждый вызывающий проходит `requirePlatformOperationsApiContext()` на слое маршрута.
  Это укрепление на будущее, владелец его не просил. Не заводится.
- `F6` (аудит-строка операторских инцидентов пишется вне транзакции основной мутации) — гипотетическая потеря
  compliance-записи при падении между двумя записями. Реального последствия для пользователя нет. Не заводится.
- `F4` (`app_platform_settings` имеет прямой DML на `saas_tariffs`, `system_settings` и др.) — это объявленное
  намеренное исключение, совпадающее с `relation-access.ts`, а не дрейф. Не дефект.
- «Отсутствующие продуктовые возможности» — глобальное управление пользователями/ролями, создание и
  редактирование организации глобальным админом, пациентская самопокупка абонемента, «позже/пропустить» на
  пункте программы, редактирование адреса: живого контрола нет вовсе. Это не дефекты этой волны и не скоуп;
  если они нужны — это отдельное решение владельца, а не находка аудита (§24.6, запрет аудит-разгона).
- Стилевые и «на всякий случай» рекомендации отчётов (ESLint-правило как самостоятельная цель, переписывание
  `serializeError` шире нужного, удаление `patient-package-staff-insert.devDbProof.test.mjs` до появления
  производного гейта) — в скоуп не берутся; удаление этого теста допускается только как последний шаг S1,
  когда его заменит производный контракт.
- `F3` (миграция `0050` с `app.set_platform_organization_is_active` якобы не применена на `bcb_webapp_dev`) —
  взято не как дефект, а как **разовая проверка** внутри S5: подтверждается интроспекцией каталога, и только
  подтвердившись, становится работой.

### Правдивая матрица приёмки на 03.09.2026

Уровни доказательства названы раздельно и не суммируются: **О** — страница открыта/прочитана; **Ю** — unit или
rollback-only proof; **В** — действие выполнено живьём; **П** — объект переоткрыт и изменение прочитано обратно;
**У** — состояние убрано/восстановлено после проверки.

| Роль | О | Ю | В | П | У | Чем доказано и что открыто |
|---|---|---|---|---|---|---|
| Врач | 72 маршрута | да | 9 flow + 17+2 действий | частично | да (`task:cleanup-acceptance`) | `flows-…10-04-00`, `actions-…08-43-24`, `actions-…08-47-51`. **Не выполнено ни разу:** продажа/предложение абонемента, списание сессии, платёж, сопутствующее заболевание, комментарий к пункту программы, пометка «проблемный». **Два отказа не устранены:** `doctor:working-schedule-change-restore`, `doctor:visit-create` |
| Админ клиники | 8 маршрутов | нет | **0** | **0** | — | Только `crawl-…08-41-52`. Ни одной выполненной мутации; приглашение сотрудника (`organization_member_invites`) не проверялось и попадает под R1 |
| Глобальный админ | 21 маршрут | частично | 1 (`trial-policy-change-restore`) | да | да | `role-mutations-…09-08-35`. Две живые попытки 23.08 закончились отказом (`500 MechanicWriteClearanceRequiredError`; `500 TEST ENV LOCK … dev_mode is locked`) и с тех пор не переподтверждены |
| Пациент | 55 маршрутов | да | 2 (`program-reminder-toggle-restore`, `chat-send`) | да | да | `role-mutations-…09-08-35`. Перенос записи, дневник, очистка дневника, взаимодействие с программой, поддержка — живьём не выполнялись |

### Системный план исправления

Порядок обязателен: общая конструкция раньше точечных правок, иначе следующий класс придёт пятым рецидивом.

- [ ] **S1. Колоночный `INSERT`-грант выводится из типизированной схемы, а не пишется руками.**
  *Последствие:* восстанавливает продажу абонемента и остальные записи `app_staff` и физически закрывает
  класс от повторения.
  *Семьи действий:* вся синхронная цепочка продажи (родитель, позиции, история, активация, платёжное
  намерение), списание/возврат сессии, каталог абонементов клиники, приглашение сотрудника, сопутствующие
  заболевания, формы записи, политики отмены/переноса, буфер приёмов, обсуждение пункта программы,
  slug-история CMS, блоки главной пациента, кандидаты на слияние, пометки пациента.
  *Конструкция:* генератор прав не импортирует TypeScript Drizzle-схему напрямую: он обязан оставаться чистым
  Node-модулем. Webapp-workspace производит из живых метаданных Drizzle машинный закоммиченный артефакт
  «таблица → колонки, которые ORM называет в `INSERT`», исключая только `generated.type !== 'byDefault'`;
  byte-equal `--check` не даёт артефакту отстать от схемы. Декларация/генератор прав читает этот артефакт как
  данные и расширяет колоночный `INSERT` только для отношений с достижимым прямым `.insert()`-callsite и
  webapp relation-capability. Отношения без Drizzle-модели (`broadcast_drafts`, `system_settings_audit`) остаются
  явными исключениями; лишняя `platform_users.session_epoch`, которая нужна только для `UPDATE`, не переносится
  в `INSERT` побочным эффектом. Расширяется ровно `INSERT`; `SELECT/UPDATE/DELETE` не трогаются. Миграции права
  не выдают: изменение схемы обязано в том же code-change перегенерировать метаданные и declaration-owned SQL.
  *Оракул приёмки:* первый аудитор уже добавил производный AST/import-graph гейт без ручного списка. Команда
  `pnpm test:db-privileges` на исходной реализации печатает 127 достижимых пар на каждую управляемую базу и
  44 неполных пары в каждой, затем краснеет первой строкой на `be_patient_package_items.id`. После исправления
  тот же гейт и `generate-cli.mjs --check` обязаны быть зелёными и побайтовыми; неразрешённый callsite обязан
  ронять гейт, а не выпадать из переписи.
  *Точечные проверки:* `--census` по обеим управляемым базам; DEV rollback-only прогон продакшн-списка колонок
  с `DEFAULT` для всей цепочки продажи.
  *Аудитор (§24.4):* первый независимый проход `d4f9ab478` выполнен: K1–K5 пойманы, временные инъекции откатились,
  продукт не менялся. Worker доводит тот же красный гейт до зелёного. Замена рукописного списка на производный
  артефакт — **разовое действие**, проверяется чтением diff и генератором, тест на отсутствие списка не пишется.
- [ ] **S2. Один принципал на шве платежей.**
  *Последствие:* возвращает врачу приём оплаты наличными и по ссылке, предоплату, возврат и платёжное
  намерение при продаже «на оплату».
  *Семьи действий:* `POST /api/doctor/booking-engine/appointments/[id]/payment` и соседние платёжные маршруты.
  *Конструкция:* перевести `pgPayments.ts`, `pgPatientPayments.ts`, `pgPaymentCaptureUnitOfWork.ts` на
  именованные корни — ровно так, как это уже сделано для delivery-targets и public-booking. Новую
  webapp-возможность `tenant_service` для прямого реляционного доступа не объявлять: она создаст второй путь
  рядом с принятой именованной дверью. Если существующая граница именованного корня объективно не может нести
  платёжную транзакцию, исполнитель останавливает этап с доказательством, а не выбирает второй паттерн сам.
  Заодно снять неверный комментарий `pgSaasBilling.ts:295`.
  Механический запрет: `runWithDbOrganizationPrincipal` вокруг реляционного доступа в webapp не должен
  собираться молча.
  *Оракул приёмки:* один тест — webapp-принципал `organization` + реляционный стейтмент обязан отказать
  заявленным сообщением, а не уйти в другую роль незаметно.
  *Точечные проверки:* webapp typecheck; unit-тесты платёжных портов.
  *Аудитор:* **повторяемое поведение**, нужен падающий тест на исходной реализации.
- [ ] **S3. Пациентские записи — только через именованную дверь.**
  *Последствие:* удаление пациентом своих дневниковых данных начинает работать (или впервые получает
  доказательство, что работает).
  *Семьи действий:* очистка дневника целиком.
  *Конструкция:* сначала **falsification** — один rollback-only прогон пути под настоящим
  `bcb_dev_webapp_patient`, чтобы установить `42501` против скрытого обхода. Только по результату:
  заменить сырые стейтменты очистки на один существующий либо расширенный `app.*` SECURITY DEFINER-корень с
  `app.require_accepted_context(...)`
  первым оператором и объявить его в декларации — та же форма, что у всех соседей в
  `pgLfkDiary.ts`/`pgSymptomDiary.ts`. Новый шаблон не изобретается.
  *Оракул приёмки:* `patient-diary-purge` rollback-only proof под пациентским логином.
  *Точечные проверки:* существующий route-тест `api/patient/diary/purge/route.route.test.ts`.
  *Аудитор:* falsification-прогон — **разовое действие** (интроспекция + одноразовый runtime-check);
  итоговая дверь — **повторяемое поведение** с падающим тестом.
- [ ] **S4. Одна безопасная дверь ошибки на API и на React, плюс механический гейт от рецидива.**
  *Последствие:* пользователь перестаёт видеть имена таблиц, тексты запросов и параметры; оператор
  начинает видеть то, что сегодня теряется.
  *Семьи действий:* 49 `route.ts` с прямым `error.message` (включая неавторизованные `booking/public/slots`
  и `booking/public/form-fields`), 11 server actions, `global-error.tsx`, оба `SegmentRouteError.tsx`.
  *Конструкция:* не новый механизм, а обязательное принятие уже существующего и правильно спроектированного
  `mapApiError`/`jsonError` (`shared/http/apiResponse.ts`, closed allowlist + безопасный fallback). Стабильные
  доменные коды (`catalog_not_found`, `payments_disabled` и т.д.) обязаны остаться различимыми — это
  требование, а не побочный эффект. Параллельно `serializeError` сохраняет машинный контекст (SQLSTATE, класс)
  в серверный лог, а пользователю уходит безопасный идентификатор корреляции. Для API нельзя переиспользовать
  React-only `error.digest`: сначала найти существующий request/correlation id в HTTP-слое; если его нет —
  генерировать один идентификатор в общей двери ответа и передавать тот же идентификатор в серверный лог и
  безопасное JSON-тело. Убрать
  `NODE_ENV === 'development' ? { detail: error.message }` из `booking-engine/working-hours/route.ts:140,194`:
  это обычный UI-ответ врачу, а не операторская поверхность.
  *Оракул приёмки:* один unit-тест на самом общем маппере с fault injection — ошибка вида
  `Object.assign(new Error('insert into be_patient_package_items …'), { code: '42501' })` через мок порта;
  `expect(JSON.stringify(body)).not.toMatch(/insert into|be_patient_package_items|SELECT|VALUES/i)` плюс
  контроль, что известный доменный код по-прежнему различим отдельно. Один тест на общей точке закрывает все
  семьи — по §10b это дешевле, чем route-level тест на каждый из 454 файлов.
  *Механический гейт:* AST/ESLint-правило, запрещающее литеральный `error.message`/`err.message` внутри
  `NextResponse.json(...)` в `app/api/**/route.ts` и в `error.tsx`-компонентах. Без него правка деградирует
  обратно — это и есть «третья ступень» §10a.
  *Аудитор:* маппер и гейт — **повторяемое поведение** (падающий тест + одна fault injection на класс);
  массовое переключение 49 маршрутов на общую дверь — **разовое действие**, проверяется `rg` и чтением diff,
  тест «на отсутствие строки» не пишется.
- [ ] **S5. Правдивая живая матрица приёмки по ролям.**
  *Последствие:* «зелёная приёмка» перестаёт означать «страница открылась».
  *Семьи действий:* врач, админ клиники, глобальный админ, пациент.
  *Конструкция:* в `runs/test-interactive-acceptance/flows.mjs` добавить ровно те действия, которых там нет и
  которые сломаны: `package:sell-manual` с переоткрытием карточки, `package:consume`, `payment:cash`,
  `comorbidity:add`, `clinic:invite-member`, `patient:mark-problematic`, `program-item:comment`; для админа
  клиники — первую в истории выполненную мутацию; для пациента — `booking:reschedule` и `diary:purge`.
  Каждый шаг фиксирует пять состояний раздельно (открыто / unit / выполнено / переоткрыто / убрано), а
  сводка запрещает складывать их в одно «PASS». Внутри этапа — разовые проверки: применена ли миграция `0050`
  на `bcb_webapp_dev`; воспроизводятся ли два отказа 23.08 у глобального админа.
  *Оракул приёмки:* `runStep` уже помечает FAIL при любом ответе ≥400; шаг `package:sell-manual` одной
  проверкой проходит всю цепочку R1 и одновременно доказывает безопасную ошибку S4, потому что смотрит на
  статус, а не на текст исходника.
  *Точечные проверки:* устранение двух незакрытых отказов `doctor:working-schedule-change-restore` и
  `doctor:visit-create` — с разбором, дефект это продукта или устаревшая фикстура прогона.
  *Аудитор:* **разовое действие** по составу матрицы (чтение артефактов и сверка уровней) и **повторяемое
  поведение** по самим шагам; здесь доказательством служит живой прогон, а не новый тест.
- [ ] **S6. Синхронизация документов и taskdb по факту.**
  *Последствие:* следующий агент не наследует ложный статус.
  *Конструкция:* после принятой владельцем живой приёмки — привести этот план, Track D и taskdb к
  фактическому состоянию. Архивные evidence-разделы не переписывать.

### Порядок выполнения

1. **Общая конструкция сначала, одной веткой:** S1 (производный грант) и S4 (общая дверь ошибки + гейт). Оба
   меняют разделяемые точки, обе правки конфликтны по файлам — вести последовательно, не параллельно.
2. **Затем ограниченные ветки-воркеры на независимом file-scope, параллельно (≤3, каждая в своём worktree):**
   S2 (платёжный шов), S3 (пациентские двери), S5 (матрица приёмки). Их файлы не пересекаются.
3. **Независимый аудит и приёмка каждого куска против ЭТОГО плана**, не против задач исполнителя (§24.5).
4. **Приземление** накопленного пакета.
5. **DEV reconcile и живая проверка ролей** после приземления.
6. **Один деплой на TEST** штатным `deploy/host/deploy-test.sh`.
7. **Проход по ролям на TEST** по матрице S5, с сохранением артефактов и журналов.
8. **S6** — закрытие документов и taskdb.

**Полный CI — один раз, на шаге 4, а не после каждой мелкой правки.** Единственный накопленный
интеграционный риск, который его оправдывает: S1 меняет генератор привилегий и сгенерированные SQL-артефакты
для всех управляемых баз, а S4 — общий HTTP-контракт ошибки, на который опираются route-тесты всех семей.
Пересечение этих двух изменений в одном пакете не покрывается ни одним targeted-гейтом по отдельности. Правки
S2/S3/S5 своих полных CI не требуют: у них есть собственные targeted-оракулы и живой прогон.

### Решения владельца этой волны уже приняты

- S2 идёт через именованные корни; второй реляционный путь не заводится.
- S4 возвращает пользователю короткий безопасный correlation id, общий с серверным логом; внутренний текст
  ошибки остаётся только оператору.
- S5 выполняет `package:sell-manual` и `payment:cash` на TEST под owner/test-учётками с обязательной уборкой.
  Отдельного owner-gate перед стартом этих этапов нет.

## Волна финальной репетиции 02.09.2026 — единый реестр новых находок

Этот раздел был точкой входа до 03.09; текущая точка входа — «Волна 03.09.2026» выше. Он добавлен после полной репетиции свежего PROD-дампа на именованном TEST и
...
**Коррекция 03.09.2026: вывод ниже недействителен.** Процитированные в этом же разделе артефакты его не
доказывают: `role-mutations-2026-09-02T09-08-35.175Z.json` содержит два `pass: false`
(`doctor:working-schedule-change-restore`, `doctor:visit-create`), у роли clinic admin нет ни одной
выполненной мутации, а продажа абонемента не выполнялась ни разу и 03.09 дала живой `42501` на TEST.
Действующий статус — «Track D не закрыт», см. «Волна 03.09.2026». Приведённые выше факты деплоя, reset и
девяти flow остаются верными как история и не переписываются.

Прежний вывод (недействителен): автономная часть Track D закрыта. Открыты только owner-operated доказательства и решения: реальный messenger
