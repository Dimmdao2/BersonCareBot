...
path, что и другие исходящие сообщения. Эти три исправления остаются открытыми до одной общей TEST-выкатки и
живой проверки; домены при ней не переключаются.
...
канонический ключ прекращает fallback-чтение, legacy читается только когда новой строки нет; целевые тесты и typecheck зелёные.
