...
доставку исходного сообщения. Деградация четырёх backup-задач правдива: на хосте нет публичного `age`-получателя
и расписаний backup; продолжение требует публичный ключ, чей приватный recovery-ключ остаётся у владельца вне VPS.
...
остаётся открытым по часовому окну последнего настоящего события и должен закрыться штатным тиком после его истечения.
