...
- [ ] **W11 — A→B не материализует обязательные настройки текущего приложения.** В TEST осталось 88 глобальных
...
- [ ] **W12 — owner-врач после свежего PROD-дампа не может пройти TEST-приёмку.** Его клиника сохранена и
  ...
  продукт под настоящей ролью.
- [ ] **W13 — действия пациента с напоминанием маскируют DB-отказ под «не найдено».** Репозиторий журнала ловит
  ...
  исходом.
- [ ] **W14 — Web Push сообщает успех при потерянной доставке.** Ошибка M2M/авторизации/БД превращается в
...
- [ ] **W15 — Telegram/MAX pre-routing проглатывает DB failure.** Ошибка определения клиники или администратора
  ...
  остаться повторяемой ошибкой, а реальное отсутствие привязки — отдельным нормальным исходом.
- [ ] **W16 — достижим старый HTTP-мост поддержки, который хранит полный текст сообщения.** Внутреннего producer
  ...
  удалить HTTP-мост, а для branded inbound — прямую пересылку без сохранения текста в БД.
- [ ] **W17 — действующее поведение ботов не полностью соответствует поздним owner-решениям.** Подтверждены четыре
  ...
  мессенджера и доставляет код, а создание/привязка завершается только в webapp.
