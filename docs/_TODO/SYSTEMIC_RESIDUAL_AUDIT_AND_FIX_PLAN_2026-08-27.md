...
## Волна финальной репетиции 02.09.2026 — единый реестр новых находок

Этот раздел — текущая точка входа. Он добавлен после полной репетиции свежего PROD-дампа на именованном TEST и
трёх параллельных read-only проходов по ботам, остаточному коду/тестам и разрешённым SQL-портам. Более ранние
закрытия ниже остаются доказательством сделанных этапов, но не отменяют новые достижимые разрывы. Находки этой
волны сначала сведены здесь; продуктовые исправления до сведения и анализа не делались.

### Что доказала живая репетиция

- A→B перенос структуры и данных до `a03e16ce6` завершился транзакционно; schema/cutover/port-context/mTLS и все
  четыре TEST-сервиса прошли свои гейты. Отдельный helper восстановления owner-входов добавлен в `6370e20df`, а
  false positive Gitleaks для опубликованного DEV/TEST-пароля классифицирован в `fcad93804`; Security этого SHA
  зелёный. Третий полный reset на этом SHA запущен отдельным процессом, transcript:
  `/tmp/bcb-test-full-reset-fcad93804.log`.
- Фактический грант на `patient_invites` синхронизирован с декларацией: `app_staff` имеет `SELECT`; присланная
  ошибка DEV не воспроизводится как расхождение текущей TEST-схемы.
- После второго reset штатный вход трёх owner-ролей работал, но продуктовая поверхность не была зелёной:
  пациентские RSC-страницы и `/app/admin/app-settings` отдавали `500`, а doctor API — намеренный
  `403 cabinet_blocked`.

### Принятые находки

- [x] **W11 — A→B не материализует обязательные настройки текущего приложения.** В TEST осталось 88 глобальных
  строк, но ноль `auth_surface_*`; отсутствуют также `error_tracking_dsn` и другие обязательные строки. Runtime
  поэтому падает громко на первом чтении. Причина системная: cutover копирует настройки из PROD-источника и
  вручную добавляет только шесть строк, тогда как baseline-леджер уже помечает более поздние миграции применёнными.
  Значит их data-seed больше никогда не выполнится. Исправление должно иметь один reviewed manifest целевых
  обязательных глобальных строк и один completeness-gate, а не очередной список из одной найденной настройки.
  Закрыто `42c756133` и коррекциями `15a6e66e6` / `7b61d526f` / `9748c3307` / `e47712b3d`: baseline выводится
  из текущего registry, отдельно несёт обязательный doctor-scope fallback и проверяется scope-aware gate.
  Полный reset `r5` завершился `PASS`, после него doctor crawl прошёл `72/72` без прежних render failures.
- [x] **W12 — owner-врач после свежего PROD-дампа не может пройти TEST-приёмку.** Его клиника сохранена и
  membership активен, но у организации нет тарифа, триала или оплаченного периода; product gate правильно
  классифицирует её как `unconfigured` и закрывает кабинет. Ослаблять продуктовый gate нельзя. TEST-reset должен
  штатно назначать owner-клинике reviewed TEST-доступ через существующую коммерческую модель, чтобы проверять
  продукт под настоящей ролью. Закрыто `42c756133`: TEST-only override назначает доступ существующей
  коммерческой моделью, не ослабляя product gate; reset `r5` и реальный doctor-login/crawl это подтвердили.
- [x] **W13 — действия пациента с напоминанием маскируют DB-отказ под «не найдено».** Репозиторий журнала ловит
  любой отказ, включая `42501`, и возвращает `not_found`; «Готово / Позже / Пропустить» не применяется, а API
  отвечает `404`. DB/isolation failure обязан остаться громкой ошибкой, настоящая отсутствующая строка — отдельным
  исходом. Закрыто `42c756133`, независимый kill-set и повторная приёмка — `d1f4692d3`.
- [x] **W14 — Web Push сообщает успех при потерянной доставке.** Ошибка M2M/авторизации/БД превращается в
  отсутствие подписок, relay всё равно отвечает `accepted`, webapp ставит `webPushDelivered: 1`, retry не
  возникает, а в одном случае ещё пишется ложная provider-attempt. До обращения к провайдеру это не попытка и не
  доставка; результат должен пройти одним типизированным fail-visible контрактом от integrator до вызывающего.
  Закрыто `42c756133`, независимый kill-set и повторная приёмка — `d1f4692d3`.
- [x] **W15 — Telegram/MAX pre-routing проглатывает DB failure.** Ошибка определения клиники или администратора
  превращается в «организации нет / не админ», событие идёт по неверному сценарию, provider получает `200`, а
  входящее сообщение может быть потеряно. Все resolver failures до выбора сценария должны остановить обработку и
  остаться повторяемой ошибкой, а реальное отсутствие привязки — отдельным нормальным исходом. Закрыто
  `42c756133`, независимый kill-set и повторная приёмка — `d1f4692d3`.
- [x] **W16 — достижим старый HTTP-мост поддержки, который хранит полный текст сообщения.** Внутреннего producer
  больше нет, но подписанный `POST /api/integrator/support/sync-user-message` снаружи вызывает DB-persistence и
  уведомление врачу; активный unit-тест закрепляет именно это устаревшее поведение. Позднее owner-решение требует
  удалить HTTP-мост, а для branded inbound — прямую пересылку без сохранения текста в БД. Закрыто удалением
  route/schema/producer/test в `42c756133`; отсутствие обхода принято `d1f4692d3`.
- [x] **W17 — действующее поведение ботов не полностью соответствует поздним owner-решениям.** Подтверждены четыре
  отдельных пользовательских последствия: произвольный медицинский текст custom-reminder может уйти в внешний
  мессенджер; branded inbound forwarding отсутствует; webapp всегда строит phone-bind ссылку на общий бот вместо
  выбранного бота клиники; при fallback на общий бот уведомление не гарантирует название клиники. Бот при этом
  учётку не создаёт — текущая базовая цепочка корректна: он подтверждает владение телефоном средствами
  мессенджера и доставляет код, а создание/привязка завершается только в webapp. Privacy-ветка закрыта
  `2f5287671` + `f1b2d158a`; branded routing и clinic title/fallback — merge `fa0316e0b` с независимым
  acceptance artifact `19e46f09c`.

### Проверено и не открывается заново

- Физических readers/writers retired `integrator_user_id`, scalar phone/email и второго delivery store в live TS
  не найдено. Оставшийся process-principal и compatibility transport — другие допустимые идентификаторы.
- Низкоуровневые transaction/probe seams используют только `BEGIN/COMMIT/ROLLBACK/SELECT 1`; нового ручного
  бизнес-SQL вне разрешённых портов не найдено.
- FIO dual-read/dual-write остаётся явно отложенным post-production workstream и не входит в текущий Track D.
- Legacy messenger-bind finish, старые callback-и напоминаний и MAX `open_app` сейчас compatibility-only или
  недостижимы; сами по себе они не являются дефектом этой волны.
- Очередь, намерение, retry-state и история возникновения не объявляются дублями по сходству полей. Уже удалён
  только доказанный второй равнозначный журнал итогов доставки.

### Один системный исправляющий проход

1. **Целевое состояние после A→B.** Ввести один reviewed baseline обязательных глобальных данных, проверить его
   полноту в cutover и дать owner-клинике TEST-only коммерческий доступ без изменения production gate.
2. **Единый контракт ошибки до side effect.** Исправить W13–W15 одним правилом: `absent` остаётся доменным
   исходом, DB/network/auth/config failure не превращается в него, не подтверждает доставку и не создаёт
   provider-attempt. Использовать существующие error/isolation типы и двери, не новые параллельные обёртки.
3. **Одна актуальная bot/support поверхность.** Удалить W16 вместе с устаревшим тестом и привести четыре шва W17
   к owner-решениям, не меняя будущий состав меню и не возвращая создание аккаунта через бот.
4. **Проверка пакетами.** Для каждого класса — целевой behavioral gate; для schema/data — инспекция и живой
   named-TEST evidence. После одного накопленного исправляющего пакета — checked push, полный TEST-reset и единый
   проход doctor / clinic admin / global admin / patient с изменяющими действиями и просмотром журналов.
5. **После принятого TEST.** Только затем выполнить штатный TEST→DEV refresh, синхронизировать этот план, Track D,
   branding-план и taskdb по факту. PROD и переключение доменов в эту волну не входят.
...
- Сколько хранить историю созданных экземпляров напоминаний после завершения?
- Сколько хранить завершённые/отменённые сессии загрузки медиа, если сам файл уже удалён?
- Сколько хранить технические ошибки входящих webhook?
- Сколько хранить журнал отказов tenant/isolation boundary?
