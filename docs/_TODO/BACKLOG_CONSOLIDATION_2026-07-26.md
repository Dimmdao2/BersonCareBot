...
**Указатель не несёт статуса.** `ВЕДЁТСЯ В <файл>:<строка>` — и всё. Без «там N/M», без «закрыто», без
«осталось». Владелец, 29.07: «закрыта ли работа в новом файле — старый знать не должен». Счётчик в чужом
файле протухает на следующий день и врёт в обе стороны.
