...
5. Owner only: verify compatibility catalog state against the owner-provided disposable prod-copy DB:
   ...
   `dbname`/`database`, service/socket-style overrides). It checks the applied 0177 hash and the **161** ENABLE +
   NO FORCE targets discovered from migrations 0160–0176, runtime role invariants (`LOGIN`, `NOSUPERUSER`, `NOCREATEDB`, `NOCREATEROLE`,
   ...
   owner-provided disposable URL. This is not the same inventory as the strict renderer's **163** policy targets:
   the DB-state checker does not cover `public.organization_member_invites` or
   `public.saas_org_entitlement_overrides`. Their ENABLE/FORCE state is an explicit open owner-triage finding;
   do not infer coverage from the 161-target gate.
