...
| Handle unmapped legacy and backfill unmapped buckets | A: map/import all valid records. B: delete only approved test/block rows during commit. C: leave approved exceptions with owner reason. | Acceptance requires unmapped zero or explicit owner-approved exceptions. |
| Resolve duplicate clusters 7 | A: collapse duplicates with approved flag. B: keep as documented distinct appointments. C: split: collapse only verified duplicates. | Prevents double counting in calendar, list, KPI, and future migration proof. |
...
| Authorize backfill commit | A: no commit yet. B: commit tolerant projection only. C: commit with delete-test, collapse-dups, and drop-stale-from-csv after fresh CSV. D: commit with an explicit approved drop-legacy list. | This is the write gate. No worker may run commit mode until owner picks exact flags. |
