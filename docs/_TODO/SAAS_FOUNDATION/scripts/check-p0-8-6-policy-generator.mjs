...
const piiOrgGatedTables = new Set([
...
for (const descriptor of descriptors) {
  ...
  const tableStatements = statements.filter((statement) => statement.includes(quotedTarget)).join("\n");
  ...
  if (legacyGlobalTables.has(descriptor.table)) {
    ...
    if (!tableStatements.includes(`"organization_id" IS NULL OR (${orgContextSql} IS NOT NULL AND "organization_id" = ${orgContextSql})`)) {
    ...
    if (!tableStatements.includes(`(${orgContextSql} IS NOT NULL AND "organization_id" = ${orgContextSql})`)) {
      fail(`${descriptor.table} must allow matching organization rows`);
    }

    if (!tableStatements.includes(`"organization_id" IS NULL AND ${dormantCompatibilityPredicate}`)) {
...
console.log("P0.8.6 policy generator OK: 3 global bootstrap hybrids and 2 PII org-gated bootstrap hybrids.");
