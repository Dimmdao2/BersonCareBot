...
const files = {
  ...
  operatorAttemptWritePort:
    'apps/integrator/src/infra/runtime/worker/operatorDeliveryAttemptWritePort.ts',
  operatorAttemptWritePortTest:
    'apps/integrator/src/infra/runtime/worker/operatorDeliveryAttemptWritePort.test.ts',
  ...
  reportOperatorFailureTest:
    'apps/integrator/src/infra/operatorIncident/reportOperatorFailure.test.ts',
...
function assertOperationalSqlAndDeploy(loaded) {
  ...
  const strictClosure = sourceTextSliceBetween(
    loaded.testDeploy,
    'run_strict_post_migration_closure(){',
    'assert_strict_closure_deploy_checkout_ready(){',
    files.testDeploy,
  );
  if (strictClosure === null) fail(`${files.testDeploy} missing strict closure function block`);
  ...
  requireOccurrenceCountAtLeast(
    files.operationalReadinessScript,
    loaded.operationalReadinessScript,
    'expect_denied',
    15,
  );
  ...
  requireOccurrenceCountAtLeast(
    files.webPushOperationalSql,
    loaded.webPushOperationalSql,
    'FROM :"c4_web_push_reminder_login_role" CASCADE',
    2,
  );
  requireOccurrenceCountAtLeast(
  ...
  requireOccurrenceCountAtLeast(
    files.webPushOperationalProof,
    loaded.webPushOperationalProof,
...
if (process.argv.includes('--self-test')) {
  ...
  const webpushRoute = read(
    'apps/webapp/src/app/api/internal/reminders/web-push-only/tick/route.ts',
  ).replaceAll('WEB_PUSH_ONLY_REMINDER_TICK_DB_SOURCE', 'REMINDER_DB_SOURCE_MISSING');
  const doc = read(files.doc).replaceAll(
    '`/api/internal/media-transcode/enqueue`',
    '`/api/internal/media-transcode/enqueue-missing`',
  );
  const operationalSql = read(files.operationalSql).replaceAll(
    'WITH INHERIT FALSE, SET TRUE',
  ...
  const operationalReadinessScriptNoWebPushHelperProbe = read(
    files.operationalReadinessScript,
  ).replace(
    "SELECT set_config('app.org', '00000000-0000-4000-8000-000000000001', true)",
