...
function psql(text, label, { expectFailure = false, user = "postgres", variables = [] } = {}) {
  return run(
    ...
    ["-X", "-qAt", "-h", socket, "-p", port, "-U", user, "-v", "ON_ERROR_STOP=1", ...variables, "-d", db],
    text,
    label,
    { expectFailure, user },
  );
...
function installFixture() {
  ...
  psql(
    ["\\set phase4_enforce_locked_context 1", ...getS5RuntimeSettingsTargets().map(renderPhase4PolicyReplacement)].join("\n\n"),
  ...
  psql("GRANT SELECT ON public.app_runtime_settings TO app_runtime_nonstaff_login;", "bootstrap policy probe grant");
}
...
function assertRoleMatrix() {
  ...
  contextSession(
    "app_runtime_staff_login", "app_staff", orgA,
    "(SELECT count(*) FROM public.app_runtime_settings)=6 AND (SELECT count(*) FROM public.app_runtime_settings WHERE organization_id='" + orgB + "')=0",
    "staff global/current-org runtime rows",
  );
  contextSession(
    "app_runtime_nonstaff_login", "app_patient", orgA,
    "(SELECT count(*) FROM public.app_runtime_settings)=4 AND NOT EXISTS (SELECT 1 FROM public.app_runtime_settings WHERE audience='server' OR organization_id='" + orgB + "')",
    "patient safe global/current-org runtime rows",
  );
  contextSession(
    "app_runtime_nonstaff_login", "app_patient", orgB,
    "(SELECT count(*) FROM public.app_runtime_settings WHERE organization_id='" + orgA + "')=0 AND (SELECT count(*) FROM public.app_runtime_settings WHERE organization_id='" + orgB + "')=2",
    "patient wrong-org denial",
  );
  contextSession(
    "app_runtime_nonstaff_login", "", orgA,
  ...
  contextSession(
    "app_runtime_staff_login", "app_staff", orgA,
    "(SELECT count(*) FROM public.app_runtime_settings_audit)=2 AND NOT EXISTS (SELECT 1 FROM public.app_runtime_settings_audit WHERE organization_id='" + orgB + "')",
  ...
  `, "staff audit UPDATE denied", { expectFailure: true });
  psql(`
    INSERT INTO app.principal_context VALUES (pg_backend_pid(),'${orgA}')
      ON CONFLICT (backend_pid) DO UPDATE SET org_id=excluded.org_id;
    SET SESSION AUTHORIZATION app_runtime_staff_login;
    ...
    DELETE FROM public.app_runtime_settings_audit
      WHERE id='30000000-0000-4000-8000-000000000001';
  `, "staff audit DELETE denied", { expectFailure: true });
  psql("SET ROLE app_patient; SELECT * FROM public.app_runtime_settings_audit;", "patient audit read denial", { expectFailure: true });
  psql("SET ROLE app_patient; SELECT * FROM public.system_settings;", "patient restricted read denial", { expectFailure: true });
  psql("SELECT * FROM public.system_settings;", "bootstrap restricted read denial", { expectFailure: true, user: "app_runtime_nonstaff_login" });
}

function assertConfigReaderMatrix() {
  contextSession(
    "s5_config_reader_login", "app_config_reader", orgA,
    "(SELECT count(*) FROM public.system_settings)=2 AND NOT EXISTS (SELECT 1 FROM public.system_settings WHERE organization_id='" + orgB + "')",
    "config reader global/exact-org restricted rows",
  );
  contextSession(
    "s5_config_reader_login", "app_config_reader", undefined,
  ...
  contextSession(
    "s5_config_reader_login", "app_config_reader", orgB,
    "NOT EXISTS (SELECT 1 FROM public.system_settings WHERE organization_id='" + orgA + "') AND (SELECT count(*) FROM public.system_settings WHERE organization_id='" + orgB + "')=1",
    "config reader wrong-org denial",
  );
  for (const role of ["app_staff", "app_patient"]) {
    psql(`SET ROLE ${role};`, `config reader cannot SET ROLE ${role}`, { expectFailure: true, user: "s5_config_reader_login" });
  }
  psql("SET ROLE app_config_reader; SELECT * FROM public.patient_files;", "config reader zero clinical privileges", { expectFailure: true, user: "s5_config_reader_login" });
  psql("SET ROLE app_config_reader; SELECT * FROM public.app_runtime_settings_audit;", "config reader zero audit privileges", { expectFailure: true, user: "s5_config_reader_login" });
  assertTrue(`SELECT
...
function assertConfigReaderArtifactRepeatability() {
  const artifact = readFileSync(path.join(root, "deploy/postgres/s5-config-reader-runtime.sql"), "utf8");
...
try {
  ...
  run(path.join(pgBin, "pg_ctl"), ["-D", data, "-l", log, "-o", `-k ${socket} -p ${port} -c listen_addresses=''`, "-w", "start"], undefined, "private PostgreSQL startup");
  serverStarted = true;
  run(path.join(pgBin, "createdb"), ["-h", socket, "-p", port, "-U", "postgres", db], undefined, "private scratch database creation");
  installFixture();
  assertRoleMatrix();
