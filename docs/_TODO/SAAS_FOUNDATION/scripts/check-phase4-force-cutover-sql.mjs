...
const noForceCompatMigrationPath =
  'apps/webapp/db/drizzle-migrations/0177_phase4_no_force_rls_compat.sql';
...
const noForceStatementPattern =
  /ALTER\s+TABLE\s+((?:"[^"]+"\.)?"[^"]+")\s+NO\s+FORCE\s+ROW\s+LEVEL\s+SECURITY\s*;/gi;
...
const expectedPrefixes = Array.from({ length: 17 }, (_, index) =>
  String(160 + index).padStart(4, '0'),
);
...
for (const file of migrationFiles) {
  ...
  if (forceRlsPattern.test(sql)) {
    fail(
      `${path} must not contain FORCE ROW LEVEL SECURITY; dormant compatibility migrations must stay NO FORCE`,
    );
  }
...
assertIncludes(
  cutoverSql,
  'FATAL: missing required psql variable phase4_bootstrap_base_role.',
  cutoverSqlPath,
);
assertIncludes(
  cutoverSql,
  'FATAL: missing required psql variable phase4_staff_role.',
  cutoverSqlPath,
);
assertIncludes(
  cutoverSql,
  'FATAL: missing required psql variable phase4_owner_role.',
  cutoverSqlPath,
);
assertIncludes(
  cutoverSql,
  "pg_roles WHERE rolname = :'phase4_bootstrap_base_role'",
  cutoverSqlPath,
);
...
assertIncludes(
  cutoverSql,
  "NOT pg_has_role(base_role.rolname, staff_role.rolname, 'member')",
  cutoverSqlPath,
);
assertIncludes(
  cutoverSql,
...
assertIncludes(cutoverSql, 'phase4_bootstrap_base_role_user_phone_history_dml', cutoverSqlPath);
assertIncludes(
  cutoverSql,
...
assertIncludes(cutoverSql, 'phase4_bootstrap_base_role_platform_user_contacts_dml', cutoverSqlPath);
assertIncludes(
  cutoverSql,
...
assertOrdered(
  cutoverSql,
  "SELECT 1 / (:'phase4_force_rls_down' IN ('0', '1'))::int",
  'BEGIN;',
  cutoverSqlPath,
);
assertOrdered(
...
assertOrdered(cutoverSql, 'phase4_owner_role_bypassrls', 'BEGIN;', cutoverSqlPath);
assertOrdered(
  cutoverSql,
...
const branchMatch = [
  ...
  (match) =>
    match.groups?.down.includes('ROW LEVEL SECURITY') ||
    match.groups?.up.includes('ROW LEVEL SECURITY'),
);
if (!branchMatch?.groups) {
  fail(
    `${cutoverSqlPath} must branch on \\if :phase4_force_rls_down with explicit down/up branches`,
  );
}
...
if (gexecCount !== 2) {
  fail(
    `${cutoverSqlPath} must execute exactly two generated statement branches, got ${gexecCount}`,
  );
}
...
const duplicateCutoverTargets = cutoverTargets.filter(
  (target, index) => cutoverTargets.indexOf(target) !== index,
);
...
const missingInCutover = generatedLockedTargets.filter(
  (target) => !cutoverTargets.includes(target),
);
...
if (missingInCutover.length > 0 || extraInCutover.length > 0) {
  const details = [
    ...
    .join('; ');
  fail(
    `${cutoverSqlPath} target list must match generated Phase 4 locked policy targets (${details})`,
  );
}
...
const missingInNoForceCompat = expectedTargets.filter(
  (target) => !noForceCompatTargets.includes(target),
);
const extraInNoForceCompat = noForceCompatTargets.filter(
...
if (missingInNoForceCompat.length > 0 || extraInNoForceCompat.length > 0) {
  const details = [
    ...
    .join('; ');
  fail(
    `${noForceCompatMigrationPath} target list must match unique ENABLE RLS targets from 0160-0176 (${details})`,
  );
}
...
assertIncludes(
  deploySaas667Sql,
  'owner role % must be NOLOGIN NOSUPERUSER BYPASSRLS',
  deploySaas667Path,
);
