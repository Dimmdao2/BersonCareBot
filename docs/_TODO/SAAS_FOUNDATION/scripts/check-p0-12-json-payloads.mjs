...
const expectedRows = new Map([
  ...
  ["integrator.rubitime_create_retry_jobs", "message_text,payload_json,last_error"],
...
function parseTsv(path, text) {
  ...
  return lines.map((line, index) => {
    ...
    if (cols.length !== headers.length) {
      throw new Error(`${path}:${index + 2} expected ${headers.length} columns, got ${cols.length}`);
    }
...
function runChecks(overrides = {}) {
  ...
  for (const row of rows) {
    ...
    if (row.tier !== expectedTier) throw new Error(`${key} tier mismatch: expected ${expectedTier}, got ${row.tier}`);
    if (!allowedClassifications.has(row.classification)) throw new Error(`${key} invalid classification ${row.classification}`);
    ...
    if ((row.tier === "INFRA" || row.tier === "TELEMETRY") && row.user_bearing === "yes") {
      ...
      if (!decision.includes("retention") && !decision.includes("scrub")) {
        throw new Error(`${key} INFRA/TELEMETRY user-bearing payload lacks retention/scrub decision`);
      }
    }
  }
