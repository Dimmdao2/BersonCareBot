...
if (
  !/INSERT INTO app\.context_signing_secrets \(id, secret\)\s+VALUES \(true, :'p2_b_signing_secret'\)/.test(
    opsSql,
  )
) {
  fail(
    'P2-B ops SQL must source the signing secret only from the p2_b_signing_secret psql variable',
  );
}
