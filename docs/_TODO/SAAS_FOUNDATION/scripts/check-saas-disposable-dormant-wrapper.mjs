...
function runChecks(overrides = {}) {
  ...
  if (!hardScript?.includes("pnpm run check:saas-disposable-dormant-wrapper")) {
    fail(`${files.packageJson} check:saas-hard-migration-protocol must include the disposable wrapper checker`);
  }
}
...
function runSelfTest() {
  ...
  const cases = [
    ...
    {
      wrapper: wrapper.replaceAll("DATABASE_URL: plan.targetOwnerUrl", "DATABASE_URL: process.env.DATABASE_URL"),
    },
    {
      wrapper: replaceRequired(
    ...
    {
      wrapper: wrapper.replaceAll("env[deploySuperuserSudoEnv] = \"1\";", "env.SUPERUSER_URL = plan.targetSuperuserUrl;"),
    },
    {
    ...
    {
      wrapper: wrapper.replaceAll("const env = deploy667ChildEnv(plan);", "const env = sanitizedChildEnv();"),
    },
    {
      wrapper: replaceRequired("default dry-run true", wrapper, "dryRun: true,", "dryRun: false,"),
    },
    {
      wrapper: replaceRequired("default execute false", wrapper, "execute: false,", "execute: true,"),
    },
    {
      wrapper: replaceRequired(
    ...
    {
      wrapper: replaceRequired("final dry-run branch bypass", wrapper, finalDryRunBranch, "runExecute(plan, options);"),
    },
    {
      wrapper: replaceRequired(
