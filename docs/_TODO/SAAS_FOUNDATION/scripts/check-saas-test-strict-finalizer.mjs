...
function load(overrides = {}) {
  return Object.fromEntries(
    Object.entries(files).map(([key, path]) => [key, overrides[key] ?? readFileSync(path, 'utf8')]),
  );
}
...
function methodSlice(label, text, start, end) {
  const startIndex = sourceTextIndexOf(text, start, label);
  if (startIndex < 0) fail(`${label} missing method start: ${start}`);
  const block = sourceTextSliceBetween(text, start, end, label);
  if (block === null) fail(`${label} missing method end: ${end}`);
  return block;
}
...
function runChecks(overrides = {}) {
  ...
  for (const forbidden of [
    ...
    if (sourceTextIncludes(loaded.appWorker, forbidden, files.appWorker)) {
  ...
  if (sourceTextIncludes(loaded.patientPlayback, 'TO app_staff', files.patientPlayback)) {
    fail(`${files.patientPlayback} still grants telemetry accessor execution to app_staff`);
  }
  if (
    /GRANT\s+(?:INSERT|UPDATE|DELETE)[^;]*\bTO\s+app_(?:staff|patient)\b/i.test(
      loaded.patientPlayback,
    )
  ) {
  ...
  if (
    sourceTextIncludes(patientResultDetails, '.innerJoin(clinicalTests', files.patientTestResults)
  ) {
    fail(
      `${files.patientTestResults} patient result details reopened the restricted clinical-test catalog`,
    );
  }
}
...
function runSelfTest() {
  ...
  const cases = [
    ...
    {
      invites: baseline.invites.replace(
        'ALTER FUNCTION app.lookup_pending_org_invite(text) OWNER TO app_owner',
        '',
      ),
    },
    ...
    {
      courses: baseline.courses.replaceAll(
        '"b4course_instance"."template_id" = "courses"."program_template_id"',
        'TRUE',
      ),
    },
    {
      appWorker: baseline.appWorker.replaceAll(
        "pg_has_role(current_user, 'app_worker', 'member')",
        'FALSE',
      ),
    },
    {
      appWorker: baseline.appWorker.replace(
        'GRANT EXECUTE ON FUNCTION app.current_org_id() TO app_worker;',
        '',
      ),
    },
    ...
    {
      finalizer: baseline.finalizer.replace(
        '\\ir patient-media-playback-telemetry-accessors.sql',
        '',
      ),
    },
    ...
    {
      prod: baseline.prod.replace(
        'psql "${DATABASE_URL}" -X -v ON_ERROR_STOP=1 -f "${PROJECT_ROOT}/${PATIENT_MEDIA_PLAYBACK_TELEMETRY_ACCESSORS}"',
        '',
      ),
    },
    {
      webappProd: baseline.webappProd.replace(
        'psql "${DATABASE_URL}" -X -v ON_ERROR_STOP=1 -f "${PROJECT_ROOT}/deploy/postgres/patient-media-playback-telemetry-accessors.sql"',
        '',
      ),
    },
    ...
    {
      c0Smoke: baseline.c0Smoke.replace(
        'test-patient-identity-capability-gate.sql',
        'removed-gate.sql',
      ),
    },
    ...
    {
      fixtureValidator: baseline.fixtureValidator.replace(
        '[ "$fixture_path" = "$canonical_fixture" ]',
        'true',
      ),
    },
    ...
    {
      codeOnly: baseline.codeOnly.replace(
        'bash "$DEPLOY_REPO/$STRICT_CLOSURE" --post-migration-closure',
        '',
      ),
    },
...
try {
  ...
  console.error(
    `check-saas-test-strict-finalizer: ${error instanceof Error ? error.message : String(error)}`,
  );
  process.exit(1);
}
