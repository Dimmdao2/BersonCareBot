...
SELECT format(
  $sql$
DO $cross_org_stat_denial$
BEGIN
  PERFORM app.increment_media_playback_resolution_stat(%L::uuid, %L::uuid, 'file', false);
  RAISE EXCEPTION 'scratch_cross_org_stat_unexpectedly_allowed';
EXCEPTION
  WHEN insufficient_privilege THEN NULL;
END
$cross_org_stat_denial$;
$sql$,
  :'patient_user',
  :'cross_media'
) \gexec
...
-- An org-only patient runtime must not turn a caller-supplied user id into an actor identity.
DELETE FROM app.principal_context WHERE backend_pid = pg_backend_pid();
INSERT INTO app.principal_context
  (backend_pid, org_id, patient_user_id, integrator_user_id, nonce, expires_epoch)
VALUES
  (
    ...
    NULL,
    NULL,
    'scratch-org-only-playback-' || pg_backend_pid()::text,
    floor(extract(epoch FROM clock_timestamp()))::bigint + 300
  );
...
BEGIN
  PERFORM app.record_media_playback_resolution_event(%L::uuid, %L::uuid, 'file', false);
  RAISE EXCEPTION 'scratch_org_only_actor_unexpectedly_allowed';
EXCEPTION
  WHEN insufficient_privilege THEN NULL;
END
$org_only_denial$;
...
-- Integrator context also has no signed patient identity and must fail closed.
DELETE FROM app.principal_context WHERE backend_pid = pg_backend_pid();
INSERT INTO app.principal_context
  (backend_pid, org_id, patient_user_id, integrator_user_id, nonce, expires_epoch)
VALUES
  (
    ...
    NULL,
    1,
    'scratch-integrator-playback-' || pg_backend_pid()::text,
    floor(extract(epoch FROM clock_timestamp()))::bigint + 300
  );
...
SELECT format(
  $sql$
DO $integrator_denial$
BEGIN
  PERFORM app.increment_media_playback_resolution_stat(%L::uuid, %L::uuid, 'file', false);
  RAISE EXCEPTION 'scratch_integrator_actor_unexpectedly_allowed';
EXCEPTION
  WHEN insufficient_privilege THEN NULL;
END
$integrator_denial$;
...
-- Staff has no EXECUTE ACL: same-org context plus a forged actor id is insufficient.
DELETE FROM app.principal_context WHERE backend_pid = pg_backend_pid();
INSERT INTO app.principal_context
  (backend_pid, org_id, patient_user_id, integrator_user_id, nonce, expires_epoch)
VALUES
  (
    ...
    NULL,
    NULL,
    'scratch-staff-playback-' || pg_backend_pid()::text,
    floor(extract(epoch FROM clock_timestamp()))::bigint + 300
  );
...
SELECT format(
  $sql$
DO $staff_forged_actor_denial$
BEGIN
  PERFORM app.record_media_playback_resolution_event(%L::uuid, %L::uuid, 'file', false);
  RAISE EXCEPTION 'scratch_staff_forged_actor_unexpectedly_allowed';
EXCEPTION
  WHEN insufficient_privilege THEN NULL;
END
$staff_forged_actor_denial$;
$sql$,
  :'patient_user',
  :'own_media'
) \gexec
...
)::int AS denied_contexts_created_no_extra_events;
