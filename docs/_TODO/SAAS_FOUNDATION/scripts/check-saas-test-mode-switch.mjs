...
function runChecks(overrides = {}) {
  ...
  if (
    ...
    fail(`${files.packageJson} must wire check:saas-test-mode-switch to bash syntax, node syntax, main check, and self-test`);
  }
  ...
  if (!hardScript?.includes("pnpm run check:saas-test-mode-switch")) {
    fail(`${files.packageJson} check:saas-hard-migration-protocol must include check:saas-test-mode-switch`);
  }
}
...
function runSelfTest() {
  ...
  const cases = [
    ...
    {
      script: baseScript.replace('API_ENV="/opt/env/bersoncarebot/api.test"', 'API_ENV="/opt/env/bersoncarebot/api.prod"'),
    },
    {
