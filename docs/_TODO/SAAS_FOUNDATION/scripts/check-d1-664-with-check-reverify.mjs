...
const scratchSmokeScripts = [files.p2c1Smoke, files.p2c2Smoke, files.composedSmoke];
...
async function runChecks(overrides = {}) {
  ...
  if (treatmentEvents.privileges !== 'SELECT') {
    fail(
      `public.treatment_program_events app_patient base grant must be SELECT, got ${treatmentEvents.privileges}`,
    );
  }
  ...
  if (channelPreferences.privileges !== 'SELECT') {
    fail(
      `public.user_channel_preferences app_patient base grant must be SELECT, got ${channelPreferences.privileges}`,
    );
  }
