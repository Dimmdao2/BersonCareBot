...
const files = {
  ...
  checklist: 'docs/archive/2026-07-plans/SAAS_FOUNDATION/P0_11_SYSTEM_SETTINGS_ORG_CHECKLIST.md',
...
function assertContains(path, text, needle) {
  if (!sourceTextIncludes(text, needle, path)) {
...
function runChecks(overrides = {}) {
  ...
  assertContains(
    files.mirrorRule,
    mirrorRule,
    'Pass `organizationId` only when the write is intentionally org-scoped',
  );
  assertContains(
    files.configDoc,
    configDoc,
    'текущие admin Settings формы пишут глобальные строки',
  );
  assertContains(
    files.configDoc,
    configDoc,
    'Чтения с org context должны сначала искать org row и затем fallback на global NULL row',
  );
  assertContains(
    files.checklist,
    checklist,
  ...
  assertContains(
    files.readme,
    readme,
    'P0.11.1-P0.11.4 system_settings storage/read/write/rules-docs are implemented',
  );
}

if (process.argv.includes('--self-test')) {
  const mirrorRule = read(files.mirrorRule).replaceAll(
