...
function assertScopedDescriptorSemantics(descriptors = buildRlsDescriptors()) {
  const scopedDescriptors = Array.from(descriptors.values()).filter(
    (descriptor) => descriptor.tier === 'SCOPED',
  );
  const violations = [];
  ...
  for (const descriptor of scopedDescriptors) {
    if (orgColumnKinds.has(descriptor.scopingKind)) {
      ...
      if (!descriptor.source && !descriptor.sourceStage) {
        violations.push(
          `${descriptor.table}: ${descriptor.scopingKind} must retain source/sourceStage`,
        );
      }
    ...
    if (descriptor.scopingKind === 'self_org_id') {
      if (descriptor.table !== 'public.be_organizations' || descriptor.orgColumn !== 'id') {
        violations.push(
          `${descriptor.table}: self_org_id is only valid for public.be_organizations.id`,
        );
      }
    ...
    if (descriptor.scopingKind === 'fk_path') {
      ...
      if (
        ...
        violations.push(
          `${descriptor.table}: fk_path descriptor is missing parent/cross-check metadata`,
        );
        continue;
      }

      if (
        fkPath.parentOrgColumn !== 'organization_id' ||
        fkPath.crossCheckOrgColumn !== 'organization_id'
      ) {
        violations.push(
          `${descriptor.table}: fk_path parent and cross-check org columns must be organization_id`,
        );
      }
    ...
    if (descriptor.scopingKind === 'polymorphic_resolver') {
      ...
      if (descriptor.orgColumn !== 'organization_id') {
        violations.push(
          `${descriptor.table}: polymorphic resolver must use materialized organization_id`,
        );
      }
    ...
    violations.push(
      `${descriptor.table}: unsupported SCOPED scopingKind ${descriptor.scopingKind}`,
    );
  }
...
function cloneDescriptorsWithMutation(mutate) {
  ...
  for (const [table, descriptor] of buildRlsDescriptors().entries()) {
    descriptors.set(table, {
      ...descriptor,
      fkPath: descriptor.fkPath ? { ...descriptor.fkPath } : undefined,
    });
  }
