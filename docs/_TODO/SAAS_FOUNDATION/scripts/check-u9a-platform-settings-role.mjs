...
for (const fragment of [
  ...
  if (!artifact.includes(fragment))
    throw new Error(`U9A platform-settings SQL missing ${fragment}`);
for (const forbidden of [
  ...
  if (artifact.includes(forbidden))
    throw new Error(`U9A platform-settings SQL contains forbidden ${forbidden}`);
if (
  /GRANT\s+[^;]*ON TABLE public\.integrator_push_outbox\s+TO app_platform_settings/i.test(artifact)
) {
...
const whitelistedKeys = [...(whitelistMatch?.[1] ?? '').matchAll(/'([a-z0-9_:]+)'/g)].map(
  (match) => match[1],
);
