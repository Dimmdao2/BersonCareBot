...
function run(overrides = {}) {
  ...
  requireFragments('global email delivery audit migration', files.deliveryAuditMigration, [
    ...
    'SECURITY DEFINER',
    'SET search_path = pg_catalog',
    "p_intent_type IS DISTINCT FROM 'message.send'",
    "NULLIF(btrim(p_intent_event_id), '') IS NULL",
  ...
  requireFragments('TEST deploy wiring', files.deploy, [
    ...
    "procedure.oid = 'app.record_global_email_delivery_attempt(text,text,text,text,text,integer,text,jsonb,timestamptz)'::regprocedure AND procedure.prosecdef AND owner.rolname = 'app_owner'",
  ...
  requireFragments('disposable SMTP ACL smoke', files.smtpSmoke, [
    ...
    'smtp_runtime_delivery_audit_broad_intent_unexpectedly_succeeded',
    'app.record_global_email_delivery_attempt(',
    "intent_event_id = 'smoke:global-email'",
    'runRuntimePathProbe',
