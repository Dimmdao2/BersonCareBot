...
import { sourceTextIncludes, sourceTextIndexOf, sourceTextReplace } from './source-text-guard.mjs';
...
function collectScopedDbTouchingRoutesMissingPrincipalSource() {
  ...
  return listRouteFiles(apiRoot).flatMap((path) => {
    ...
    if (!dbSignals.some((signal) => sourceTextIncludes(text, signal, relativePath))) return [];
    if (principalSources.some((source) => sourceTextIncludes(text, source, relativePath)))
      return [];
    return [relativePath];
  });
}
...
function runChecks(overrides = {}) {
  ...
  if (/client\.release\(\)/.test(loaded.schedulerLocks)) {
    fail(
      `${files.schedulerLocks} must not bypass prepared-client cleanup with raw client.release()`,
    );
  }
  ...
  requireFragments(
    files.webappPatientOrganizationService,
    loaded.webappPatientOrganizationService,
    [
  ...
  for (const [label, text] of [
    ...
    requireFragments(label, text, ['stampBootstrapPrincipal']);
  }
  ...
  if (missingPrincipalRoutes.length > 0) {
    fail(
      `SCOPED DB-touching webapp routes missing a principal source:\n${missingPrincipalRoutes.join('\n')}`,
    );
  }
...
if (process.argv.includes('--self-test')) {
  const webappPoolProvider = sourceTextReplace(
    read(files.webappPoolProvider),
    'applyDbPrincipalToConnection(client, principalSnapshot, principalApplyOptions)',
    'applyDbPrincipalToConnection(client, principalSnapshot)',
    files.webappPoolProvider,
  );
  try {
