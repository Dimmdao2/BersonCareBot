...
function runChecks(overrides = {}) {
  ...
  requireOccurrenceCountAtLeast(files.preflight, loaded.preflight, "assertNoSecretLeak(output, ", 2);
...
if (process.argv.includes("--self-test")) {
  const preflight = read(files.preflight).replace(/assertNoSecretLeak\(output, [^)]+\);/g, "// removed by self-test");
  try {
    runChecks({ preflight });
  } catch {
