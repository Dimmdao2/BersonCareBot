...
for (const [index, line] of lines.entries()) {
  ...
  if (fields.length !== header.length) {
    throw new Error(`Expected ${header.length} fields in ${fkPathPath}:${lineNumber}, got ${fields.length}`);
  }
...
for (const [tableName, row] of rows.entries()) {
  ...
  if (notScoped.length > 0) {
    throw new Error(`P0.4.BE related tables are not SCOPED in tiers-218.tsv: ${notScoped.join(", ")}`);
  }
}

const needsOrgTables = new Set(readLines(needsPath));
const wronglyNeedsOrg = Array.from(rows.keys()).filter((tableName) => needsOrgTables.has(tableName));

if (wronglyNeedsOrg.length > 0) {
  throw new Error(`P0.4.BE FK-path tables must stay outside needs-orgid-FINAL: ${wronglyNeedsOrg.join(", ")}`);
}
...
if (wronglyBatched.length > 0) {
  throw new Error(`P0.4.BE FK-path tables must stay outside p0-4-batches.tsv: ${wronglyBatched.join(", ")}`);
}
