...
const expectedPublicDirectOrgPolicyTargets = 111;
...
if (tierRows.length !== 240) {
  fail(`Expected 240 tier rows, got ${tierRows.length}`);
}
  ...
  if (descriptor.tier === "BOOTSTRAP") {
    if (descriptor.scopingKind === "bootstrap_hybrid") {
      ...
      if (
        ...
        JSON.stringify(descriptor.safeAudiences) !== JSON.stringify(["public", "authenticated_client"])
      ) {
...
console.log("P0.8.1 RLS descriptor model OK: 240 descriptors cover tiers-218.tsv exactly once.");
console.log(
