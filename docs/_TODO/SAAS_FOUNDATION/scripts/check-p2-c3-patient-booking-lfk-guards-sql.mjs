...
const compactGrantsGenerator = grantsGenerator.replace(/\s+/g, " ");
...
requireFragments("P2-C3 notification-only reschedule update shape", compactOpsSql, [
  "AND NEW.created_at IS NOT DISTINCT FROM OLD.created_at THEN RETURN NEW;",
]);

requireFragments("P0.5b grant reconciliation", compactGrantsGenerator, [
