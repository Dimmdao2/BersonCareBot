...
function parseTsv(path, text) {
  ...
  return lines.map((line, index) => {
    ...
    if (fields.length !== headers.length) {
      throw new Error(`${path}:${index + 2} expected ${headers.length} fields, got ${fields.length}`);
    }
...
function assertUuid(label, value, { nullable = false } = {}) {
  ...
  if (!uuidPattern.test(value)) throw new Error(`${label} is not a deterministic RFC4122 v4 UUID: ${value}`);
}

function assertNoRealIdentifiers(rows) {
  const joined = JSON.stringify(rows);
  for (const forbidden of ["@", "+7", "telegram", "rubitime", "smsc", "max_id", "phone_normalized"]) {
...
function assertScratchSqlCreatesRepresentativeRows(sql) {
  for (const table of [
    ...
    if (!sql.includes(`INSERT INTO ${table}`)) throw new Error(`scratch SQL does not seed ${table}`);
  }

  if (sql.includes("''::uuid")) throw new Error("scratch SQL must render missing UUID values as NULL, not empty strings");
...
function runChecks(overrides = {}) {
  ...
  if (new Set(Object.values(syntheticFixtureIds)).size !== Object.values(syntheticFixtureIds).length) {
    throw new Error("synthetic fixture IDs must be unique");
  }
  if (new Set(Object.values(syntheticIntegratorUserIds)).size !== Object.values(syntheticIntegratorUserIds).length) {
  ...
  if (parsedManifest.length !== rows.length) throw new Error("manifest row count does not match fixture rows");

  const fixtureIds = rows.map((row) => row.fixtureId);
  if (new Set(fixtureIds).size !== fixtureIds.length) throw new Error("fixture rows contain duplicate fixture_id");

  assertSetContainsAll("fixture family coverage", rows.map((row) => row.family), requiredFixtureFamilies);
  assertSetContainsAll("fixture organization coverage", rows.map((row) => row.organizationKey), ["org_a", "org_b"]);
  assertSetContainsAll("fixture patient coverage", rows.map((row) => row.patientKey), [
    "patient_a1",
    "patient_a2",
    "patient_b1",
  ]);
  assertSetContainsAll("fixture table coverage", rows.map((row) => row.table), [
    "public.be_organization_members",
    "public.org_enrollments",
    "public.be_package_items",
    "public.be_patient_package_items",
  ...
  const sameOrgPatientRows = rows.filter((row) => row.organizationKey === "org_a" && row.patientKey.startsWith("patient_a"));
  assertSetContainsAll("same-org patient wall rows", sameOrgPatientRows.map((row) => row.patientKey), [
    "patient_a1",
    "patient_a2",
  ]);
