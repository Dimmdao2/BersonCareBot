...
const registrarFiles = {
  ...
  registerBersoncareRelayOutboundRoute: "apps/integrator/src/integrations/bersoncare/relayOutboundRoute.ts",
  registerOperatorAlertRelayRoute: "apps/integrator/src/integrations/bersoncare/operatorAlertRelayRoute.ts",
  registerBersoncareRequestContactRoute: "apps/integrator/src/integrations/bersoncare/requestContactRoute.ts",
  registerBersoncareSendOtpRoute: "apps/integrator/src/integrations/bersoncare/sendOtpRoute.ts",
  registerBersoncareReminderRulesRoute: "apps/integrator/src/integrations/bersoncare/reminderRulesRoute.ts",
  registerBersoncareSettingsSyncRoute: "apps/integrator/src/integrations/bersoncare/settingsSyncRoute.ts",
  registerBersoncareUserMergeM2mRoutes: "apps/integrator/src/integrations/bersoncare/userMergeM2mRoute.ts",
  registerOperatorHealthProbeRoute: "apps/integrator/src/integrations/bersoncare/operatorHealthProbeRoute.ts",
  registerBersoncareBookingLifecycleRoute: "apps/integrator/src/integrations/bersoncare/bookingLifecycleRoute.ts",
};
...
const expectedInventoryEntries = [
  ...
  { id: "send-sms", method: "POST", path: "/api/bersoncare/send-sms", source: registrarFiles.registerBersoncareSendSmsRoute },
  { id: "send-email", method: "POST", path: "/api/bersoncare/send-email", source: registrarFiles.registerBersoncareSendEmailRoute },
  { id: "relay-outbound", method: "POST", path: "/api/bersoncare/relay-outbound", source: registrarFiles.registerBersoncareRelayOutboundRoute },
  { id: "operator-alert-relay", method: "POST", path: "/api/bersoncare/operator-alert-relay", source: registrarFiles.registerOperatorAlertRelayRoute },
  { id: "request-contact", method: "POST", path: "/api/bersoncare/request-contact", source: registrarFiles.registerBersoncareRequestContactRoute },
  { id: "send-otp", method: "POST", path: "/api/bersoncare/send-otp", source: registrarFiles.registerBersoncareSendOtpRoute },
  { id: "reminder-rules", method: "POST", path: "/api/integrator/reminders/rules", source: registrarFiles.registerBersoncareReminderRulesRoute },
  { id: "settings-sync", method: "POST", path: "/api/integrator/settings/sync", source: registrarFiles.registerBersoncareSettingsSyncRoute },
  { id: "users-canonical-pair", method: "POST", path: "/api/integrator/users/canonical-pair", source: registrarFiles.registerBersoncareUserMergeM2mRoutes },
  { id: "users-merge", method: "POST", path: "/api/integrator/users/merge", source: registrarFiles.registerBersoncareUserMergeM2mRoutes },
  { id: "operator-health-probe", method: "POST", path: "/internal/operator-health-probe", source: registrarFiles.registerOperatorHealthProbeRoute },
  { id: "booking-lifecycle", method: "POST", path: "/api/bersoncare/booking/lifecycle-event", source: registrarFiles.registerBersoncareBookingLifecycleRoute },
  { id: "telegram-webhook", method: "POST", path: "/webhook/telegram", source: conditionalRegistrarFiles.registerTelegramWebhookRoutes },
  { id: "telegram-long-polling", method: "LOOP", path: "telegram getUpdates", source: "apps/integrator/src/integrations/telegram/longPolling.ts" },
  { id: "max-webhook", method: "POST", path: "/webhook/max", source: conditionalRegistrarFiles.registerMaxWebhookRoutes },
];
...
function assertRouteInventoryMatchesSource(loaded) {
  ...
  for (const registrar of knownRegistrars) {
    if (!mountedRegistrars.includes(registrar)) {
      fail(`${files.routes} no longer mounts ${registrar}; update C3 inventory/checker intentionally`);
    }
  }
  ...
  for (const route of discoveredRoutes) {
    const expected = expectedInventoryEntries.find(
      (entry) => entry.method === route.method && entry.path === route.path && entry.source === route.source,
    );
    if (!expected) {
      fail(`C3 inventory missing registered route ${route.method} ${route.path} from ${route.source}`);
    }
  }
...
if (process.argv.includes("--self-test")) {
  ...
  const doc = read(files.doc).replace("`POST /api/bersoncare/send-otp`", "`POST /api/bersoncare/send-otp-missing`");
