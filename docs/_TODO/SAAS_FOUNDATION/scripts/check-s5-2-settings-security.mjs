...
function requireFragments(label, source, fragments) {
  for (const fragment of fragments) {
    if (!source.includes(fragment)) fail(`${label} missing required fragment: ${fragment}`);
  }
}

function forbidFragments(label, source, fragments) {
  for (const fragment of fragments) {
    if (source.includes(fragment)) fail(`${label} contains forbidden fragment: ${fragment}`);
  }
}

const configReader = readFileSync(files.configReader, "utf8");
const grants = readFileSync(files.grants, "utf8");
if (configReader !== renderS5ConfigReaderSql()) fail(`${files.configReader} is not generator-synchronized`);
...
if (descriptors.get("public.app_runtime_settings_audit")?.scopingKind !== "bootstrap_runtime_audit") {
...
if (scripts["check:saas-s5-2-settings-security"] !== "node docs/_TODO/SAAS_FOUNDATION/scripts/check-s5-2-settings-security.mjs") {
  fail("package.json check:saas-s5-2-settings-security command is missing or changed");
}
if (scripts["smoke:saas-s5-2-settings-security"] !== "node docs/_TODO/SAAS_FOUNDATION/scripts/smoke-s5-2-settings-security.mjs") {
