...
const expected = new Map([
  ["public.comments", ["exercise", "lfk_complex", "test", "test_set", "recommendation", "lesson", "stage_item_instance", "stage_instance", "program_instance"]],
  ["public.patient_home_block_items", ["content_page", "content_section", "course", "static_action"]],
  ["public.treatment_program_template_stage_items", ["exercise", "recommendation", "lesson", "clinical_test"]],
  ["public.treatment_program_instance_stage_items", ["exercise", "recommendation", "lesson", "clinical_test"]],
...
function parseTsv(text) {
  ...
  return lines.map((line, index) => {
    ...
    if (cols.length !== headers.length) {
      throw new Error(`${artifactPath}:${index + 2} expected ${headers.length} columns, got ${cols.length}`);
    }
...
function assertSchemaChecksMatchArtifact(rows) {
  ...
  const checks = new Map([
    ["public.comments", extractCheckValues(schema["apps/webapp/db/schema/entityComments.ts"], "comments_target_type_check")],
    ["public.patient_home_block_items", extractCheckValues(schema["apps/webapp/db/schema/schema.ts"], "patient_home_block_items_target_type_check")],
    ["public.treatment_program_template_stage_items", extractCheckValues(schema["apps/webapp/db/schema/treatmentProgramTemplates.ts"], "item_type = ANY")],
    ["public.treatment_program_instance_stage_items", extractCheckValues(schema["apps/webapp/db/schema/treatmentProgramInstances.ts"], "item_type = ANY")],
    ["public.material_ratings", extractCheckValues(schema["apps/webapp/db/schema/materialRatings.ts"], "material_ratings_target_kind_check")],
    ["public.treatment_program_events", extractCheckValues(schema["apps/webapp/db/schema/treatmentProgramEvents.ts"], "treatment_program_events_target_type_check")],
  ]);
...
function assertNoItemRefFk() {
  ...
  if (/itemRefId:[\s\S]{0,120}\.references\(/.test(allSchema) || /item_ref_id[\s\S]{0,120}FOREIGN KEY/i.test(allSchema)) {
...
function runChecks(overrides = {}) {
  ...
  const extraTables = [...new Set(rows.map((row) => row.table))].filter((table) => !expected.has(table));
