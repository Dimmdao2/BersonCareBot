...
function runAppSmoke() {
  ...
  if (result.status !== 0) {
    throw new Error(`app dormant smoke failed: pnpm ${args.join(" ")} exited with ${result.status ?? "unknown status"}`);
  }
}

if (process.argv.includes("--self-test")) {
  const checklist = read(files.checklist).replace("- [x] Dev-bypass still works in development.", "- [ ] Dev-bypass still works in development.");
