...
function assertP0102UserReferenceTierGuard({ descriptors = buildRlsDescriptors() } = {}) {
  ...
  for (const [table, reasons] of userRefReasonsByTable.entries()) {
    ...
    if (descriptor.tier === "BOOTSTRAP" && !descriptor.source) {
      undocumentedAllowed.push({ table, tier: descriptor.tier, reason: "missing BOOTSTRAP source" });
    }

    if (descriptor.tier === "LEGACY" && descriptor.source !== "legacy_frozen_until_sunset") {
      undocumentedAllowed.push({ table, tier: descriptor.tier, reason: "LEGACY source must be legacy_frozen_until_sunset" });
    }

    if (descriptor.tier === "SCOPED" && !descriptor.source && !descriptor.sourceStage) {
      undocumentedAllowed.push({ table, tier: descriptor.tier, reason: "SCOPED descriptor must declare source/sourceStage" });
    }
  }
  ...
  if (unsupported.length > 0) {
    const details = unsupported
      ...
      .map(({ table, tier, reasons }) => `${table} (${tier}) -> ${Array.from(reasons).sort().join("; ")}`)
      .join("\n");

    fail(`Unsupported platform_users reference tier(s). User refs may be SCOPED/BOOTSTRAP/LEGACY only:\n${details}`);
  }
  ...
  return {
    ...
    scoped: Array.from(userRefReasonsByTable.keys()).filter((table) => descriptors.get(table)?.tier === "SCOPED").length,
    bootstrap: Array.from(userRefReasonsByTable.keys()).filter((table) => descriptors.get(table)?.tier === "BOOTSTRAP").length,
    legacy: Array.from(userRefReasonsByTable.keys()).filter((table) => descriptors.get(table)?.tier === "LEGACY").length,
  };
}
...
function runSelfTest() {
  ...
  expectFailure(
    ...
    cloneDescriptorsWithTierOverride({ table: "public.content_section_slug_history", tier: "BOOTSTRAP" }),
    /must remain SCOPED/,
  );
