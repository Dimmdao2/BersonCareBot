...
function requireBlockFragments(label, text, startFragment, endFragment, fragments) {
  const startIndex = sourceTextIndexOf(text, startFragment, label);
  if (startIndex < 0) fail(`${label} missing block start: ${startFragment}`);
  const block = sourceTextSliceBetween(text, startFragment, endFragment, label, true);
  if (block === null) fail(`${label} missing block end after start: ${endFragment}`);
...
function assertCleanupIsNotBestEffort(testDeploySaasText) {
  const cleanupBlocks = [
    ...
    [
      'cleanup_elevation',
      getBlock('cleanup_elevation', testDeploySaasText, '\ncleanup_elevation(){', '\n}'),
    ],
...
function assertPreExistingMigratorMembershipFails(testDeploySaasText) {
  ...
  requireOrderedFragments(
    `${files.testDeploySaas} pre-existing migrator membership residue guard`,
    grantBlock,
    [
...
function assertBootstrapOldHistorySeedNull(rehearsalText) {
  ...
  for (const match of insertMatches) {
    ...
    if (!bootstrapTuple) {
      fail(
        `${files.rehearsal} has bootstrapOldHistoryId outside a user_phone_history VALUES tuple`,
      );
    }
...
if (process.argv.includes('--self-test')) {
  const cases = [
    ...
    {
      forceSql: read(files.forceSql).replace(
        'phase4_bootstrap_base_role_user_phone_history_dml',
        'phase4_missing',
      ),
    },
    ...
    {
      rehearsal: read(files.rehearsal).replaceAll(
        'runWithDbBootstrapPrincipal',
        'runWithoutBootstrapPrincipal',
      ),
    },
    {
      rehearsal: read(files.rehearsal).replaceAll(
        'fb1_bootstrap_org_pii_hidden_ok',
        'fb1_bootstrap_org_pii_missing',
      ),
    },
    {
