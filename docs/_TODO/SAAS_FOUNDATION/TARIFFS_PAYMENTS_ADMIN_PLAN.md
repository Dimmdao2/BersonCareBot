...
- [ ] **4b.2** Ручка «неоплата» — лестница из этапа 2, на кабинет и на каждую механику. Конечных состояния два:
      `только чтение` и `выключено`. Значение `скрыто` не заводить: выключено и означает «раздела нет».
- [ ] **4b.3** Ручка «переход на тариф меньше», значения по классу механики. Числовые: `не давать переход` ·
...
- [ ] **4b.4** Вычистить из кода и планов правила по конкретным сущностям («пациентов только при создании»,
...
- [ ] **4b.5** Свойства механизма, которые НЕ настраиваются и обязаны выполняться при любых значениях: данные не
