...
#### Этап 4b. Три ручки — механизм вместо захардкоженной политики (владелец 30.07)

- [ ] **4b.1** Ручка «что делать при достижении квоты» у каждой числовой механики: `не создавать новое` ·
      `только чтение` · `скрыть`. Плюс необязательный порог предупреждения.
- [ ] **4b.2** Ручка «что делать при неоплате» — это уже спроектированная лестница (этап 2): на кабинет и на каждую
      механику отдельно, четыре поля. Свести терминологию в одно место, чтобы ручка называлась одинаково везде.
- [ ] **4b.3** Ручка «что делать при переходе на тариф меньше при переборе»: `не давать переход` ·
      `дать переход, заморозить рост` · `дать переход, лишнее в только чтение`.
- [ ] **4b.4** Вычистить из кода и планов все правила по конкретным сущностям («пациентов только при создании»,
...
- [ ] **5.2** Число пациентов: само число + три ручки (см. §5 канона). Поведение на пределе, при неоплате и при
      понижении НЕ хардкодить — это значения владельца. Архивирование освобождает место (свойство механизма).
- [ ] **5.3** Число филиалов: число + три ручки, значения ставит владелец.
- [ ] **5.4** Объём файлов в байтах: атомарная проверка суммы при загрузке + три ручки. Значение владельца по
      ручке 3 уже названо 30.07: переход не блокировать, загрузку заморозить.
