...
- [ ] **6a.4** `SAAS_S4_TARIFFS_STORE_ENTITLEMENTS.md` (713 строк, 83 открытых чекбокса) — та же тема, старше. Свести
      нельзя вслепую: часть его пунктов уже сделана, часть отменена решениями владельца 30.07, часть жива. Нужен проход
      сверки построчно: каждый из 83 пунктов помечается «сделано (доказательство)», «отменено решением владельца
      (ссылка)» или «живо → переезжает в §5a». Только после этого файл архивируется с надгробием. Пока не сделано —
      файл остаётся, но в его шапке стоит предупреждение о приоритете §5a.
