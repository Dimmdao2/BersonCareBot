...
> `d1b8a1718`). Phase 5 (интеграционная приёмка) и §5a (квоты/механики/лестница) — **§5a этапы 1–6 закрыты
> (#1069, 05.08); открыты только 7.3/7.4 (owner-приёмка на TEST)**. Phase 5 fixtures/full CI — частично открыт.
...
- [x] **Т10. Поведение после завершения ОПЛАЧЕННОГО периода — одна общая настройка.** Владелец 04.08: «я
      ...
      **оплаченного** периода, это другое событие, и своей настройки у него сейчас нет. Проверить при работе,
      не расползлось ли это поведение по механикам так же, как в Т1, и свести в одно место.

      **Доказательство (#1069 T10):** `saas_paid_period_policy` + `set_paid_period_policy` в
