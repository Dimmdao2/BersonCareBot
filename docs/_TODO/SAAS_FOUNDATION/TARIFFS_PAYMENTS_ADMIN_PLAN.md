...
- [ ] **4b.3** **ЧАСТИЧНО, повторно открыто сверкой кода 20.08:** значения разделены по классу механики и
      сохраняются как данные (`a60a39122`, миграция `0281`); для объёма файлов стоит именно `заморозить рост`.
      Но вариант возможности `оставить только чтение` не реализует обещанное поведение: evaluator лишь не блокирует
      переход, после назначения целевой тариф имеет `mechanics[mechanic] = false`, а SQL-дверь возвращает `disabled`.
      Поэтому `read_only` сейчас совпадает с `disable_immediately`. Ручка «переход на тариф меньше», значения по классу механики. Числовые: `не давать переход` ·
