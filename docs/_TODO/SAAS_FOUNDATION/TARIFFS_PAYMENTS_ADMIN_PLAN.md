...
- [ ] **3.1c** ⚠️ РЕШЕНИЕ ЛИДА 30.07 о шве для интегратора. Воркер отказался закрывать находку №1 и был прав: у
      ...
      **каноническая функция в схеме `public`**, а не общий пакет и не межпроцессный HTTP. Провенанс, чтобы не спорить
      ...
      ОДНОЙ базе, а `apps/webapp/INTEGRATOR_CONTRACT.md:5,16` прямо говорит: прежнее «никаких прямых чтений базы между
      сервисами» относилось к временам РАЗНЫХ баз, а при единой интегратор читает и пишет `public`, а HTTP остаётся
      только для того, что действительно является контрактом на границе процессов. То есть HTTP здесь не отвергнут «по
      вкусу» — он не тот инструмент, и старое поведение через API мы не возвращаем. Дверь — обычный SQL-вызов из обоих
      процессов: в репозитории этот приём уже применён
