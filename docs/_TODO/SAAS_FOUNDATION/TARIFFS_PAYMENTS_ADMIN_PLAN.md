...
| **Р-17**       | Владелец 19.08 принял выводы исследования мировой практики (`SEAT_INVOICE_WORLD_PRACTICE_2026-08-19.md`) целиком: «окей, я принимаю, всё». Три следствия, ЗАМЕНЯЮЩИЕ соответствующие части Р-15: (1) пропорция считается ОТ МОМЕНТА добавления места до конца периода, а не от начала суток — так делают все пять изученных биллингов, от суток не считает никто; (2) срок жизни счёта — длительность ОТ ВЫСТАВЛЕНИЯ, а не «до конца суток»; просроченный ОТМЕНЯЕТСЯ и выставляется новый, а не продлевается (практики «до конца суток» и «до шести утра» не существует ни в одном первоисточнике); (3) место открывается СРАЗУ, пропорция уходит в следующий счёт — норма самообслуживания у Stripe, Slack, Atlassian, Google Workspace. Пункт (3) отменяет «новый сотрудник разрешается только после оплаты счёта» из Р-15. Пояс: практика подтвердила Р-16 — биллинг считает в UTC и показывает в ОДНОМ поясе продавца, личный пояс человека на денежный путь не пускают нигде. | 19.08 | 🔄 не сделано: ветка `wt/seat-invoice-20260819` построена по прежней редакции Р-15 | 5.6 |
