...
⛔ Поэтому: **не переиспользовать `grace_days`/`grace_ends_at` под льготный период.** Либо существующее продление
триала остаётся отдельным понятием и получает честное имя, либо владелец решает, что продление больше не нужно —
но молча менять смысл поля нельзя: от него зависит, что человек видит в день окончания триала.
