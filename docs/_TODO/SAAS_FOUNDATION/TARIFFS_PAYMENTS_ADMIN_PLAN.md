...
| **Р-5** | «нет доступа и нет никаких механик вне тарифа; либо есть закончившийся тариф, либо отсутствует регистрация; при регистрации сразу даётся тариф с триалом, который настраивается в админке» | 31.07 | ⏳ режется веткой Р-12 | 2.13 |
| **Р-6** | «просто сразу требуется выбор тарифа и оплата»; повторный триал не выдаётся — «перестала платить, вернулась — нет ни активного тарифа, ни триала уже повторного» | 31.07 | ⏳ | 2.13 |
...
| **Р-10** | «при оплате тарифа все настройки оплаченного тарифа фиксируются на оплаченный период для конкретной клиники. Не важно что меняется после - клиника уже оплатила и пока оплата не закончилась - имеет доступ к оплаченному» | 01.08 | 🔄 круг 2: числа читались мимо снимка | 2.12 |
...
| **Р-12** | «Совместимость это бред - вырезай нахуй полностью»; «Клиника зарегистрирована - получает либо триал либо отправляется на оплату тарифа - это настраивается в админке»; «без триала - бред»; «единственное что надо это какой выбран или назначен тариф. триал - это не состояние клиники, это состояние доступа к тарифу без реальной оплаты» | 01.08 | 🔄 в работе | 2.13 |
...
- [ ] 🔴 **2.13 ЧЕТЫРЁХ СОСТОЯНИЙ ДОСТУПА НЕ СУЩЕСТВУЕТ — ВЫРЕЗАТЬ.** Решения **Р-12** (основное), **Р-5** и **Р-6** реестра §5а-0. Дословные формулировки — там.
...
- [ ] 🔴 **2.12 ОПЛАЧЕННЫЙ ПЕРИОД ЗАМОРАЖИВАЕТ УСЛОВИЯ ТАРИФА.** Решения **Р-10** и **Р-11** реестра §5а-0. Дословные формулировки — там.
