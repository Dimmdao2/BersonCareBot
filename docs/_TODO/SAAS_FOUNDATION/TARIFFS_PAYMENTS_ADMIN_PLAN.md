...
- [ ] **3.1d** ⚠️ НАХОДКА ВЛАДЕЛЬЦА 30.07: вызов двери у интегратора написан СЫРЫМ SQL. Факты, проверенные по коду:
      вызов идёт через порт (`DbPort.query`), но это сырой запрос — `apps/integrator/src/infra/db/directPublic/
      writeDiaryLfkDirect.ts:171`; вся семья `directPublic` (семь файлов) написана так же, сырым SQL, drizzle в ней не
      используется НИ В ОДНОМ файле, при том что drizzle у интегратора есть (`apps/integrator/package.json:41`,
      `infra/db/drizzle.ts`, `integratorDrizzleSchema.ts`). `AGENTS.md:493` запрещает сырой SQL для НОВОГО кода.
      Требуется: вызов двери перевести на drizzle-порт интегратора. Остальную семью `directPublic` в этой работе НЕ
      трогать — это отдельный долг, и переписывать её заодно значит расширять скоуп молча.
- [ ] **3.1e** ⛔ ВОПРОС ВЛАДЕЛЬЦУ, не решать самому: почему запись дневника и ЛФК вообще живёт в интеграторе
      (`writeDiaryLfkDirect`). Владелец 30.07: «это похоже на устаревшую историю с дневником и лфк в интеграторе… и при
      чём тут diarylfk?». Факт: файл живой, на него ссылается исполнитель сценариев (`executeAction.ts:419,2836,3174`) и
      соседний прямой писатель напоминаний. Мой вызов двери я прицепил именно туда, потому что аудит нашёл ВТОРОЙ
      резолвер жизненного цикла ровно в этом файле — то есть я лечил симптом там, где он проявился. Если запись дневника
      из интегратора — наследие и должна уйти в вебапп, то и дверь там не нужна, и правильный фикс другой. До ответа
      владельца путь не переносим и не удаляем.
