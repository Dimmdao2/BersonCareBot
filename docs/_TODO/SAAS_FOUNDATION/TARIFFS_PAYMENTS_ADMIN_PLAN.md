...
- [ ] 🔴 **2.13 ЧЕТЫРЁХ СОСТОЯНИЙ ДОСТУПА НЕ СУЩЕСТВУЕТ — ВЫРЕЗАТЬ.** Решения **Р-12** (основное), **Р-5** и **Р-6** реестра §5а-0. Дословные формулировки — там.
      ...
      **СОСТОЯНИЕ 01.08 — поведение закрыто, три хвоста остались.** Код сведён (`b0a5dce7e` → `ff2be7ef4`),
      ...
      **WORKER 02.08 (#1069):** migration `0305_tariff_snapshot_access_doors_local.sql` возвращает
      `app.saas_billing_effective_tariff(...)` во все три двери после регрессии `0297`; удаление четырёх
      legacy states и тела квот сохранены. Journal синхронизирован. Пункт остаётся открытым: лид применяет
      migration только на DEV через `migrate-dev.sh` и повторяет ровно три сценария красного DEV oracle.
