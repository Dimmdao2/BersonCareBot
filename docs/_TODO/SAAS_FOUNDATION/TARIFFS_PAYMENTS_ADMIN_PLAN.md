...
| **Р-15**       | **Действующая редакция (19.08, после принятия мировой практики).** Место открывается СРАЗУ, пропорциональная доплата уходит в следующий счёт — норма самообслуживания у Stripe, Slack, Atlassian, Google Workspace. Пропорция считается ОТ МОМЕНТА добавления места до конца оплаченного периода, не от начала суток: так делают все пять изученных биллингов, от суток не считает никто. ⛔ ОТМЕНЕНО 20.08 (Р-19): ~~Счёт живёт заданную ДЛИТЕЛЬНОСТЬ ОТ ВЫСТАВЛЕНИЯ; просроченный ОТМЕНЯЕТСЯ и выставляется новый с пересчитанной суммой, а не продлевается.~~ Срок у счёта ОДИН — конец периода; дальше Р-18. Отдельным счётом место оплачивается один раз, на момент открытия; со следующего периода стоимость входит в общий счёт — «никто не хочет оплачивать десять тысяч счетов одновременно». Точка отсчёта времени — пояс приложения (Р-16). ⛔ Решать «можно ли продать место и почём» обязано ОДНО место в коде, а не две двери с разными ответами (§5). Основание: `SEAT_INVOICE_WORLD_PRACTICE_2026-08-19.md`, владелец 19.08 — «окей, я принимаю, всё». | 19.08 | 🔄 не сделано; ветка `wt/seat-invoice-20260819` СНЯТА С СВЕДЕНИЯ 20.08 — её посылка отменена дважды: расчёт от начала суток и срок «до конца суток» сняты действующей редакцией Р-15 и Р-19, «отмена закрывает место» снята Р-17. ⚠️ СУЖДЕНИЕ АГЕНТА, НЕ РЕШЕНИЕ ВЛАДЕЛЬЦА (помечено 20.08, владелец: «это ты писал»): ~~Мержить нечего, работу надо делать заново по действующим строкам.~~ Замерено 20.08: ветка меняет 24 файла, +1912/−561, из них строк, завязанных на отменённую посылку — 32, строк про одну дверь — 27. Отменённое это доля, а не вся ветка, поэтому формулировка «делать заново» была сильнее, чем факты позволяют. Действующий курс — влить ветку с ручным разведением конфликтов (ветка и feat правили одни и те же функции в `pgSaasBilling.ts`/`inMemorySaasBilling.ts`/`ports.ts`) и вычистить отменённое. **Дефект при этом ЖИВОЙ в feat:** строку счёта `seat_overage` собирают ДВЕ двери — `app/api/clinic/billing/seatOveragePurchase.ts` и `app/api/clinic/invites/route.ts`, обе через `modules/saas-billing/seatOverageQuote.ts`, и на кончившемся оплаченном периоде они отвечают по-разному: одна отказывает, вторая выставляет полный месячный тариф за ноль оставшихся дней со сроком услуги, кончающимся раньше начала. Готовый структурный гейт против второй двери лежит в снятой ветке (`scripts/check-seat-overage-single-door.mjs`, 212 строк) и переносится вместе с работой, а не отдельно: он требует двери `decideSeatOverage`, которой в feat нет | 5.6 |

| **Р-16**       | «Давай по Москве всё делать. В конце концов это столица России, приложение российское, сделано для российских клиник. Рабочее время практически у всех организаций дневное — даже если это Новосибирск. Если счёт действует до нуля часов сегодняшнего дня, значит уж до шести часов они должны оплатить. Если не оплатили — на следующий день новый счёт, значит сегодня у них специалист не работал. Что усложняется? Зачем усложняются?» ⛔ Уточняет Р-15: «конец суток» — это конец суток по МОСКВЕ, единая точка отсчёта для всей работы, за которой не стоит человек с экраном (срок счёта, сводки, окна хранения). Пояс организации под это НЕ заводится. | 19.08 | 🔄 в работе: ветка `wt/seat-invoice-20260819` считает по глобальной настройке пояса — привести к московским суткам явно | 5.6 |
