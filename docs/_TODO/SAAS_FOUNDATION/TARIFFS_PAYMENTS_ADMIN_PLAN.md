...
- [x] **Т9. Периоды оплаты задаются в тарифе, а не в коде; «день» как период — убрать.** Владелец 04.08,
      ...
      **Доказательство (#1069 T9):** каталог `saas_billing_periods` + FK на тарифах; `day` → `month` на
      ...
      ⛔ **Т9 заменено Т14 (05.09, #1069, `docs/OWNER_DECISIONS.md`).** «Период — характеристика тарифа»
      читалось как per-tariff набор; владелец переопределил модель на ОДНУ глобальную сетку периодов,
      общую для всех тарифов, с полной матрицей цена×период на каждом активном тарифе.
      `saas_tariffs.price_minor`/`billing_period` больше не источник цены — только замороженное легаси-поле,
      из которого миграция `20260905T120000_one_global_billing_period_price_matrix.sql` бэкфиллит
      нормализованную `saas_tariff_period_prices (tariff_id, billing_period_code)`. Подписки хранят
      текущий/отложенный `billing_period_code` парой с тарифом (составные FK на эту таблицу).
...
- [x] **4b.3** ✅ **ЗАКРЫТО 02.09 после независимого FAIL и correction-pass** (`8571d5311` → audit/oracle `7dbb1a38a` → fix `1a6b94194`). Единственная DB-дверь теперь различает сохранённые варианты capability downgrade: `read_only` сохраняет чтение и запрещает запись; `disable_immediately`, отсутствие и неизвестное значение дают fail-closed `disabled`. Исправление не заменяет функцию старым телом: миграция якорно меняет фактически действующее определение через `pg_get_functiondef` и отказывает при отсутствующем/неоднозначном якоре, поэтому сохраняет более поздние global paid-period и integrator-context слои. Owner-aware preflight и полная матрица выполнены rollback-only; постоянного применения на DEV/TEST не было. Оракулы global paid-period `3/3`, integrator roots `1/1`, targeted Vitest `97/97`, migration/static/generated gates и typecheck — PASS.
- [x] **4b.4** ✅ **ЗАКРЫТО тем же принятым проходом.** Поведение выбирают класс механики и сохранённые данные, без веток по имени: два допустимых capability-значения дают два разных состояния; числовые и местовая механики не попадают под capability-политику; чужая организация получает `42501`; данные не удаляются, а возврат на больший тариф восстанавливает доступ. Acceptance-оракул: `deploy/postgres/privileges/downgrade-capability-policy.devDbProof.test.mjs`.
...
- [x] **5.6** Правила смены тарифа (см. Р-14): повышение сразу; понижение с начала следующего расчётного периода и только после
      ...
      **РЕШЕНИЕ ВЛАДЕЛЬЦА 30.07 — объём файлов выведен из этого правила.** Дословно: «при понижении не блокирует
      ...
      ⛔ **Пропорциональная доплата при повышении (Р-14, денежная формула 03.08) заменена Т14 (05.09,
      #1069).** Владелец 05.09: смена тарифа/периода вступает в силу ТОЛЬКО на границе оплаченного периода
      (`currentPeriodEndsAt`) — никакого немедленного/пропорционального апгрейда. `proration.test.ts` и
      немедленный upgrade-путь — устаревшая модель, не активная авторитетность.
