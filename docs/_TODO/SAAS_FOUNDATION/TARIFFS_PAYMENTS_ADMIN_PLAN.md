...
- [ ] **Т8. Как задаётся скидка.** Владелец назвал два варианта — процент со скидкой и округлением, либо точная
      ...
      даёт хвосты, которые потом всё равно округляют руками. Открытый вопрос владельцу — нужен ли ГЛОБАЛЬНЫЙ
      процент как запасной вариант для тарифов, у которых льготная цена не задана (иначе её придётся проставлять
      каждому тарифу вручную).
