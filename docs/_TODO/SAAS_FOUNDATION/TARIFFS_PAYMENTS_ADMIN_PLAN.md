...
| **Р-19**       | «короче перевыставление — бред, убирать» (20.08). ⛔ Отменяет перевыставление счёта за место ЦЕЛИКОМ — и кнопку администратора, и порт, и маршрут `POST …/reissue`. Основание, разобранное с владельцем в том же обмене: у перевыставления не осталось ни одного сценария. Сумма при нём по Р-17 не пересчитывается, значит преемник — копия того же счёта. «Счёт протух раньше периода» не существует: владелец 20.08 дословно — «У него срок до конца период, потом он протух и долг включился в стоимость следующего периода (и там по счету либо автосписание, либо при неуспехе — счет ждет ручную оплату). Доступ при этом по правилам платформы (сколько-то только чтение, потом блок)». То есть срок у счёта ОДИН — конец периода, и дальше работает Р-18, где перевыставление не участвует. | 20.08 | 🔄 к исполнению: снять маршрут `reissue`, метод порта `reissueSeatOverageInvoice`, кнопку в `PlatformPaymentsSection.tsx`; `reissueWithSuccessor` в `invoiceOperations.ts` остаётся, пока его использует захват оплаты | — |
