...
- [ ] **Т2. Триггеры уведомлений неполные.** Сейчас их два — успешная и неуспешная оплата. Нужны ещё три, и
      названы они ради маркетинга: **старт триала**, **завершение триала**, **регистрация — то есть первый вход
      в кабинет**.
- [ ] **Т3. Шаблоны писем негде править, и место для них не то.** Владелец: «не вижу места где правятся
      шаблоны. Я бы вообще настройку этих рассылок вынес в отдельную вкладку и правил там через полноценный
      редактор — письма для маркетинга должны быть красивыми». То есть отдельная вкладка + полноценный
      редактор, а не поле в форме настроек.
...
- [x] **Т10. Поведение после завершения ОПЛАЧЕННОГО периода — одна общая настройка.** Владелец 04.08: «я
      ...
      **Доказательство (#1069 T10):** `saas_paid_period_policy` + `set_paid_period_policy` в
      ...
      `CommercialConstructorClient.tsx`.
...
- [ ] **Т7. Два новых триггера уведомлений: льготный период начат / завершён.** Срабатывают **только если
      оплаты нет** — «тогда я могу слать письма с предложением о покупке со льготой и уведомление о том что
      скидка скоро закончится, но только тем кто ещё не купил после завершения триала». Идут в один список с
      триггерами из Т2 (старт триала, завершение триала, регистрация).
