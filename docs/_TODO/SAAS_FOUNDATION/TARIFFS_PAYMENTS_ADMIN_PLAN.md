...
- [x] **2.1** ~~Поля лестницы в тарифе на ДВУХ уровнях: система целиком и каждая механика отдельно.~~
      ...
      **Действует сейчас:** системная лестница («Доступ к системе») наследуется всеми механиками; отдельная полная
      лестница у механики — только явное opt-in-исключение в `mechanic_access_policies` (UI: «Исключения по
      механикам»). См. **Т1** ниже. Инвариант §2.1c (оплата тарифа клиникой вне лестницы) не отменён.
...
- [ ] **Т1. Доступ к системе управляет всеми вложенными механиками.** ✅ **Подтверждено владельцем 05.08 (#1069):**
      T1 — поздний и верный канон; §5a **2.1** «поля лестницы на каждой механике по умолчанию» не поднимать как
      актуальное требование. Сейчас настройки «как работает доступ по
      завершении периода» и «уведомления по триггерам» продублированы У КАЖДОЙ механики отдельно. Владелец:
      «это не логично и избыточно… Доступ к системе определяет работу всех вложенных механик (я думаю так и
      работает вся индустрия) — это логично и снижает нагрузку вычислений доступа». Убрать дублирование:
      системный доступ задаёт поведение, механика наследует; собственная настройка у механики остаётся только
      там, где без неё нельзя, и тогда это должно быть видно как исключение.
...
- [x] **Т4. «Тариф триала» против «человек выбирает тариф сам» — разобрано 03.08, поведение верное, экран
      врёт.** Вопрос владельца: что будет, если разрешить выбор тарифа, а человек выберет не тот, что указан в
...
- [ ] **Т9. Периоды оплаты задаются в тарифе, а не в коде; «день» как период — убрать.** Владелец 04.08,
