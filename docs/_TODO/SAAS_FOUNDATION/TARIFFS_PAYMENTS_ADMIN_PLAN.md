...
> открыт сверкой кода 20.08:** после закрытия Т10, Т13 и 4.8 автономно остаются `4b.3/4b.4` (развести варианты
> downgrade) и полный двухклинический regression sweep Phase 2; `4a.6`, `7.3` и `7.4` требуют живой
> owner-приёмки. Phase 5 full CI — частично открыт.
> **02.09:** `4b.3/4b.4` были объявлены закрытыми, но независимый аудит того же дня вернул **FAIL** и открыл
> их обратно — миграция кандидата откатывает две уже применённые миграции той же функции
> (`docs/_TODO/SAAS_FOUNDATION/AUDIT_TARIFF_DOWNGRADE_CAPABILITY_2026-09-02.md`). **Тем же днём — correction-pass:**
> миграция переписана на якорную замену действующего определения (приём 0820/0823), проверена owner-aware
> rollback-only preflight'ом и ручной rollback-wrapped пробой с фикстурами аудита; см. пометки у 4b.3/4b.4 ниже.
> Постоянно на DEV/TEST не применялась — независимая приёмка (§24.5) ещё не пройдена, чекбоксы остаются `[ ]`.
> Остаток строки (regression sweep Phase 2, `4a.6`/`7.3`/`7.4`) не тронут.
...
> commit/file:line/перепрогнанный зелёный тест — большинство Phase 1-3), **24 оставались открытыми** `[ ]`
> (в основном Phase 4/5 целиком + два открытых пункта внутри Phase 3 — trial policy
...
> чекера.
>
> **OWNER DECISION 2026-08-20:** поведение всей системы после окончания оплаченного периода и неоплаты следующего
> периода настраивается **в одном глобальном месте для всех тарифов**. Отдельные `downgradePolicies` по механикам
> убрать из интерфейса. Глобальная настройка обязана реально управлять кабинетом, всеми включёнными механиками и
> всеми mutation-path; скрытые per-mechanic compat-значения не являются источником поведения неоплаты.
>
...
| **Р-2**        | «переменная даёт цифру и число. Какими словами это видит клиника, решает шаблон»                                                                                                                                                                                                                                                              | 31.07 | ✅ сделано (#1069 `2.6a`, `5eb1789d4`): шаблоны уведомлений + cabinet warnings; лестница T1 закрыта (`20ede5edc`)                                                                                            | 2.6a       |
...
| **Р-14 / D-9** | «в мире понижение вступает в силу со следующего платёжного цикла — конечно, в этом цикле тариф уже оплачен». Повышение действует сразу; понижение — с начала следующего расчётного периода. Решение владельца 03.08: при повышении клиника доплачивает разницу цен пропорционально оставшейся части текущего оплаченного периода; новый тариф и его снимок включаются сразу, следующий полный период оплачивается целиком по новому тарифу. | 30.07, 03.08 | ✅ fixer `314eed328`: зелёный сохранённый oracle | 5.6        |

| **Р-15**       | **Действующая редакция (19.08, после принятия мировой практики).** Место открывается СРАЗУ, пропорциональная доплата уходит в следующий счёт — норма самообслуживания у Stripe, Slack, Atlassian, Google Workspace. Пропорция считается ОТ МОМЕНТА добавления места до конца оплаченного периода, не от начала суток: так делают все пять изученных биллингов, от суток не считает никто. ⛔ ОТМЕНЕНО 20.08 (Р-19): ~~Счёт живёт заданную ДЛИТЕЛЬНОСТЬ ОТ ВЫСТАВЛЕНИЯ; просроченный ОТМЕНЯЕТСЯ и выставляется новый с пересчитанной суммой, а не продлевается.~~ Срок у счёта ОДИН — конец периода; дальше Р-18. Отдельным счётом место оплачивается один раз, на момент открытия; со следующего периода стоимость входит в общий счёт — «никто не хочет оплачивать десять тысяч счетов одновременно». Точка отсчёта времени — пояс приложения (Р-16). ⛔ Решать «можно ли продать место и почём» обязано ОДНО место в коде, а не две двери с разными ответами (§5). Основание: `SEAT_INVOICE_WORLD_PRACTICE_2026-08-19.md`, владелец 19.08 — «окей, я принимаю, всё». | 19.08 | 🔄 ветка `wt/seat-invoice-20260819` влита в `wt/seat-door-20260820` (`docs/REPORTS/SEAT_SINGLE_DOOR_2026-08-20.md`) ручным разведением конфликтов, отменённое вычищено: перевыставление (Р-19) и «отмена закрывает место» (Р-17) убраны целиком из `ports.ts`/`service.ts`/`pgSaasBilling.ts`/`inMemorySaasBilling.ts`; единственная дверь `decideSeatOverage` (`modules/saas-billing/seatOverage.ts`) больше не считает `invoiceValidityDays`/`invoiceExpiresAt` — срок счёта за место равен `servicePeriodEndsAt`. Структурный гейт `scripts/check-seat-overage-single-door.mjs` перенесён и подключён в lint. Пояс (Р-16) при этом остался НЕ заведён явно — суток в расчёте нет вовсе (см. статус Р-16 ниже), см. отчёт о причине. Живой прогон на TEST/приёмка владельца ещё НЕ проводились | 5.6 |

| **Р-16**       | «Давай по Москве всё делать. В конце концов это столица России, приложение российское, сделано для российских клиник. Рабочее время практически у всех организаций дневное — даже если это Новосибирск. Если счёт действует до нуля часов сегодняшнего дня, значит уж до шести часов они должны оплатить. Если не оплатили — на следующий день новый счёт, значит сегодня у них специалист не работал. Что усложняется? Зачем усложняются?» ⛔ Уточняет Р-15: «конец суток» — это конец суток по МОСКВЕ, единая точка отсчёта для всей работы, за которой не стоит человек с экраном (срок счёта, сводки, окна хранения). Пояс организации под это НЕ заводится. | 19.08 | 🔄 к счёту за место больше НЕ ОТНОСИТСЯ: в действующей редакции Р-15 суток в расчёте нет вовсе (остаток считается от момента, срок счёта — длительность), поэтому пояс с денежного пути убран целиком (`3f3373d6b`). Для остальной работы «за которой не стоит человек с экраном» — сводки, окна хранения — решение в силе и не проверялось | 5.6 |

| **Р-17**       | «отмена неоплаченного счёта администратором — это с чего бы его отменять? как делается у других — разве они дают админу просто отменить счет? Может перевыставить его». ⛔ Уточняет Р-15 в части «просроченный ОТМЕНЯЕТСЯ»: отмена как САМОСТОЯТЕЛЬНОЕ действие оператора над счётом за место отменяется — у неё нет применения. Гашение остаётся только ТЕХНИЧЕСКОЙ частью перевыставления, у которого есть преемник: аннулирование БЕЗ преемника означает «долга не было», что для оказанной услуги ложь. Сумма при перевыставлении не пересчитывается «на сейчас». Это НЕ спор с формулой Р-15 «новый с пересчитанной суммой»: отрезок услуги у преемника тот же, поэтому пересчёт от начала этого отрезка даёт ровно прежнее число. Пересчёт от момента ПЕРЕВЫСТАВЛЕНИЯ был бы скидкой за просрочку — та самая находка F-B слепого аудита (место дешевело до 3,2 % цены после девяти перевыставлений). Основание: `SEAT_UNPAID_PRACTICE_2026-08-19.md` вопрос 2 (ни Stripe, ни ЮKassa, ни Slack/Atlassian/Google/Microsoft не дают оператору арендатора рядовой кнопки «отменить счёт»). | 19.08 | ⛔ ОТМЕНЕНО В ЧАСТИ ПЕРЕВЫСТАВЛЕНИЯ решением владельца 20.08 (см. Р-19). Действует только отказ отмены: маршрут отвечает `seat_invoice_not_cancellable`, вердикт один — `modules/saas-billing/invoiceOperations.ts`. Перевыставление удаляется целиком | 5.6 |

| **Р-19**       | «короче перевыставление — бред, убирать» (20.08). ⛔ Отменяет перевыставление счёта за место ЦЕЛИКОМ — и кнопку администратора, и порт, и маршрут `POST …/reissue`. Основание, разобранное с владельцем в том же обмене: у перевыставления не осталось ни одного сценария. Сумма при нём по Р-17 не пересчитывается, значит преемник — копия того же счёта. «Счёт протух раньше периода» не существует: владелец 20.08 дословно — «У него срок до конца период, потом он протух и долг включился в стоимость следующего периода (и там по счету либо автосписание, либо при неуспехе — счет ждет ручную оплату). Доступ при этом по правилам платформы (сколько-то только чтение, потом блок)». То есть срок у счёта ОДИН — конец периода, и дальше работает Р-18, где перевыставление не участвует. | 20.08 | ✅ сделано: маршрут `POST …/reissue` удалён (66 строк), метод порта `reissueSeatOverageInvoice` снят из `ports.ts`/`service.ts`/`pgSaasBilling.ts`/`inMemorySaasBilling.ts`, диалог и кнопка «Перевыставить» убраны из `PlatformPaymentsSection.tsx`, вердикт `saasBillingInvoiceReissueVerdict` снят из `invoiceOperations.ts` как более не вызываемый; `reissueWithSuccessor` оставлен — общий помощник упорядочивания преемника, которым продление периода гасит старый счёт (Р-18); typecheck+lint+тесты `saas-billing`/`saas-billing/payments` зелёные | 5.6 |

| **Р-18**       | «Если до конца периода счет не оплачен — делать его просроченным и включать долг в стоимость следующего периода: он либо автооплатится, либо весь доступ закрыт по правилам тарифов (как настроил глоб-админ)». ⛔ Второе предложение — ГРАНИЦА: правил доступа при неоплате эта строка не заводит, она пользуется уже существующими правилами тарифа. Место за уже прошедший период задним числом НЕ отбирается — услуга оказана. Долг входит в следующий счёт строкой (`carried_debt_minor`) и гасится этим счётом как преемником, иначе одна услуга осталась бы оплачиваемой дважды. Оценка «оплачен или нет» делается по КОНЦУ периода: досрочная оплата следующего периода долг не подхватывает. Основание: `SEAT_UNPAID_PRACTICE_2026-08-19.md` вопрос 1 (аналог состояния Stripe `unpaid`). | 19.08 | ✅ сделано, ветка `wt/invoice-reissue-20260819`: обе двери выставления считают долг одним правилом, `saasBillingPeriodAmountMinor` требует `carriedDebtMinor` обязательным полем | 5.6 |


**Отменённые формулировки — держать помеченными, не удалять:**
- Прежняя редакция Р-15 (19.08, до исследования практики) ОТМЕНЕНА им же: «счёт действует до конца суток», «новый сотрудник разрешается только после оплаты счёта» и расчёт от начала суток заменены на длительность от выставления, доступ сразу и пропорцию от момента добавления. Владелец принял мировую практику словами «окей, я принимаю, всё».
...
- [x] **2.1** ~~Поля лестницы в тарифе на ДВУХ уровнях: система целиком и каждая механика отдельно.~~
      ⛔ **УСТАРЕЛО КАК ДЕЙСТВУЮЩИЙ КАНОН — ЗАМЕНЕНО → T1 (03.08), подтверждение владельца 05.08 (#1069).**
      Историческая формулировка (31.07, закрыта аудитами `audit-stage2-final` / `audit-seam-final`): по каждому
      уровню владелец задаёт длительность терпения, длительность режима только чтения и конечное состояние.
      **Действует сейчас (05.08, #1069):** одна системная лестница («Доступ к системе»); все механики наследуют;
      UI «Исключения по механикам» и enforcement через `mechanic_access_policies` **сняты** (миграция `0374`).
      ⛔ **УСТАРЕЛО / ОШИБКА АГЕНТА:** «opt-in-исключение в `mechanic_access_policies`». См. **Т1** ниже.
      Инвариант §2.1c (оплата тарифа клиникой вне лестницы) не отменён.
...
- [x] **2.1c** ✅ **ЗАКРЫТО 31.07** (`d3123e6a4`): организация в конечном состоянии «блок» открывает и оплачивает свой тариф, приём денег от пациентов при этом погашен. Прежняя проверка была бутафорской (`switch` без эффекта, параметр без вызывающих) и заменена поведенческим тестом. ИНВАРИАНТ ДВУХ ПОТОКОВ ДЕНЕГ (уточнение владельца 30.07). Механика «приём оплат» гасит приём денег
      ...
      невыходимым.
- [x] **2.2** ~~Уровень механики сильнее системного~~ ⛔ **УСТАРЕЛО → T1 + 05.08 (#1069):** per-mechanic ladder
      снят; единственная лестница — `system_access_policy`, механики наследуют. Enforcement не читает
      `mechanic_access_policies` (миграция `0374`).
...
- [x] **2.6a** Снять агентские константы, которые сегодня решают за владельца. ✅ **ЗАКРЫТО 05.08 (#1069, measure).**
      ...
      закрыт дефект «нет тарифа → безлимитное хранилище». **ЗАКРЫТО 02.08:** `сумма` и
      `дата_начала_периода_автооплаты` берутся серверным `DoctorSectionLayout` из
      `getOrganizationBillingOverview()` для счёта продления своей организации, совпадающего с границей
...
- [x] **3.2** Механический запрет обхода: дойти до записи данных механики мимо порта нельзя. Тем же приёмом, что уже
...
- [x] **4.7** Курсы · CMS · каталог и пакеты упражнений · абонементы · приём оплат · платная подписка пациента ·
      ...
      **Абонементы — ГОТОВО (03.08):** `subscriptions` проведён через видимость карточки пациента у врача и раздел
      пациента, прямые read/write API; `read_only` сохраняет чтение и убирает package mutations, `disabled` скрывает
      обе поверхности. Независимый аудит `a9c540db8`, единственный fixer `f0aa724a1`, слияние `a4714b5bd`.
      Живая проверка на TEST прошла всю цепочку и сняла оба открытых вопроса — приёмку и «уже купленный абонемент
      продолжает работать»: создание при включённой механике → `200`; пациент увидел абонемент → `200`; после
...
лидом и оказался вопросом формулировки, а не поведения.

- [x] **Т1. Доступ к системе управляет всеми вложенными механиками — одна лестница, без исключений.** ✅
      **Подтверждено владельцем 05.08 (#1069):** «убрать нахер эти миллион настроек лестниц. Оставить одну.»
      **Доказательство:** `20ede5edc` (миграция `0374`, единая `system_access_policy` ladder).
      Снято: дублирование настроек у каждой механики, UI «Исключения по механикам», запись/чтение
...
- [x] **Т4. «Тариф триала» против «человек выбирает тариф сам» — разобрано 03.08, поведение верное, экран
      врёт.** ⛔ **УСТАРЕЛО → T5 (03.08):** отдельного «тарифа триала» нет; триал на первом тарифе организации.
      Вопрос владельца: что будет, если разрешить выбор тарифа, а человек выберет не тот, что указан в
      «тарифе триала». Ответ по факту кода (`deploy/postgres/c5a-platform-operations-runtime.sql`, провижининг
      организации): **эти две настройки никогда не конкурируют.** Порядок жёсткий: если есть АКТИВНОЕ правило
      триала с событием `organization_provisioned` — создаётся триал с тарифом триала, а настройка стартового
      тарифа не читается вообще; если активного триала нет — применяется стартовый тариф; если и он не задан —
      организация остаётся без тарифа, и человек выбирает сам (решение владельца §2.13: «нет активного тарифа и
      нет триала — доступа нет»). Значит описанная владельцем ситуация недостижима: пока триал активен, выбора
      на регистрации не предлагается. ⚠️ Работа тут не в коде, а в экране: два выпадающих списка стоят на
      соседних вкладках и выглядят альтернативами, подсказка про приоритет висит только у одного из них, а на
      вкладке триала не сказано, что он перебивает. Это уходит в Т1 как часть переделки.

### Триал и льготный период — решение владельца 03.08

Продиктовано владельцем. Ниже сначала его правила, затем измеренное состояние кода — они расходятся, и это
расхождение важнее всего остального в разделе.

**Правила владельца:**

- [x] **Т5. Выбор тарифа и срок триала — ДВЕ независимые настройки.** ✅ модель закрыта 03.08 (`ac185efb1`, land `c93c0f2d4`, миграция `0346`): `saas_trial_policy` лишился `tariff_id`, триал ложится на фактический первый тариф организации; «второй тариф триала не даёт» покрыто тестом. Админ-экран под это — отдельный слайс. Отдельно настраивается, даём ли мы человеку
      выбор тарифа. Отдельно — срок триала, и он **не зависит от выбранного тарифа**: применяется к любому
      тарифу, который оказался первым. Дословно: «не важно, мы назначили или человек выбрал — первый раз человек
      получает триал. Все последующие — сразу оплата». То есть триал перестаёт быть «тарифом» и становится
      **однократным периодом на первый тариф организации**, каким бы он ни был.
- [x] **Т6. Льготный период — отдельная опция поверх правила «После триала».** ✅ модель закрыта 03.08 тем же коммитом: продление триала снято во всех ТРЁХ дверях доступа, правило «После триала» применяется мгновенно после `ends_at`; льготное окно живёт своими полями `discount_window_days`/`discount_ends_at` и на доступ не влияет. Перед сносом проверено: открытых продлений на DEV не было, никто доступа не потерял. Нынешние варианты
      (заблокировать / только чтение / другой тариф) владелец оставляет. Сверх них добавляется независимый срок,
...
- [x] **Т10. Поведение после завершения ОПЛАЧЕННОГО периода — одна общая настройка.** Владелец 04.08: «я
      ...
      **Как надо делать:** что происходит, когда оплаченный период кончился — блокировка, только чтение или
      переход на другой тариф — задаётся **одной общей настройкой системы**, а не отдельно у каждой механики.
      Это тот же принцип, который владелец уже сформулировал в Т1 для доступа: «Доступ к системе определяет
      работу всех вложенных механик… это логично и снижает нагрузку вычислений доступа».

      **Чем отличается от уже существующего:** блок «После триала» (Т6) описывает конец ТРИАЛА. Здесь — конец
      **оплаченного** периода, это другое событие. На момент постановки отдельной настройки не было; сейчас
...
- [x] **Т9. Периоды оплаты задаются в тарифе, а не в коде; «день» как период — убрать.** Владелец 04.08,
      ...
      имеет смысл настраивать не в хардкоде а добавлением варианта в тариф».

      **Измерено 04.08 (лид):** в коде это закрытый union `tariffBillingPeriod: 'day' | 'month' | 'year'`
      (`apps/webapp/src/modules/saas-billing/ports.ts:53,175`, там же `SaasBillingPeriod` и
      `paidPeriodEndsAt` в `service.ts:355,447`). То есть ровно то, что владелец назвал неправильным:
      «день» присутствует, «полгода» отсутствует вовсе, а набор нельзя расширить из админки — только правкой
      кода и выкаткой.

      **Как надо делать:** период — характеристика тарифа, задаваемая при его создании/редактировании, а не
      перечисление в типе. Добавление нового варианта (полгода, квартал, что угодно) должно быть заведением
      значения в тарифе, без правки кода. «День» из доступных вариантов убрать — владелец назвал его
      бессмысленным для оплаты.

      ⚠️ Осторожно: `paidPeriodEndsAt` считает конец оплаченного периода от этого значения, а тарифы уже
      оплачиваются на TEST (#1057 закрыт 04.08) — менять надо так, чтобы уже выданные оплаченные периоды не
      сдвинулись. Это не «переименовать enum», а миграция данных вместе с типом.

      **Доказательство (#1069 T9):** каталог `saas_billing_periods` + FK на тарифах; `day` → `month` на
      тарифах без сдвига `current_period_ends_at`; selectable периоды из API/UI; арифметика в
...
- [x] **Т8. Как задаётся скидка.** ✅ закрыто 03.08: `saas_tariffs.discounted_price_minor` — явная льготная цена у каждого тарифа, глобального процента нет по решению владельца; «тариф без льготной цены скидки не даёт» покрыто тестом. Владелец назвал два варианта — процент со скидкой и округлением, либо точная
      льготная цена, настраиваемая отдельно у каждого тарифа: «Я думаю что второе более правильно». Рекомендация
      лида — да, точная цена у тарифа: маркетинговые цены должны быть круглыми, а процент от разных базовых цен
      даёт хвосты, которые потом всё равно округляют руками. ✅ Закрыто владельцем 03.08: **глобальный процент как запасной вариант НЕ нужен** — «тарифов
      всё-таки 3-4, а не десятки». Льготная цена задаётся у каждого тарифа явно; тариф без неё просто не даёт
      скидки, и это видно в админке, а не прячется за умолчанием.

**⚠️ Измеренное состояние кода — «льготный период» уже существует, но означает ДРУГОЕ.**

`saas_trial_policy` сегодня: `duration_days`, `grace_days`, `post_trial_behavior ∈ ('read_only','blocked','tariff')`,
`post_trial_tariff_id`. И `grace` в текущей реализации — это **продление самого триала**, а не окно оплаты со
скидкой: в `0305_tariff_snapshot_access_doors_local.sql` пока `v_now <= trial.grace_ends_at`, организация
продолжает получать `trial.tariff_id`, то есть полный доступ триала, и только ПОСЛЕ окончания grace вступает
`post_trial_behavior`.

У владельца иначе: правило «После триала» вступает **сразу** по окончании триала, а льготное окно идёт
**параллельно** и влияет только на цену, не на доступ. Это разная семантика на одном имени.

**✅ Решено владельцем 03.08: продление самого триала — «бессмыслица», оно УДАЛЯЕТСЯ.** Нынешнее поведение
`grace` (организация продолжает получать тариф триала после его окончания) снимается целиком. Остаётся:

1. триал кончился → **сразу** вступает правило «После триала» (заблокировать / только чтение / другой тариф);
2. параллельно идёт **льготный период** — окно, в котором можно оплатить со скидкой; на доступ оно не влияет.

⛔ Поле переиспользовать нельзя даже после этого решения: у нового понятия другой смысл и другой момент
срабатывания. Существующее продление снимается ЯВНО (миграция + чистка расчёта доступа в
`0305_tariff_snapshot_access_doors_local.sql`), и в том же проходе проверяется, что ни у одной живой организации
не осталось незакрытого `grace_ends_at`, на который опирался доступ.

- [x] **4.8** Рассылки подключены к модели собственных каналов клиники: отправка требует механики `mailings`
      и `branding`, затем для каждого выбранного канала проверяет его тарифную механику и tenant-scoped
      настройку клиники. Evidence: `apps/webapp/src/app/app/doctor/broadcasts/actions.ts`
      (`requireBrandedMailingMutation`, `assertClinicBroadcastChannels`) и
      `actions.entitlement.unit.test.ts`; незакрытые живые проверки/шифрование самих каналов остаются в #1071
      и не являются вторым пунктом тарифного плана.
- [x] **4.9** Три механики («Сегодня», разминки, промо) — обычные тарифные механики и подчиняются лестнице как все;
  ...
  - [x] **4.9A** «Сегодня» использует `patient_home_today` на видимости и direct URL пациента, а в `read_only`
        оставляет врачу чтение настроек и ссылку на него; записи сохраняются и получают существующий видимый отказ.
        Evidence: `apps/webapp/src/app/app/accessLifecycleSurfaces.ui.test.tsx` (disabled direct URL и read-only
        navigation), `apps/webapp/src/app/api/tariffMechanics.route.test.ts` (write refusal), targeted Vitest.
  - [x] **4.9B** Промо использует один `getMechanicSurfaceVisibility`, который
...
- [x] **4b.2** ✅ **ЗАКРЫТО 31.07** (`a60a39122`, сверено `ab5a8de02`): ровно два конечных состояния в типах и валидации, значения `скрыто` нет нигде. Найден и убран настоящий баг — лестница допускала ТРЕТЬЕ состояние `full_access`, и оно предлагалось в конструкторе как «Полный доступ». Ручка «неоплата» — единая лестница из этапа 2 на кабинет; включённые механики наследуют её через общий access-door, отдельной лестницы на механику нет. Конечных состояния два:
      `только чтение` и `выключено`. Значение `скрыто` не заводить: выключено и означает «раздела нет».
- [ ] **4b.3** ⛔→🔧 **ОТКРЫТО ОБРАТНО 02.09 независимым аудитом — FAIL, приземлять нельзя**
      (`docs/_TODO/SAAS_FOUNDATION/AUDIT_TARIFF_DOWNGRADE_CAPABILITY_2026-09-02.md`). Целевая ветка
      `NOT mechanic_included` → `read_only` корректна и проверена живьём, но миграция взяла базовое тело
      функции от `20260819T210005` и потому молча стирает две уже применённые миграции той же функции:
      `20260820T175432` (глобальная политика после оплаченного периода) и `20260823T030000`
      (роль `app_integrator_tenant_service` + перенос инициализации после гейта). Достижимые последствия,
      доказанные на `bcb_webapp_dev` в транзакции с ROLLBACK: организация с истёкшим оплаченным периодом и
      политикой `read_only`/`blocked` получает `grace` и **право записи** (было `read_only`/`disabled`,
      запись запрещена), при политике `tariff` теряет механику (`disabled` вместо `full_access`), а
      интегратор получает `42501` на двери механик. Исправление: менять ДЕЙСТВУЮЩЕЕ определение
      (якорная замена через `pg_get_functiondef`, как в 0820/0823), а не подменять его телом от 19.08.
      **Исправлено 02.09 (correction-pass по этому же FAIL, коммит из ветки
      `wt/systemic-tariff-downgrade-20260902`):** миграция
      `20260901T231600_a_downgraded_capability_reads_its_own_tariff_policy.sql` переписана с
      top-level `CREATE OR REPLACE` на `DO $migration$` с якорной заменой через `pg_get_functiondef`
      — тот же приём, что в `20260820T175432`/`20260823T030000`: читает ДЕЙСТВУЮЩЕЕ на момент запуска
      определение `app.resolve_organization_mechanic_access`, вставляет `tariff.downgrade_policies` в
      CTE `snapshot` и меняет только ветку `WHEN NOT mechanic_included`, fail loud (`RAISE EXCEPTION`),
      если якорь не найден или встречается не ровно один раз (проверка через
      `length(text) - length(replace(text, anchor, ''))`, а не одиночный `IF v_rewritten = v_definition`).
      Ни `global_paid_policy_history`/`effective_global_paid_policy`/`was_tightened` (0820), ни
      `app_integrator_tenant_service` в gate-списке и перенос инициализации после гейта (0823) телом не
      затронуты — оба маркера подтверждены живьём на итоговом определении после (не примененного
      постоянно) прогона. Доказательства: owner-aware rollback-only preflight
      (`migrate-local.mjs --rollback-only`) — exit 0, `pending=1`, DO выполнился и ROLLBACK; отдельная
      ручная проба — тот же `DO`-блок кандидата плюс фикстуры оракула 4b.3/4b.4 (все 12 комбинаций
      таблицы из §3 аудита + чужая организация) в ОДНОЙ транзакции с ROLLBACK на `bcb_webapp_dev` — дала
      побайтно целевую матрицу аудита (`courses:read_only|false` вместо `disabled`, все прочие значения
      `disabled`, включённая/числовые/местовая механики без изменений, чужая организация `REFUSED 42501`);
      после ROLLBACK — ledger, фикстурные тарифы/организации на DEV без изменений (тот же критерий, что
      в §5 аудита). Постоянно на DEV/TEST/PROD миграция не применялась (прямой запрет брифа). Живые
      оракулы `global-paid-period-access.devDbProof.test.mjs` (3/3) и
      `integrator-narrow-delivery-roots.devDbProof.test.mjs` (1/1) — зелёные на baseline DEV (0820/0823
      применены, кандидат нет), что и требуется: они ловят именно откат, а откат этой правкой не
      делается. `downgrade-capability-policy.devDbProof.test.mjs` как процесс `node --test` на baseline
      DEV по-прежнему 4 pass/1 fail (ожидаемо: файл проверяет ПОСТОЯННО применённое определение, а
      постоянно применять запрещено этим же брифом) — красная строка объясняется в §10b-логике самим
      файлом и закрывается тем же прогоном на первом стенде, где миграция реально применяется
      (TEST/приёмка). Гейты: `check-drizzle-migration-order.sh` — OK; `findMigrationNameViolations`/
      `findMigrationStaticViolations`/`findMigrationTimestampCollisions` — `[]`; `check:db-privileges-generated`
      — побайтно; точечный vitest (5 файлов, 97/97) и `pnpm --dir apps/webapp typecheck` — exit 0;
      `git diff --check` — чисто. **Статус: исправлено этим проходом, ждёт независимой приёмки (§24.5) —
      не переводить в ЗАКРЫТО без неё.** Ниже — исходная запись исполнителя (до FAIL), оставлена как есть:
- [x] ~~**4b.3**~~ ✅ **ЗАКРЫТО 02.09** (`apps/webapp/db/drizzle-migrations/20260901T231600_a_downgraded_capability_reads_its_own_tariff_policy.sql`):
      значения разделены по классу механики и сохраняются как данные (`a60a39122`, миграция `0281`); для объёма
      файлов стоит именно `заморозить рост`. Дефект был в одном месте, ниже evaluator: `evaluateTariffTransition`
      (`service.ts`) уже верно не блокирует переход при `disable_immediately`/`read_only`, но после назначения
      целевого тарифа единственная живая дверь `app.resolve_organization_mechanic_access` при
      `NOT mechanic_included` безусловно возвращала `'disabled'`, не читая `downgrade_policies` тарифа вовсе —
      `read_only` и `disable_immediately` после перехода совпадали. Правка: тот же CTE `snapshot` теперь
      SELECT-ит `tariff.downgrade_policies` (уже приходит через существующий `saas_billing_effective_tariff`
      LATERAL join, новой колонки не потребовалось), а `resolved_state` при исключённой механике читает
      `downgrade_policies ->> p_mechanic`: `'read_only'` → состояние `read_only`, всё остальное (`disable_immediately`,
      несохранённое значение) → прежний fail-closed `disabled`. Числовые механики (`freeze_growth`/`block`) эту
      ветку не проходят вообще — `included` уже держит `files`/`branches` всегда включёнными, заморозку исполняет
      их собственная дверь записи (`transactionQuotaPort`), как и раньше. Ручка «переход на тариф меньше»,
      значения по классу механики. Числовые: `не давать переход` · `дать переход, заморозить рост`.
      Возможности: `не давать переход` · `выключить сразу` · `оставить только чтение`. Для объёма файлов значение
      владельца уже названо — `заморозить рост`.
- [ ] **4b.4** ⛔→🔧 **ОТКРЫТО ОБРАТНО 02.09 независимым аудитом** — тем же изменением, что и 4b.3, поэтому
      закрывается только вместе с ним. Проверено и подтверждено аудитом: хардкода по имени механики нет,
      два сохранённых значения дают два разных состояния, приёмная сторона (`checkEntitlement`) различает
      их верно, числовые и местовая механики под capability-политику не попадают, чужая организация не
      резолвится (`42501`). Блокирует ровно откат чужих миграций из 4b.3, а не сама семантика.
      Acceptance-оракул на эту семантику добавлен аудитом:
      `deploy/postgres/privileges/downgrade-capability-policy.devDbProof.test.mjs` (opt-in
      `RUN_DOWNGRADE_CAPABILITY_POLICY_DB=1`, сейчас красный на названном дефекте — зеленеет после
      исправленной миграции). **Исправлено 02.09 тем же correction-pass, что 4b.3** (см. запись 4b.3
      выше — один и тот же migration-файл, одно исправление). Ручная rollback-wrapped проба
      воспроизвела все пять assertions этого оракула, включая обе стороны cross-org изоляции, с
      кандидатом активным в транзакции: разница со «старым» дефектом — ровно клетка
      `courses:read_only|false` вместо `courses:disabled|false`, всё остальное (fail-closed на прочие
      значения, включённая механика, числовые/местовая механики, `REFUSED 42501` для чужой организации)
      не сдвинулось. Статус тот же, что у 4b.3: исправлено, ждёт независимой приёмки. Ниже — исходная
      запись исполнителя, оставлена как есть:
- [x] ~~**4b.4**~~ ✅ **ЗАКРЫТО 02.09**: хардкода по имени механики по-прежнему нет — решает класс и значение из
      данных. Утверждение «разные значения → разное поведение» было ложно для capability-пары
      `read_only`/`disable_immediately` (обе заканчивались `disabled`); тем же изменением, что закрыло 4b.3,
      единственная точка исполнения (`resolve_organization_mechanic_access`) теперь производит два разных
      состояния из двух разных сохранённых значений — приёмная сторона (`checkEntitlement` в
      `requireEntitlement.ts`) уже умела различать `read_only` (чтение доступно, запись — нет) и `disabled`
      (заблокировано целиком) и не менялась. **20.08:** controls удалены из commercial constructor по команде
      владельца; сохранённый backend lifecycle не меняется, а отсутствие поля сохраняет существующее значение
      `downgradePolicies` (без UI-изменений, как и требовалось). Доказательство (без касания DEV/TEST): точечные
      `pnpm --dir apps/webapp exec vitest run src/modules/org-entitlements/service.test.ts
      src/infra/repos/pgOrgEntitlements.test.ts src/modules/saas-billing/service.test.ts` — 153/153 зелёные;
      `pnpm --dir apps/webapp typecheck` — exit 0; `bash apps/webapp/scripts/check-drizzle-migration-order.sh` —
      OK; статический `findMigrationStaticViolations`/`findMigrationNameViolations` — пусто; `git diff --check` —
      чисто. Живой прогон нового SQL-текста на настоящем Postgres (owner-aware rollback-only preflight на DEV) —
      обязанность независимого аудита/приёмки, этим проходом не выполнялся по прямому запрету брифа трогать
      DEV/TEST/PROD runtime.
...
- [x] **5.5** Освобождение объёма: в карточке пациента есть авторизованное удаление с подтверждением и видимым
...
- [x] **5.6** Правила смены тарифа (см. Р-14): повышение сразу; понижение с начала следующего расчётного периода и только после
      ...
      **РЕШЕНИЕ ВЛАДЕЛЬЦА 03.08 — денежная формула повышения:** клиника доплачивает разницу между новым и текущим
      ...
      и его снимок действуют сразу. Следующий полный расчётный период оплачивается целиком по цене нового тарифа.
...
- [x] **5.8** Контракт деплоя для каждой новой функции и триггера: права под исполняющей ролью; при появлении
      `SECURITY DEFINER` во владении `app_owner` — счётчик в `deploy/host/deploy-test-saas.sh` и два контрактных теста.

      ✅ **ЗАКРЫТО системным Track D 29.08:** владельцы функций, исполняющие роли и права сведены в единую
      декларацию и генерируемые артефакты; двусторонняя сверка каталога и живой reconcile DEV/TEST прошли.
      Текущий источник состояния — `docs/_TODO/DB_PRIVILEGE_LAYER_REBUILD/PLAN.md`, а исторический замер ниже
      не является новой задачей.
...
> **Pre-TEST census 05.08 (#1069, `123029848`): READY for TEST deploy.** §5a этапы 1–6 + owner deltas **Т1** (`20ede5edc`),
> **Т2** (`79280dc3f`/`ef45212c3`), **Т7** (`ef45212c3`), **Т9**/**Т10** — закрыты с evidence; **не помечать открытыми**.
> Карточка `#1069` остаётся `doing` до owner-приёмки **7.3** (живой прогон лестницы на TEST) и **7.4** (конструктор).
...
8. **⛔ БЛОКЕР Р-15: единственная дверь продажи места читает ДВЕ таблицы, у которых нет общего читателя.**
   Замерено вживую на DEV 19.08 (ветка `wt/seat-invoice-20260819`, дефект существует с `f4e17ac89`, то есть
   ПРЕДШЕСТВУЕТ приведению к действующей редакции Р-15). `decideSeatOverage` нужны и `saas_billing_subscriptions`
   (период, счётчик мест), и `be_organization_members`/`organization_member_invites` (сколько мест занято). Дверь
   зовут два принципала, и ни одному не хватает половины:
   - **приглашение** (`app_staff`): `select … current_period_starts_at …` → `42501 permission denied for table
     saas_billing_subscriptions`. Гранты `app_staff` на эту таблицу: `current_period_ends_at, organization_id,
     paid_additional_seats, source, status` — **`current_period_starts_at` нет**, а пропорция без начала периода
     не считается;
   - **покупка** (`app_clinic_billing`): `has_table_privilege('app_clinic_billing','be_organization_members',
     'SELECT')` = `f`, и, что важнее грантов, у `be_organization_members` **FORCE RLS без единой политики для
     `app_clinic_billing`** (политики есть у `app_staff`, `app_tenant_service` и seam-владельцев). Даже с выданными
     колоночными грантами дверь под этим принципалом видит НОЛЬ участников и поэтому отвечает `seat_available`
     всегда: место не продаётся никогда, счёт не выставляется никогда.

   Проверено выдачей временных грантов на DEV и их снятием: с грантами приглашение доходит до цены (850,00 ₽),
   покупка возвращает `{"ok":true,"outcome":"seat_available"}` при трёх занятых местах из трёх.

   **Это вопрос владельцу, а не работа исполнителя:** оба лечения — расширение грантов с новой RLS-политикой либо
   definer-шов (как `app.provision_specialist_owner`) — меняют границы доступа, а права принадлежат генератору
   (`AGENTS.md` §1). Пока не решено, Р-15 не может быть «сделано» ни в какой редакции.
