...
- [ ] **2.6a** Снять агентские константы, которые сегодня решают за владельца. ⚠️ **ЧАСТИЧНО 31.07**
      ...
      · **Стартовый тариф и триал — ДВЕ НАСТРОЙКИ, не решение в коде** (владелец 31.07, дословно): «У нас
        должна быть настройка — какой тариф выдаётся при регистрации. Если настройка пустая — значит тариф
        человек выбирает. И также настройка, какой даётся триал после регистрации — он и подключается».
        ⛔ Как не делать: не зашивать стартовый тариф в код провижининга и не выводить его «по флажку внутри
        тарифа» — это две отдельные настройки регистрации, ссылающиеся на существующие тариф и триал.
        Пустая настройка тарифа — легальное состояние: человек выбирает сам.
      · **Переменные «сумма» и «дата автооплаты» отдают ЗНАЧЕНИЕ, а не формулировку** (владелец 31.07):
        «переменная даёт цифру и число. Какими словами это видит клиника, решает шаблон». Значит работа —
        протянуть данные из биллинга (#1057); текстов вокруг них не сочинять.

      · **Уведомления лестницы — СПИСОК, а не число** (владелец 31.07): «надо настраивать частоту и количество
        уведомлений с текстами, причём надо возможность добавлять разные шаблоны — первое напоминание, второе
        предупреждение, третье ещё одно, но с другими сроками, потом уведомление о режиме только чтения и так
        далее». Строка списка: **срок** (за сколько до/после окончания периода) · **условие** (успешная оплата /
        ошибка оплаты) · **шаблон текста** с подстановкой переменных (тариф, сумма, имя клиники, дата начала
        периода автооплаты). Число строк задаёт владелец, ограничений нет. Тексты и переменные — данные, не код.
      ...
      · **Число мест специалистов — ОБЯЗАТЕЛЬНОЕ поле; тариф без него НЕ СОХРАНЯЕТСЯ** (владелец 31.07:
        ...
        Единица — предзаполненное значение в конструкторе («при создании тарифа по умолчанию пусть ставится
        одно — это разумный минимум»). Чинится валидацией сохранения: состояния «в тарифе пусто» в базе быть не
        должно, поэтому веток «пусто → нет предела» и «пусто → считаем одно» не заводить.

      · **Клиники без тарифа не существует** (владелец 31.07): «нет доступа и нет никаких механик вне тарифа;
        либо есть закончившийся тариф, либо отсутствует регистрация; при регистрации сразу даётся тариф с
        триалом, который настраивается в админке». Понятие «механики, включённые по умолчанию вне тарифа»
        упраздняется. Нынешняя ветка «клиника заведена до тарифов → включено всё» — временная совместимость.

      · **Нет активного тарифа и нет триала → доступа нет** (владелец 31.07): «просто сразу требуется выбор
        тарифа и оплата»; повторный триал не выдаётся — «перестала платить, вернулась — нет ни активного тарифа,
        ни триала уже повторного».

      · **Стартовый тариф с триалом настраивается в админке** (владелец 31.07): «клинику завели — выдаётся
        настроенный на этот случай тариф с триалом; потом клиника выбирает нужный тариф и оплачивает; при
        неоплате первично выданный тариф работает как настроено на этот случай, и переход на другие тарифы идёт
        по обычным правилам (нет ли превышений-блокеров)». Проверить, что стартовый тариф и его лестница
        настраиваются, а не зашиты.

      · **Предупреждение о приближении к пределу — только у ЧИСЛА ПАЦИЕНТОВ и ОБЪЁМА ФАЙЛОВ** (владелец 31.07:
        «процент для предупреждения надо считать только от количества доступных клиентов и объёма файлов»).
        С мест специалистов порог снять совсем — там превышение разрешено и оплачивается, предупреждать не о чем.
        У филиалов предупреждения нет.
