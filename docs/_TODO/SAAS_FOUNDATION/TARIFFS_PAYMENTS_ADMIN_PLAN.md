...
<<<<<<< HEAD
| **Р-15**       | «Счёт на добавление нового сотрудника действует до конца суток. Новый сотрудник разрешается только после оплаты счёта. Не оплатили до конца суток? Значит счёт не актуален, надо перевыставить заново. Таким образом каждый день будет меняться сумма оставшегося времени до конца периода. Отдельным счётом это оплачивается только один раз, на момент открытия нового места — до конца периода клиника оплатила, получила. Со следующего периода стоимость включена в один счёт: никто не хочет оплачивать десять тысяч счетов одновременно» ⛔ Решать «можно ли продать место и почём» обязано ОДНО место в коде, а не две двери с разными ответами (§5 «Один общий проход»). | 19.08 | 🔄 не сделано. Дверь сведена в одну (`f4e17ac89`), но слепой аудит 19.08 — **FAIL**: срок «до конца суток» на денежном пути не проверяет никто, просроченный счёт открывает место. Находки и доказательства: [`SEAT_INVOICE_R15_BLIND_AUDIT_2026-08-19.md`](SEAT_INVOICE_R15_BLIND_AUDIT_2026-08-19.md) | 5.6 |
=======
| **Р-15**       | **Действующая редакция (19.08, после принятия мировой практики).** Место открывается СРАЗУ, пропорциональная доплата уходит в следующий счёт — норма самообслуживания у Stripe, Slack, Atlassian, Google Workspace. Пропорция считается ОТ МОМЕНТА добавления места до конца оплаченного периода, не от начала суток: так делают все пять изученных биллингов, от суток не считает никто. Счёт живёт заданную ДЛИТЕЛЬНОСТЬ ОТ ВЫСТАВЛЕНИЯ; просроченный ОТМЕНЯЕТСЯ и выставляется новый с пересчитанной суммой, а не продлевается. Отдельным счётом место оплачивается один раз, на момент открытия; со следующего периода стоимость входит в общий счёт — «никто не хочет оплачивать десять тысяч счетов одновременно». Точка отсчёта времени — пояс приложения (Р-16). ⛔ Решать «можно ли продать место и почём» обязано ОДНО место в коде, а не две двери с разными ответами (§5). Основание: `SEAT_INVOICE_WORLD_PRACTICE_2026-08-19.md`, владелец 19.08 — «окей, я принимаю, всё». | 19.08 | 🔄 код приведён к действующей редакции в ветке `wt/seat-invoice-20260819` (`3f3373d6b`): пропорция от момента (целые сутки остатка от КОНЦА периода), срок счёта — настройка `invoiceValidityDays` от выставления и не позже конца услуги, место открывается выставлением счёта, просроченный отменяется и перевыставляется тиком биллинга. **Не «сделано»:** живой прогон на DEV прошёл только дверь приглашения (цена 850,00 ₽ против 881,25 ₽ прежней редакции); дверь покупки на настоящей базе не продаёт место НИКОГДА — см. риск 8.10 (права/RLS, дефект существует с `f4e17ac89`). Аудита и приёмки владельца не было | 5.6 |
...
>>>>>>> 88b483d70a55c9bd9538357bf848e767960a28c4
