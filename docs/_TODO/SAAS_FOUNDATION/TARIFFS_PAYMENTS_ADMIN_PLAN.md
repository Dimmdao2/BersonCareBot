...
- [ ] **2.1c** ⚠️ ИНВАРИАНТ ДВУХ ПОТОКОВ ДЕНЕГ (уточнение владельца 30.07). Механика «приём оплат» гасит приём денег
...
- [ ] **2.6a** Снять агентские константы, которые сегодня решают за владельца (список — аудит Opus 30.07, раздел B):
      порог предупреждения 80% (`org-entitlements/service.ts:226`) → поле рядом с числом; ⚠️ **ИСПРАВЛЕНО 31.07** — вместо «числа предупреждений»
      нужен **СПИСОК УВЕДОМЛЕНИЙ**. Владелец дословно: «количество предупреждений — про два сказал не я, а агенты…
      надо настраивать частоту и количество уведомлений с текстами, причём надо возможность добавлять разные
      шаблоны — первое напоминание, второе предупреждение, третье ещё одно, но с другими сроками, потом
      уведомление о режиме только чтения и так далее». То есть поле лестницы — упорядоченный список строк, у
      каждой свой СРОК и свой ШАБЛОН ТЕКСТА; число строк владелец задаёт сам, ничем не ограничен. Прежняя
      формулировка приписывала владельцу «два» — это было решение агентов;
      **⚠️ ВЛАДЕЛЕЦ ОТВЕТИЛ ПО КАЖДОЙ 31.07 — читать это, а не формулировки выше:**
      · **число мест специалистов — ОБЯЗАТЕЛЬНОЕ поле, тариф без него НЕ СОХРАНЯЕТСЯ** (владелец 31.07:
        «количество разрешённых специалистов должно быть явно настроено в тарифе, иначе он не сохранится»;
        про нынешний код: «берёт „нет предела“ — такое поведение невозможно»). Единица — лишь предзаполненное
        значение в конструкторе («при создании тарифа по умолчанию пусть ставится одно — это разумный минимум»).
        Чинится ВАЛИДАЦИЕЙ СОХРАНЕНИЯ, а не рантайм-подстановкой: состояния «в тарифе пусто» в базе быть не должно,
        поэтому веток «пусто → нет предела» и «пусто → считаем одно» не заводить.
      · **список включённых по умолчанию механик — упраздняется как понятие**: «клиники без тарифа быть просто не
        может — нет доступа и нет никаких механик вне тарифа; либо есть закончившийся тариф, либо отсутствует
        регистрация; при регистрации сразу даётся тариф с триалом, который настраивается в админке». Нынешняя
        ветка «заведена до тарифов → включено всё» — временная совместимость, а не правило.
      · **нет триала и нет активного тарифа** → «просто сразу требуется выбор тарифа и оплата»; повторный триал не
        выдаётся («перестала платить, вернулась — нет ни активного тарифа, ни триала уже повторного»). Это правило
        владельца, а не агентское «ограничить платные механики».
      · **сев `graceDays 7 / chargeAttempts 3 / readOnlyDays 21`** — ПРОВЕРЕНО ПО КОДУ 31.07: в миграции `0259`
        такого сева нет, значения читаются из настроек. Пункт устарел.
      · **`start_event`** — недоразумение, снимается. Владелец: «клинику завели — выдаётся настроенный на этот
        случай тариф с триалом; потом клиника выбирает нужный тариф и оплачивает; при неоплате первично выданный
        тариф работает как настроено на этот случай, и переход на другие тарифы идёт по обычным правилам (нет ли
        превышений-блокеров)». Старт от заведения клиники — так и задумано: с него начинается триал стартового
        тарифа. Проверять надо, что стартовый тариф и его лестница настраиваются в админке. По коду 31.07 поле
        `start_event` уже настраиваемое (CHECK требует лишь непустого значения).
      · **порог предупреждения — ПЕРЕДЕЛАТЬ, сегодня он не там** (владелец 31.07: «процент считается от числа
        включённых в тариф мест специалистов — бред. Не надо никаких предупреждений. Маразм. Процент для
        предупреждения надо считать только от количества доступных клиентов и объёма файлов»). Снять порог с мест
        специалистов совсем (там превышение разрешено и оплачивается — предупреждать не о чем) и завести его у
        ЧИСЛА ПАЦИЕНТОВ и у ОБЪЁМА ФАЙЛОВ. У филиалов предупреждения нет. Константы 80% в коде нет — поле
        настраиваемое, вопрос только в том, к каким механикам оно привязано.
...
- [ ] **2.9** Политика живёт данными и меняется на ходу (уточнение владельца 30.07: «эти политики могут меняться,
      ...
      тарифе меняет поведение без перезапуска и без правки кода.
- [ ] **2.10** Изменение политики действует ВПЕРЁД: организация, уже идущая по лестнице, пересчитывается от новой
...
- [x] **3.1b** ✅ **ЗАКРЫТО 31.07** (`04338eba0`): единый адаптер видимости с тремя поверхностями (навигация специалиста, навигация пациента, прямая ссылка), закрыты дыры на прямых ссылках — статья пациента и страница разминки отдают «не найдено» при выключенной у владеющей клиники механике. Проверено лидом поломкой. Прежняя формулировка: адаптер видимости раздела — сегодня пустая заглушка: требование «ни он не видит в кабинете этого
...
- [ ] **3.1c** ⚠️ РЕШЕНИЕ ЛИДА 30.07 о шве для интегратора. Воркер отказался закрывать находку №1 и был прав: у
...
- [ ] **3.1d** Вызов двери — на drizzle, отдельным портом, и это ТОЛЬКО НАШ код. Решения владельца 30.07: «вся семья
...
- [x] **4a.3** ✅ **ЗАКРЫТО 31.07** (`c07520710`): дефектов не найдено — механика `cms_pages` нигде не читается на пути публичной записи и виджета; добавлены три теста-сторожа, закрепляющие отсутствие гейта на профиле клиники, шагах публичной записи и скрипте виджета. Проверено лидом ДОБАВЛЕНИЕМ гейта (тест краснеет). Прежняя формулировка: граница CMS проверяется тестом: при выключенной CMS страница профиля клиники, публичная страница записи
      и виджет для внешнего сайта продолжают работать (`app/book/embed.js/route.ts` и публичная страница).
- [x] **4a.4** ✅ **ЗАКРЫТО 31.07** (`97847e21f`): найдено и исправлено нарушение вашего решения — дневники пациента были обычной «возможностью» и при выключении или ступени «только чтение» блокировались у ОБЕИХ сторон, включая самого пациента. Переведены в класс «никогда» (критичная механика, как в каноне §4a), 13 путей записи сняты с гейта. Старый тест закреплял неправильное поведение и переписан. Проверено лидом поломкой. Прежняя формулировка: внешний календарь и дневники пациента прогнать по чек-листу этапа 4 целиком (видимость у обеих сторон,
      видимый отказ, строка в реестре, тест со снятием защиты) — сейчас они только «переведены на порт».
- [x] **4a.5** ✅ **ЗАКРЫТО 31.07** (`3f1633003`): ложные записи сняты — у брендирования и каталога упражнений реальные пути записи заведены в реестр защищённых точек; проверка расширена функцией `validateDeclaredNoSurfaceClaims` (`scripts/check-s4-entitlement-coverage`), которая ловит именно ложное «поверхности нет». Проверено лидом: записей `branding`/`exercise_catalog` в `DECLARED_NO_SURFACE` больше нет, тест покрытия зелёный (5/5). Оговорка: проверка держится тестом `protectedActionRegistryCoverage.unit.test.ts` (входит в набор), отдельным шагом линта скрипт не подключён. Прежняя формулировка: ложные записи `DECLARED_NO_SURFACE` в реестре защищённых точек: у брендирования стоит «нет действия
...
- [x] **4b.4** ✅ **ЗАКРЫТО 31.07**: хардкода по имени механики в политике нет, решает класс механики и значение из данных. Главное доказательство этапа на месте и проверено лидом поломкой: разные значения → разное поведение без правки кода. Прежняя формулировка: вычистить из кода и планов правила по конкретным сущностям («пациентов только при создании»,
...
- [x] **5.2** ✅ **ЗАКРЫТО 31.07** (`fa15dc5ac`): атомарная проверка в пишущей транзакции, архивирование освобождает место, ведение существующей карточки не блокируется. Доказательство гонки на настоящем PostgreSQL, проверено лидом поломкой. Прежняя формулировка: число пациентов: само число + две ручки (см. §5 канона). Поведение при неоплате и при понижении
...
- [x] **5.7** ✅ **ЗАКРЫТО 31.07**: три исполняемых доказательства на настоящем одноразовом PostgreSQL 16 — `check-patient-count-quota-race.mjs`, `check-branches-quota-race.mjs`, `check-storage-quota-race.mjs`. Лид проверил разом: снятие решающей строки `used + increment > quota.limit` роняет ВСЕ ТРИ, откат — все три зелёные. Прежняя формулировка: доказательство гонки на каждое число (пациенты, филиалы, объём): настоящий PostgreSQL, из двух
...
- [x] **6.2** ✅ **ЗАКРЫТО 31.07** (`edfe998be`): отчёт по всем организациям — кто за пределом и кто на какой ступени. ⚠️ Пробел, названный честно: числа пациентов и объёма файлов между организациями пока не читаются (гранта нет), в отчёте это видно как отсутствие данных, а не как ноль.
