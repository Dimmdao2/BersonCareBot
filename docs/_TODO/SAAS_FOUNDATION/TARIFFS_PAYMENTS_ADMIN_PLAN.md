...
- [ ] **2.6c** Развилка, которую нельзя решать самому: канон §5.5 говорит «деньги клиники не блокируем никогда», а
      пункт 4.7 подчиняет приём оплат лестнице, у которой конечное состояние — раздела нет. Вынести владельцу вопросом,
      в коде до ответа не трогать. То же с брендированием: «нейтральное оформление, но сообщение уходит» (§5.7) — это
      выбор агента, а не его слова.
...
- [ ] **6a.4** `SAAS_S4_TARIFFS_STORE_ENTITLEMENTS.md` (713 строк, 83 открытых чекбокса) — та же тема, старше.
      **РЕШЕНИЕ ВЛАДЕЛЬЦА 30.07: «магазин пока отложен. как сделаем сам магазин, так и сделаем в тарифах настройку».**
      Значит сверка резко упрощается и делится на две части: всё, что про МАГАЗИН (платформенные пакеты, витрина,
      покупка, курирование библиотек), — отложено владельцем, помечается `owner-deferred 30.07` со ссылкой на это
      решение и НЕ переезжает в §5a; тарифная настройка магазина приезжает вместе с самим магазином. Всё, что про
      механики, права и лимиты, — сверяется построчно с §5a: «сделано (доказательство)» либо «живо → переезжает».
      После этого файл архивируется с надгробием, где обе части названы явно. Рубильники `exercise_catalog` и
      `exercise_packages` в §5a остаются: это просто выключатели существующих поверхностей, магазин они не строят.
- [ ] **6a.5** После 6a.4 по теме тарифов и механик остаётся ровно комплект: план (§5a этого файла) · канон
      (`QUOTAS_AND_MECHANICS_DESIGN`) · справочник (`QUOTAS_RESEARCH`) · журнал прогонов (`runs/tariff-mechanics/`).
