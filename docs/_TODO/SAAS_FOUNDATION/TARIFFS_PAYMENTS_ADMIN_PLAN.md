...
- [ ] **2.6a** Снять агентские константы, которые сегодня решают за владельца (список — аудит Opus 30.07, раздел B):
      ...
      **⚠️ ВЛАДЕЛЕЦ ОТВЕТИЛ ПО КАЖДОЙ 31.07 — читать это, а не формулировки выше:**
      · **место специалиста = 1** — законно как значение ПО УМОЛЧАНИЮ В КОНСТРУКТОРЕ при создании тарифа
        («при создании тарифа по умолчанию пусть ставится одно — это разумный минимум»), владелец его видит и
        меняет. НЕ рантайм-подстановка «в тарифе пусто → одно» и не «нет предела»: в рантайме отсутствие числа —
        ошибка настройки, видимый отказ.
