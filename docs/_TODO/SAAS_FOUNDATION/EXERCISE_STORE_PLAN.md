...
**Ответ (инженерное решение, 11.09): НЕ отдельная учётная запись, а роль автора на существующей
платформенной личности + переключатель контекста.** Причины:

1. Отдельная учётка тянет за собой второй логин, второй пароль, второй сброс пароля, второй путь
   восстановления и вторую 2FA — всю обвязку идентичности в двух экземплярах ради 3–6 человек.
2. Автор, который вместе с этим ведёт практику (а именно такие авторы и нужны — известные специалисты),
   получит два несвязанных входа и будет путаться, какой из них «его».
3. У нас уже есть модель «личность + возможности», по которой сделано управление клиникой. Автор — ещё
   одна возможность на личности, а не вторая вселенная.
4. Ручная выдача доступа при этом сохраняется буквально: возможность «автор» ставит только глобальный
   администратор.
5. Переключение «Автор / Кабинет» делается как уже существующий режим администратора — то есть механика
   в продукте есть, изобретать нечего.

Поддомен при таком решении — это **тот же продукт под другим хостом и с другим деревом страниц**, ровно
как брендированные хосты клиник: ещё один резолв хоста в существующем шве, а не отдельная инфраструктура.
