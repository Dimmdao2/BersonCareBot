...
### Открыто (в этот скоуп НЕ входит, решается отдельно)

- Цена подписки на магазин и размер фонда авторам (владелец ещё не закрыл; аванс исключён — «на аванс нет
  денег», взамен предложена реферальная доля от базового тарифа за приведённых).
- Атрибуция приведённых автором подписчиков — без неё реферальная модель не считается.
...
- [ ] Третий владелец контента: `owner_kind` сегодня знает `platform` и `organization`, нужен автор.
- [ ] Привязка существующих и новых элементов к автору.
...
- ⚠️ Расчёт выплат зависит от незакрытых решений владельца: цена подписки и размер фонда. Экран строится
  так, чтобы процент и цена были данными, а не константой в коде.

### Owner-gates, открытые на момент написания

1. Цена подписки на магазин и размер фонда авторам (аванс исключён: «на аванс нет денег»).
2. Реферальная доля от базового тарифа за приведённых автором — принимаем или нет.
3. Атрибуция приведённых (персональная ссылка автора + «кем приведён» у организации) — без неё
   реферальная модель не считается вообще.
