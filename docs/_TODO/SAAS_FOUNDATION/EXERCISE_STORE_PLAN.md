...
### Автор в карточке — только при купленной подписке

- [ ] Имя автора в карточке элемента показывается **только если у клиники есть купленная подписка на
  библиотеку**. Без подписки карточка платформенного элемента не рекламирует автора.
- [ ] Фильтр по автору живёт по тому же правилу: нет подписки — нет фильтра.

### Фильтр слоя на входе в каталог — для всех трёх типов элементов

- [ ] На входе в каталог — переключатель «только мои элементы» / «включая платформенные». Владелец:
  «у нас изначально должен быть фильтр… только личные элементы или элементы, в том числе, платформы».
- [ ] Правило одинаково для **упражнений, тестов и рекомендаций** — это не упражненческая фича.
- ✅ Дешёво: флаг `includePlatformBase` в чтении каталога уже существует, речь о выводе его в интерфейс.
