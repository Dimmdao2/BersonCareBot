...
- [x] **B0.3 Удалить моки (владелец 01.08: «моки удаляй»; ранее — «мы не будем держать опасные моки на проде»).** Как только оплата через ЮKassa работает
      ...
      ⚠️ **Приёмка прогоном тестовой картой ещё НЕ проведена** — до неё пункт не считается принятым
      владельцем, как и записано в баннере выше. Осталось: прописать ключи тестового магазина в
      `system_settings` и провести платёж от корзины до захвата.
