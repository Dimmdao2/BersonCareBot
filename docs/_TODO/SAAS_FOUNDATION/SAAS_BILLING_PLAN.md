...
> **Приёмка — не тесты, а живой платёж:** тестовой картой через тестовый магазин ЮKassa (`shopId 1425962`,
> ключ в `system_settings`), от корзины до захвата, с подтверждением по вебхуку. Пока такой платёж не
> проведён — пункт открыт, чем бы ни были зелены прогоны.
...
- [ ] 🔴 **Проверка подлинности вебхука ЮKassa выдумана — реальные уведомления будут отвергаться все до
      единого.** `yookassaPaymentProvider.verifyWebhook` принимает уведомление, если во входящем запросе есть
      либо заголовок `Authorization: Basic shopId:secretKey`, либо `x-yookassa-signature` с HMAC-SHA256 тела.
      Ни того, ни другого ЮKassa не присылает: Basic — это НАШ исходящий заголовок к ним, а заголовка с
      подписью у неё нет вовсе. Документация ЮKassa («Уведомления», раздел про подлинность) предлагает
      ровно два способа: **проверить IP-адрес отправителя** по опубликованному списку
      (`185.71.76.0/27`, `185.71.77.0/27`, `77.75.153.0/25`, `77.75.156.11`, `77.75.156.35`,
      `77.75.154.128/25`, `2a02:5180::/32`) и **перезапросить объект платежа по API**, чтобы убедиться,
      что статус из уведомления актуален. Второе — настоящий барьер: тело уведомления вообще перестаёт
      быть источником истины, подделать нечего. Переделать `verifyWebhook` на эту пару и снять из него
      HMAC. Оракул после правки: подделанное тело с валидного IP не проводит оплату, потому что API
      отдаёт другой статус/сумму.
