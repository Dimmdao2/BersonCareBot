...
- [ ] 🔴 **B1.4 КАТАЛОГ ТОВАРОВ ВЫРЕЗАЕТСЯ ЦЕЛИКОМ. Решение владельца 01.08, дословно: «вырезай каталог».**
      ...
      **Целевое:** курс продаётся из курса, абонемент — из абонемента, и оба идут через единую дверь
      ...
      ⛔ **Порядок обязателен:** собственный путь продажи курса и абонемента появляется ВМЕСТЕ с
      вырезанием, а не после. Иначе платных курсов не станет на время между.
      ⛔ **Прод:** на dev покупок ноль, про прод неизвестно. Миграция, удаляющая таблицы, не должна
      выполняться на проде, пока их пустота не подтверждена. Это условие, а не пожелание.
