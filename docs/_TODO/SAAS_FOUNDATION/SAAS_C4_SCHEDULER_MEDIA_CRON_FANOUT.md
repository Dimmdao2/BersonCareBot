...
| `media-worker-claim` | `FOR UPDATE SKIP LOCKED` claim | `apps/media-worker/src/jobs/claim.ts` | tick infra source; claimed row materializes org | `media_transcode_jobs`, `media_files` | Claim copies `COALESCE(job.organization_id, media_files.organization_id)` and returns `organizationId`. | Existing claim tests + C4 checker verify the coalesce and return field. |
| `media-worker-process` | transcode metadata updates and terminal state | `apps/media-worker/src/processTranscodeJob.ts` | `organization`, source `media-worker:process-transcode-job`, from claimed `job.organizationId` | `media_files`, `media_transcode_jobs`, fake/local S3 in tests | Job with org runs under organization principal; missing org fails closed in locked mode before S3/DB work. Legacy mode keeps a named fallback `media-worker:process-transcode-job:legacy-missing-org`. | Focused principal tests verify org principal and locked missing-org denial. |
| `media-worker-sql` | media-worker SQL chokepoint | `apps/media-worker/src/runMediaWorkerSql.ts`, `withClient.ts`, `poolProvider.ts` | current ALS principal | all media-worker SQL | Locked mode accepts only claimed-job `organization` principals and allowlisted `infra` source `media-worker:tick`; rejects missing, bootstrap, patient, staff, integrator, and unknown infra sources before checkout; cleanup uses principal release/reset. | Static checker verifies pre-checkout guard and allowlist. |
