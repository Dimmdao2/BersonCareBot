...
- after strict policy installation, apply `deploy/postgres/c4-operational-runtime.sql` using the three distinct
  ...
  recreates the media policies. Before restart, each DB base login must call release directly, then `SET ROLE` only
  its own SET-only capability; positive exact-surface probes, cross-contour negatives and authenticated media-control
  readiness must pass.
  ...
  contain the three distinct operational DB URLs and the control-only media configuration. The script must run the shared C2 preflight against
