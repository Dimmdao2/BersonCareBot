...
- [ ] **R1.1 — AUDIT FAIL.** `SAAS_ENFORCE_ROADMAP.md` section 0 ("FINAL RESULT / fixed definition of done") is rewritten to the
      ...
      signing-key, and review prose still required the superseded flip/OFF path.
...
- [ ] **R1.4 — AUDIT FAIL.** *(fixes R0.3, which the audit failed)* The R0 scope register in `SAAS_ENFORCE_ROADMAP.md` is
      ...
      found H6 `profile`, H0's method-level Store matrix, and the global/org `system_settings` write split absent.
- [ ] **R1.5 — AUDIT FAIL.** *(fixes R0.4 and R0.9, which the audit failed)* Two defects:
      ...
      **(b)** the R0 row in `TENANT_HARD_MODE_LOG.md` records a PASS claiming the C4 checker "asserts the real
      ...
      still cited the bypassable C4 checker "after the R0 checker repair" as evidence and R0 retained the false claim.
- [ ] **R1.6 — AUDIT FAIL.** *(fixes R0.7, which the audit failed — and it was the orchestrator's bad instruction)* R0 was told
