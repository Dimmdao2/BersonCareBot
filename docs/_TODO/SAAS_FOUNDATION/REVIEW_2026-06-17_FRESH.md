...
- **~20+ child clinical tables** reachable only via a parent FK (no user column) are also invisible to the heuristic: clinical\__\_update/\_status_history, online_intake_answers/attachments/status_history, support_conversation_messages, support_questions, treatment_program_instance_stage_/\_events, test_results, reminder history/journal tables, lfk_complex_exercises.
