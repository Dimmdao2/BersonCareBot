...
Запрет должен проверяться на сервере при сохранении/публикации платного продукта. Скрытие поля или предупреждение только в интерфейсе недостаточны. Пока сущность платного продукта не реализована, runtime-gate поставить некуда; это обязательный критерий будущей подписочной инициативы.
