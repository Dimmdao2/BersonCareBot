...
**Статус:** реализация приостановлена. Сначала исследование из этапа 0 и решение владельца: готовый независимый
helpdesk или собственная система. Production-код не начат.
...
- 🔴 **ОБРАЩЕНИЕ БЕЗ ВХОДА ОБЯЗАНО РАБОТАТЬ. Решение владельца 01.08, дословно: «техподдержка на старте
  нужна чаще всего тем кто не может залогиниться».** Этим ОТМЕНЕНЫ три строки, стоявшие здесь раньше:
  «форма создания обращения доступна только после входа», «гостевой POST поддержки удаляется» и ссылка на
  D-12 как на запрет. ⚠️ **Их приписали владельцу агенты.** D-12 в `OWNER_PUNCHLIST_2026-07-28.md` — это
  строка таблицы «Развилки владельца», колонка «Рекомендация / safe default»: вопрос, на который он НЕ
  отвечал, а не его решение (закрытые им строки в той таблице зачёркнуты и помечены «ЗАКРЫТО ВЛАДЕЛЬЦЕМ»).
  В §31 `OWNER_PRODUCT_RULES.md`, где лежат его настоящие слова от 28.07, требования входа нет вовсе.
  Сам владелец 01.08: «выдумка агентов».
  **Следствие для проекта:** человек, который не может войти, — не крайний случай, а ОСНОВНОЙ адресат
  поддержки на старте. Значит нужен приём обращения без учётной записи, способ ответить такому человеку
  и защита от спама, не требующая входа. Как именно — проектная работа, не выдумка на ходу.
- закрытое обращение хранится в идентифицируемом виде 180 дней после последнего изменения; затем персональное
  содержимое уничтожается, адресный legal hold применяется только к конкретному спору/претензии и длится не более
  трёх лет, а необратимо обезличенные метрики могут сохраняться без срока.

Новое поручение владельца от 24.08.2026, которое останавливает прежний переход прямо к собственной реализации:

- поддержка должна переживать отказ основного приложения, поэтому нельзя строить её на том же webapp/API/DB пути;
- до написания собственного helpdesk исследовать простой готовый **российский** продукт и отдельный
  runtime/failure domain;
- второй сервер планируется также для резервных копий и журналов безопасности, поэтому размещение там публичной
  поддержки не считается автоматически допустимым: исследование обязано отдельно описать изоляцию контуров;
- окончательного решения «готовый продукт» или «своя система» пока нет. Разделы 3–10 ниже сохраняют прежний
  кандидат собственной реализации, но не дают authority начинать её до закрытия этапа 0 владельцем.
...
### 3.4 Retention и уничтожение

- Срок считается от последнего изменения закрытого тикета; reopening сбрасывает отсчёт.
- Через 180 дней purge уничтожает `title`, реплики, вложения/object keys и идентифицирующие user/org references.
- Остаются только необратимо обезличенные агрегаты: категория, timestamps/SLA-метрики и результат без свободного
  текста и возможности восстановить автора.
- Legal hold ставится только global admin на конкретный спор/претензию с обязательными основанием, сроком окончания
  и audit event; общий срок — до трёх лет, бессрочного hold нет.
- Явно присланные пароли, токены, медицинские документы и иные данные, не нужные для поддержки платформы,
  удаляются сразу после обнаружения, не ожидая 180 дней.
- Purge охватывает primary storage, attachments и штатный жизненный цикл резервных копий; журнал уничтожения
  содержит только ticket id, policy version, timestamp и результат, без удалённого содержимого.

Основание: принцип ограничения срока хранения из 152-ФЗ ст. 5 и обязанность уничтожения из ст. 21; шестимесячный
операционный срок соответствует распространённым настраиваемым helpdesk deletion schedules, а трёхлетний адресный
hold — общему сроку исковой давности по ГК РФ ст. 196. Это не разрешает хранить все обращения три года.
...
### Этап 0 — независимый контур и готовый продукт

- [ ] Зафиксировать минимальный независимый путь: пользователь может открыть обращение без входа и без работающих
      webapp API, основной PostgreSQL и основного application runtime.
- [ ] Сравнить готовые простые российские helpdesk-продукты и собственный вариант по размещению/локализации ПДн в РФ,
      anonymous intake, email/messenger-каналам, API/экспорту, retention/удалению, SSO и стоимости эксплуатации.
- [ ] Для self-hosted варианта описать отдельный failure/security domain; не давать публичному helpdesk доступ к
      хранилищу резервных копий или журналам безопасности даже при размещении на одном физическом сервере.
- [ ] Перед любым implementation brief получить выбор владельца: готовый продукт и способ размещения либо возврат
      к собственной модели из разделов 3–10.
...
- [ ] Удалить/запретить внешний `t.me` в `support_contact_url` и UI; внутренние support-ссылки ведут в
      authenticated flow.
- [ ] Добавить 180-day retention/purge job, адресный legal hold до трёх лет, expiry вложений/backups и evidence
      уничтожения по §3.4.
