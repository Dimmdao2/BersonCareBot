...
- [ ] Экспортировать только данные субъекта в выбранном org context; чужой tenant/internal secrets исключены.
      Техническая реализация выгрузки не входит в первый deletion/retention slice и остаётся следующим вариантом,
      но будущий DSAR/export contract не отменяется.
