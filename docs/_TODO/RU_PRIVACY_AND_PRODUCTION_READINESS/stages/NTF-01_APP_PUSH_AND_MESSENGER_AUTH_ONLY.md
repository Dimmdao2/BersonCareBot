...
| Booking create/cancel/reschedule / 24h/2h / payment lifecycle | Integrator `bookingLifecycleRoute.ts` creates lifecycle/booking-reminder message intents and target-dependent queue work; webapp `patientWebPushNotify.ts` appends lifecycle notification records and relays Web Push. Canonical lifecycle is `support_conversation_messages` (`source=appointment_lifecycle`); appointment is linked object. | Current lifecycle and reminder copy includes date/time; booking confirmation email includes service/location and `.ics`; payment-captured is declared in `bookingLifecycleNotifications.ts`. Target `routine_product`, `T1` for date/time/status/payment amount only after exact matrix; no credentials; route `/app/patient?notifications=1` or booking. | Tests: `bookingLifecycleRoute` tests, `patientWebPushNotify.test.ts`, `sendBookingConfirmationEmail.test.ts`. Hazard: 24h/2h push creation is coupled to existing messenger target/job success; pending kinds include `reminder_dispatch`; email receipt/confirmation is service allowlist, never fallback. `active_dependency`; `apps/integrator/src/integrations/bersoncare/bookingLifecycleRoute.ts`, `apps/webapp/src/modules/patient-notifications/patientWebPushNotify.ts`, `apps/webapp/src/modules/patient-booking/sendBookingConfirmationEmail.ts`, `apps/webapp/src/modules/patient-booking/bookingLifecycleNotifications.ts`. |
...
- **Rows for later controlled cutover:** `reminder_dispatch`, `doctor_broadcast_intent`, `operator_alert`, plus any
  legacy rows whose `payload_json.intent.payload.delivery.channels` selects Telegram/MAX/SMS/email. This census does
  not query their presence or count. The later change window must classify them as auth allowlist / cancel / archive
  metadata-only / already-sent; it must never replay product rows to messenger.
