...
Короткий актуальный список того, что требуется от владельца/юриста сейчас, перед конкретным этапом и только в
production window, находится в [`OWNER_ACTIONS.md`](OWNER_ACTIONS.md) §0. Таблица ниже — полный decision registry;
она не означает, что все open gates нужно решать одновременно.
