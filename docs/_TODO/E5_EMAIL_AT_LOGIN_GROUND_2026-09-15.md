...
> **⚠️ Развилки 1 и 2 ЗАКРЫТЫ владельцем 15.09 — читай ответ, а не «Рекомендацию» ниже.**
> Ответ лежит в `MERGE_MECHANISM_REWRITE_2026-09-14.md:177-183`, дословно: «просто просить при
> следующем входе в приложение, потом требовать через 14 дней, потом отказ без почты»; «кто не
> даст - те не заходят просто». То есть лестница из трёх ступеней — просьба при следующем входе,
> требование через 14 дней, отказ во входе, — и **мягкой бессрочной версии нет**. «Безопасный
> default» в пунктах 1 и 2 написан агентом ДО этого ответа и им СНЯТ: он запрещал клинические
> данные сразу, а владелец разрешил их на первые 14 дней сознательно.
>
> Почему эта врезка здесь: 16.09 закрывающий аудит рантайм-двери взял оракулом снятый безопасный
> default и выдал четыре MUST FIX против реализованной лестницы владельца — то есть потребовал
> переделать ровно то, что владелец решил. Ошибка не аудитора: ответ был записан в соседнем файле,
> а этот план остался с открытой развилкой. **Открытым в пункте 1 остаётся только состав
> исключений** (сейчас в коде: экран привязки, профиль, поддержка, help, `/legal`, выход) — лестницу
> владелец закрыл, список разрешённых во время гейта путей — нет.
