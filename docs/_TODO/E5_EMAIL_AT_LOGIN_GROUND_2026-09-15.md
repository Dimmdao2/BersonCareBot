...
## 5. Решения по Э5 и что ещё не решено

### Решено — это и есть требование, развилки больше нет

1. **Лестница обязательности почты.** Три ступени: просьба при следующем входе в приложение →
   требование через 14 дней с первой просьбы → отказ во входе без почты. Мягкой бессрочной версии
   нет; потеря когорты без почты принята осознанно. Клинические данные в первые 14 дней открыты —
   это часть решения, а не недоделка.
   *Владелец, 15.09; канон решения — `MERGE_MECHANISM_REWRITE_2026-09-14.md`, этап Э5.*
2. **Уже живые сессии не отзываются.** Гейт применяется при следующем входе, массового разлогина
   нет. Отдельной категории «сессия выдана до выката» в коде не заводится.
   *Владелец, 15.09, там же.*
3. **Ссылка-приглашение врача конфликтом не является.** Человек вводит почту, получает код, кабинет
   врача привязывается к учётке, слияние идёт штатно; при расхождении ФИО задаётся вопрос об имени —
   и только после кода, а не до него.
   *Владелец, 15.09; канон — `AUTH_AND_IDENTITY_CANON.md` §18в.*
4. **Заявка клиники телефон не привязывает.** Все контакты кроме почты едут врачу метаданными
   заявки; совпадение телефона перестаёт быть событием идентичности. Слияние начинает сам человек,
   подтверждая контакт в своём кабинете.
   *Владелец, 15.09; канон — §18в.*
5. **Mini App вырезается целиком, меню в ботах не строится**, поэтому отдельного обхода почты через
   бот не существует: за ботом остаются только выдача кода и доставка уведомления.
   *Владелец, 16.09; канон — `AUTH_AND_IDENTITY_CANON.md`, раздел о снятых входах.*
6. **Э5 не включается, пока доставка почты не подтверждена живой приёмкой.** Скрытого
   per-request обхода при недоступной доставке не делается: либо канал работает и гейт включён, либо
   гейт выключен целиком. Это инженерное условие включения, решать его владельцу не требуется.

### Не решено — три вопроса, ответ нужен до соответствующей работы

1. **Состав исключений во время гейта.** Лестница закрыта, список разрешённых путей — нет. Сейчас в
   коде: экран привязки почты, профиль, поддержка, help, `/legal`, выход. Вопрос: остаются ли
   профиль, help и `/legal` целиком или сужаются до привязки, повторной отправки кода, поддержки и
   выхода.
2. **Граница публичной записи.** Предложение: не делать почту условием создания записи или оплаты —
   запись и платёж не теряются из-за недоступной почты, — а требовать её только перед переходом в
   кабинет.
3. **Что человек видит после медицинского defer.** Предложение: сохранить доказанную текущую учётку,
   показывать статус разбора, поддержку и выход, но не данные найденной учётки до решения врача.
...
- Три оставшихся продуктовых вопроса раздела 5 за владельца не решены.
- Строка вердикта в очередь/`feat` не записывалась; автор этого замера свою работу не аудировал.

## Решение владельца 16.09: структурный lint-гейт над пациентской дверью СНЯТ

Владелец, дословно: **«удалять гейт»**.

Речь о статическом анализаторе `apps/webapp/scripts/check-patient-api-business-access-door.mjs`, который
требовал общую дверь бизнес-доступа у каждого обработчика `api/patient/**` и непубличного `api/booking/**`.
Он удалён вместе со строкой в `lint` (`apps/webapp/package.json`) и упоминанием в `guards.md`.

**Что снято и что осталось — это разные вещи, и путать их нельзя:**

- снят ИНСТРУМЕНТ ПРОВЕРКИ — анализатор, который пытался доказать наличие двери разбором синтаксиса;
- сама ДВЕРЬ (`requirePatientApiBusinessAccess`, политика `patientRouteApiPolicy.ts`, экран привязки почты)
  осталась на месте и обязательна по канону; её держат обзор и наборы маршрутов.

Почему владелец так решил и почему это разумно: за одиннадцать кругов независимого аудита анализатор закрыл
десяток синтаксических форм, и каждый следующий круг находил новую — значение по умолчанию в деструктуризации,
тернарный контейнер, вычислимое имя свойства. Ни один круг не закрыл ни одного пункта плана владельца.
Анализатор при этом начал давать и ложные красные на ЗАКОННОМ коде (круг 11), то есть мешать работе. Замер
границы у такого инструмента отсутствует по построению: он доказывает форму записи, а не поведение двери.

Отчёты кругов 1–11 (`AUDIT_E5A_GATE_ROUND*.md`) оставлены как запись того, что именно инструмент умел и на чём
ломался: если когда-нибудь вернёмся к машинной проверке этой двери, начинать надо с этого списка форм, а не с
нуля.
