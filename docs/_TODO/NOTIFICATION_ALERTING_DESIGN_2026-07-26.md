...
> **УТОЧНЕНИЕ ВЛАДЕЛЬЦА 2026-07-27, СУЖЕНО 15.09.2026.** В бою аварийные алерты оператору идут во ВСЕ каналы по умолчанию и мимо пользовательских предпочтений (§21); набор каналов и адресов — настройка владельца в кабинете. **На стендах их доставка подавлена:** режим ТЕСТ глушит алерт целиком (`dispatchOperatorAlert.ts`, ранний выход по `env.TEST`, владелец 15.09: «надо дописать чтобы не орал»), на DEV уходит только письмо в петлевую ловушку (`AGENTS.md` §1b.2). Прежняя формулировка «мимо dev-фильтра» отменена и здесь не действует. Обычные уведомления админу (поддержка, статусы) — по §21, «туда куда настроено». Действующее правило: `docs/ARCHITECTURE/OWNER_PRODUCT_RULES.md` §28 пункт 3.
