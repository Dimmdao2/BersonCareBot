...
- проверка завершения — fault injection отдельно для missing config, enqueue failure, provider rejection и
  exhausted retries: пользователь не получает ложного «отправлено», владелец получает алерт, инцидент виден и
  закрывается только после успешной контрольной доставки.
