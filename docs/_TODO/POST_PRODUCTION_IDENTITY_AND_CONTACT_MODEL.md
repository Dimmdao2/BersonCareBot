...
### 4.7 Нейтральный auth-ответ скрывает критические сбои доставки от оператора

Public login/reset обязан сохранять anti-enumeration: по ответу нельзя определить, существует ли конкретный
email/телефон. Но нейтральный ответ пользователю сейчас ошибочно воспринимается как доказанная отправка, а
внутренняя ошибка enqueue/provider/config может быть проглочена без инцидента. Это разные границы:

- UI сообщает «если подтверждённый адрес существует, запрос принят», а не утверждает, что письмо уже отправлено;
- глобально известная недоступность канала/сервера может показываться всем одинаково, без проверки существования
  адреса: «сервис доставки временно недоступен, мы уже разбираемся»;
- каждый сбой конфигурации, постановки в очередь, исчерпания retry или отказа провайдера пишет структурированное
  событие без адреса и OTP, открывает/обновляет критический operator incident и получает correlation id;
- статусы разделяют как минимум `queued`, `smtp_accepted`, `provider_rejected/bounced` и подтверждённую доставку,
  если провайдер её умеет доказывать: успешный SMTP `DATA` и `accepted[]` не называется попаданием в ящик;
- SMTP `messageId`/безопасный provider reference сохраняется для трассировки, а асинхронные bounce/DSN или
  provider webhook обрабатываются тем же incident-контуром;
- инцидент обязан громко уведомить владельца по всем настроенным **независимым** административным каналам; сбой
  email нельзя алертить только через тот же email-провайдер;
- dashboard/health probe показывает открытый инцидент до подтверждённого восстановления, с dedupe/debounce, но
  без превращения повторяющегося критического отказа в тишину;
- проверка завершения — fault injection отдельно для missing config, enqueue failure, синхронного provider
  rejection, асинхронного bounce и exhausted retries: пользователь не получает ложного «отправлено», владелец
  получает алерт, инцидент виден и закрывается только после успешной контрольной доставки.

### 4.8 Политика 2FA не должна быть зашита в роль

Решение владельца 16.08: пользователь подключает фактор в настройках своего аккаунта, а глобальный
администратор выбирает в настройках платформы режим `выключена / добровольная / обязательная`. Текущий bounded
fix оставляет `добровольную`: незаведённый фактор не блокирует кабинет, заведённый обязательно проверяется.
После production нужен единый типизированный policy reader для login и guards, admin UI и fault-проверки всех
трёх режимов. Не возвращать старый булевый `auth_2fa_enabled`, который смешивал доступность механики и
...
10. Построить единый delivery-failure incident/alert контур для auth-кодов и остальных обязательных уведомлений:
    нейтральный public UX, структурированная телеметрия, независимые admin-алерты и fault-injection proof.
11. Реализовать трёхрежимную admin-политику 2FA (`disabled | optional | required`) одной точкой решения для
    login/guards; личный enrollment остаётся в настройках аккаунта.
...
- Любой критический сбой обязательной доставки создаёт наблюдаемый инцидент и независимый admin-alert; public UI
  не раскрывает существование адреса и не утверждает недоказанную отправку.
