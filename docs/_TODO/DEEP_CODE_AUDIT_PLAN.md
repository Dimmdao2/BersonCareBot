...
|---|---|---|---|---|---|
| `N1-001` | N1 · **CLOSED** | `app/app/patient/profile/DiaryDataPurgeSection.tsx`; `app/api/patient/diary/purge*/route.ts`; `app/app/patient/profile/page.tsx` | Существующий diary-purge flow подключён к единой privacy/data-control поверхности профиля; отдельный account purge не возвращался. Route acceptance и независимая приёмка зафиксированы в `DEEP_CODE_HUMAN_GAPS_CORRECTION_2026-09-02.md`; landed merge `923082777`. | Пациент снова может удалить собственные symptom/LFK diary data из продукта. | Закрыто подключением существующей двери, без второго purge service. |
| `N1-002` | N1 · **CLOSED** | `app/api/doctor/patients/[userId]/email-change/route.ts`; `app/api/patient/email-change/confirm/route.ts`; `shared/ui/patient/EmailAccountPanel.tsx`; `modules/auth/emailAuth.ts` | Существующая authenticated email-панель принимает admin-started challenge; DB lookup теперь обязательно фильтруется по purpose, поэтому более новый `email_verify` не затеняет `patient_email_change`. Независимый audit, acceptance 4 files / 7 tests и транзакционный DEV-preflight — PASS; landed merge `923082777`. | Пациент может завершить начатую администратором смену email, не попав в другой OTP flow. | Закрыто одной purpose-aware confirmation door, без третьего OTP consumer. |
| `N2-001` | N2 · **CLOSED** | `packages/shared-contracts/src/phone.ts`; process adapters webapp/integrator/platform-merge | Нормализация телефона вынесена в один dependency-neutral shared export; прежние process-файлы только делегируют. Независимый fault-injection audit — PASS; landed merge `a7cec664e`. | Все процессы получают один identity/contact key для одного номера. | Закрыто единым shared contract. |
| `N2-002` | N2 · **CLOSED** | `packages/shared-contracts/src/hlsStorageLayout.ts`, `hlsMasterPlaylist.ts`; adapters webapp/media-worker | HLS key/layout/playlist rules вынесены в один shared source; worker и webapp используют его через тонкие адаптеры. S3 lifecycle acceptance и независимая поломка доверия к poster path доказали границу; landed merge `a7cec664e`. | Worker и webapp больше не могут разойтись в путях и доверенных HLS-артефактах. | Закрыто единым shared media contract. |
| `N2-003` | N2 · **CLOSED** | `packages/shared-contracts/src/platformIntegrationAvailability.ts`; adapters webapp/integrator | Integration IDs, envelope и parser теперь имеют один shared source; UI и delivery сохраняют свои process adapters. Независимый fail-open fault injection — красный, штатная приёмка — PASS; landed merge `a7cec664e`. | Control-plane и delivery одинаково понимают включённые интеграции. | Закрыто единым shared contract. |
...
| `N1-Q2` | N1 · **SYMPTOM DECISION RESOLVED**; manual rehabilitation input remains **OWNER QUESTION** | `patient/diary/symptoms/SymptomTrackingRow.tsx`, `modules/diaries/components/SymptomChart.tsx`, `api/patient/diary/symptom-stats/route.ts`; retained `QuickAddPopup.tsx`, LFK clients and `quick-add-context` route | Patient diary now shows active doctor-assigned symptom trackings (excluding system wellbeing); one `PatientModal` records only a current 0–10 instant value through the existing Diaries path and refreshes its month history. The retained QuickAdd/LFK branches stay unmounted. | The symptom manual-input decision is implemented without restoring tabs, QuickAdd, self-created symptoms, daily choice, or clinical-domain bridge. Manual rehabilitation/LFK input has not been decided or implemented. | Do not reopen the symptom decision. Owner decision is still required before wiring or deleting the manual rehabilitation/LFK branch. |
...
1. **CLOSED — два человеческих разрыва закрыты одной существующей дверью:** `N1-001` подключён к profile privacy
   surface как resource cleanup; `N1-002` использует существующий purpose-aware authenticated email panel/confirm
   flow. Параллельные purge/OTP services не создавались; evidence — `923082777`.
2. **CLOSED — cross-process pure contracts вынесены в один пакет:** phone normalization (`N2-001`), HLS
   layout/playlist (`N2-002`) и integration availability schema/parser (`N2-003`) имеют один source и тонкие
   process adapters; evidence — `a7cec664e` и независимый audit report.
