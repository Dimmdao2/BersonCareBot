...
Кем: исполнители — Codex по тир-карте (`/home/dev/brain/docs/MODEL_TIERS.md`), не Claude. Это прямое требование
заказа.
...
| `N2-R1` | N2 · **REAL GAP** (сохранённый `Р1`) | `syncCalendarTimezoneFromDevice`; `syncPlatformUserCalendarTimezoneFromDevice` | Две двери одного правила calendar timezone уже расходились по условию «писать только если отличается»; точная исходная evidence и commit указаны в `Р1` выше. | Следующая правка опять может попасть только пациенту или только сотруднику, давая разную календарную дату одному человеку. | Одна параметризованная identity-timezone write door с role/principal adapters. |
...
### Упорядоченный correction plan — без исправлений в этом проходе
...
3. **Не смешивать уже заведённые workstreams:** `N2-R2` завершает `#1092`; `N2-R1` получает отдельную bounded
   consolidation работу после проверки call-site цены; `N2-R3` не переоткрывать.
