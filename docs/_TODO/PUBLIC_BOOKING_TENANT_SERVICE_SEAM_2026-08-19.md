...
- [x] ~~F1. Оплаченное число клиентов снова один проход.~~ **SUPERSEDED 19.08, миграция 0053 —
      см. ниже.** Исходная находка F1 (создателей строки `org_enrollments` ровно два, а проверка
      была только у одного) была измерена верно, но её закрытие в 0052 — подключить публичную дверь
      к той же проверке лимита — оказалось решением ПРОТИВ владельца: живой человек, пришедший
      записаться на приём, получал отказ из-за чужого тарифного лимита. Владелец (§33.2, дословно):
      «Запись на приём сама по себе лимита не расходует и не должна его расходовать… ограничивать
      его количеством записавшихся это бред». Разбирательство, почему предупреждение до начала
      работы не было учтено полностью, не проводилось — только правка.

- [x] **F1 (REVERTED, миграция 0053). Публичная запись не расходует лимит клиентов.** Вызов
      `app.assert_org_patient_count_quota_available` убран из
      `app.enroll_current_patient_in_public_booking_clinic` целиком; сама функция проверки не
      менялась и не удалена — у неё снова ровно один вызывающий, писатель карточек персонала
      (`ensureInvitedOrganizationClientRelationship`), как и до 0052. Второй копии правила не
      заведено (§5). Живое доказательство на DEV: клиника на пределе клиентов (limit=1, уже 1
      клиент) принимает публичную запись — `enroll_current_patient_in_public_booking_clinic`
      возвращает `{"effect": "created"}` без `53400`; тот же вызов под контекстом персонала
      (`ensureInvitedOrganizationClientRelationship`) на той же клинике по-прежнему отказывает.
      Другие места записи (F2 компенсация, F3 канал) тарифных механик не расходуют — проверено
      построчно по всей миграции 0052, других вызовов `quota`/`tariff`-логики в записи нет.
