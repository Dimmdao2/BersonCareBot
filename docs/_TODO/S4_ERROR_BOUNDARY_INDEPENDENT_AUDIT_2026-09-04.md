...
## 9. Дополнение: correction-проход по этому же оракулу (04.09.2026, отдельный исполнитель)

**Вердикт §1–§8 выше не меняется: он остаётся `FAIL` на кандидате `babdc87e8`.** Этот раздел лишь
дописывает, что было сделано по нему дальше, и не переписывает историю аудита. Kill-set K1–K14,
карта инъекций FI-1…FI-DRIFT и перепись §3 переиспользованы как есть; второй слепой проход не
проводился — поверхность двери не менялась (§24.5).

**Найденное дополнение к §1.** MUST FIX аудита указан верно, но был неполон: после механической
правки мока ведущим (`c3ebdae26`) фазовый набор дал **пять** красных файлов, а не ноль. Причина
расхождения — в §6 записан итог только проекта `unit` (`1 failed / 226 passed (227)`); проекты
`route` и `ui` из `test:webapp:behavior` в тот прогон не дошли, потому что цепочка
`test:unit && test:route && test:ui` обрывается на первом падении. Вывод §1 «остальные 44 файла не
падают» верен для `unit` и неверен для `route`: там ещё три файла мокают
`@bersoncare/db-principal` без `getCurrentObservabilityContext` — `payments/patientAcquiring`,
`payments/saasWebhook`, `clinic/invites`.

**Второй класс, которого нет в §2 kill-set.** K2 назван как «известный доменный код схлопнут в
fallback» и проверялся на HTTP-двери, где он пойман. На server-action транспорте тот же класс остался
непроверенным: `safeActionErrorCode` схлопывал в `forbidden`/`toggle_failed` авторское русское
предложение отказа по тарифу, которое панель показывает врачу дословно. Красный оракул на это в
репозитории уже был — `api/tariffMechanics.route.test.ts`, два утверждения, — и в fixer-проход он
попал как заданный.

**Что сделано исполнителем (продуктовый код аудитор по-прежнему не менял).** Три мока доведены тем же
паттерном, что кандидат применил к `admin/commercial` и `clinic/billing`; отказ по тарифу переведён на
уже существующий доверенный класс двери `TypedApiResponseError` через новый третий транспорт
`entitlementMutationRefusalError` рядом с `entitlementMutationRefusalMessage`/`…RefusalResponse` —
без второй двери, без параметризации `safeActionErrorCode`, без подстрочных эвристик. Полное evidence,
причинность каждого падения, три новые инъекции и прогоны — в
`docs/_TODO/S4_ERROR_BOUNDARY_IMPLEMENTATION_2026-09-04.md`, раздел «S4 — correction-проход».

**Пятый красный файл — не S4.** `modules/auth/passwordAuth.route.test.ts` зависит от переменной
окружения `TEST_ACCOUNT_PHONES` (внесено `592ed97e8` от 27.08, за восемь дней до кандидата); ни один
файл этого пути кандидатом не тронут. Остаётся открытым не-S4 блокером фазового набора.

**Рекомендации §5 и вопросы §7 не тронуты** — `String(error)`, касты, деструктуризация, correlation id
для server actions и 69 файлов замороженного долга остаются вопросами владельцу. Строку очереди в
`accepted` correction-проход не переводит.
