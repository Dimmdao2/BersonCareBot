...
> **Статус документа: historical implementation evidence для подчинённого punch-list.** Он не подтверждает
> выполнение более поздних требований из
> [`SAAS_PRODUCT_UX_INITIATIVE/OWNER_REVIEW_2026-07-18.md`](./SAAS_PRODUCT_UX_INITIATIVE/OWNER_REVIEW_2026-07-18.md).
> В частности, отметка старого кластера «Сегодня» как DONE не закрывает новый единый контракт KPI-карточек.
> При расхождении scope/status канон owner-review побеждает; этот файл сохраняет только доказательства по своим ID.
...
| **SCH-G5** | логика | **TODO — решение владельца 2026-07-25 (вопроса НЕТ)** | Недельный график — ЭТО график. Смысл недельного графика: в указанные дни недели слоты есть ВСЕГДА по нему, без ручной настройки каждого дня. Значит поведение «день без per-date `be_working_days` → слоты по weekday `be_working_hours`» — **ПРАВИЛЬНОЕ и есть сам замысел** (это НЕ «fallback vs строго-графо-ведомо», недельный и дневной — оба график). Реализовать weekday-график: `booking-scheduling/service.ts:433-462` + `computeSlots.ts:161-175`. Никакого вопроса/model-decision. |
| **PRG-4** | новая фича | **TODO — решение владельца 2026-07-25 (НЕ hold, НЕ многодневная)** | Задача УЗКАЯ: при РЕДАКТИРОВАНИИ программы упражнений КОНКРЕТНОГО клиента уметь создать НОВОЕ упражнение ТОЛЬКО внутри его программы + загрузить туда видео. Не «многодневная эпопея», отдельный sign-off §5 не нужен. Реализация: индивидуальное упражнение, привязанное к его program-instance (scope/isPersonal), presign-роут доктора для видео (HLS), в edit-mode программы. Дизайн-ориентир (не догма): `docs/_TODO/PROGRAM_INDIVIDUAL_ITEM_DESIGN.md`. |
