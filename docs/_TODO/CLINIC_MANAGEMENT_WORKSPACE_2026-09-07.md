...
- [x] Режим кабинета — собственное свойство тарифа, а не вывод из числа мест (owner 10.09.2026: «Число мест -
  не показатель, админ клиники может начинать с одного себя и приглашать других - у соло механика приглашений
  отключена в принципе… значит надо сделать и само свойство и переключатель в настройках тарифа - и режим
  кабинета (соло или клиника) строить на основе этого свойства»). Свойство — механика `clinic_team`
  («Режим кабинета» в конструкторе тарифов: `Соло-специалист` / `Клиника с командой`); поля числа мест и цены
  доп. места показываются только в режиме клиники, соло сохраняется с `includedSeats = 1` и без цены места.
  Правило переписано во ВСЕХ трёх дверях: `resolveDoctorWorkspaceComposition` (число мест больше не решает,
  `used > 1` остался только страховкой после понижения тарифа), `isMechanicIncludedFromSnapshot`/
  `normalizeTariffInput` и в базе — миграция
  `20260910T142000_the_cabinet_mode_is_a_tariff_property_not_a_seat_count.sql` (якорная переписка живых
  `app.resolve_organization_mechanic_access` и `app.accept_org_invite`, плюс backfill флага тарифам, которые
  продают больше одного места). У соло в кабинете не показываются ни строка механики «Режим клиники», ни
  число мест. Evidence: живой DEV — тариф с 1 местом и режимом «Клиника с командой» даёт management-кабинет
  (вкладка «Команда», `/app/manage`), тот же тариф в режиме «Соло-специалист» — единый хаб `/app/settings`
  без «Команды» и мест, `GET/POST /api/clinic/invites` отвечают `403 entitlement_required`; переключатель
  прошёл круг клиника→соло через сам интерфейс конструктора.
