...
### Что уже работает
- `exchangeTelegramInitData` — вход в Mini App через подпись Telegram initData (`apps/webapp/src/modules/auth/service.ts:284`).
- `exchangeIntegratorToken` — обмен JWT от бота на сессию вебаппа.
- `request_contact` в интеграторе — бот запрашивает номер через Telegram-кнопку, сохраняет в `channel_users` (`apps/integrator/src/kernel/domain/usecases/handleUpdate.ts:125-146`).
- `isPhoneRequestButton` — формирование клавиатуры с `request_contact: true` (`apps/integrator/src/kernel/domain/executor/helpers.ts:314`).
- SMS OTP flow — `PhoneInput` → `check-phone` → `phone/start` → `phone/confirm` → сессия.
- Email OTP — уже есть в backend (`deliveryChannel: "email"` в `phone/start`), но целевое состояние: не публиковать в auth UI, использовать как подключаемый канал в профиле.
- Yandex OAuth — каркас: `oauth/start`, `oauth/callback`, `oauthService.ts`, `pgOAuthBindings.ts`.
- `PhoneInput` с валидацией РФ — inline-ошибка «Введите корректный номер (10 цифр)».

### Что сломано / неоптимально
- **`integratorSmsAdapter.ts`**: HTTP-ошибки интегратора → `code: "invalid_phone"` → «Неверный формат номера» в UI. Нужен отдельный код `delivery_failed`.
- **Только РФ-номера**: `normalizePhone` и `isValidRuMobileNormalized` отсекают международные номера.
- **Нет Telegram Login Widget**: в вебе единственный способ войти — ввод номера.
- **PIN показывается в auth-flow**, хотя по продукту его нужно временно скрыть из публичного входа.
- **Дублирование `normalizePhone`**: `apps/webapp/src/modules/auth/phoneNormalize.ts` (РФ-only) и `apps/integrator/src/kernel/domain/usecases/handleUpdate.ts:13` (более толерантный, принимает международные).
...
| `modules/auth/phoneNormalize.ts` | normalizePhone (РФ-only) |
| `modules/auth/phoneValidation.ts` | isValidRuMobileNormalized |
...
| `shared/ui/auth/PhoneInput.tsx` | UI: поле ввода телефона (РФ) |
...
| `app/api/auth/oauth/callback/route.ts` | API: callback Yandex OAuth |
...
| `shared/ui/auth/PhoneInput.tsx` | 2 | Замена на InternationalPhoneInput |
