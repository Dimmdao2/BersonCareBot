...
> ⚠️ **ЧАСТИЧНО УСТАРЕЛО (2026-05-01).** §5 «Карточка пациента — главный экран» (tabs «Заметки и визиты /
> Назначения / Дневники / Сообщения / Учётная запись», hero «Что важно сейчас», компонент `ClientProfileCard.tsx`)
> заменён фактически реализованной картой на новом роуте/компоненте. Актуально:
> [`../design/bersoncare-карточка-пациента-CURRENT-SPEC.md`](../design/bersoncare-карточка-пациента-CURRENT-SPEC.md)
> (4 вкладки Карточка/Программа/Файлы/Учётка, без Обзора, без hero, звезда+portal-invite в шапке). Остальные разделы
> (§1–4, 6–15: кластеры меню, «Сегодня», каталоги назначений, контент, коммуникации) по-прежнему живой ориентир.
>
...
Исторически меню было плоским списком с разделителями. **По состоянию на 2026-05-02** в коде — кластеры + аккордеон + отдельный пункт «Библиотека файлов» между «Контент приложения» и «Коммуникации» ([`DOCTOR_MENU_RESTRUCTURE_EXECUTION_AUDIT.md`](done/DOCTOR_MENU_RESTRUCTURE_EXECUTION_AUDIT.md)); бейджи на «Онлайн-заявки» и «Сообщения» — [`DOCTOR_NAV_BADGES_PLAN.md`](done/DOCTOR_NAV_BADGES_PLAN.md). Ниже — целевая текстовая модель для дальнейшей редакции названий/IA без смены структуры кластеров.
...
- **Реализация этапа 2 меню (2026-05-02):** в продукте «Библиотека файлов» — **отдельный верхнеуровневый пункт между** кластерами «Контент приложения» и «Коммуникации», не внутри сайдбара CMS ([`DOCTOR_MENU_RESTRUCTURE_PLAN.md`](done/DOCTOR_MENU_RESTRUCTURE_PLAN.md)). Ниже черновая блок-схема §3 всё ещё показывает библиотеку внутри блока «Контент» — при следующей редакции TARGET имеет смысл выровнять текст/диаграмму с фактом в коде.

## 4. «Сегодня» врача — рабочий день, не отчёт

**Факт (2026-05-02):** MVP экрана «Сегодня» реализован ([`DOCTOR_TODAY_DASHBOARD_PLAN.md`](done/DOCTOR_TODAY_DASHBOARD_PLAN.md), [`LOG.md`](LOG.md)) — записи на сегодня, новые заявки, непрочитанные сообщения, ближайшие записи; метрики на `/stats`. Ниже — целевая полнота inbox (включая то, что пока **не** сделано: «К проверке», проактивная лента).
...
- На каждой карточке элемента — **«Где используется»**: «упражнение в 5 шаблонах», «комплекс назначен 12 пациентам». **Реализовано** ([`ASSIGNMENT_CATALOG_USAGE_ARCHIVE_PLAN.md`](done/ASSIGNMENT_CATALOG_USAGE_ARCHIVE_PLAN.md)).
...
**Шаг 2026-05-02 (вариант C):** в репозитории уже есть `kind` (`article` \| `system`) и `system_parent_code` для кластеров вместо полного enum выше; целевая схема в дереве остаётся ориентиром на следующие итерации — см. [`CMS_RESTRUCTURE_PLAN.md`](done/CMS_RESTRUCTURE_PLAN.md).
...
- **SUPERSEDED AS TARGET — 2026-07-27:** compose form cannot select channels; §21 resolves them. См. строку **«Уведомления»** в [`CURRENT_AUTHORITY_MAP.md`](../CURRENT_AUTHORITY_MAP.md).
...
| **Карточка пациента**        | **Микро-проход (2026-05-02):** без классического аккордеона, sticky-шапка, специалист выше контактов — [`DOCTOR_CLIENT_PROFILE_REPACK_EXECUTION_AUDIT.md`](done/DOCTOR_CLIENT_PROFILE_REPACK_EXECUTION_AUDIT.md). **Цель:** табы + hero — этап 6 глубокая часть заморозки [`PLAN_DOCTOR_CABINET.md`](PLAN_DOCTOR_CABINET.md).                    |
| **CMS**                      | **Сделано:** вариант C — `kind` + `system_parent_code`, фильтры patient-home ([`CMS_RESTRUCTURE_PLAN.md`](done/CMS_RESTRUCTURE_PLAN.md)). **Дальше:** расширение до полного enum / табов хаба — roadmap IV этапы 2–3.                                                                                                                            |
...
**Актуализация 2026-05-04 (owner, см. [`ROADMAP_2.md`](ROADMAP_2.md), [`LOG.md`](LOG.md)):** редизайн **UI/IA кластера «Назначения»** в кабинете врача — **практически закрыт** (возможны точечные хвосты по приёмке). По каталогам — execution B1–B7 закрыт архивным [`AUDIT_GLOBAL`](../archive/2026-05-initiatives/ASSIGNMENT_CATALOGS_REWORK_INITIATIVE/AUDIT_GLOBAL.md). **Шаблоны программ** — остаётся **небольшой хвост** в списке/конструкторе; **не блокер** для следующего цикла patient polish. **Курсы** — **отложены** владельцем (§15.7).
...
Курсы не входят в «Назначения», не используют движок программ. См. [`../COURSES_INITIATIVE/README.md`](../COURSES_INITIATIVE/README.md). Ранее планировалось: старт **последней** инициативой после ядра пациентского `PROGRAM_PATIENT_SHAPE_PLAN` и оплаты. **Отложено владельцем (2026-05-04):** не в ближайшую очередь — см. [`LOG.md`](LOG.md) и [`ROADMAP_2.md`](ROADMAP_2.md).
