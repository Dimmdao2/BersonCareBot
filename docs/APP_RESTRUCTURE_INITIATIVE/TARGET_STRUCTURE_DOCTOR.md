...
Исторически меню было плоским списком с разделителями. **По состоянию на 2026-05-02** в коде — кластеры + аккордеон + отдельный пункт «Библиотека файлов» между «Контент приложения» и «Коммуникации» ([`DOCTOR_MENU_RESTRUCTURE_EXECUTION_AUDIT.md`](DOCTOR_MENU_RESTRUCTURE_EXECUTION_AUDIT.md)). Ниже — целевая текстовая модель для дальнейшей редакции (баджи и уточнения названий — следующие этапы).
