...
4. Получения важных уведомлений (от специалиста + по личным напоминаниям).
...
├ Поддержка (чат со специалистом; форма обращения встроена в чат)
├ Помощь / FAQ
├ Установить как приложение (PWA-подсказки)
└ Выход (если не TG WebApp)
```

> **Факт реализации (PWA, 2026-05):** публичный лендинг и установка из браузера — на **`/`** ([`docs/PWA_INITIATIVE/`](../PWA_INITIATIVE/README.md)); пункт «Установить как приложение» в кабинете — маршрут **`/app/patient/install`** (инструкции в сессии пациента).
...
- Темы рассылок от специалиста (рассылки врача, новости, важное) = «что мне присылают».
...
4. ~~**Чек-ин самочувствия — куда сохраняется?**~~
   **Закрыто (2026-05-08):** сохраняется в **дневник симптомов** — `symptom_entries` для системного трекинга **`general_wellbeing`** (legacy `patient_daily_mood` снят миграцией). Пациент не создаёт новые symptom trackings сам; недельная полоска на главной — `GET /api/patient/mood/week`. См. `apps/webapp/src/modules/patient-mood/patient-mood.md`. Правила «ещё раз за короткое время» (окно **5 мин**), зеркальная instant после разминки (`WELLBEING_GENERAL_MIRROR_NOTE`) и отсутствие **409** — **2026-05-17**; см. тот же `patient-mood.md`, `apps/webapp/src/modules/patient-practice/patient-practice.md`, запись **2026-05-17** в [`LOG.md`](LOG.md).
...
- Врач задаёт на элементе экземпляра флаг **`is_actionable`** (`true` — попадает в чек-лист с галочкой; `false` — persistent, без completion). При появлении новой строки экземпляра (назначение из шаблона или добавление рекомендации в программу) по умолчанию **`false`** (постоянная), переключение — в UI врача.
