...
> ⚠️ **ЧАСТИЧНО УСТАРЕЛО (2026-05-04).** §4 «Часть 2 — Снять заморозку doctor card этапа 6» / §2.1 «Doctor card
> hero программы + tab-layout» (табы «Программы / Тесты к проверке / ЛФК / Чат / История» + hero в
> `ClientProfileCard.tsx`) и строка «❄ FROZEN намеренно» в таблице §0 описывают план, который в итоге реализован
> по-другому. Актуально: [`../design/bersoncare-карточка-пациента-CURRENT-SPEC.md`](../design/bersoncare-карточка-пациента-CURRENT-SPEC.md)
> (4 вкладки Карточка/Программа/Файлы/Учётка, без hero). §2.2 и §2.3 того же раздела (cross-patient inbox, deep
> link `focusItemId`) остаются в силе — реализованы отдельно и подтверждены как закрытые. Остальные разделы
> документа (patient IA, каталоги, D5, фоновые хвосты) не затронуты.
...
**Синхронизация 2026-06-02:** §1.7 `/help` и §3.3 CMS enum (pragmatic) — **закрыты** (фаза 6 workqueue). §1.2 diary → **TODO**; §1.3 cancelled; §1.4–§1.6 closed; §3.1 D5 → **TODO**; §3.2 **0040** closed; §4.2 Курсы — вне активного цикла. **Workqueue фазы 0–7 закрыты** — **[`ACTIVE_WORKQUEUE.md`](../ACTIVE_WORKQUEUE.md)**, **[`docs/TODO.md`](../TODO.md)** §Doctor card; план [`.cursor/plans/archive/active_workqueue_plan_30236040.plan.md`](../../.cursor/plans/archive/active_workqueue_plan_30236040.plan.md).
...
**Синхронизация 2026-06-02 (фаза 7 workqueue):** §**4.1** proactive MVP на «Сегодня» + B6 превью шаблонов — **✅ закрыты** (модуль `doctor-proactive-insights`; LOG §фаза 7; расширение сигналов — backlog этап 8).
...
| **§1.1b** (старый план визуального редизайна detail под `treatment-programs` + URL `stages/[stageId]`)                                                          | ✅ **закрыто по факту**; **устарело как ТЗ**                      | Актуальная реализация и карта модулей: [`../../apps/webapp/src/app/app/patient/treatment/program-detail/README.md`](../../apps/webapp/src/app/app/patient/treatment/program-detail/README.md) · мини-инициатива: [`../archive/2026-05-initiatives/PATIENT_TREATMENT_PROGRAM_PAGE_INITIATIVE/README.md`](../archive/2026-05-initiatives/PATIENT_TREATMENT_PROGRAM_PAGE_INITIATIVE/README.md) · детальный текст §1.1b в этом файле — только контекст |
...
5. **Проверки выбираются по риску.** В рамках мини-итераций — узкие проверки (lint затронутых файлов, `tsc --noEmit` пакета, узкий `vitest`). Полный `pnpm run ci` запускается только для конкретного непокрытого интеграционного риска по [`AGENTS.md` §9](../../AGENTS.md#9-full-ci-gate); push сам по себе таким риском не является.
...
- Если активной программы нет — empty state с пояснением «Здесь появится программа после назначения врачом» и ссылкой на `/messages` для связи со специалистом.
...
- Мини-инициатива: [`docs/archive/2026-05-initiatives/PATIENT_TREATMENT_PROGRAM_PAGE_INITIATIVE/README.md`](../archive/2026-05-initiatives/PATIENT_TREATMENT_PROGRAM_PAGE_INITIATIVE/README.md).
...
_(Далее в исторической версии следовали блоки: порядок секций, цветовые зоны, B1–B7, маршрут `stages/[stageId]`, файлы, DoD — см. git history файла `ROADMAP_2.md` до 2026-05-13 при необходимости.)_
...
- Внутри — чат со специалистом как основной режим; форма «отправить письмо администратору» — fallback внутри той же страницы (не отдельный URL).
...
- Группа «Связь со специалистом» — единый пункт «Поддержка» (после 1.5).
...
**Статус (2026-06-02):** частичный **MVP** на «Сегодня» и в карточке клиента — модуль `doctor-proactive-insights`, сигналы `wellbeing_low_streak` и `program_inactivity` для `on_support`; бейдж «Сегодня» = тесты + сигналы. **Backlog:** настраиваемые пороги, пропуски ЛФК/сессий, боль, записи на приём, полный агрегатор этапа 8.
