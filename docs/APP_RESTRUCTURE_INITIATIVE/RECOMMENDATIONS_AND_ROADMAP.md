...
2. **Ссылки «Полезная информация» / `/help`.** **2026-05-01:** убраны битые плитки без контента (см. [`LOG.md`](LOG.md) §пункт 6). **2026-06-02 (фаза 6):** `/help` из CMS; **2026-06-03:** IA canonical slug (`services-pricing`, адреса по городам и др. — `help-content/canonicalSlugs.ts`); плитки «Как подготовиться» / «Стоимость» при `preparation` + `services-pricing` (legacy `cost`); монтирование на «Запись» — план patient_help_booking фаза 2.
