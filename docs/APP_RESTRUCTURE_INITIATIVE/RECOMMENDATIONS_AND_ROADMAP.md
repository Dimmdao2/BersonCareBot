...
> ⚠️ **ЧАСТИЧНО УСТАРЕЛО (2026-05-01).** §II.3 «Карточка пациента `/clients/[userId]`» (целевые табы «Заметки и
> визиты / Назначения / Дневники / Сообщения / Учётная запись» + hero) и часть «Этап 6» про глубокую переработку
> карточки (её «заморозку») описывают решение, впоследствии заменённое другим макетом. Актуально:
...
1. **Дублирующиеся сущности.**
   ...
   - Поддержка: `/messages` (чат) и `/support` (одно письмо админу) — для пациента это один сценарий «связаться со специалистом».
   - Контент: «Ситуации» на главной + страница `/sections` («Уроки и тренировки») — содержательно дублируются с разной формулировкой.

2. **Ссылки «Полезная информация» / `/help`.** **2026-05-01:** убраны битые плитки без контента (см. [`LOG.md`](LOG.md) §пункт 6). **2026-06-02 (фаза 6):** `/help` из CMS; **2026-06-03:** план `patient_help_booking_surface` **закрыт** — плитки на «Запись», city-aware адрес, `/app/patient/about`, `CMS_EDITOR_CHECKLIST.md`, `/help/booking` → about.
...
- **`/messages` + `/support`:** для пациента это один сценарий «связаться со специалистом» — стоит сделать одну точку входа.
