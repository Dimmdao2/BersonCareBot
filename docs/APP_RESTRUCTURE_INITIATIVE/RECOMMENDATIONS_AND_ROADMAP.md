...
2. **Ссылки «Полезная информация» / `/help`.** **2026-05-01:** убраны битые плитки «Подготовка»/«Стоимость» без контента (см. [`LOG.md`](LOG.md) §пункт 6). **2026-06-02 (фаза 6):** `/help` из CMS; `CabinetInfoLinks` снова показывает «Как подготовиться» / «Стоимость» **только** при опубликованных статьях slug `preparation` / `cost`; компонент готов, монтирование на экран «Запись» — отдельно.
