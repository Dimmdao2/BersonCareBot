...
| Режим техработ patient app | **Операционный guard (2026-05-02):** при включении в админке клиенты видят один экран под `/app/patient` (сообщение, внешняя запись Rubitime, ближайшие записи); без новых env; ключи `patient_app_*` / `patient_booking_url`. Детали и чек-листы закрытия — [`PATIENT_MAINTENANCE_MODE_EXECUTION_AUDIT.md`](PATIENT_MAINTENANCE_MODE_EXECUTION_AUDIT.md), запись в [`LOG.md`](LOG.md). **Не** заменяет целевую IA из §I.3 / [`TARGET_STRUCTURE_PATIENT.md`](TARGET_STRUCTURE_PATIENT.md). |
