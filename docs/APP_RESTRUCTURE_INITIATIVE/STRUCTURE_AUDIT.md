...
> Соответствует своей дате 2026-05-01 (immutable snapshot, не переписывается — см. ниже). §II.3 «Карточка
> пациента — `ClientProfileCard.tsx`» фиксирует факт **на тот момент**; текущая карточка пациента живёт на другом
> роуте/компоненте и описана в
