...
**Статус (2026-05-04):** экраны каталогов в цепочке **упражнения → клинические тесты → наборы тестов → комплексы ЛФК → рекомендации** — **почти доведены** до целевого UX и поведения (возможны точечные хвосты по приёмке). **Шаблоны программ лечения** — **ещё доделываются** (список, конструктор, CTA/модалки). Ниже — живое ТЗ и чек-листы; правки по мере закрытия остатков.
