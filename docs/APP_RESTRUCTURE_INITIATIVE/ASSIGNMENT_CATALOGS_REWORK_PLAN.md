...
**Статус (2026-05-04, обновление sync owner):** цепочка каталогов **упражнения → клинические тесты → наборы тестов → комплексы ЛФК → рекомендации** и **IA/UI блока «Назначения»** в кабинете врача — **практически закрыты** (возможны точечные хвосты по приёмке). **Шаблоны программ лечения** — остаются **небольшие правки** (список, конструктор, CTA/модалки); **не блокер** для patient IA и карточки врача — см. [`ROADMAP_2.md`](ROADMAP_2.md). Ниже — живое ТЗ и чек-листы.

**Первоначальная версия документа:** 2026-05-03.
