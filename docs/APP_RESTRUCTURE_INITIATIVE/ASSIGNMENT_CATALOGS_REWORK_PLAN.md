...
**Scope:**
- Превьюшки в списке шаблонов (миниатюра / счётчик этапов / счётчик items).
...
|---|---|---|
| Q1 | Финальный список значений `assessmentKind` для клин. тестов (предложен `mobility/pain/sensitivity/strength/neurodynamics/proprioception/balance/endurance` — нужен ли расширить/сократить) | До B2 |
...
| Переименовать в коде `recommendations.domain` → `recommendations.kind` (полный refactor) | По решению Q4 |
| Расширить `CreatableComboboxInput` до **многозначного** (теги) | По запросу |
| Админ UI для модерации `measure_kinds` (переименовать/объединить/удалить) | Q6 = да |
| `publication_status` на упражнениях / клин. тестах / рекомендациях | По запросу клиники |
| Отдельный API для bulk-операций над items внутри контейнера (наборы тестов, шаблоны) | После B3/B6, по нагрузке |
