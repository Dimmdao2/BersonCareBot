...
- ~~Библиотека файлов~~ — **перенесена в основное меню врача** (отдельный верхнеуровневый пункт, этап 2, 2026-05-02); внутри CMS-хаба ссылки на медиатеку нет.
...
- ~~Слот-разделы CMS не имеют защиты от переименования slug~~ — **снято для опасных операций:** блокировки при ссылках с главной пациента и для встроенных slug (см. §5.3 выше). **Остаётся:** довести продуктовую типизацию `kind` до целевого enum из roadmap (сейчас в коде вариант C: `article` \| `system` + `system_parent_code`).
