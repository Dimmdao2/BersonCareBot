...
> ⚠️ **ЧАСТИЧНО УСТАРЕЛО (2026-05-04).** §7 «Хвосты PLAN_DOCTOR_CABINET этапа 6 (карточка пациента у врача)»
> (недостающие hero/tab-layout «Программы / Тесты к проверке / ЛФК / Чат / История») и связанная строка в §8
> («Этап 6 PLAN_DOCTOR_CABINET: hero программы + tab-layout») описывают backlog карточки, которая с тех пор
> пересобрана по другому макету. Актуально:
> [`../design/bersoncare-карточка-пациента-CURRENT-SPEC.md`](../design/bersoncare-карточка-пациента-CURRENT-SPEC.md).
> Остальная приёмка (домен программы лечения, каталоги, UX-фиксы A/B) остаётся верной как исторический отчёт.
...
> **Синхронизация 2026-05-04:** ниже — хвосты, закрытые в ходе acceptance-фиксов; **отдельно** остаётся небольшой продуктовый хвост по списку/конструктору шаблонов (не блокер для других работ — см. [`ROADMAP_2.md`](ROADMAP_2.md)).
