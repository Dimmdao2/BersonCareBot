...
3. API:
   - `GET /api/references/:categoryCode` → список items (is_active=true).
   - `POST /api/doctor/references/:categoryCode` → добавить item (doctor-only).
   - `PATCH /api/admin/references/:itemId/archive` → пометить удалённым (admin-only, soft-delete).
4. Правила удаления:
   - **Врач не может удалять** значения справочника.
   - **Админ** может помечать удалёнными (soft-delete: `is_active = false`).
   - Помеченные удалёнными **остаются видимыми** в уже привязанных местах (симптомы, комплексы), но **скрываются** в списке справочника и недоступны для нового добавления.
5. Добавить поле `tenant_id UUID` (nullable) в `reference_categories` для будущей мультитенантности.

**Критерий:**
- Таблицы созданы, seed данные загружены.
- API возвращает справочники.
- Доктор может добавлять значения, админ может архивировать.
- Архивированные не появляются в выпадающих списках, но видны в привязанных записях.
...
3. Все новые поля — **nullable** (обратная совместимость с тестовыми данными).
4. `symptom_title` (свободный текст) остаётся основным полем — пользователь может отслеживать **произвольные** симптомы (тревожность, усталость, боль в пятке и т.д.), не только из справочника. Справочники — опциональное дополнение.

**Критерий:**
- Миграция применяется без ошибок.
- Существующие тестовые данные не затронуты.
- API возвращает новые поля (null для старых записей).
- Пользователь может создать симптом только с произвольным названием (без справочников).
