# Этап 18: Перенос инфраструктуры на российский сервер 🔒 ОТВЕТСТВЕННОСТЬ ВЛАДЕЛЬЦА

> Приоритет: P3
> **Статус:** выполняется владельцем самостоятельно. Вероятен перенос образа сервера целиком.
> Инструкции по коду от агентов не требуются.
> Ниже — справочная информация для владельца (не задание для авто-агента).
