...
1. **Верстка главной в бот-режиме** — сейчас `materials` и `assistant` рендерятся существующими компонентами (`PatientHomeLessonsSection`, отсутствует `assistant`). Нужно:
   - Создать компонент «Помощник» (управление напоминаниями о разминках) → `PatientHomeAssistantSection`
   - Проверить, что `PatientHomeLessonsSection` нормально выглядит как единственный/главный блок в бот-режиме
