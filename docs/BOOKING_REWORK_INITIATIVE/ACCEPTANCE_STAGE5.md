...
**Статус:** `done` (2026-06-06 — ручной проход и подпись владельца)  
**ROADMAP:** [`ROADMAP.md`](ROADMAP.md) §11 · [`LOG.md`](LOG.md) §2026-06-06
...
- [x] Агентская часть (§5.A ниже) — выполнена и проверена.
- [x] Ручной проход владельца по §5.B — пройден.
- [x] Владелец явно подтверждает: **«Новый интерфейс записи принят»**.
- [x] Обновлены `ROADMAP.md` (этап 5 → `done`), `LOG.md` (запись о приёмке).
...
- [x] `/app/doctor/admin/booking` — вкладки «Обзор и настройка», «Форма и публичная запись», «Оплата», «Интеграция Rubitime» отображаются корректно
- [x] Обзор: runbook-шаги — ссылки кликабельны, ведут к нужным секциям страницы
- [x] Обзор: секции «Локации», «Услуги», «Доступность», «Правила» в одной прокрутке
- [x] Форма+публичная: форма записи и виджет на одном экране
- [x] Оплата и Rubitime: вкладки работают, контент присутствует
- [x] Навигация в меню: «Настройки записи» отображается в кластере «Администрирование»
- [x] «Мердж пациентов» — отсутствует в «Работа с пациентами»; присутствует в «Администрирование»

### Работа врача — Записи

- [x] `/app/doctor/appointments` — вкладки «Записи» и «Расписание» в тулбаре
- [x] Будущие записи сгруппированы по датам, сортировка ASC (ближайшая дата первая)
- [x] Переключатель «Архив»: записи в порядке DESC (свежие сверху)
- [x] Кнопка «Загрузить ещё» подгружает следующую страницу архива
- [x] Клик на запись раскрывает действия (DoctorAppointmentActions)
- [x] «Создать запись» открывает диалог; форма работает; после создания список обновляется
- [x] Вкладка «Расписание» видна только при `role=admin`; содержит секции расписания
...
| Владелец постановки | 2026-06-06 | ✅ «Новый интерфейс записи принят» |
