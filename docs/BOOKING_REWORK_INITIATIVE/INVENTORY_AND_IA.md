...
| **2.3b cutover (ops)** | `booking_slots_read_source=canonical` — после smoke и `mapped_ok` на staging. `booking_doctor_appointments_read_source=canonical` — **defer → этап 4** (календарь). |
