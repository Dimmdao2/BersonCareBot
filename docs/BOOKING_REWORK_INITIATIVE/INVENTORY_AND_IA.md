...
- Календарь и список клиентов могут читать отсюда.
...
- Критично для: bridge projection, `resolveCanonicalFromBranchService`, abonnement service resolution (этап 3).
...
|------|----------|---------|
| `booking_doctor_appointments_read_source` | `rubitime_legacy` \| `canonical` | Список записей врача, KPI, **календарь** |
...
**Patient / public create & slots (критичный контракт):**

- `GET /api/booking/slots`, `GET /api/booking/public/slots` — query `branchServiceId`
- `POST /api/booking/create`, `POST /api/booking/public/create` — body `branchServiceId` для `in_person`
- UI: `/app/patient/booking/new/*`, `/book/new/*`, `useBookingSlots`, `useCreateBooking`, `ConfirmStepClient`
- Memberships/products available: резолв `serviceId` через `resolveInPersonContext(branchServiceId)`
...
**Admin (скрыть из UX, не удалять из API до этапа 2):**

- `BookingPublicWidgetSection` — параметр ссылки с `branchServiceId`
...
| **Этап 2 (adapter)** | Patient/public in_person: адресация `{ branchId, serviceId }` (+ default specialist). `branchServiceId` резолвится внутри сервиса через mapping adapter. Rubitime create по-прежнему через mapped Rubitime IDs. |
| **Не делать сейчас** | Удаление `booking_branch_services` или breaking change API create/slots без adapter-слоя. |
...
|---------|-------------------|--------|-------|
| `booking_doctor_appointments_read_source` | `rubitime_legacy` | `appointment_records` или `be_appointments` | — |
...
Календарь (`/app/doctor/calendar`) следует `booking_doctor_appointments_read_source` (alias `calendarReadSource`).
