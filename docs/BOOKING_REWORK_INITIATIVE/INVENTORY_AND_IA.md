...
**Статус:** `historical / superseded` (инициатива закрыта 2026-06-06 — [`README.md`](README.md))
**Связанный roadmap:** [`ROADMAP.md`](ROADMAP.md)

Документ фиксирует исторический снимок кабинета записи, источники данных, контракты переходного периода и
целевую карту вкладок **до** крупных UI-правок этапа 1. Он не является каноном текущей навигации.
Актуальный контракт: единый `/app/doctor/schedule?tab=cal`, где прошлые и будущие записи открываются
навигацией по диапазонам; `/calendar` и `/appointments` — legacy 308. См.
[`schedule.md`](../../apps/webapp/src/app/app/doctor/schedule/schedule.md) и
[`DOCTOR_CABINET_NAVIGATION.md`](../ARCHITECTURE/DOCTOR_CABINET_NAVIGATION.md).

---

## 1a. Историческая навигация после этапа 5 (снимок 2026-06-04)

> Весь раздел — исторический baseline. Текущая структура описана в канонических документах выше.
...
## 3. Историческая целевая карта вкладок (IA)
...
| `booking_rubitime_bridge_enabled`                      | boolean                          | Двусторонняя синхронизация Rubitime (live: `AppointmentMirrorSync`, 2026-06-05)         |
...
2. **Rubitime API create/cancel/reschedule** — при `booking_slots_read_source=rubitime` или bridge-enabled; mapped appointments — **mirror outbound** (`booking-appointment-sync`, staff/patient/admin).
