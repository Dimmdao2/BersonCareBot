...
- GitHub Actions [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml): job `test-webapp` запускает `pnpm test:webapp` **без** `DATABASE_URL` → globalSetup **сразу выходит**, миграции в CI не выполняются.
