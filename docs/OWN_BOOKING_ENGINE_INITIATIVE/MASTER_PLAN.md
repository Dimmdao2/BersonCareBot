...
- **Слоты (этап 2):** собственный движок `booking-scheduling` (`0089`: working_hours, schedule_blocks, exclusion на пересечения); patient slots — Rubitime или канон по **`booking_slots_read_source`** (`rubitime` | `canonical`, seed `0100` → `rubitime`); UI пациента без multi-slot (`slotCount=1`).
