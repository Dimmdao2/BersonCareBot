...
- **Write (этап 2–3, done):** при подключённых `bookingEngine` + `bookingScheduling` в `buildAppDeps` пациентский и публичный `createBooking` создают `be_appointments` и `patient_bookings` с `canonical_appointment_id`. При **`booking_slots_read_source=rubitime`** create идёт **Rubitime-first** (обязательный `syncPort.createRecord`, rollback при сбое канона); при **`canonical`** — канон primary, Rubitime best-effort при `booking_rubitime_bridge_enabled`. Legacy-путь через integrator остаётся только без канонического DI (in-memory/тесты).
- **Слоты (этап 2):** собственный движок `booking-scheduling` (`0089`: working_hours, schedule_blocks, exclusion на пересечения); patient slots — Rubitime или канон по **`booking_slots_read_source`** (`rubitime` | `canonical`, seed **`0100`** в `public` + `integrator.system_settings` → `rubitime`); UI пациента без multi-slot (`slotCount=1`). Admin UI: **`BookingWorkingHoursSection`**, scoped **`BookingScheduleBlocksSection`** (`/app/doctor/admin/booking`).
...
- **Read (переходный режим, 2026-05-30):** список `/app/doctor/appointments`, метрики дашборда и **календарь** `/app/doctor/calendar` по умолчанию читают **legacy Rubitime** (`appointment_records`) через `booking_doctor_appointments_read_source=rubitime_legacy`. Канон (`be_appointments`) + canonical free slots — только при явном cutover (`canonical`). Слоты пациента — отдельный ключ `booking_slots_read_source` (default seed `rubitime`). Create при `booking_slots_read_source=rubitime` — **Rubitime-first** (не зависит от bridge flag).
...
- **Календарь (этап 8, done):** `modules/booking-calendar` + read switch (`pgBookingCalendar` | `pgBookingCalendarLegacy`); legacy mapper + **`calendarLegacyFilters`** (soft scope, dedupe); API `GET /api/doctor|admin/booking-engine/calendar` (`serviceId`, `includeFreeSlots`, response `readSource`, `freeSlotsEnabled`); UI `/app/doctor/calendar` (день/неделя/месяц, фильтры, карточка с lifecycle/оплатой/абонементом; legacy events read-only); free/busy слоты через `booking-scheduling` только при `freeSlotsEnabled`; список `/app/doctor/appointments` — тот же read switch (`appointment_records` по умолчанию); GCal зеркало `syncCanonicalAppointmentToCalendar` (`be:{id}`), integrator hook на `booking.*` и `payment_captured`. Q3: luxon+shadcn grid. План: [`.cursor/plans/archive/own_booking_stage8_calendar.plan.md`](../../.cursor/plans/archive/own_booking_stage8_calendar.plan.md); стабилизация перехода — [`.cursor/plans/archive/rubitime_transition_stabilize.plan.md`](../../.cursor/plans/archive/rubitime_transition_stabilize.plan.md) (`completed`, 2026-05-30).
...
- **C8. Rubitime-мост** — на этапах, меняющих запись, поддерживать двустороннюю синхронизацию, пока мост включён. Live mirror: `AppointmentMirrorSync` (2026-06-05, [`ACCEPTANCE_MIRROR_SYNC.md`](../BOOKING_REWORK_INITIATIVE/ACCEPTANCE_MIRROR_SYNC.md)).
...
- [x] Собственная БД — канонический источник для **write** и append-only истории; **переходный read** (2026-05-30): список записей и календарь врача по умолчанию из Rubitime (`appointment_records`) до cutover на `canonical`; карточка клиента и каноническая история — `be_*` (этапы 8–9 ✓).
