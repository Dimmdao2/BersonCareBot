...
- **Read (переходный режим, 2026-05-30):** список `/app/doctor/appointments` и метрики дашборда по умолчанию читают **legacy Rubitime** (`appointment_records`) через `booking_doctor_appointments_read_source=rubitime_legacy`. Канон (`be_appointments`) — только при явном cutover (`canonical`). Календарь `/app/doctor/calendar` по-прежнему читает канон.
