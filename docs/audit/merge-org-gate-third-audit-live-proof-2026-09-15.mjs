 ...
 * The candidate's org-scoped active-program index is installed inside the same outer transaction,
 * so DEV keeps its landed schema after the mandatory final ROLLBACK.
 * Run (loads DEV process config without printing it):
 * sudo -n bash -lc 'cd /home/dev/dev-projects/bcb-wt-merge-org-gate && set -a && source /home/dev/dev-projects/BersonCareBot/apps/webapp/.env.dev && set +a && export NODE_ENV=test && exec apps/webapp/node_modules/.bin/tsx --tsconfig apps/webapp/tsconfig.json docs/audit/merge-org-gate-third-audit-live-proof-2026-09-15.mjs [namefilter]'
 */
...
import { buildMergePreview } from '../../apps/webapp/src/infra/platformUserMergePreview.ts';
import { canSubmitManualMerge } from '../../apps/webapp/src/app/app/admin/account-merge/accountMergeLogic.ts';
...
const note = (u, org) => [
  `INSERT INTO doctor_notes(user_id, author_id, text, organization_id, note_date) VALUES ($1::uuid,$2::uuid,'n',$3::uuid,CURRENT_DATE)`,
  [u, DOC, org],
];
const noteNullOrg = (u) => [
  `INSERT INTO doctor_notes(user_id, author_id, text, organization_id, note_date) VALUES ($1::uuid,$2::uuid,'n',NULL,CURRENT_DATE)`,
  [u, DOC],
];
...
const trauma = (u, org) => [
  `INSERT INTO clinical_anamnesis_trauma(patient_user_id, year, what, type, created_by, organization_id) VALUES ($1::uuid,'2019','knee','sport',$2::uuid,$3::uuid)`,
  [u, DOC, org],
];
const complaint = (u, org) => [
  `INSERT INTO clinical_complaint(patient_user_id, text, organization_id) VALUES ($1::uuid,'боль',$2::uuid)`,
  [u, org],
];
const lfk = (u, org, tpl) => [
...
const symptomReal = (u, org) => [
  `INSERT INTO symptom_trackings(user_id, platform_user_id, symptom_title, symptom_key, organization_id) VALUES ($1::text,$1::uuid,'колено','knee_pain',$2::uuid)`,
  [u, org],
];
const program = (u, org, source, status) => [
  `INSERT INTO treatment_program_instances(patient_user_id, title, assignment_source, status, organization_id) VALUES ($1::uuid,'p',$2,$3,$4::uuid)`,
  [u, source, status, org],
];
...
const SCENARIOS = [
  ...
  },
  {
    ...
    after: async (q, t) => ({
      diagnoses_on_target: (
        await q(`SELECT count(*)::int c FROM clinical_diagnosis WHERE patient_user_id=$1`, [t])
      ).rows[0].c,
      traumas_on_target: (
        await q(`SELECT count(*)::int c FROM clinical_anamnesis_trauma WHERE patient_user_id=$1`, [
          t,
        ])
      ).rows[0].c,
      want: '1 and 1',
  ...
  },
  {
    ...
    oracle:
      '§18 «у одного человека спокойно живут две организации, каждая со СВОИМИ НАЗНАЧЕНИЯМИ» + план Э1',
    pair: ['217', '218'],
    expect: 'merge',
    ...
    after: async (q, t) => {
      const active = (
        await q(
          `SELECT organization_id::text organization_id
           FROM treatment_program_instances
          WHERE patient_user_id=$1::uuid AND status='active'
          ORDER BY organization_id`,
          [t],
        )
      ).rows.map((row) => row.organization_id);
      if (active.length !== 2 || active[0] !== ORG_X || active[1] !== ORG_Y) {
        throw new Error(`cross-org active programs were not preserved: ${JSON.stringify(active)}`);
      }
      return { active_program_organizations_on_target: active, want: [ORG_X, ORG_Y] };
    },
  },
  {
  ...
  // ---- point 3: transfer must not be lost ----
  {
    ...
    fx: (t, d) => [
      ...
      [
        ...
        VALUES (gen_random_uuid(),$1::uuid,$2::uuid, now()+interval '1 day', now()+interval '1 day'+interval '30 min',30,'native','confirmed')`,
        [d, ORG_X],
      ],
      intake(d, ORG_X),
      [
        ...
        VALUES ($1::uuid,'conv-7c3a-302','telegram','organization','open',now(),now(),$2::uuid)`,
        [d, ORG_X],
      ],
      [
        `INSERT INTO symptom_trackings(user_id, platform_user_id, symptom_title, symptom_key, organization_id)
        VALUES ($1::text,$1::uuid,'Общее самочувствие','general_wellbeing',$2::uuid),($1::text,$1::uuid,'Разминка','warmup_feeling',$2::uuid)`,
        [d, ORG_X],
      ],
      [
        ...
        SELECT st.user_id, st.platform_user_id, st.id, 7, 'daily', now(), 'webapp', $2::uuid FROM symptom_trackings st WHERE st.platform_user_id=$1::uuid`,
        [d, ORG_X],
      ],
      [
        `INSERT INTO message_log(user_id, platform_user_id, sender_id, text, category, outcome, organization_id)
        VALUES ($1::text,$1::uuid,'system','рассылка','broadcast','sent',$2::uuid)`,
        [d, ORG_X],
      ],
    ...
    after: async (q, t, d) => {
      const tables = [
        ['patient_bookings', 'platform_user_id'],
        ['be_appointments', 'platform_user_id'],
        ['online_intake_requests', 'user_id'],
        ['support_conversations', 'platform_user_id'],
        ['symptom_trackings', 'platform_user_id'],
        ['symptom_entries', 'platform_user_id'],
        ['message_log', 'platform_user_id'],
        ['treatment_program_instances', 'patient_user_id'],
      ];
      const on_target = {};
      const left_on_duplicate = {};
      for (const [tbl, col] of tables) {
        on_target[tbl] = (
          await q(`SELECT count(*)::int c FROM ${tbl} WHERE ${col}=$1`, [t])
        ).rows[0].c;
        left_on_duplicate[tbl] = (
          await q(`SELECT count(*)::int c FROM ${tbl} WHERE ${col}=$1`, [d])
        ).rows[0].c;
      }
      return {
        on_target,
        left_on_duplicate,
        want: 'each on_target >=1, each left_on_duplicate = 0',
      };
    },
  },
  {
    ...
    oracle:
      '§18 «Всё это переносится» — медицина едет на целевую учётку, оставаясь при своей организации',
    pair: ['303', '304'],
    expect: 'merge',
    fx: (t, d) => [note(t, ORG_X), note(d, ORG_Y), diagnosis(d, ORG_Y), symptomReal(d, ORG_Y)],
    after: async (q, t, d) => ({
      notes_on_target: (await q(`SELECT count(*)::int c FROM doctor_notes WHERE user_id=$1`, [t]))
        .rows[0].c,
      diagnoses_on_target: (
        await q(`SELECT count(*)::int c FROM clinical_diagnosis WHERE patient_user_id=$1`, [t])
      ).rows[0].c,
      symptom_orgs_on_target: (
        await q(`SELECT organization_id::text o FROM symptom_trackings WHERE platform_user_id=$1`, [
          t,
        ])
      ).rows.map((r) => r.o),
      left_on_duplicate: (await q(`SELECT count(*)::int c FROM doctor_notes WHERE user_id=$1`, [d]))
        .rows[0].c,
  ...
  // ---- manual support path must not be gated ----
  {
    ...
    oracle:
      '§18а «Что может техподдержка: … перенести любые данные между аккаунтами в любом направлении»',
    pair: ['401', '402'],
    expect: 'merge',
    reason: 'manual',
    fx: (t, d) => [
      ...
      trauma(d, ORG_X),
      note(d, ORG_X),
      lfk(t, ORG_X, 'bcc82563-7d4a-4488-8fc2-4a6bfbc2a5ae'),
      lfk(d, ORG_X, 'bcc82563-7d4a-4488-8fc2-4a6bfbc2a5ae'),
      program(t, ORG_X, 'doctor', 'active'),
      program(d, ORG_X, 'doctor', 'active'),
    ],
    after: async (q, t) => {
      const detail = {
        notes_on_target: (await q(`SELECT count(*)::int c FROM doctor_notes WHERE user_id=$1`, [t]))
        ...
        traumas_on_target: (
          await q(
            `SELECT count(*)::int c FROM clinical_anamnesis_trauma WHERE patient_user_id=$1`,
            [t],
          )
        ).rows[0].c,
        programs_on_target: (
          await q(
            `SELECT count(*)::int c FROM treatment_program_instances WHERE patient_user_id=$1`,
            [t],
          )
        ).rows[0].c,
        lfk_assignments_on_target: (
          await q(
            `SELECT count(*)::int c FROM patient_lfk_assignments WHERE patient_user_id=$1`,
            [t],
          )
        ).rows[0].c,
        active_lfk_assignments_on_target: (
          await q(
            `SELECT count(*)::int c FROM patient_lfk_assignments WHERE patient_user_id=$1 AND is_active=true`,
            [t],
          )
        ).rows[0].c,
        active_programs_on_target: (
          await q(
            `SELECT count(*)::int c FROM treatment_program_instances WHERE patient_user_id=$1 AND status='active'`,
            [t],
          )
        ).rows[0].c,
      };
      if (
        detail.notes_on_target !== 1 ||
        detail.traumas_on_target !== 1 ||
        detail.programs_on_target !== 2 ||
        detail.lfk_assignments_on_target !== 2 ||
        detail.active_lfk_assignments_on_target !== 1 ||
        detail.active_programs_on_target !== 1
      ) {
        throw new Error(`manual support transfer lost data: ${JSON.stringify(detail)}`);
      }
      return {
        ...detail,
        want: 'notes 1, traumas 1, programs 2 and LFK assignments 2 with exactly 1 active each',
      };
    },
  },
];
...
async function main() {
  ...
  let previewQueryTail = Promise.resolve();
  const previewPool = {
    query: (text, values) => {
      const pending = previewQueryTail.then(() => client.query(text, values));
      previewQueryTail = pending.then(
        () => undefined,
        () => undefined,
      );
      return pending;
  ...
  try {
    ...
    await q(
      `INSERT INTO be_organizations(id,title) VALUES ($1,'audit3 org X'),($2,'audit3 org Y')`,
      [ORG_X, ORG_Y],
    );
    await q(
      `INSERT INTO platform_users(id,display_name,role) VALUES ($1,'audit3 doctor','doctor')`,
      [DOC],
    );
    ...
    for (const s of SCENARIOS) {
      ...
      try {
        await q(
          `INSERT INTO platform_users(id,display_name,role) VALUES ($1,$3,'client'),($2,$4,'client')`,
          [t, d, `t${ts}`, `d${ds}`],
        );
        ...
        if (s.previewMustAllow) {
          ...
          if (!preview.ok || !preview.mergeAllowed || preview.hardBlockers.length > 0) {
            throw new Error(`manual preview blocked canonical merge: ${JSON.stringify(preview)}`);
          }
          if (!canSubmitManualMerge(preview, manual(t, d))) {
            throw new Error('manual merge button stayed disabled for canonical merge');
          }
        }
        ...
        try {
          ...
          const n = e?.name ?? '';
          actual =
            n === 'MergeConflictError' || n === 'MergeDependentConflictError' ? 'block' : 'CRASH';
  ...
  for (const r of results) {
    ...
    console.log(
      `${r.observe ? 'OBSERVE' : ok ? 'PASS   ' : 'FAIL   '} ${r.name}  expected=${r.expect} actual=${r.actual}`,
    );
  ...
  console.log(
    `${results.filter((r) => !r.observe).length - failed}/${results.filter((r) => !r.observe).length} graded scenarios matched the oracle`,
  );
