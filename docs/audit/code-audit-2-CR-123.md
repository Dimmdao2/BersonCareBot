...
| **4. oklch vs color-mix** | ADVISORY        | Agree with audit #1 that it matters for _consistency_, and I add a stronger reason — see A1. Not a merge blocker.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
i.e. the team previously hit an _invalid/transparent_ result when leaning on the
...
:109) — _not_ oklch — so `oklch(from var(--primary) …)` relies on the engine's
...
`waitFor` only retries when its callback **throws**; a callback that _returns_
...
covered (CR-2 only guards `kind === "nonworking"`, so a click inside a _break_
...
- **I add (audit #1 missed):**
  1. The ineffective `waitFor` in the new tests (A2) — audit #1 rated tests
     "PASS" with no reservation. The tests are _correct in outcome today_ but
     _fragile in mechanism_.
