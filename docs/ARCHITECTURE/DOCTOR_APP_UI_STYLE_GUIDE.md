...
Структуру и содержание карточки пациента задаёт
[`docs/design/bersoncare-карточка-пациента-CURRENT-SPEC.md`](../design/bersoncare-карточка-пациента-CURRENT-SPEC.md);
из §9 этого гайда действует только общая структура `article > entity-header > action-strip > TabsList > TabsContent`.
...
через `100vh`, высоту шапки или нижнего меню самостоятельно. Статичные поверхности заканчиваются
общим нижним зазором `--doctor-page-bottom-gutter`; непрерывный список доходит до строки нижней
навигации и владеет небольшим белым отступом после последней записи.
...
Основной flat-list использует геометрию блока «На сопровождении», divider `1px #f0efeb` и full-row hover для
интерактивных строк. Первичная строка сущности — `text-base font-normal`; вторичная информация остаётся `text-xs` /
`text-sm` по роли. Вложенные карточки §5a–§5d сохраняют собственную геометрию.
...
Модалки открывать через `DoctorModal` и обязательно задавать один тип геометрии: `panel`, `dialog`, `fullscreen-text` или `fullscreen-media`. `panel`: до `539px` канонический bottom-drawer со свайпом вниз, с `540px` — правая панель. `dialog` оставляет компактное подтверждение или пикер диалогом; навигационный mobile-shell переключается отдельно на границе `md` (`768px`). Fullscreen-типы не смешиваются с panel/dialog-презентациями. Legacy `doctor/primitives/Dialog` остаётся совместимым адаптером и на mobile делегирует тому же drawer; локальная mobile-разметка модалки запрещена.

В mobile drawer и правой панели нижние действия используют одну сетку: одна кнопка занимает правую половину, две или три идут одним рядом равными долями, четыре — двумя рядами по две. Desktop-dialog сохраняет компактные кнопки по содержимому справа.

На `540–767px` правая панель начинается под мобильной шапкой и закрывается узкой вертикальной ручкой по левому краю; крестика и затемнения в этом режиме нет. С `768px` панель начинается под шапкой страницы, использует обычный крестик и первый слой затемнения только над телом страницы — шапка и левое меню остаются светлыми; вложенные панели второй backdrop не добавляют. У компактной панели слегка скруглены только левые углы.
