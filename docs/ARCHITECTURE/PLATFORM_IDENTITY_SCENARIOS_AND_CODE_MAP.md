...
| *(нет enum — не trusted)*                   | `ensureClientFromAppointmentProjection` в том же файле: телефон из записи Rubitime заполняет/обогащает канон **без** `patient_phone_trust_at` (снимок данных записи, не активация tier).                         |
