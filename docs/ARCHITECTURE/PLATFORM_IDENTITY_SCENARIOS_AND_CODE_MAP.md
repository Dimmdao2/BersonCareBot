...
| Admin: справочный поиск по ФИО и расширенный ручной merge | `GET /api/doctor/clients/name-match-hints`, `GET /api/doctor/clients/merge-user-search`; UI `/app/doctor/clients/name-match-hints` (ссылки с `scope=all`), `AdminMergeAccountsPanel.tsx` внутри аккордеона карточки (`ClientProfileCard`), ленивые запросы кандидатов/preview при открытой секции, отмена гонок `merge-preview`, ошибки поиска отдельно от пустого списка; Track D (#987) снял и маршрут `POST /api/doctor/clients/integrator-merge`, и сброс фантомной числовой личности у дубликата — см. `ARCHITECTURE/PLATFORM_USER_MERGE.md`; инфра `platformUserNameMatchHints.ts`, `searchMergeUsersForManualMerge` в `platformUserMergePreview.ts` |
...
**Канон URL Mini App (webapp, с 2026-05):** точки входа **`/app/tg`** (Telegram) и **`/app/max`** (MAX) — общий RSC `apps/webapp/src/app/app/AppEntryRsc.tsx`; integrator собирает подписанный `?t=` в `apps/integrator/src/integrations/webappEntryToken.ts` (`buildWebappEntryUrlFromSource`). Legacy `?ctx=bot|max` на `/app` обрабатывает `apps/webapp/src/middleware/platformContext.ts` (`handlePlatformContextRequest`; при `ctx=max` на `/app` — редирект на `/app/max`). **С 2026-05-27:** на **`/app/tg`** / **`/app/max`** без `ctx` proxy вызывает **`applyMessengerEntryPathCookies`** (platform + surface cookie до RSC/auth), чтобы client **`PwaAppAccessGate`** не уводил MAX/TG WebView на лендинг **`/`** при уже существующей сессии и пустом initData. Порядок эвристик «pathname / cookie vs query JWT» для справки покрыт функцией **`classifyEntryHintFromRequest`** в том же файле (unit-тесты); **канон сессии** — только серверный `AppEntryRsc` + `classifyUnauthenticatedAppEntry`, без проброса отдельного entry-hint заголовка. На клиенте при редкой гонке двух initData строк **`AuthBootstrap`** выбирает канал поверхности первым (**`pickInitDataForMessengerTick`** / **`flowHint`**) — см. `docs/ARCHITECTURE/MINIAPP_AUTH_FIX_EXECUTION_LOG.md` §Remediation miniapp audit и план `miniapp-audit-fixes_813ba600`.

**Сценарий «первый заход в Telegram/Max» (фаза B — закрыта):**

1. Нет строки в `user_channel_bindings` для `(channel_code, external_id)` → **до** `INSERT` в `platform_users` собираются кандидаты из **`resolutionHints`** из **подписанного** webapp-entry токена: (а) query `?t=` при `exchangeIntegratorToken`; (б) Mini App — то же значение в **`start_param`** внутри `initData` (после проверки подписи Telegram на сервере); (в) веб Login Widget — опциональное поле **`webappEntryToken`** в JSON POST `/api/auth/telegram-login`, если на странице входа в URL есть `?t=` (клиент подмешивает токен в тело запроса; поле **не** входит в hash виджета). Подсказки: UUID в `sub` (канонический `platformUserId`), телефон из токена **только** при совпадении с каноном с **`patient_phone_trust_at`** (`pgIdentityResolution.ts`, `findTrustedCanonicalUserIdByPhone`). Сырой client-controlled UUID в `start_param` **без** HMAC **не** принимается.
...
3. **Интегратор → токен:** Track D (#987) убрал числовую личность из payload. Обычный `/start` не несёт identity человека вовсе (только `sub` вида `tg:<id>` и `bindings`); канонический **`platformUserId`** попадает в токен лишь там, где привязка УЖЕ разрешила человека. Ключевое пространство закрыто контрактом `contracts/webapp-entry-token.json` (`additionalProperties: false`), поэтому старый подписанный токен с вытесненным полем отвергается как неизвестный ключ.
...
| Привязка телефона из integrator / signed HTTP bind     | `@bersoncare/platform-merge` **`applyMessengerPhonePublicBind`** → полный merge зависимостей и запись подтверждённого телефона/канала в канонический `public` слой |
...
| Legacy-import записи → ensure клиента по телефону             | Исторический `appointment.record.upserted` мог вызвать `ensureClientFromAppointmentProjection` в `apps/webapp/src/infra/repos/pgUserProjection.ts`: телефон записи попадал в канон **без** `patient_phone_trust_at` |
...
**Исторический сценарий внешнего импорта записи с телефоном:** в БД мог появиться или обогатиться `phone_normalized` **без** автоматической доверенной активации; tier **patient** по телефону и native-запись требуют OTP/мессенджер или иной путь из trusted policy. При первом входе в мессенджер цель — **привязать** канал к этому канону доверенным путём, а не создавать второго клиента.
...
| API `/api/patient/*`, **`/api/booking/*`**, server actions | **`requirePatientApiBusinessAccess`** / **`requirePatientAccessWithPhone`**; **`POST /api/booking/create`** и **`cancel`** дополнительно — **`requirePatientBookingTrustedPhoneAccess`**; перечень patient-business API — **`patientApiPathIsPatientBusinessSurface`**; общий **`patientClientBusinessGate`**                                                            | Совпадает с политикой страниц (whitelist навигации vs бизнес-gate)                              |
...
| _(нет enum — не trusted)_           | Legacy `ensureClientFromAppointmentProjection`: телефон из импортированной записи заполнял/обогащал канон **без** `patient_phone_trust_at` (снимок данных записи, не активация tier).                          |
...
| `admin_manual_profile_patch`        | **RETIRED 2026-08-13:** enum/repository compatibility marker remains, but the unsafe global HTTP profile PATCH and its UI caller are removed; no active route establishes this trust source. |
...
| ------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Порядок на первом входе мессенджера:** до `INSERT` — поиск канона по подсказкам и слияние кандидатов | `findOrCreateByChannelBinding` (`pgIdentityResolution.ts`): сначала существующий binding; иначе `collectMessengerResolutionCandidates` → `mergeCanonicalPlatformUserCandidates` → при отсутствии кандидатов — новый `platform_users` + binding. Подсказки: UUID в `sub` (после `resolveCanonicalUserId`), телефон из токена **только** через `findTrustedCanonicalUserIdByPhone` (согласование с DoD §3 §5 / SPEC §10). |
| **Интегратор кормит канон, не подменяет tier на web**                                                  | `user.upserted` / `contact.linked` → `upsertFromProjection` без дублирующего `updatePhone` после `contact.linked` (`events.ts`). `appointment.record.upserted` → `ensureClientFromAppointmentProjection` (**без** `patient_phone_trust_at` для телефона из записи; tier/trusted — только продуктовые пути), `events.ts`, `pgUserProjection.ts`.                                                                                                                                   |
...
| Интегратор → поле токена                           | Канонический `platformUserId` (uuid) в webapp-entry payload — только когда привязка уже разрешила человека (`webappEntryToken.ts`). Числового поля личности в payload нет (Track D, #987).                                                                                                                                        |
...
| **Личность человека в ссылке бота**                                                                   | Ссылка обычного `/start` не называет человека вовсе; канонический `platformUserId` появляется только при уже разрешённой привязке — ограничение данных, не дыра в B. |
...
**Фаза B (закрыта по канал ↔ канон, см. §10):** hints из signed entry-токена (`?t=`, `start_param`, `webappEntryToken`); `tg:`/`max:` в `sub`; канонический `platformUserId` в токене при уже разрешённой привязке; diary → канон; логи merge/insert в `pgIdentityResolution`; сырой client UUID в `start_param` без HMAC не принимается.
...
| Интеграционный риск фазы D закрыт проверками                                                                                                                            | полный CI — только по смысловому критерию [`AGENTS.md` §9](../../AGENTS.md#9-full-ci-gate), не по факту push |
