...
| Rubitime / запись → ensure клиента по телефону                | `ensureClientFromAppointmentProjection` в `apps/webapp/src/infra/repos/pgUserProjection.ts` (телефон записи в канон **без** `patient_phone_trust_at`; снимок для учёта записей), вызов из `events.ts` (`appointment.record.upserted`) |
...
**Сценарий «Rubitime создал/обновил канон с телефоном»:** в БД появляется или обогащается `phone_normalized` **без** автоматической доверенной активации; tier **patient** по телефону и native-запись требуют OTP/мессенджер или иной путь из trusted policy. При первом входе в мессенджер цель — **привязать** канал к этому канону доверенным путём, а не создавать второго клиента (фаза B плана).
