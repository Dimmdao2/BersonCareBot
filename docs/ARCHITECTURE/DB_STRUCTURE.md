...
- `broadcast_audit_recipients` — получатели рассылки `(audit_id, platform_user_id)` для доступа к **`/app/patient/broadcasts/[auditId]`** (в т.ч. push-only); миграция Drizzle **`0080_broadcast_audit_recipients`**.
