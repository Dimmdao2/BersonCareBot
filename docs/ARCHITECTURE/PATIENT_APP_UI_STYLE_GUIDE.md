...
- **Patient shell (с 2026-05):** по умолчанию {@link PatientShellHeaderBar} (профиль/назад + заголовок; на desktop — вкладки в той же полоске) и {@link PatientBottomNav} (только mobile). Chrome на всю ширину колонки shell (`safe-bleed-x`). Откат: `PATIENT_SHELL_NAV_VARIANT = "top"`.
