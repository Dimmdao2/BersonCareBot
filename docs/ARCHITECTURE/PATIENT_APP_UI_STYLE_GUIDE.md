...
- **Кнопки:** native `button`, shared button primitives и `[role='button']` используют единый точный радиус `8px` через `--button-radius`; локальные radius overrides не применяются.
