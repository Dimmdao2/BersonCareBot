...
Точные штатные owner-учётные записи для DEV/TEST и граница их использования заданы единожды в
[`AGENTS.md` §1a «Обычный вход на DEV/TEST»](../../AGENTS.md#обычный-вход-на-devtest); этот документ их не дублирует.
...
`TEST=true` и `TEST_ACCOUNT_*` задаются deploy-окружением, а не `system_settings`. Подробные логи и диагностические поверхности включаются автоматически в DEV/TEST. Локальный DEV подавляет внешнюю доставку — кроме `email` в петлевой SMTP-приёмник, единственного исключения; нормативная редакция и его условия живут в `AGENTS.md` §1b.2, здесь их пересказ не дублируется. Развернутый TEST пропускает через финальный integrator-gate только исходных получателей из `TEST_ACCOUNT_*`, без редиректа и без изменения preview.
