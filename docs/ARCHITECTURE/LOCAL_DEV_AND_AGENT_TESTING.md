...
## 3b. Агент поднимает СВОЙ dev, а не пользуется чужим (владелец, 01.08)

Владелец 01.08: «у агентов есть `pnpm migrate` и legacy migration тоже — и у них же есть `pnpm dev:turbo`
и у тебя всё есть». То есть ответ «живьём проверить не смог, сервер занят чужой веткой» — не ограничение
среды, а невыполненная работа: свой прогон поднимается сам.

**Ловушка, из-за которой это нельзя делать «как обычно»:** `pnpm dev`, `pnpm dev:turbo` и `pnpm webapp:dev`
СНАЧАЛА убивают слушателей порта (`scripts/kill-local-dev-ports.sh`) и занимают 5200. Общий постоянный
сервер лида и параллельные прогоны других агентов от этого падают — так уже дважды обрывались проверки.

**Рецепт для агента в своём клоне** (порт свой, скрипт-убийца не задействован):

```bash
cd apps/webapp
pnpm migrate                       # накатить миграции своей ветки на dev-базу; идемпотентно
npx next dev -H 127.0.0.1 -p 5210  # СВОЙ порт: 5210–5219, НЕ 5200 и не через pnpm dev
```

- Порт 5200 занят общим сервером лида — не занимать и не убивать.
- 4200 — интегратор, 3200 и 6200 — прод на этом же боксе, их скрипт отказывается трогать сам.
- Миграции: `pnpm migrate` (drizzle) и `pnpm migrate:legacy` — обе доступны агенту.
- Дальше вход и проверка — как в §4 ниже, только на своём порту.

**В брифе исполнителю ссылаться на этот раздел**, а не писать «dev на 5200»: иначе агент увидит чужой
сервер, решит, что среда не позволяет, и вернёт непроверенную работу.
