...
## 3b. Один сервер на всех, поднятый на сведённой ветке (владелец, 01.08)

Владелец 01.08, дословно: «да просто надо поднимать сервер для всех сразу. Свести ветки и поднять. Если
прям надо поднять самому — то отдельно можно, но это бред».

**Как это работает.** На боксе живёт ОДИН постоянный dev-сервер на общей ветке, порт 5200. Проверка живьём
делается на нём, а значит порядок такой: работа доводится до конца → проходит независимый аудит →
**приземляется в общую ветку** → проверяется на общем сервере. Сервер подхватывает изменения сам.

Отсюда следствие для оркестратора: **не копить приземления.** Ветка, дошедшая до аудита и не сведённая,
превращается в работу, которую физически негде посмотреть, — и исполнитель возвращает непроверенное.
Именно так дважды вышло 01.08 (возврат по К2 и ответ провайдера по К4).

**Чего НЕ делать никому:**

- ⛔ Не запускать `pnpm dev`, `pnpm dev:turbo`, `pnpm webapp:dev` ради своей проверки: они сначала
  **убивают слушателей порта** (`scripts/kill-local-dev-ports.sh`) и занимают 5200, роняя общий сервер и
  чужие идущие прогоны. За 01.08 так дважды обрывалась проверка посреди работы.
- ⛔ Не возвращать работу со словами «среда не позволяет проверить». Инструменты есть: `pnpm migrate`,
  `pnpm migrate:legacy`, все режимы запуска. Если проверка не сделана — назвать точную команду и её вывод.

**Свой отдельный прогон — исключение, а не норма.** Он оправдан, только когда проверять надо ДО
приземления и приземлить нельзя (например, правка сносит общий сервер или ломает чужую ветку). Тогда —
свой порт из 5210–5219 и запуск мимо скриптов-убийц:

```bash
cd apps/webapp
pnpm migrate                       # миграции своей ветки на dev-базу, идемпотентно
npx next dev -H 127.0.0.1 -p 5210  # свой порт; 5200 не занимать
```

Порты 4200 (интегратор), 3200 и 6200 (прод на этом же боксе) не трогать — скрипт отказывается от
прод-портов сам.
