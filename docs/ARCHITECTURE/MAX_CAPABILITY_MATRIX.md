...
| Mechanic | Telegram | MAX | Notes |
|----------|----------|-----|------|
| Open Web App (Mini App) | full | full | Оркестратор собирает Telegram-style `web_app: { url }` из фактов (`links.webapp*Url`). [`deliveryAdapter`](../../apps/integrator/src/integrations/max/deliveryAdapter.ts) отправляет в MAX API кнопку **`type: open_app`** с полем **`web_app`** (URL мини-приложения), опционально **`contact_id`**: сначала `meta.userId`, иначе числовой **`recipient.chatId`** (личка / фоновые отправки) — мини-приложение открывается **в клиенте MAX** с MAX Bridge (`initData`). Строки **`keyboard`** (reply keyboard) сливаются с **`inline_keyboard`** в одно вложение. Обычные URL без `web_app` остаются **`link`** (внешняя вкладка). См. [документацию MAX](https://dev.max.ru/docs-api) — типы кнопок, [MAX Bridge](https://dev.max.ru/docs/webapps/bridge). |
