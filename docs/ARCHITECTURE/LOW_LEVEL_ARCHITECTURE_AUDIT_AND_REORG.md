...
_Документ подготовлен по состоянию кодовой базы на момент анализа. План-файл не изменялся._

---

## API route import-policy (актуальное состояние)

**Правило:** `apps/webapp/src/app/api/**/route.ts` не должны импортировать `@/infra/*` напрямую — бизнес-логика и низкоуровневый wiring проходят через слой композиции (`buildAppDeps` / фабрики в `app-layer`).

**Статус:** трек B инициативы TEST_AND_API_DI_OPTIMIZATION завершён; в `route.ts` **нет** прямых импортов `@/infra/*` (проверка: `rg '@/infra' apps/webapp/src/app/api --glob '**/route.ts'`). Исторический снимок allowlist и журналы кластеров — в `docs/archive/2026-04-docs-cleanup/test-api-di-optimization/`.

**Нарушение:** новый прямой импорт `@/infra/*` из `route.ts` без явного решения команды и обновления политики считается регрессией.
