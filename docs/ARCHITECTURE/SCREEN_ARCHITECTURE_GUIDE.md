...
> Единый язык **структуры** экранов: зоны, шаблоны страниц и правила композиции.
> Визуальный слой задаётся отдельно для каждой зоны её актуальным style guide, CSS и primitives.
>
> **Статус:** база для проектирования всех новых экранов и последующей унификации существующих.
> **Дата:** 2026-07-11. **Поправка 2026-09-08:** старый Design DNA v1.0/v1.1 архивирован как
> устаревшее врачебное направление и не является источником ни doctor, ни patient UI.
>
> **Приоритет источников (при расхождении):** owner-решения → `AGENTS.md` → зональный
> `DOCTOR_APP_UI_STYLE_GUIDE.md` / `PATIENT_APP_UI_STYLE_GUIDE.md` → этот структурный документ → код.
...
>
> **Важно про токены:** doctor и patient — раздельные визуальные системы. Токены одной зоны не становятся
> целью другой автоматически. Новый patient visual target становится authority только после owner-подтверждения.
...
| Визуальный слой                      | Свои doctor guide, primitives и tokens                                                                         | Свои patient guide, primitives и tokens                              |
...
│  │  │  ┌────────────────────────────────────┐   │   │   │
│  │  │  │ 4. PRIMITIVE — примитивы зоны (§6)   │   │   │   │  Section, StatCard, MediaThumb, кнопки…
│  │  │  └────────────────────────────────────┘   │   │   │
│  │  └───────────────────────────────────────────┘   │   │
│  └──────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────┘
...
На мобильном viewport любая doctor-модалка проходит через `DoctorModal` и открывается его единым bottom-drawer
от нижнего края на фиксированную высоту `calc(100dvh - 3.5rem)`: оставляет 56 px сверху, держит контент у верхнего
края, прокручивает переполнение внутри и закрывается свайпом вниз. Боковые панели навигации к нему не относятся.
...
3. **Reuse-first.** Сначала примитив своей зоны (§6), потом shadcn/base-ui `primitives/*`, и только при явной продуктовой причине — кастом, зафиксированный в логе инициативы. Новых npm UI-зависимостей не добавляем.
...
### Общие композиционные инварианты

Эти правила относятся к структуре. Конкретная типографика, палитра, радиусы и материал берутся только из
актуального визуального слоя своей зоны.

15. **Списки — плоские строки.** Строка = волосяная линия во всю ширину, слева сразу имя/текст; декоративные pill не заменяют структуру списка.
...
17. **Цвет поддерживает смысл.** Акцент и функциональные цвета не должны подменять иерархию и состояние контрола.
18. **Статусы компактны.** Статус не должен превращаться в доминирующую декоративную плашку.
19. **Типографика семантична.** Page, section, body и meta имеют закрытую шкалу внутри своей зоны.
20. **Форма компонента следует его роли.** Радиус и материал берутся из зональных primitives, не задаются локально на странице.
21. **Оверлеи единообразны внутри зоны.** Модалки используют зональный primitive и один motion/scroll contract.
...
| **C4** | ~~Лаб-токены «Direction E» не сверены с задеплоенными.~~ **SUPERSEDED 2026-09-08.** | — | Старый общий DNA архивирован. Каждая зона использует свой текущий visual layer; новый target требует отдельного owner-подтверждения. |
...
2. **Определи зону** (врач/пациент) → её оболочка и визуальный слой.
...
2. **Visual target каждой зоны.** Старый общий Design DNA и его token-migration отменены owner-correction
   2026-09-08. Новые значения шрифта, canvas, accent, radius и motion вводятся только отдельным подтверждённым
   решением для doctor или patient, без автоматического переноса между зонами.
...
- **Архив старого Design DNA (не authority):** `docs/archive/2026-07-design-dna/`.
