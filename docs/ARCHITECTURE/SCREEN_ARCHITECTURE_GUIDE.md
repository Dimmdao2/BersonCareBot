# SCREEN ARCHITECTURE GUIDE — BersonCare
...
- **Пациент:** запись `booking/new` (+ `city/service/slot/confirm/done`) через `BookingWizardShell` — «Шаг N из M» + back-ссылка поверх `PatientAppShell`.
...
| **P2** | У записи параллельная оболочка `BookingWizardShell`, дублирующая title/back; `booking/new/page.tsx` перерисовывает заголовок, которым уже владеет shell. | `booking/new/BookingWizardShell.tsx`, `booking/new/page.tsx:77` | **P0** | Визард использует слоты `PatientAppShell` (`patientShellAboveTitleSlot` для «Шаг N из M»); не рисует свой `<h1>`. Ввести общий визард-примитив (§10). |
...
| **P4** | Ad-hoc кнопки/ссылки в обход CTA-примитивов (bespoke «Задать вопрос», «Адрес кабинета», raw `text-primary underline`). | `booking/new/page.tsx:105`, `profile/page.tsx:61`, `BookingWizardShell.tsx:57` | **P1** | Только `patientButton*Class` / `patientInlineLinkClass`. Запрет inline-Tailwind для CTA/поверхностей — вынести в правило. |
...
| **P8** | Выход за паддинг колонки вручную (`-mx-4 w-[calc(100%+2rem)]`). | `booking/new` promo-banner | **P2** | Ввести примитив full-bleed-обёртки (напр. `patientBleedClass`) вместо разовых расчётов. |
