...
>
> **Важно про токены:** Design DNA — это целевой визуальный слой (`bersoncare-theme.css`, Nunito, моно-синий `#386FBA`, общий холст `#f2f2f0`). Задеплоенные сейчас токены зон (`doctorVisual.ts` / `patientVisual.ts` / `tailwind-engine.css` / `doctor.css` / `globals.css`) — **текущая реализация**, которую предстоит выровнять под DNA. Этот структурный гайд к значениям токенов нейтрален: зоны/шаблоны/композиция не зависят от того, какой именно синий и шрифт подставлены.
...
| DNA (идентичность)                   | Общий целевой слой: `docs/design/dna/bersoncare-theme.css` (Nunito, `#386FBA`, `#f2f2f0`) — един для обеих зон | ← тот же DNA                                                         |
...
| **C4** | ~~Лаб-токены «Direction E» не сверены с задеплоенными.~~ **РЕШЕНО.** | ✅     | Авторитет — **Design DNA v1.0+v1.1** (`docs/design/dna/bersoncare-theme.css`: canvas `#f2f2f0`, accent `#386FBA`, Nunito). Direction E (`docs/design/style-directions/`) — его исследовательский предок, DNA — финализированная версия. Задеплоенные токены зон (`doctorVisual.ts`/`patientVisual.ts`/`doctor.css`/`globals.css`) — **текущая реализация**, подлежит выравниванию под DNA отдельной инициативой (см. §10.6). |
...
3. **Миграция токенов под DNA** (новое, следствие C4). Задеплоенные токены зон ещё не выровнены под `bersoncare-theme.css` (Nunito, `#386FBA`, `#f2f2f0`, шкала радиусов 5·8·11·14, микротень, vibrancy). Это **отдельная инициатива уровня токенов/DNA**, а не часть структурной унификации §8 — запускать её отдельной задачей. _Нужно ваше слово: делать выравнивание токенов сейчас или после структурных P0?_ Рекомендация: сначала структурные P0 (§9), затем токен-миграция под DNA — чтобы не переверстывать дважды.
   ...
   **Инструмент для токен-миграции — TweakCN** (`tweakcn.com`). Визуальный no-code редактор темы shadcn/ui + Tailwind: цвета, радиусы, тени, шрифты правятся в UI и экспортируются готовым набором CSS-переменных (`:root` / `@theme`, Tailwind v3/v4). Подходит именно для этого шага — быстро собрать и итерировать целевой набор токенов под DNA и выгрузить его в `bersoncare-theme.css`, не подбирая hex вручную. **Важные оговорки под нашу архитектуру:**
   ...
   - Базовая модель shadcn (`base-color: slate`) не несёт наши нюансы (общий холст `#f2f2f0`, моно-синий `#386FBA`, Nunito, микротень, vibrancy) — их доводить вручную поверх экспорта, сверяясь с Design DNA v1.0+v1.1.
