...
- **`integrator_user_id` не входит в `fields`**: два **разных** non-null `integrator_user_id` — в **v1** жёсткий блокер в preview и отказ в `mergePlatformUsersInTransaction` (**как до Stage 5**, ошибка из транзакции merge, audit `user_merge` с `phase: merge_transaction`). В **v2** (`platform_user_merge_v2_enabled` в `system_settings`, admin) перед транзакцией M2M `canonical-pair` должен подтвердить одну canonical пару; manual apply пропускает relaxed-path только если snapshot проверенной пары совпал с `integrator_user_id`, перечитанными под `FOR UPDATE` в merge tx. При drift между gate и apply сервер возвращает `integrator_ids_changed_since_gate`. Операторский integrator merge: `POST /api/doctor/clients/integrator-merge` или прямой вызов integrator API (см. `docs/PLATFORM_USER_MERGE_V2/STAGE_5_FEATURE_FLAG_AND_FLOW_SWITCH.md`).
