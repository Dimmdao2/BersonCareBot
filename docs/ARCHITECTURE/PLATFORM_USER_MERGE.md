...
- Пакет [`packages/platform-merge/`](../../packages/platform-merge/) содержит **`mergePlatformUsersInTransaction`**, **`pickMergeTargetId`**, **`classifyMergeFailure`** и **`applyMessengerPhonePublicBind`**. Канонический merge-движок вызывают browser finish через [`pgUserByPhone.ts`](../../apps/webapp/src/infra/repos/pgUserByPhone.ts), опциональный M2M [`messengerPhoneHttpBindExecute.ts`](../../apps/webapp/src/app-layer/integrator/messengerPhoneHttpBindExecute.ts) и реэкспорт webapp [`pgPlatformUserMerge.ts`](../../apps/webapp/src/infra/repos/pgPlatformUserMerge.ts). M2M-коллизия не сливает без человека: возвращает браузерный маршрут к общему account/FIO prompt. После `pnpm install` или pull держите **`packages/platform-merge/dist`** актуальным (скрипт [`scripts/ensure-booking-sync-built.sh`](../../scripts/ensure-booking-sync-built.sh) пересобирает пакет при отсутствии экспорта **`classifyMergeFailure`** или внутренностей merge вроде **`dedupeSingletonSymptomTrackingsForMerge`** в `dist/pgPlatformUserMerge.js`).
