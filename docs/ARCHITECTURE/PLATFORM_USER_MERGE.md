...
## Фаза v2 (Phase 6): integrator-side canonical merge — снята (Track D, #987)

Инициатива закрывала сценарий «две разные non-null числовые личности»: canonical-модель в БД integrator, merge двух `users.id`, переписывание `projection_outbox`, realignment webapp projection-таблиц, feature-flag и порядок «сначала integrator merge, потом webapp merge». **Track D (#987) вытеснил публичную числовую личность целиком**, поэтому самого сценария больше нет: ни одна строка `platform_users` её не несёт, блокеры `different_non_null_integrator_user_id` / `integrator_canonical_merge_required` / `integrator_merge_status_unavailable`, флаг `platform_user_merge_v2_enabled` и маршрут `POST /api/doctor/clients/integrator-merge` сняты. Ссылки ниже — история инициативы.
...
Реализация **Platform User Merge v2** и репозиторное закрытие зафиксированы **2026-04-10** в [`STAGE_C_CLOSEOUT.md`](../archive/2026-04-initiatives/PLATFORM_USER_MERGE_V2/STAGE_C_CLOSEOUT.md). С Track D (#987) весь этот режим снят с рантайма вместе с публичной числовой личностью: флага, блокеров и порядка «сначала integrator merge» в действующем контракте нет.
...
- Пакет [`packages/platform-merge/`](../../packages/platform-merge/) содержит **`mergePlatformUsersInTransaction`**, **`pickMergeTargetId`**, **`classifyMergeFailure`** и **`applyMessengerPhonePublicBind`**: один и тот же merge-движок и тот же hot-path для webapp [`phoneMessengerBind.ts`](../../apps/webapp/src/modules/auth/phoneMessengerBind.ts) (`applyMessengerContactPreOtp` — канонический write по подтверждённому телефону, integrator своего write больше не делает: `user.phone.link` выведен из рантайма 2026-08-26), опционального M2M [`messengerPhoneHttpBindExecute.ts`](../../apps/webapp/src/app-layer/integrator/messengerPhoneHttpBindExecute.ts) и реэкспорта webapp [`pgPlatformUserMerge.ts`](../../apps/webapp/src/infra/repos/pgPlatformUserMerge.ts). После `pnpm install` или pull держите **`packages/platform-merge/dist`** актуальным (скрипт [`scripts/ensure-booking-sync-built.sh`](../../scripts/ensure-booking-sync-built.sh) пересобирает пакет при отсутствии экспорта **`classifyMergeFailure`** или внутренностей merge вроде **`dedupeSingletonSymptomTrackingsForMerge`** в `dist/pgPlatformUserMerge.js`).
- **Channel-link (webapp → integrator):** `POST /api/integrator/channel-link/complete` при **`409`** возвращает `error: "conflict"` и опционально **`mergeReason`** — машинный код из [`completeChannelLinkFromIntegrator`](../../apps/webapp/src/modules/auth/channelLink.ts). Disposable stub → узкий **claim** в [`channelLinkClaim.ts`](../../apps/webapp/src/modules/auth/channelLinkClaim.ts); «живой» аккаунт → сначала full auto-merge через `mergePlatformUsersInTransaction(..., "phone_bind")`; только hard blocker становится конфликтом `channel_link_ownership_conflict` с `candidateIds`. Перед массовым переносом `symptom_trackings` в merge выполняется dedupe singleton-ключей (`general_wellbeing`, `warmup_feeling`) в `pgPlatformUserMerge.ts`.
- Авто-merge перед UPDATE телефона подтягивает дубликаты по **тому же номеру** (Track D, #987: ключа числовой личности для этого поиска больше нет); при логическом «hard blocker» возвращаются коды вида `merge_blocked_*`, `channel_already_bound_to_other_user`, запись в `admin_audit_log` (`messenger_phone_bind_blocked`) и **первый** deduped Telegram ping админу (integrator).
- Webapp `phone-messenger-bind` login использует тот же merge-engine для типового пути: мессенджер уже привязан к contact-only/onboarding user, а введённый телефон принадлежит другой канонической карточке пациента. Hot-path — `applyMessengerPhonePublicBind` (ключ пути — сама привязка канала, `resolveBoundPlatformUserId`); при отсутствии привязки webapp пробует прямой pair-merge через `mergePlatformUsersInTransaction(..., "phone_bind")`. Direct OTP `profile_bind` переносит server-resolved current `userId` + organization-scope через challenge и merge-ит владельца телефона в текущего пользователя; `/confirm` не принимает эти значения от клиента. Успешный merge продолжает OTP/session flow; hard blocker пишет открытую строку `messenger_phone_bind_blocked` / `messenger_phone_bind_anomaly` в `admin_audit_log`, доступную через общий лог и историю карточки клиента.
- Email lookup/register: при нескольких canonical-строках с одним `email_normalized` webapp пробует auto-merge только для безопасных дублей (например password-holder + contact-only). Авторизованный email claim после верного OTP использует текущего пациента как target и выполняет merge под server-resolved organization principal. Любой автоматический caller общего merge-engine блокируется, если `user_password_credentials` есть у обеих строк; hard blocker возвращает `email_conflict`.
- **Инвариант одной TX (`applyMessengerPhonePublicBind`):** после **каждого** успешного автослияния (шаг «дубликат по телефону» и merge с предпочтённым каноном) выполняется повторное разрешение UUID канала через `user_channel_bindings` (`resolveBoundPlatformUserId`): merge переносит привязки на канона, локальный указатель процесса обязан сразу указывать на активную строку `platform_users` (`merged_into_id IS NULL`). Иначе второй merge в том же проходе использует уже смерженный id и ошибочно падает `merge_blocked_ambiguous_candidates`.
...
- Strong IDs:
  - `phone_normalized`
  - привязка канала `(channel_code, external_id)` в `user_channel_bindings`
- Ограничения strong IDs:
  - `UNIQUE DEFERRABLE INITIALLY IMMEDIATE` (миграция `061_platform_users_merge.sql`).
  - Track D (#987) убрал из strong IDs публичную числовую личность: она вытеснена из `platform_users`.
...
- **Числовой личности в `fields` нет и быть не может**: Track D (#987) вытеснил её из `platform_users`, поэтому ни блокера «две разные non-null личности», ни gate `canonical-pair`, ни маршрута `POST /api/doctor/clients/integrator-merge` в контракте не осталось. Арбитражу подлежат только перечисленные скаляры, привязки каналов, OAuth и стратегия `channelPreferences`.
...
- Сценария «две разные non-null числовые личности» больше нет: Track D (#987) вытеснил публичную личность, вместе с ней сняты флаг `platform_user_merge_v2_enabled` и все три integrator-блокера. История инициативы и чеклисты: [`STAGE_C_CLOSEOUT.md`](../archive/2026-04-initiatives/PLATFORM_USER_MERGE_V2/STAGE_C_CLOSEOUT.md).
