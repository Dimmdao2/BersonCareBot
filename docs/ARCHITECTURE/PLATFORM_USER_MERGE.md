...
- Webapp `phone-messenger-bind` login использует тот же merge-engine для типового пути: мессенджер уже привязан к contact-only/onboarding user, а введённый телефон принадлежит другой канонической карточке пациента. Если у binding owner есть `integrator_user_id`, используется hot-path `applyMessengerPhonePublicBind`; если id отсутствует/пустой, webapp пробует прямой pair-merge через `mergePlatformUsersInTransaction(..., "phone_bind")`. Direct OTP `profile_bind` переносит server-resolved current `userId` + organization-scope через challenge и merge-ит владельца телефона в текущего пользователя; `/confirm` не принимает эти значения от клиента. Успешный merge продолжает OTP/session flow; hard blocker пишет открытую строку `messenger_phone_bind_blocked` / `messenger_phone_bind_anomaly` в `admin_audit_log`, доступную через общий лог и историю карточки клиента.
