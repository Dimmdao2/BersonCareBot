...
| Управление клиникой _(clinic admin)_ | Врачи, Расписание                                                                                            |
...
обеспечивается шеллом, не пунктом меню. Пункт имеет tier `staff`: он доступен как специалисту, так и
управляющему клиники, но это не расширяет остальные clinical-разделы.

Пункты меню фильтруются по `accessTier`:

- `doctor` (или отсутствие tier) — обычный доступ к кабинету специалиста.
- `staff` — специалист или управляющий участник клиники; сейчас так открывается общий экран расписания.
- `clinic_admin` — управляющий участник клиники (`canManageOrganization`); отдельные management-ссылки.
...
| Расписание / записи                       | `/app/doctor/schedule?tab=cal`           | Единый canonical calendar: обычному врачу сервер всегда применяет собственный specialist scope; `clinic_admin` выбирает `Моё / Вся клиника / специалист` текущей клиники, и тот же scope получают calendar/KPI/nearest-window. Legacy `/calendar` и `/appointments` дают **308** на `?tab=cal`. |
...
Server-resolved bootstrap передаёт только собственный specialist ID, `canManageAllSpecialists` и действующих
специалистов текущей организации. Deep-link использует `scope=mine|clinic|specialist` +
`specialist=<uuid>`; API-запросы calendar/KPI/nearest-window получают `scope` и `specialistId` из одного
нормализованного состояния. Обычный врач не получает scope-controls.
