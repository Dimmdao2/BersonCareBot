...
- **Web Push VAPID (пара ключей для будущей подписки/отправки; только БД, не env):** **`web_push_vapid`** — объект `{ publicKey, privateKey }` (строки base64url) в `system_settings` (admin); UI на вкладке **«Параметры приложения»**; запись через `PATCH /api/admin/settings` → `updateSetting` → зеркало `integrator.system_settings`. Чтение для sender — **`getWebPushVapidKeyPair`** в `apps/webapp/src/modules/system-settings/webPushVapidRuntime.ts` (не `getConfigValue`). План и DoD: [`PWA_INITIATIVE/WEB_PUSH_VAPID_ADMIN.plan.md`](../PWA_INITIATIVE/WEB_PUSH_VAPID_ADMIN.plan.md).
