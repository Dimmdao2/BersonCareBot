...
- **Private media / VIDEO_HLS_DELIVERY (admin):** `video_hls_pipeline_enabled`, `video_hls_new_uploads_auto_transcode`, `video_playback_api_enabled`, `video_default_delivery`, **`video_presign_ttl_seconds`** (TTL presigned GET для `GET /api/media/[id]` и полей HLS в playback JSON; UI во вкладке «Параметры приложения» рядом с блоком приложения), **`video_watermark_enabled`** (опциональный burn-in watermark при транскоде в `apps/media-worker`; путь к шрифту — bootstrap env **`MEDIA_WORKER_WATERMARK_FONT`** или системный TTF на хосте воркера, см. `docs/archive/2026-05-initiatives/VIDEO_HLS_DELIVERY/PHASE_10_WATERMARK_POLICY.md`).
