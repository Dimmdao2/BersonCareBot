...
Закрепление владельца задаётся переменной окружения; роль администратора не выдаётся списками в БД.
Остальная операционная конфигурация хранится в `system_settings` по правилам ниже.
...
- Environment identity и safety: **`TEST`** и **`TEST_ACCOUNT_*`** принадлежат deploy-env DEV/TEST, а не БД. Они включают diagnostics/подробные логи среды, обход maintenance для owner-аккаунтов и финальный TEST delivery-gate интегратора. Preview всегда показывает настоящую аудиторию; TEST не перенаправляет сообщения, а подавляет получателей вне env-списка. **На DEV env-список получателей не применяется вовсе** (редакция 15.09.2026): туда уходит только `email` в петлевой SMTP-приёмник, остальные каналы молчат независимо от класса сообщения — `AGENTS.md` §1b.2 и `OWNER_PRODUCT_RULES.md` §23. В БД из настроек режима остаётся техническое обслуживание, потому что оператор меняет его без редеплоя. Прочие product-флаги, включая `important_fallback_delay_minutes` и `sms_fallback_enabled`, по-прежнему живут в `system_settings`.
...
**Таблицы-справочники интегратора** — несекретные бизнес-контракты; актуальный каталог записи принадлежит native booking engine webapp.
