...
- **Integrator `linkedPhone`:** строковый ключ **`integrator_linked_phone_source`** (`public_then_contacts` \| `public_only` \| `contacts_only`) — как объединять телефон из `public.platform_users` и legacy `integrator.contacts` при гейте `/start` и меню; редактирование в `/app/doctor/admin/technical` (админ, диагностика), чтение из `public.system_settings`.
