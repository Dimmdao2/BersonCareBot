...
- **Справка (`help`):** служебный раздел `content_sections.slug = help` (миграция `0103_help_content_section.sql`); страницы с ролью `help_article` (`CONTENT_PAGE_ROLES`). В сайдбаре CMS — «Статьи справки» (`?section=help`), не в общем списке тематических статей. Пациент: `/app/patient/help`, `/app/patient/help/[slug]`; `/app/patient/content/[slug]` для `section=help` редиректит на канонический URL. Канонические slug для плиток кабинета: `preparation`, `cost` (`help-content/canonicalSlugs.ts`) — плитки показываются только для опубликованных статей.
