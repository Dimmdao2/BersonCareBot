...
- **Справка (`help`):** служебный раздел `content_sections.slug = help` (миграция `0103_help_content_section.sql`); страницы с ролью `help_article` (`CONTENT_PAGE_ROLES`). В сайдбаре CMS — «Статьи справки» (`?section=help`), не в общем списке тематических статей. Пациент: `/app/patient/help`, `/app/patient/help/[slug]`; `/app/patient/content/[slug]` для `section=help` редиректит на канонический URL. Канонические slug: `preparation`, `after-visit`, `services-pricing`, `app-guide`, `address-spb`, `address-msk`, `about`, `booking` (`help-content/canonicalSlugs.ts`); чеклист публикации — `help-content/CMS_EDITOR_CHECKLIST.md`; legacy `cost` → alias для `services-pricing`. Плитки на `/app/patient/booking` (`CabinetInfoLinks`) — только для опубликованных статей; city-aware «Адрес»; статическая `/app/patient/about` (ссылка на сайт); статья `/help/booking` — ссылка на about. После save/lifecycle help — `revalidatePatientContentPaths` сбрасывает `/help` и `booking`.
