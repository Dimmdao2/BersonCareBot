...
- **Выбор файлов в формах (библиотека, не произвольный URL):**
  ...
  - **Тело страницы (Markdown):** единый doctor-facing Tiptap [`MarkdownEditor.tsx`](../../apps/webapp/src/shared/ui/doctor/markdown/MarkdownEditor.tsx) используется во всех write-поверхностях и сохраняет прежний Markdown/GFM контракт; отдельные Toast UI/textarea реализации выведены. Кнопка **«Библиотека или загрузка»** — [`MediaLibraryInsertDialog.tsx`](../../apps/webapp/src/shared/ui/doctor/markdown/MediaLibraryInsertDialog.tsx): тот же shell/panel, список `GET /api/admin/media` (`kind=all`); вставка передаёт в колбэк `kind`/`mimeType` строки; сниппет Markdown строит [`markdownSnippetForMediaUrl`](../../apps/webapp/src/shared/ui/doctor/markdown/markdownMediaSnippet.ts) (картинки по `kind === image`, по `image/*` MIME или по расширению, включая HEIC/AVIF и т.д.; видео/аудио/прочие файлы — обычная markdown-ссылка `[имя](url)`).
  - **Рендер Markdown у пациента и в простом doctor-preview:** пациентский слой [`markdownRenderTree.tsx`](../../apps/webapp/src/shared/ui/patient/markdown/markdownRenderTree.tsx) (`remark-gfm`, `rehype-sanitize`) и [`MarkdownEmbeddedLink.tsx`](../../apps/webapp/src/shared/ui/patient/markdown/MarkdownEmbeddedLink.tsx) остаётся отдельным от Tiptap write-редактора; doctor preview использует свой [`MarkdownPreview.tsx`](../../apps/webapp/src/shared/ui/doctor/markdown/MarkdownPreview.tsx). URL **YouTube** и **RuTube** нормализуются в [`hostingEmbedUrls.ts`](../../apps/webapp/src/shared/lib/hostingEmbedUrls.ts) и показываются как `<iframe>` (allowlist доменов после парсинга); ссылки на **`/api/media/{uuid}`** (относительные или абсолютные при совпадении доверенного origin) после успешного **`GET /api/media/{uuid}/playback`** дают **`PatientMediaPlaybackVideo`** для `video/*`, `<audio controls>` для `audio/*`; при ошибке, неподходящем MIME или отсутствии сессии остаётся обычная ссылка (см. [`PATIENT_MEDIA_PLAYBACK_VIDEO.md`](./PATIENT_MEDIA_PLAYBACK_VIDEO.md), [`MEDIA_HTTP_ACCESS_AUTHORIZATION.md`](./MEDIA_HTTP_ACCESS_AUTHORIZATION.md)). Доверенные origin для абсолютных URL в тексте: `window.location.origin` и опционально **`NEXT_PUBLIC_APP_BASE_URL`** (по смыслу тот же базовый URL, что **`APP_BASE_URL`** процесса — см. [`CONFIGURATION_ENV_VS_DATABASE.md`](./CONFIGURATION_ENV_VS_DATABASE.md)). Полное playback/embed-поведение проверяется на пациентском экране, а не по внутреннему WYSIWYG-представлению Tiptap.
...
- **Справка (`help`):** служебный раздел `content_sections.slug = help` (миграция `0103_help_content_section.sql`); страницы с ролью `help_article` (`CONTENT_PAGE_ROLES`). В сайдбаре CMS — «Статьи справки» (`?section=help`), не в общем списке тематических статей. Пациент: `/app/patient/help`, `/app/patient/help/[slug]`; `/app/patient/content/[slug]` для `section=help` редиректит на канонический URL. Канонические slug: `preparation`, `after-visit`, `services-pricing`, `app-guide`, `address-spb`, `address-msk`, `about`, `booking` (`help-content/canonicalSlugs.ts`); чеклист публикации — `help-content/CMS_EDITOR_CHECKLIST.md`; legacy `cost` → alias для `services-pricing`. Плитки на `/app/patient/booking/new` (`CabinetInfoLinks`) — только для опубликованных статей; city-aware «Адрес»; статическая `/app/patient/about` (ссылка на сайт); статья `/help/booking` — ссылка на about. После save/lifecycle help — `revalidatePatientContentPaths` сбрасывает `/help` и `booking/new`.
