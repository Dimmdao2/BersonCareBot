...
| **Файлы пациентов** (оба прода) | **Yandex Object Storage** `https://storage.yandexcloud.net`, bucket `bcb-patient-media` — **шифрование по умолчанию (SSE-KMS)**, ключи `PATIENT_S3_*`. Пустой `PATIENT_S3_BUCKET` = разделение выключено, оба назначения падают в библиотечный бакет |
