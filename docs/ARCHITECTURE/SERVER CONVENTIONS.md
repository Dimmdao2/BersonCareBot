...
с переименованием хоста, а не после. Прод-окружение заводится в декларации третьим — база
`therapysto_prod`, логины `therapysto_prod_*` — одним проходом с переименованием ролей, потому что
`pg_hba` сверяет клиентский сертификат по `clientname=CN`, где CN — имя роли: переименование роли
обесценивает её сертификат, и делать это дважды нельзя.
...
часть CA и четыре клиентские пары. Ключи ролей открыты группе `therapysto-app-prod`, и контейнер получает её
