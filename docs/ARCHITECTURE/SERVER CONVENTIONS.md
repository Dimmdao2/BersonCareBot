...
7. **Ключи и секреты приезжают вместе с базой TEST — отдельно создавать их не надо, а вот перевыпустить
   на проде надо.** Перенос восстанавливает базу TEST целиком, значит `public.system_settings` едет как
   есть, вместе с ключом капчи (`auth_altcha_hmac_secret`) и — когда владелец их заведёт — ключами Яндекс
   SmartCaptcha. Ничего «создавать на проде» в смысле внешнего сервиса для нашей капчи не требуется: ключ
   ALTCHA генерируется у нас, кнопкой в кабинете администратора («Авторизация» → «Сгенерировать» →
   сохранить). Но **секрет, поживший на TEST, не должен оставаться секретом прода**: дампы TEST копируются
   на dev-бокс, разворачиваются в DEV и открываются агентами. Поэтому шагом cutover — сгенерировать на
   проде НОВЫЙ ключ поверх приехавшего, одним нажатием, и только потом включать капчу, если она нужна.
   Обратная грабля уже случилась: на TEST ключ приехал ПУСТЫМ из базового среза старого прода, и пока
   капча была жёстко включена в коде, это запирало учётную запись навсегда — задачка не выдаётся, а
   счётчик неудач обнуляется только успешным входом. Этот класс закрыт 14.09 (пустой ключ теперь читается
   как «капча выключена», а включить её без ключа нельзя), но строка здесь именно поэтому.
   Ключи Яндекса берутся в консоли Яндекс Облака (клиентский — публичный, серверный — секрет) и тоже
   заводятся для прода отдельно, а не наследуются от TEST. Схема целиком — `AUTH_AND_IDENTITY_CANON.md`,
   часть V, §25.
