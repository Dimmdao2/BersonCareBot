...
рендишн (`mediaPreviewWorker.ts`) читает оригинал оттуда же и пишет стандартный рендишн в горячий, **оригинал
не удаляется** (реверс решения 19.08, см. `SECURITY_CANON.md` §5). Отсутствие `S3_RAW_BUCKET` при включённом
