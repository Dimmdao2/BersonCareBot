...
### Telegram-туннель (почему он есть)
Telegram заблокирован из РФ на гос-уровне; прод-сервер `135.x` (Selectel) Telegram **напрямую не достаёт**. Прод-бот ходит в Telegram через **AmneziaWG split-tunnel** (`AllowedIPs=149.154.160.0/20`, identity `10.9.0.5`) на старый сервер `151.x` (он Telegram достаёт), тот форвардит. Поэтому `awg-quick@awg0` на `151.x` критичен — его остановка убивает прод-бота. **Тест-бот на `151.x` достаёт Telegram напрямую** (туннель не нужен — `151.x` сам exit-точка).
...
- **🔴 Изоляция отправок (данные настоящие!):** allowlist получателей = админ (`+79643805480`) + тест-юзер Дмитрий Берсон (`+79189000782`); для остальных ВСЕ каналы (Telegram/MAX/SMS/email/**web-push**) режутся; **maintenance forced ON**; настройки залочены на уровне БД.
- Тест-БД `bersoncarebot_test` на том же PG16 (`:5432`); порты приложения **`:3300`** (integrator) / **`:6300`** (webapp, чтобы не пересечься с dev `:5200` и прод-портами); **ТЕСТ-токен** бота (не прод); доступ к `test.bersoncare.ru` залочен по IP.
- Деплой: ветка **`test`** → CI → `deploy-test.sh` на `151.x`.
