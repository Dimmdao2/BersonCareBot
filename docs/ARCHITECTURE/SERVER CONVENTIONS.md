...
| Среда                           | S3                                                                                                                                                                                                                                                   |
| ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **PROD** (`135.x`)              | **Selectel** `https://s3.ru-7.storage.selcloud.ru`, регион `ru-7`, bucket `saas-s3` (горячий, отдаётся), `S3_FORCE_PATH_STYLE=true`. Сырой бакет загрузок `saas-s3-cold` (М7, `S3_RAW_BUCKET`) — **пока НЕ существует**: класс хранения Selectel выставляется только в панели при создании контейнера и не меняется после (замер 11.09.2026), поэтому бакет создаёт владелец вручную; агент Selectel не трогает вовсе |
