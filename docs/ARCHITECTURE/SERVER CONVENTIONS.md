...
| Край | **nginx 1.24.0**, `server_name _` (ловит всё), `proxy_pass http://therapysto_webapp`; отдельный сайт `meet.therapysto.ru` → `127.0.0.1:8000` |
| TLS | ОДИН именованный сертификат certbot `therapysto`, до 2026-12-09 |
| Имена в сертификате | `therapysto.ru`, `www.therapysto.ru`, `admin.therapysto.ru`, `therapygo.ru`, `www.therapygo.ru`, `berson.therapygo.ru`, `app.bersoncare.ru`, `meet.therapysto.ru`, `turn.therapysto.ru` |
| Caddy | **бинаря нет**, юнит `inactive`, `/opt/therapysto/env/caddy.prod` отсутствует — переход на край с выпуском по требованию НЕ сделан |
...
**Следствие про сертификат, которое стоит помнить.** `server_name _` ловит любое имя, поэтому
доступность брендированного хоста упирается НЕ в nginx, а в список имён в сертификате: имени нет в
сертификате — TLS не соберётся, и приложение своего шанса не получит. Wildcard тут не выпустить:
HTTP-01 его не умеет, а DNS-01 требует API регистратора, от которого владелец отказался 12.09
(«Зачем тебе рег ру апи? Не будет у тебя их»). Поэтому каждое новое имя клиники — это либо ручное
расширение этого сертификата, либо переход на Caddy с выпуском по требованию
(`onDemandTlsAuthorization.ts`), который пока не сделан.
