...
- **Деплой (факт):** деплой-репо `/opt/projects/bersoncarebot-test` (ветка `feat/doctor-ui-rebuild`, владелец `deploy`), env `/opt/env/bersoncarebot/{api,webapp}.test`, юниты `bersoncarebot-{api,worker,scheduler,webapp,media-worker}-test`. **Ветки `test` и автодеплоя НЕТ** (CI деплоит только `main`→прод). Доставка feat→test — **одной командой** `bash deploy/host/deploy-test.sh` (от `dev`): git-bundle ветки через `/tmp` (deploy не читает `/home/dev`, push гейтован) → **force-align** тест-checkout (зеркало, не merge) → build → migrate → рестарт. Полная инструкция: `deploy/HOST_DEPLOY_README.md` → «Тест-деплой на 151.x».
