...
- `INTERNAL_JOB_SECRET` — опционально; если задан, Bearer для внутренних POST **`/api/internal/*`**: purge удаления медиа (**`media-pending-delete/purge`**), multipart cleanup (**`media-multipart/cleanup`**), превью (**`media-preview/process`**), retention playback (**`media-playback-stats/retention`**), retention ошибок HLS‑прокси (**`media-hls-proxy-errors/retention`**), reconcile транскода (**`media-transcode/reconcile`** — см. флаги в admin `system_settings`). Cron и nginx для loopback: **`deploy/HOST_DEPLOY_README.md`**.
