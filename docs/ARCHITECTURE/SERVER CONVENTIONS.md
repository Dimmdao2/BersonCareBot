...
### Сетевой периметр: TEST открыт, PROD закрыт файрволом (владелец, 19.08)

**Слова владельца 19.08, дословно:** «прод живет с файерволом и там все закрыто».

Замерено на `151.x` (TEST/DEV): `ufw status` — inactive, `iptables -L INPUT -n` — политика `ACCEPT`
и ноль правил, nginx слушает `0.0.0.0:80` и `0.0.0.0:443`. Ограничение доступа к `test.bersoncare.ru`
живёт ВНУТРИ конфигурации одного сайта (`allow/deny` в server-блоке), а не на уровне сети.

На `135.x` (PROD) — наоборот: периметр закрыт файрволом. Не проверялось и проверяться не будет:
probe прода запрещён, источник — прямое утверждение владельца.

**Почему это правило есть и кого оно касается.** Любая работа, добавляющая НОВЫЙ входящий путь —
прежде всего подключение собственного домена клиники, где чужое имя наводится на наш адрес, — не имеет
права переносить наблюдение с TEST на PROD. На TEST новый вход заработает без правки сети; на PROD
он потребует ОТДЕЛЬНОГО решения по периметру, и это решение принимает владелец, а не проектировщик.
Вывод «файрвол править не придётся», сделанный по замеру на `151.x`, для прода НЕВЕРЕН.
