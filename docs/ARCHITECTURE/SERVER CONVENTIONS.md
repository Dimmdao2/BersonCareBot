...
### Приёмник на `bcb-second` — ПОСТАВЛЕН 15.09, ждёт ключа прода

**Направление — ПРОД ТОЛКАЕТ.** Решение владельца 15.09: «мы же С ПРОДА отправляем а НЕ НА ПРОД
бэкапы». Прежний замысел с ТЯГОЙ отменён вместе со скриптом `pull-backups-from-prod.sh` (он никогда
не был установлен): тяга требовала новой двери по SSH на боевой машине и ослабления прав на каталог
дампов, то есть расширяла поверхность атаки прода ради бэкапов.

Возражение «толкающий прод сможет стереть копии» снимается не направлением, а замком НА ПРИЁМНИКЕ.
В `authorized_keys` учётки `bcbrecv`:

```
restrict,command="rrsync -wo -no-del /opt/backups/incoming" <тип> <ключ> offsite-push@therapysto-prod
```

Проверено поломками 15.09, а не обещано: запись проходит; `--delete` → «option --delete has been
disabled on this server»; чтение назад → «reading from write-only server is not allowed»; любая
другая команда → «SSH_ORIGINAL_COMMAND does not run rsync».

**Запрет удаления НЕ запрещает перезапись** — и это вторая половина замка, без которой первая
бесполезна. Залив мусор под уже известным именем, прод убил бы историю. Поэтому
`/opt/backups/scripts/receive-store.sh` на приёмнике каждые пять минут уносит принятое из `incoming`
в недостижимый для этого ключа `store` (0700 root) с `--ignore-existing`, а столкнувшиеся имена
уезжают в карантин `rejected/`, а не поверх хорошей копии. Срок хранения приёмник держит свой,
повторяя боевой: часовые 48 ч, суточные 35 дней, недельные 84 дня.

Приёмнику `age` и приватный ключ НЕ нужны — он хранит файлы, которые сам прочитать не может. Это
намеренно.

Сторона прода — `deploy/backups/offsite-push.sh` и задание `backup_offsite_push` в манифесте
(ежечасно в 27 минут, молчание дольше трёх часов — авария). На прод ещё не выкачено. Приватная
половина ключа отправки рождается на боевой машине и оттуда не уходит; первый прогон создаёт её и
честно падает, пока приёмник не принял публичную половину.

### Учения по восстановлению

`deploy/backups/restore-drill.sh` — забрать свежий артефакт с прода, сверить контрольную сумму,
расшифровать фразой владельца, поднять во временную базу на dev-боксе и сверить число строк.
Операторские: фраза вводится руками, поэтому по расписанию они не идут и идти не должны — иначе
фразу пришлось бы держать на машине.
