...
| **OLD / DEV / RELAY / TEST**                       | `151.241.228.122` (исходный хост = dev-box) | (1) **AmneziaWG egress-релей** Telegram для прод-бота — `awg-quick@awg0`, UDP `51822`, **НЕ ТРОГАТЬ** (прод-бот зависит); (2) dev-окружение (Next `:5200`); (3) старый прод **остановлен и замаскирован** (`bersoncarebot-*-prod.service` → `/dev/null`, снятые unit-файлы в root-only архиве `/var/backups/bersoncarebot-disabled-prod-units-20260729/`, cron.d в `/root/bcb-cron-disabled-*`); (4) именованный **TEST** с разделёнными Therapysto/TherapyGo поверхностями | `test.therapysto.ru`, `admin.test.therapysto.ru`, `test.therapygo.ru`, `*.test.therapygo.ru`, `meet.test.therapysto.ru`, `turn.test.therapysto.ru` → `151.241.228.122`; `test.bersoncare.ru` — переходный алиас |
