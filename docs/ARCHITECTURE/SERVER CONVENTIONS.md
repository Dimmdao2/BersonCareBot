...
**Слово-напоминание владельца: «поэт».** Это подсказка к фразе, а не сама фраза; задал её владелец
14.09.2026 и просил записать здесь, чтобы не потерялась.

Если подсказка не помогла — сделать нельзя ничего. Ни агент, ни хостер, ни перебор доступ не вернут:
`scrypt` с work factor 18 существует ровно затем, чтобы перебор не работал. Остаётся снять новую пару
и начать хранение заново, а всё, что зашифровано прежним замком, списать.
