...
| TLS | Caddy выпускает **по сертификату на имя** сам (HTTP-01/TLS-ALPN). Имя клиники — по требованию, через дверь разрешения `https://therapygo.ru/api/public/domains/ask`; любой не-2xx = отказ, заказ не начинается |
...
**Две вещи, которые эта развязка оставила незакрытыми.**

1. **Линия certbot `therapysto` больше не продлится целиком.** В блоке `:80` оставлен проход для
   чужих ACME-токенов из `/var/www/html`, но он срабатывает только для имён, которых Caddy НЕ
   обслуживает: платформенные имена край уводит на HTTPS раньше (замер 14.09 — `turn.therapysto.ru`
   отдаёт токен `200`, `therapysto.ru` отвечает `308`). Certbot проверяет КАЖДОЕ имя линии, поэтому
   продление всей линии из девяти имён упадёт. Срок — до 2026-12-09, то есть не срочно, но забывать
   нельзя: единственный оставшийся потребитель файлов линии — **coturn** (`turn.therapysto.ru`,
   `deploy/jitsi/bin/sync-coturn-tls.sh` кладёт ему копию). Развилка: либо сузить линию certbot до
   одного `turn.therapysto.ru`, либо перевести coturn на файлы из хранилища Caddy.
2. **Файл журнала края создаёт проверка конфигурации от root** и служба под учёткой `caddy` его уже
   не открывает. Ровно на этом переключение 14.09 упало при верном конфиге, оставив прод без края.
   Починено в `cutover-edge-to-caddy.sh` (владелец каталога восстанавливается после проверки).
