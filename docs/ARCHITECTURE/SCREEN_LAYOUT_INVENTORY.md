# Screen Layout Inventory

> ⚠️ **ЧАСТИЧНО УСТАРЕЛО (2026-07-13).** §«L4. Doctor Patients Workbench» — «Right: selected client preview/rail»
> удалено (owner ruling: правая мини-карточка-превью на списке «Клиенты» была ошибкой агента). §«L5. Doctor Entity
> Card» — «overview/program tabs» ссылается на вкладку «Обзор», которой больше нет. Актуально:
> [`docs/design/bersoncare-карточка-пациента-CURRENT-SPEC.md`](../design/bersoncare-карточка-пациента-CURRENT-SPEC.md)
> (клик по строке в «Клиенты» открывает полную карту; карточка — 4 вкладки Карточка/Программа/Файлы/Учётка). Прочие
> layout-секции (L1–L3, L6, Shell Widths) супесессии не подвержены.

> Фактическая карта внешних раскладок экранов на 2026-07-13.
