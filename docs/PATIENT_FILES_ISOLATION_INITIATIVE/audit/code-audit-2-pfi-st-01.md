...
- Behavioral improvement over the old code: the prior version emitted `[firstName, lastName]` (Имя Фамилия — _wrong_ order); now Фамилия Имя Отчество. The `fio !== "Клиент"` guard then falls back to `displayName` and finally "Клиент" — preserves the previous fallback chain.
