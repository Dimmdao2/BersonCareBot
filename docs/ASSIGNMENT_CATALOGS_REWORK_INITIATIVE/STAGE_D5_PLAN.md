...
**Отложено (owner pause):** переименование колонки и массовый рефактор контрактов **не** входят в текущий объём работ. До снятия паузы канон: колонка `recommendations.domain`, подпись UI «Тип», справочник типов в БД (D3). Spike / gate / EXEC — только после явного возобновления этапа (запись в [`LOG.md`](LOG.md)).
