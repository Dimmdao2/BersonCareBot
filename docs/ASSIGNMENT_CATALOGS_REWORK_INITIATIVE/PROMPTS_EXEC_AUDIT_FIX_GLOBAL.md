...
Сделай: Drizzle колонки + measure_kinds + API + CreatableComboboxInput + ClinicalTestForm + фильтры списка + backfill scoring. Q2: только каталог, без инстансного UX qualitative.
Целевые проверки; НЕ pnpm run ci. LOG.md + коммит.
```

## B2 — AUDIT

```text
Аудит B2: схема additive, backfill, форма всех schema_type, combobox, API measure_kinds, фильтр региона реально фильтрует.
...
```text
EXEC B4: рекомендации — поля по PRE_IMPLEMENTATION (domain остаётся, UI «Тип», регион, тексты); без merge старых domain с новыми в одной миграции.
...
```text
EXEC B5: ЛФК UX + «глаз» по PRE_IMPLEMENTATION (индикатор vs кнопка), фильтры B1, карточка/список.

AUDIT B5 → AUDIT_STAGE_B5.md + MANDATORY FIX INSTRUCTIONS.
FIX → LOG; коммит.
```
...
```text
EXEC B7: universal comment по STAGE_B7 + PRE_IMPLEMENTATION (в т.ч. local_comment на lfk_complex_exercises; patient Plan UI — v1 минимально).

AUDIT B7 → AUDIT_STAGE_B7.md + MANDATORY FIX INSTRUCTIONS.
