...
4. **Эскалация после повторного провала:** если два coder/fixer-прохода подряд не закрыли один и тот же этап, класс замечаний или audit-fail, оркестратор не запускает третий такой же круг и не подменяет ремонт запуском "ещё одного аудитора". Следующий проход должен быть именно coder/fixer по лестнице: `gpt-5.6-sol`; если Sol не справился или недоступен — Sonnet как другой взгляд; если и Sonnet не справился — Opus. Если Opus тоже не закрыл проблему, полностью остановиться, записать blocked note с путями к логам/аудитам и ждать владельца.
