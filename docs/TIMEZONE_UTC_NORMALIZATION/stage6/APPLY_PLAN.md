...
- **Legacy (две отдельные БД):** транзакции **независимы**. Если COMMIT на integrator прошёл, а webapp упал — восстановление из backup или повторный целевой прогон только webapp-части после анализа (не автоматизировано).
- **Unified PostgreSQL (актуальный production):** один `DATABASE_URL`, схемы `integrator` и `public` — кросс-схемные операции могут быть в **одной** транзакции там, где это реализовано в коде; риск «рассинхрон двух коммитов» для этого сценария не тот же, что при двух кластерах.
