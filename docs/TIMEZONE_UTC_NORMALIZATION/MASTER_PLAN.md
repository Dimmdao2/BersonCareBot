...
### Что уже правильно
- `timestamptz` в PostgreSQL — хранит UTC instant.
- `formatBusinessDateTime.ts` (webapp) — форматирует из instant + IANA.
- `system_settings.app_display_timezone` — единая настройка отображения в webapp.
- `scheduleNormalizer.ts` — слоты нормализуются с `+03:00` (правильно для MSK, но хардкод).
- `parseBusinessInstant` — компенсация для наивных строк в webapp (костыль, не корень).

### Что сломано
- **`connector.ts:109`** — `recordAt` из Rubitime пробрасывается как есть (наивная строка) на части путей; основной webhook-путь нормализуется (Stage 3).
- **`writePort.ts:131→178`** — наивная строка идёт в `upsertRecord` (integrator DB) и в проекцию (webapp DB), если upstream не очистил время.
- **`bookingRecords.ts:57`** — `VALUES ($1, $2, $3, ...)` без `::timestamptz` — PG кастит по session TZ.
- **`pgAppointmentProjection.ts:82`** — `$3::timestamptz` — PG кастит наивную строку по session TZ.
- **Integrator display timezone (Stage 4)** — источник истины: `system_settings.app_display_timezone` (admin), `getAppDisplayTimezone({ db, dispatchPort? })`; не env. Устаревшие `APP_DISPLAY_TIMEZONE` / `BOOKING_DISPLAY_TIMEZONE` только для узких скриптов (`getAppDisplayTimezoneSync`) и с предупреждением в логах.
- **Хардкоды `+03:00`** — `scheduleNormalizer.ts`, `bookingM2mApi.ts` (`DEFAULT_SLOT_TZ`).

### Таблицы, хранящие время записей

| Таблица (БД) | Колонка | Тип | Кто пишет | Проблема |
|---|---|---|---|---|
| `rubitime_records` (integrator) | `record_at` | `timestamptz` | `writePort` → `upsertRecord` | Наивная строка → неверный instant |
| `appointment_records` (webapp) | `record_at` | `timestamptz` | `pgAppointmentProjection` | Наивная строка из проекции → неверный instant |
| `patient_bookings` (webapp) | `slot_start`, `slot_end` | `timestamptz` | `pgPatientBookings` | Native: корректно (из слотов). Compat: наивная строка из проекции |
...
- **S4.T03** — `env.ts`: убрать `APP_DISPLAY_TIMEZONE`, `BOOKING_DISPLAY_TIMEZONE` из zod-схемы. Оставить `RUBITIME_RECORD_AT_UTC_OFFSET_MINUTES` как deprecated fallback (лог warn при использовании).
...
**Gate:** `pnpm run ci` зелёный. Grep по `+03:00` / `RUBITIME_RECORD_AT_UTC_OFFSET` / `BOOKING_DISPLAY_TIMEZONE` в src — ноль вхождений (кроме тестовых фикстур и комментариев).
...
- [ ] Внутри системы нет наивных дат в `record_at` / `slot_start` / `slot_end`.
- [ ] Нет env-переменных для timezone (кроме секретов и DSN).
- [ ] Нет хардкодов `+03:00` в продуктовом коде.
- [ ] Integrator и webapp читают display-timezone из одного источника (БД).
- [ ] Филиал хранит свою IANA-зону.
- [ ] `normalizeToUtcInstant` — единственная функция интерпретации наивных дат.
- [ ] Для невалидных времени/зоны реализован вариант A: запись не теряется, но инцидент + Telegram-алерт обязательны.
- [ ] Механизм инцидентов и алертов одинаково применим для всех интеграций, где есть бизнес-обязательные datetime.
- [ ] Контрактные тесты (Stage 8) зелёные.
- [ ] `pnpm run ci` зелёный.
- [ ] Бэкфилл исторических данных применён.
