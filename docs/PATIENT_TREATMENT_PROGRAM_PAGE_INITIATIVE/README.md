...
Мини-инициатива по **странице программы лечения** в кабинете пациента: data-enabler, деталь, визуальный редизайн детали (**вкладки**, без отдельного маршрута этапа у пациента), список программ.

**Канонические URL (webapp):** **`/app/patient/treatment`**, **`/app/patient/treatment/[instanceId]`**; редирект с **`/app/patient/treatment-programs`** — см. `apps/webapp/next.config.ts`.
