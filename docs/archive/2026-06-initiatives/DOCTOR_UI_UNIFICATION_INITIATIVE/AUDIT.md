...
## Сводка после фаз 0–5 (2026-06-04)

| Сигнал | Состояние |
|--------|-----------|
| `rounded-2xl` в `doctor/**/*.tsx` | **0** (устранено) |
| Голый `<h2>` без `className` | **0** |
| Маршрутов `page.tsx` | **78**; реестр §8 — только `completed`, `cancelled` или `n/a` |
| Foundation | `doctorVisual.ts`, `DoctorSection` / `DoctorEmptyState` / `DoctorMetricList`, `doctorClientCardChrome` |
| Split-каталоги | 6 page clients + courses — `DoctorCatalogFiltersToolbar` / `CatalogSplitLayout` |
| Page-level `shadow-sm` на списках | убрано; намеренные исключения — см. [`LOG.md`](LOG.md) § «Принятые исключения» |

Исторический baseline (до правок) — в [`LOG.md`](LOG.md) фаза 0. Детальные таблицы §1–7 ниже сохраняют колонку «Current issue» как снимок на момент аудита; колонка **Status** — актуальна.
...
**Статус инициативы (2026-06-04):** частично пройден, **первично принят** при закрытии phased-плана. Полная постраничная таблица ниже — **шаблон** для будущих отдельных задач (глобальная доработка UI), не для активного plan Build.

Исторические отметки по фазам — в [`LOG.md`](LOG.md).
