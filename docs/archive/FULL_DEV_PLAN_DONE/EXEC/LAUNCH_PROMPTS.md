...
>
> **Правила CI**: targeted-проверки после каждого шага (`tsc --noEmit`, `vitest run <файлы>`, `eslint <файлы>`). Полный `pnpm run ci` — только в конце пака и перед push.
>
> **Правило Tailwind**: при касании любого файла с UI — заменять legacy-классы из globals.css на Tailwind + shadcn. Если класс больше нигде не используется — удалить из globals.css.
...
При касании UI-файлов: заменяй legacy-классы из globals.css (.button, .panel, .feature-card и т.д.) на Tailwind + shadcn. Если класс больше нигде не используется — удали из globals.css.

После каждого шага: targeted-проверки (tsc --noEmit, vitest run <затронутые файлы>, eslint <затронутые файлы>). Полный pnpm run ci — только после всех трёх шагов. При FAIL — чини. Обновляй @docs/FULL_DEV_PLAN/finsl_fix_report.md
```
...
После шага: targeted-проверки (tsc --noEmit, vitest run <затронутые тесты>, eslint). Полный pnpm run ci — в конце. Обновляй @docs/FULL_DEV_PLAN/finsl_fix_report.md
```
...
При касании UI-файлов: заменяй legacy-классы из globals.css на Tailwind + shadcn. Если класс больше нигде не используется — удали из globals.css.

После каждого шага: targeted-проверки (tsc --noEmit, vitest run <затронутые файлы>, eslint). Полный pnpm run ci — после всех трёх шагов. Обновляй @docs/FULL_DEV_PLAN/finsl_fix_report.md
```
...
При касании UI-файлов: заменяй legacy-классы из globals.css на Tailwind + shadcn. Удаляй неиспользуемые классы.

После каждого шага: targeted-проверки (tsc --noEmit, vitest run <затронутые файлы>, eslint). Полный pnpm run ci — после обоих шагов. Обновляй @docs/FULL_DEV_PLAN/finsl_fix_report.md
```
...
I.11 — Бейдж 29 непрочитанных: если мусорные данные — почистить. Расписание reminders — реализуется в Pack D (шаг D.2), здесь не трогаем.
...
Выполняй шаги строго по порядку из файла инструкций. После каждого шага: targeted-проверки (tsc --noEmit, vitest run <затронутые файлы>, eslint <затронутые файлы>). Полный pnpm run ci — только после последнего шага. При FAIL — исправь (до 3 попыток). При касании UI-файлов: заменяй legacy-классы из globals.css на Tailwind + shadcn; если класс больше нигде не используется — удали из globals.css. Обновляй @docs/FULL_DEV_PLAN/finsl_fix_report.md
...
Выполняй шаги строго по порядку. После каждого шага: targeted-проверки (tsc --noEmit, vitest run <затронутые файлы>, eslint). Полный pnpm run ci — только после последнего шага. При FAIL — исправь (до 3 попыток). При касании UI-файлов (B.4–B.6): заменяй legacy-классы из globals.css на Tailwind + shadcn; удаляй неиспользуемые. Обновляй @docs/FULL_DEV_PLAN/finsl_fix_report.md
...
Выполняй шаги строго по порядку. После каждого шага: targeted-проверки (tsc --noEmit, vitest run <затронутые файлы>, eslint). Полный pnpm run ci — только после последнего шага. При FAIL — исправь (до 3 попыток). Обновляй @docs/FULL_DEV_PLAN/finsl_fix_report.md
...
Выполняй шаги строго по порядку. После каждого шага: targeted-проверки (tsc --noEmit, vitest run <затронутые файлы>, eslint). Полный pnpm run ci — только после последнего шага. При FAIL — исправь (до 3 попыток). При касании UI-файлов (D.2, D.4): заменяй legacy-классы из globals.css на Tailwind + shadcn; удаляй неиспользуемые. Обновляй @docs/FULL_DEV_PLAN/finsl_fix_report.md
...
Выполняй только первые 4 шага (E.1–E.4). После каждого шага: targeted-проверки (tsc --noEmit, vitest run <затронутые файлы>, eslint). Полный pnpm run ci — после E.4. При FAIL — исправь (до 3 попыток). Обновляй @docs/FULL_DEV_PLAN/finsl_fix_report.md
...
```text
Продолжи пакет E — шаги E.5–E.7. Выполняй по порядку. После каждого шага: targeted-проверки (tsc --noEmit, vitest run <затронутые файлы>, eslint). Полный pnpm run ci — после E.7. Обновляй @docs/FULL_DEV_PLAN/finsl_fix_report.md
...
Выполняй шаги строго по порядку. После каждого шага: targeted-проверки (tsc --noEmit, vitest run <затронутые файлы>, eslint). Полный pnpm run ci — только после последнего шага. При FAIL — исправь (до 3 попыток). При касании UI-файлов (F.4, F.6, F.7): заменяй legacy-классы из globals.css на Tailwind + shadcn; удаляй неиспользуемые. Обновляй @docs/FULL_DEV_PLAN/finsl_fix_report.md
