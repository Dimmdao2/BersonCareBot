...
[`declaration.ts`](./declaration.ts) — исполняемый источник DEV/TEST/PROD для:
...
## Три базы, два кластера

Объявлены три базы, и связь «база → среда» живёт в ОДНОМ месте — `REV10_DATABASE_ENV` в `declaration.ts`:
`bcb_webapp_dev` → `dev`, `bersoncarebot_test` → `test`, `therapysto_prod` → `prod`. Незнакомое имя базы
эта карта ОТКАЗЫВАЕТ; раньше на её месте стоял двусторонний тернарник, и любое неизвестное имя молча
получало логины среды `dev`.

`therapysto_prod` — новый прод на `135.106.187.95`, заведён 10.09.2026. До этого декларация знала ровно два
имени баз, поэтому прод работал на базе, буквально названной `bersoncarebot_test`. Форма прода — ровно форма
TEST: те же порты, те же канонические роли, те же членства; отличаются только имена логинов
(`therapysto_prod_webapp_staff|_patient|_global_admin`, `therapysto_prod_integrator`), имена password-env
(`THERAPYSTO_PROD_*_PASSWORD`) и база. Имена без `bersoncarebot`/`bcb` — решение владельца 10.09.2026
(«Именование на новом проде» в `docs/ARCHITECTURE/SERVER CONVENTIONS.md`).

**Кластеров при этом два, а не три.** `dev` и `test` делят один PostgreSQL на dev-боксе, `prod` стоит на
своём хосте; это записано в `cluster.colocated`. Артефакт базы применяется к ОДНОМУ кластеру, поэтому он
называет только принципалов своего кластера: имена логинов соседнего кластера в него не попадают, а роль,
объявленная на среду (`RoleDecl.envs` — мигратор деплоя `bcb_dev_migrator`/`bcb_test_migrator`/
`therapysto_prod_migrator`), не создаётся в чужом кластере. Всё остальное — канонические `app_*` роли, швы,
политики, каталог порт-контекста — общее для всех трёх баз.

Кластерные примитивы `--shared-role-baseline` и `--shared-role-verify` ТРЕБУЮТ `--db`: он называет не
объекты базы, а ЦЕЛЕВОЙ КЛАСТЕР. Раньше аргумент был необязательным, и без него раскладывалось и
сверялось надмножество всех объявленных ролей. Пока кластер был один, надмножество совпадало с истиной;
со вторым кластером оно стало молча неверным для обоих, а `--shared-role-verify` — который требует
СУЩЕСТВОВАНИЯ каждой ожидаемой роли — начал падать на чужом миграторе в исправном кластере. Поэтому
умолчания больше нет: вызывающий, который не может назвать базу, не сможет и применить результат.

Переход СТАРОГО прода (`bersoncare.ru` на `135.106.162.170`) из снимка A в B0 — по-прежнему отдельная
атомарная миграция с rollback после явного разрешения владельца; DEV/TEST-конфигурацию нельзя молча выдать
за тот target.
