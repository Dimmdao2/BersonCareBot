# АРХИВ — канонизация записей / Rubitime cutover

> **HISTORICAL / NON-EXECUTABLE:** Rubitime выведено 2026-07-27. Этот runbook описывает завершённый переход и
> не является текущей PROD-инструкцией. Текущий `151.241.228.122` — DEV/RELAY/TEST; PROD — только
> `135.106.162.170` (`adelaide`). Команды ниже не выполнять.
...
**Контекст и обоснование:** [`APPOINTMENTS_PARITY_S0.md`](APPOINTMENTS_PARITY_S0.md), [`SYNC_BEHAVIOR_ANALYSIS.md`](SYNC_BEHAVIOR_ANALYSIS.md). Историческим источником истины тогда была Rubitime CSV-выгрузка.
...
2. **Исторический доступ к prod-БД** через env-файл (см. текущий канон
   [`SERVER CONVENTIONS.md`](../../../docs/ARCHITECTURE/SERVER%20CONVENTIONS.md)); команду из этого архива не выполнять.
...
- Историческое поведение синка: [`SYNC_BEHAVIOR_ANALYSIS.md`](SYNC_BEHAVIOR_ANALYSIS.md).
