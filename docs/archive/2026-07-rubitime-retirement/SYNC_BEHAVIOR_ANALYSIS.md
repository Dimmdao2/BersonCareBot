# АРХИВ — разбор синхронизации записей Rubitime (2026-06-14)

> Rubitime выведено 2026-07-27. Документ описывает исторический двусторонний sync и не является текущей архитектурой.

Контекст: owner-запрос «очень внимательно разобрать, как происходят записи, отмены и переносы в обе стороны (наша БД ↔ Rubitime), воспроизвести все варианты, понять cancel vs delete и направление синка». Анализ по коду + архитектуре ([RUBITIME_BOOKING_PIPELINE.md](ARCHITECTURE/RUBITIME_BOOKING_PIPELINE.md), удалённый после canonical cutover `apps/webapp/src/modules/booking-appointment-sync/README.md`, контракт [BOOKING_MIRROR_INTEGRITY_CONTRACT](../../../docs/BOOKING_REWORK_INITIATIVE/BOOKING_MIRROR_INTEGRITY_CONTRACT.md)).
...
3. **Стале-активные записи (есть у нас, нет в Rubitime).** Owner прав: «нет в Rubitime → почти всегда некорректно». Причины: (а) перенос/удаление в Rubitime не доехало (наш `created` остался); (б) create в кабинете при неотлаженном механизме, не ушёл в Rubitime. Правило реконсиляции: **legacy-запись, отсутствующая в текущей выгрузке Rubitime → устаревшая** (гасить). Это и есть авто-детект по CSV (предложен в backfill-скрипт).
4. **overlap (§1)** — закрыть бэкфиллом + cutover; проверить null-specialist путь.
5. **`assertSlotAvailable` против canonical при `slots=rubitime`** — на create пациента она вообще не зовётся (полагается на Rubitime); для staff зовётся, но против canonical. После cutover на canonical как единый источник — консистентно.

## 4. Матрица воспроизведения (что прогнать; ожидаемый результат)

> Это историческое требование воспроизведения больше не действует. Rubitime runtime отсутствует; архивный анализ
> не является основанием возвращать provider mock/staging. Старое покрытие:
> [ACCEPTANCE_MIRROR_SYNC.md](../../../docs/BOOKING_REWORK_INITIATIVE/ACCEPTANCE_MIRROR_SYNC.md).
