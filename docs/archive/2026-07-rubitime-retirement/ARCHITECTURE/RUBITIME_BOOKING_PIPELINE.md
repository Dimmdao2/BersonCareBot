# АРХИВ — Rubitime: вебхук, журнал, проекция, имена и статусы

> Rubitime выведено из эксплуатации 2026-07-27. Документ сохранён только как историческое описание удалённого контура и не является инструкцией для текущего runtime.
...
2. Integrator → M2M **`POST /api/integrator/patient-notifications/web-push`** (`intentType: appointment_lifecycle`) → текст в **PWA-чат** (`/app/patient/messages`) + Web Push с тем же `openUrl`. См. [`PATIENT_SUPPORT_CHAT_INBOX.md`](../../../../docs/ARCHITECTURE/PATIENT_SUPPORT_CHAT_INBOX.md). Slot-напоминания (`appointment_reminder`) в чат **не** пишутся.
...
| 2026-06-05 | **Двустороннее зеркалирование (`AppointmentMirrorSync`):** live inbound/outbound для mapped appointments; единый `buildCanonicalInboundSnapshot`; см. § ниже и [`ACCEPTANCE_MIRROR_SYNC.md`](../../../../docs/BOOKING_REWORK_INITIATIVE/ACCEPTANCE_MIRROR_SYNC.md).                                                                                                                                                                                                                                                                       |
| 2026-06-05 | **Mirror integrity hardening:** partial-failure flags в patient/staff API; echo-guard / stale-mapping skip legacy fanout; revive-guard в `patient_bookings`; lifecycle `FOR UPDATE` + `state_conflict`; Rubitime-first rollback при package/product failure; `empty_patch` → 400 на M2M `update-record`. Контракт: [`BOOKING_MIRROR_INTEGRITY_CONTRACT.md`](../../../../docs/BOOKING_REWORK_INITIATIVE/BOOKING_MIRROR_INTEGRITY_CONTRACT.md).                                                                                             |
| 2026-06-06 | **Gaps closeout:** rubitime-first — no `createAppointment` fallback; create-rollback **`deleteRecord`** (не `cancelRecord`); doctor legacy `rubitime/cancel` → `update-record` status 4; patient partial UI (`rubitimeMirrorFailed` warning toast); shared `rubitimeCreateRollback`. План: [`.cursor/plans/archive/booking_gaps_closeout_e5b725fb.plan.md`](../../../../.cursor/plans/archive/booking_gaps_closeout_e5b725fb.plan.md); LOG [`BOOKING_REWORK_INITIATIVE/LOG.md`](../../../../docs/BOOKING_REWORK_INITIATIVE/LOG.md) §2026-06-06. |
...
Исторический код: удалённые `apps/webapp/src/modules/booking-appointment-sync/README.md` и
`infra/repos/pgBookingRubitimeBridge.ts`; сохранившийся `integrator/events.ts`; удалённый
`apps/integrator/.../normalizeUpdateRecordPatch.ts`.

### Целостность mirror (2026-06, hardening)

- **Canonical-first после commit:** отмена/перенос пациента и staff manual-cancel применяли канон до best-effort Rubitime; HTTP **`ok: true`** с partial flags при сбое внешнего шага (исторический набор флагов — см. [`BOOKING_MIRROR_INTEGRITY_CONTRACT.md`](../../../BOOKING_REWORK_INITIATIVE/BOOKING_MIRROR_INTEGRITY_CONTRACT.md) § Partial outcomes).
...
Подробнее: [`OWN_BOOKING_ENGINE_INITIATIVE/CANONICAL_MODEL.md`](../../legacy-underscore/OWN_BOOKING_ENGINE_INITIATIVE/CANONICAL_MODEL.md), [`patient-booking.md`](../../../../apps/webapp/src/modules/patient-booking/patient-booking.md), [`booking-calendar.md`](../../../../apps/webapp/src/modules/booking-calendar/booking-calendar.md).
