...
- **Трек БД/бэкфилл (исходный чат, завершённая история):** довести скрипт `apps/webapp/scripts/backfill-canonical-from-legacy-appointments.ts` до полноценного: толерантный режим (пропускать+собирать конфликты), **схлопывание дублей по всей базе** (могли возникнуть при разработке/миграциях на новый механизм), удаление тестовых/блок-записей (`+79189000782` «Берсон», `+70000000000` «БЛОК ОКНА»), харднинг проекции. Архивный артефакт-вердикт: [APPOINTMENTS_PARITY_S0.md](../archive/2026-07-rubitime-retirement/APPOINTMENTS_PARITY_S0.md). Этот пункт не является текущим поручением и не разрешает возобновлять Rubitime.
...
> **SUPERSEDED:** Rubitime выведено 2026-07-27. Весь S0 ниже сохранён как историческое обоснование артефакта;
> его DB-сравнения, катовер и sync-проверки не являются текущей задачей и не исполняются.
...
**Исторический артефакт.** Отчёт
[`docs/archive/2026-07-rubitime-retirement/APPOINTMENTS_PARITY_S0.md`](../archive/2026-07-rubitime-retirement/APPOINTMENTS_PARITY_S0.md)
сохранён только как evidence завершённого retirement и не является текущим заданием на катовер.
