...
3. **MP4-fallback после готового HLS нет:** playback-контракт не выводит `delivery` иначе как из `video_processing_status` строки — «переключиться на прогрессивный MP4, когда HLS сломан» такой развилки в коде нет, независимо от того, жив ли ещё исходный объект `media_files.s3_key` (он больше не удаляется транскодом, см. `VIDEO_DELIVERY_COST_AND_METERING_2026-09-11.md`). При фатальной ошибке HLS плеер делает **одно** обновление playback JSON (на случай истёкшего presigned) и, если это не помогло, показывает штатную ошибку с кнопкой «Повторить».
...
- Если браузер не умеет **ни** нативный HLS, **ни** `hls.js` (MSE), видео с готовым HLS воспроизвести нечем: плеер показывает штатную ошибку и пишет событие `hls_js_unsupported` в телеметрию. Подменить его прогрессивным MP4 нельзя — playback-контракт такой развилки не предлагает.
