...
> **05.09 reconcile (#1069, Т14, `docs/OWNER_DECISIONS.md`):** одна глобальная сетка периодов, полная матрица
> цена×период у каждого активного тарифа (`saas_tariff_period_prices`); подписка хранит
> текущий/отложенный `billing_period_code`. Немедленный/пропорциональный upgrade-путь этого плана (упомянутые
> ниже «Доплата за повышение тарифа», `createProratedTariffUpgradeInvoice`, `proration.test.ts`) — ОТМЕНЁН:
> смена тарифа/периода и отмена подписки вступают в силу только на границе `currentPeriodEndsAt`. Разделы ниже с
> историей отладки апгрейд-инвойса — это evidence прошлой (уже неактивной) модели, не текущее поведение.
> Возврат клиника→платформа убран целиком (см. `PAYMENTS_CABINET_PLAN.md` → К2); пациентские возвраты не
> затронуты.
...
> каждой механики~~ **→ T1 + 05.08 (#1069): одна системная лестница, без per-mechanic исключений**. Дословно 30.07: «ты вообще не должен решать
> что ограничивать а что нет. ты должен дать мне механизм». Значения 7/3/21, засеянные миграцией
> `0259_saas_billing_foundation.sql:238-242`, тоже подлежат снятию (пункт 2.6a). Читать эту фазу можно только как
> описание платёжной механики; лестницу брать из §5a → **Т1**.
