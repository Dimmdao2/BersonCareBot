...
## Раскатка cutover'ом отмечает миграцию применённой, но её данные не переносит — регистрация клиники умирает

**Найдено 11.09.2026 замером, не рассуждением.** На DEV и на TEST таблица
`public.reference_catalog_baselines` пуста — 0 строк. Последствие немедленное и полное: триггер
`be_organizations_reference_catalog_snapshot` зовёт `app.seed_reference_catalog_snapshot`, та берёт
последний baseline через `SELECT … INTO STRICT` и падает `P0002 query returned no rows`, поэтому
**любой `INSERT` в `be_organizations` отказывает**. То есть зарегистрировать новую клинику было
невозможно ни на DEV, ни на TEST. Проверено прямым INSERT в транзакции с ROLLBACK на обеих базах:
до правки «ОТКАЗ [P0002] query returned no rows», после — «клиника создалась, справочных категорий
засеяно 9».

**Механизм, а не просто протухшие данные.** Данные несёт forward-миграция
`20260821T025935_restore_reference_catalog_baselines` (два `INSERT` плюс собственный
`BCB-MIGRATION-VERIFY` на две строки и их md5). Но артефакт раскатки
`deploy/postgres/generated/prod-to-target/ledgers-and-baseline.sql` **вписывает эту миграцию в
леджер как уже применённую** (строка 60), а её строк не несёт. Мигратор применяет по watermark
`created_at`, а не по хешу — значит на любой cutover-цели миграция пропускается навсегда, её VERIFY
не исполняется никогда, и таблица остаётся пустой молча. Это тот же класс, что уже записан в памяти
как `drizzle-migrator-watermark-not-hash`.

**Что сделано 11.09:** на DEV и TEST применён канонический снимок
`deploy/postgres/reference-catalog-baselines.sql` (идемпотентный, с собственной проверкой md5); обе
базы теперь создают организацию. Это лечение симптома на двух базах, не механизма.

**Что надо к раскатке прода — и почему именно здесь:**

- [ ] Проверить `reference_catalog_baselines` на НОВОМ проде `135.106.187.95` (чтение, `therapysto_prod`).
      Если пусто — новая клиника там не регистрируется, и это обнаружится первым же живым клиентом.
      Ведущий 11.09 попытался прочитать её и был остановлен гейтом доступа — нужен явный owner-go или
      выполнение через порт-агента.
- [ ] Починить механизм, а не базы: артефакт cutover'а обязан либо нести строки глобальных
      не-арендаторских данных, либо НЕ вписывать в леджер миграции, чьи данные он не переносит.
      Второе честнее: тогда миграция отработает сама и её VERIFY сработает.
- [ ] Проверить, нет ли ещё миграций того же класса — вписанных в `ledgers-and-baseline.sql`
      backfill-миграций, чьи данные артефакт не несёт. Это разовое действие, проверяется взглядом и
      `rg`, а не тестом.
