...
**УСТАРЕЛО/ЗАМЕНЕНО → владелец, 18.08 (позже в тот же день):** «константа 30 дней в коде» отменена. Срок
жизни счёта — **админ-настройка**, одна на **все** счета (и выставленные вручную, и автоматические при
продлении), администратор её меняет. Отдельного срока на конкретный счёт владелец не хочет: «если это уже
было — не надо это убирать, надо это сделать сразу так». Дом настройки —
`saas_billing_payment_provider.value.lifecyclePolicy.invoiceValidityDays`; 30 дней остаются документированным
дефолтом на случай незаданной настройки.
...
- [x] 1.3 Единственный дом срока жизни счёта: `modules/saas-billing/invoiceValidity.ts` —
      ...
      **УСТАРЕЛО/ЗАМЕНЕНО → 1.3a (владелец, 18.08, тот же день).** Константа в коде отменена; см. правку
      решения выше.
- [x] 1.3a Срок жизни счёта — админ-настройка, действующая на КАЖДЫЙ счёт.
      Дом значения: `modules/saas-billing/settings.ts` — `lifecyclePolicy.invoiceValidityDays` внутри
      системной настройки `saas_billing_payment_provider` (новых ключей, таблиц и миграций нет).
      `invoiceValidity.ts` больше не решает число: `saasBillingInvoiceExpiresAt(issuedAt, validityDays)`
      требует его от вызывающего, а `SAAS_BILLING_INVOICE_VALIDITY_DAYS = 30` остаётся только дефолтом.
      Значение раздаётся через `resolvePaymentProvider()` (`service.ts:158`), поэтому у каждого пути
      выставления оно уже под рукой. Пять путей выставления, все переведены: ручной счёт
      (`service.ts:createManualSaasBillingInvoice`), счёт продления из кабинета
      (`createRenewalSaasBillingInvoice`), фоновый тик продления (`runDueSaasBillingRenewals` →
      `createSaasBillingRenewalInvoiceIfAbsent`), место сверх тарифа (`purchaseSeatOverage` — раньше жил
      до конца периода услуги), апгрейд тарифа с пропорцией (`createProratedTariffUpgradeCheckout`).
      Редактируется администратором: `app/app/admin/payments/SaasBillingProviderSettings.tsx`
      («Срок оплаты счёта, дней»). Поле «Срок действия (до)» в форме выставления НЕ возвращено —
      владелец хочет одну настройку, а не выбор на каждый счёт.
      Тесты: `settings.unit.test.ts` (адрес значения, дефолт, дисциплина парсинга, обратный путь через
      redact/merge), `service.test.ts` («срок жизни счёта берётся из настройки на КАЖДОМ пути
      выставления» — пять путей + дефолт + сохранённая идемпотентность двойного клика),
      `SaasBillingProviderSettings.ui.test.tsx` (админ видит и меняет).
...
- Найдено при работе над этапом 1, **не чинил**: ключ идемпотентности ручного счёта теперь
  ...
  После 1.3a свойство сохранено и закреплено тестом: срок в хеш НЕ входит, поэтому смена настройки
  между двумя кликами по одной форме не рождает второй счёт на те же деньги.
- Не делал в рамках 1.3a: срок счёта передаётся провайдеру (`invoice.expiresAt` в `createIntent`)
  по-прежнему только на ручном пути — там, где ЮKassa выставляет собственный счёт. Автоматические
  пути провайдеру счёт не выставляют, и расширять их payload владелец не просил.
