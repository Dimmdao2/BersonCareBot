...
- GitHub Actions [`.github/workflows/ci.yml`](../../../.github/workflows/ci.yml): job `test-webapp-core` (matrix `VITEST_SHARD` 1/3–3/3 + кэш Vitest) на каждом PR/push запускает `pnpm test:webapp:fast`; отдельный job `test-webapp-behavior` запускает текущие `unit`/`route` проекты через `pnpm test:webapp:behavior`. Те же классы входят в локальный `pnpm run ci`. Все webapp-тесты в CI без `DATABASE_URL` → globalSetup сразу выходит, миграции в CI не выполняются.
...
- **B2 (closed):** неявная зависимость от `migrate:legacy` в globalSetup снята; legacy переведён в явный opt-in флаг.
