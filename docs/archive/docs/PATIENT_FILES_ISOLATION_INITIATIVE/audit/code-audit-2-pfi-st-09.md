...
3. **(POSITIVE) Test design is correct and non-vacuous.** The two ST-09 cases set
   ...
   called — this is exactly the right way to prove the guard is bypassed _because of
   the displayName-only branch_, not because the file happens to be outside a subtree.
...
'*.tsx' ':!*test*'` returns empty. `route.ts` identical in main and ST-09 head.
   ...
   anything to be _added_ to production code; the executor's read is sound.
