...
_patient's_ userId, then use `folder.id` as the scoping `folderId`. Asserts cover kind,
...
Note (ADVISORY, not a defect): a *separate* exported helper`clientFilesSubtreeFolderIdsSql`(lines 169–179) uses a raw recursive-CTE`sql\`...\`` fragment, but it is unrelated to PFI-ST-08,
