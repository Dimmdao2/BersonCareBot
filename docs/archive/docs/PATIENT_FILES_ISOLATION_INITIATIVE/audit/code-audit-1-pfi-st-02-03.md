...
- `fallbackName` is computed eagerly before the try/catch block (line 134), but this is inconsequential — `phoneNormalized` is already fetched. The fallback name is only _used_ in the second `catch` block (23505 collision), not the primary path. Correct.
