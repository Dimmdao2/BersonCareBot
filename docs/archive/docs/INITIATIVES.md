...
> **Актуализация 2026-09-01:** execution-status берётся из taskdb (`project=bcb`) через
> `node /home/dev/brain/tools/taskdb.mjs`, а продуктовый scope — из owner-review и roadmap конкретной инициативы.
> Ни одна статическая таблица ниже не открывает работу сама по себе: сначала позднее owner-решение, затем
> фактический код, затем taskdb и активный plan. Старые планы не исполняются только из-за незакрытой галочки.

## Текущий execution registry

- Очередь и статусы: `node /home/dev/brain/tools/taskdb.mjs list bcb`.
- Вопросы владельцу: `node /home/dev/brain/tools/taskdb.mjs waiting`.
- Единственная карта документов-владельцев: [`CURRENT_AUTHORITY_MAP.md`](CURRENT_AUTHORITY_MAP.md).

## Исторический execution snapshot (2026-07-22; не исполнять)
...
| Exercise store                    | [`_TODO/SAAS_FOUNDATION/EXERCISE_STORE_PLAN.md`](_TODO/SAAS_FOUNDATION/EXERCISE_STORE_PLAN.md)                                                                                                    | Канонический план создан по поручению владельца 05.08; taskdb-карточка/исполнение отдельно не активированы                                                    |
| RU privacy / production readiness | [`_TODO/RU_PRIVACY_AND_PRODUCTION_READINESS/README.md`](_TODO/RU_PRIVACY_AND_PRODUCTION_READINESS/README.md)                                                                                     | owner-activated umbrella `#898`; privacy/legal/product stages; инфраструктура вынесена в отдельную строку                                                    |
| Infrastructure security           | [`_TODO/INFRASTRUCTURE_SECURITY_PLAN.md`](_TODO/INFRASTRUCTURE_SECURITY_PLAN.md)                                                                                                                | единый план host/LUKS/S3/backup/DR/secrets/TLS/logs/incident response/Security CI; production actions только по owner gate                                    |
