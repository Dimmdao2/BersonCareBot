...
- [-] ~~Первый release: `patient` / `staff` / оба. Рекомендация: patient-first, staff продолжает работать в web.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~Первый store: RuStore/direct Android / Google Play / App Store и порядок. Рекомендация: Android device~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  prototype первым; iOS architecture проверяется одновременно, публикация — после Apple account/build gate.
- [-] ~~Один platform binary подтверждён; per-organization white-label native apps остаются вне scope.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~Billing/store policy: покупать SaaS внутри app, только управлять уже купленной подпиской или скрыть checkout до~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  отдельной IAP/store-policy реализации.

### AI work

- [-] ~~Зафиксировать current dependency map: RSC/cookies/redirects/API/PWA gates/file upload/media/auth.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~В изолированном worktree/temporary proof собрать disposable Capacitor shell с локальным `index.html`;~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
...
- [-] ~~Доказать на Android device/emulator и iOS simulator: API call, session exchange prototype, authenticated~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  screen, app link, push token registration stub, logout/revoke.
- [-] ~~Выбрать reuse boundary: shared contracts/primitives versus mobile-only adapters; дублирование domain logic~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  запрещено.
- [-] ~~Составить exact API gap list. Mobile не получает прямой DB access и не вводит параллельный backend.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~Измерить bundle/startup/network/error UX и пересчитать `MOB-04` по экранным группам.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
...
- [-] ~~Создать exact-scoped mobile package только после ADR: Capacitor core/CLI/platforms, locked versions, no~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  unreviewed community plugins.
- [-] ~~Bundle ID/application ID, environment mapping DEV/TEST/PROD и allowed origins не содержат tenant identity.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~Native projects являются воспроизводимыми source artifacts; signing credentials не коммитятся.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~Android build на Linux; iOS build только на owner-approved macOS/Xcode runner.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~Реализовать startup, maintenance/update-required, offline/server-unavailable и safe external-link screens.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~CI собирает unsigned/test artifacts и проверяет dependency/security/privacy manifests; release signing —~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
...
- [-] ~~Короткоживущий access token + rotating refresh token; refresh хранится Keychain/Keystore и может быть отозван~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  по device session. Web HttpOnly cookie flow сохраняется отдельно.
- [-] ~~Login через email/OAuth и Telegram/MAX code не передаёт access/refresh tokens в URL или messenger.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~Bind/login code одноразовый, rate-limited, purpose-bound; старые mini-app init-login paths выводятся по~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  `NTF-01`, а не поддерживаются как второй mobile auth.
- [-] ~~Universal Links/App Links имеют allowlist; custom scheme не принимает session/token и не открывает внешний URL~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  внутри privileged WebView.
- [-] ~~Logout, password/account security event и offboarding отзывают mobile sessions и push targets.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~Device list показывает пользователю активные sessions без сырых push tokens/device identifiers.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
...
- [-] ~~Event producers создают notification intent, не вызывают APNs/FCM/Web Push напрямую.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~Provider-neutral delivery target различает `web_vapid`, `fcm`, `apns`; native token не записывается в~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  `user_web_push_subscriptions`.
- [-] ~~Target принадлежит platform user/device; tenant authorization повторно проверяется по event/resource context.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~Token registration/rotation/logout/invalid-token/offboarding идемпотентны и имеют retention policy.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~Integrator delivery chokepoint получает adapters APNs/FCM; server credentials — restricted DB-backed settings~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  после S5/crypto gate, не env и не app bundle.
- [-] ~~App tap открывает только внутренний allowlisted route; payload не содержит presigned URL, secret или raw~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  clinical/free-text content.
- [-] ~~Web Push остаётся compatibility transport для browser users до отдельного retirement decision.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
...
- [-] ~~Модель данных на устройстве: что кешируется, TTL, logout/offboarding wipe, OS backup inclusion/exclusion.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~Refresh tokens и keys — Keychain/Keystore; clinical data не хранится в Preferences/localStorage по умолчанию.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~App switcher/privacy screen и screenshot policy для чувствительных экранов определены threat model, не~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  blanket-запретом без UX оценки.
- [-] ~~Crash/analytics logs не содержат payload, токены, ФИО, телефоны, сообщения, файлы или diagnoses.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~Apple Privacy Manifest, App Privacy и Google Data Safety совпадают с фактическими SDK/data flows.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~APNs/FCM/Apple/Google внесены в processing/vendor/transborder register и закрыты `G-04B` до production.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~Rooted/jailbroken device policy, TLS pinning decision и app integrity controls имеют явный verdict; отсутствие~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  pinning не маскируется словом «невозможно».

## 10. MOB-06 — store/release

- [-] ~~Real-device matrix: supported OS, clean install/update, permission denied/re-enabled, background/killed push,~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  deep links, offline, slow network, expired/revoked session, multi-device and accessibility.
- [-] ~~Apple 4.2 evidence показывает app-like value: native push/deep links/secure session плюс минимум один реально~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  полезный native capability (например biometric app lock или safe file share/upload), а не пустой wrapper.
- [-] ~~Billing UI прошёл store-policy review. SaaS/feature unlock не ведёт на CloudPayments WebView вопреки правилам~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
  store; решение зафиксировано до submission.
- [-] ~~Demo/reviewer access не раскрывает production ПДн; privacy/support/delete-account links доступны.~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
- [-] ~~Signed artifact привязан к source SHA, SBOM/dependency report и release notes; rollback = предыдущий binary +~~ — ОТМЕНЕНО ВЛАДЕЛЬЦЕМ 2026-09-09: заменено планом двух тонких Capacitor-обёрток.
