...
overview: 'Массовые рассылки врача через broadcast_audit + public.outgoing_delivery_queue (kind=doctor_broadcast_intent) и integrator outgoingDeliveryWorker; журнал и health; опция «Прикрепить / обновить меню» — реализовано.'
status: completed
todos:
...
Исходный файл `~/.cursor/plans/doctor_broadcast_delivery_336bbbc0.plan.md` **отсутствует на диске**; каноническая копия — этот файл: [`.cursor/plans/archive/doctor_broadcast_delivery_336bbbc0.plan.md`](./doctor_broadcast_delivery_336bbbc0.plan.md). Продуктовая и техническая правда — в [docs/ARCHITECTURE/DOCTOR_BROADCASTS.md](../../docs/ARCHITECTURE/DOCTOR_BROADCASTS.md).
...
**Состояние:** основной контур и хвост меню **реализованы** в репозитории.
...
| Схема аудита               | [`apps/webapp/db/schema/schema.ts`](../../apps/webapp/db/schema/schema.ts) (`message_body`, `delivery_jobs_total`, `attach_menu_after_send`, …)                                                                                                                                                                                                  |
| Integrator                 | [`apps/integrator/src/infra/delivery/deliveryContract.ts`](../../apps/integrator/src/infra/delivery/deliveryContract.ts), [`outgoingDeliveryWorker.ts`](../../apps/integrator/src/infra/runtime/worker/outgoingDeliveryWorker.ts), [`doctorBroadcastIntentMenu.ts`](../../apps/integrator/src/infra/runtime/worker/doctorBroadcastIntentMenu.ts) |
...
3. **Глобальные TG/MAX setup из webhook** ([`setupMenuButton.ts`](../../apps/integrator/src/integrations/telegram/setupMenuButton.ts), [`setupCommands.ts`](../../apps/integrator/src/integrations/max/setupCommands.ts)) — **не** вызывать из воркера рассылок; опция меню — только per-chat `replyMarkup` / inline на `message.send` (см. `doctorBroadcastIntentMenu.ts`).

## «Прикрепить / обновить меню» — выполнено

| Шаг        | Реализация                                                                                                                                                                                                                                                                                                                                                                                                               |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Данные     | Колонка `broadcast_audit.attach_menu_after_send`; в `payload_json` флаг `attachMenu`.                                                                                                                                                                                                                                                                                                                                    |
| Webapp     | `BroadcastCommand.attachMenuAfterSend`, форма [`LabeledSwitch`](../../apps/webapp/src/components/common/form/LabeledSwitch.tsx) (по умолчанию выкл.; с каналом «бот»).                                                                                                                                                                                                                                                   |
| Integrator | Перед `dispatchOutgoing`: [`doctorBroadcastIntentMenu.ts`](../../apps/integrator/src/infra/runtime/worker/doctorBroadcastIntentMenu.ts) + [`delivery.ts`](../../apps/integrator/src/kernel/domain/executor/handlers/delivery.ts) (`enrichMessageSendPayloadWithMaxMainInlineIfApplicable`), [`buildMainReplyKeyboardMarkup`](../../apps/integrator/src/kernel/domain/executor/helpers.ts); без глобального меню Bot API. |
| UI журнал  | «Меню в чате обновлялось.» при `attachMenuAfterSend`.                                                                                                                                                                                                                                                                                                                                                                    |
| Тесты      | `outgoingDeliveryWorker.test.ts`, `doctorBroadcastIntentMenu.test.ts`, webapp `service` / `deliveryJobs` / `BroadcastAuditLog` / `BroadcastForm.test`.                                                                                                                                                                                                                                                                   |
...
- [x] Опция «Прикрепить / обновить меню» — см. таблицу выше.

## Проверки при закрытии хвоста меню

- [x] `pnpm --dir apps/integrator run test` — зелёный (полный пакет).
- [x] `pnpm --dir apps/webapp run test` — зелёный после выравнивания ожиданий в `BroadcastForm.test.tsx`.
- [x] [`DOCTOR_BROADCASTS.md`](../../docs/ARCHITECTURE/DOCTOR_BROADCASTS.md) — раздел про меню и каналы.
...
## Синхронизация смежных доков

- [`docs/ARCHITECTURE/SPECIALIST_CABINET_STRUCTURE.md`](../../docs/ARCHITECTURE/SPECIALIST_CABINET_STRUCTURE.md) §9.7 — формулировка приведена в соответствие с фактической доставкой через очередь и `DOCTOR_BROADCASTS.md`.
