...
if (webappEventsPort) {
  ...
  const link = await readPort?.readDb?.({
  ...
  if (integratorUserId) {
    ...
    if (topics.length > 0) {
      ...
      webappEventsPort
        ...
        .catch((err) =>
          logger.warn({ err, eventType: 'preferences.updated' }, 'projection emit failed'),
        );
    }
  }
}
...
const dbWritePort =
  input.dbWritePort ?? createDbWritePort({ db: dbPort, webappEventsPort, readPort: dbReadPort });
```
