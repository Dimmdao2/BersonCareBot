...
> **Архив (2026-05-05).** Инициатива как отдельный активный пакет **закрыта**: Phases 0–6 выполнены. Бывший **Phase 7** (отложенные маршруты и расширенный coverage) **не ведётся единым блоком** — любые дальнейшие выравнивания экранов входят в **соответствующие мини-инициативы по маршруту/фиче** (см. [`ROADMAP_2.md`](../../APP_RESTRUCTURE_INITIATIVE/ROADMAP_2.md) §1, принципы UI). Норматив для новых работ: [`PATIENT_APP_UI_STYLE_GUIDE.md`](../../ARCHITECTURE/PATIENT_APP_UI_STYLE_GUIDE.md).

Документированная follow-up инициатива после `PATIENT_APP_STYLE_TRANSFER_INITIATIVE`.

Цель: отдельно, без смешивания со style-transfer, проверить и при необходимости выровнять patient UI с существующими UI primitives в стиле shadcn (`Button`, `Card`, `Badge`, …). Недостающие **`Accordion` / `Collapsible`** добавлены в shared-слой (Phase 1); кабинет — Phase 2; **`FeatureCard` на `Card`** — Phase 3 (2026-05-04); профиль **`ProfileAccordionSection` → `Collapsible`** — Phase 4 (2026-05-04); уведомления **`ChannelNotificationToggles` → `Switch`** — Phase 5 (2026-05-04); **form controls** (support, diary + FAB `QuickAddPopup`, профиль OTP/purge, intake) — Phase 6 (2026-05-04). ~~Phase 7+~~ — см. блок «Архив» выше.
...
Базовый визуальный стандарт для всех фаз: [`PATIENT_APP_UI_STYLE_GUIDE.md`](../../ARCHITECTURE/PATIENT_APP_UI_STYLE_GUIDE.md).
...
| ~~Phase 7+~~ | **перенесено** в мини-инициативы по экранам | [`MASTER_PLAN.md`](MASTER_PLAN.md) §Phase 7 (архивная формулировка); операционно — [`ROADMAP_2.md`](../../APP_RESTRUCTURE_INITIATIVE/ROADMAP_2.md) §1 |

Связь с глобальным roadmap: [`APP_RESTRUCTURE_INITIATIVE/README.md`](../../APP_RESTRUCTURE_INITIATIVE/README.md), [`APP_RESTRUCTURE_INITIATIVE/LOG.md`](../../APP_RESTRUCTURE_INITIATIVE/LOG.md) (в т.ч. запись 2026-05-05 об архиве этой папки).
...
- [`PATIENT_APP_UI_STYLE_GUIDE.md`](../../ARCHITECTURE/PATIENT_APP_UI_STYLE_GUIDE.md) — общий стандарт shared/shadcn/patient layer, который эта инициатива не должна нарушать.
