...
- **Цель:** убрать визуальный хаос в нижней части главной пациента без изменения CMS-модели и без переписывания runtime-сборки блоков.
- **Порядок и сетка:**
  - На mobile блоки снова идут в порядке массива `blocks` из backend/CMS `sort_order`; явные mobile `order-*` не используются, чтобы врач мог переносить блоки в настройках.
  - На desktop оставлена принудительная dashboard-сетка через `lg:col-start`, `lg:col-span`, `lg:order-*`: `daily_warmup + useful_post`, `situations + booking`, `progress + next_reminder`, затем ряд `mood_checkin / sos / plan` по 4 колонки, `subscription_carousel` на всю ширину, `courses` в хвост.
  - `lg:grid-flow-row-dense` сохранён, чтобы пары `8 + 4` держались на одной строке независимо от DOM-порядка.
- **Progress / reminder:**
  - Desktop-круг streak в блоке «Сегодня выполнено» возвращён; внутренние отступы и вертикальное положение поправлены, чтобы круг не был прижат к краям.
  - Текст `дней подряд` разбит на две строки и удерживается внутри круга; на mobile правая колонка стала компактнее и не создаёт горизонтальный overflow.
  - `next_reminder` на desktop выровнен по высоте с progress (`lg:h-[150px]`), третья строка описания скрыта на `lg`, CTA остался компактным.
- **Mood:**
  - Убрана строка «Оцените своё состояние» и дефолтный статус «Выберите оценку от 1 до 5.» до действия пользователя.
  - Блок сделан ниже и плотнее (`h-[132px] sm:h-[136px] lg:h-[136px]`); линия emotion-иконок чуть опущена от заголовка.
  - Иконки эмоций увеличены до заполнения круглого слота, border стал тонким и бледным в цвет иконки; hover-border усилен до `*-400`; цифры под иконками опущены ниже.
- **SOS:**
  - Вся карточка больше не является ссылкой и не двигается при hover; кликабельна только CTA-кнопка.
  - Copy зафиксирован как `Если болит сейчас` + `Рекомендации по облегчению боли`.
  - CTA перенесён вправо и ниже к нижнему краю, текст заменён на `Быстрая помощь`, цвет сделан темнее и спокойнее, без прежней яркой красной кнопки.
- **План реабилитации:**
  - `Мой план` переименован в `Мой план реабилитации`.
  - В empty-state убрана фраза `Нет активного плана`; подпись стала `Назначит специалист или выберите готовую программу`.
  - CTA `К программам лечения` заменён на `Выбрать курс`, перенесён вправо и ниже; активный план сохранил CTA `Смотреть план`.
  - Leading-icon и заголовок выровнены по ритму соседних карточек, иконки в блоках сделаны круглыми и одинакового визуального размера.
- **Booking / subscription / courses:**
  - Кнопки booking выровнены симметрично: равная ширина, больше расстояние между кнопками, отступы слева/справа согласованы с карточкой.
  - `subscription_carousel` на desktop занимает всю ширину; одна карточка может растягиваться на всю строку.
  - Empty-state `courses` больше не рендерится на главной, чтобы не добавлять пустой шум внизу.
- **Общие визуальные токены:**
  - Радиусы карточек, hero и бейджей уменьшены примерно вдвое через `--patient-card-radius-*`, `--patient-hero-radius-*`, `--patient-pill-radius`; кнопки не трогались.
  - Для заголовков блоков добавлены токены `--patient-block-heading-font-size`, `--patient-block-heading-line-height`, `--patient-block-heading-font-weight`; `patientHomeBlockHeadingClass` читает size/line-height/weight из них.
- **Открытая проблема — шрифты/заголовки:** визуальное несоответствие заголовков `Как вы себя чувствуете?`, `Выберите пользу для себя:`, `Материалы по подписке` **не закрыто**. Текущий фикс централизовал числовые параметры через CSS-переменные, но не доказал единый фактический font-family/rendering во всех контекстах. Проблема переносится в следующий чат как отдельная задача: проверить computed styles в браузере, font inheritance и различие `h2` внутри карточки vs section-heading вне карточки.
- **Проверки:** `pnpm --filter @bersoncare/webapp lint` — exit 0 после токенизации заголовков; targeted `vitest` по `PatientHomeMoodCheckin`, `PatientHomeSosCard`, `PatientHomePlanCard`, `PatientHomeToday` — **4 files / 14 tests passed**. Полный webapp suite отдельно не прогонялся после финальных micro-polish правок.
...
- **Параллельные косяки (зафиксированы как наблюдения, без правок в этом проходе):**
  ...
  3. **Mood vs Plan высоты на lg:** при `lg:items-stretch` карточки выравнивались по строке, но содержимое внутри было визуально неравновесным. Позже в волне 2026-05-01 это решено compact-рядом `mood/sos/plan`, удалением empty-state title и переносом CTA плана вниз вправо.
...
- **Диалог элементов блока:** при каждом **открытии** снова подставляются `initialItems` с сервера (сброс удалений/ошибок), чтобы не показывать устаревшие строки после сохранения или `refresh`.
- **Copy для `useful_post`:** в описании диалога «Изменить элементы» — напоминание про порядок материалов (приоритет карточки), бейдж «Новый пост» и текст заголовка на обложке.
...
- План Phase 4.5 отмечен выполненным: в репозитории [`.cursor/plans/archive/phase_4.5_patient_home_a2e6bd38.plan.md`](../../../../.cursor/plans/archive/phase_4.5_patient_home_a2e6bd38.plan.md) (`phaseStatus: completed`, todos `completed`); тот же frontmatter синхронизирован с файлом плана в каталоге Cursor пользователя (`phase_4.5_patient_home_a2e6bd38.plan.md`).
