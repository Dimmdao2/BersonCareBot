...
| Смена только `title` | `updateInstance`                                       | _нет события_                        | **Defer** §8 — не требуется эталоном                    |
...
| Назначение программы | `assignTemplateToPatient`                              | _нет_                                | В §8 нет типа «создан экземпляр» — ожидаемо             |
