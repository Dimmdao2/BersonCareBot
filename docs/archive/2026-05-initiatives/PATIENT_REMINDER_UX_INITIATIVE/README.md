...
- **«Выполнить» в боте:** кнопка `rem_done` (подпись «Выполнить») → webapp `POST .../reminders/occurrences/done`; после успеха исходное сообщение напоминания **удаляется**; при закрытии всех **sent**-напоминаний за локальный день доставки (зона `app_display_timezone`) — отдельное сообщение по шаблону **`reminder.dayAllDone`**. Подробно: [`apps/webapp/src/modules/reminders/reminders.md`](../../../../apps/webapp/src/modules/reminders/reminders.md) §«Бот».
- **Открытие из бота (primary CTA, 2026-05-17; pick разминки уточнён 2026-05-28):** при `reminder_intent` **`warmup`** / **`exercises`** / **`stretch`** целевой URL — `/app/patient/go/daily-warmup?from=reminder` и `/app/patient/go/plan-start-lesson` (RSC-редирект). **Разминка:** deeplink с `from=reminder` ведёт на **следующую** после текущей главной страницу (`push_reminder` pick); CTA с главной — **home** pick (presented / последняя выполненная). Текст push — та же «следующая» разминка (`loadWarmupPushDynamicContext`). Ротация presented после просмотра видео — [`patient-home.md`](../../../../apps/webapp/src/modules/patient-home/patient-home.md). Интегратор: `buildPatientReminderDeepLink` + приоритет над кастомным `deepLink` для intent warmup/exercises/stretch; подпись разминки — «Начать разминку». Webapp: префикс `/app/patient/go/` в `patientRouteApiPolicy`.
