...
- **Открытие из бота (primary CTA, 2026-05-17):** при `reminder_intent` **`warmup`** / **`exercises`** / **`stretch`** целевой URL — `/app/patient/go/daily-warmup` и `/app/patient/go/plan-start-lesson` (RSC-редирект на актуальную разминку дня и на «Начать занятие» по плану, как CTA на главной). Интегратор: `buildPatientReminderDeepLink` + приоритет над кастомным `deepLink` правила для этих intent; подпись кнопки разминки — «Начать разминку». Webapp: префикс `/app/patient/go/` в `patientRouteApiPolicy` (`need_activation` и список без bind-phone в legacy snapshot).
