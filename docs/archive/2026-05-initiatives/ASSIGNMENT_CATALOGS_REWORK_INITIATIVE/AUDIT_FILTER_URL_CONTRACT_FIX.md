...
**Фактическая реализация (ветка):** diff от merge-base `origin/main`…`HEAD`, с фокусом на коммит **`db5ba9ca`** (`fix(doctor-catalogs): region URL code-only and client-side list filters`) и связанные файлы в `apps/webapp` + `docs/archive/2026-05-initiatives/ASSIGNMENT_CATALOGS_REWORK_INITIATIVE/LOG.md`.
