...
1. [x] Pre-check после завершения A зафиксирован в LOG.
2. [x] Список шаблонов: превью/счётчики по ТЗ (иконка + счётчики + бейдж; см. [`AUDIT_STAGE_B6.md`](AUDIT_STAGE_B6.md) §10 про миниатюру).
3. [x] Конструктор: layout + CTA + статус-бейдж.
4. [x] Модалка библиотеки: превью для exercise/lfk_complex/test_set/recommendation/lesson.
5. [x] `editLocked` bug проверен; если есть — исправлен точечно.
6. [x] Существующие тесты конструктора зелёные; добавить тесты под новые куски UI по необходимости.
7. [x] Smoke: черновик → опубликовать → архив (паритет списка после publish — unit; полный ручной/E2E — deferred, см. AUDIT §8/§10).
