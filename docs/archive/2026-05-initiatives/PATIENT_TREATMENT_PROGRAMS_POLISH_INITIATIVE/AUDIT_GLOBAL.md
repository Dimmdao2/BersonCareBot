...
История ветки `feature/app-restructure-initiative` содержит смежные коммиты по treatment program; **точный `git diff` до merge-base с `main` в этом аудите не использовался как единственный источник правды** — первичная правда по закрытию этапов: **журнал + AUDIT*STAGE*\***.
