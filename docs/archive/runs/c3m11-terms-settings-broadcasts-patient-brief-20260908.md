...
Continuation after an owner-requested stop: preserve and finish the existing uncommitted changes in this worktree. The platform has exactly one public terminology resolver, `resolvePatientTerms(patientLabel, supportGroupLabel)`. Do not add or call any resolver from page templates. React components obtain ready values only from `useDoctorPatientTerms()` or `usePatientTerms()` and render those fields. Finish this bounded list, validate without full CI, and commit all task changes before exiting.
