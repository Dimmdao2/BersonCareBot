...
## Дополнение: master-detail экран справочников (2026-04-16)

Выполнено дополнительное обновление UX и технической части справочников.

- UI переведен в master-detail:
  - слева постоянный список категорий;
  - справа таблица значений выбранной категории.
- Перенесено на пакетное сохранение:
  - одна кнопка `Сохранить справочник`;
  - сохраняются все изменения текущего справочника (порядок, правки, новые строки).
- Порядок строк:
  - drag-and-drop через `@dnd-kit`;
  - колонка `#` показывает актуальный порядок.
- Архивация:
  - действие в `...` в конце строки;
  - фильтр в двух режимах: `Без архивных` / `Показать архив` (только архивные).
- Backend/domain:
  - в `ReferencesPort` добавлен bulk-метод `saveCatalog`;
  - реализован в `pgReferences` (транзакционно) и `inMemoryReferences`;
  - в `pgReferences.saveCatalog` добавлена строгая проверка обновляемых строк (`rowCount === 1`), чтобы не проглатывать несоответствия.
- Cache/revalidation:
  - сохранена инвалидация `revalidatePath` для списка и выбранной категории;
  - сохранен `ReferenceCacheBuster` для сброса клиентского кэша справочников в формах.
