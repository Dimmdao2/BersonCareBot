# АРХИВ: разбор магазина упражнений, 05.08 — 11.09.2026

⛔ **Это НЕ план и не канон.** Действующий план — [`docs/_TODO/SAAS_FOUNDATION/EXERCISE_STORE_PLAN.md`](../../_TODO/SAAS_FOUNDATION/EXERCISE_STORE_PLAN.md).

Файл сохранён только как след обсуждения: варианты, которые рассматривались и были отменены, вопросы
владельцу и ответы на них, два поколения нумерации этапов. Все действующие решения и все замеры, на которых
они стоят, перенесены в план. Исполнять отсюда нельзя ничего.
...
⛔ **Бессрочный доступ (perpetual) исключён владельцем 11.09.2026, §3c.** Ниже он ещё разобран как
вариант — читать только как разбор мировой практики, не как проектное решение. То же касается упоминаний
perpetual в §2.9, этапах 3, 4 и 7.
...
⛔ **Отменено §3d (11.09.2026): кабинет автора — отдельный поддомен и своя папка, а не зона `/app/author/**`
внутри приложения.** Переключателя кабинетов нет ни в каком виде.
...
⛔ **Отменено §3c (10–11.09.2026): второго тумблера нет, ключ `platform_base_packs` не заводится.**
Остаётся один существующий ключ `exercise_catalog`; `exercise_packages` — сирота, см. §3i.
...
⛔ **Отменено §3h/§3i (11.09.2026): `owner_kind='author'` и передача владения НЕ вводятся.** Строка
остаётся у автора, публикация — состояние витрины плюс `author_id`, доступ покупателя — предикат прав.
...
⛔ **Часть про perpetual отменена (§3c).** Механика лестницы и revoke остаётся в силе, но с правилом §3f:
уже назначенное **доживает** — см. §3j про снимок.
...
> ⛔ **ЭТОТ ОТВЕТ ОТМЕНЁН ВЛАДЕЛЬЦЕМ 10.09.2026.** Дословно: «Устарел этот план — выключатель каталога. Если он
> у нас уже есть, значит пусть будет, то есть в тарифе я его могу включить или выключить. Это хорошо… не надо
> его удалять, ни в коем случае. Эта механика пускай остаётся. Если я там включил в тарифе, тогда всё, что
> связано с упражнениями, становится доступным».
>
> **Действующая модель:** `exercise_catalog` — рубильник ВСЕГО раздела упражнений и программ (выключен → у
> клиники раздела нет вообще, «тариф на запись и заметки»; включён → свои упражнения, комплексы, шаблоны
> программ, рекомендации, тесты). Общая библиотека платформы — это НАБОРЫ, и у каждого набора два независимых
> способа попасть к клинике: он приложен к тарифу (входит без доплаты) и/или у него есть цена (докупается
> аддоном, как пакет объёма). Второго тумблера «общая библиотека» не вводим. Ниже — исторический текст 05.08.

**Ответ (по смыслу Б, уточнение владельца) — ИСТОРИЯ, не исполнять:** в тарифе **не должно быть** выключателя «каталог» — каталог упражнений/комплексов/… это **базовая механика платформы**, всегда доступна для своих (clinic-owned) элементов.  
...
⛔ **Ответ устарел: метка владельца «автор» не вводится (§3h/§3i).**
...
## 3c. Поправка владельца 10–11.09.2026 — действующая коммерческая модель

Записано по итогам разбора «тариф или аддон». Это ДЕЙСТВУЮЩАЯ модель; §3 «Вопрос 3» и часть этапа 2 ею
отменены (перечёркнуто на месте).

1. **Тумблер `exercise_catalog` остаётся** и решает одно: есть у клиники раздел упражнений и программ или
   нет. Выключен — тариф «только запись и заметки». Включён — свои упражнения, комплексы, шаблоны программ,
   рекомендации, тесты **и стартовая база платформы сразу**. Второго тумблера «общая библиотека» нет:
   отдельного тарифа «упражнения без нашей базы» владелец не хочет — пустой старт не продукт.
2. **Развилка «тариф или аддон» решается тем, ЧЕЙ контент, а не тумблером.** Наш контент входит в тариф
   (он не стоит нам ничего на каждой новой продаже и отличает тарифы друг от друга). Чужой контент —
   только покупка. Одна и та же вещь никогда не бывает одновременно плюшкой тарифа и аддоном: клинике
   дешевле перейти на тариф выше, чем докупать к младшему.
3. **Аддонов ровно два вида:** место под файлы (уже построено) и авторские наборы.
4. **Комиссия платформы — ПЛОСКАЯ.** Владелец 11.09: она покрывает наши постоянные расходы — хранение
   видео, трафик, биллинг, — а не маркетинг. Поэтому схема «меньший процент, если покупателя привёл сам
   автор» (TrainHeroic) отклонена: набор с одним покупателем при ней уходит в минус. Конкретный процент —
   открытый вопрос; отраслевая норма плоской ставки 30% (Apple, Google, Steam, Unity).
5. **Экономический смысл магазина для нас — не комиссия, а привлечение.** Владелец 11.09: «процент с его
   магазина — копейки на покрытие расходов, основная выгода в привлечении через него подписчиков на
   основной пакет». Следствия для проектирования: автору нужны витрина набора со своей ссылкой и
   статистика продаж (иначе ему нечем приводить аудиторию), цену набора назначает автор, а наша база
   держится намеренно скромной и общеупотребительной, чтобы не конкурировать с авторами.
6. **Магазин строим, но включение — отдельное решение владельца** («магазин пусть будет, но включать его
   или нет будем по ходу обсуждений»). Значит обратимость эксперимента — требование к архитектуре, а не
   пожелание: без копирования файлов, доступ грантом с провенансом, деньги снимком в строке заказа.
8. **Процент комиссии НЕ хардкодится нигде** (владелец 11.09): это меняемое значение, цифра назначается
   при финансовом расчёте. Ставка плоская для всех авторов. Цена набора — с рамками платформы (стандартная
   цена либо зависящая от объёма/известности), автор может назначать свою внутри рамок.
9. **Спящий набор: платы за хранение с автора НЕ берём** (владелец 11.09: «захоронение брать вообще нет
   смысла»). Собираем статистику; не отбивается за назначенный срок — набор выгружается автору и снимается
   с полки. Если автор хочет держать те же видео себе, он делает это как специалист и платит за объём
   диска, как все: кабинет автора (отдельный поддомен) и кабинет специалиста — разные режимы с разными
   расчётами, видео при этом те же самые.
10. **Только подписка.** «Бессрочный доступ» (perpetual) из модели исключён: подписка на набор — всегда
    подписка.
11. **Пилот — 1–3 автора по договору**, кандидаты у владельца есть. Публичная оферта и автоматические
    выплаты — только если наборы начнут покупать.
12. **ОТКРЫТАЯ РАЗВИЛКА (главная по деньгам, 11.09): поштучные подписки на наборы vs одна подписка на весь
    авторский пул.** Наблюдение владельца: пять наборов по отдельности (колено + шея + плечо + …) дают
    ~2000 ₽/мес против ~500 ₽/мес у российских библиотек упражнений без сопровождения — поштучная продажа
    проигрывает арифметике, а не качеству. Идея владельца: продавать доступ ко всем авторским наборам, а
    авторам платить по статистике использования; его же опасение — «как не оказаться в убытке».
    **Ответ на опасение (проверенная механика, Envato Elements):** платформа отдаёт в фонд ФИКСИРОВАННУЮ
    долю собранной выручки и делит её между авторами, чей контент использовал КАЖДЫЙ конкретный подписчик
    (subscriber share, user-centric). Убыток исключён конструкцией: делится только собранное, наша доля —
    процент. **Чего делать нельзя:** платить автору фиксированную ставку за использование — на этом
    сломался Scribd/Everand (расход растёт с использованием, выручка нет → отмена unlimited, кредитная
    модель, троттлинг, отток). **Что стоит скопировать дополнительно:** порог попадания в распределение
    (Spotify с 2024 — 1000 прослушиваний за 12 мес + минимум уникальных слушателей против накрутки); у нас
    это N разных пациентов у M разных специалистов, и это же — критерий «спящего набора» из п.9.
    Развилка влияет на схему данных (подписка на пул vs подписка на набор, учёт использования по
    назначениям, распределение), поэтому закрывается ДО этапа 4.
13. **Риск пересъёмки набора клиникой** (владелец 11.09). Технической защиты нет и не будет; лечится
    экономикой: пул-подписка не растёт от числа наборов, поэтому отписываться незачем; правило «уже
    назначенное доживает» ОБЯЗАНО быть ограничено концом текущей программы (иначе дыра: назначить всем
    пациентам и отписаться); водяной знак клиники — маркер перезалива, не защита; авторский набор ценен
    обновлениями методики, а переснявший получает срез одного дня.

7. **Сравнение с зарубежными аналогами — про механику расчётов, не про оценку спроса.** Владелец 11.09:
   в российском пространстве такого игрока нет, зарубежные недоступны по языку и по платежам. Спрос
   измеряется на своих клиниках; выручка Physitrack (€13,5M ≈ 1,3 млрд ₽) читается как «одному игроку
   этой механики хватает на публичную компанию», а не как «ниша мелкая».
...
✅ **ЗАКРЫТО 11.09.2026 (§3j): снимок оставляем.** Замер: `pgTreatmentProgramItemSnapshot.ts:143-210`
копирует карточку в строку назначения один раз, пациент читает снимок. Клиническая история задним числом
не переписывается; обновление активных назначений — отдельная явная операция, не молчаливая.
...
## 3k. Соответствие двух нумераций этапов

В файле живут две шкалы: старая «Этап 0–7» (§4, написана 30.07–05.08) и действующая «S0–S8» (§3g–§3j,
11.09). Приёмка идёт по **S**; старая шкала остаётся как источник чек-листов поведения, не как порядок работ.

| Старое | Действующее | Примечание |
| --- | --- | --- |
| Этап 0 — reality lock | — | выполнен, замеры внизу §3d–§3i свежее |
| Этап 1 — ownership на пяти каталогах | ⛔ отменён | `owner_kind='author'` не вводится (§3i) |
| — | **S0** — чтение платформенного/магазинного слоя | новый, без него витрина пуста |
| Этап 3 (invite, кабинет) | **S1 + S2** | поддомен вместо зоны в приложении |
| Этап 2 (pack core) | **S3** | публикация как состояние витрины |
| Этап 3 (модерация) | **S4** | плюс модерация изменения и удаления |
| Этап 5 (каталоги, фильтр) | **S5** | фильтр библиотек — мультивыбор (§3h) |
| Этап 4 (billing, покупка) | **S6** | аддон по рельсу пакетов объёма |
| Этап 6 (начисления, кабинет) | **S7 + S8** | доли — данные админа, не константы |
| Этап 7 — launch gate | после S8 | живая приёмка владельцем |

Ссылки вида «до этапа 3/4/5» в §3b и §3c читать по этой таблице.
...
⛔ **Этап в этом виде отменён (§3h/§3i): он строит `owner_kind='author'` на пяти таблицах — модель, которой
больше нет.** Живая замена — S0 (чтение платформенного слоя) и S1 (автор как личность) в §3g/§3i.
...
- [ ] ⛔ ОТМЕНЕНО владельцем 10.09.2026 — ~~Tariff registry: один boolean `platform_base_packs`; личные каталоги
      always available~~. См. «Поправка владельца 10.09.2026» ниже: `exercise_catalog` ОСТАЁТСЯ рубильником
      раздела упражнений, а входящие в тариф наборы выбираются в самом тарифе.
- [ ] ⛔ ОТМЕНЕНО владельцем 10.09.2026 — ~~Старые `exercise_catalog`/`exercise_packages` удалены из
      constructor/resolver/callsites~~. Дословно: «не надо его удалять, ни в коем случае. Эта механика пускай
      остаётся». Остаётся требование: raw platform item не виден клинике, пока не входит в доступный набор.
- [ ] Base access вычисляется resolver динамически, без per-org×pack fan-out rows.
- [ ] ⚠️ ИЗМЕНЕНО 10.09.2026 — какие наборы входят в тариф, выбирается в конструкторе тарифа (не «все базовые
      или никакие»); новый набор сам собой в существующий тариф не въезжает.
...
⛔ **Строка ниже противоречит действующему правилу §3f/§3j «уже назначенное доживает».** Приёмку вести по
§3j: revoke прекращает НОВЫЕ назначения, активные доживают; текст карточки уже в снимке, видео живёт по
гранту со сроком.
...
## 3d. Кабинет автора — постановка владельца 11.09.2026

Владелец назвал скоуп первого этапа магазина дословно. Фиксирую как требования, а не как пересказ.

### Вход и личность автора

- **Отдельный поддомен**: «нужен домен автора, то есть какой-нибудь домен там `author.therapista.ru`.
  Это я сделаю». Домен заводит владелец, наше дело — маршрутизация и дерево страниц на этом хосте.
- **Автор не обязан быть клиникой или специалистом на подписке**: «совершенно не обязательно, что этот
  автор должен быть клиникой, там, специалистом на подписке — вообще нет».
- **Доступ выдаёт только владелец вручную**: «я только сам буду выдавать этот доступ, конечно же».
  Саморегистрации автора нет — иначе через роль автора получат бесплатный вход в приложение.
- Открытый вопрос владельца: «Нужна ли нам отдельная учётка? Я, кстати, не знаю. Или нам нужен новый вид
  аккаунта, типа автор… может быть, нужно, чтобы было переключение между автором и приложением».

⛔ **МОЙ ОТВЕТ ПРО ПЕРЕКЛЮЧАТЕЛЬ ОТМЕНЁН ВЛАДЕЛЬЦЕМ 11.09.2026.** Я предложил роль автора на существующей
личности плюс переключатель «Автор / Кабинет». Отклонено категорически, дословно: «Не надо делать
переключатель, авторский кабинет. Нельзя так делать. Вообще категорически… никаких переключений не надо ни
в коем случае трогать текущий интерфейс… Отдельный поддомен, отдельная платформа. Автор точка тераписта
точка ру. Всё… Максимум ссылку добавить для автора в кабинете, если она у него там есть. А вообще нахуй-то
не надо».

**Действующее решение владельца:**

1. **Отдельный поддомен = отдельный вход и отдельная поверхность.** `author.therapista.ru` — своё дерево
   страниц, **своя папка** в коде. Докторский интерфейс не трогаем вообще, новых элементов в него не
   добавляем.
2. **Переключателя кабинетов НЕТ.** Ни в каком виде. Максимум — ссылка на авторский поддомен в кабинете у
   того, кто действительно автор, и даже она под вопросом («а вообще не надо»). По умолчанию — не делаем.
3. **Учётная запись может быть та же.** Не вторая личность и не второй пароль — но и не переключатель:
   автор просто заходит своим логином на авторский поддомен, и там его пускает право «автор». Одна
   личность, два хоста, две поверхности.
4. **Авторов добавляет только глобальный админ.** Саморегистрации нет ни в каком виде.
5. **Переиспользуем максимально — через общие примитивы, а не через общие страницы.** Владелец: «мы можем
   использовать докторскую библиотеку, и это будет нормально, но пусть это будет общими примитивами…
   стиль внешне будет у него другой, может отличаться — другие цвета, другие формы, другие радиусы,
   глобальная вёрстка всё то же самое… пусть это будет общим пользованием элементов, связанных с
   библиотеками».
   То есть: **элементы работы с библиотекой упражнений — общие; страницы и оформление — свои.**
6. Замечание владельца на будущее: автору критичнее инструменты массовой загрузки и управления большим
   числом упражнений — «хотя, с другой стороны, это и специалистам нужно будет». Значит массовые
   инструменты строим в общих элементах, чтобы потом достались обоим, а не переписывались дважды.

### Что автор видит и делает в своём кабинете

- **Аналитика по своим упражнениям за период**: сколько назначений сделано из его набора.
- **Сколько специалистов в системе приобрели доступ к магазину** (общее число подписчиков магазина).
- **Его доля суммарно** — деньги за период.
- **Доли других авторов не видит**: «доля других там он видеть не должен, то есть он видит только по
  своим калькуляциям».
- **Загрузка новых упражнений и изменение существующих** — и то и другое уходит на модерацию.

### Модерация — обязательна, автор не публикует сам

Владелец: «сам по себе автор без модерации администратора платформенного не решает, подойдёт это
упражнение, не подойдёт. То есть я должен сохранять качество контента… просматривать все эти видео хотя бы
поверхностно, видеть что добавлено, и утверждать».

Требование: **и новая загрузка, и изменение существующего проходят рассмотрение.** Модерация только на
создание — дыра: одобренное упражнение можно потом подменить другим видео.

### Что видит глобальный администратор

Отдельная вкладка «Магазин упражнений» в настройках платформы:

- список авторов и число упражнений у каждого;
- открыть любое упражнение автора и посмотреть его целиком;
- принять или отклонить загрузку и изменение;
- вся биллинговая статистика по магазину.

### Что из этого в системе уже есть, а что придётся заводить

| Нужно | Состояние |
|---|---|
| Маршрутизация по поддомену | **есть** — шов брендированных хостов |
| Роль/возможность на личности + выдача админом | **есть** — модель возможностей арендатора |
| Переключатель контекста в интерфейсе | **есть** как образец — режим администратора |
| Владелец упражнения = автор | **частично**: `owner_kind` знает `platform` и `organization`, автора — нет |
| Сущность автора (реквизиты, условия, доля) | **нет** |
| Очередь модерации (заявка → решение → кто и когда) | **нет** |
| Статистика назначений по автору | **нет** |
| Расчёт выплат за период | **нет** |

### Деньги — данные, а не скоуп разработки (владелец 11.09.2026)

Цена подписки, размер фонда, любые проценты — **коммерческие решения владельца, и они ничего в разработке
не держат**. Экраны и расчёт строятся так, что цена и процент вводятся в интерфейсе платформенного админа и
лежат в базе; кода, который ждёт этих чисел, не существует. Разработке для работы нужна только форма ввода и
формула «сумма × процент», а не значения.

## 3e. Сторона покупателя и мультитип нагрузки — постановка владельца 11.09.2026

### Покупка доступа к магазину

- **Цену пакета библиотеки ставит платформенный администратор** — как и у пакетов объёма, цена это
  админ-данные, а не константа в коде.
- **Специалист покупает аддон сам** из своего кабинета. Рельс для этого уже построен и обкатан на объёме:
  `saas_storage_packages` + цены за период подписки + `decideStoragePackagePurchase` с пропорцией от
  момента покупки до конца оплаченного периода. Магазин — второй потребитель той же двери, новой денежной
  формулы не требуется.
- ⚠️ Известное ограничение: счёт сегодня «шапка на вид» (`tariff_period | seat_overage |
  storage_package`), позиций в нём нет. Аддон магазина — четвёртый вид, и если он может попасть в один
  счёт с другими, понадобятся позиции счёта.

### Витрина: видно автора, фильтр по автору

Владелец: «человек видит автора, может фильтровать по авторам… фильтровать по всем остальным — упражнениям
для спины, там ещё чего-то».

- В карточке упражнения из магазина **виден автор** — это и есть витрина автора, ради которой он вообще
  приходит (см. §3c: ценность магазина для автора — привлечение подписчиков).
- Фильтр по автору встаёт рядом с существующими фильтрами каталога (регион, тип нагрузки и прочие).

### Тип нагрузки становится мультиполем (отдельная правка, «сразу по ходу работы»)

Владелец дословно: «у нас сейчас можно выбрать несколько регионов на упражнение, но только один тип
нагрузки. Тип нагрузки должен стать мультиполем… упражнение может быть одновременно силовая и статика,
может быть силовая эксцентрика и статика, может быть нейродинамика и мобилизации, нейродинамика и
растяжки, стабилизация и силовая». И про интерфейс: «при создании или изменении упражнения этот тип должен
быть поле такое же, как регион, то есть тоже мультивыбор… просто сам элемент надо скопировать, так как он
работает для региона. А в интерфейсе выбора упражнения этого неважно, там нечего менять».

Замер текущего состояния:

| | Регион | Тип нагрузки |
|---|---|---|
| Хранение | связь многие-ко-многим `lfk_exercise_regions` (330 связей на 151 упражнение, ≈2,2 на упражнение) **плюс** одиночный `lfk_exercises.region_ref_id` (заполнен у 134) | **одиночная колонка** `lfk_exercises.load_type` (text) |
| Значений в ходу | — | 10 (`strength` 43, `high_rep` 42, `mobilization` 14, `stretch` 10, `static_hold` 7, `eccentric` 5, `balance` 5, `neurodinamica` 3, `statodynamic` 2, `plyometric` 1); у 19 упражнений пусто |

Значит работа: связь многие-ко-многим по образцу регионов + перенос существующих 132 значений в связи +
мультивыбор в форме создания и изменения упражнения (тот же элемент, что у региона) + фильтр каталога
учитывает несколько типов. Интерфейс выбора упражнения врачом не меняется.

⚠️ Решение по форме хранения: повторяю форму регионов **включая** сохранение одиночной колонки как
основного типа. Не потому что так красивее, а потому что регионы уже так живут, и читающих мест много —
разойдись эти два поля по устройству, каждое такое место придётся разбирать отдельно.

## 3f. Уточнения владельца 11.09.2026 — что у автора СВОЁ, а что общее

### Создание и изменение элементов — одинаково у доктора и у автора

Владелец дословно: «надо сделать так, чтобы изменения карточки и механизмы создания изменялись и у
доктора, и у автора одинаково, потому что любой доктор, который использует приложение, сам для себя будет
автором. То есть если он загружает свои упражнения, ему точно так же нужны удобные механизмы. Поэтому
здесь их не надо ограничивать».

**Следствие:** механика создания и редактирования карточки — ОДНА для обоих. Улучшили автору — улучшилось
доктору, и наоборот. Никаких «авторских» полей и никакой урезанной версии для доктора.

**У автора добавляется ровно две вещи: статистика и модерация.** Больше ничего.

### Модерация — на создание, на изменение И на удаление

- Новая загрузка → на рассмотрение.
- Изменение существующего → на рассмотрение (владелец сказал это дважды: «модерация, естественно,
  добавляется на изменения. Я тебе это сказал прямо»).
- **Удаление → тоже запрос**, а не действие. Владелец: «удаление автор тоже запрос на удаление делает,
  если он хочет почистить какие-то упражнения, потому что мы должны посмотреть, используются ли они или
  нет. Я должен это принять, одобрить».
- **Что значит «удалено» по смыслу:** «самоудаление — это, в общем-то, отключение новых назначений и
  доживание всех уже назначенных». То есть упражнение снимается с витрины и больше не назначается, а у
  пациентов, которым оно уже назначено, продолжает работать до конца их программы. Совпадает с правилом
  отзыва доступа из §3c.

### Массовая загрузка — ОТЛОЖЕНА, не проектировать сейчас

Владелец: «с массовой загрузкой это я там, мы с тобой обсудим, как это сделать. Тут дохуя работы ещё до
этого момента». Мой пункт про «строим массовые инструменты в общих элементах» снят — ничего по массовой
загрузке в этот скоуп не входит.

Что владелец зафиксировал про неё на будущее, чтобы не потерять:
- **Сейчас путь такой:** человек загружает видео в библиотеку → переименовывает их → создаёт из них пакет.
  Владелец оценивает путь как «в целом достаточно нормальный».
- **Возможное улучшение:** убрать этап — автосоздание прямо из массовой загрузки, если файлы уже названы
  так, как должны называться элементы. Тогда загрузка сразу порождает элементы.
- **Создаваемых видов элемента три:** упражнения, тесты, рекомендации. Автосоздание должно уметь все три.

## 3g. Этапы реализации магазина — порядок и чек-лист (11.09.2026)

Порядок выбран по зависимостям, а не по важности: каждый следующий этап опирается на предыдущий, и на
каждом есть что показать владельцу живьём. Владелец 11.09: «дальше по дизайну будем разбираться, когда я
увижу, зайду, увижу под глобальным админом и под автором эту новую поверхность» — значит первые этапы
должны довести до состояния «можно зайти и посмотреть», а не до идеального оформления.

### S1. Автор как сущность и право на личности

- [ ] Таблица авторов: кто, публичное имя для витрины, реквизиты для выплат, условия (доля, период).
- [ ] Право «автор» на существующей платформенной личности. **Выдаёт только глобальный админ**,
  саморегистрации нет ни в каком виде.
- [ ] Экран у глобального админа: выдать/снять право, привязать личность к автору.
- Живая проверка: владелец выдаёт право своей учётке и видит автора в списке.

### S2. Поддомен автора — отдельный вход, отдельная поверхность

- [ ] Резолв хоста `author.<домен>` в шве брендированных хостов (тот же механизм, что у клиник, — не
  новая инфраструктура).
- [ ] Вход существующей учётной записью; **у кого нет права «автор» — отказ**, а не пустой кабинет.
- [ ] Своё дерево страниц в **своей папке**. Докторский интерфейс не меняется ни одной строкой,
  переключателя кабинетов НЕТ (см. §3d).
- Живая проверка: владелец заходит на поддомен своим логином и попадает в авторскую поверхность; вторая
  учётка без права получает отказ.

### S3. Упражнение принадлежит автору

- ⛔ ~~Третий владелец контента: нужен автор как `owner_kind`.~~ **Снято 11.09.2026, см. §3h** — автор не
  вид владельца; публикация переводит элемент в платформенный слой, автор остаётся атрибуцией.
- [ ] Публикация в магазин = перевод элемента в платформенный слой, **без копирования строки и объекта S3**.
- [ ] `author_id` как атрибуция и получатель доли на строке элемента.
- [ ] Чтение каталога учитывает автора наравне с платформой и организацией (флаг включения базы уже есть).
- ⚠️ Механика создания и изменения карточки — **общая с доктором** (§3f). Отдельной авторской формы нет.

### S4. Модерация — сердце этапа

- [ ] Очередь заявок с тремя видами: создание, изменение, **удаление** (владелец: «удаление автор тоже
  запрос на удаление делает… я должен это принять, одобрить»).
- [ ] Автор отправляет, а не публикует: до решения элемент не виден покупателям.
- [ ] Экран у глобального админа: список авторов, число упражнений, открыть элемент и посмотреть видео
  целиком, принять или отклонить.
- [ ] Одобренное удаление = снятие с витрины и прекращение новых назначений; **уже назначенное доживает
  до конца программы пациента** (то же правило, что при отзыве доступа, §3c).
- Живая проверка: владелец видит заявку, открывает видео, отклоняет и принимает.

### S5. Витрина для специалиста

- [ ] Автор виден в карточке упражнения из магазина — это и есть его витрина, ради которой он приходит.
- [ ] Фильтр по автору рядом с существующими фильтрами каталога.
- ⚠️ Зависит от мультитипа нагрузки (§3e), потому что фильтры каталога правятся одним заходом.

### S6. Продажа доступа к магазину

- [ ] Пакет магазина как аддон: **цену ставит платформенный админ**, это админ-данные.
- [ ] Покупка специалистом из своего кабинета по рельсу пакетов объёма (`decideStoragePackagePurchase` —
  пропорция от покупки до конца оплаченного периода). Новой денежной формулы не нужно.
- [ ] Лимит внутри аддона, а не в базовом тарифе: специалист без ЛФК магазин не покупает, значит лимит
  его не касается.
- ⚠️ Счёт сегодня «шапка на вид» без позиций. Если аддон магазина может попасть в один счёт с другими
  видами — понадобятся позиции счёта.

### S7. Статистика автора

- [ ] Назначения его упражнений за период.
- [ ] Сколько специалистов приобрели доступ к магазину.
- [ ] Его доля суммарно за период.
- [ ] **Долей других авторов не видит** — только свои калькуляции.

### S8. Биллинговая статистика магазина у глобального админа

- [ ] Выручка магазина, разбивка по авторам, начисления к выплате за период.
- ⚠️ Процент и цена — вводимые данные, не константы в коде: экран считает «сумма × процент», значения
  приходят из настроек платформенного админа.

### Ничего не ждёт решений по деньгам

Ни один этап S1–S8 не блокируется ценой, процентом или моделью выплат: это данные админа, см. конец §3d.
Никаких owner-gates по коммерции в этом плане нет и быть не должно.

### Явно НЕ входит

- Массовая загрузка и автосоздание элементов из неё — снято владельцем 11.09 («с массовой загрузкой мы с
  тобой обсудим, как это сделать; тут дохуя работы ещё до этого момента»).
- Переключатель кабинетов в любом виде.
- Любые изменения докторского экрана выбора упражнений.

## 3h. Постановка владельца 11.09.2026 (вечер): доступ другой клиники, фильтр слоёв, универсальная единица

### Что на самом деле значил «третий владелец» — и почему он не нужен

Мой ярлык «третий владелец рядом с платформой и организацией» был плохой. Владелец переформулировал
задачу точно: «если у нас все видео загружены одной клиникой и доступны только внутри этой клиники, то
вопрос как другим клиникам при покупке выдать разрешение на эти упражнения?»

Это и есть вся проблема, и решается она **без новой разновидности владельца**:

- Сегодня `owner_kind` знает `organization` и `platform`. Платформенный слой — это ровно «контент, который
  не принадлежит ни одной клинике и виден тем, кому положено»; читается он уже существующим флагом
  включения базы (`includePlatformBase` в `pgLfkExercises.ts`).
- Значит **публикация в магазин = перевод элемента в платформенный слой**, а автор — не вид владельца, а
  **атрибуция и получатель доли** (`author_id` на строке) плюс фильтр витрины.
- Пока автор не опубликовал — элемент остаётся `organization`, личным, и это дословно совпадает со словами
  владельца: «если я его библиотеку не сделал глобальный, магазинный, то он всё равно что другой автор,
  он зашел и создал себе библиотеку упражнений».
- Доступ клиники-покупателя — это не право на чужую организацию, а **entitlement на платформенный слой**:
  купил — видит опубликованное, не купил — не видит. Отзыв доступа уже описан (§3c): новые назначения
  прекращаются, назначенное доживает.

Следствие для S3: пункт «третий владелец контента» **снимается**. Вместо него — «публикация переводит
элемент в платформенный слой, `author_id` остаётся атрибуцией» и инвариант «строка и объект в S3 не
копируются» (он уже сформулирован в этапе 5 выше).

### Фильтр библиотек — мультивыбор по образцу филиалов (поправка владельца 11.09.2026, вечер)

⛔ Моя формулировка «автор виден в карточке только при купленной подписке» **неверна и отменена**. Владелец:
«Фильтр работает по принципу филиалов. То есть мы можем выбрать библиотеки, которые хотим видеть сейчас на
экране. Это мультивыбор. Вот точно так же открывается модалка, включаем галочками, как бы, либо все, либо
отдельные».

- [ ] Модалка фильтра библиотек — **мультивыбор галочками**, устройство копируется с фильтра филиалов.
      Строки: «платформенные», «собственные», и каждая приобретённая авторская библиотека.
- [ ] Платформенные всегда можно выключить и оставить только свои — и наоборот показывать всё.
- [ ] Одинаково для **упражнений, тестов и рекомендаций** — это не упражненческая фича.
- [ ] **Та же модалка — точка входа в покупку.** Владелец: «мы её открываем, как бы мы видим, что
      платформенные, собственные и приобрести библиотеку». Строка «приобрести библиотеку» ведёт на страницу
      библиотеки, где видны все авторы и их наборы упражнений.
- [ ] **Авторы как значения фильтра появляются после покупки** — не «скрыты до покупки», а до покупки их
      попросту нет в выборе, потому что нет самой библиотеки. Купил — все авторы этой библиотеки доступны
      для фильтрации и видны внутри неё.
- ✅ Дешёво по чтению: флаг `includePlatformBase` в чтении каталога уже существует; речь о том, чтобы
      вместо одного булева флага чтение принимало **набор выбранных библиотек**.

### Это витрина подписки, а не магазин пакетов

Владелец: «получается, что нам не нужен магазин библиотек, нам нужны витрины, как бы, которые просто будут
показывать, что входит в подписку на библиотеку».

- Один аддон = **вся библиотека целиком, все авторы внутри**. Поштучных покупок пакетов нет.
- Обоснование уже посчитано в §3e: продавать десять отдельных пакетов бессмысленно, объёма не хватает.
- Авторы разводятся **по узким сегментам** (колено, кисть, спина, плечо, таз и послеродовое) — это и есть
  ответ на «согласятся ли авторы»: они не конкурируют за один и тот же пакет.
- [ ] Поштучная покупка пакетов **в скоуп не входит**, но механизм выбора строится так, что её можно
      добавить позже без переделки: набор выбранных библиотек — это уже список, а не булев флаг.
- [ ] Где видна возможность купить: как минимум в этой модалке фильтров и в настройках тарифа.
      ⚠️ Владелец: «я пока не подумал ещё до конца, где это будет видно» — место показа уточняется на живой
      приёмке S5/S6, механика от этого не зависит.

### Универсальная единица с тегами — замер, а не мнение

Владелец: «нам надо было сделать универсальную единицу; упражнения, рекомендации, тесты должны были быть
универсальной единицей… а список значений, которые к ним подключаются… должно было быть просто тегами» —
и объяснил зачем: «мы сейчас можем добавить ещё какой-то тип библиотеки… по нутрициологии… или какие-то по
психологии упражнения. Что проще — нам отдельно новую сущность создать или переиспользовать существующую?»

Замер по DEV-схеме (`information_schema.columns`, 11.09.2026):

| | владение | слой каталога | классификатор региона | теги |
|---|---|---|---|---|
| `lfk_exercises` | `organization_id` + **`owner_kind`** | **`catalog_scope`** | `region_ref_id` + M2M `lfk_exercise_regions` (с `owner_kind`) | есть |
| `tests` | только `organization_id` | **нет** | `body_region_id` + M2M `clinical_test_regions` (без `owner_kind`) | есть |
| `recommendations` | только `organization_id` | **нет** | `body_region_id` + M2M `recommendation_regions` (без `owner_kind`) | есть |

Интуиция владельца подтверждается цифрами: **классификатор региона уже существует в трёх копиях с разными
именами колонок**, а слой владения и каталога есть только у упражнений. Если магазин, модерация и биллинг
написать против `lfk_exercises`, то для тестов и рекомендаций всё это придётся писать второй и третий раз.

**Но сливать три сущности в одну таблицу — не ответ.** Различаются они не метаданными, а поведением:
упражнение назначается с подходами и получает отметку выполнения; тест получает ответы и считает баллы
(`test_attempts`, `test_results`, `scoring`); рекомендация читается. Одна таблица на три поведения — это
три несовместимых группы NULL-колонок и `if (type === …)` в каждом экране.

**Ответ — разделить общее и особенное:** общий «хребет» библиотечного элемента (id, тип, владелец, слой
каталога, автор, статус модерации, заголовок, теги/классификаторы, медиа) + своя таблица полезной нагрузки
на каждый тип, которая ссылается на хребет. Тогда добавление нутрициологии или психологии = новая таблица
нагрузки, а магазин, модерация, авторство, витрина, фильтры и выплаты **не трогаются вообще**.

- [ ] Хребет вводится **до S3/S4/S6**, потому что это ровно тот код, который иначе пишется трижды.
  Цена сейчас: одна таблица + по одному FK на три существующие + бэкфилл. Цена потом: переписать магазин.
- [ ] Магазин, модерация и авторство с первого дня пишутся против идентичности хребта (тип + id), а не
  против `lfk_exercises`.
- [ ] Поведение (назначение, отметка, баллы теста) остаётся в своих таблицах и **в этом заходе не
  переписывается**.
- ⚠️ Сведение трёх таблиц регионов в один классификатор на хребте — отдельный этап: он трогает живые
  докторские экраны. В скоуп магазина не тащим, но новые фильтры магазина строим уже на хребте.

## 3i. Как устроить «общее и особенное», ничего не сломав — с ценой работы

### Сначала поправка: публикация НЕ переворачивает владельца строки

⛔ Написанное в §3h «публикация = перевод элемента в платформенный слой» **неверно по механизму** (аудит
Opus 11.09.2026 + собственная проверка). Цель остаётся — покупатель получает доступ без третьего владельца, —
но достигается она иначе. Почему переворот `owner_kind` нельзя:

1. **Автор теряет возможность править свою же карточку.** `update`/`archive` в `pgLfkExercises.ts:778-787`,
   `:868-880` требуют `organization_id = <моя>`; у платформенной строки `organization_id IS NULL` по CHECK.
   Это прямо ломает закон «механика правки одинакова у доктора и автора» (§3f).
2. **Элемент исчезает из личного каталога своей же клиники** — `list` берёт только свою организацию.
3. **Байты уходят из квоты автора.** `files_used` считается как сумма `media_files.size_bytes` по
   `organization_id` (миграция `20260910T160500`). Обнулили организацию — объём перестал считаться,
   против решения 10.09 «считаем всё, что загружено в аккаунт».
4. **Дерево разваливается молча.** Триггер `app.enforce_lfk_child_owner()` требует совпадения владельца у
   `lfk_exercise_media`, `lfk_exercise_regions` и `media_files`, а на родителе триггера нет: `UPDATE`
   владельца пройдёт, а регионы и видео отвалятся из выборок, где джойн идёт по `owner_kind`.

**Правильный механизм:** строка остаётся `owner_kind='organization'` у автора. Публикация — это
**состояние витрины** (`store_status`) и `author_id`, а доступ клиники-покупателя — **один предикат
entitlement**, который добавляется в тот же общий проход чтения. Третий `owner_kind` не вводится, и второй
смысл у существующего не перегружается.

### Отдельно: платформенный слой сегодня мёртв

Замер на DEV (снят с прод-дампа): `lfk_exercises` 151/151 и `media_files` 192/192 — `owner_kind='organization'`,
платформенных строк **ноль**. На `lfk_exercises` единственная читающая политика для `app_staff` —
`rev10_saas_org_dormant_p0_8_3`: `organization_id = app.current_org_id()`. Политики, пускающей платформенные
строки, нет; FORCE RLS включён, `bypassrls` ни у одной рантайм-роли.

Значит ветка `includePlatformBase` расширяет SQL, а RLS возвращает ноль строк — каталог и плеер разъехались
(видео играет через definer-шов `app.read_platform_media_row`). Обещание «включил тариф — получил базовую
библиотеку платформы» сегодня не работает **вообще**, и это надо закрыть до S5/S6, иначе витрина покажет
пустой экран.

- [ ] **S0. Чтение платформенного/магазинного слоя.** Политика на `lfk_exercises`, `lfk_exercise_media`,
  `lfk_exercise_regions` (+ `tests`/`recommendations`, если входят в v1) либо перевод чтения каталога на
  definer-шов. Цена: `deploy/postgres/privileges/declaration.ts` (3–7 мест) + регенерация
  `deploy/postgres/generated/privileges.*.sql`. **Без этого S3, S5 и S6 показывают пустой экран.**

### «Общее и особенное» — что именно общее

- **Общее** (одинаково для упражнения, теста, рекомендации и любого будущего типа): принадлежность
  магазину, автор, статус модерации, цена/доля, статистика назначений, права покупателя.
- **Особенное** (остаётся там, где лежит сейчас): поля карточки и **поведение** — упражнение назначается с
  подходами и получает отметку; тест получает ответы и считает баллы (`test_attempts`, `test_results`,
  `scoring`); рекомендация читается.

### Как сделать, не сломав существующее: одна полиморфная таблица вместо общего хребта

⛔ Тяжёлый вариант «общий хребет `library_items` + FK на три таблицы + бэкфилл» из §3h **отменён**: он
окупается только если хребет забирает себе **чтение**, а чтение — это ровно живые докторские экраны,
которые трогать запрещено. Платить пришлось бы новой идентичностью, тремя FK, бэкфиллом и двойной записью,
а три разных каталожных запроса всё равно остались бы.

**Делаем так:** одна новая таблица магазина/модерации с полиморфным ключом `(item_type, item_ref_id)`.
Существующие три таблицы **не трогаются вообще** — ни колонки, ни бэкфилла, ни докторских экранов.

Это не изобретение, а **уже действующий канон репозитория**: `treatment_program_instance_stage_items` несёт
`item_type CHECK IN ('exercise','recommendation','lesson','clinical_test')` с `item_ref_id` без FK и одним
центральным валидатором `apps/webapp/src/infra/repos/pgTreatmentProgramItemRefValidation.ts`.

- [ ] Магазин, модерация, авторство и выплаты с первого дня пишутся против `(item_type, item_ref_id)`,
      а не против `lfk_exercises`.
- [ ] Валидатор ссылок расширяется теми же тремя типами — один chokepoint, не три ветки.
- [ ] Нутрициология или психология потом = **новый тип в валидаторе + своя таблица нагрузки**; магазин,
      модерация, витрина, фильтры и выплаты не трогаются.

### Цена работы — замер, не оценка на глаз

| Что | Замер |
|---|---|
| Строк данных под риском | `lfk_exercises` 151, `lfk_exercise_regions` 330 |
| Тесты | `tests` **2**, `test_attempts` **0**, `test_results` **0**, `test_sets` 2, `clinical_test_regions` 0 |
| Рекомендации | `recommendations` **4**, `recommendation_regions` 0 |
| Где живёт SQL каждой сущности | по одному порту: `pgLfkExercises.ts`, `pgClinicalTests.ts`, `pgRecommendations.ts` |
| Сколько мест импортируют эти порты | 4 / 1 / 1 (без тестов) |
| Цена новой таблицы | миграция + `schema.ts` + 3–7 мест в `declaration.ts` + регенерация трёх артефактов прав |

Вывод: **тесты и рекомендации фактически пусты (6 строк вместе)** — это подтверждает слова владельца
«тесты у нас вообще ещё не живые». Приводить их к одному устройству дёшево именно сейчас, и делать это надо
не переписыванием, а тем, что новый общий слой к ним просто применяется.

- ⚠️ Что в этом заходе **не** делаем: не сводим три таблицы регионов, не меняем поведение назначения и
  отметок, не переписываем докторские экраны, не вводим общую таблицу идентичности элементов.

## 3j. Постановка владельца 11.09.2026 (поздний вечер): границы автора, старт без платформенных комплексов, порядок с тестами

### Что автор создаёт, а что собирает специалист

Владелец: «Автор может создавать упражнения и комплексы ЛФК из них, а программа уже собирает, как бы,
специалист под себя из разных авторов и так далее. Да, поэтому туда не лезем».

- [ ] Зона автора: **упражнения** и **комплексы ЛФК** (быстрые наборы для заполнения программ).
- ⛔ **Программы и шаблоны программ в зону автора НЕ входят** — их собирает специалист под себя, смешивая
  разных авторов. Магазин туда не лезет.
- Следствие: полиморфный ключ магазина (§3i) получает типы «упражнение», «комплекс» и позже «тест» и
  «рекомендация»; тип «программа» и «шаблон программы» в магазин не заводится вообще.

### Платформенных комплексов на старте нет — вместо них пример в новом аккаунте

Владелец: «на старте надо ли делать, но скорее всего нет, пусть будет всё пустое… можем при создании
аккаунта нового просто копировать ему… не делать какую-то платформенную механику для шаблонов программ, а
просто сделать механизм, чтобы ему пример заполнялся: несколько комплексов ЛФК, собраны из них шаблоны
программ, один-два, средне наполненных. Дальше он уже просто быстрее в этом разберётся и будет сам
работать. Наверное, можно вот так сделать».

- [ ] Платформенного слоя для **комплексов и шаблонов программ не заводить**. Пусто на старте.
- [ ] Вместо него — **разовое копирование примера в новый аккаунт при создании**: несколько комплексов ЛФК
      из уже доступных упражнений и один-два средне наполненных шаблона программы.
- ✅ Почему это дешевле платформенной механики: копия принадлежит клинике с первой секунды, значит не нужны
      ни политика чтения чужого слоя, ни грант, ни отзыв — специалист сразу правит пример как своё.
- ⚠️ Это **не** отменяет S0 (чтение платформенного слоя) — S0 нужен упражнениям магазина, где копировать
      контент нельзя по инварианту «строка и объект не копируются».

### Тесты можно доделать позже — переделывать не придётся

Владелец: «мы же можем сейчас всё вот это реализовать, а тесты, как бы, мы можем уже там доделать потом,
правильно?»

Да, и именно потому, что §3i вынес общий слой **наружу**, а не внутрь сущностей. Магазин, модерация,
авторство и выплаты держат `(тип элемента, id элемента)`; тест становится продаваемым в тот день, когда его
тип добавляют в валидатор ссылок — сами `tests` при этом не переписываются. Замер подтверждает дешевизну:
`tests` 2 строки, попыток 0, результатов 0.

### «Это же один юнит, правильно?» — да, но уже назначенный пациент правку НЕ увидит

Владелец: «Мы же сможем потом улучшать, изменять упражнения… Оно просто будет меняться для всех. Это же
один юнит, правильно? Главное, как бы, данные не убивать».

Проверено по коду, это ровно развилка D2 (§3b), и теперь она закрыта замером:

- **Карточка действительно одна.** Правка видна сразу в каталоге, в конструкторе и всем, кто назначит её
  после правки.
- **Но при назначении программа сохраняет снимок элемента.** `buildSnapshot('exercise', id)` в
  `apps/webapp/src/infra/repos/pgTreatmentProgramItemSnapshot.ts:143-210` копирует в строку назначения
  `title`, `description`, `contraindications`, `difficulty`, `loadType` и список медиа; пишется он один раз
  при назначении (`pgTreatmentProgramInstance.ts:1456,1470`), и пациент читает **снимок**, а не карточку.
- Значит правка **не догоняет** уже назначенного пациента.

**Решение D2 — снимок оставляем** (это и есть «данные не убивать»): клиническая история не должна
переписываться задним числом. Следствия, которые надо знать и которые идут в план:

- [ ] Исправление ошибки в карточке **не доедет** до пациентов, которым она уже назначена — если правка
      важная (противопоказание, техника), нужна отдельная явная операция «обновить снимок у активных
      назначений», а не молчаливое обновление.
- ✅ Побочный выигрыш для магазина: правило «уже назначенное доживает» для **текста карточки уже выполнено
      по построению** — он скопирован в снимок. Отдельно доживать должно только **видео**: его байты идут
      через авторизованную выдачу по id, поэтому именно там нужен грант с явным сроком.

### S0 подтверждён владельцем

«Платформенный слой сегодня мёртв… А, что у нас его, в принципе, ну, надо делать, конечно. Надо делать» —
этап S0 (§3i) принят в работу.
