...
- [ ] При открытии без скролла видно: кто, ближайшая запись, сопровождение, что горит, активный этап, сводка задач.
- [ ] Срочное собрано в Action Strip; каждый чип ведёт в нужное место.
- [ ] Лечение (программа + самочувствие) — на «Обзоре»; учётка/админка/история — в своих табах.
- [ ] Самочувствие: спарклайн в «Обзоре», полный график — по клику.
- [ ] Старые `#doctor-client-section-*` и `?chat=1` открывают правильный таб без 404 якоря.
- [ ] Mobile: чат, сопровождение, задачи доступны без горизонтального хаоса.
- [ ] Задачи: список в карточке + сводка невыполненных в Hero; напоминание приходит по выбранным каналам и не дублируется.
- [ ] Нет нарушений module isolation; новые сущности на Drizzle; конфиг каналов не в env.
