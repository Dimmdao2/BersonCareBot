...
- Полный клинический timeline с богатой семантикой событий (после 2B); cross-patient inbox на «Сегодня» (фаза 5); CMS/help (фаза 6). ~~Черновик редактора программы (фаза 3)~~ — **закрыта** 2026-06-02 (`treatment-program-shared/instanceEditorDraft*`). ~~Фильтры picker «добавить из библиотеки» (фаза 4)~~ — **закрыта** 2026-06-02 (`TreatmentProgramLibraryPickerToolbar`; без «Без региона»/«Без типа» — см. LOG §фаза 4).
