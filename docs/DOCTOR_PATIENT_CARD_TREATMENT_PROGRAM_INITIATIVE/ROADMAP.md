...
- **CTA «Назначить новое»** — selector из библиотеки (шаблонов).
...
| Группы | UI группировки: **drag-and-drop** items между группами; CRUD группы (`title`, `description`, `schedule_text`, `sort_order`) — этап A3. |
| Рекомендации в этапе | Toggle **actionable / persistent** при добавлении (по умолчанию actionable). |
| Элементы в инстансе | **«Отключить» / «Включить»** (иконка глаза) — не удаление из инстанса; удаление жёсткое только в шаблоне (§1.8 плана). |
...
### Этап 2 — Назначение из карточки: выбор шаблона → создание instance

- UI: **«Назначить новое»** → selector шаблонов (фильтры как в других каталогах врача по возможности).
- Backend: использование существующего пути создания instance (или расширение порта/сервиса без нарушения слоёв).
- После создания: редирект или inline-открытие **редактора инстанса** (см. этап 3).

**Критерий:** врач может назначить программу, не уходя в каталог шаблонов как в единственный путь.
...
1. Из карточки пациента врач может **назначить программу**, выбрав **опубликованный шаблон**, и получить работающий **`treatment_program_instance`**.
