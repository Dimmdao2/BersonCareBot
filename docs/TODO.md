...
- **Редактор текстов — Tiptap Simple Editor (не markdown, не AI-editor).** Замена markdown-редакторов **везде, где они стоят** (рассылки/CMS-статьи/рекомендации + discovery-sweep) — вариант 1 (markdown остаётся форматом хранения, миграции нет): план **[docs/_TODO/EDITOR_TIPTAP_MIGRATION_PLAN.md](_TODO/EDITOR_TIPTAP_MIGRATION_PLAN.md)**, taskdb **#931**. Исполнение — по решению основного оркестратора (план-only).
- **HTML-конверт транзакционных писем (не Tiptap).** Уведомления/напоминания о записи/OTP-коды: текст — как сейчас (простой шаблон), визуально настраивается только **один HTML-шаблон-обёртка** на организацию (брендинг + плейсхолдеры `{{title}}/{{body}}`, правится как HTML-код, санитизация + allowlist + branding-gate). Часть **N1B (#930)**; канон — TOOLING_AND_PACKAGES_DECISIONS.md §«HTML-конверт транзакционных писем».
