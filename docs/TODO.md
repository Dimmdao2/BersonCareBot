...
- [x] Редактор назначенной программы: **metadata-черновик in-memory**, одно «Сохранить» (legacy batch PATCH); смена статуса этапа только после сохранения (модалка); **завершение программы → все этапы completed**; краткая лента изменений в карточке.
- [x] **Редактор инстанса — structural draft (план batch-toolbar, фазы 1–2, 2026-06-03):** reorder/add/delete/hide, модалка библиотеки → `InstanceEditorDraft`; `structuralPending` при «Сохранить» без metadata flush; status этапа/программы — immediate API; unsaved gate — metadata-only (`isFlushableDirty`); guard + RTL smoke. План: [`.cursor/plans/instance-editor-batch-toolbar_3d597170.plan.md`](../.cursor/plans/instance-editor-batch-toolbar_3d597170.plan.md). **Следующее:** фаза 3 — server `editor-batch`.
