...
- **Редактор текстов — Tiptap Simple Editor (не markdown, не AI-editor).** Замена существующих markdown-редакторов на 3 экранах доктора (content/broadcasts/recommendations) — вариант 1 (markdown остаётся форматом хранения, миграции нет): план **[docs/_TODO/EDITOR_TIPTAP_MIGRATION_PLAN.md](_TODO/EDITOR_TIPTAP_MIGRATION_PLAN.md)**, taskdb **#931**. Тот же редактор — WYSIWYG-начинка для HTML-тела писем в **N1B (#930)**. Исполнение — по решению основного оркестратора (план-only).
