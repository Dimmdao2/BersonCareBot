...
- **Сделано (2026-05-15):** при `webapp.channelLink.complete` после успешного webapp `completeChannelLink` синхронные `writeDb` (`user.phone.link` / для Telegram ещё `user.state.set`) выполняются **последовательно**; если `user.phone.link` не применился (`userPhoneLinkApplied`/indeterminate) — `status: failed`, шаблон `channelLink.completeFailed.*`, **без** welcome MAX / reply-keyboard Telegram; `values.channelLink.ok === false` при `webappComplete: true` (ссылка в webapp закрыта, слой бота — нет). Тесты: `executeAction.test.ts` (`webapp.channelLink.complete fails … phone.link`). План закрытия: [`.cursor/plans/archive/phone_bind_mismatch_ux.plan.md`](../.cursor/plans/archive/phone_bind_mismatch_ux.plan.md).
