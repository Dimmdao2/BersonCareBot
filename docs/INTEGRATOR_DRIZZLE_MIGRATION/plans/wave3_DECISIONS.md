...
### 2) `messengerPhoneHttpBindExecute.ts`
...
**Ответ:** строго **1 PR = 1 фаза**; допускается только `00+09` в одном PR при необходимости ускорить старт.  
...
- [ ] `rg` baseline → таблица Class A/B/C в `RAW_SQL_INVENTORY.md` (дата снимка)
- [ ] В `LOG.md` записаны фиксированные решения 1–10 из этого файла
- [ ] Permanent zones ADR (platform-merge, booking-sync, claim, migrate, projection health)
- [ ] PR map: фаза → ветка → expected tests и owner
- [ ] В `wave3_INDEX.md` указаны зависимости фаз (08 → 09/10 parallel, 11→12→13→14→15→16→17 sequence)
