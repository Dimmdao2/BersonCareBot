...
| `src/infra/repos/s3MediaStorage.ts`                  | Медиа-файлы: advisory lock по media id, delete/update статусов, транзакции с CMS-метаданными, поиск сиротских файлов.                                 | В      | **Wave 2 P5 + R0/S3V partial:** `deleteHard` checkout through `withPoolClient`; purge-loop TX still Class C tail | Целостность медиа и S3                             |
