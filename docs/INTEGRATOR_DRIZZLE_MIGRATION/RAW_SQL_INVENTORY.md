...
| Class B `pool.query` | `runWebappSql.ts` (transport), `pgAdminPlatformUserStats.ts` (uuid[] workaround), `broadcastChannelCounts.ts` (Drizzle ANY-array workaround), `webappPoolProvider.ts` (SAAS principal-aware pool: wraps `pool.query` to stamp the DB principal per connection), `modules/auth/sessionRevocationSchema.ts` (C-1 2026-07-26: BOOT-time assertion that `platform_users.session_epoch` exists — one short-lived connection, one read-only `information_schema` query, closed immediately; deliberately outside the principal-aware pool because it runs before there is a request, a principal or a pool) |
...
| Runtime tail (unique) | Class B pool (**5**) + Class B client (**2**) + Class C (**6**); `webappPoolProvider.ts` учтён и в pool, и в client (использует оба) → **12** distinct файлов |
...
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ----- | ------ | ------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| `src/infra/db/client.ts`  | Реализация `DbPort.query` / транзакция `BEGIN`/`COMMIT`/`ROLLBACK` на `pg` client; health `SELECT 1` на пуле.                                | **C** | С      | **R0/S3AA:** checkout via `withClient.ts`; `DbPort` transport/TX semantics preserved | Регресс всех репозиториев на TX            |
...
| `src/infra/runtime/worker/outgoingDeliveryWorker.ts`            | Воркер `public.outgoing_delivery_queue`: blocked TG/MAX → `failure_class=recipient_blocked_bot`, `skipped` attempts, `blocked_recipient_count` (не `error_count`); маркер `user_channel_bindings.bot_blocked_at` (UPSERT/clear on success); `SELECT` статуса напоминания; для `doctor_broadcast_intent` — опционально **`doctorBroadcastIntentMenu`** перед **`dispatchOutgoing`** при `payload_json.attachMenu`; `UPDATE public.broadcast_audit` (`sent_count` / `blocked_recipient_count`; `error_count` только operator-dead). | С      | **Wave 2 P1 done** (точечный SQL → `runIntegratorSql`); dispatch/retry без изменений              | Счётчики рассылок врача, blocked ≠ degraded health, идемпотентность очереди                          |
...
| `src/infra/db/withClient.ts`                        | R0/S3 chokepoint: единственный webapp checkout helper for `PoolClient` / transaction handles.                                             | С      | **R0/S3:** `withPoolClient` / `withPoolTransaction` / `startPoolTransaction` | Будущий tenant/app principal hook                                  |
...
| `src/infra/adminAuditLog.ts`                        | Журнал админ-действий: вставка, списки/фильтры, транзакции при дедупликации.                                                             | С      | **Wave 3 P14C + R0/S3S done:** `runWebappPgText`; dedupe TX through `withPoolTransaction` | Комплаенс / расследования                                           |
...
| `src/infra/repos/s3MediaStorage.ts`                  | Медиа-файлы: advisory lock по media id, delete/update статусов, транзакции с CMS-метаданными, поиск сиротских файлов.                                 | В      | **Wave 2 P5 + R0/S3W done:** checkout/TX control through `withPoolClient` / `startPoolTransaction` | Целостность медиа и S3                             |
...
| `src/infra/repos/mediaPreviewWorker.ts`              | Превью воркер: длинная транзакция обновления статусов превью и связанных таблиц.                                                                      | В      | **Wave 2 P5 + R0/S3O:** `runWebappSql`; worker transaction checkout via `withPoolTransaction` | Превью в каталоге                                  |
...
| `src/infra/repos/pgUserProjection.ts`                | Связка webapp user ↔ integrator / platform_users: выборки, insert, merge-апдейты, транзакции.                                                         | В      | **Wave 3 P14B + R0/S3R:** `runWebappPgText`/`txPgText`; projection transaction checkout via `withPoolTransaction` | Идентичности пользователей                         |
...
| `src/infra/repos/pgDoctorClientCreate.ts`            | Создание клиента врачом (phone/email conflict, INSERT + phone history TX).                                                                            | С      | **Wave 3 P13C done / R0 S1AB:** `runWebappPgText`; Class C TX moved from app-layer repo facade | Дубликаты клиентов                                 |
...
| `src/infra/repos/pgOnlineIntake.ts`                  | Online intake: shared advisory lock по user id + операции заявки.                                                                                     | С      | **Wave 2 P3 + Wave 3 P12A + R0/S3P:** lock + `runWebappPgText`; transaction checkout via `withPoolTransaction` | Двойная заявка                                     |
...
| `src/infra/repos/broadcastChannelCounts.ts`          | Счётчики каналов для тайлов broadcast-формы; `getChannelCountsByUserIds` — Class B `pool.query` (ANY uuid[] workaround).   | Н      | **Wave 3 P15G:** `getChannelConnectionCounts` на Drizzle; `getChannelCountsByUserIds` — Class B permanent (Drizzle sqlToQuery ANY bug) | Broadcast channel tiles |
| `src/app/api/doctor/account/timezone/route.ts`       | Доктор: GET/PATCH `calendar_timezone` platform_users.                                                                        | Н      | **Wave 3 P15G:** Class B `pool.query` (simple SELECT/UPDATE) | Часовой пояс календаря |
...
| `src/infra/platformUserFullPurge.ts`                 | Полное удаление данных пользователя (каскады DELETE/UPDATE по множеству таблиц).                                                                      | В      | **Wave 3 P12D + R0/S3X done:** `runPurgeClientPgText` / `runPurgePoolPgText`; integrator cleanup TX through `startPoolTransaction` | GDPR / потеря данных                               |
...
| `src/infra/strictPlatformUserPurge.ts`               | Строгий purge: delete медиа, advisory lock.                                                                                                           | В      | **Wave 3 P12D + R0/S3U done:** TX checkout/control through `startPoolTransaction`; load/post-commit → `runPgPoolPgText` (P11) | Медиа / блокировки                                 |
