...
| Class C `client.query` | **9** файлов — TX `BEGIN`/`COMMIT`/`ROLLBACK`; R0/S3 moves checkout to `withClient.ts` |
| `rg -l` (incl. JSDoc) | legacy snapshot; current R0/S3 gate uses `webappPhase15F.verify.test.ts` |
| Runtime tail (unique) | **13** файлов = Class B pool (**3**) + Class B client (**1**) + Class C (**9**) |
