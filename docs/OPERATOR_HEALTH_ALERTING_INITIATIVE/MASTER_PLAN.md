...
> **Доставка уведомлений (Wave 2 закрыт, 2026-06-09):** канон — [`ROADMAP_WAVE2.md`](ROADMAP_WAVE2.md) + [`SCOPE_DECISIONS.md`](SCOPE_DECISIONS.md); план — [`.cursor/plans/archive/operator_health_alerting_wave2.plan.md`](../../.cursor/plans/archive/operator_health_alerting_wave2.plan.md). Этот файл — **справочник по интеграциям и фазам A–G**; §3.3 (email обязателен), §7 DoD (recovery push) **пересмотрены**: recovery — строка в суточной сводке; email — backlog. [`PHASE_E`](PHASE_E_RESOLUTION_AND_RECOVERY_NOTIFICATIONS.md) — **superseded**.
...
- Вкладка **«Здоровье системы»** (`/app/doctor/system-health`, admin mode): `GET /api/admin/system-health` (сбор [`collectAdminSystemHealthData`](../../apps/webapp/src/app-layer/health/collectAdminSystemHealthData.ts)) агрегирует webapp DB, integrator `GET /health`, projection `GET /health/projection` (очередь `projection_outbox`: pending/dead/retries/`lastSuccessAt`), медиа-превью, метрики playback и клиента плеера, **ошибки HLS-прокси**, **очередь доставки уведомлений**, блок **`remindersPipeline`** (очередь **`reminder_dispatch`**, **`occurrenceHistory`** / **`deliveryEvents`** за 24 ч UTC, **`patientReminderM2mIdempotencyKeysActive`** по `idempotency_keys` с шаблоном `prn:%:channels`, см. [`adminReminderPipelineMetrics.ts`](../../apps/webapp/src/app-layer/health/adminReminderPipelineMetrics.ts)), **`cronJobs`** (host cron / internal periodic jobs — [`cronJobRegistry.ts`](../../apps/webapp/src/modules/operator-health/cronJobRegistry.ts), журнал [`LOG.md`](LOG.md) § 2026-05-28), **метрики транскода** (`media_transcode_jobs` + кандидаты legacy-reconcile по DRY-предикату), **`videoTranscode.status`** (`ok` \| `degraded` \| `error`, пороги в [`adminHealthThresholds.ts`](../../apps/webapp/src/modules/operator-health/adminHealthThresholds.ts)), тик **`POST /api/internal/media-transcode/reconcile`** в **`operator_job_status`**, открытые операторские инциденты и статусы host backup. Отдельно вкладка **«Статистика»**: `GET /api/admin/reminder-stats` ([`loadAdminReminderStats.ts`](../../apps/webapp/src/app-layer/stats/loadAdminReminderStats.ts)). Воркер projection оценивается **косвенно** (нет systemd API в variant 1).
