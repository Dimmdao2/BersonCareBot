...
- **Здоровье системы** (admin): [`/app/doctor/system-health`](../../apps/webapp/src/app/app/doctor/system-health/page.tsx) — [`SystemHealthSection.tsx`](../../apps/webapp/src/app/app/settings/SystemHealthSection.tsx), сбор [`collectAdminSystemHealthData.ts`](../../apps/webapp/src/app-layer/health/collectAdminSystemHealthData.ts) → `GET /api/admin/system-health` (в т.ч. **`remindersPipeline`**: срез очереди `reminder_dispatch`, **`occurrenceHistory`** / **`deliveryEvents`** за 24 ч UTC, **`patientReminderM2mIdempotencyKeysActive`** — [`adminReminderPipelineMetrics.ts`](../../apps/webapp/src/app-layer/health/adminReminderPipelineMetrics.ts); **`cronJobs`** — сводка host cron / internal periodic jobs из **`operator_job_status`** — реестр [`cronJobRegistry.ts`](../../apps/webapp/src/modules/operator-health/cronJobRegistry.ts), сбор [`collectCronJobsHealth.ts`](../../apps/webapp/src/app-layer/health/collectCronJobsHealth.ts), журнал [`LOG.md`](LOG.md) § 2026-05-28). Баннер на «Сегодня» — `adminDoctorTodayHealthBannerFromSystemHealth` (критичные сигналы без media/video preview; **cronJobs** в баннер не входят). См. [`DOCTOR_CABINET_NAVIGATION.md`](DOCTOR_CABINET_NAVIGATION.md).
- **Аналитика по уведомлениям** (admin): `GET /api/admin/reminder-stats` — **`loadContentEngagementStats`** ([`loadAdminReminderStats.ts`](../../apps/webapp/src/app-layer/stats/loadAdminReminderStats.ts)), UI [`ReminderStatsSection.tsx`](../../apps/webapp/src/app/app/settings/ReminderStatsSection.tsx) на **`/app/doctor/analytics/notifications`**. Тот же JSON для врача: **`GET /api/doctor/content-stats`** (см. [`api.md`](../../apps/webapp/src/app/api/api.md)).
...
- **Операторский bulk reset из UI** (закрыть все инциденты, archive dead projection / `reminder_dispatch`): закрытый план [`.cursor/plans/archive/health_ui_operator_actions.plan.md`](../../.cursor/plans/archive/health_ui_operator_actions.plan.md), [`LOG.md`](LOG.md) § 2026-06-07; API — `POST /api/admin/operator-incidents/resolve-all`, расширенный `health-failure-archive/clear`.
