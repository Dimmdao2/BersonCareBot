...
- **Профиль — канал OTP:** блок «Подтверждение входа»; сохранение в `user_channel_preferences.is_preferred_for_auth` только для **`telegram` / `max` / `email` / `sms`** (политика `preferredAuthChannelPolicy.ts`; **`web_push` / `vk`** — запрещены, наследие в БД маскируется при чтении); миграция `040_auth_preferred_channel.sql`; `check-phone` отдаёт `preferredOtpChannel`, `AuthFlowV2` использует `pickOtpChannelWithPreferencePublic` (на публичном `/app` **SMS** недоступен; **email** и мессенджеры — по `methods`, см. `apps/webapp/src/modules/auth/auth.md` → **Email**).
