...
Инициатива **завершена**. Все 7 этапов выполнены + дополнительный фикс мобильного back-slot.

| Этап | Фаза | Содержание | Статус |
|------|------|------------|--------|
| 1 | A0 | Порядок вкладок Чаты→Комментарии→Заявки→Рассылки + LOG.md + каркас скролла | ✅ Готово |
| 2 | A1 | Чаты: ширины пейнов, отступы треда, ellipsis, пустое состояние, CatalogSplitLayout | ✅ Готово |
| 3 | A2 | Заявки: мультитоггл статусов (без «Все»), CatalogSplitLayout | ✅ Готово |
| 4a | A4 | Бэкенд рассылок: 5-канальная модель (TG·MAX·Push·SMS·Email), реальные счётчики, email-фанаут | ✅ Готово |
| 4b | A4 | UI формы рассылки: порядок полей, ReferenceSelect аудитории, 4 категории, 5 каналов, дефолты | ✅ Готово |
| 5a | B.1 | Бэкенд-агрегация комментариев: loadDoctorCommentPatients, loadDoctorPatientExercisesWithComments, listUnreadCountsForViewerByStageItems | ✅ Готово |
| 5b | B.2 | UI drill-down комментариев: 3 состояния правого пейна, навигация, шапка с крошкой | ✅ Готово |
| 6 | B.3 | Микро-график статистики упражнения (reps/вес/тяжесть) в шапке чата — ExerciseMicroChart | ✅ Готово |
| 7 | B.4 | Журнал рассылок: single-open аккордеон + сводка + полный текст + «Открыть ошибки →» | ✅ Готово |
| Доп. | — | Мобильный back-slot во всех 4 split-табах коммуникаций | ✅ Готово |

**Сквозной аудит:** `tsc` — 0 ошибок; `eslint` — 0; ~317+ тестов зелёные; `webappPhase15F.verify.test.ts` — 5/5.

---

## Отложенные follow-ups

Следующие задачи зафиксированы как **вне scope** инициативы и требуют отдельного прогона:

| Follow-up | Причина откладывания |
|-----------|---------------------|
| Новые сегменты аудитории §5.1 (На сопровождении, С программой, Приём в месяце, …) | Требуют новых DB-фильтров в `broadcastEligible.ts` + возможного пересечения с зоной `doctor-appointments` |
| «Выбрать вручную» в аудитории (диалог с чекбоксами пациентов) | Хранение `userId[]` вместо enum-фильтра, большой отдельный кусок UI+бэкенда |
| B.4 вложенная разбивка «Доставка по каналам» с причинами/именами | Исторические рассылки не содержат per-job/per-recipient разбивку; нужна схема + бэкенд |
| B.4 «Повторить N неудачным» | Требует retry-роут + хранение списка неудавшихся получателей |
| B.4 «Создать на основе» | Требует prefill формы через колбэк или URL-параметр (подъём состояния или новый проп) |
| UI прошлых программ в комментариях (state B) | Кнопка/раскрытие прошлых программ — вторично по README; данные готовы (`includePastPrograms=true`) |
| Точный per-message unread-счётчик в левом пейне комментариев | Сейчас: число stageItem с непрочит. последним сообщением (приближение); реальный счёт требует fanout по программам |
| **Live-проверка email-рассылки** | Новый путь отправки (`fanOutBroadcastEmail`) реализован guarded; нужна SMTP-конфигурация в dev и ручной прогон |
| Дефолт фильтра заявок сменён на «показать все» | Старый дефолт был «только новые»; возможно продуктовое уточнение нужно |
