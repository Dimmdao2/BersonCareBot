...
11. **Переиспользование shared кода**: перед созданием нового UI-компонента, хука, утилиты или типа — проверить, нет ли готового в `components/`, `shared/`, соседних модулях. Переиспользовать, если это **уменьшает количество сущностей**, а не добавляет лишних зависимостей и не запутывает модуль. Принцип: меньше дублирования → лучше; лишняя связность → хуже.
12. **Готовые библиотеки вместо велосипедов**: если задачу решает уже установленная или широко используемая библиотека — использовать её. Не писать свою реализацию там, где есть проверенное решение с нулевой ценой подключения.
13. **Zod для валидации**: все входящие данные в route handlers и сервисных портах валидируются через `zod`. Zod уже установлен и используется в проекте — новый код обязан следовать той же практике. Не изобретать ручную валидацию там, где `z.parse()` / `z.safeParse()` решает задачу.
