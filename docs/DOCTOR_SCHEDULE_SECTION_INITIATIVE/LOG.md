...
## Post-audit cleanup (2026-06-13) — тех-долг

- **Удалён неиспользуемый `weeklist`** из `CalendarViewMode` + `parseCalendarQuery`. Его единственным
  потребителем была устаревшая страница `/app/doctor/calendar` (`DoctorBookingCalendarClient` + `page.tsx`),
  которая уже 308-редиректится на `/app/doctor/schedule?tab=cal` → удалена как мёртвый код (rg-проверка:
  импортировалась только своей же страницей; общие `DoctorCalendarEventPanel`/`ToolbarFilter`/`CreateFormField`/
  `PatientSearch` оставлены — их использует новый `ScheduleCalendarTab`). Smoke-тест RSC-страниц обновлён.
- **CI:** lint/typecheck/тесты/оба билда — зелёные; шаг `audit` падает на пред-существующем repo-wide
  advisory `esbuild@0.25.12` (GHSA-gv7w-rqvm-qjhr) — не из этой инициативы, решается отдельным bump'ом deps.
