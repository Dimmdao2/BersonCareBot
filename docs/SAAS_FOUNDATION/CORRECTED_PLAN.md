# SAAS Foundation — PLAN v5 (canonical, 2026-06-17)
...
|---|---|---|
| **SCOPED** | **148** | per-patient/per-tenant. **104 need `organization_id`** (44 are `be_*`: 41 direct org + `be_organizations` self-scoped + `be_package_items`/`be_patient_package_items` via FK-path). RLS by org (+ patient by actor). |
...
| **INFRA** | 27 | migrations/ledgers/idempotency/operator-health/webhook-status + **outbox/queue pumps** (projection_outbox, delivery_attempt_logs, integrator_push_outbox, outgoing_delivery_queue — no user/org path) + **mailing-topic taxonomy** (global). No person data. |
...
## Sizing (v4 — 104 scoped incl integrator + child denorm + bridge)
