...
- **D7 — Integration config per org via `system_settings` (scope), NOT env** — mandatory per `.cursor/rules/000-critical-integration-config-in-db`; respect public↔integrator mirror via `updateSetting`; new keys → `ALLOWED_KEYS`. (Supersedes the earlier "SecretsResolver + env fallback".)
