...
**Зачем опционально ПЕРВИЧНЫЙ_AUDIT (шаг 0):** это не часть MASTER_PLAN и **не обязателен**. Он нужен, когда к коду подходят «с холодной»: давно не трогали ветку, несколько параллельных PR, новый исполнитель — и **неочевидно**, что уже сделано и что фаза A не продублирует существующие куски. Результат — короткая **карта «уже есть / частично / нет» по фазам A–E** (включая промежуточную **C.02** из `MASTER_PLAN.md` §5, если релевантно), без вердикта о закрытии инициативы. Если состояние репозитория и так известно — **сразу Фаза A — EXEC**.
...
## Фаза B — повторный AUDIT после FIX

Сверь код с целями фазы B в `MASTER_PLAN.md` §5 и связанными пунктами DoD. Зафиксируй в `docs/PLATFORM_IDENTITY_ACCESS/SCENARIOS_AND_CODE_MAP.md` **§10** либо статус «закрыта» с перечнем реализованного, либо таблицу остатков с отсылками к фазам C/E; обнови `MASTER_PLAN.md` §5 при смене статуса; строку в `AGENT_EXECUTION_LOG.md`. Правки кода — только если аудит выявил P0/P1 по фазе B.
...
4. **RSC под `/app/patient/*`:** пройди grep по `requirePatientAccess(`; где страница грузит данные из БД по `userId`, а связанные server actions уже используют `requirePatientAccessWithPhone` — устрани рассинхрон (через усиленный layout из п.3 и/или замену guard на странице).

Опционально: переименуй `requirePatientApiSessionWithPhone` → `requirePatientApiBusinessAccess` с алиасом на старое имя; обнови `apps/webapp/src/app-layer/guards/guards.md`.

Обнови `docs/PLATFORM_IDENTITY_ACCESS/SCENARIOS_AND_CODE_MAP.md` §7 и §11 (явно: booking API, patient layout / RSC vs tier). `pnpm run ci`, строка в `docs/PLATFORM_IDENTITY_ACCESS/AGENT_EXECUTION_LOG.md` с формулировкой вроде «C.02: booking + layout/RSC».

**Не** реализуй в этом задании полный модуль **route & API policy** фазы D (whitelist в одном файле-модуле) — только закрой зазоры из §5 C.02 плана.
...
4. Нет страниц `/app/patient/*` с `requirePatientAccess` + запросами в БД по `userId`, которые обходят тот же критерий, что server actions с `requirePatientAccessWithPhone` (или это закрыто layout).
5. Доки: §7/§11 в `SCENARIOS_AND_CODE_MAP.md`, запись в `AGENT_EXECUTION_LOG.md`.

Верни: соответствует / нет; пути к файлам; остатки к фазе D.

---

## Фаза C.02 — FIX

Устрани находки **аудита фазы C.02** (или сверки с `MASTER_PLAN.md` §5 C.02). Минимальный дифф; не подменяй полную фазу D. Обнови тесты booking при смене кодов ответов/контракта. `pnpm run ci`, `AGENT_EXECUTION_LOG.md`.
...
Ты работаешь в репозитории BersonCareBot. Непосредственно перед этим выполнен **GLOBAL_AUDIT** (финальный сквозной аудит после фаз A–E, включая C.02 по плану §5 при наличии). Используй **его выводы** как вход (если в чате нет отчёта — не вызывай этот промпт; сначала выполни GLOBAL_AUDIT).
