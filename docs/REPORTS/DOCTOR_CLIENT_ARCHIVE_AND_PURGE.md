...
- **Явно удаляется в webapp:** `phone_otp_locks`, `phone_challenges`, `appointment_records`, `message_log`, `patient_bookings`, `reminder_rules`, `doctor_notes`, `support_conversations`, `patient_lfk_assignments`, `content_access_grants_webapp`, `user_notification_topics`, `user_channel_preferences`, `online_intake_requests`, `user_channel_bindings`, `user_pins`, `login_tokens`, `user_oauth_bindings`, а также дневники симптомов и ЛФК. ~~`news_item_views`~~ — таблица **снята** миграцией webapp `0016_drop_news_broadcast_channels.sql` (APP_RESTRUCTURE этап 1); в актуальной схеме отсутствует, код purge на неё не опирается.
