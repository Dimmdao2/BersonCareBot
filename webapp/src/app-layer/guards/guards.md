...
- **requirePatientPhone** — требует привязанный номер телефона у пациента; иначе редирект на `/app/patient/bind-phone?next=...`. Вызывать только на маршрутах из `patientPathsRequiringPhone` (кабинет, покупки, дневник симптомов, дневник ЛФК). Остальные разделы (меню, уроки, скорая помощь, контент) доступны без телефона.
