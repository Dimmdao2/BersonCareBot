...
describe('error tracking runtime', () => {
  it.each([
    ...
    await expect(
      initErrorTrackingWithLoader({ ...baseInput, enabled, dsn }, load),
  ...
  it('initializes errors-only SDK options after valid opt-in', async () => {
    ...
    ).resolves.toEqual({ enabled: true, release: 'build-123' });
        ...
        values: [
          {
            ...
            stacktrace: {
              frames: [
                {
                  filename: `/srv/repo/apps/integrator/src/main.ts?token=${marker}`,
                  function: 'start',
                  lineno: 42,
                },
  ...
  it('resolves release from BUILD_ID, bounded git SHA, then dev/unknown', () => {
    expect(resolveErrorTrackingRelease({ buildId: 'release-7', cwd: '/missing' })).toBe(
      'release-7',
    );
    expect(
      resolveErrorTrackingRelease({ buildId: '', cwd: process.cwd(), nodeEnv: 'production' }),
    ).toMatch(/^[0-9a-f]{7,12}$/);
    expect(
      resolveErrorTrackingRelease({
        buildId: 'bad release',
