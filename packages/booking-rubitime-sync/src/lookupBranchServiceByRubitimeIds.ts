...
export async function lookupBranchServiceByRubitimeIds(
  ...
  const res = await db.query<{
    ...
    `SELECT
     ...
     JOIN public.booking_cities c ON c.id = b.city_id
     JOIN public.booking_branch_services bs ON bs.branch_id = b.id
     JOIN public.booking_services s ON s.id = bs.service_id
     JOIN public.booking_specialists sp ON sp.id = bs.specialist_id
