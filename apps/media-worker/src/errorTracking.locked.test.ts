...
describe('media-worker error tracking locked startup', () => {
  ...
  it('runs both config reads, tracker init, and readiness inside the allowed locked principal', async () => {
      ...
      const client = {
        query: vi.fn(async (sql: string, values?: readonly unknown[]) => {
          ...
          if (sql.includes('app.read_media_worker_runtime_setting')) {
            ...
            const value = key === 'error_tracking_enabled' ? true : 'https://public@example.test/1';
    ...
    expect(configEntries).toEqual(
      expect.arrayContaining(['config:error_tracking_enabled', 'config:error_tracking_dsn']),
    );
    expect(configEntries).toHaveLength(2);
    expect(order.findIndex((entry) => entry === 'config:error_tracking_enabled')).toBeLessThan(
      initIndex,
    );
    expect(order.findIndex((entry) => entry === 'config:error_tracking_dsn')).toBeLessThan(
      initIndex,
    );
    ...
    expect(
      executedSql.filter((sql) => sql === 'SET ROLE app_operational_media_worker'),
    ).toHaveLength(3);
