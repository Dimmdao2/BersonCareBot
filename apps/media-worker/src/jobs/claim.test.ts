...
describe('claimNextJob', () => {
  ...
  it.each([
    ...
    'quarantines invariant violations (%s, %s)',
    async (jobOrganizationId, mediaOrganizationId) => {
      const client = createClientMock();
      client.query
        .mockResolvedValueOnce(undefined)
        .mockResolvedValueOnce({
      ...
      expect(client.query).toHaveBeenCalledWith('COMMIT');
      expect(
        client.query.mock.calls.filter((call) => String(call[0]).includes("status = 'processing'")),
      ).toHaveLength(0);
    },
  );
