...
const rejectedLockedDbPrincipals: ReadonlyArray<{
  ...
  },
  {
    name: 'patient',
    run: (fn) =>
      runWithDbPatientPrincipal({ organizationId: ORG_ID, platformUserId: PATIENT_USER_ID }, fn),
    expectedMessage: 'DB patient principal is not allowed on media-worker pool in locked mode',
  },
  {
    name: 'staff',
    run: (fn) =>
      runWithDbStaffPrincipal({ organizationId: ORG_ID, platformUserId: STAFF_USER_ID }, fn),
  ...
  },
  {
    ...
    run: (fn) =>
      runWithDbIntegratorPrincipal(
        { organizationId: ORG_ID, integratorUserId: INTEGRATOR_USER_ID },
        fn,
      ),
...
describe('media-worker DB client helpers', () => {
  ...
  it('wraps promise-form pool.query with locked DB principal options', async () => {
    ...
    await runWithDbInfraPrincipal({ source: 'media-worker:tick' }, () => pool.query('SELECT ok'));
    await pool.end();
  ...
  for (const testCase of rejectedLockedDbPrincipals) {
    it(`rejects ${testCase.name} locked DB principal before pool.query checkout`, async () => {
      ...
      await expect(testCase.run(() => pool.query('SELECT ok'))).rejects.toThrow(
        testCase.expectedMessage,
      );
      await pool.end();
