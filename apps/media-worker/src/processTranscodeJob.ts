...
async function processTranscodeJobInner(outer: TranscodeContext, job: ClaimedJob): Promise<void> {
  ...
  try {
    ...
    /*
     * Требование 3 (`VIDEO_DELIVERY_COST_AND_METERING`, «холодный бакет для исходников + отключить
     * удаление исходника»): исходный объект `media.s3_key` больше не удаляется здесь. Владелец
     * отдельно решил перевести библиотеку на холодный бакет для исходников — это отдельный этап,
     * а не эта ветка; здесь только отключение удаления, которое раньше происходило в этой точке.
     */
  } catch (e) {
