...
export type PreviewContext = {
  ...
  /** Куда ложится ВЫХОД наряда — рендишн и оба эскиза. Всегда бакет выдачи («горячий»). */
   ...
   * КРОМЕ ещё не перенесённых старых ключей, которые физически остались в горячем: решает форма
   * ключа, а не только цель. Тот же шов, что у очереди видео (`storageBindings.ts`).
   *
   * Разделение обязательно: с 11.09.2026 свежая библиотечная загрузка ложится в сырой бакет, и
   * чтение исходника через `storageFor` промахивалось бы мимо объекта на КАЖДОЙ новой картинке.
   */
...
async function storeVideoPoster(
  ...
  order: MediaPreviewOrder,
  binding: StorageBinding,
  sourceBinding: StorageBinding,
  sourceKey: string,
): Promise<void> {
  const poster = await withTempDir('media-prev-v-', async (dir) => {
    const inputFile = await downloadToTemp(sourceBinding, sourceKey, dir, 'input.bin');
...
export async function processPreviewOrder(
  ...
  /*
   * ВЫХОД всегда в бакет выдачи, ИСХОДНИК — из того, где он физически лежит (М7). Это два разных
   * бакета, и путать их нельзя ни в одну сторону: рендишн в сыром бакете никто никогда не прочитает,
   * а исходника в горячем у свежей загрузки просто нет.
   */
  const binding = ctx.storageFor(order.storageTarget);
  const plan = order.plan;
  const sourceBinding =
    plan.kind === 'hosted_thumbnail'
      ? binding
      : ctx.sourceStorageFor(order.storageTarget, plan.sourceKey);
  try {
    switch (plan.kind) {
      case 'image': {
        const source = await withTempDir('media-prev-img-', async (dir) => {
          const path = await downloadToTemp(sourceBinding, plan.sourceKey, dir, 'input.bin');
      ...
      case 'heic':
        await storeStandardRendition(
          ...
          order,
          binding,
          await decodeHeicJpeg(ctx, sourceBinding, plan.sourceKey),
        );
        break;
      case 'hosted_thumbnail':
        ...
        break;
      case 'video_poster':
        await storeVideoPoster(ctx, order, binding, sourceBinding, plan.sourceKey);
        break;
    }
