...
async function handleDeliveryInner(
  ...
  if (action.type === 'message.compose') {
    ...
    firstPayload = await enrichMessageSendPayloadWithMaxMainInlineIfApplicable(
      firstPayload,
      ctx,
      deps,
    );
    ...
    const intents: OutgoingIntent[] = [
      {
        ...
        payload: {
          ...
          ...(messageId === null ? {} : { messageId }),
          delivery: {
            channels: channels.length > 0 ? channels : [ctx.event.meta.source],
            maxAttempts: 1,
          },
        },
      },
      ...
      jobs: [
        {
          ...
          targets: Array.isArray(job.payload.targets)
            ? (job.payload.targets as Array<{ resource: string; address: Record<string, unknown> }>)
            : [],
  ...
  if (action.type === 'message.retry.enqueue') {
    if (!deps.executeAction) {
      return {
        actionId: action.id,
        status: 'skipped',
        error: 'message.retry.enqueue: executeAction not injected',
      };
    }
