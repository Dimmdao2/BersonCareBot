...
describe('executeAction', () => {
  ...
  it('fans out message.send to multiple channels when deliveryTargetsPort returns bindings', async () => {
    ...
    const telegramIntent = result.intents?.find(
      (i) =>
        i.type === 'message.send' &&
        (i.payload as { delivery?: { channels?: string[] } }).delivery?.channels?.[0] ===
          'telegram',
    );
    const maxIntent = result.intents?.find(
      (i) =>
        i.type === 'message.send' &&
        (i.payload as { delivery?: { channels?: string[] } }).delivery?.channels?.[0] === 'max',
    );
    ...
    const result = await executeAction(
      ...
      },
      {
        ...
        base: {
          ...
          facts: {
            links: {
              bookingUrl: 'https://app.example/book',
  ...
  it('user.phone.link no_channel_binding: webapp CTA when facts.links.webappHomeUrl present (telegram)', async () => {
    ...
    expect(result.intents?.[0]?.payload).toMatchObject({
      replyMarkup: {
        inline_keyboard: [
          [{ text: 'Открыть мини-приложение', web_app: { url: 'https://app.example/mini' } }],
        ],
      },
    });
  });
  ...
  it('user.phone.link no_channel_binding: webapp CTA when facts present (max)', async () => {
    ...
    expect(result.intents?.[0]?.payload).toMatchObject({
      ...
      replyMarkup: {
        inline_keyboard: [
          [{ text: 'Открыть мини-приложение', web_app: { url: 'https://app.example/max' } }],
        ],
      },
    });
  });
  ...
  it('reminders.rule.toggle resolves userId from channelUserId when userId not in params', async () => {
    ...
    const result = await executeAction(
      ...
      callbackCtx,
      { readPort: { readDb }, writePort: { writeDb } },
    );
    expect(result.status).toBe('success');
  ...
  it('sends draft by creating conversation and notifying admin', async () => {
    ...
    expect(result.intents?.[0]).toMatchObject({
      ...
      payload: {
        ...
        replyMarkup: {
          inline_keyboard: [[expect.objectContaining({ text: 'Ответить' })]],
        },
      },
    });
  ...
  it('conversation.openWithMessage sends only a neutral notification with Reply', async () => {
    ...
    const visibleText = (result.intents?.[0]?.payload as { message?: { text?: string } }).message
      ?.text;
  ...
  it('draft.send appends to existing open conversation and cancels draft', async () => {
    ...
    expect(writeDb).not.toHaveBeenCalledWith(
      expect.objectContaining({ type: 'conversation.open' }),
    );
  ...
  it('routes open conversation user and admin messages through conversation actions', async () => {
    ...
    const notificationIntent = userResult.intents?.find((i) => i.type === 'message.send');
  ...
  it('handles notifications.get and notifications.toggle', async () => {
    ...
    expect(toggleResult.values).toEqual({
      notifications: {
        notify_spb: true,
        notify_msk: true,
        notify_online: false,
  ...
  it('builds message.edit, message.replyMarkup.edit and callback.answer intents', async () => {
    const templatePort = {
      renderTemplate: vi.fn().mockImplementation(async ({ templateId }) => ({
        text:
          templateId === 'notifications.togglePrefix'
            ? '✅/❌'
    ...
    expect(replyMarkupResult.intents?.[0]).toMatchObject({
      ...
      payload: {
        ...
        messageId: 78,
        replyMarkup: {
          inline_keyboard: [[{ text: '✅ Петербург', callback_data: 'notify_toggle_spb' }]],
        },
      },
    });
    ...
    expect(canonicalReplyMarkupResult.intents?.[0]).toMatchObject({
      ...
      payload: {
        ...
        messageId: 79,
        replyMarkup: {
          inline_keyboard: [[{ text: '✅ Текст', callback_data: 'notifications.toggle.msk' }]],
        },
      },
    });
    ...
    expect(callbackResult.intents?.[0]).toMatchObject({
      type: 'callback.answer',
  ...
  it('lowers telegram presentation actions to message.send', async () => {
    ...
    expect(replyKeyboardWithWebAppResult.intents?.[0]).toMatchObject({
      ...
      payload: {
        ...
        replyMarkup: {
          keyboard: [
            [{ text: 'Помощник', web_app: { url: 'https://webapp.example/app/tg?t=dummy' } }],
          ],
          resize_keyboard: true,
          one_time_keyboard: false,
          is_persistent: true,
    ...
    expect(inlineKeyboardResult.intents?.[0]).toMatchObject({
      ...
      payload: {
        ...
        message: { text: 'inline' },
        replyMarkup: {
          inline_keyboard: [[{ text: '📅 Запись на приём', callback_data: 'booking.open' }]],
        },
      },
    });
  ...
  it('attaches main reply keyboard to user messages without explicit keyboard when enabled', async () => {
    ...
    const result = await executeAction(
      ...
      },
      {
        ...
        event: {
          ...ctx.event,
          meta: { ...ctx.event.meta, source: 'telegram' },
          payload: { incoming: { text: 'question', chatId: 123 } },
        },
      },
      {
        templatePort,
    ...
    expect(result.intents?.[0]).toMatchObject({
      ...
      payload: {
        ...
        replyMarkup: {
          keyboard: [
            [{ text: '📅 Запись на приём' }, { text: 'Приложение', web_app: { url: homeUrl } }],
          ],
          resize_keyboard: true,
          one_time_keyboard: false,
          is_persistent: true,
    ...
    const result = await executeAction(
      ...
      },
      {
        ...
        event: {
          ...ctx.event,
          meta: { ...ctx.event.meta, source: 'telegram' },
          payload: { incoming: { action: 'menu.more', text: 'Помощник', chatId: 123 } },
        },
      },
      {
        templatePort,
  ...
  it('does not attach main reply keyboard when linkedPhone is false (contact gate)', async () => {
    const templatePort = {
      renderTemplate: vi.fn().mockImplementation(async ({ templateId }) => ({
        text:
          templateId === 'questionAccepted' ? 'Вопрос принят. Я отвечу вам в ближайшее время.' : '',
      })),
    };
  ...
  describe('support relay', () => {
    ...
    it('conversation.user.message: returns refusal intent when type not allowed user->admin', async () => {
      ...
      const result = await executeAction(
        ...
        userCtx,
        {
          ...
          supportRelayPolicy: createSupportRelayPolicy(
            ['text', 'photo'],
            ['text', 'photo', 'document'],
          ),
        },
    ...
    it('conversation.user.message: never copies the patient message into the doctor chat', async () => {
      ...
      const notificationIntent = result.intents?.find((i) => i.type === 'message.send');
      expect(notificationIntent).toBeDefined();
      expect(notificationIntent?.payload).toMatchObject({
        recipient: { chatId: 999 },
    ...
    it('conversation.user.message: routes MAX text to MAX admin chat without telegram copy', async () => {
      ...
      const adminIntents = result.intents?.filter(
        (i) =>
          i.type === 'message.send' &&
          (i.payload as { recipient?: { chatId?: number }; delivery?: { channels?: string[] } })
            .recipient?.chatId === 156854402,
      );
    ...
    it('conversation.admin.reply: uses message.copy when allowed non-text type', async () => {
      ...
      const readDb = vi
        .fn()
        .mockImplementation((query: { type: string; params?: { conversationId?: string } }) => {
          if (query.type === 'conversation.byId') {
            return Promise.resolve({
              id: 'conv-2',
              source: 'telegram',
      ...
      expect(
        result.writes?.some(
          (w) => w.type === 'conversation.message.add' && w.params?.text === '[photo]',
        ),
      ).toBe(true);
      const copyIntent = result.intents?.find((i) => i.type === 'message.copy');
      ...
      const confirmIntent = result.intents?.find(
        (i) =>
          i.type === 'message.send' &&
          (i.payload as { recipient?: { chatId?: number } })?.recipient?.chatId === 999,
      );
    ...
    it('conversation.admin.reply: sends text to user and confirmation to admin', async () => {
      ...
      const userSendIntent = result.intents?.find(
        (i) =>
          i.type === 'message.send' &&
          (i.payload as { recipient?: { chatId?: number } })?.recipient?.chatId === 789,
      );
    ...
    it('conversation.admin.reply: sends MAX text to MAX user and confirms in MAX admin chat', async () => {
      ...
      const userSendIntent = result.intents?.find(
        (i) =>
          i.type === 'message.send' &&
          (i.payload as { recipient?: { userId?: number } })?.recipient?.userId === 456,
      );
      expect(userSendIntent).toBeDefined();
      expect(
        (userSendIntent?.payload as { delivery?: { channels?: string[] } }).delivery?.channels,
      ).toEqual(['max']);
      const adminConfirmIntent = result.intents?.find(
        (i) =>
          i.type === 'message.send' &&
          (i.payload as { recipient?: { chatId?: number } })?.recipient?.chatId === 156854402,
      );
      expect(adminConfirmIntent).toBeDefined();
      expect(
        (adminConfirmIntent?.payload as { delivery?: { channels?: string[] } }).delivery?.channels,
      ).toEqual(['max']);
    });
  });
  ...
  it('webapp.channelLink.complete dispatches contact request for Max when needsPhone', async () => {
    ...
    expect(result.status).toBe('success');
    expect(
      (result.values as { channelLink?: { contactPromptSent?: boolean } }).channelLink
        ?.contactPromptSent,
    ).toBe(true);
  });
  ...
  it('webapp.channelLink.complete uses generic failure template when error is channel_link_claim_failed', async () => {
    const completeChannelLink = vi
      .fn()
      .mockResolvedValue({ ok: false, error: 'channel_link_claim_failed' });
    const webappEventsPort = {
      completeChannelLink,
      emit: vi.fn(),
      listSymptomTrackings: vi.fn(),
  ...
  it('webapp.channelLink.complete maps channel_owned_by_real_user to conflict failure template', async () => {
    const completeChannelLink = vi
      .fn()
      .mockResolvedValue({ ok: false, error: 'channel_owned_by_real_user' });
    const webappEventsPort = {
      completeChannelLink,
      emit: vi.fn(),
      listSymptomTrackings: vi.fn(),
  ...
  it('webapp.channelLink.complete syncs phone and sends welcome intents for Telegram when phone already on platform', async () => {
    ...
    const renderTemplate = vi
      .fn()
      .mockImplementation(async ({ templateId }: { templateId: string }) => {
    ...
    expect(writeDb).toHaveBeenCalledWith(
      expect.objectContaining({
        type: 'user.phone.link',
        params: expect.objectContaining({ resource: 'telegram' }),
      }),
    );
    expect(writeDb).toHaveBeenCalledWith(expect.objectContaining({ type: 'user.state.set' }));
    expect(renderTemplate).toHaveBeenCalled();
    const welcome = result.intents?.find((i) => i.type === 'message.send');
    expect(welcome).toBeDefined();
    expect((welcome?.payload as { message?: { text?: string } })?.message?.text).toContain(
      'Номер привязан',
    );
    const row = (welcome?.payload as { replyMarkup?: { keyboard?: unknown[][] } })?.replyMarkup
      ?.keyboard?.[0];
    expect(row).toEqual([
  ...
  it('webapp.channelLink.complete sends afterChannelLinked for Max when phone already on platform', async () => {
    ...
    expect((send?.payload as { delivery?: { channels?: string[] } }).delivery?.channels).toEqual([
      'max',
    ]);
  });
  ...
  it('webapp.channelLink.complete fails Max when user.phone.link returns not applied (no afterChannelLinked)', async () => {
    ...
    expect(writeDb).toHaveBeenCalledWith(
      expect.objectContaining({
        type: 'user.phone.link',
        params: expect.objectContaining({ resource: 'max' }),
      }),
    );
    ...
    const failureSend = result.intents?.find(
      (i) =>
        i.type === 'message.send' && i.meta?.eventId?.includes('channel-link-phone-sync-failed'),
    );
    expect(failureSend).toBeDefined();
    expect((failureSend?.payload as { message?: { text?: string } })?.message?.text).toContain(
      'generic',
    );
    expect(
      (result.values as { channelLink?: { ok?: boolean; webappComplete?: boolean } })?.channelLink
        ?.ok,
    ).toBe(false);
    expect(
      (result.values as { channelLink?: { webappComplete?: boolean } })?.channelLink
        ?.webappComplete,
    ).toBe(true);
  });
  ...
  describe('diary.symptom.afterTrackingCreated', () => {
    ...
    it('sends intensity prompt with 0-10 inline buttons when tracking is found by title', async () => {
      ...
      const firstButton = (
        payload.replyMarkup?.inline_keyboard?.[0] as { callback_data?: string }[]
      )?.[0];
    ...
    it('sends fallback tracking-created message when no tracking found by title', async () => {
      ...
      expect(result.intents?.[0]?.payload).toMatchObject({
        ...
        replyMarkup: {
          inline_keyboard: [
            [{ text: expect.stringContaining('К списку'), callback_data: 'diary.symptom.open' }],
          ],
        },
      });
    });
  ...
  describe('reminders.dispatchDue', () => {
    ...
    it('calls deliveryTargetsPort with topic and drops telegram when bindings omit telegramId', async () => {
      ...
      const writeDb = vi.fn().mockResolvedValue(undefined);
      const getTargetsByChannelBinding = vi
        .fn()
        .mockResolvedValue({ channelBindings: { maxId: 'max-ext-1' } });
      const deliveryTargetsPort = {
      ...
      const firstCall = enqueueReminderOutboxMock.mock.calls[0]?.[1] as
        | { channel?: string }
        | undefined;
  ...
  describe('reminders.snooze.callback (telegram)', () => {
    ...
    it('emits callback.answer before message.edit', async () => {
      ...
      expect((result.intents?.[0]?.payload as { callbackQueryId?: string }).callbackQueryId).toBe(
        'cb-snooze-1',
      );
    });
  });
  ...
  describe('reminders.skip.applyPreset (telegram)', () => {
    it('reason none: marks skipped; ack intents are callback.answer then message.edit when messageId set', async () => {
      const readDb = vi
        .fn()
        .mockImplementation(async (q: { type: string; params?: Record<string, unknown> }) => {
      ...
      expect(result.status).toBe('success');
      expect(writeDb).toHaveBeenCalled();
      const markSkipped = writeDb.mock.calls.find(
        (c) => c[0]?.type === 'reminders.occurrence.markSkippedLocal',
      );
      ...
      expect(
        edit && 'payload' in edit ? (edit.payload as { messageId?: unknown }).messageId : null,
      ).toBe(9001);
    });
    ...
    it('reason none: without messageId uses message.send + callback.answer', async () => {
      ...
      const send = result.intents?.find((i) => i.type === 'message.send');
      expect(
        send && 'payload' in send
          ? (send.payload as { message?: { text?: string } }).message?.text
          : null,
      ).toBe('ack no mid');
    });
    ...
    it('reason none: postOccurrenceSkip with reason null runs before markSkippedLocal when webapp port set', async () => {
      ...
      const markIdx = writeDb.mock.calls.findIndex(
        (c) => c[0]?.type === 'reminders.occurrence.markSkippedLocal',
      );
  ...
  describe('reminders.messengerTopic.disable.callback', () => {
    ...
    it('telegram: calls webapp disable, escapes HTML paragraphs, attaches two-url follow-up keyboard', async () => {
      ...
      expect(
        payload && typeof payload === 'object' && 'message' in payload
          ? (payload as { message: { text?: string } }).message?.text
          : '',
      ).toContain('&lt;b&gt;нет&lt;/b&gt;');
      const rm =
        payload && typeof payload === 'object' && 'replyMarkup' in payload
          ? (
              payload as {
                replyMarkup: { inline_keyboard: { url?: string; web_app?: { url: string } }[][] };
              }
            ).replyMarkup
          : null;
    ...
    it('max: resolves messengerChannel max for postMessengerTopicDisable', async () => {
      ...
      const readDb = vi
        .fn()
        .mockImplementation(async (q: { type: string; params?: Record<string, unknown> }) => {
          if (q.type === 'user.byIdentity') return { userId: 'int-bot-off-max' };
  ...
  describe('reminders.done.callback (telegram)', () => {
    ...
    it('answers callback, deletes message; no celebration when day not complete', async () => {
      ...
      expect(
        del && 'payload' in del ? (del.payload as { messageId?: unknown }).messageId : null,
      ).toBe(777);
    });
  ...
  describe('reminders.skip.applyFreeText (max)', () => {
    it('uses message.edit on reply target when replyToMessageId is present', async () => {
      const readDb = vi
        .fn()
        .mockImplementation(async (q: { type: string; params?: { occurrenceId?: string } }) => {
          if (q.type === 'user.byIdentity') return { userId: 'user-1' };
          if (
            q.type === 'reminders.occurrence.ownerUserId' &&
            q.params?.occurrenceId === 'occ-ft'
          ) {
            return 'user-1';
          }
          return null;
    ...
    it('marks each unanswered row with question.markAnswered', async () => {
      ...
      const markCalls = writeDb.mock.calls.filter(
        (c) =>
          c[0] &&
          typeof c[0] === 'object' &&
          (c[0] as { type?: string }).type === 'question.markAnswered',
      );
      expect(markCalls).toHaveLength(2);
      expect((markCalls[0]?.[0] as { params?: { questionId?: string } }).params?.questionId).toBe(
        'q1',
      );
      expect((markCalls[1]?.[0] as { params?: { questionId?: string } }).params?.questionId).toBe(
        'q2',
      );
    });
...
describe('resolveTargets guardrail: webapp-backed resolution', () => {
  it('uses deliveryTargetsPort and does NOT call readPort for user.lookup', async () => {
    ...
    const getTargetsByPhone = vi
      .fn()
      .mockResolvedValue({ channelBindings: { telegramId: '555', maxId: 'max-1' } });
  ...
  it('falls back to phoneNormalized address when deliveryTargetsPort returns null bindings', async () => {
    ...
    expect(targets).toEqual([
      { resource: 'telegram', address: { phoneNormalized: '+79991234567' } },
    ]);
    expect(readDb).not.toHaveBeenCalled();
  });
  ...
  it('webapp.phoneMessengerBind.complete login shows phoneAuthAccountCreated with main menu keyboard', async () => {
    ...
    const renderTemplate = vi.fn().mockImplementation(async (input: { templateId?: string }) => {
      if (input.templateId === 'phoneAuthAccountCreated')
        return { text: 'Аккаунт создан. Код 123456.' };
    ...
    expect(writeDb).toHaveBeenCalled();
    const replyKeyboard = result.intents?.find(
      (i) => i.type === 'message.send' && (i.payload as { replyMarkup?: unknown }).replyMarkup,
    );
  ...
  it('webapp.phoneMessengerBind.complete login on max shows phoneAuthAccountCreated with inline main menu', async () => {
    ...
    const maxCtx: DomainContext = {
      ...
      base: {
        ...ctx.base,
        conversationState: 'await_phoneauth:auth_maxtoken',
        facts: { links: { webappHomeUrl: 'https://app.example/home' } },
      },
      event: {
        type: 'message.received',
        meta: {
        ...
        payload: {
          incoming: {
            ...
            channelId: 'max-u-1',
            phone: '+79991234567',
            userState: 'await_phoneauth:auth_maxtoken',
          },
        },
      },
    };
    ...
    const result = await executeAction(
      {
        ...
        type: 'webapp.phoneMessengerBind.complete',
        mode: 'sync',
        params: { channelCode: 'max', externalId: 'max-u-1' },
      },
      maxCtx,
      { webappEventsPort, templatePort: { renderTemplate }, writePort: { writeDb }, contentPort },
    );
    expect(result.status).toBe('success');
    expect(renderTemplate).toHaveBeenCalledWith(
    ...
    const urlIntent = result.intents?.find((i) => {
      const markup = (
        i.payload as { replyMarkup?: { inline_keyboard?: Array<Array<{ url?: string }>> } }
      ).replyMarkup;
  ...
  it('webapp.phoneMessengerBind.complete login replay shows menu without duplicate code text', async () => {
    ...
    expect(result.status).toBe('success');
    expect(renderTemplate).toHaveBeenCalledWith(
      expect.objectContaining({ templateId: 'chooseMenu', source: 'telegram' }),
    );
    expect(renderTemplate).not.toHaveBeenCalledWith(
      expect.objectContaining({ templateId: 'phoneAuthReturnToApp' }),
    );
  ...
  it('webapp.phoneMessengerBind.complete fails without success when userPhoneLinkApplied is false', async () => {
    ...
    const writeDb = vi.fn().mockImplementation(async (mutation: { type?: string }) => {
      if (mutation.type === 'user.phone.link') {
        return { userPhoneLinkApplied: false, phoneLinkReason: 'phone_owned_by_other_user' };
      }
      return undefined;
    });
    const tgCtx: DomainContext = {
    ...
    const renderTemplate = vi.fn().mockResolvedValue({ text: 'Ошибка привязки' });
    const result = await executeAction(
      {
        id: 'pa-link-fail',
        type: 'webapp.phoneMessengerBind.complete',
  ...
  it('webapp.phoneMessengerBind.complete fails without success when phone link is indeterminate', async () => {
    const completePhoneMessengerBind = vi.fn().mockResolvedValue({
      ok: true,
      purpose: 'login',
      otpCode: '123456',
    ...
    const renderTemplate = vi.fn().mockResolvedValue({ text: 'Повторите позже' });
    const result = await executeAction(
      {
        id: 'pa-indet',
        type: 'webapp.phoneMessengerBind.complete',
    ...
    expect(result.status).toBe('failed');
    expect(renderTemplate).toHaveBeenCalledWith(
      expect.objectContaining({ templateId: 'phoneAuthFailed', source: 'telegram' }),
    );
  });
  ...
  it('webapp.phoneMessengerBind.complete uses phoneAuthMismatch template on phone_mismatch', async () => {
    const completePhoneMessengerBind = vi
      .fn()
      .mockResolvedValue({ ok: false, error: 'phone_mismatch' });
    const webappEventsPort = {
      completePhoneMessengerBind,
      emit: vi.fn(),
      listSymptomTrackings: vi.fn(),
