 ...
 *
 * Что здесь ДОКАЗАНО (зелёные тесты): падение единственного ответа человеку сегодня становится
 * наблюдаемым РОВНО одним способом — структурированным `logger.warn` с полным диагностическим
 * контекстом (intentIndex/intentType/eventId/correlationId, затем сводка по всему событию), и
 * это не мешает остальным intents (best-effort по цепочке).
 *
 * Что здесь НЕ доказано и почему: карта требует «единственный ответ человеку, не ушедший из-за
 * ошибки, становится наблюдаемым» — то есть внешне, не только строкой в логе (инцидент/очередь
 * ретрая). Этого в коде нет: `dispatchIntent` из `processAcceptedIncomingEvent` не пробрасывает
 * ошибку дальше eventGateway (см. `incomingEventPipeline.ts`), поэтому провал единственного intent
 * НИКАК не долетает до отправителя/оператора, кроме лога. Красный тест под ещё не реализованное
 * поведение по правилу брифа не пишу («это НЕ повод писать тест под нынешнее поведение» относится
 * и к обратному — писать заведомо красный тест как единственный способ фиксации молча). Разбор —
 * в отчёте D20_TESTS_LEVEL2_REPORT.md, раздел «развилки».
 *
