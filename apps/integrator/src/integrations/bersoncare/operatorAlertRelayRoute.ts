...
function validSignature(timestamp: string, rawBody: string, signature: string, secret: string): boolean {
  const ts = Number(timestamp);
  if (!Number.isFinite(ts) || Math.abs(Math.floor(Date.now() / 1000) - ts) > WINDOW_SECONDS) return false;
...
function buildIntent(payload: Payload): OutgoingIntent {
  ...
  if (payload.channel === 'telegram' || payload.channel === 'max') {
    return {
      type: 'message.send', meta,
      payload: {
        recipient: payload.channel === 'max' ? { userId: payload.recipient } : { chatId: payload.recipient },
        message: { text: payload.text }, delivery: { channels: [payload.channel] },
      },
    };
  }
  ...
  return { type: 'message.send', meta: { ...meta, source: 'web_push' }, payload: {
...
export async function registerOperatorAlertRelayRoute(app: FastifyInstance, deps: OperatorAlertRelayDeps): Promise<void> {
  ...
  app.post('/api/bersoncare/operator-alert-relay', async (request, reply) => {
    ...
    if (typeof timestamp !== 'string' || typeof signature !== 'string') return reply.code(400).send({ ok: false, error: 'missing_headers' });
    if (!deps.sharedSecret) return reply.code(503).send({ ok: false, error: 'service_unconfigured' });
    if (!validSignature(timestamp, rawBody, signature, deps.sharedSecret)) return reply.code(401).send({ ok: false, error: 'invalid_signature' });
    ...
    if (expiry !== undefined && Date.now() <= expiry) return reply.code(200).send({ ok: true, status: 'duplicate' });
    if (inFlight.has(key)) return reply.code(503).send({ ok: false, error: 'dispatch_in_flight' });
    inFlight.add(key);
    try {
      await runWithOptionalOrganizationPrincipal(payload.organizationId, () => deps.dispatchPort.dispatchOutgoing(buildIntent(payload)));
      dedup.set(key, Date.now() + DEDUP_TTL_MS);
      return reply.code(200).send({ ok: true, status: 'accepted' });
    } catch (error) {
      if (isOutboundMessagePolicyDenied(error)) return reply.code(403).send({ ok: false, error: 'egress_policy_denied' });
