...
import { normalizeToUtcInstant, tryNormalizeToUtcInstant } from './normalizeToUtcInstant.js';

describe('normalizeToUtcInstant', () => {
  it('S2: naive space + Europe/Moscow → UTC', () => {
    expect(normalizeToUtcInstant('2026-04-07 11:00:00', 'Europe/Moscow')).toBe(
      '2026-04-07T08:00:00.000Z',
    );
  });

  it('S2: naive space + Europe/Samara → UTC (proves IANA, not MSK hardcode)', () => {
    expect(normalizeToUtcInstant('2026-04-07 11:00:00', 'Europe/Samara')).toBe(
  ...
  it('S2: ISO with explicit positive offset → UTC', () => {
    expect(normalizeToUtcInstant('2026-04-07T11:00:00+03:00', 'Europe/Moscow')).toBe(
      '2026-04-07T08:00:00.000Z',
    );
  });

  it('S2: naive T-separator + Europe/Moscow → UTC', () => {
    expect(normalizeToUtcInstant('2026-04-07T11:00:00', 'Europe/Moscow')).toBe(
      '2026-04-07T08:00:00.000Z',
    );
  });
  ...
  it('naive with fractional seconds via IANA zone', () => {
    expect(normalizeToUtcInstant('2026-04-07 11:00:00.5', 'Etc/UTC')).toBe(
      '2026-04-07T11:00:00.500Z',
    );
    expect(normalizeToUtcInstant('2026-04-07T11:00:00.123456', 'Etc/UTC')).toBe(
      '2026-04-07T11:00:00.123Z',
    );
  });
  ...
  it('ISO with numeric offset without colon → UTC', () => {
    expect(normalizeToUtcInstant('2026-04-07T11:00:00+0300', 'Europe/Moscow')).toBe(
      '2026-04-07T08:00:00.000Z',
    );
  });
  ...
  it('America/New_York summer wall time → deterministic UTC', () => {
    expect(normalizeToUtcInstant('2024-07-15 12:00:00', 'America/New_York')).toBe(
  ...
  it('spring-forward gap: Luxon maps non-existent wall time to next valid local (IANA-based, deterministic)', () => {
    expect(normalizeToUtcInstant('2024-03-10 02:30:00', 'America/New_York')).toBe(
  ...
  it('negative numeric offset → UTC', () => {
    expect(normalizeToUtcInstant('2026-04-07T11:00:00-05:00', 'Europe/Moscow')).toBe(
