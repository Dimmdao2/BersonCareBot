...
describe('resolveLinkedPhoneNormalized', () => {
  it('public_then_contacts prefers public when both set', () => {
    expect(
      resolveLinkedPhoneNormalized('public_then_contacts', '+70001112233', '+79990001122'),
    ).toBe('+70001112233');
  });

  it('public_then_contacts falls back to legacy when public empty', () => {
    expect(resolveLinkedPhoneNormalized('public_then_contacts', null, '+79990001122')).toBe(
      '+79990001122',
    );
    expect(resolveLinkedPhoneNormalized('public_then_contacts', '   ', '+79990001122')).toBe(
      '+79990001122',
    );
  });
  ...
  it('contacts_only ignores public', () => {
    ...
    expect(resolveLinkedPhoneNormalized('contacts_only', null, '+79990001122')).toBe(
      '+79990001122',
    );
  });
});
