...
function makeMockDb(capture: {
  ...
  const query = vi.fn(async (sql: string, params: unknown[]) => {
    ...
    if (sql.includes('SELECT ri.user_id::text AS id')) {
      return { rows: [{ id: 'uid-tg', channel_id: '123' }] } as Awaited<
    ...
    if (
      sql.includes('SELECT user_id::text AS user_id FROM identities') &&
      !sql.includes('FROM identities i')
    ) {
  ...
  const tx = vi.fn(async (fn: (txDb: DbPort) => Promise<void>) =>
    fn({ query, tx, integratorDrizzle: drizzle } as DbPort),
  );
...
function makeDirectWriteMockDb(overrides?: {
  ...
  const tx = vi.fn(async <T>(fn: (txDb: DbPort) => Promise<T>) => {
    const txDb = {
      query,
      tx: vi.fn(async <N>(nested: (inner: DbPort) => Promise<N>) => nested(txDb)),
...
function makeWriteWrapperDb(): {
  db: DbPort;
  query: ReturnType<typeof vi.fn>;
  tx: ReturnType<typeof vi.fn>;
} {
  const query = vi.fn(async () => ({ rows: [], rowCount: 1 })) as DbPort['query'] &
...
describe('writePort user.upsert projection payload', () => {
  ...
  it('D1: user.upsert silently no-ops for a non-numeric telegram externalId (channel anchor unresolved)', async () => {
    ...
    expect(
      queries.some(
        (q) =>
          q.sql.includes('public.platform_users') || q.sql.includes('public.user_channel_bindings'),
      ),
    ).toBe(false);
  });
  ...
  it('D1: user.upsert defers (no write, no throw) when candidate resolution is ambiguous', async () => {
    ...
    expect(
      queries.some(
        (q) =>
          q.sql.includes('INSERT INTO public.platform_users') ||
          q.sql.includes('UPDATE public.platform_users'),
      ),
    ).toBe(false);
  });
  ...
  it('user.phone.link strict no_channel_binding: no integrator contacts write', async () => {
    ...
    const query = vi.fn(async (sql: string, params: unknown[]) => {
      ...
      if (
        ...
        return { rows: [{ user_id: 'uid-tg' }], rowCount: 1 } as Awaited<
          ReturnType<DbPort['query']>
        >;
      }
      if (sql.includes('INSERT INTO contacts') && sql.includes('ON CONFLICT')) {
  ...
  it('D1: notifications.update writes public.user_notification_topics directly (no preferences.updated fanout)', async () => {
    ...
    expect(queries.some((q) => q.sql.includes('INSERT INTO public.user_channel_bindings'))).toBe(
      false,
    );

    const topicInserts = queries.filter((q) =>
      q.sql.includes('INSERT INTO public.user_notification_topics'),
    );
  ...
  it('D1: notifications.update no-ops when no integrator identity is linked yet', async () => {
    ...
    expect(
      queries.some(
        (q) =>
          q.sql.includes('public.platform_users') ||
          q.sql.includes('public.user_notification_topics'),
      ),
    ).toBe(false);
  });
