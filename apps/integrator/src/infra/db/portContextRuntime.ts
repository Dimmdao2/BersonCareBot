...
function pem(path: string, name: string): string {
  try { return readFileSync(path, 'utf8'); } catch { throw new Error(`${name} must name a readable PEM file`); }
}
...
export function createIntegratorPortContextRuntimeConfig(env: Record<string, string | undefined>): IntegratorPortContextRuntimeConfig {
  ...
  for (const [name, value] of Object.entries(parsed)) {
    if (typeof value !== 'object' || value === null || Array.isArray(value)) throw new Error(`port capability ${name} must be an object`);
  ...
  return {
    pool: {
      ...
      ssl: {
        ...
        ca: pem(requireValue(env.INTEGRATOR_DB_TLS_CA_FILE, 'INTEGRATOR_DB_TLS_CA_FILE'), 'INTEGRATOR_DB_TLS_CA_FILE'),
        cert: pem(requireValue(env.INTEGRATOR_DB_TLS_CERT_FILE, 'INTEGRATOR_DB_TLS_CERT_FILE'), 'INTEGRATOR_DB_TLS_CERT_FILE'),
        key: pem(requireValue(env.INTEGRATOR_DB_TLS_KEY_FILE, 'INTEGRATOR_DB_TLS_KEY_FILE'), 'INTEGRATOR_DB_TLS_KEY_FILE'),
...
export function integratorPortContextPrincipal(
  ...
  switch (descriptor.contextClass) {
    case 'integrator':
      if (principal.kind !== 'integrator') throw new Error('Integrator request capability requires an integrator principal');
      return { ...base, integratorUserId: principal.integratorUserId, organizationId: principal.organizationId };
    case 'tenant_service':
      if (principal.kind !== 'organization') throw new Error('Tenant service capability requires an organization principal');
      return { ...base, organizationId: principal.organizationId };
    case 'service':
      if (principal.kind !== 'infra') throw new Error('Service capability requires an explicit infra principal');
      return base;
    case 'pre_session':
      if (principal.kind !== 'bootstrap' || !descriptor.functionIdentity) throw new Error('Resolver capability requires an explicit named-root descriptor');
      return { ...base, requestId: randomUUID() };
    default:
      throw new Error(`Integrator capability ${key} has unsupported context class ${descriptor.contextClass}`);
  }
}
