...
**Режим разработки и тестирование UI:** полный гайд для агентов — [`docs/ARCHITECTURE/LOCAL_DEV_AND_AGENT_TESTING.md`](../../docs/ARCHITECTURE/LOCAL_DEV_AND_AGENT_TESTING.md) (обычный owner login, `pnpm dev` / `webapp:dev` / `dev:turbo` / `dev:visual`, порты).

**Режим разработки (вход в браузере):** используйте обычный email/password, OAuth или messenger-вход уже зарегистрированной owner-учётки. Чистый public/login: `/api/auth/dev-public`; регистрация кабинета: `/api/auth/dev-public?view=registration` (без fake authenticated role). Полная матрица ролей и применение pending migrations к существующей точной `bcb_webapp_dev` через `deploy/host/migrate-dev.sh` — в гайде выше.
