...
- **Messenger phone bind (unified DB):** the canonical bind updates `public.platform_users` / `public.user_channel_bindings`; orchestrator link-data reads the confirmed phone only from the terminal canonical `public.platform_users` identity. Stable machine reasons and user copy: `docs/archive/2026-04-initiatives/WEBAPP_FIRST_PHONE_BIND/PRODUCT_REASONS_AND_UX_TABLE.md`.
- **Optional `POST /api/integrator/messenger-phone/bind` (M2M, external caller only):** implemented in webapp as `executeMessengerPhoneHttpBind`; integrator's parallel `user.phone.link` write was retired 2026-08-26. **Do not** use this from the normal Telegram/Max phone-link hot path when integrator and webapp share one PostgreSQL. **Auth:** `x-bersoncare-timestamp`, `x-bersoncare-signature` (HMAC-SHA256 of `timestamp`, a dot and raw JSON, with `INTEGRATOR_WEBHOOK_SECRET`, base64url), **`x-bersoncare-idempotency-key`** (required, max 256 chars). **Body:** `channelCode` (`telegram` \| `max`), `externalId`, `phoneNormalized`; optional `correlationId`, optional matching `idempotencyKey`. **Idempotency:** only success is cached; semantic hash covers those three identity fields. **Responses:** **`200`** `{ ok: true, platformUserId, idempotencyKey }` · **`409`** `{ ok: false, reason: "human_account_confirmation_required", continuationUrl, idempotencyKey }` — continue in the browser where the shared account/FIO prompt lives · **`422`** other deterministic refusals · **`503`** `{ ok: false, reason: "db_transient_failure", idempotencyKey, indeterminate?: true }` · **`401`** invalid signature · **`400`** invalid request. Checklist: `docs/archive/2026-04-initiatives/WEBAPP_FIRST_PHONE_BIND/STAGE_06_OPTIONAL_HTTP_BIND_ROUTE.md`.
- **Phone messenger bind complete (`POST /api/integrator/phone-messenger-bind/complete`):** same signed headers as channel-link M2M. **Body:** `setupToken` (`auth_*`), `channelCode`, `externalId`, provider-proven `phoneNormalized`. It creates an OTP challenge and moves the secret to `otp_ready`; integrator never writes the canonical contact. **`purpose: login`** returns `{ ok: true, purpose: "login", otpCode, accountCreated, challengeId }`; **`purpose: profile_bind`** returns `{ ok: true, purpose: "profile_bind" }`. For both purposes the authorized PWA poll completes through `POST /api/auth/phone/messenger-bind/finish`, which consumes the stored OTP server-side and owns the canonical bind and shared account/FIO prompt. Replay at `otp_ready` is idempotent; `used_token` returns `{ ok: true, status: "already_used" }`. Integrator action: `webapp.phoneMessengerBind.complete`; `/start auth_*` → `start.phoneauth`; state `await_phoneauth:<token>` until contact. Runbook: `docs/OPERATIONS/PHONE_MESSENGER_AUTH_RUNBOOK.md`.
- **Channel-link complete (`POST /api/integrator/channel-link/complete`):** same signed headers as other integrator M2M POSTs (`x-bersoncare-timestamp`, `x-bersoncare-signature` over `timestamp + "." + rawBody`, `INTEGRATOR_WEBHOOK_SECRET`, base64url). **Body (JSON):** `linkToken` (`link_*`), `channelCode` (`telegram` \| `max`), `externalId`. **200** `{ ok: true, needsPhone?, phoneNormalized? }` on success; **200** `{ ok: true, status: "already_used", needsPhone }` when the token was already consumed (idempotent replay). **409** `{ ok: false, error: "conflict", mergeReason? }` when the row in `user_channel_bindings` cannot be reconciled with the token user — stable webapp codes include `channel_owned_by_real_user`, `channel_link_claim_rejected`, `channel_link_claim_failed` (see `apps/webapp/src/modules/auth/channelLink.ts`, `channelLinkClaim.ts`). **Integrator:** `webappEventsClient.completeChannelLink` passes **`mergeReason` in preference to `error`** when the HTTP response is not OK, so `executeAction` / `channelLinkCompleteFailureTemplateKey` can choose `channelLink.completeFailed.conflict` vs `…generic` vs `…expired`. **After HTTP success** with `phoneNormalized`, the scenario step `webapp.channelLink.complete` sends the welcome template directly (`max:afterChannelLinked` / Telegram welcome keyboard) — `result.phoneNormalized` is already the canonical webapp phone (read from `platform_users`/`user_contacts` to compute `needsPhone`, not observed from the provider), so there is no integrator-side write left to run (`user.phone.link` was retired 2026-08-26; re-writing an already-canonical value back into integrator's own DB was always a no-op duplicate). Product copy and policy: `apps/webapp/src/modules/auth/auth.md` (§Channel link → integrator). Plan / closure: [`.cursor/plans/archive/phone_bind_mismatch_ux.plan.md`](../../.cursor/plans/archive/phone_bind_mismatch_ux.plan.md).
...
## Flow 6c: BersonCare → Integrator (telegram-bot-identity)
...
**Метод и URL:** `POST {INTEGRATOR_API_URL}/api/bersoncare/telegram-bot-identity`
...
Текст ответа Telegram наружу и в логи **не попадает**: на неверный токен Bot API возвращает его в описании запроса.
