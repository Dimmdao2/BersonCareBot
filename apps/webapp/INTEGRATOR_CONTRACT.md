...
- **Phone messenger bind complete (`POST /api/integrator/phone-messenger-bind/complete`):** same signed headers as channel-link M2M. **Body (JSON):** `setupToken` (`auth_*` from deep link), `channelCode` (`telegram` \| `max`), `externalId`, `phoneNormalized` (contact from messenger, E.164). **200** `{ ok: true, otpCode, accountCreated, challengeId }` — OTP already created in webapp; integrator sends user template `*:phoneAuthAccountCreated` with `{{code}}` (does not call send-otp again). **409** `phone_mismatch` \| `conflict` + `mergeReason` (`phone_owned_by_other_user`, `channel_owned_by_other_user`). **200** `{ ok: true, status: "already_used" }` on consumed token replay. **Integrator:** action `webapp.phoneMessengerBind.complete`; `/start auth_*` → `start.phoneauth`; state `await_phoneauth:<token>` until contact. **Webapp client:** `POST /api/auth/phone/messenger-bind/start` (purpose `login` \| `profile_bind`), `POST /api/auth/phone/messenger-bind/status` until `otp_ready`. See `apps/webapp/src/modules/auth/auth.md`.
