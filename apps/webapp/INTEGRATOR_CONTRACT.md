...
Входящее сообщение пациента из бота в thread `webapp:platform:{platformUserId}`. После записи в thread — уведомление staff (doctor/admin) по матрице `/app/settings` (topics `doctor_patient_messages` / `doctor_patient_program_notes`: **по умолчанию** `web_push` + `telegram` + `max` при наличии подписки/привязок; fallback env `doctor_telegram_ids` / `admin_telegram_ids` только если per-staff telegram/max не доставлены).
