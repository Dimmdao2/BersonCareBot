...
- **ДЕЙСТВУЮЩЕЕ ПРАВИЛО (владелец 30.07 велел убрать путающую легаси-формулировку):** при единой PostgreSQL интегратор
  читает и пишет `public` — но **только через свой слой портов** (`apps/integrator/src/infra/db/**`: репозитории и
  транзакции), никогда из доменного или сценарного кода напрямую. M2M HTTP остаётся исключительно для того, что
  действительно является контрактом на границе процессов (идемпотентность, авторизация, маршрутизация), и **не** является
  способом писать канонические данные. Прежняя формулировка «no direct database reads between services» ОТМЕНЕНА и в
  живых документах не повторяется: она относилась ко временам разных баз и путала агентов, заставляя их возвращать HTTP
  там, где нужен обычный SQL через порт. История правила — в git, отдельного архивного файла для неё не держим.
