...
- **Phone messenger bind complete (`POST /api/integrator/phone-messenger-bind/complete`):** same signed headers as channel-link M2M. **Body (JSON):** `setupToken` (`auth_*` from deep link), `channelCode` (`telegram` \| `max`), `externalId`, `phoneNormalized` (contact from messenger, E.164). **200** `{ ok: true, otpCode, accountCreated, challengeId }` — OTP already created in webapp; integrator sends user template `*:phoneAuthAccountCreated` with `{{code}}` (does not call send-otp again). **200** `{ ok: true, otpCode, accountCreated, challengeId, replay?: true }` when secret is already **`otp_ready`** (idempotent replay of contact before `phone/confirm` consumed the secret) — same `challengeId` and code from challenge store. **409** `phone_mismatch` \| `conflict` + `mergeReason` (`phone_owned_by_other_user`, `channel_owned_by_other_user`). **200** `{ ok: true, status: "already_used" }` when secret is **`consumed`** / `used_token` (no `otpCode`). **Integrator:** action `webapp.phoneMessengerBind.complete`; `/start auth_*` → `start.phoneauth`; state `await_phoneauth:<token>` until contact. **Webapp client:** `POST /api/auth/phone/messenger-bind/start` (purpose `login` \| `profile_bind`), `POST /api/auth/phone/messenger-bind/status` until `otp_ready`. Runbook: `docs/OPERATIONS/PHONE_MESSENGER_AUTH_RUNBOOK.md`. Module doc: `apps/webapp/src/modules/auth/auth.md` (§ Phone messenger bind).
