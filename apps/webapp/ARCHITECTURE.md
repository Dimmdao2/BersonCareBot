...
- **Один кластер PostgreSQL, но логин и роли у приложений РАЗНЫЕ.** DEV и TEST разведены по отдельным логинам:
  интегратор ходит под `bcb_*_integrator` (`canonicalRole: 'app_integrator_request'`), вебапп — под тремя логинами
  по принципалу (`*_webapp_staff` / `*_webapp_patient` / `*_webapp_global_admin`). У логина интегратора нет
  членства в широкой роли вебаппа `app_tenant_service`: организационный реляционный доступ идёт через
  `app_integrator_tenant_service`, а доставка и разрешение личности — через отдельные роли своего назначения.
  Канон — `REV10_ENV_MAPPING` в `deploy/postgres/privileges/declaration.ts`; по PROD сверяйся с фактическим env на
  прод-хосте, а не с этим файлом.
- **Узкая роль не означает ноль записей в `public`.** `app_integrator_tenant_service` имеет колоночные права ровно на
  восемь отношений под `FORCE RLS`: два отношения `integrator.*` допускают запись, шесть `public.*` — только чтение;
  медицинских таблиц среди них нет. При этом интегратор намеренно вызывает ограниченные именованные
  `SECURITY DEFINER`-корни, которые пишут перечисленные поверхности `public` — идентичность и привязки каналов,
  правила/историю/доставку напоминаний, доставочные и аудиторские события, очередь и календарное отображение.
  Поэтому точный контракт — «нет широкого или прямого табличного доступа к медицинскому канону», а не «нет ни одного
  пути записи канона». Все обращения проходят через общий DB-port/транзакционный chokepoint, описанный в
  `docs/ARCHITECTURE/DATABASE_UNIFIED_POSTGRES.md`; модули вебаппа интегратор не импортирует.
...
**Права:** в декларации DEV/TEST логин интегратора уже отделён от `app_tenant_service` и получает узкие роли приёма,
разрешения личности, организационного чтения, расписания и доставки. Прямых табличных прав на медицинские отношения у
него нет; восемь отношений узкой организационной роли закрыты `FORCE RLS`. Запись в `public` не запрещена целиком:
она остаётся только у названных relation/capability-путей и именованных корней с объявленной поверхностью. План работ —
`docs/_TODO/UI_FINISH_AND_REAUDIT_2026-07-22/WORK_ORDER.md`, Track D-полный.
