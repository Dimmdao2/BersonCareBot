...
-- BCB-MIGRATION-VERIFY: SELECT position('auditErrorCount' in pg_get_functiondef(to_regprocedure('app.read_operator_health_digest_window(timestamp with time zone,timestamp with time zone)'))) > 0
-- The daily digest used five direct relation reads. The audit-log read was forbidden to the
-- runtime worker, so the whole digest failed after entering its send slot. Keep the raw journals
-- closed and expose exactly the one window snapshot the digest consumes.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'incidentsOpened', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object('integration', incident.integration, 'errorClass', incident.error_class)
