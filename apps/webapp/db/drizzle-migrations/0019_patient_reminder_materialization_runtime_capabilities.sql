...
-- root. The write root is the only runtime surface that can compose occurrence state with the
-- ready delivery queue; the historical split roots lose every runtime caller below.
