-- TEMPORARY LOCAL MIGRATION NUMBER 0376 -- final number assigned at merge.
-- #1069 T9: catalog-backed tariff billing periods (`day` retired from selectable set).
-- #1069 T10: global post-paid-period behavior (distinct from post-trial policy).

CREATE TABLE IF NOT EXISTS public.saas_billing_periods (
...
INSERT INTO public.saas_billing_periods (code, label, months, is_selectable, sort_order)
VALUES
  ('day', 'День (снят)', 1, false, 0),
  ('month', 'Месяц', 1, true, 10),
  ('half_year', 'Полгода', 6, true, 20),
  ('year', 'Год', 12, true, 30)
...
ALTER TABLE public.saas_tariffs
  DROP CONSTRAINT IF EXISTS saas_tariffs_billing_period_fkey;
--> statement-breakpoint

ALTER TABLE public.saas_tariffs
  ADD CONSTRAINT saas_tariffs_billing_period_fkey
  FOREIGN KEY (billing_period) REFERENCES public.saas_billing_periods(code);
--> statement-breakpoint

ALTER TABLE public.saas_billing_invoices
  DROP CONSTRAINT IF EXISTS saas_billing_invoices_tariff_billing_period_check;
--> statement-breakpoint

ALTER TABLE public.saas_billing_invoices
  DROP CONSTRAINT IF EXISTS saas_billing_invoices_tariff_billing_period_fkey;
--> statement-breakpoint

ALTER TABLE public.saas_billing_invoices
  ADD CONSTRAINT saas_billing_invoices_tariff_billing_period_fkey
  FOREIGN KEY (tariff_billing_period) REFERENCES public.saas_billing_periods(code);
--> statement-breakpoint

CREATE TABLE IF NOT EXISTS public.saas_paid_period_policy (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT saas_paid_period_policy_key_check CHECK (key = 'global'),
  CONSTRAINT saas_paid_period_policy_post_behavior_check
    CHECK (post_paid_period_behavior = ANY (ARRAY['read_only'::text, 'blocked'::text, 'tariff'::text])),
  CONSTRAINT saas_paid_period_policy_post_tariff_check
    CHECK (
      (post_paid_period_behavior = 'tariff' AND post_paid_period_tariff_id IS NOT NULL)
      OR (post_paid_period_behavior <> 'tariff' AND post_paid_period_tariff_id IS NULL)
    ),
  CONSTRAINT saas_paid_period_policy_post_tariff_id_fkey
    FOREIGN KEY (post_paid_period_tariff_id) REFERENCES public.saas_tariffs(id)
);
--> statement-breakpoint
...
VALUES ('global', 'read_only', NULL, true)
