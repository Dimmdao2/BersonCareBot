...
-- BCB-MIGRATION-VERIFY: SELECT pg_catalog.pg_get_functiondef('app.read_current_patient_booking_slot_snapshot(uuid,uuid,text,text)'::regprocedure) LIKE '%COALESCE(v_min_notice_text::integer, 0)%' AND pg_catalog.pg_get_functiondef('app.read_current_patient_booking_slot_snapshot(uuid,uuid,text,text)'::regprocedure) LIKE '%COALESCE(v_max_consecutive_slot_text::integer, 3)%'
--
-- Live audit 2026-09-12 (same repro as 20260912T094500): once the service step could reach the
-- slot step, the booking wizard still bounced back — this time from
-- `app.read_current_patient_booking_creation_snapshot` calling this function, which raises
-- SQLSTATE 22023 "patient booking runtime settings are unavailable" whenever an organization has
-- no per-org row for `booking_min_notice_hours` or `booking_max_consecutive_slot_hours` — the exact
-- same gap as the sibling door `app.read_current_patient_booking_runtime_integer` (already closed
-- by the previous migration) and the public twin `app.read_public_booking_slot_snapshot` (already
-- tolerant via COALESCE). A stored-but-invalid value still fails loud below; only a genuinely
-- missing row now takes its `registry.ts` default (booking_min_notice_hours='0',
-- booking_max_consecutive_slot_hours='3').
