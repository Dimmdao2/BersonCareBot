...
-- BCB-MIGRATION-VERIFY: SELECT position('blockedCount' in pg_get_functiondef(to_regprocedure('app.read_curated_system_health()'))) > 0
-- Сколько файлов ждут починки среды — теперь приезжает той же дверью, что и остальное здоровье.
--
-- До этого счёт шёл прямым `SELECT count(*) FROM media_files` из сборщика здоровья. В rev10-режиме
-- роль выбирает порт-контекст, у тика здоровья это `app_worker`, а у `app_worker` прав на
-- `media_files` нет и по стене быть не должно: сборщик здоровья не арендатор. На проде это 119 раз
-- в сутки `permission denied for table media_files` в логе и ноль вместо сигнала на панели.
--
-- Новых прав не появляется: функция УЖЕ читает `public.media_files` и уже считает по
-- `preview_status`. Добавлена одна цифра в тот же объект `mediaPreview`.
...
WITH media_preview AS MATERIALIZED (
  SELECT jsonb_build_object(
    ...
    'blockedCount', count(*) FILTER (WHERE preview_status = 'blocked'),
