...
-- BCB-MIGRATION-VERIFY: SELECT pg_catalog.pg_get_functiondef('app.process_media_pending_delete_step(text,uuid,integer,uuid)'::regprocedure) LIKE '%''storageTarget'', leased.storage_target%'
--
-- Уборщик удаляет объекты по выданной ему аренде и до этой правки не знал, в каком бакете они
-- лежат: он бы попробовал удалить видео пациента в библиотечном хранилище, не нашёл его,
-- отчитался об успехе и оставил файл лежать в шифрованном бакете навсегда.
--
-- Поэтому аренда несёт хранилище строки наравне с ключами.

CREATE OR REPLACE FUNCTION app.process_media_pending_delete_step(p_action text, p_media_id uuid, p_limit integer, p_claim_token uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
