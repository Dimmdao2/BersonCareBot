...
-- BCB-MIGRATION-VERIFY: SELECT NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'user_phone_history' AND column_name = 'organization_id') AND position('organization_id' in pg_catalog.pg_get_functiondef('app.pre_session_phone_confirm_resolve(text,text,boolean,text)'::regprocedure)) = 0 AND position('user_phone_history' in pg_catalog.pg_get_functiondef('app.pre_session_messenger_channel_resolve(text,text,text,text,text,uuid)'::regprocedure)) > 0
--
-- TENANT_WALL_DEBT_2026-09-12. РЕШЕНИЕ ВЛАДЕЛЬЦА 12.09.2026, дословно: «История принадлежит
-- человеку». Вопрос был поставлен ровно как развилка — «стена по клинике» против «история
-- принадлежит человеку», — и выбран второй вариант. Отсюда эта миграция: колонка
-- `public.user_phone_history.organization_id` удаляется целиком.
--
-- ЧТО ОНА НА САМОМ ДЕЛЕ ЗНАЧИЛА. Колонка ОБЕЩАЛА стену арендатора, которой никогда не было.
-- Замерено контекст-флипом на DEV: сессия клиники B читала строки человека, помеченные клиникой A
-- (B_SEES_A=1). Это не сломанная стена, а стена, которой не существовало: политика персонала
-- `rev10_staff_member_managed_226` спрашивает НЕ КОЛОНКУ, а членство ЧЕЛОВЕКА
-- (EXISTS по be_organization_members / org_enrollments) — собственный `organization_id` строки не
-- проверял НИКТО. Пациентская политика `rev10_patient_self_managed_226` тем более: она про свою
-- строку, `platform_user_id = app.current_actor_user_id()`.
--
-- То же говорят и данные, и код. Оба pre-session корня, подтверждающие номер, писали в эту колонку
-- ЛИТЕРАЛЬНЫЙ NULL — то есть главный путь подтверждения телефона клинику в ней не проставлял
-- вовсе. Живых строк на DEV 94, и ни одной с NULL: их проставлял единственный прикладной писатель
-- `apps/webapp/src/infra/repos/pgPhoneHistory.ts`, беря клинику из текущего принципала — то есть
-- колонка хранила «где человек был в момент смены номера», а не «чья это запись».
--
-- ЧТО СТАНОВИТСЯ СТЕНОЙ. Ровно то, что ею и было, только теперь это единственное написанное:
-- пациент — своя строка по `app.current_actor_user_id()`; персонал и tenant-service — человек
-- состоит в ТЕКУЩЕЙ клинике. Для tenant-service стена переезжает с прямой колонки на членство
-- человека (`declaration.ts`: из REV10_TENANT_DIRECT_ORG в REV10_TENANT_MEMBERSHIP_BASE по
-- `platform_user_id`) — это не новый механизм, а тот же, что уже держит ветку персонала.
--
-- ПОЧЕМУ ЗДЕСЬ ЕСТЬ `DROP POLICY`, НО НЕТ НИ ОДНОГО `CREATE POLICY`. Права и политики живут в
-- артефакте прав (`deploy/postgres/privileges/declaration.ts` → `deploy/postgres/generated/`) и
-- накатываются reconcile'ом ПОСЛЕ миграций; миграции их не сочиняют. Но удалить колонку нельзя,
-- пока на неё ссылаются политики: Postgres называет ровно три зависимости —
-- `rev10_tenant_insert_226`, `rev10_tenant_select_226`, `rev10_tenant_update_226`. Они сняты
-- ПОИМЁННО, а не `DROP COLUMN ... CASCADE`: каскад снёс бы то же самое молча, без списка в тексте
-- миграции. Reconcile тут же поднимает их заново — уже в форме по членству. Индекс
-- `idx_user_phone_history_organization_id` и внешний ключ `user_phone_history_organization_id_fkey`
-- уходят вместе с колонкой сами (auto-зависимости), поэтому отдельными строками не названы.
--
-- ПОБОЧНО ЗАКРЫВАЕТСЯ ОДНА ЖИВАЯ ПРОБЛЕМА ЧИСТКИ. FK стоял `ON DELETE CASCADE`, то есть удаление
-- КЛИНИКИ стирало номера ЧЕЛОВЕКА — прямое следствие ложного владения и прямое противоречие
-- решению владельца. Больше стирать нечему: чистка остаётся одна, по человеку
-- (`journal-lifecycle-registry.ts`, userPurge по `platform_user_id`).
--
-- Тела обоих корней приведены ЦЕЛИКОМ (иначе `CREATE OR REPLACE` не выразить). Изменение в каждом
-- РОВНО одно: из списка колонок INSERT в `public.user_phone_history` убраны `organization_id` и
-- отвечавший ему литеральный NULL. Вставка в `public.admin_audit_log` в мессенджер-корне свой
-- `organization_id` СОХРАНЯЕТ: это другая таблица, там клиника принадлежит записи журнала по праву.
-- Владелец у обоих — `app_seam_identity_lookup_owner`, поэтому сегменты свои, со своим заголовком.
CREATE OR REPLACE FUNCTION app.pre_session_phone_confirm_resolve(p_phone_normalized text, p_display_name text, p_phone_number_proven boolean, p_confirming_channel text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION app.pre_session_messenger_channel_resolve(p_channel_code text, p_external_id text, p_phone_normalized text, p_display_name text, p_confirming_channel text, p_session_user_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'app', 'public', 'pg_temp'
