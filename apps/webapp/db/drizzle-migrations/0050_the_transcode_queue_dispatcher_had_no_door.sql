...
--
-- Почему ни одна работа, поставленная после ввода порт-контекста, не могла быть взята. Роль у
-- диспетчера верная:
...
-- ветки зовут `app.current_org_id()` подзапросом, то есть InitPlan'ом, а сама `app.current_org_id()`
-- принимает контекст только ролей `app_staff | app_clinic_billing | app_patient |
-- app_integrator_request | app_tenant_service | app_worker` и на любой другой роли не возвращает
-- NULL, а поднимает 42501. Диспетчер очереди межарендный по построению: одного `organization_id`
-- у него нет и быть не может — предъявить контекст, который бы устроил этот предикат, он не может
-- в принципе.
--
-- Почему это выглядело как «сломалось вчера», а не «не работало никогда». Отказ приходит ровно
-- тогда, когда узлу есть что фильтровать: InitPlan считается при первом исполнении узла, а на
-- ПУСТОЙ очереди узел до него не доходит, и запрос спокойно отвечает «нечего делать». Воспроизведено
-- на живом bcb_webapp_dev 19.08 тем же боевым маршрутом при возвращённых старых грантах и старой
-- политике: очередь пуста — `{"kind":"idle"}` без единого отказа; кладём одну строку `pending` —
-- `42501 accepted organization context required, CONTEXT: PL/pgSQL function current_org_id()
-- line 7 at RAISE`, буква в букву как на TEST. То есть петля падения начинается с первой же работы,
-- поставленной после ввода порт-контекста, и не прекращается, пока эта работа висит.
--
