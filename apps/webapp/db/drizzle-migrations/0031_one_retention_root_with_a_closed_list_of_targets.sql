...
-- REVOKE с этой функции здесь НЕ пишется, в отличие от соседней 0029. Владелец шва
-- `app_seam_retention_sweep_owner` рождается вместе с этой работой, и на момент миграции у него
-- ещё нет USAGE на схему `app`: мигратор выдаёт ему только временный CREATE. Отзыв у PUBLIC и
-- у всех рантайм-ролей, как и единственный GRANT EXECUTE, приходит следующим шагом того же
-- прогона — из `deploy/postgres/generated/privileges.<база>.sql`, который применяет reconcile.
