...
-- merely to advance the journal. TEST/fresh installs use the privileged migration principal and
-- install the functions here; the ordinary DEV path installs the identical definition from the
-- canonical C5A runtime overlay after the migration ledger advances.
DO $clinic_billing_quota_capability_install$
BEGIN
  IF NOT has_schema_privilege(current_user, 'app', 'CREATE') THEN
    RAISE NOTICE '0332: app capability DDL deferred to the canonical runtime overlay';
    RETURN;
  END IF;

  -- The legacy accessor has had older RETURNS TABLE shapes. PostgreSQL cannot replace those in
  -- place, and the narrow wrapper depends on it after the first successful installation.
  ...
  EXECUTE $ddl$
    CREATE OR REPLACE FUNCTION app.read_org_enforced_quota_usage(p_organization_id uuid)
    RETURNS TABLE(
...
END
$clinic_billing_quota_capability_install$;

DO $clinic_billing_usage_capability$
BEGIN
  IF to_regprocedure('app.read_current_org_tariff_transition_usage()') IS NULL THEN
    IF has_schema_privilege(current_user, 'app', 'CREATE') THEN
      RAISE EXCEPTION '0332 clinic billing quota capability was not installed'
        USING ERRCODE = '42501';
    END IF;
    RAISE NOTICE '0332: narrow capability assertion deferred to the canonical runtime overlay';
    RETURN;
  END IF;
