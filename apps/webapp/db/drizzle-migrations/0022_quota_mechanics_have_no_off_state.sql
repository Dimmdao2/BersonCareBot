...
-- TEMPORARY LOCAL MIGRATION NUMBER 0022
--
-- Owner live pass 18.08, L-1 (дословно): «Механика выключена если не включена ее галочка. ЕСЛИ
...
--
-- The live door for every mechanic decision is this function, and for `files`, `patient_count` and
-- `branches` it read inclusion off the presence of a number (`quotas ? p_mechanic`). Those three
-- mechanics do not exist in a tariff's `mechanics` map at all, so an unfilled number was the only
-- switch — and on СТАРТ/ПРОФИ/КЛИНИКА (`quotas = {}`) it read as «доступа нет»: a clinic owner on
-- those tariffs could not create a single location (POST …/branches → 403 entitlement_required).
--
-- The one changed branch below makes a limit-bearing mechanic always part of the tariff; its number
-- is a ceiling, enforced in the write transaction (decideStockQuota), not a presence flag. Nothing
-- else in the body changes — this is a CREATE OR REPLACE of the shipped definition.
