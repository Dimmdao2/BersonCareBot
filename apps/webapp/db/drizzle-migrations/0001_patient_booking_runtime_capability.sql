...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'minNoticeHours', app.read_current_patient_booking_runtime_integer('booking_min_notice_hours'),
    'maxConsecutiveSlotHours', app.read_current_patient_booking_runtime_integer('booking_max_consecutive_slot_hours')
  );
END
$function$;
...
BEGIN
  SELECT pg_get_functiondef(
    'app.read_current_patient_booking_rows(text,timestamp with time zone)'::regprocedure
  ) INTO v_definition;
  IF position('row.slot_start >= p_now' IN v_definition) = 0
     OR position('row.slot_start < p_now OR row.status' IN v_definition) = 0 THEN
    RAISE EXCEPTION 'unexpected read_current_patient_booking_rows B0 definition';
  END IF;
  v_definition := replace(v_definition, 'row.slot_start >= p_now', 'row.slot_end > p_now');
  v_definition := replace(
    v_definition,
    'row.slot_start < p_now OR row.status',
    'row.slot_end <= p_now OR row.status'
  );
