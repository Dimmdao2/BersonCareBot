...
-- marked 'sent' -- see the sibling change in outgoingDeliveryWorker.ts. Exclude it from archiving
-- exactly like 'recipient_blocked_bot' already is: it is an expected, non-alarming terminal state,
-- not an operator-actionable failure to file away.
  ...
  IF p_probe IN ('outgoing_delivery', 'outgoing_reminder_dispatch') THEN
    ...
    WITH candidates AS MATERIALIZED (
      ...
      WHERE queue.status = 'dead'
        AND (queue.failure_class IS NULL OR queue.failure_class NOT IN ('recipient_blocked_bot', 'reminder_not_dispatched'))
        AND CASE
...
-- Track D final cutover (#987), Class B: 'deliveryEvents' now reads public.outgoing_delivery_queue
-- (kind='reminder_dispatch') instead of the retired public.reminder_delivery_events -- one row per
-- occurrence+generation+channel is already the canonical per-channel delivery lifecycle; a fourth
-- terminal journal duplicating it is dropped below alongside its equivalent integrator-side mirror
-- integrator.user_reminder_delivery_logs. 'failed' now means status='dead' with an alarming
-- failure_class (same exclusion as deadTotal/deadByKind above it, extended for
-- 'reminder_not_dispatched' -- see the sibling change in outgoingDeliveryWorker.ts).
...
-- confirmed by direct measurement 23.08.2026) recording the same per-delivery-attempt fact that
-- public.outgoing_delivery_queue's per-row status/sent_at/dead_at/last_error already carries as the
-- single canonical per-channel delivery lifecycle (Track D brief, section B). Neither table has a
-- live application reader left after the function rewrite above; reminder_delivery_events' only
-- reader was app.read_curated_system_health_pre_0196(), rewritten above to read outgoing_delivery_queue
-- instead. Dropped together, forward-only, no CASCADE (no other object references either).
