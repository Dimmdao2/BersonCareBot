-- TEMPORARY LOCAL MIGRATION NUMBER 0285 — the lead assigns the final number at merge.
-- #1069 §5a items 2.6 / 2.6a: снятие агентских констант из лестницы доступа.
--
-- 1. `warningCount` (одно число, выбранное агентом) заменяется на `notifications` — СПИСОК строк
--    владельца «срок · условие · шаблон текста» (его решение 31.07). Существующие политики
--    получают пустой список: сколько уведомлений и с каким текстом — данные, которых у нас нет,
--    и подставлять их за владельца нельзя.
-- 2. Порог предупреждения снимается с мест специалистов совсем (превышение там оплачивается) и
--    с филиалов (владелец назвал только пациентов и объём файлов).
-- 3. Двери жизненного цикла отдают строки уведомлений и якорь «окончание периода» вместо числа.

-- 1. Ladder policies: warningCount -> notifications (empty list, not an invented text).
UPDATE public.saas_tariffs
SET system_access_policy =
  (system_access_policy - 'warningCount')
  || jsonb_build_object(
       'notifications',
       COALESCE(system_access_policy -> 'notifications', '[]'::jsonb)
     )
WHERE system_access_policy IS NOT NULL;
--> statement-breakpoint

UPDATE public.saas_tariffs AS target
SET mechanic_access_policies = COALESCE(rebuilt.policies, '{}'::jsonb)
FROM (
  SELECT
    tariff.id,
    jsonb_object_agg(
      entry.key,
      (entry.value - 'warningCount')
      || jsonb_build_object(
           'notifications',
           COALESCE(entry.value -> 'notifications', '[]'::jsonb)
         )
    ) AS policies
  FROM public.saas_tariffs AS tariff
  CROSS JOIN LATERAL jsonb_each(tariff.mechanic_access_policies) AS entry
  GROUP BY tariff.id
) AS rebuilt
WHERE rebuilt.id = target.id;
--> statement-breakpoint

-- 2a. Branches never had an owner-configured warning threshold; drop the key where it leaked in.
UPDATE public.saas_tariffs
SET quotas = jsonb_set(quotas, '{branches}', (quotas -> 'branches') - 'warningAtPercent')
WHERE quotas -> 'branches' ? 'warningAtPercent';
--> statement-breakpoint

UPDATE public.saas_org_entitlement_overrides
SET quota = quota - 'warningAtPercent'
WHERE mechanic = 'branches' AND quota ? 'warningAtPercent';
--> statement-breakpoint

-- 2b. Specialist seats: no threshold at all (owner 31.07 — «не надо никаких предупреждений»).
ALTER TABLE public.saas_tariffs
  DROP CONSTRAINT IF EXISTS saas_tariffs_included_seats_warning_check;
--> statement-breakpoint

ALTER TABLE public.saas_tariffs
  DROP COLUMN IF EXISTS included_seats_warning_at_percent;
--> statement-breakpoint

-- 3a. Patient projection loses the dropped column (return type change needs DROP + CREATE).
...
-- 3b. Mechanic door: the warning payload carries the owner's rows and the period anchor.
...
-- 3c. Cabinet door: same payload change, same anchor.
