...
-- BCB-MIGRATION-VERIFY: SELECT NOT EXISTS (SELECT 1 FROM public.system_settings WHERE key ~ '^auth_surface_(staff|platform_admin)_.*_enabled$') AND EXISTS (SELECT 1 FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('app.read_authenticated_runtime_setting(text,text,uuid,boolean)') AND pg_catalog.strpos(p.prosrc, '^auth_surface_patient_') > 0 AND pg_catalog.strpos(p.prosrc, '^auth_surface_(staff|platform_admin|patient)_') = 0);
--
-- Staff and platform-admin door composition is code, not data. Remove every persisted surface
-- switch for those doors so an admin action cannot turn an OTP or phone channel into a password
-- replacement. Patient switches stay untouched.
--
-- Rights analysis: data-only deletion from the existing public.system_settings root. No objects,
-- roles, policies or privileges are created or changed.

DELETE FROM public.system_settings
WHERE key ~ '^auth_surface_(staff|platform_admin)_.*_enabled$';
...
-- BCB-MIGRATION-VERIFY: SELECT pg_catalog.strpos(p.prosrc, '^auth_surface_patient_') > 0 AND pg_catalog.strpos(p.prosrc, '^auth_surface_(staff|platform_admin|patient)_') = 0 FROM pg_catalog.pg_proc p WHERE p.oid = pg_catalog.to_regprocedure('app.read_authenticated_runtime_setting(text,text,uuid,boolean)')
--
-- Публичная проекция — allowlist. После удаления строк сотрудничьие ключи не должны
-- оставаться допустимыми для чтения: единственная настраиваемая поверхность здесь — patient.
--
...
BEGIN
  ...
  IF p_scope NOT IN ('admin', 'doctor')
     ...
     OR NOT (
       ...
       OR p_key ~ '^auth_surface_patient_(email|sms|telegram|max|oauth_google|oauth_yandex|oauth_vk|oauth_apple|passkey|telegram_login_widget)_enabled$'
     ) THEN
    RETURN;
  END IF;
