-- Fix reminder scope resolution against the canonical integrator text identifiers.
-- The UUID-shaped payload guard remains unchanged, while the indexed occurrence column
-- is compared directly to a text value. This replaces one existing SECURITY DEFINER
-- function, so the pinned app_owner SECURITY DEFINER count remains 105.
