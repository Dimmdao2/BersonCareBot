...
export const BE_ORGANIZATION_MEMBER_ROLE_VALUES = [
  'owner',
  'admin',
  'doctor',
  'assistant',
...
export const ORG_ENROLLMENT_STATUS_VALUES = [
  'active',
  'invited',
  'discharged',
  'archived',
...
export const beOrganizations = pgTable(
  ...
  (table) => [
    index('idx_be_organizations_is_active').using(
      'btree',
...
export const beBranches = pgTable(
  ...
  (table) => [
    index('idx_be_branches_org').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_be_branches_city').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
      table.cityCode.asc().nullsLast().op('text_ops'),
    ),
    foreignKey({
...
export const beSpecialists = pgTable(
  ...
  (table) => [
    index('idx_be_specialists_org').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
...
export const beOrganizationMembers = pgTable(
  ...
  (table) => [
    index('idx_be_organization_members_org').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_be_organization_members_user').using(
      'btree',
      table.platformUserId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_be_organization_members_specialist').using(
      'btree',
...
export const orgEnrollments = pgTable(
  ...
  (table) => [
    index('idx_org_enrollments_org').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_org_enrollments_user').using(
      'btree',
      table.platformUserId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
    ...
    check(
      ...
      sql`(${table.portalActivatedAt} IS NULL AND ${table.portalActivatedVia} IS NULL) OR (${table.portalActivatedAt} IS NOT NULL AND ${table.portalActivatedVia} IN ('patient_invite_email_otp', 'public_booking_phone_otp', 'public_booking_verified_email', 'public_booking_session', 'public_lead_verified_email'))`,
    ),
  ],
);
...
export const beClinicServices = pgTable(
  ...
  (table) => [
    index('idx_be_clinic_services_org').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
    ...
    check(
      'be_clinic_services_buffer_after_check',
      sql`buffer_after_minutes >= 0 AND buffer_after_minutes % 5 = 0`,
    ),
...
export const beSpecialistServiceAvailability = pgTable(
  ...
  (table) => [
    index('idx_be_ssa_specialist').using(
      'btree',
      table.specialistId.asc().nullsLast().op('uuid_ops'),
    ),
...
export const beAppointments = pgTable(
  ...
  (table) => [
    ...
    index('idx_be_appointments_patient').using(
      'btree',
      table.platformUserId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_be_appointments_status').using(
      'btree',
