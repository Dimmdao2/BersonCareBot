...
describe('S5-1 app runtime settings schema/data contract', () => {
  ...
  it('creates the audit table and exactly one same-transaction INSERT/UPDATE trigger owner', () => {
    ...
    expect(migration.match(/CREATE TRIGGER app_runtime_settings_audit_change/g) ?? []).toHaveLength(
      1,
    );
    expect(migration.match(/INSERT INTO public\.app_runtime_settings_audit/g) ?? []).toHaveLength(
      1,
    );
  });
  ...
  it('keeps the normal migration list exactly aligned with the S5-0 runtime registry', () => {
    ...
    expect(unsupportedClientFallbackMigration).toContain(
    ...
    expect(platformIntegrationAvailabilityMigration).toContain(
  ...
  it('keeps the legacy trigger active while bypassing only an application-owned explicit dual-write', () => {
    ...
    expect(dualWriteMigration).toContain(
      "IF NEW.key = 'patient_booking_url' AND NEW.scope = 'admin'",
    );
    ...
    expect(dualWriteMigration).not.toContain(
      'DROP FUNCTION public.sync_registered_app_runtime_setting',
    );
  });
});
