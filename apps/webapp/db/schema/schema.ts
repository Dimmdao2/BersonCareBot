...
export const phoneChallenges = pgTable(
  ...
  (table) => [
    index('idx_phone_challenges_expires_at').using(
      'btree',
      table.expiresAt.asc().nullsLast().op('int8_ops'),
    ),
    index('idx_phone_challenges_phone').using(
      'btree',
...
export const supportConversations = pgTable(
  ...
  (table) => [
    uniqueIndex('idx_support_conversations_integrator_id').using(
      'btree',
    ...
    index('idx_support_conversations_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_support_conversations_platform_user_id')
    ...
    unique('support_conversations_integrator_conversation_id_key').on(
      table.integratorConversationId,
    ),
  ],
);
...
export const userPhoneHistory = pgTable(
  'user_phone_history',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    platformUserId: uuid('platform_user_id').notNull(),
  ...
  (table) => [
    ...
    index('idx_user_phone_history_user').using(
      'btree',
    ...
    uniqueIndex('uq_user_phone_history_user_active')
      .using('btree', table.platformUserId.asc().nullsLast().op('uuid_ops'))
      .where(sql`(valid_to IS NULL)`),
    check(
...
export const messageLog = pgTable(
  ...
  (table) => [
    index('idx_message_log_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_message_log_platform_user_id')
      .using('btree', table.platformUserId.asc().nullsLast().op('uuid_ops'))
      .where(sql`(platform_user_id IS NOT NULL)`),
    index('idx_message_log_sent_at').using(
      'btree',
    ...
    check(
      'message_log_outcome_check',
      sql`outcome = ANY (ARRAY['sent'::text, 'partial'::text, 'failed'::text])`,
    ),
  ],
);
...
export const userChannelBindings = pgTable(
  ...
  (table) => [
    index('idx_user_channel_bindings_lookup').using(
      'btree',
      table.channelCode.asc().nullsLast().op('text_ops'),
      table.externalId.asc().nullsLast().op('text_ops'),
    ),
    index('idx_user_channel_bindings_user_id').using(
      'btree',
    ...
    unique('user_channel_bindings_channel_code_external_id_key').on(
      table.channelCode,
      table.externalId,
    ),
    check(
      'user_channel_bindings_channel_code_check',
      sql`channel_code = ANY (ARRAY['telegram'::text, 'max'::text, 'vk'::text])`,
    ),
  ],
);
...
export const broadcastAudit = pgTable(
  'broadcast_audit',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    organizationId: uuid('organization_id'),
    ...
    channels: text('channels')
      .array()
      .notNull()
      .default(sql`ARRAY['bot_message'::text, 'sms'::text]`),
  ...
  (table) => [
    index('idx_broadcast_audit_executed_at').using(
      'btree',
      table.executedAt.desc().nullsFirst().op('timestamptz_ops'),
    ),
    index('idx_broadcast_audit_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
  ],
...
export const supportQuestions = pgTable(
  'support_questions',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    organizationId: uuid('organization_id'),
  ...
  (table) => [
    index('idx_support_questions_conversation_id')
      .using('btree', table.conversationId.asc().nullsLast().op('uuid_ops'))
      .where(sql`(conversation_id IS NOT NULL)`),
    index('idx_support_questions_created').using(
      'btree',
      table.createdAt.desc().nullsFirst().op('timestamptz_ops'),
    ),
    uniqueIndex('idx_support_questions_integrator_id').using(
      'btree',
      table.integratorQuestionId.asc().nullsLast().op('text_ops'),
    ),
    index('idx_support_questions_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
...
export const supportQuestionMessages = pgTable(
  ...
  (table) => [
    uniqueIndex('idx_support_question_messages_integrator_id').using(
      'btree',
      table.integratorQuestionMessageId.asc().nullsLast().op('text_ops'),
    ),
    index('idx_support_question_messages_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_support_question_messages_question_created').using(
      'btree',
      table.questionId.asc().nullsLast().op('timestamptz_ops'),
    ...
    unique('support_question_messages_integrator_question_message_id_key').on(
      table.integratorQuestionMessageId,
    ),
  ],
);
...
export const symptomEntries = pgTable(
  ...
  (table) => [
    index('idx_symptom_entries_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_symptom_entries_platform_user_id')
      .using('btree', table.platformUserId.asc().nullsLast().op('uuid_ops'))
      .where(sql`(platform_user_id IS NOT NULL)`),
    index('idx_symptom_entries_tracking_recorded').using(
      'btree',
      table.trackingId.asc().nullsLast().op('timestamptz_ops'),
      table.recordedAt.desc().nullsFirst().op('uuid_ops'),
    ),
    index('idx_symptom_entries_user_type_recorded').using(
      'btree',
      table.userId.asc().nullsLast().op('timestamptz_ops'),
      table.entryType.asc().nullsLast().op('text_ops'),
    ...
    check(
      'symptom_entries_entry_type_check',
      sql`entry_type = ANY (ARRAY['instant'::text, 'daily'::text])`,
    ),
    check(
      'symptom_entries_source_check',
      sql`source = ANY (ARRAY['bot'::text, 'webapp'::text, 'import'::text])`,
    ),
...
export const contentAccessGrantsWebapp = pgTable(
  ...
  (table) => [
    index('idx_content_access_grants_webapp_expires_at').using(
      'btree',
      table.expiresAt.desc().nullsFirst().op('timestamptz_ops'),
    ),
...
export const appointmentRecords = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex('idx_appointment_records_integrator_record_id').using(
      'btree',
      table.integratorRecordId.asc().nullsLast().op('text_ops'),
    ),
    index('idx_appointment_records_phone_normalized')
      .using('btree', table.phoneNormalized.asc().nullsLast().op('text_ops'))
      .where(sql`(phone_normalized IS NOT NULL)`),
    index('idx_appointment_records_phone_not_deleted')
      .using(
        'btree',
        table.phoneNormalized.asc().nullsLast().op('timestamptz_ops'),
        table.recordAt.desc().nullsFirst().op('timestamptz_ops'),
      )
      .where(sql`((deleted_at IS NULL) AND (phone_normalized IS NOT NULL))`),
    index('idx_appointment_records_record_at')
      .using('btree', table.recordAt.asc().nullsLast().op('timestamptz_ops'))
      .where(sql`(record_at IS NOT NULL)`),
    index('idx_appointment_records_status').using(
      'btree',
...
export const contentPages = pgTable(
  ...
  (table) => [
    index('idx_content_pages_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_content_pages_section').using(
      'btree',
      table.section.asc().nullsLast().op('text_ops'),
    ),
    index('idx_content_pages_section_sort').using(
      'btree',
      table.section.asc().nullsLast().op('text_ops'),
...
export const emailChallenges = pgTable(
  ...
  (table) => [
    ...
    index('idx_email_challenges_user_id').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
...
export const channelLinkSecrets = pgTable(
  ...
  (table) => [
    index('idx_channel_link_secrets_expires').using(
      'btree',
      table.expiresAt.asc().nullsLast().op('timestamptz_ops'),
    ),
    index('idx_channel_link_secrets_user_channel').using(
      'btree',
    ...
    check(
      'channel_link_secrets_channel_code_check',
      sql`channel_code = ANY (ARRAY['telegram'::text, 'max'::text, 'vk'::text])`,
    ),
  ],
);
...
export const userChannelPreferences = pgTable(
  ...
  (table) => [
    uniqueIndex('idx_user_channel_preferences_one_auth_pref')
      .using('btree', table.userId.asc().nullsLast().op('text_ops'))
      .where(sql`(is_preferred_for_auth = true)`),
    index('idx_user_channel_preferences_platform_user_id')
      .using('btree', table.platformUserId.asc().nullsLast().op('uuid_ops'))
      .where(sql`(platform_user_id IS NOT NULL)`),
    index('idx_user_channel_preferences_user_id').using(
      'btree',
      table.userId.asc().nullsLast().op('text_ops'),
    ),
    uniqueIndex('uq_user_channel_preferences_platform_user_channel').using(
      'btree',
      table.platformUserId.asc().nullsLast().op('uuid_ops'),
      table.channelCode.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
      columns: [table.platformUserId],
      foreignColumns: [platformUsers.id],
    ...
    check(
      'user_channel_preferences_channel_code_check',
      sql`channel_code = ANY (ARRAY['telegram'::text, 'max'::text, 'vk'::text, 'sms'::text, 'email'::text, 'web_push'::text])`,
    ),
  ],
);
...
export const userOauthBindings = pgTable(
  ...
  (table) => [
    ...
    unique('user_oauth_bindings_provider_provider_user_id_key').on(
      table.provider,
      table.providerUserId,
    ),
    check(
      'user_oauth_bindings_provider_check',
      sql`provider = ANY (ARRAY['google'::text, 'apple'::text, 'yandex'::text])`,
    ),
  ],
);

export const lfkSessions = pgTable(
  'lfk_sessions',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    organizationId: uuid('organization_id'),
  ...
  (table) => [
    index('idx_lfk_sessions_complex_completed').using(
      'btree',
      table.complexId.asc().nullsLast().op('timestamptz_ops'),
      table.completedAt.desc().nullsFirst().op('timestamptz_ops'),
    ),
    index('idx_lfk_sessions_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_lfk_sessions_user_completed').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ...
    check(
      'lfk_sessions_difficulty_0_10_check',
      sql`(difficulty_0_10 IS NULL) OR ((difficulty_0_10 >= 0) AND (difficulty_0_10 <= 10))`,
    ),
    check(
      'lfk_sessions_pain_0_10_check',
      sql`(pain_0_10 IS NULL) OR ((pain_0_10 >= 0) AND (pain_0_10 <= 10))`,
    ),
...
export const supportConversationMessages = pgTable(
  ...
  (table) => [
    index('idx_support_conv_msg_conv_created').using(
      'btree',
      table.conversationId.asc().nullsLast().op('timestamptz_ops'),
      table.createdAt.desc().nullsFirst().op('timestamptz_ops'),
    ),
    index('idx_support_conv_msg_unread_incoming')
      .using('btree', table.conversationId.asc().nullsLast().op('uuid_ops'))
      .where(sql`((read_at IS NULL) AND (sender_role <> 'user'::text))`),
    index('idx_support_conv_msg_unread_user_msgs')
      .using('btree', table.conversationId.asc().nullsLast().op('uuid_ops'))
      .where(sql`((read_at IS NULL) AND (sender_role = 'user'::text))`),
    index('idx_support_conversation_messages_conversation_created').using(
      'btree',
      table.conversationId.asc().nullsLast().op('uuid_ops'),
      table.createdAt.asc().nullsLast().op('uuid_ops'),
    ),
    uniqueIndex('idx_support_conversation_messages_integrator_id').using(
      'btree',
      table.integratorMessageId.asc().nullsLast().op('text_ops'),
    ),
    index('idx_support_conversation_messages_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
...
export const phoneMessengerBindSecrets = pgTable(
  ...
  (table) => [
    index('idx_phone_messenger_bind_secrets_expires').using(
      'btree',
      table.expiresAt.asc().nullsLast().op('timestamptz_ops'),
    ),
    index('idx_phone_messenger_bind_secrets_phone_status').using(
      'btree',
      table.phoneNormalized.asc().nullsLast().op('text_ops'),
      table.status.asc().nullsLast().op('text_ops'),
    ),
    ...
    check(
      'phone_messenger_bind_secrets_channel_code_check',
      sql`channel_code = ANY (ARRAY['telegram'::text, 'max'::text])`,
    ),
    check(
      'phone_messenger_bind_secrets_purpose_check',
      sql`purpose = ANY (ARRAY['login'::text, 'profile_bind'::text])`,
    ),
    check(
      'phone_messenger_bind_secrets_status_check',
      sql`status = ANY (ARRAY['pending_contact'::text, 'otp_ready'::text, 'failed'::text, 'consumed'::text, 'expired'::text])`,
    ),
  ],
);
...
export const loginTokens = pgTable(
  ...
  (table) => [
    ...
    check(
      'login_tokens_status_check',
      sql`status = ANY (ARRAY['pending'::text, 'confirmed'::text, 'expired'::text])`,
    ),
  ],
);

export const referenceCategories = pgTable(
  ...
  (table) => [
    index('idx_reference_categories_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
...
export const referenceItems = pgTable(
  ...
  (table) => [
    index('idx_ref_items_category').using(
      'btree',
      table.categoryId.asc().nullsLast().op('int4_ops'),
      table.sortOrder.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_reference_items_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('reference_items_category_deleted_active_sort_idx').using(
      'btree',
...
export const referenceCatalogBaselines = pgTable(
  ...
  (table) => [
    check(
      'reference_catalog_baselines_definition_object_check',
      sql`jsonb_typeof(${table.definitionJson}) = 'object'`,
    ),
  ],
);

export const referenceCatalogSnapshotReceipts = pgTable(
  'reference_catalog_snapshot_receipts',
  {
...
export const doctorNotes = pgTable(
  ...
  (table) => [
    index('idx_doctor_notes_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_doctor_notes_user_created').using(
      'btree',
      table.userId.asc().nullsLast().op('timestamptz_ops'),
      table.createdAt.desc().nullsFirst().op('timestamptz_ops'),
    ),
...
export const symptomTrackings = pgTable(
  'symptom_trackings',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    organizationId: uuid('organization_id'),
  ...
  (table) => [
    index('idx_symptom_trackings_deleted')
      .using('btree', table.userId.asc().nullsLast().op('text_ops'))
      .where(sql`(deleted_at IS NOT NULL)`),
    index('idx_symptom_trackings_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_symptom_trackings_platform_user_id')
      .using('btree', table.platformUserId.asc().nullsLast().op('uuid_ops'))
      .where(sql`(platform_user_id IS NOT NULL)`),
    index('idx_symptom_trackings_user_active').using(
      'btree',
    ...
    check(
      'symptom_trackings_side_check',
      sql`(side IS NULL) OR (side = ANY (ARRAY['left'::text, 'right'::text, 'both'::text]))`,
    ),
  ],
);

export const lfkComplexes = pgTable(
  'lfk_complexes',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    organizationId: uuid('organization_id'),
  ...
  (table) => [
    index('idx_lfk_complexes_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_lfk_complexes_platform_user_id')
      .using('btree', table.platformUserId.asc().nullsLast().op('uuid_ops'))
      .where(sql`(platform_user_id IS NOT NULL)`),
    index('idx_lfk_complexes_user_active').using(
      'btree',
      table.userId.asc().nullsLast().op('text_ops'),
    ...
    check(
      'lfk_complexes_origin_check',
      sql`origin = ANY (ARRAY['manual'::text, 'assigned_by_specialist'::text])`,
    ),
    check(
      'lfk_complexes_side_check',
      sql`(side IS NULL) OR (side = ANY (ARRAY['left'::text, 'right'::text, 'both'::text]))`,
    ),
  ],
);
...
export const motivationalQuotes = pgTable(
  ...
  (table) => [
    index('idx_motivational_quotes_active').using(
      'btree',
      table.isActive.asc().nullsLast().op('int4_ops'),
      table.sortOrder.asc().nullsLast().op('int4_ops'),
    ),
    index('idx_motivational_quotes_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
  ],
...
export const lfkExercises = pgTable(
  'lfk_exercises',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
  ...
  (table) => [
    index('idx_lfk_exercises_archived').using(
      'btree',
      table.isArchived.asc().nullsLast().op('bool_ops'),
    ),
    index('idx_lfk_exercises_catalog_scope_owner').using(
      'btree',
      table.ownerKind.asc().nullsLast().op('text_ops'),
      table.organizationId.asc().nullsLast().op('uuid_ops'),
      table.catalogScope.asc().nullsLast().op('text_ops'),
      table.isArchived.asc().nullsLast().op('bool_ops'),
      table.updatedAt.desc().nullsFirst().op('timestamptz_ops'),
    ),
    index('idx_lfk_exercises_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_lfk_exercises_region')
      .using('btree', table.regionRefId.asc().nullsLast().op('uuid_ops'))
      .where(sql`(NOT is_archived)`),
    foreignKey({
    ...
    check(
      'lfk_exercises_difficulty_1_10_check',
      sql`(difficulty_1_10 IS NULL) OR ((difficulty_1_10 >= 1) AND (difficulty_1_10 <= 10))`,
    ),
    check(
      'lfk_exercises_catalog_scope_check',
      sql`catalog_scope = ANY (ARRAY['catalog'::text, 'personal'::text])`,
    ),
    check(
      'lfk_exercises_owner_check',
...
export const lfkExerciseRegions = pgTable(
  'lfk_exercise_regions',
  {
    ownerKind: text('owner_kind').default('organization').notNull(),
    organizationId: uuid('organization_id'),
  ...
  (table) => [
    primaryKey({
      columns: [table.exerciseId, table.regionRefId],
      name: 'lfk_exercise_regions_pkey',
    }),
    index('idx_lfk_exercise_regions_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
    ...
    index('idx_lfk_exercise_regions_region_ref').using(
      'btree',
      table.regionRefId.asc().nullsLast().op('uuid_ops'),
    ),
    check(
      'lfk_exercise_regions_owner_check',
      sql`(owner_kind = 'organization' AND organization_id IS NOT NULL) OR (owner_kind = 'platform' AND organization_id IS NULL)`,
    ),
  ],
);
...
export const lfkComplexTemplateExercises = pgTable(
  ...
  (table) => [
    index('idx_lfk_complex_template_exercises_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_template_exercises_order').using(
      'btree',
    ...
    unique('lfk_complex_template_exercises_template_id_exercise_id_key').on(
      table.templateId,
      table.exerciseId,
    ),
    check(
      'lfk_complex_template_exercises_max_pain_0_10_check',
      sql`(max_pain_0_10 IS NULL) OR ((max_pain_0_10 >= 0) AND (max_pain_0_10 <= 10))`,
    ),
    check(
      'lfk_complex_template_exercises_side_check',
      sql`(side IS NULL) OR (side = ANY (ARRAY['left'::text, 'right'::text, 'both'::text, 'damaged'::text, 'healthy'::text]))`,
    ),
    check(
      'lfk_complex_template_exercises_owner_check',
      sql`(owner_kind = 'organization' AND organization_id IS NOT NULL) OR (owner_kind = 'platform' AND organization_id IS NULL)`,
    ),
  ],
);

export const lfkExerciseMedia = pgTable(
  'lfk_exercise_media',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    ownerKind: text('owner_kind').default('organization').notNull(),
  ...
  (table) => [
    index('idx_lfk_exercise_media_exercise').using(
      'btree',
      table.exerciseId.asc().nullsLast().op('int4_ops'),
      table.sortOrder.asc().nullsLast().op('int4_ops'),
    ),
    index('idx_lfk_exercise_media_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
...
export const lfkComplexTemplates = pgTable(
  ...
  (table) => [
    index('idx_lfk_complex_templates_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
    ...
    check(
      'lfk_complex_templates_status_check',
      sql`status = ANY (ARRAY['draft'::text, 'published'::text, 'archived'::text])`,
    ),
    check(
      'lfk_complex_templates_owner_check',
      sql`(owner_kind = 'organization' AND organization_id IS NOT NULL) OR (owner_kind = 'platform' AND organization_id IS NULL)`,
    ),
  ],
);

export const patientLfkAssignments = pgTable(
  ...
  (table) => [
    index('idx_assignments_patient').using(
      'btree',
      table.patientUserId.asc().nullsLast().op('uuid_ops'),
      table.isActive.asc().nullsLast().op('bool_ops'),
    ),
    index('idx_patient_lfk_assignments_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    uniqueIndex('idx_patient_lfk_assign_active_template')
      .using(
        'btree',
        table.organizationId.asc().nullsLast().op('uuid_ops'),
        table.patientUserId.asc().nullsLast().op('uuid_ops'),
...
export const lfkComplexExercises = pgTable(
  ...
  (table) => [
    index('idx_lfk_complex_exercises_complex').using(
      'btree',
      table.complexId.asc().nullsLast().op('int4_ops'),
      table.sortOrder.asc().nullsLast().op('int4_ops'),
    ),
    index('idx_lfk_complex_exercises_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
    ...
    unique('lfk_complex_exercises_complex_id_exercise_id_key').on(
      table.complexId,
      table.exerciseId,
    ),
    check(
      'lfk_complex_exercises_max_pain_0_10_check',
      sql`(max_pain_0_10 IS NULL) OR ((max_pain_0_10 >= 0) AND (max_pain_0_10 <= 10))`,
    ),
    check(
      'lfk_complex_exercises_side_check',
      sql`(side IS NULL) OR (side = ANY (ARRAY['left'::text, 'right'::text, 'both'::text, 'damaged'::text, 'healthy'::text]))`,
    ),
  ],
);
...
export const authRateLimitEvents = pgTable(
  ...
  (table) => [
    index('idx_auth_rate_limit_events_scope_key_time').using(
      'btree',
...
export const mediaFiles = pgTable(
  'media_files',
  {
    ...
    previewNextAttemptAt: timestamp('preview_next_attempt_at', {
      withTimezone: true,
      mode: 'string',
    }),
  ...
  (table) => [
    ...
    index('idx_media_files_folder_created')
      .using(
        'btree',
    ...
    index('idx_media_files_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_media_files_preview_status')
      .using('btree', table.previewStatus.asc().nullsLast().op('text_ops'))
      .where(sql`(preview_status = 'pending'::text)`),
    index('idx_media_files_purge_queue')
      .using('btree', table.nextAttemptAt.asc().nullsFirst().op('timestamptz_ops'))
      .where(sql`(status = ANY (ARRAY['pending_delete'::text, 'deleting'::text]))`),
    index('idx_media_files_uploaded_by').using(
      'btree',
    ...
    check(
      ...
      sql`status = ANY (ARRAY['ready'::text, 'pending'::text, 'deleting'::text, 'pending_delete'::text])`,
    ),
    check(
      'media_files_owner_check',
      sql`(owner_kind = 'organization' AND organization_id IS NOT NULL) OR (owner_kind = 'platform' AND organization_id IS NULL)`,
    ),
    check(
...
export const mediaTranscodeJobs = pgTable(
  ...
  (table) => [
    index('idx_media_transcode_jobs_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_media_transcode_jobs_pending_pick')
      .using(
        'btree',
        table.nextAttemptAt.asc().nullsFirst().op('timestamptz_ops'),
        table.createdAt.asc().nullsLast().op('timestamptz_ops'),
      )
      .where(sql`(status = 'pending'::text)`),
    uniqueIndex('media_transcode_jobs_one_active_per_media')
    ...
    check(
      'media_transcode_jobs_status_check',
      sql`status = ANY (ARRAY['pending'::text, 'processing'::text, 'done'::text, 'failed'::text])`,
    ),
  ],
);
...
export const bookingCities = pgTable(
  ...
  (table) => [
    index('idx_booking_cities_is_active').using(
      'btree',
      table.isActive.asc().nullsLast().op('bool_ops'),
    ),
...
export const contentSections = pgTable(
  ...
  (table) => [
    index('idx_content_sections_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_content_sections_sort').using(
      'btree',
      table.sortOrder.asc().nullsLast().op('int4_ops'),
...
export const contentSectionSlugHistory = pgTable(
  ...
  (table) => [
    ...
    index('idx_content_section_slug_history_new_slug').using(
      'btree',
      table.newSlug.asc().nullsLast().op('text_ops'),
    ),
    index('idx_content_section_slug_history_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
  ],
...
export const patientHomeBlocks = pgTable(
  ...
  (table) => [
    index('idx_patient_home_blocks_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
  ],
...
export const patientHomeBlockItems = pgTable(
  ...
  (table) => [
    index('idx_patient_home_block_items_block_sort').using(
      'btree',
      table.blockCode.asc().nullsLast().op('text_ops'),
      table.sortOrder.asc().nullsLast().op('int4_ops'),
    ),
    index('idx_patient_home_block_items_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
    ...
    check(
      ...
      sql`target_type = ANY (ARRAY['content_page'::text, 'content_section'::text, 'course'::text, 'static_action'::text])`,
    ),
  ],
);
...
export const onlineIntakeRequests = pgTable(
  ...
  (table) => [
    index('idx_online_intake_requests_created_at').using(
      'btree',
      table.createdAt.desc().nullsFirst().op('timestamptz_ops'),
    ),
    index('idx_online_intake_requests_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_online_intake_requests_status').using(
      'btree',
      table.status.asc().nullsLast().op('text_ops'),
    ),
    index('idx_online_intake_requests_type').using(
      'btree',
      table.type.asc().nullsLast().op('text_ops'),
    ),
    index('idx_online_intake_requests_user_id').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
    ...
    check(
      'online_intake_requests_status_check',
      sql`status = ANY (ARRAY['new'::text, 'in_review'::text, 'contacted'::text, 'booked'::text, 'rejected'::text, 'closed'::text])`,
    ),
    check(
      'online_intake_requests_type_check',
      sql`type = ANY (ARRAY['lfk'::text, 'nutrition'::text])`,
    ),
  ],
);

export const patientBookings = pgTable(
  'patient_bookings',
  {
    id: uuid().primaryKey().notNull(),
    organizationId: uuid('organization_id'),
  ...
  (table) => [
    ...
    index('idx_patient_bookings_branch_service_id').using(
      'btree',
      table.branchServiceId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_patient_bookings_service_id').using(
      'btree',
      table.serviceId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_patient_bookings_slot_start').using(
      'btree',
      table.slotStart.asc().nullsLast().op('timestamptz_ops'),
    ),
    index('idx_patient_bookings_status').using(
      'btree',
      table.status.asc().nullsLast().op('text_ops'),
    ),
    index('idx_patient_bookings_user_id').using(
      'btree',
      table.platformUserId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
    ...
    check(
      'patient_bookings_category_check',
      sql`category = ANY (ARRAY['rehab_lfk'::text, 'nutrition'::text, 'general'::text])`,
    ),
    check('patient_bookings_check', sql`slot_end > slot_start`),
    check(
      'patient_bookings_status_check',
      sql`status = ANY (ARRAY['creating'::text, 'awaiting_payment'::text, 'confirmed'::text, 'cancelling'::text, 'cancel_failed'::text, 'cancelled'::text, 'rescheduled'::text, 'completed'::text, 'no_show'::text, 'failed_sync'::text])`,
    ),
  ],
);

export const onlineIntakeAnswers = pgTable(
  'online_intake_answers',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
  ...
  (table) => [
    index('idx_online_intake_answers_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_online_intake_answers_request_id').using(
      'btree',
    ...
    unique('online_intake_answers_request_id_question_id_key').on(
      table.requestId,
      table.questionId,
    ),
  ],
...
export const onlineIntakeAttachments = pgTable(
  'online_intake_attachments',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    requestId: uuid('request_id').notNull(),
    organizationId: uuid('organization_id'),
  ...
  (table) => [
    index('idx_online_intake_attachments_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_online_intake_attachments_request_id').using(
      'btree',
      table.requestId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
    ...
    check(
      'online_intake_attachments_attachment_type_check',
      sql`attachment_type = ANY (ARRAY['file'::text, 'url'::text])`,
    ),
    check(
      'online_intake_attachments_check',
      sql`((attachment_type = 'file'::text) AND (s3_key IS NOT NULL)) OR ((attachment_type = 'url'::text) AND (url IS NOT NULL))`,
    ),
  ],
);

export const onlineIntakeStatusHistory = pgTable(
  ...
  (table) => [
    index('idx_online_intake_status_history_changed_at').using(
      'btree',
      table.changedAt.desc().nullsFirst().op('timestamptz_ops'),
    ),
    index('idx_online_intake_status_history_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_online_intake_status_history_request_id').using(
      'btree',
      table.requestId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
...
export const reminderOccurrenceHistory = pgTable(
  ...
  (table) => [
    ...
    index('idx_reminder_occurrence_history_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_reminder_occurrence_history_seen_at')
      .using('btree', table.seenAt.asc().nullsLast().op('timestamptz_ops'))
      .where(sql`(seen_at IS NULL)`),
    index('idx_reminder_occurrence_history_skipped_at')
      .using('btree', table.skippedAt.desc().nullsFirst().op('timestamptz_ops'))
    ...
    check(
      'chk_reminder_occurrence_snooze_pair',
      sql`((snoozed_at IS NULL) AND (snoozed_until IS NULL)) OR ((snoozed_at IS NOT NULL) AND (snoozed_until IS NOT NULL))`,
    ),
    check(
...
export const reminderRules = pgTable(
  ...
  (table) => [
    uniqueIndex('idx_reminder_rules_integrator_rule_id').using(
      'btree',
    ...
    index('idx_reminder_rules_linked_object')
      .using(
        'btree',
        table.linkedObjectType.asc().nullsLast().op('text_ops'),
        table.linkedObjectId.asc().nullsLast().op('text_ops'),
      )
      .where(sql`((linked_object_type IS NOT NULL) AND (linked_object_id IS NOT NULL))`),
    index('idx_reminder_rules_linked_object_type')
      .using('btree', table.linkedObjectType.asc().nullsLast().op('text_ops'))
      .where(sql`(linked_object_type IS NOT NULL)`),
    index('idx_reminder_rules_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_reminder_rules_platform_user_id')
      .using('btree', table.platformUserId.asc().nullsLast().op('uuid_ops'))
      .where(sql`(platform_user_id IS NOT NULL)`),
    index('idx_reminder_rules_platform_user_updated_at')
      .using(
        'btree',
        table.platformUserId.asc().nullsLast().op('timestamptz_ops'),
    ...
    check(
      ...
      sql`(linked_object_type = 'custom'::text) OR ((custom_title IS NULL) AND (custom_text IS NULL))`,
    ),
    check(
      'chk_reminder_rules_custom_required',
      sql`(linked_object_type IS DISTINCT FROM 'custom'::text) OR ((custom_title IS NOT NULL) AND (btrim(custom_title) <> ''::text))`,
    ),
    check(
      'chk_reminder_rules_linked_object_type',
      sql`(linked_object_type IS NULL) OR (linked_object_type = ANY (ARRAY['lfk_complex'::text, 'content_section'::text, 'content_page'::text, 'custom'::text, 'rehab_program'::text]))`,
    ),
    check(
      'chk_reminder_rules_object_id_required',
      sql`(linked_object_type IS NULL) OR (linked_object_type = 'custom'::text) OR ((linked_object_id IS NOT NULL) AND (btrim(linked_object_id) <> ''::text))`,
    ),
    check(
...
export const adminAuditLog = pgTable(
  ...
  (table) => [
    index('idx_admin_audit_log_action').using(
      'btree',
      table.action.asc().nullsLast().op('text_ops'),
    ),
    index('idx_admin_audit_log_conflict_key')
      .using('btree', table.conflictKey.asc().nullsLast().op('text_ops'))
      .where(sql`(conflict_key IS NOT NULL)`),
    uniqueIndex('idx_admin_audit_log_conflict_open')
      .using('btree', table.conflictKey.asc().nullsLast().op('text_ops'))
      .where(sql`((conflict_key IS NOT NULL) AND (resolved_at IS NULL))`),
    index('idx_admin_audit_log_created').using(
      'btree',
      table.createdAt.desc().nullsFirst().op('timestamptz_ops'),
    ),
    index('idx_admin_audit_log_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_admin_audit_log_target')
      .using('btree', table.targetId.asc().nullsLast().op('text_ops'))
...
export const mediaFolders = pgTable(
  ...
  (table) => [
    index('idx_media_folders_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_media_folders_parent_id').using(
      'btree',
      table.parentId.asc().nullsLast().op('uuid_ops'),
    ),
    uniqueIndex('uq_media_folders_child_name')
      .using(
        'btree',
        table.parentId.asc().nullsLast().op('uuid_ops'),
    ...
    check(
      'media_folders_name_check',
      sql`(length(TRIM(BOTH FROM name)) > 0) AND (char_length(name) <= 180)`,
    ),
    check(
      'media_folders_kind_check',
      sql`kind = ANY (ARRAY['standard'::text, 'client_files_root'::text, 'client_patient'::text])`,
    ),
  ],
);
...
export const mediaUploadSessions = pgTable(
  ...
  (table) => [
    index('idx_media_upload_sessions_expires')
      .using('btree', table.expiresAt.asc().nullsLast().op('timestamptz_ops'))
      .where(sql`(status = ANY (ARRAY['initiated'::text, 'uploading'::text, 'completing'::text]))`),
    index('idx_media_upload_sessions_organization_id').using(
      'btree',
      table.organizationId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_media_upload_sessions_owner').using(
      'btree',
      table.ownerUserId.asc().nullsLast().op('uuid_ops'),
    ),
    uniqueIndex('uq_media_upload_sessions_one_active_per_media')
      .using('btree', table.mediaId.asc().nullsLast().op('uuid_ops'))
      .where(sql`(status = ANY (ARRAY['initiated'::text, 'uploading'::text, 'completing'::text]))`),
    foreignKey({
      columns: [table.mediaId],
    ...
    check(
      'media_upload_sessions_part_size_bytes_check',
      sql`(part_size_bytes >= 1) AND (part_size_bytes <= 536870912)`,
    ),
    check(
      'media_upload_sessions_status_check',
      sql`status = ANY (ARRAY['initiated'::text, 'uploading'::text, 'completing'::text, 'completed'::text, 'aborted'::text, 'expired'::text, 'failed'::text])`,
    ),
  ],
);
...
export const users = pgTable(
  ...
  (table) => [
    index('idx_users_merged_into_user_id')
      .using('btree', table.mergedIntoUserId.asc().nullsLast().op('int8_ops'))
    ...
    check(
      'users_merged_into_user_id_not_self_check',
      sql`(merged_into_user_id IS NULL) OR (merged_into_user_id <> id)`,
    ),
  ],
);
...
export const messageDrafts = pgTable(
  ...
  (table) => [
    uniqueIndex('message_drafts_identity_source_uidx').using(
      'btree',
      table.identityId.asc().nullsLast().op('int8_ops'),
      table.source.asc().nullsLast().op('text_ops'),
    ),
    index('message_drafts_source_updated_idx').using(
      'btree',
      table.source.asc().nullsLast().op('text_ops'),
...
export const conversations = pgTable(
  ...
  (table) => [
    uniqueIndex('conversations_open_user_source_uidx')
      .using(
        'btree',
        table.userIdentityId.asc().nullsLast().op('int8_ops'),
        table.source.asc().nullsLast().op('text_ops'),
      )
      .where(sql`((closed_at IS NULL) AND (status <> 'closed'::text))`),
    index('conversations_status_last_message_idx').using(
      'btree',
      table.status.asc().nullsLast().op('text_ops'),
    ...
    check(
      ...
      sql`status = ANY (ARRAY['open'::text, 'waiting_admin'::text, 'waiting_user'::text, 'closed'::text])`,
    ),
  ],
);
...
export const conversationMessages = pgTable(
  ...
  (table) => [
    index('conversation_messages_conversation_created_idx').using(
      'btree',
    ...
    check(
      'conversation_messages_sender_role_check',
      sql`sender_role = ANY (ARRAY['user'::text, 'admin'::text, 'system'::text])`,
    ),
  ],
);
...
export const userQuestions = pgTable(
  ...
  (table) => [
    index('user_questions_answered_created_idx')
      .using(
        'btree',
        table.answered.asc().nullsLast().op('timestamptz_ops'),
        table.createdAt.desc().nullsFirst().op('timestamptz_ops'),
      )
      .where(sql`(answered = false)`),
    index('user_questions_conversation_id_idx')
      .using('btree', table.conversationId.asc().nullsLast().op('text_ops'))
      .where(sql`(conversation_id IS NOT NULL)`),
    foreignKey({
      columns: [table.conversationId],
      foreignColumns: [conversations.id],
...
export const questionMessages = pgTable(
  'question_messages',
  {
    id: text().primaryKey().notNull(),
    questionId: text('question_id').notNull(),
  ...
  (table) => [
    index('question_messages_question_created_idx').using(
      'btree',
...
export const telegramUsers = pgTable(
  ...
  (table) => [
    index('telegram_users_last_start_at_idx').using(
      'btree',
      table.lastStartAt.asc().nullsLast().op('timestamptz_ops'),
    ),
    index('telegram_users_last_update_id_idx').using(
      'btree',
...
export const integrationDataQualityIncidents = pgTable(
  ...
  (table) => [
    index('idx_integration_data_quality_incidents_last_seen').using(
      'btree',
      table.lastSeenAt.desc().nullsFirst().op('timestamptz_ops'),
    ),
    unique('integration_data_quality_incidents_dedup').on(
      table.integration,
      table.entity,
      table.externalId,
      table.field,
    ...
    check(
      'integration_data_quality_incidents_error_reason_check',
      sql`error_reason = ANY (ARRAY['invalid_datetime'::text, 'invalid_timezone'::text, 'unsupported_format'::text, 'invalid_branch_id'::text, 'query_failed'::text, 'missing_or_empty'::text, 'invalid_iana'::text, 'backfill_unresolvable'::text])`,
    ),
    check(
      'integration_data_quality_incidents_status_check',
      sql`status = ANY (ARRAY['open'::text, 'resolved'::text, 'unresolved'::text])`,
    ),
  ],
);

export const emailSendCooldowns = pgTable(
  'email_send_cooldowns',
  {
    userId: uuid('user_id').notNull(),
  ...
  (table) => [
    ...
    primaryKey({
      columns: [table.userId, table.emailNormalized],
      name: 'email_send_cooldowns_pkey',
    }),
  ],
);

export const userNotificationTopics = pgTable(
  ...
  (table) => [
    index('idx_user_notification_topics_user').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
...
export const userNotificationTopicChannels = pgTable(
  ...
  (table) => [
    index('idx_user_notification_topic_channels_user').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
...
export const userWebPushSubscriptions = pgTable(
  ...
  (table) => [
    uniqueIndex('uq_user_web_push_subscriptions_endpoint').using(
      'btree',
      table.endpoint.asc().nullsLast().op('text_ops'),
    ),
    index('idx_user_web_push_subscriptions_user').using(
      'btree',
      table.userId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
