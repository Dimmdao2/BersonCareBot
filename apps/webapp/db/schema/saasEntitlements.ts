...
export const saasTariffs = pgTable(
  "saas_tariffs",
  {
    ...
    quotas: jsonb().$type<Record<string, unknown>>().notNull().default(sql`'{}'::jsonb`),
    /**
...
export const saasOrgEntitlementOverrides = pgTable(
  ...
  (table) => [
    ...
    index("idx_saas_org_entitlement_overrides_org_expiry").on(table.organizationId, table.expiresAt),
    foreignKey({
...
export const saasOrganizationTrials = pgTable(
  ...
  (table) => [
    ...
    foreignKey({ columns: [table.organizationId], foreignColumns: [beOrganizations.id], name: "saas_organization_trials_org_fkey" }).onDelete("cascade"),
    foreignKey({ columns: [table.tariffId], foreignColumns: [saasTariffs.id], name: "saas_organization_trials_tariff_fkey" }).onDelete("restrict"),
    foreignKey({ columns: [table.postTrialTariffId], foreignColumns: [saasTariffs.id], name: "saas_organization_trials_post_tariff_fkey" }).onDelete("restrict"),
    check("saas_organization_trials_dates_check", sql`${table.startedAt} < ${table.endsAt} AND ${table.endsAt} <= ${table.graceEndsAt}`),
    check("saas_organization_trials_extension_count_check", sql`${table.extensionCount} >= 0`),
    check("saas_organization_trials_status_check", sql`${table.status} = ANY (ARRAY['active'::text, 'ended'::text])`),
    check(
