...
import {
  checkIntegratorCanonicalPair,
  callIntegratorUserMerge,
} from './integratorUserMergeM2mClient';
...
describe('integratorUserMergeM2mClient (Stage 5 stubbed fetch flow)', () => {
  ...
  it('canonical-pair: parses ok response', async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      new Response(
        JSON.stringify({ ok: true, sameCanonical: true, canonicalA: '1', canonicalB: '1' }),
        {
          status: 200,
  ...
  it('users merge: parses ok result', async () => {
    vi.mocked(fetch).mockResolvedValueOnce(
      new Response(
        JSON.stringify({ ok: true, result: { winnerId: '1', loserId: '2', dryRun: true } }),
        {
          status: 200,
        },
      ),
  ...
  it('sequence canonical then merge uses two fetch calls (operator flow shape)', async () => {
    vi.mocked(fetch)
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({ ok: true, sameCanonical: false, canonicalA: '10', canonicalB: '20' }),
          {
            status: 200,
          },
        ),
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ ok: true, result: { winnerId: '10', loserId: '20' } }), {
          status: 200,
        }),
      );
