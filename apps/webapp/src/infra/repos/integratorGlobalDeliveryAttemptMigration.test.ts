...
const journal = readFileSync(join(repoDir, 'db/drizzle-migrations/meta/_journal.json'), 'utf8');
...
describe('integrator global delivery-attempt audit migration', () => {
  it('uses a narrow app_owner capability instead of a direct API-login table grant', () => {
    ...
    expect(runtimeOverlay).toContain('REVOKE INSERT ON TABLE integrator.delivery_attempt_logs');
    expect(runtimeOverlay).toContain(
