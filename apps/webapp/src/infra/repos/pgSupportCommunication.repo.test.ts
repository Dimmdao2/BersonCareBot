...
describe("createPgSupportCommunicationPort", () => {
  ...
  describe("platform support inbox reads", () => {
    it("selects the last personal message and defines unanswered by its sender", async () => {
      runWebappPgTextMock.mockResolvedValueOnce({ rows: [] });
      const port = createPgSupportCommunicationPort();

      await port.listPlatformSupportConversations({
        unansweredOnly: true,
        limit: 100,
      });
      ...
      expect(sql).toContain("m.integrator_message_id LIKE 'broadcast:%'");
      expect(sql).toContain(
        "m.integrator_message_id LIKE 'booking-created:%'",
      );
      expect(sql).toContain(
        "last_personal.sender_role = 'user'",
      );
      expect(sql).not.toContain("JOIN platform_users");
      expect(runWebappPgTextMock.mock.calls[0]?.[1]).toEqual([true, 100]);
    });

    it("returns only personal messages in the card query", async () => {
      runWebappPgTextMock
          ...
          rows: [
            {
              ...
              last_message_text: "Помогите",
              last_sender_role: "user",
              last_message_integrator_id: "telegram:user:1",
              last_message_source: "telegram",
              message_count: 1,
            },
          ],
        })
      ...
      await port.getPlatformSupportConversation(
        "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",
      );

      const messageSql = String(
        runWebappPgTextMock.mock.calls[1]?.[0] ?? "",
      );
      expect(messageSql).toContain(
        "m.source IN ('doctor_broadcast', 'appointment_lifecycle')",
      );
      expect(messageSql).toContain("AND NOT");
      expect(messageSql).not.toContain("platform_users");
    });
  });
});
