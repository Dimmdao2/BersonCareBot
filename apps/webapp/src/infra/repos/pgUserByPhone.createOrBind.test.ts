...
describe('pgUserByPhonePort.createOrBind', () => {
  ...
  it('does not let a Telegram display-name hint replace a structured profile label', async () => {
    ...
    expect(String(sql)).toContain(
      'first_name IS NOT NULL OR last_name IS NOT NULL OR patronymic IS NOT NULL',
    );
  ...
  it('rolls back when domain SQL fails before COMMIT', async () => {
    ...
    await expect(pgUserByPhonePort.createOrBind(phone, telegramCtx)).rejects.toThrow(
      'phone_lookup_failed',
    );
  ...
  it('merges duplicate owners when binding insert conflicts', async () => {
    ...
    runWebappPgTextMock
      ...
      .mockResolvedValueOnce({ rows: [], rowCount: 1 });
    enrichPickMergeCandidatesWithBookingCountsMock.mockResolvedValueOnce([
      mergeRow,
      { ...mergeRow, id: 'user-b' },
    ]);
  ...
  it('profile bind merges the existing phone owner into the authenticated patient', async () => {
    runWebappPgTextMock
      .mockResolvedValueOnce({ rows: [] })
      .mockResolvedValueOnce({
        rows: [{ id: 'phone-owner', display_name: 'Phone', role: 'client' }],
      })
      .mockResolvedValueOnce({
        rows: [{ id: 'profile-user', display_name: 'Profile', role: 'client' }],
      })
      .mockResolvedValueOnce({ rows: [], rowCount: 1 });
    mergePlatformUsersInTransactionMock.mockResolvedValueOnce({
  ...
  it('rejects invalid channel context at Zod boundary', async () => {
    await expect(
      pgUserByPhonePort.createOrBind(phone, {
        channel: 'telegram',
        chatId: '  ',
        displayName: 'x',
      }),
