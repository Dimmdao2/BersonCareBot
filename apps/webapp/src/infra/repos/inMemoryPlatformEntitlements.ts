...
export function createInMemoryPlatformEntitlementsPort(): PlatformEntitlementsPort {
  ...
  const trials = new Map<string, { tariffId: string; endsAt: string; status: "active" | "ended" }>();
  ...
  return {
    ...
    async assignTariff(organizationId, tariffId) {
      ...
      const currentManualTariffId = trial?.status === "active"
        ? null
        : organizationTariffs.get(organizationId) ?? null;
