...
const migration = readFileSync(
  path.resolve(
    ...
    "../../../db/drizzle-migrations/0265_password_login_bruteforce_protection.sql",
  ),
  "utf8",
);
...
describe("password login brute-force migration", () => {
  it("stores the canonical account counter and temporary deadline on password credentials", () => {
    expect(migration).toContain("ADD COLUMN IF NOT EXISTS failed_attempts integer NOT NULL DEFAULT 0");
  ...
  it("reuses existing accessors without adding grants or changing RLS", () => {
    ...
    expect(migration).toContain("CREATE OR REPLACE FUNCTION app.set_staff_security_self_password_hash");
  });
});
