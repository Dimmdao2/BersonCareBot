...
describe.skipIf(!enabled)("email_challenges atomic attempts increment (explicit mutating DEV/scratch opt-in)", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      if (!functionExists.rows[0]?.exists) {
        throw new Error("0247_email_challenge_atomic_attempts is not installed on the approved DEV/scratch database");
      }
    } finally {
      client.release();
    }
