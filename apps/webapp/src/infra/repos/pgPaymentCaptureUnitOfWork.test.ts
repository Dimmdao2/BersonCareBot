...
vi.mock("@/infra/db/drizzleMutationTx", () => ({
  runInDrizzleMutationTransaction: (fn: () => unknown) =>
    runInDrizzleMutationTransactionMock(fn),
}));
...
describe("createPgPaymentCaptureUnitOfWork", () => {
  beforeEach(() => {
    ...
    runInDrizzleMutationTransactionMock.mockImplementation(async (fn: () => Promise<unknown>) => fn());
  ...
  it("holds a session lock through post-commit work and unlocks after success", async () => {
    ...
    await expect(
      createPgPaymentCaptureUnitOfWork().runSerializedPostCommit("org-1", "intent:intent-1", async () => {
        order.push("delivery");
        return "ok";
      }),
    ).resolves.toBe("ok");
  ...
  it("unlocks and propagates a post-commit delivery failure", async () => {
    ...
    await expect(
      createPgPaymentCaptureUnitOfWork().runSerializedPostCommit("org-1", "event:event-1", async () => {
