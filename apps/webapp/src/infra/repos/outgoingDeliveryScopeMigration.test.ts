...
describe('0260 outgoing delivery scope text identifier fix', () => {
  ...
  it.each([
    ...
    'keeps UUID payload validation but compares reminder text keys without a column cast in the %s',
    (_label, sql) => {
  ...
  it('keeps the UUID-to-UUID operator and broadcast branches unchanged', () => {
    expect(migration).toContain('WHERE incident.id = v_incident_id');
    expect(migration).toContain('WHERE audit.id = v_broadcast_audit_id;');
