...
const repository = readFileSync(new URL('./pgSupportCommunication.ts', import.meta.url), 'utf8');
...
describe('platform support conversation isolation', () => {
  ...
  it('rehydrates the same deny boundary after a no-ACL TEST restore', () => {
    ...
    expect(overlay).toContain('support_conversations_platform_operations_select');
    expect(overlay).toContain('support_conversation_messages_platform_operations_select');
  });

  it('keeps the historical migration slot registered', () => {
    expect(journal).toContain('"tag": "0265_platform_support_conversations_read"');
  });
