...
function installTenantMediaSqlAdapter(): void {
  runWebappSqlMock.mockImplementation((_db: unknown, fragment: unknown) => {
    ...
    const hasSubmissionBypass = /program_item_submission[\s\S]*\bOR\b[\s\S]*organization_id/i.test(
      query,
    );
    const hasExactOrganizationPredicate =
      principalOrganizationId !== undefined &&
      /organization_id/i.test(query) &&
      !hasSubmissionBypass;
...
describe('createS3MediaStoragePort', () => {
  ...
  it('evaluates organization A/B for list, total, search, picker and direct media reads', async () => {
    ...
    await expect(
      getMediaRowForPlayback('bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbb2'),
    ).resolves.toBeNull();
    ...
    await expect(
      getMediaRowForPlayback('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa2'),
    ).resolves.toBeNull();
  });
  ...
  it('denies doctor B the org A submission while uploader A and staff A remain allowed', async () => {
    ...
    expect(
      canAccessProgramSubmissionMedia(
        appSession('aaaaaaaa-0000-4000-8000-000000000099', 'doctor'),
        {
          usagePurpose: submissionA!.usage_purpose,
          uploadedBy: submissionA!.uploaded_by,
        },
  ...
  it('protects a foreign pending row from invalid-S3 rollback and preserves same-org confirm/delete', async () => {
    ...
    getCurrentDbPrincipalOrganizationIdMock.mockReturnValue(ORG_B);
    await expect(deletePendingMediaFileById('eeeeeeee-eeee-4eee-8eee-eeeeeeeeeee1')).resolves.toBe(
      false,
    );
    await expect(confirmMediaFileReady('eeeeeeee-eeee-4eee-8eee-eeeeeeeeeee2')).resolves.toBe(
      false,
    );
