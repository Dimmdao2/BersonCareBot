...
describe("createS3MediaStoragePort", () => {
  ...
  it("upload puts object to S3 and inserts ready row", async () => {
    ...
    expect(result.url).toContain("11111111-1111-4111-8111-111111111111");
    expect(result.url).toContain("pic.png");
  });

  // ----- getUrl -----

  it("getUrl returns S3 public URL when s3_key is set", async () => {
    ...
    expect(url).toBe("https://fs.test/b/media/abc/file.png");
  });
  ...
  it("deleteHard calls s3DeleteObject for S3-backed file", async () => {
    // lock -> select row -> mark deleting -> delete row -> unlock
    queryMock
      .mockResolvedValueOnce({ rows: [] })
    ...
    expect(s3DeleteObjectMock).toHaveBeenCalledWith("media/x/f.mp4");
  });

  it("deleteHard returns false when row has no s3_key", async () => {
    // lock -> select row -> mark deleting -> rollback status -> unlock
    queryMock
      .mockResolvedValueOnce({ rows: [] })
