...
describe("pgPatientFiles principal scoping", () => {
  ...
  it("createFile stamps media_files and patient_files with current organization principal", async () => {
    ...
    const insertMock = vi.fn().mockReturnValueOnce({ values: mediaValues }).mockReturnValueOnce({ values: patientValues });
    ...
    expect(mediaValues).toHaveBeenCalledWith(expect.objectContaining({ organizationId: ORG_A, folderId: FOLDER_ID }));
    expect(patientValues).toHaveBeenCalledWith(expect.objectContaining({ organizationId: ORG_A, mediaFileId: MEDIA_ID }));
  });
      ...
      fn({
        select: vi
          ...
          .mockReturnValueOnce({
            from: () => ({
              where: () => ({
                limit: vi.fn().mockResolvedValue([{ organizationId: ORG_A, patientUserId: PATIENT_ID }]),
              }),
            }),
          })
          .mockReturnValueOnce({
            from: () => ({
              where: () => ({
                limit: vi.fn().mockResolvedValue([{ organizationId: ORG_B, patientUserId: PATIENT_ID }]),
              }),
            }),
          }),
      }),
      ...
      fn({
        select: vi
          ...
          .mockReturnValueOnce({
            from: () => ({
              where: () => ({
                limit: vi.fn().mockResolvedValue([{ organizationId: ORG_A, patientUserId: DOCTOR_ID }]),
              }),
            }),
          }),
      }),
