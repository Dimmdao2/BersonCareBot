...
describe("loadAdminNotificationTargetsFromDb (C-4, 2026-07-26)", () => {
  ...
  it("splits telegram/max by channel_code and collects phone/email separately, deduping", async () => {
    runWebappPgTextMock.mockResolvedValue({
      rows: [
        { phone_normalized: "+79990001122", email_normalized: "admin@example.test", channel_code: "telegram", external_id: "111" },
        { phone_normalized: "+79990001122", email_normalized: "admin@example.test", channel_code: "max", external_id: "222" },
        // Second admin, telegram only, no phone.
        { phone_normalized: null, email_normalized: "other@example.test", channel_code: "telegram", external_id: "333" },
    ...
    expect(result.email.sort()).toEqual(["admin@example.test", "other@example.test"]);
  });

  it("returns empty lists when nobody currently holds the admin role", async () => {
    runWebappPgTextMock.mockResolvedValue({ rows: [] });
    const result = await loadAdminNotificationTargetsFromDb();
    expect(result).toEqual({ telegram: [], max: [], sms: [], email: [] });
  });

  it("ignores a row with no bound channel and no phone (admin with nothing bound)", async () => {
    runWebappPgTextMock.mockResolvedValue({
      rows: [{ phone_normalized: null, email_normalized: null, channel_code: null, external_id: null }],
    });
    const result = await loadAdminNotificationTargetsFromDb();
    expect(result).toEqual({ telegram: [], max: [], sms: [], email: [] });
  });
