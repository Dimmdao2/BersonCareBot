...
/**
 * Долг за место, переезжающий в счёт периода, который начинается в `periodStartsAt` — ЕДИНСТВЕННОЕ
 * место, где решается «что считается долгом».
 *
 ...
 *
 * Долг — это неоплаченный счёт за место, чей отрезок услуги ЗАКОНЧИЛСЯ не позже начала нового
 * периода. Место за уже прошедший период при этом не отбирается: услуга оказана, и счётчик
 * `paid_additional_seats` здесь не трогается ни на единицу — отбор задним числом не делает ни один
 * из изученных биллингов (`SEAT_UNPAID_PRACTICE_2026-08-19.md` §1.1).
 *
 * Строки берутся `FOR UPDATE`: между чтением суммы и гашением долга никто не должен успеть его
 * оплатить или перевыставить.
 */
async function readSeatDebtForPeriod(
  tx: Transaction,
  input: {
    organizationId: string;
    saasBillingSubscriptionId: string;
    periodStartsAt: string;
    /** «Сейчас» выставления. Оба условия обязательны, и второе — не перестраховка: клиника вправе
     *  оплатить следующий период ДОСРОЧНО, посреди текущего, и тогда счёт за место ещё не просрочен
     *  — владелец сказал «если ДО КОНЦА ПЕРИОДА счёт не оплачен», а конец периода ещё не наступил.
     *  Без этого условия досрочная оплата отбирала бы у клиники возможность закрыть место отдельно. */
    asOf: string;
...
/**
 * Гашение долга ПОСЛЕ того, как появился счёт-преемник, куда он переехал суммой.
 *
 * Старый счёт уходит в `void` со ссылкой на преемника — это не «долга не было», а «сумма на том
 * счёте»; отличить одно от другого по статусу невозможно, поэтому преемник хранится строкой
 * (ограничение `saas_billing_invoices_superseded_is_void_check` не даёт связать преемника, оставив
 * старый счёт оплачиваемым). Без гашения одна и та же услуга осталась бы оплачиваемой дважды.
 */
async function carrySeatDebtInto(
  tx: Transaction,
  debtRows: ReadonlyArray<typeof saasBillingInvoices.$inferSelect>,
...
async function refreshSaasBillingInvoicePurchasedTariff(
  ...
  const result = await tx.execute(
    sql`SELECT app.refresh_saas_billing_invoice_purchased_tariff(${saasBillingInvoiceId}::uuid, ${organizationId}::uuid, ${tariffId}::uuid, ${billingPeriodCode}::text) AS refreshed`,
  );
...
async function promotePaidInvoice(
  ...
  await tx
    ...
    .set({
      ...
      // #1069 owner decision 2026-09-05 (period grid) — the invoice's OWN pair is what gets
      // promoted/cleared here, never the subscription's prior pending value: this is the single
      // capture-time promotion, matching how `tariffId`/`pendingTariffId` are promoted/cleared
      // right above.
      billingPeriodCode: invoice.tariffBillingPeriod,
      pendingBillingPeriodCode: null,
      status: 'active',
      lifecycleState: 'active',
      cancelledAt: null,
      currentPeriodStartsAt: invoice.servicePeriodStartsAt,
  ...
  return {
    ...
    async getOrganizationBillingOverview(organizationId) {
      ...
      return {
        ...
        invoices: invoiceRows.map((row): SaasBillingInvoiceReadRow => ({
    ...
    async chooseOrganizationFirstTariff({ tariffId, billingPeriodCode, actorId }) {
      const result = await runWebappSql<{ payload: unknown }>(
        getWebappSqlDb(),
        sql`SELECT app.choose_organization_first_tariff(${tariffId}::uuid, ${actorId}::uuid, ${billingPeriodCode}::text) AS payload`,
      );
    ...
    async listPlatformInvoices(filter): Promise<SaasBillingPlatformInvoiceRow[]> {
      ...
      return rows.map(({ invoice, organizationTitle }): SaasBillingPlatformInvoiceRow => ({
        ...
        return work({
          async loadManualAssignmentState(organizationId) {
            ...
            const [manualSaasBillingSubscription] = await tx
              ...
              .orderBy(
                desc(
                  sql<number>`CASE WHEN ${saasBillingSubscriptions.status} = 'active' THEN 1 ELSE 0 END`,
                ),
          ...
          async setManualSaasBillingSubscription({
            ...
            const [row] = await tx
              ...
              .values({
                ...
                tariffSnapshot,
                pendingTariffId,
                // `undefined` (cancel-pending path, caller leaves this untouched) must still insert
                // as `null` on a brand-new row — there is no prior pending value to inherit.
                pendingBillingPeriodCode: pendingBillingPeriodCode ?? null,
              })
              .onConflictDoUpdate({
    ...
    async createSaasBillingInvoice(input) {
      return getDrizzle().transaction(async (tx) => {
        ...
        const purchasedPair = purchasedTariffPeriodPair(subscription);
        ...
        const [tariffRow] = await tx
          ...
          .from(saasTariffs)
          .where(eq(saasTariffs.id, purchasedPair.tariffId))
          .limit(1);
        ...
        // #1069 owner decision 2026-09-05 (period grid) — the (tariff, period) PAIR being
        // purchased resolves the amount from `saas_tariff_period_prices`, never a single tariff
        // price column; `tariff` below is what every existing line further down already reads.
        const tariff = {
        ...
        // Валюта периода — своя переменная, а не поле объекта: сужение типа по проверке выше внутри
        // колбэка ниже не живёт, а «валюта может быть null» здесь уже неправда.
        const periodCurrency = tariff.currency;
        // Решение владельца 18.08: «Удалили/отключили сотрудника — со следующего периода стоимость
        ...
        // Долг за место с прошлого периода едет сюда строкой — та же дверь, что у фонового тика.
        const seatDebt = await readSeatDebtForPeriod(tx, {
          organizationId: input.organizationId,
          saasBillingSubscriptionId: input.saasBillingSubscriptionId,
        ...
        if (existingRenewal) {
          ...
          // already standing for the period is only refreshed when the TARIFF or its PERIOD
          // changed under it (#1069 owner decision 2026-09-05 — a period-only change on the same
          // tariff is itself a valid scheduled change).
          ...
          // Долг сюда приходит ТЕМ ЖЕ правилом: сверяется тот, что уже вписан в этот черновик, а не
          // сегодняшний. Долг, появившийся после того как черновик выставлен, не переоценивает его —
          // он поедет в счёт следующего периода, ровно как поступает счётчик мест строкой выше.
          if (
            existingRenewal.tariffId === tariff.tariffId &&
            existingRenewal.tariffBillingPeriod === tariff.tariffBillingPeriod &&
        ...
        // Счёт-преемник появляется ПЕРВЫМ, долг гасится только им — тот же порядок и та же причина,
        // что при перевыставлении: между двумя шагами долг не должен переставать существовать.
        const { successor } = await reissueWithSuccessor({
          issueSuccessor: () =>
            insertSaasBillingInvoiceIdempotent(tx, {
          ...
          // Долг переезжает только в СВЕЖЕ выставленный счёт. Повторный вызов, отдавший уже
          // существующую строку, ничего не гасит: сумма в ней уже посчитана своим прогоном.
          retireSuperseded: async ({ invoice, created }) => {
            if (created) await carrySeatDebtInto(tx, seatDebt.rows, invoice.id);
          },
        });
        return successor;
      });
    ...
    async captureSaasBillingPaymentSucceeded(input) {
      return getDrizzle().transaction(async (tx) => {
        ...
        const [invoice] = await tx
          ...
          .where(
            and(
              eq(saasBillingInvoices.id, invoiceIdentity.id),
              eq(saasBillingInvoices.organizationId, input.organizationId),
            ),
          )
          .limit(1)
        ...
        return captureSaasBillingPaidInvoice(invoice, {
          ...
          settleSuperseded: async () =>
            (await releaseCarriedSeatDebt(tx, invoice.id, input.organizationId)) === 'released'
              ? 'released'
              : 'blocked',
          // Снять долг не с чего: преемник уже оплачен. Услуга оплачена дважды по-настоящему, и
          // арифметикой это не чинится — это возврат, работа оператора. Счёт НЕ переводится в
          // `paid`, но и оплата не выбрасывается: она попадает туда, где оператор её видит.
          refuse: async (reason) => {
            if (reason === 'closed') return { captured: false, duplicate: true };
            await tx.insert(adminAuditLog).values({
              organizationId: input.organizationId,
              actorId: null,
            ...
            if (!wasPaid) {
              ...
              await tx
                ...
                .set({
                  ...
                  paidAt: input.paidAt,
                  updatedAt: new Date().toISOString(),
                  ...(cameFromSupersededPath ? { supersededByInvoiceId: null } : {}),
                })
    ...
    async createManualSaasBillingInvoice(input) {
      const [authority] = await getDrizzle()
        .select({
          ...
          tariffId: saasTariffs.id,
          tariffName: saasTariffs.name,
          // #1069 owner decision 2026-09-05 (period grid) — the row's OWN period, never the
          // tariff's frozen legacy column: this invoice bills whatever period the organization
          // actually carries.
          tariffBillingPeriod: saasBillingSubscriptions.billingPeriodCode,
        })
        .from(saasBillingSubscriptions)
      ...
      return getDrizzle().transaction(async (tx) => {
        ...
        if (!verdict.allowed) {
          return verdict.refusal === 'seat_invoice_not_cancellable'
            ...
            : { outcome: 'invoice_not_cancellable' as const, status: invoice.status };
        }
    ...
    async requireOwnTariffBillingSubscription(organizationId) {
      return getDrizzle().transaction(async (tx) => {
        ...
        // #1069 owner decision 2026-09-05 (period grid) — a brand-new row created here (the
        // organization has a tariff via `be_organizations.tariff_id` but no billing subscription
        // row yet) has no clinic-picked period either; fall back to the cheapest-sort-order
        ...
        const [row] = await tx
          ...
          .values({
            ...
            saasBillingAccountId: account.id,
            tariffId: tariff.id,
            billingPeriodCode: cheapestPeriod.code,
        ...
        // once. #1069 owner decision 2026-09-05 (period grid) — extended to the PAIR
        // (`purchasedTariffPeriodPair`): price, currency and seat price are all read off THAT
        // (tariff, period) row. `null` only before the very first choice was ever made.
        ...
        const [targetTariff] = await tx
          ...
          .from(saasTariffs)
          .where(and(eq(saasTariffs.id, targetTariffId), eq(saasTariffs.isActive, true)))
          .limit(1);
        if (!targetTariff) throw new Error('saas_billing_tariff_not_billable');
        // Every write path that reaches this row (the fresh insert above, `setManualSaasBillingSubscription`,
        // `promotePaidInvoice`) always names a period alongside its tariff — a pair-less row here
        // would mean one of those doors regressed, not a state this rule should paper over with a
        // guessed default.
        ...
        return {
          ...
          currentBillingPeriodCode: row.billingPeriodCode,
          purchasedTariffPriceMinor: periodPrice?.priceMinor ?? null,
    ...
    async createSaasBillingRenewalInvoiceIfAbsent(input) {
      return getDrizzle().transaction(async (tx) => {
        ...
        const periodCurrency = authority.currency;
        ...
        // Тот же долг за место и та же дверь, что у клиентского пути выше: конец оплаченного
        // периода — момент, когда неоплаченный счёт за место перестаёт стоять сам по себе и
        // становится строкой следующего счёта.
        const seatDebt = await readSeatDebtForPeriod(tx, {
          organizationId: input.organizationId,
          saasBillingSubscriptionId: input.saasBillingSubscriptionId,
          periodStartsAt: input.servicePeriodStartsAt,
        ...
        const tariffSnapshot = await readTariffSnapshotForPeriod(tx, authority.tariffId);
        const { successor: inserted } = await reissueWithSuccessor({
          ...
          // Повторный тик по уже выставленному периоду сюда попадает с `null` и долг не трогает:
          // он уже уехал в тот счёт, который выставил первый тик.
          retireSuperseded: async (row) => {
            if (row) await carrySeatDebtInto(tx, seatDebt.rows, row.id);
          },
        });
        if (inserted) {
    ...
    async promoteDueSaasBillingPaidInvoice({ organizationId, saasBillingSubscriptionId, asOf }) {
      return getDrizzle().transaction(async (tx) => {
        const [subscription] = await tx
          ...
          .where(
            and(
              eq(saasBillingSubscriptions.id, saasBillingSubscriptionId),
              eq(saasBillingSubscriptions.organizationId, organizationId),
            ),
          )
          .limit(1)
    ...
    // removed. `findSaasBillingRefundByProviderRef`/`confirmSaasBillingRefund` below stay: they
    // only settle a refund row that ALREADY existed before this cutover, so historical rows keep
    // resolving to their real provider status instead of being stuck `pending` forever.
    ...
    async confirmSaasBillingRefund({ saasBillingRefundId, organizationId, status, confirmedAt }) {
      return getDrizzle().transaction(async (tx) => {
        ...
        const [invoiceIdentity] = await tx
          ...
          .from(saasBillingInvoices)
          .where(eq(saasBillingInvoices.id, refundIdentity.saasBillingInvoiceId))
          .limit(1);
        if (!invoiceIdentity) throw new Error('saas_billing_invoice_not_found');
        const [subscription] = await tx
          .select()
          .from(saasBillingSubscriptions)
          .where(eq(saasBillingSubscriptions.id, invoiceIdentity.saasBillingSubscriptionId))
          .limit(1)
          .for('update');
        if (!subscription) throw new Error('saas_billing_subscription_not_found');
        const [invoice] = await tx
          .select()
          .from(saasBillingInvoices)
          .where(eq(saasBillingInvoices.id, invoiceIdentity.id))
          .limit(1)
          .for('update');
        ...
        const [refund] = await tx
          .select()
          .from(saasBillingRefunds)
          .where(eq(saasBillingRefunds.id, refundIdentity.id))
          .limit(1)
          .for('update');
        ...
        const [row] = await tx
          ...
          .where(
            and(eq(saasBillingRefunds.id, refund.id), eq(saasBillingRefunds.status, 'pending')),
          )
          .returning();
    ...
    // #1069 owner decision 2026-09-05 (period grid) — cancel-at-period-end. Same direct-update
    // shape as `revokeSaasBillingAutopayConsent`/`saveSaasBillingSubscriptionPaymentMethod` right
    // above and below: writes exactly one column, never touches tariff/period/snapshot, and never
    // takes access away before `currentPeriodEndsAt`.
    async cancelOwnTariffBillingSubscription({ organizationId, cancelledAt }) {
      const [existing] = await getDrizzle()
        .select({ cancelledAt: saasBillingSubscriptions.cancelledAt })
        .from(saasBillingSubscriptions)
        .where(
          and(
            eq(saasBillingSubscriptions.organizationId, organizationId),
        ...
        .limit(1);
      if (!existing) return { outcome: 'no_subscription' as const };
      if (existing.cancelledAt !== null) return { outcome: 'already_cancelled' as const };
      const [row] = await getDrizzle()
        .update(saasBillingSubscriptions)
        .set({ cancelledAt, updatedAt: new Date().toISOString() })
        .where(
          and(
            eq(saasBillingSubscriptions.organizationId, organizationId),
            eq(saasBillingSubscriptions.source, 'paid_subscription'),
            isNull(saasBillingSubscriptions.cancelledAt),
          ),
        )
        .returning({ id: saasBillingSubscriptions.id });
      return row ? { outcome: 'cancelled' as const } : { outcome: 'already_cancelled' as const };
    },
