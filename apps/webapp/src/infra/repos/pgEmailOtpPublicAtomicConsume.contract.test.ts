...
describe('email OTP public atomic consume contract', () => {
  ...
  it('is additive after 0231 and declares the ordered migration journal entry', () => {
    ...
    expect(
      readRepo('apps/webapp/db/drizzle-migrations/0136_email_otp_query_indexes.sql'),
    ).toContain('idx_email_challenges_email');
  });
  ...
  it('keeps raw OTPs and direct challenge-table access out of the public path', () => {
    ...
    expect(publicFlow).toContain(
      'consumeLatestEmailChallenge(email, hashEmailChallengeCode(code))',
    );
  ...
  it('holds the SECURITY DEFINER boundary to app_owner, app_patient, and the canonical bootstrap login', () => {
    ...
    expect(migration).not.toMatch(
      /GRANT\s+(?:SELECT|INSERT|UPDATE|DELETE|ALL)[^;]*email_challenges/i,
    );
    ...
    expect(
      handoff.match(/app\.email_otp_public_consume_latest_challenge\(text,text\)/g),
    ).toHaveLength(3);
    ...
    expect(d34Checker.replace(/'/g, '"')).toContain(
