...
describe("pgPhoneOtpLimits", () => {
  ...
  it("routes every bootstrap login-limit action through a narrow definer and names no sealed table", async () => {
    ...
    await expect(registerPhoneOtpLockout("+79990000000", 2_000_000_000)).resolves.toBe(2_000_000_120);
    ...
    expect(runWebappPgTextMock.mock.calls.every((call) => call[1]?.[0] === "+79990000000")).toBe(true);
  });
});
