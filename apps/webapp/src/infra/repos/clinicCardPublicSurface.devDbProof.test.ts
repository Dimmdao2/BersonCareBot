 ...
 * что маршрут отдаёт только члена набора, — но НЕ то, кто в этот набор попадает.
 *
...
describe.skipIf(!ENABLED)('публичная дверь визитки: что она выпускает наружу (§17.B, §17.H)', () => {
  ...
  it('наружу выходит только услуга, которую клиника опубликовала, и только своя', () => {
    ...
    /* ⛔ «Идентификатор услуги наружу не выходит» (§3.2) здесь НЕ проверяется, и это не пропуск.
       Защита стоит ступенью выше теста (§10a, ступень 1): у шва `app_seam_public_clinic_card_owner`
       колоночного SELECT на `be_clinic_services.id` НЕТ вовсе (замерено:
       `has_column_privilege(...,'id','SELECT')` = false), поэтому дверь физически не может его
       прочитать — попытка даёт 42501, а не тихую утечку. Обходить нечем, проверять нечего. */
  });
});
