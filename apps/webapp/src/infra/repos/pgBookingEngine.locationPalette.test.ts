...
describe("pgBookingEngine physical location palette assignment", () => {
  ...
  it("serializes concurrent creates, counts inactive physical rows, and excludes Online", async () => {
    ...
    runWebappTransactionMock.mockImplementation(async (callback: (tx: unknown) => Promise<unknown>) => {
            ...
            where: vi.fn(async () => {
              ...
              return [{
                value: rows.filter((row) =>
                  row.cityCode.toLowerCase() !== "online" && row.title.toLocaleLowerCase("ru") !== "онлайн"
                ).length,
              }];
            }),
          })),
    ...
    const create = (title: string) => port.createPhysicalBranchWithDefaultColor({
