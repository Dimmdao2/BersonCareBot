...
export function createPgBroadcastAuditPort(): BroadcastAuditPort {
  return {
    async append(entry): Promise<BroadcastAuditEntry> {
      ...
      const r = await pool.query(
        `INSERT INTO broadcast_audit (
           ...
           preview_only, audience_size, delivery_jobs_total, sent_count, error_count
    ...
    async list(limit = 50): Promise<BroadcastAuditEntry[]> {
      const pool = getPool();
      const r = await pool.query(
        `SELECT id, actor_id, category, audience_filter, message_title, message_body, channels, executed_at, preview_only, audience_size, delivery_jobs_total, sent_count, error_count
