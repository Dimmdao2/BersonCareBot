  ...
  return {
    async append(entry): Promise<BroadcastAuditEntry> {
      ...
      const r = await pool.query(
        `INSERT INTO broadcast_audit (actor_id, category, audience_filter, message_title, channels, preview_only, audience_size, sent_count, error_count)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
         RETURNING id, actor_id, category, audience_filter, message_title, channels, executed_at, preview_only, audience_size, sent_count, error_count`,
        [
          entry.actorId,
          entry.category,
    ...
    async list(limit = 50): Promise<BroadcastAuditEntry[]> {
      const pool = getPool();
      const r = await pool.query(
        `SELECT id, actor_id, category, audience_filter, message_title, channels, executed_at, preview_only, audience_size, sent_count, error_count
