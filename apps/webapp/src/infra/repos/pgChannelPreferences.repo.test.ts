...
describe('pgChannelPreferencesPort (repo SQL parity)', () => {
  ...
  it('upsertPreference runs INSERT then SELECT for channel', async () => {
    runWebappPgTextMock.mockResolvedValueOnce({ rows: [], rowCount: 1 }).mockResolvedValueOnce({
      rows: [
        {
    ...
    expect(
      runWebappPgTextMock.mock.calls.some((c) =>
        String(c[0]).includes('INSERT INTO user_channel_preferences'),
      ),
    ).toBe(true);
  });
  ...
  it('setPreferredAuthChannel clears preferred with early COMMIT when channel is null', async () => {
    ...
    expect(
      runWebappPgTextMock.mock.calls.some((c) =>
        String(c[0]).includes('is_preferred_for_auth = false'),
      ),
    ).toBe(true);
    expect(
      runWebappPgTextMock.mock.calls.some((c) =>
