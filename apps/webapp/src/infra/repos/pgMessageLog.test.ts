...
describe('pgMessageLog', () => {
  ...
  it('listByUser uses canonical-aware user filter and LIMIT/OFFSET params', async () => {
    ...
    const countCall = calls.find(
      (c) => String(c[0]).includes('COUNT(*)') && String(c[0]).includes('FROM message_log'),
    );
  ...
  it('listAll applies filters and pagination', async () => {
    ...
    const listCall = runWebappPgTextMock.mock.calls.find((c) =>
      String(c[0]).includes('ORDER BY sent_at DESC'),
    );
    ...
    const countCall = runWebappPgTextMock.mock.calls.find(
      (c) =>
        String(c[0]).includes('COUNT(*)::text AS c') && String(c[0]).includes('FROM message_log'),
    );
    expect(countCall).toBeDefined();
