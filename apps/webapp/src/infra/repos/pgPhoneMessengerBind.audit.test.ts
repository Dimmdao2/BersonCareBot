...
describe("pgPhoneMessengerBind admin audit log", () => {
  ...
  it("preserves global existing-row repeat update for blocked audit rows", async () => {
    getCurrentDbPrincipalOrganizationIdMock.mockReturnValue(ORG_ID);
    runWebappPgTextMock
      .mockResolvedValueOnce({ rows: [{ id: "cccccccc-cccc-4ccc-8ccc-cccccccccccc", repeat_count: 1 }], rowCount: 1 })
      .mockResolvedValueOnce({ rows: [], rowCount: 1 });
    ...
    expect(updateValues).toEqual(["cccccccc-cccc-4ccc-8ccc-cccccccccccc", expect.any(String), "error"]);
  });
});
