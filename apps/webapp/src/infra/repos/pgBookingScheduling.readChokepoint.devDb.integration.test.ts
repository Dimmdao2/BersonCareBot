...
describe.skipIf(!enabled)("taskdb #821 Phase 2: listWorkingHoursAdmin two-clinic isolation (dev DB, opt-in)", () => {
  ...
  afterAll(async () => {
    const client = await pool.connect();
    try {
      await client.query(`DELETE FROM be_working_hours WHERE organization_id IN ($1, $2)`, [clinicAId, clinicBId]);
      await client.query(`DELETE FROM be_organizations WHERE id IN ($1, $2)`, [clinicAId, clinicBId]);
    } finally {
      client.release();
      await pool.end();
    }
  ...
  it("negative: Clinic B staff never sees a Clinic A row, and vice versa, even when the query is " +
    "issued under a run()-scoped principal and awaited via Promise.all (payment-timeline/route.ts " +
    "shape) alongside an unrelated concurrent read", async () => {
    const port = createPgBookingSchedulingPort();

    const [rowsSeenByA, rowsSeenByB] = await Promise.all([
      runWithDbOrganizationPrincipal(clinicAId, () => port.listWorkingHoursAdmin({ organizationId: clinicAId })),
      runWithDbOrganizationPrincipal(clinicBId, () => port.listWorkingHoursAdmin({ organizationId: clinicBId })),
    ]);
