...
import type { NativePushAppId, NativePushProvider, NativePushTokenCipher } from '@/modules/web-push/nativePush';
...
export function createPgNativePushTargetsPort(cipher: NativePushTokenCipher) {
  const db = getDrizzle();
  return {
    async register(input: { userId: string; appId: NativePushAppId; provider: NativePushProvider; installationId: string; token: string }) {
      const installationIdHash = nativePushHash(input.installationId); const tokenHash = nativePushHash(input.token); const encrypted = cipher.encrypt(input.token);
    ...
    async listActive(userId: string): Promise<NativePushTarget[]> { const rows = await db.select().from(nativePushTargets).where(and(eq(nativePushTargets.userId, userId), isNull(nativePushTargets.deactivatedAt))); return rows.map((r) => ({ id: r.id, appId: r.appId as NativePushAppId, provider: r.provider as NativePushProvider, token: cipher.decrypt(r.tokenCiphertext, r.tokenKeyId) })); },
    ...
    async status(userId: string, appId: NativePushAppId) { const rows = await db.select({ provider: nativePushTargets.provider, active: nativePushTargets.deactivatedAt }).from(nativePushTargets).where(and(eq(nativePushTargets.userId, userId), eq(nativePushTargets.appId, appId))); return { active: rows.some((r) => r.active === null), providers: rows.filter((r) => r.active === null).map((r) => r.provider) }; },
  };
}
