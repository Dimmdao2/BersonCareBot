...
const METRIC_SQL_MARKERS: Record<DoctorAnalyticsMetricKey, string[]> = {
  ...
  clients_phone_only: ["pu.email_verified_at IS NULL", "channel_code IN ('telegram', 'max')", "bot_blocked_at IS NULL"],
  clients_app_guests: ["pu.phone_normalized IS NULL", "channel_code IN ('telegram', 'max')", "bot_blocked_at IS NULL"],
  clients_segment_telegram_only: ["channel_code = 'telegram'", "channel_code = 'max'", "bot_blocked_at IS NULL"],
  clients_segment_max_only: ["channel_code = 'max'", "channel_code = 'telegram'", "bot_blocked_at IS NULL"],
  clients_segment_email_only: ["pu.email_verified_at IS NOT NULL"],
  clients_segment_telegram_email: ["pu.email_verified_at IS NOT NULL", "channel_code = 'telegram'", "bot_blocked_at IS NULL"],
  clients_segment_max_email: ["pu.email_verified_at IS NOT NULL", "channel_code = 'max'", "bot_blocked_at IS NULL"],
  clients_segment_phone_email_no_messenger: [
    ...
    "bot_blocked_at IS NULL",
  ],
  clients_messenger_bot_blocked_telegram: ["bot_blocked_at IS NOT NULL", "channel_code = $1::text"],
  clients_messenger_bot_blocked_max: ["bot_blocked_at IS NOT NULL", "channel_code = $1::text"],
