...
describe.skipIf(!enabled)(
  'decaying OTP lockout atomic escalation (explicit mutating DEV/scratch opt-in)',
  () => {
    ...
    beforeAll(async () => {
      ...
      try {
        ...
        if (!email.rows[0]?.exists) {
          throw new Error(
            '0248_otp_decaying_lockout is not installed on the approved DEV/scratch database',
          );
        }
        ...
        if (!phoneColumn.rows[0]?.exists) {
          throw new Error(
            "0248_otp_decaying_lockout's phone_otp_locks.lockout_cycle column is missing",
          );
        }
        ...
        if (!phoneAccessor.rows[0]?.exists) {
          throw new Error(
            "0252_patient_action_accessors's phone lockout accessor is not installed",
          );
        }
      } finally {
        client.release();
    ...
    it('email: serializes two real concurrent escalations through the row lock -- cycle 1 (120s) then cycle 2 (240s), never two cycle-1s', async () => {
      ...
      try {
        ...
        try {
          await setup.query('DELETE FROM public.email_otp_locks WHERE user_id = $1::uuid', [
            userId,
          ]);
        } finally {
          observer.release();
    ...
    it('phone: serializes two real concurrent escalations through the row lock -- cycle 1 (120s) then cycle 2 (240s), never two cycle-1s', async () => {
      ...
      try {
        ...
        await Promise.allSettled([first.query('ROLLBACK'), second.query('ROLLBACK')]);
        try {
          await setup.query('DELETE FROM public.phone_otp_locks WHERE phone_normalized = $1', [
            phone,
          ]);
        } finally {
          observer.release();
