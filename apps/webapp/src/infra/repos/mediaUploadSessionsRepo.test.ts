...
describe('multipart complete organization wall', () => {
  ...
  it('claims only organization A media, never a foreign organization B session', async () => {
    ...
    const client = {} as import('pg').PoolClient;
    await expect(
      claimUploadSessionForCompletingTx(client, 'sess-foreign', 'owner-a', 'org-a'),
    ).resolves.toBeNull();
  ...
  it('classifies NULL-owned media as not found for organization A', async () => {
    runWebappSqlMock.mockResolvedValue({ rows: [] });
    const result = await classifyMultipartCompleteRejection(
      {} as import('pg').Pool,
      'sess-null',
      'owner-a',
      'org-a',
...
describe('mediaUploadSessionsRepo pool wrappers', () => {
  ...
  it('claimUploadSessionForCompleting runs claim in a shared transaction helper', async () => {
    ...
    await expect(
      claimUploadSessionForCompleting('sess-1', 'owner-1', 'org-a'),
