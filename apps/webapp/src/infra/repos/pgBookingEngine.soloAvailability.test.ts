...
function transactionHarness(options: { failPreferredUpdate?: boolean } = {}) {
  const preferred = exactRow(
    'eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee',
    true,
    '2026-07-19T00:00:00.000Z',
  );
  const duplicate = exactRow(
    ...
    update: vi.fn(() => ({
      set: vi.fn((value: unknown) => {
        ...
        return {
          where: vi.fn(() => ({
            returning: vi.fn(async () => {
              if (options.failPreferredUpdate && current === 1)
                throw new Error('preferred_update_failed');
...
describe('pgBookingEngine solo location availability transaction', () => {
  ...
  it('propagates a preferred-row failure so the transaction rolls back the location and duplicate writes', async () => {
    ...
    await expect(
      createPgBookingEnginePort().setSoloServiceLocationAvailability(input),
    ).rejects.toThrow('preferred_update_failed');
