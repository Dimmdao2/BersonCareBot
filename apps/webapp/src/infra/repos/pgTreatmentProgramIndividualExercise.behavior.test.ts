...
function buildTx(params: { selectRows: DbRow[][]; insertRows?: DbRow[][] }) {
...
describe('pgTreatmentProgramInstance individual exercise boundaries', () => {
  ...
  it('creates through the direct port only inside the exact organization', async () => {
    ...
    runMutationMock.mockImplementation((fn: (tx: typeof harness.tx) => Promise<unknown>) =>
      fn(harness.tx),
    );

    const result =
      await createPgTreatmentProgramInstancePort().createIndividualExerciseAndStageItem(baseInput);
