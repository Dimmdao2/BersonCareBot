...
describe('locked patient runtime capabilities', () => {
  ...
  it('keeps patient support activity server-owned, same-transaction and capability-only', () => {
    const migration = text(
      '../../../db/drizzle-migrations/0234_current_patient_support_activity.sql',
    );
  ...
  it('keeps the E1 ACLs capability-only and does not grant patient aggregate-table updates', () => {
    ...
    const analyticsMigration = text(
      '../../../db/drizzle-migrations/0200_current_patient_product_analytics.sql',
    );

    expect(e1).toContain('GRANT EXECUTE ON FUNCTION app.record_current_patient_analytics_event');
    expect(e1).not.toMatch(
      /GRANT\s+(?:INSERT|UPDATE)[^;]*product_analytics_(?:hourly|user_hourly)[^;]*TO app_patient/is,
    );
    expect(e1).toContain(
    ...
    expect(analyticsMigration).toContain(
      'GREATEST(public.product_analytics_user_hourly.last_seen_at',
    );
    expect(analyticsMigration).toContain(
      "p_event_type NOT IN ('app_open', 'page_view', 'heartbeat')",
    );
  ...
  it('keeps patient UI settings and calendar writes behind signed bounded capabilities', () => {
    const migration = text(
      '../../../db/drizzle-migrations/0202_current_patient_ui_capabilities.sql',
    );
    const overlay = text('../../../../../deploy/postgres/e1-webapp-runtime-config.sql');
  ...
  it('keeps branch/service booking enrichment inside the signed capability, not a direct app_patient join (taskdb #1046)', () => {
    const migration = text('../../../db/drizzle-migrations/0262_remove_rubitime_data.sql');
    const bookings = text('./pgPatientBookings.ts');
    ...
    expect(bookings).not.toMatch(
      /\bJOIN\s+be_(branches|clinic_services|appointments|specialist_service_availability)\b/i,
    );
  });
});
