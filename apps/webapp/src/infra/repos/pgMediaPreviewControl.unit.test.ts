 ...
 *
 *  1. `standard_rendition_at` ставится тем же оператором, что перенаправляет `s3_key` — это
 *     единственный факт строки о том, что объект наш, и интерфейс показывает файл по нему одному;
 *  2. вытесненный исходник — это ПРЕЖНИЙ ключ строки, а не то, что прислал воркер, и удаляется он
 *     только после того, как строка уже указывает на рендишн;
...
describe('completeMediaPreviewImage', () => {
  it('records standard_rendition_at in the same statement that repoints s3_key', async () => {
    nextSelectRows = [{ s3Key: 'media/raw/photo.jpg', storageTarget: 'patient' }];
  ...
  it('deletes the row previous object, not a key the worker names', async () => {
    nextSelectRows = [{ s3Key: 'media/raw/photo.jpg', storageTarget: 'patient' }];
    ...
    expect(s3DeleteObject).toHaveBeenCalledWith('media/raw/photo.jpg', 'patient');
  });

  it('deletes nothing when the row already points at the rendition', async () => {
    nextSelectRows = [{ s3Key: `media/${MEDIA_ID}/standard.webp`, storageTarget: 'library' }];
