...
vi.mock('@/infra/repos/pgCanonicalPlatformUser', () => ({
  ...
  findTrustedCanonicalUserIdByPhone: (...args: unknown[]) =>
    findTrustedCanonicalUserIdByPhoneMock(...args),
}));

vi.mock('@/infra/repos/pgUserProjection', () => ({
  mergeCanonicalPlatformUserCandidates: (...args: unknown[]) =>
    mergeCanonicalPlatformUserCandidatesMock(...args),
}));
...
describe('pgIdentityResolutionPort', () => {
  ...
  it('findOrCreateByChannelBinding rolls back when insert fails', async () => {
    runWebappPgTextMock
      .mockResolvedValueOnce({ rows: [] })
      .mockRejectedValueOnce(new Error('insert_failed'));
