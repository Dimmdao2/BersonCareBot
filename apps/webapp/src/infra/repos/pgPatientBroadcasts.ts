...
import type {
  ...
  PatientBroadcastView,
} from '@/modules/patient-broadcasts/ports';

const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

export function createPgPatientBroadcastsPort(): PatientBroadcastsPort {
  return {
    async getBroadcastForPatient(
      auditId: string,
      ...
      return {
        title,
        body: extractBroadcastBodyContent(
          title,
          typeof row.message_body === 'string' ? row.message_body : '',
        ),
