...
describe.sequential('inMemoryReferencesPort', () => {
  ...
  it('lists categories and all category items for management', async () => {
    ...
    const allItems =
      await inMemoryReferencesPort.listItemsForManagementByCategoryCode('symptom_type');
  ...
  it('softDeleteItem removes row from management and active lists', async () => {
    ...
    expect(
      (await inMemoryReferencesPort.listItemsForManagementByCategoryCode('symptom_type')).some(
        (i) => i.id === added.id,
      ),
    ).toBe(true);
