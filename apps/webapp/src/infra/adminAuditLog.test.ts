...
vi.mock('@bersoncare/db-principal', () => ({
  ...
  applyDbPrincipalToConnection: vi.fn(async () => false),
  applyDbPrincipalToTransaction: vi.fn(async () => false),
...
describe('writeAuditLog', () => {
  ...
  it('uses default organization when no db principal is active', async () => {
    ...
    expect(runWebappPgTextMock.mock.calls[0]?.[1]?.[0]).toBe(
  ...
  it('logs and swallows DB errors', async () => {
    ...
    await expect(
      writeAuditLog(poolStub, { actorId: null, action: 'test_action' }),
...
describe('upsertOpenConflictLog', () => {
  ...
  it('merges seenEventTypes from details and eventType', async () => {
    ...
    const updateCall = runWebappPgTextMock.mock.calls.find((c) =>
      String(c[0]).includes('UPDATE admin_audit_log'),
    );
  ...
  it('handles unique race by updating existing open conflict row', async () => {
    ...
    runWebappPgTextMock.mockImplementation(async (sql: string) => {
      if (sql.includes('SELECT id, details, repeat_count')) {
        ...
        return {
          rowCount: 1,
          rows: [{ id: 'r2', details: { seenEventTypes: [] }, repeat_count: 1 }],
        };
      }
    ...
    expect(res).toEqual({ kind: 'conflict', insertedFirst: false });
    expect(
      runWebappPgTextMock.mock.calls.some((c) => String(c[0]).includes('UPDATE admin_audit_log')),
    ).toBe(true);
  });
  ...
  it('insert uses custom action and conflictKey when provided', async () => {
    ...
    const insertCall = runWebappPgTextMock.mock.calls.find((c) =>
      String(c[0]).includes('INSERT INTO admin_audit_log'),
    );
...
describe('listAdminAuditLog', () => {
  ...
  it('excludeActionPrefix adds NOT LIKE filter', async () => {
    ...
    await listAdminAuditLog(poolStub, {
      page: 1,
      limit: 10,
  ...
  it('list query joins platform_users for actor_display_name', async () => {
    ...
    const listCall = runWebappPgTextMock.mock.calls.find((c) =>
      String(c[0]).includes('LEFT JOIN platform_users'),
    );
