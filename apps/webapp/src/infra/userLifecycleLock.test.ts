...
import {
  withTwoUserLifecycleLocksExclusive,
  withUserLifecycleLock,
} from '@/infra/userLifecycleLock';
...
describe('withUserLifecycleLock', () => {
  ...
  it('runs exclusive lock after BEGIN and before callback body', async () => {
    ...
    expect(pgAdvisoryXactLock).toHaveBeenCalledWith(
      expect.anything(),
      '00000000-0000-4000-8000-000000000001',
    );
  ...
  it('uses shared lock when mode is shared', async () => {
    ...
    expect(pgAdvisoryXactLockShared).toHaveBeenCalledWith(
      expect.anything(),
      '00000000-0000-4000-8000-000000000002',
    );
