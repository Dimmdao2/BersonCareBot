...
function row(
  p: Partial<MergePreviewPlatformUserRow> & { id: string },
): MergePreviewPlatformUserRow {
  return {
...
describe('analyzeMergePreviewModel', () => {
  ...
  it('flags conflicting telegram bindings', () => {
    ...
    const m = analyzeMergePreviewModel(
      ...
      baseOpts({
        ...
        duplicateBindings: [
          { channel_code: 'telegram', external_id: '222', created_at: new Date() },
        ],
      }),
    );
  ...
  it('flags oauth conflict for same provider', () => {
    ...
    const m = analyzeMergePreviewModel(
      ...
      baseOpts({
        targetOauth: [
          { provider: 'google', provider_user_id: 'ga', email: null, created_at: new Date() },
        ],
        duplicateOauth: [
          { provider: 'google', provider_user_id: 'gb', email: null, created_at: new Date() },
        ],
      }),
    );
  ...
  it('hard blocker: different non-null integrator_user_id', () => {
    ...
    const m = analyzeMergePreviewModel(target, duplicate, baseOpts({}));
    expect(m.hardBlockers.some((b) => b.code === 'different_non_null_integrator_user_id')).toBe(
      true,
    );
    expect(m.mergeAllowed).toBe(false);
    expect(m.v1MergeEngineCallable).toBe(false);
  ...
  it('v2 canonical aligned: no integrator hard blocker when pair preview says aligned', () => {
    ...
    expect(m.hardBlockers.some((b) => b.code === 'integrator_canonical_merge_required')).toBe(
      false,
    );
    expect(m.mergeAllowed).toBe(true);
  ...
  it('hard blocker: shared phone with meaningful data on both (shared-phone guard)', () => {
    ...
    expect(m.hardBlockers.some((b) => b.code === 'shared_phone_both_have_meaningful_data')).toBe(
      true,
    );
    expect(m.mergeAllowed).toBe(false);
  });
