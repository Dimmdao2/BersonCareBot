...
describe('buildMergePreview (read-only load path)', () => {
  beforeEach(() => {
    ...
    checkCanonicalMock.mockResolvedValue({
      ok: true,
      sameCanonical: true,
      canonicalA: '1',
      canonicalB: '1',
  ...
  it('issues only SELECT/COUNT queries (no mutations) for happy path', async () => {
    runPgPoolPgTextMock.mockImplementation(
      async (_pool: unknown, sql: string, values?: unknown[]) => {
        const s = String(sql);
  ...
  it('searchMergeCandidates returns read-only candidate rows', async () => {
    runPgPoolPgTextMock.mockResolvedValueOnce({ rows: [platformRow(T)] }).mockResolvedValueOnce({
      rows: [
        {
          id: D,
          display_name: 'Dup',
