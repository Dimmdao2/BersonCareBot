...
function createFakeClient(
  queryImpl?: (sql: string, values?: readonly unknown[]) => Promise<unknown>,
): FakeClient {
  return {
...
describe('webapp pool provider', () => {
  ...
  it('routes the platform principal through staff transport, SET ROLEs narrowly, and cleans up', async () => {
    ...
    await runWithDbPlatformPrincipal(
      { platformUserId: STAFF_USER_ID, source: 'platform-test' },
  ...
  it('routes patient, bootstrap, and missing principals to the nonstaff pool before checkout', async () => {
    ...
    await runWithDbPatientPrincipal(
      { organizationId: ORG_ID, platformUserId: PATIENT_USER_ID },
      () => pool.query('SELECT patient_marker'),
    );
    await runWithDbBootstrapPrincipal({ source: 'unit-test' }, () =>
      pool.query('SELECT bootstrap_marker'),
    );
  ...
  it('keeps a bootstrap snapshot on the nonstaff pool when the request principal mutates during checkout', async () => {
    ...
    expect(
      statements.some((statement) => String(statement).includes('app.install_signed_context')),
    ).toBe(false);
  });
  ...
  it('routes only the exact Web Push reminder infra source to its operational pool and cleans it', async () => {
    ...
    await runWithDbInfraPrincipal(
      { source: WEB_PUSH_REMINDER_INFRA_SOURCE, organizationId: ORG_ID },
    ...
    expect(pools[2]?.clients[0]?.query).toHaveBeenCalledWith(
      "SELECT set_config('app.org', $1, false)",
      [ORG_ID],
    );
  ...
  it('fails closed before checkout when the exact Web Push reminder source has no operational URL', async () => {
    ...
    await expect(
      runWithDbInfraPrincipal({ source: WEB_PUSH_REMINDER_INFRA_SOURCE }, () =>
        pool.query('SELECT denied'),
      ),
  ...
  it('keeps the operational pool when request traffic uses one legacy connection string', async () => {
    ...
    await runWithDbInfraPrincipal({ source: WEB_PUSH_REMINDER_INFRA_SOURCE }, () =>
      pool.query('SELECT marker'),
    );
  ...
  it('classifies an injected PostgreSQL RLS denial without forwarding SQL or error text', async () => {
    const rlsError = Object.assign(
      new Error('new row violates row-level security policy for table private'),
      {
        code: '42501',
      },
    );
    ...
    expect(JSON.stringify(reportSaasIsolationEventBestEffortMock.mock.calls)).not.toContain(
      'tenant_data',
    );
    expect(JSON.stringify(reportSaasIsolationEventBestEffortMock.mock.calls)).not.toContain(
      'private',
  ...
  it.each([
    ...
    'preserves the %s operation family through an async pool query failure',
    async (family) => {
      const rlsError = Object.assign(
