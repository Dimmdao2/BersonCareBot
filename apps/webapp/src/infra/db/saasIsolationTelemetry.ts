...
function writerConnectionString(): string {
  const value = (process.env.DATABASE_URL_NONSTAFF || env.DATABASE_URL || process.env.DATABASE_URL || "").trim();
...
function operatorConnectionString(): string {
  ...
  if (!value) throw new Error(`${SAAS_ISOLATION_OPERATOR_DATABASE_URL_ENV} is required for operator diagnostics`);
  return value;
}
