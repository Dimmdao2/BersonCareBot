...
import type { PatientInvitePublicPreview } from "@/modules/patient-invites/ports";
...
async function request(path: string, body: Record<string, string>): Promise<{ response: Response; json: ApiResult }> {
...
export function JoinPatientClient({ preview }: { preview: PatientInvitePublicPreview | null }) {
    ...
    return (
      <main className="mx-auto flex min-h-screen w-full max-w-md items-center px-4 py-8">
        <div className="w-full rounded-xl border border-border bg-card p-5 text-center">
          <h1 className="text-lg font-semibold text-foreground">Ссылка недействительна</h1>
          <p className="mt-2 text-sm text-muted-foreground">Попросите специалиста создать новую ссылку.</p>
        </div>
      </main>
    );
  }
  ...
  async function confirm() {
    ...
    setError(null);
    try {
      const { response, json } = await request("/api/join/email/confirm", { code });
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-md items-center px-4 py-8">
      <div className="flex w-full flex-col gap-4 rounded-xl border border-border bg-card p-5">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Приглашение от «{preview.organizationTitle}».</p>
          {preview.recipientHint ? (
            <p className="mt-1 text-sm text-muted-foreground">Адрес для подтверждения: {preview.recipientHint}</p>
          ) : null}
        </div>
        {step === "email" ? (
          <>
          ...
          <>
            ...
            <Button type="button" variant="outline" disabled={pending} onClick={() => { setStep("email"); setCode(""); setError(null); }}>
              Ввести email заново
            </Button>
          </>
        )}
