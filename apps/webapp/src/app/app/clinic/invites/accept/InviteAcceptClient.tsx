...
import { OtpCodeForm, type OtpConfirmResult, type OtpResendOutcome } from "@/shared/ui/patient/auth/OtpCodeForm";
...
export function InviteAcceptClient({ token }: { token: string }) {
  ...
  const lookup = useCallback(async () => {
    ...
    try {
      ...
      if (typeof json.invited_email !== "string" || (json.invited_role !== "admin" && json.invited_role !== "doctor")) {
      ...
      setPreview({
        ...
        organizationTitle: typeof json.organizationTitle === "string" ? json.organizationTitle : null,
      });
    } catch {
  ...
  async function start(): Promise<StartOutcome> {
    ...
    try {
      ...
      setRetryAfterSeconds(typeof json.retryAfterSeconds === "number" ? json.retryAfterSeconds : 60);
      return { kind: "ok" };
    } catch {
    ...
    return (
      <main className="mx-auto flex min-h-screen w-full max-w-md items-center px-4 py-8">
        <div className="flex w-full flex-col gap-3 rounded-lg border border-border bg-card p-4">
          ...
          {!pageIssue.terminal ? (
            <Button type="button" variant="outline" onClick={() => void lookup()}>Повторить</Button>
          ) : null}
          <Button type="button" variant="outline" onClick={() => router.replace("/app")}>Ко входу</Button>
        </div>
      </main>
    );
  }
  ...
  if (!preview) {
    return <main className="mx-auto flex min-h-screen max-w-md items-center px-4 text-sm text-muted-foreground">Проверяем приглашение…</main>;
  }
      ...
      <div className="flex w-full flex-col gap-4 rounded-lg border border-border bg-card p-4">
        <div className="flex flex-col gap-1">
          ...
          <p className={patientMutedTextClass}>
            {preview.organizationTitle ? `${preview.organizationTitle} приглашает вас ${roleLabel}.` : `Вас приглашают ${roleLabel}.`}
          </p>
          <p className={patientMutedTextClass}>Подтвердите email {preview.invitedEmail} одноразовым кодом.</p>
        </div>
        {!challengeId ? (
          <>
            {startIssue ? <p className="text-sm text-[var(--patient-color-danger)]">{startIssue.message}</p> : null}
            <Button
              type="button"
              onClick={async () => { applyStartOutcome(await start()); }}
