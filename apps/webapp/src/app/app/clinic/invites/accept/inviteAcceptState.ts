...
const tokenIssues: Record<"invalid_token" | "expired_token" | "reused_token", InviteAcceptIssue> = {
  ...
  reused_token: {
    ...
    message: "Эта ссылка уже использована, отозвана или заменена новой. Попросите актуальное приглашение.",
    terminal: true,
  },
};
  ...
  switch (code) {
    case "invalid_body":
      return {
        ...
        message: "Обновите страницу и повторите попытку. Если ошибка останется, попросите новое приглашение.",
        terminal: false,
      };
    case "email_mismatch":
      return {
        title: "Приглашение предназначено для другого email",
        message: "Эта ссылка работает только с email, указанным в приглашении. Попросите отправителя проверить адрес.",
        terminal: true,
      };
    case "invalid_code":
      return {
    ...
    case "email_conflict":
      return {
        ...
        message: "Этот email нельзя связать с приглашением автоматически. Войдите в нужный аккаунт или обратитесь в поддержку.",
        terminal: true,
      };
    case "network_error":
      return {
        title: phase === "lookup" ? "Не удалось проверить приглашение" : "Не удалось связаться с сервером",
