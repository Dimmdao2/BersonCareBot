...
function uploadErrorMessage(error: string | undefined): string {
  if (error === 'file_storage_limit_not_configured') {
    return 'Невозможно загрузить файл: нет действующего тарифа. Назначьте тариф, чтобы загружать файлы.';
  }
  if (error === 'file_storage_limit_reached') {
    return 'Невозможно загрузить файл: хранилище заполнено. Увеличьте объём файлов в тарифе, чтобы загружать новые файлы.';
  }
