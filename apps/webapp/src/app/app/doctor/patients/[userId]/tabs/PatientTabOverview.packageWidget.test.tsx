...
function buildFetchMock(packages: PackageMockItem[] = [], program?: ProgramMock) {
  return vi.fn(async (url: string, init?: RequestInit) => {
    ...
    if (u.includes('conversations/ensure')) {
      return new Response(JSON.stringify({ ok: true, messages: [], unreadFromUserCount: 0 }), {
        status: 200,
      });
    }
    ...
    if (u.includes('/api/doctor/treatment-program-instances/')) {
      return new Response(
        JSON.stringify({
          ok: true,
          item: program ? { ...program.active, stages: program.stages } : null,
        }),
        { status: program ? 200 : 404 },
      );
    }
    if (u.includes('/treatment-program-instances')) {
      return new Response(JSON.stringify({ ok: true, items: program ? [program.active] : [] }), {
        status: 200,
      });
    }
    if (u.includes('/clinical')) {
      return new Response(JSON.stringify({ ok: true, state: { complaints: [] }, visits: [] }), {
        status: 200,
      });
    }
...
describe('PatientTabOverview — Package KPI widget (ST-07)', () => {
      ...
      buildFetchMock([
        {
          ...
          validUntil: null,
          balance: {
            items: [{ quantityInitial: 5, remaining: 3, serviceTitle: 'Массаж' }],
          },
        },
      ]),
    );
  ...
  it('renders multiple active package details separated by comma', async () => {
      ...
      buildFetchMock([
        {
          ...
          balance: {
            items: [
              { quantityInitial: 10, remaining: 5, displayRemaining: 6, serviceTitle: 'ЛФК' },
            ],
          },
        },
        {
          ...
          balance: {
            items: [
              { quantityInitial: 4, remaining: 2, displayRemaining: 3, serviceTitle: 'Массаж' },
            ],
          },
        },
      ]),
    ...
    expect(
      screen.getByText('6 x ЛФК аб #004 от 04.06.2026, 3 x Массаж аб #005 от 05.06.2026'),
    ).toBeInTheDocument();
  });
        ...
        initialPackages={[
          {
            ...
            balance: {
              items: [
                { quantityInitial: 2, remaining: 1, displayRemaining: 2, serviceTitle: 'ЛФК' },
              ],
            },
          },
        ]}
