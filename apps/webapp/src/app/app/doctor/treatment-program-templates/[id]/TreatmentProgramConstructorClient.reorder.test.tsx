...
let capturedPipelineReorder:
  | ((activeId: string, overId: string) => void | Promise<void>)
  | undefined;
  ...
  it('item DnD between custom groups PATCHes groupId then POST items/reorder', async () => {
    ...
    await waitFor(() => {
      ...
      const reorderHit = fetchMock.mock.calls.some(([input, init]) => {
        const url = requestUrl(input as RequestInfo);
        if ((init as RequestInit | undefined)?.method !== 'POST' || !url.includes('/items/reorder'))
          return false;
    ...
    await waitFor(() => {
      expect(
        fetchMock.mock.calls.some(([input, init]) => {
          const url = requestUrl(input as RequestInfo);
          return (
            (init as RequestInit | undefined)?.method === 'POST' && url.includes('/items/reorder')
          );
        }),
      ).toBe(true);
    });
    ...
    await waitFor(() => {
      expect(
        fetchMock.mock.calls.some(([input, init]) => {
          const url = requestUrl(input as RequestInfo);
          return (
            (init as RequestInit | undefined)?.method === 'POST' && url.includes('/stages/reorder')
          );
        }),
      ).toBe(true);
    });
