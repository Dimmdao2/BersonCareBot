...
async function runWriteOption<T>(options: LfkTemplateWriteOptions | undefined, fn: () => Promise<T>) {
...
describe("doctor lfk template actions principal boundaries", () => {
  ...
  it("creates from editor with separate create and exercise-update principal sources", async () => {
    createTemplateMock.mockImplementation(async (_input, _actorId, options: LfkTemplateWriteOptions) =>
      runWriteOption(options, async () => {
        expect(principalState.inside).toBe(true);
        return { id: "tpl-1" };
      }),
    );
    updateExercisesMock.mockImplementation(async (_templateId, _exercises, options: LfkTemplateWriteOptions) =>
      runWriteOption(options, async () => {
        expect(principalState.inside).toBe(true);
      }),
    );
  ...
  it("keeps template pre-read outside principal before draft update writes", async () => {
    ...
    updateTemplateMock.mockImplementation(async (_templateId, _input, options: LfkTemplateWriteOptions) =>
      runWriteOption(options, async () => {
        expect(principalState.inside).toBe(true);
      }),
    );
  ...
  it("archives and unarchives under distinct principal sources", async () => {
    archiveTemplateMock.mockImplementation(async (_templateId, _archiveOptions, options: LfkTemplateWriteOptions) =>
      runWriteOption(options, async () => {
        expect(principalState.inside).toBe(true);
      }),
    );
    unarchiveTemplateMock.mockImplementation(async (_templateId, options: LfkTemplateWriteOptions) =>
      runWriteOption(options, async () => {
        expect(principalState.inside).toBe(true);
      }),
    );
