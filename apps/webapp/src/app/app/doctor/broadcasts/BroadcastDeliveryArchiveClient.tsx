...
export function BroadcastDeliveryArchiveClient() {
  ...
  const load = useCallback(async (cursor: string | null, append: boolean) => {
    ...
    try {
      ...
      qs.set('limit', '40');
      const res = await fetch(`/api/doctor/health-failure-archive?${qs}`, {
        credentials: 'include',
        cache: 'no-store',
      });
  ...
  return (
    <div className="space-y-4">
            ...
            <Card key={row.id}>
              <CardHeader className="py-3">
                <CardTitle className="text-sm font-medium">
                  {s(row.summaryJson, 'broadcast_title_short')} ·{' '}
                  {s(row.summaryJson, 'recipient_short_name')}
                </CardTitle>
              </CardHeader>
      ...
      <p className="text-xs text-muted-foreground">
        Счётчики журнала рассылки при архивации не меняются.
      </p>
    </div>
  );
}
