...
describe('BroadcastAuditEntryDetail', () => {
  it('shows title, full text, audience, channel, errors, and non-delivery metrics', () => {
    render(
      <BroadcastAuditEntryDetail entry={makeEntry()} onClose={() => {}} onOpenErrors={() => {}} />,
    );

    const detail = screen.getByTestId('broadcast-selected-detail');
    expect(
      within(detail).getByRole('heading', { name: 'Напоминание о приёме' }),
    ).toBeInTheDocument();
  ...
  it('has exactly one close control in the top panel', async () => {
    ...
    const { container } = render(
      <BroadcastAuditEntryDetail entry={makeEntry()} onClose={onClose} onOpenErrors={() => {}} />,
    );

    const closeButtons = screen.getAllByRole('button', { name: 'Закрыть просмотр рассылки' });
    expect(closeButtons).toHaveLength(1);
    expect(container.querySelector('section > div:first-child')?.contains(closeButtons[0])).toBe(
      true,
    );
