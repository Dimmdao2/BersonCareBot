...
describe('patient-home owner server actions', () => {
  ...
  it('fails closed for an ordinary specialist', async () => {
    ...
    await expect(savePatientHomePracticeTargetAction(3)).resolves.toEqual({
      ok: false,
      error: 'forbidden',
    });
  ...
  it('allows an owner to save both repeat cooldowns in the organization', async () => {
    await expect(
      savePatientHomeRepeatCooldownsAction({ warmupRepeatMinutes: 30, planItemRepeatMinutes: 45 }),
  ...
  it('rejects a platform admin without a clinical workspace', async () => {
    ...
    await expect(savePatientHomePracticeTargetAction(4)).resolves.toEqual({
      ok: false,
      error: 'forbidden',
    });
    expect(updateSettingMock).not.toHaveBeenCalled();
