...
export async function saveContentPage(
   ...
   * разрешённых хостингов (решение владельца 19.08). Внешняя ссылка проходит allowlist и
   * канонизацию здесь, до записи в базу.
   *
   * `videoType` хранит только способ доставки (`api` | `url`). Конкретный хост не является
   * состоянием страницы: он вычисляется из URL там, где нужен плееру или аналитике.
   */
  ...
  if (editingId) {
    ...
    if (existingById.slug !== slug) {
      try {
        ...
        return {
          ok: false,
          error: 'Страница сохранена, но не удалось обновить ссылки на главной и в напоминаниях.',
        };
      }
    }
