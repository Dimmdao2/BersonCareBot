...
export function AdminMergeAccountsPanel({
  ...
  const summaryLines = useMemo(() => {
    ...
    for (const ch of ['telegram', 'max', 'vk'] as const) {
      ...
      if (w === 'both')
        lines.push(`${CH_LABELS[ch]}: без конфликта (перенос привязок как при авто-merge)`);
      else
        lines.push(
          `${CH_LABELS[ch]}: оставить привязку стороны «${w === 'target' ? 'целевая' : 'дубликат'}»`,
        );
    }
    ...
    >
      ...
      <>
        ...
        <div className="space-y-1.5">
          ...
          <Select value={secondUserId} onValueChange={(v) => setSecondUserId(v ?? '')}>
            <SelectTrigger id="merge-dup-select" className="w-full">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        {preview && resolution ? (
          <div className="space-y-4 border-t border-border/60 pt-4">
            {preview.hardBlockers.length > 0 ? (
              ...
              >
                <p className="font-medium text-destructive">
                  Жёсткие блокировки — merge недоступен
                </p>
                  ...
                  >
                    <div className="text-sm">
                      ...
                      <span className="block text-xs text-muted-foreground mt-1">
                        цел.: {o.targetProviderUserId ?? '—'} · дубл.:{' '}
                        {o.duplicateProviderUserId ?? '—'}
                      </span>
                    </div>
                    <RadioGroup
            ...
            <div className="space-y-1.5">
              <Label htmlFor="merge-ch-prefs">
                Предпочтения каналов (user_channel_preferences)
              </Label>
              <Select
                ...
                onValueChange={(v) =>
                  setResolution((r) =>
                    r
                      ? {
                          ...r,
                          channelPreferences: (v ??
                            'keep_newer') as ManualMergeResolution['channelPreferences'],
                        }
                      : r,
                  )
                }
              >
                <SelectTrigger id="merge-ch-prefs" className="w-full max-w-md">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="rounded-md border border-amber-500/40 bg-amber-500/5 p-3 text-sm space-y-2">
              ...
              {!preview.mergeAllowed ? (
                ...
                <p className="text-xs text-muted-foreground">
                  Проверьте выбор OAuth и соответствие resolution паре.
                </p>
              ) : (
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {!preview.mergeAllowed ? (
                <span className="text-xs text-destructive">
                  Кнопка отключена из‑за жёстких блокировок.
                </span>
              ) : null}
            </div>
          </div>
