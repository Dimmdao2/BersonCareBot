    ...
    const load = async () => {
      ...
      try {
        const [metricsRes, dayRes] = await Promise.all([
          fetch(
            `/api/doctor/comments/exercise-metrics?instanceId=${instanceId}&stageItemId=${stageItemId}&windowDays=${windowDays}`,
          ),
          fetch(
            `/api/doctor/clients/${userId}/program-day-activity?instanceId=${instanceId}&windowDays=${windowDays}`,
          ),
        ]);
        ...
        <div className="rounded-lg border border-border bg-card p-4">
          ...
          <div className="mt-4 text-xs text-muted-foreground">
            ...
            {metricPoints.length > 0 && (
              <p>
                First point: reps={metricPoints[0]?.reps}, weight={metricPoints[0]?.weightKg}, diff=
                {metricPoints[0]?.difficulty}
              </p>
            )}
          </div>
        </div>
