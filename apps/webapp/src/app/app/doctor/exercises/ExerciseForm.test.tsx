...
vi.mock('@/shared/ui/doctor/ReferenceSelect', () => ({
  ReferenceSelect: ({ value, name }: { value: string | null; name?: string }) => (
    <>
      ...
      <input
        type="text"
        data-testid={name ? `${name}-select` : 'ref-select'}
        readOnly
        value={value ?? ''}
      />
    </>
...
describe('ExerciseForm', () => {
  ...
  it('resets controlled fields when exercise id changes without remount key', () => {
    const saveAction = vi.fn(
      async (_prev: SaveDoctorExerciseState | null): Promise<SaveDoctorExerciseState> => ({
        ok: true,
      }),
    );
  ...
  it('opens archive warning on USAGE_CONFIRMATION_REQUIRED and resubmits with acknowledgeUsageWarning', async () => {
    ...
    expect(
      await screen.findByRole('heading', { name: /элемент уже используется/i }),
    ).toBeInTheDocument();
