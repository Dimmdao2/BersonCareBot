...
async function runWriteOption<T>(options: LfkExerciseWriteOptions | undefined, fn: () => Promise<T>) {
...
describe("doctor exercise action shared principal writes", () => {
  ...
  it("creates an exercise inside doctor workspace principal with the create source", async () => {
    createExerciseMock.mockImplementation(async (_input, _actorId, options: LfkExerciseWriteOptions) =>
  ...
  it("archives inside doctor workspace principal with the archive source", async () => {
    archiveExerciseMock.mockImplementation(async (
      _id,
      _archiveOptions,
    ...
    const result = await archiveDoctorExerciseCore(formWith({ id: exerciseId, acknowledgeUsageWarning: "1" }));
  ...
  it("unarchives inside doctor workspace principal with the unarchive source", async () => {
    ...
    expect(unarchiveExerciseMock).toHaveBeenCalledWith(exerciseId, { runExerciseWrite: expect.any(Function) });
    expect(withDoctorWorkspacePrincipalMock).toHaveBeenCalledWith(
      workspaceContext(),
