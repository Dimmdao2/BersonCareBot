          ...
          <div className="flex flex-col gap-4">
            ...
            <DoctorField label="Описание" width="full">
              <MarkdownEditor
                ...
                // `values` сбрасывается эффектом ПОСЛЕ рендера, а `key` пересоздаёт редактор НА ТОМ ЖЕ
                // рендере — редактор монтировался с описанием ПРЕДЫДУЩЕЙ записи (для первого выбора из
                // списка это пустая форма создания) и, будучи неуправляемым, обновление эффекта уже не
                // подхватывал. Остальные поля управляемые, поэтому теряется ровно описание: доктор выбирал
                // рекомендацию в списке, видел пустой редактор и сохранением затирал текст. По прямой ссылке
                // на карточку дефект не воспроизводится — там форма монтируется сразу с записью.
                defaultValue={recommendation?.bodyMd ?? ''}
                // Подпись поля теперь несёт обёртка `DoctorField label="Описание"` (пришло из ветки
                // единого элемента поля) — собственная подпись редактора продублировала бы её.
                label={null}
                helpText={null}
