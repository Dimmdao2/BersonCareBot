...
export async function saveRecommendationCore(
  formData: FormData,
): Promise<
  ...
  try {
    if (id) {
      ...
      if (cur.isArchived) {
        return {
          ok: false,
          error: 'Рекомендация в архиве. Верните из архива, чтобы редактировать.',
        };
      }
      await deps.recommendations.updateRecommendation(
        id,
        {
          title,
          bodyMd,
...
export async function archiveRecommendationCore(
  ...
  try {
    ...
    logger.warn(
      { event: 'doctor_recommendation_archive_unexpected_error', recommendationId: id, err: e },
      'archive failed',
    );
