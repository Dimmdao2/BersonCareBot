...
export function NotificationsAnalyticsClient({ isActive = true }: { isActive?: boolean }) {
  ...
  const remindersFailRate =
    remindersSentTotal > 0 ? (remindersFailedTotal / remindersSentTotal) * 100 : 0;
            ...
            <DoctorStatCard
              ...
              hint={
                remindersSentTotal > 0
                  ? `Доля: ${formatPct(remindersFailRate / 100)}`
                  : 'Нет отправок'
              }
              onClick={() =>
                setMetricDialog({ metric: 'notif_reminders_failed', title: 'Ошибки отправки' })
              }
            />
            <DoctorStatCard
              id="notif-open-rate"
