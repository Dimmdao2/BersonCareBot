...
### TODO#1: выделенный список комментариев к упражнениям
`comments/page.tsx` — пока информативный стаб. Данные уже считаются в
`loadTodayExerciseCommentAttention` (`loadDoctorTodayDashboard.ts`). Нужно извлечь загрузчик в
shared app-layer (`loadDoctorExerciseCommentAttention`) и переиспользовать на странице со
списком клиентов на сопровождении. До этого комментарии доступны на «Сегодня» (диалог) и в
карточке пациента.
