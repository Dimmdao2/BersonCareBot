...
### TODO#2: кросс-вкладочные бейджи непрочитанных 🚧 в работе
...
План (блоки → коммиты):
- [x] **Block 1** — `loadDoctorCommunicationsBadges.ts`: `chats` = `unreadFromUsers()`,
  `intake` = `listForDoctor({ status: "new" }).total`; устойчивый (ошибка→0, нули опускаются) + unit-тесты.
- [ ] **Block 2** — подключить загрузчик на 4 страницах вкладок (messages / online-intake /
  comments / broadcasts) и передать `badges` в `DoctorCommunicationsTabsNav`.
- [ ] **Block 3** — отметить done, запись в журнал, проверки (typecheck/lint/тесты + живой dev).
...
- **2026-06-11 · TODO#2 Block 1** — добавлен общий загрузчик бейджей
  ...
  unit-тесты. Подключение к страницам — Block 2.
