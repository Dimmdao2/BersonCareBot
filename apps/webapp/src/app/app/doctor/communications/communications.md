...
SSR-предзагрузка непрочитанных комментариев (`loadDoctorExerciseCommentsForTab`) + пациентов
с комментариями (`loadDoctorCommentPatients`) → рендерит клиентский
...
### Чаты (`messages/DoctorSupportInbox.tsx`)

Split-layout `lg:grid-cols-[1fr_1.2fr]` (чат шире списка). Поллинг ~1/сек только при активном табе
...
- **(A) Лента всех комментариев** (пациент не выбран, дефолт): плоский список из
  `TodayExerciseCommentAttentionItem[]`; кнопка «Загрузить ещё» (cursor-пагинация); клик → state B.
- **(B) Упражнения пациента** (пациент выбран): загрузчик
  `GET /api/doctor/comments/patients/{patientUserId}/exercises`; группировка по этапам
  (активные развёрнуты, закрытые свёрнуты); строка = превью миниатюры + название + бейдж
  (красный unread / серый total); шапка: ссылка на имя пациента + «★» + счётчики + «×» (B→A).
- **(C) Чат по упражнению** (упражнение выбрано): тред через
  `GET /api/doctor/treatment-program-instances/{id}/items/{itemId}/discussion`; хлебная крошка
  пациент→упражнение; «Закрыть» (C→B); ответ через
  `POST .../program-note-reply`; mark-read при первом рендере; микро-график `ExerciseMicroChart`
  в шапке (reps/вес/тяжесть, последние 7 дней, `GET /api/doctor/comments/exercise-metrics`).

**Навигация:** пациент→B; упражнение→C; «Закрыть» C→B; «×» в шапке B→A.

**Левый пейн:** список пациентов (`CommentPatientRow[]`); поиск по displayName/phone/telegramId/maxId;
тоггл «★ На сопровождении»; бейдж `unreadCount`.
...
- `app/api/doctor/comments/patients/[patientUserId]/exercises/route.ts`
...
- Каналы: 5 чекбоксов Telegram · MAX · Push · SMS · Email; под каждым — реальный счётчик из БД;
  дефолт — Telegram + MAX + Push.

**Журнал рассылок (правый пейн):** single-open аккордеон (`useState openId`); свёрнутая строка —
сводка Аудитория · Каналы · Категория; раскрытая — полный текст + «Открыть ошибки →».

**5-канальная модель:** `telegram`, `max`, `push`, `sms`, `email`.
Legacy `bot_message` → нормализуется в `telegram+max`. Email-фанаут guarded (требует SMTP-конфига).
Подробнее — `modules/doctor-broadcasts/README.md`.
...
- **4 отдельных компонента-таба** (свои файлы, правятся изолированно):
  ...
  - **Комментарии** — drill-down: список пациентов → упражнения по этапам → чат упражнения (3 состояния).
...
- **2026-06-14 · cleanup** — удалён мёртвый `comments/DoctorExerciseCommentsList.tsx`: на него не
  осталось импортов (рендер списка инлайн в `DoctorCommentsTab`, `ProgramItemDiscussionMessageBody`
  в табе не используется). typecheck + eslint по `comments` — зелёные.
...
- **2026-06-11 · TODO#3 Block 2** — таб «Комментарии» на doctor-wide запрос + ленивая история/поиск:
  ...
  `DoctorCommentsTab` (клиент, SSR-пропы, «Загрузить ещё»; на тот момент reuse
  `DoctorExerciseCommentsList`, позже переведён на инлайн-рендер, компонент удалён 2026-06-14),
