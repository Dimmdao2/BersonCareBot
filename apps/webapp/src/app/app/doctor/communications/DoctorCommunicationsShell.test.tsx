        ...
        default: function IntakeStub({ deepLinkParams, onDeepLinkChange }: CommunicationsTabProps) {
          return (
            <div data-testid="tab-intake">
              ...
              <button data-testid="intake-set-id" onClick={() => onDeepLinkChange('id', 'req-abc')}>
                set-id
              </button>
            </div>
          );
...
function renderShell(props: Partial<React.ComponentProps<typeof DoctorCommunicationsShell>> = {}) {
  return render(
...
describe('DoctorCommunicationsShell', () => {
  ...
  it('читает ?id из URL и передаёт в intake-таб', async () => {
    window.history.pushState(null, '', '/app/doctor/communications?tab=intake&id=req-123');
    renderShell({ initialTab: 'intake' });
  ...
  it('читает ?archive из URL и передаёт в broadcasts-таб', async () => {
    window.history.pushState(null, '', '/app/doctor/communications?tab=broadcasts&archive=1');
    renderShell({ initialTab: 'broadcasts' });
