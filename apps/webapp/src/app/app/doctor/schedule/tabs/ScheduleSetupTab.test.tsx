...
vi.mock('@/app/app/doctor/admin/booking/BookingRulesPageClient', () => ({
  BookingRulesPageClient: ({
    allowPastUnlinkPastPackageSessions,
  }: {
    allowPastUnlinkPastPackageSessions?: boolean;
  }) => (
    <div
      data-testid="section-rules"
      data-allow-unlink={String(allowPastUnlinkPastPackageSessions)}
    />
  ),
}));
        ...
        json: async () => {
          if (String(url).includes('/api/doctor/settings')) {
            return {
              ...
              settings: [
                {
                  key: 'booking_calendar_default_window',
                  valueJson: { value: { startMinute: 540, endMinute: 1140 } },
                },
