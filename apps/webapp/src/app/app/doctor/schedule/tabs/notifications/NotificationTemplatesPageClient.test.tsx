...
const templates: ManagedNotifTemplateEntry[] = NOTIF_TEMPLATE_EVENTS.flatMap((event) =>
  NOTIF_TEMPLATE_AUDIENCES.map((audience) => ({
    ...
    metadata: { revision: 0, effectiveSource: "hardcoded" as const, updatedAt: null, updatedBy: null, writeToken: null },
  })),
);
...
describe("NotificationTemplatesPageClient", () => {
  ...
  it("groups templates by audience and exposes per-channel safe variable buttons", () => {
    render(
      <NotificationTemplatesPageClient
        ...
        presentation={{
          ...
          metadata: { revision: 0, effectiveSource: "hardcoded", updatedAt: null, updatedBy: null, writeToken: null },
        }}
      />,
    );
    ...
    expect(within(doctorSection).queryByRole("button", { name: "телефон" })).not.toBeInTheDocument();
  ...
  it("uses save responses to refresh template and presentation revision metadata", async () => {
    ...
    apiJsonMock
      .mockResolvedValueOnce({ ok: true, template: savedTemplate })
    ...
    await waitFor(() => expect(within(patientSection).getByText("organization, ревизия 1")).toBeInTheDocument());
    expect(apiJsonMock.mock.calls[0]?.[1]).toEqual(expect.objectContaining({
      body: expect.stringContaining('"expectedUpdatedAt":null'),
    }));

    fireEvent.click(screen.getByRole("button", { name: "Сохранить оформление" }));
    await waitFor(() => expect(screen.getByText("Источник: organization, ревизия 1")).toBeInTheDocument());
    expect(apiJsonMock.mock.calls[1]?.[1]).toEqual(expect.objectContaining({
