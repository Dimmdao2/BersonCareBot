...
- **Панель часов** (справа, E4): «Начало», «Конец» и перерывы на одной сетке — подпись над полем времени,
  Локация → футер равной ширины «Очистить шаблон/расписание» слева и «Сохранить» справа →
  `PUT /api/doctor/booking-engine/working-days { breaks: [...]  }`. Выбор дня недели сохраняется как
  постоянное расписание по этому дню недели (заголовок «Постоянное расписание по <дням недели>»).
