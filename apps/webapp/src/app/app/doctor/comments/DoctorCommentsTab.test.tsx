...
const FEED_A = makeFeedItem({
  stageItemId: ITEM1,
  patientDisplayName: 'Иванов Иван',
  patientUserId: P1,
});
const FEED_B = makeFeedItem({
  stageItemId: ITEM2,
...
describe('DoctorCommentsTab — состояние A (клиент не выбран)', () => {
  it('использует единый desktop split 45/55', async () => {
    ...
    await waitFor(() => {
      expect(
        document.querySelector('#doctor-communications-comments')?.firstElementChild,
      ).toHaveClass('lg:grid-cols-[minmax(0,9fr)_minmax(0,11fr)]');
      expect(screen.getAllByRole('button', { name: /Иванов Иван/i }).length).toBeGreaterThanOrEqual(
        1,
      );
    });
  });
  ...
  it('показывает пациентов в левой панели (all-mode)', async () => {
    ...
    expect(screen.getAllByRole('button', { name: /Петрова Мария/i }).length).toBeGreaterThanOrEqual(
      1,
    );
  });
  ...
  it('показывает empty-state в левой панели если нет пациентов (all-mode)', async () => {
    vi.stubGlobal('fetch', stubFetchAllMode([]));
    render(<DoctorCommentsTab {...defaultProps({ initialPatients: [] })} />);
    await waitFor(() => {
...
function stubFetchMulti(exerciseResult = EXERCISES_RESULT) {
  return vi.fn().mockImplementation((url: string) => {
    ...
    if (s.includes('/api/doctor/comments/patients/') && s.includes('/exercises')) {
      return Promise.resolve({
        ok: true,
        json: async () => ({ ok: true, data: exerciseResult }),
      } as unknown as Response);
    }
    if (s.includes('/api/doctor/comments/patients')) {
      return Promise.resolve({
        ok: true,
        json: async () => ({ ok: true, patients: [PAT_A, PAT_B] }),
      } as unknown as Response);
    }
    if (s.includes('/api/doctor/exercise-comments')) {
      return Promise.resolve({
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockImplementation((url: string) => {
        ...
        if (s.includes('exercise-metrics')) {
          return Promise.resolve({
            ok: true,
            json: async () => ({ ok: true, points: [] }),
          } as unknown as Response);
        }
        if (s.includes('program-day-activity')) {
          return Promise.resolve({
            ok: true,
            json: async () => ({ ok: true, days: [] }),
          } as unknown as Response);
        }
  ...
  function stubMixedFetch() {
    return vi.fn().mockImplementation((url: string) => {
      ...
      if (s.includes('/api/doctor/comments/patients/') && s.includes('/exercises')) {
        return Promise.resolve({
          ok: true,
          json: async () => ({ ok: true, data: EXERCISES_MIXED }),
        } as unknown as Response);
      }
      if (s.includes('/api/doctor/comments/patients')) {
        return Promise.resolve({
