...
function makeDeps(onSupportIds: string[], rows: DoctorExerciseCommentRow[]) {
  return {
    doctorClientsPort: {
...
describe('loadDoctorExerciseCommentsForTab', () => {
  ...
  it('paginates: hasMore=true and nextCursor set when port returns limit+1 rows', async () => {
    ...
    const result = await loadDoctorExerciseCommentsForTab(
      deps,
      { viewerUserId: VIEWER },
      { limit: 2 },
    );
  ...
  it('passes analytics excludedUserIds as audience to listClients', async () => {
    ...
    await loadDoctorExerciseCommentsForTab(deps, {
      viewerUserId: VIEWER,
