...
describe('TestSetForm', () => {
  it('resets title when test set id changes', () => {
    const saveAction = vi.fn(
      async (_prev: SaveTestSetState | null): Promise<SaveTestSetState> => ({ ok: true }),
    );
    const archiveAction = vi.fn(
      async (_prev: ArchiveTestSetState | null, _fd: FormData): Promise<ArchiveTestSetState> => ({
        ok: true,
      }),
    );
  ...
  it('disables publish until a draft test set exists (no id)', () => {
    ...
    render(
      <TestSetForm
        ...
        clinicalTestsLibrary={[libraryRow]}
      />,
    );
  ...
  it('archives with usage acknowledgement when template refs exist', async () => {
    ...
    expect(
      await screen.findByRole('heading', { name: /набор уже используется/i }),
    ).toBeInTheDocument();
