...
vi.mock('@dnd-kit/sortable', () => ({
  SortableContext: ({ children }: { children: ReactNode }) => (
    <div data-testid="sortable-wrap">{children}</div>
  ),
...
describe('MeasureKindsTableClient', () => {
  ...
  it('does not PATCH when a label is empty and shows a dialog', async () => {
    ...
    expect(
      await screen.findByText('Подпись не может быть пустой (проверьте все строки).'),
    ).toBeInTheDocument();
  });
  ...
  it('shows JSON error body on 422 from PATCH', async () => {
    ...
    fetchMock.mockResolvedValue(
      new Response(
        JSON.stringify({
          ok: false,
          error: 'Список устарел: обновите страницу и попробуйте снова',
        }),
        {
          status: 422,
        },
    ...
    render(
      <MeasureKindsTableClient
        initialItems={[
          { id: '00000000-0000-4000-8000-000000000001', code: 'x', label: 'Alpha', sortOrder: 0 },
        ]}
      />,
    );
    ...
    expect(
      await screen.findByText('Список устарел: обновите страницу и попробуйте снова'),
    ).toBeInTheDocument();
  });
  ...
  it('shows non-JSON error hint when server returns plain text on error', async () => {
    ...
    render(
      <MeasureKindsTableClient
        initialItems={[
          { id: '00000000-0000-4000-8000-000000000001', code: 'x', label: 'One', sortOrder: 0 },
        ]}
      />,
    );
  ...
  it('dispatches catalog event and refreshes router after successful PATCH', async () => {
    ...
    fetchMock.mockResolvedValue(
      new Response(
        JSON.stringify({
          ok: true,
          items: [
            { id: '00000000-0000-4000-8000-000000000001', code: 'x', label: 'Saved', sortOrder: 0 },
          ],
        }),
        { status: 200 },
      ),
    ...
    render(
      <MeasureKindsTableClient
        initialItems={[
          { id: '00000000-0000-4000-8000-000000000001', code: 'x', label: 'Old', sortOrder: 0 },
        ]}
      />,
    );
