...
// treatment-program reorder. Oracle: the removed `treatmentProgramReorderHelpers.test.ts`
// (commit a380533b4dca81f6502f2688881694715e1ae7bd) — all four exported functions still have the
// same signature and behavior in the current source. This is pure business logic through the
// module's public API (no DOM/DnD library involved), so a unit test is the cheapest layer that
// sees a reorder-math regression; the DnD-driven UI wrapper is a thinner, more expensive duplicate
// of the same assertions and is not needed for this behavior.
