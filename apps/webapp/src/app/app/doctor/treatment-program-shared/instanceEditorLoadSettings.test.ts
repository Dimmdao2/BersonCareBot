...
describe('instanceEditorLoadSettings', () => {
  ...
  it('validateInstanceEditorLoadSettingsPatch accepts null fields', () => {
    expect(
      validateInstanceEditorLoadSettingsPatch({ reps: null, sets: null, maxPain: null }),
    ).toBeNull();
  });
  ...
  it('isStaleInstanceEditorSaveError distinguishes catalog and validation errors', () => {
    ...
    expect(
      isStaleInstanceEditorSaveError('Объект для типа «exercise» не найден или недоступен'),
    ).toBe(false);
  ...
  it('formatInstanceEditorSaveError appends refresh hint', () => {
    expect(formatInstanceEditorSaveError('Элемент не найден', true)).toContain(
      'Программа обновлена',
    );
    expect(formatInstanceEditorSaveError('Некорректный порядок', true)).toBe(
