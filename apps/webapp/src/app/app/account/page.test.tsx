...
vi.mock('@/app-layer/principal/staffSecuritySelfPrincipal', () => ({
  runWithStaffSecuritySelfPrincipal: vi.fn(
    (_userId: string, _source: string, callback: () => unknown) => callback(),
  ),
}));
vi.mock('@/app-layer/doctor/accountTimezone', () => ({
  getDoctorAccountTimezone: getTimezoneMock,
}));
...
vi.mock('@/app/app/settings/DoctorAccountEmailSection', () => ({
  DoctorAccountEmailSection: accountEmailMock,
}));
vi.mock('@/app/app/settings/DoctorTimezoneSection', () => ({
  DoctorTimezoneSection: timezoneMock,
}));
...
vi.mock('@/shared/ui/doctor/pwa/StaffPwaInstallSection', () => ({
  StaffPwaInstallSection: installMock,
}));
...
describe('shared staff account', () => {
  beforeEach(() => {
    ...
    getProfileEmailFieldsMock.mockResolvedValue({
  ...
  it('keeps personal profile available without exposing clinical defaults to a management-only account', async () => {
    loadContextMock.mockResolvedValue({
      ...clinicalContext,
      workspaceContext: {
        ...clinicalContext.workspaceContext,
        specialistId: null,
  ...
  it('reuses the staff PWA installer on the install tab', async () => {
    ...
    expect(screen.getByRole('link', { name: 'Установить приложение' })).toHaveAttribute(
      'aria-current',
      'page',
    );
