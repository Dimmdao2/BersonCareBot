...
const shellMock = vi.hoisted(() => vi.fn(({ children }: { children: ReactNode }) => <main>{children}</main>));

vi.mock("@/app-layer/guards/requireRole", () => ({ requireStaffPersonalInstallPage: guardMock }));
vi.mock("@/shared/ui/doctor/shell/DoctorWorkspaceShell", () => ({ DoctorWorkspaceShell: shellMock }));
...
describe("global-admin personal install page", () => {
  beforeEach(() => {
    ...
    guardMock.mockResolvedValue({
      user: { userId: "aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee", role: "admin", displayName: "Owner", bindings: {} },
      adminMode: true,
    });
  });
