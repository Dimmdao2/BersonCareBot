...
describe('legacy settings compatibility', () => {
  ...
  it('routes the explicit old personal and install entries to the one account area', async () => {
    await expect(
      SettingsPage({ searchParams: Promise.resolve({ tab: 'specialist' }) }),
    ).rejects.toThrow('redirect:/app/account');
    await expect(
      SettingsPage({ searchParams: Promise.resolve({ tab: 'install' }) }),
  ...
  it('keeps the canonical Settings root out of a plain specialist account', async () => {
    ...
    await expect(SettingsPage({ searchParams: Promise.resolve({}) })).rejects.toThrow(
  ...
  it('builds the section nav from only the sections this viewer may reach (Defect #1)', async () => {
    ...
    expect(settingsTabsNavMock).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        visibleTabs: ['organization', 'billing'],
      }),
      undefined,
    );
    ...
    expect(settingsTabsNavMock).toHaveBeenCalledWith(
      expect.objectContaining({
        activeTab: 'organization',
        visibleTabs: ['organization', 'team', 'billing'],
      }),
      undefined,
    );
  ...
  it('shows billing to owner and clinic admin, then denies it to a specialist', async () => {
    getOrgEntitlementsSnapshotMock.mockResolvedValue({
      tariff: {
        id: 'tariff-1',
        name: 'ПОЛНЫЙ ДОСТУП - РАЗРАБОТЧИК',
        mechanics: { payments: true, courses: false },
        quotas: {},
        includedSeats: null,
      },
      overrides: [],
    ...
    render(await SettingsPage({ searchParams: Promise.resolve({ tab: 'billing' }) }));
    expect(getSaasBillingOverviewMock).toHaveBeenLastCalledWith('org-1');
    ...
    await expect(
      SettingsPage({ searchParams: Promise.resolve({ tab: 'billing' }) }),
    ).rejects.toThrow('redirect:/app/account');
  });
  ...
  it('keeps historical platform admin-tab redirects', async () => {
    ...
    await expect(
      SettingsPage({ searchParams: Promise.resolve({ adminTab: 'system-health' }) }),
    ).rejects.toThrow('redirect:/app/admin/system-health');
    await expect(
      SettingsPage({ searchParams: Promise.resolve({ adminTab: 'health-archive' }) }),
    ).rejects.toThrow('redirect:/app/admin/health-archive');
    await expect(
      SettingsPage({ searchParams: Promise.resolve({ adminTab: 'audit-log' }) }),
    ).rejects.toThrow('redirect:/app/admin/audit-log');
    await expect(
      SettingsPage({ searchParams: Promise.resolve({ adminTab: 'product-analytics' }) }),
    ).rejects.toThrow(/redirect:\/app\/doctor\//);
    expect(requireWorkspaceMock).not.toHaveBeenCalled();
  });
