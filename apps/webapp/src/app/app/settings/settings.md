...
- **Врач / админ** видят страницу настроек кабинета; у **администратора** доступен переключатель admin mode и дополнительные вкладки (режим, параметры приложения, авторизация, доступ и роли, интеграции, каталог записи, **лог операций** — чтение `admin_audit_log` через `GET /api/admin/audit-log`).
