...
describe("describeCommercialAccessState", () => {
  it("names compatibility mode without a raw enum", () => {
    expect(describeCommercialAccessState({ lifecycle: "active", tariffId: null, source: "compatibility" })).toBe(
      "Совместимость: коммерческий тариф ещё не подключён администратором платформы, доступ работает в режиме до введения тарифов.",
    );
  });

  it("names no_trial", () => {
    expect(describeCommercialAccessState({ lifecycle: "active", tariffId: null, source: "no_trial" })).toBe(
  ...
  it("falls back to a dateless trial-active sentence when no end date is present", () => {
    expect(describeCommercialAccessState({ lifecycle: "active", tariffId: "t1", source: "trial" })).toBe(
  ...
  it("reports blocked access", () => {
    expect(describeCommercialAccessState({ lifecycle: "blocked", tariffId: "t1", source: "trial" })).toBe(
      "Доступ заблокирован — обратитесь к администратору платформы.",
    );
  });

  it("reports read_only access", () => {
    expect(describeCommercialAccessState({ lifecycle: "read_only", tariffId: "t1", source: "trial" })).toBe(
      "Доступ только для чтения — обратитесь к администратору платформы.",
    );
  });

  it("reports an active assigned tariff plainly", () => {
    expect(describeCommercialAccessState({ lifecycle: "active", tariffId: "t1", source: "assignment" })).toBe(
      "Тариф активен.",
    );
  });
  ...
  it("reports an active post-trial tariff plainly", () => {
    expect(
      describeCommercialAccessState({ lifecycle: "active", tariffId: "t1", source: "post_trial_tariff" }),
    ).toBe("Тариф активен.");
  });
});
