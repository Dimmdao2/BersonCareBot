  ...
  beforeEach(() => {
    ...
    fetchMock
      ...
      .mockResolvedValueOnce({
        ok: true,
        text: async () => JSON.stringify({ ok: true, specialists: [], services: [] }),
      })
      .mockResolvedValueOnce({
        ok: true,
        text: async () => JSON.stringify({ ok: true, products: [] }),
      })
  ...
  it('saves cancellation policy without hardcoded overwrite', async () => {
    ...
    await waitFor(() => {
      const saveCall = fetchMock.mock.calls.find(
        (c) =>
          c[0] === '/api/admin/booking-engine/policies' && (c[1] as RequestInit)?.method === 'POST',
      );
