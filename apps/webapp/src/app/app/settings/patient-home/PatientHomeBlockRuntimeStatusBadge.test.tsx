...
function status(
  p: Partial<PatientHomeBlockRuntimeStatus> & Pick<PatientHomeBlockRuntimeStatus, 'kind'>,
...
describe('PatientHomeBlockRuntimeStatusBadge', () => {
  it('renders Скрыт for hidden', () => {
    render(
      <PatientHomeBlockRuntimeStatusBadge
        status={status({ kind: 'hidden', blockCode: 'booking' })}
      />,
    );
  ...
  it('renders Пусто for empty', () => {
    render(
      <PatientHomeBlockRuntimeStatusBadge
        status={status({ kind: 'empty', blockCode: 'daily_warmup' })}
      />,
    );
