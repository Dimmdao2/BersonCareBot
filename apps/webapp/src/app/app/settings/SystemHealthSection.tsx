...
function statusBadgeVariant(status: string): "secondary" | "outline" | "destructive" {
  ...
  if (status === "degraded" || status === "no_signal" || status === "no_source") return "outline";
  return "destructive";
}
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border/60 p-3">
            ...
            <Badge variant={statusBadgeVariant(data?.webappDb ?? "down")}>{data?.webappDb ?? "down"}</Badge>
          </div>
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border/60 p-3">
            <span>bersoncarebot-api-prod /health</span>
            <div className="flex items-center gap-2">
              <Badge variant={statusBadgeVariant(data?.integratorApi.status ?? "error")}>
                {data?.integratorApi.status ?? "error"}
              </Badge>
              {data?.integratorApi.db ? (
                <Badge variant={statusBadgeVariant(data.integratorApi.db)}>db: {data.integratorApi.db}</Badge>
              ) : null}
            </div>
          </div>
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border/60 p-3">
            <span>projection_outbox</span>
            <Badge variant={statusBadgeVariant(data?.projection.status ?? "error")}>
              {data?.projection.status ?? "error"}
            </Badge>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border/60 p-3">
            ...
            <Badge variant={statusBadgeVariant(workers.api)}>{workerLabel(workers.api)}</Badge>
          </div>
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border/60 p-3">
            <span>bersoncarebot-worker-prod</span>
            <Badge variant={statusBadgeVariant(workers.worker)}>{workerLabel(workers.worker)}</Badge>
          </div>
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border/60 p-3">
            <span>bersoncarebot-webapp-prod</span>
            <Badge variant={statusBadgeVariant(workers.webapp)}>{workerLabel(workers.webapp)}</Badge>
          </div>
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border border-border/60 p-3">
            <span>media cron workers</span>
            <Badge variant="outline">нет источника</Badge>
          </div>
        </CardContent>
      </Card>
