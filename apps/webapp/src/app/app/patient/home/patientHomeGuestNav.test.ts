...
import {
  appLoginWithNextHref,
  hrefForPatientHomeDrilldown,
  stripApiMediaForAnonymousGuest,
} from './patientHomeGuestNav';
...
describe('patientHomeGuestNav', () => {
  ...
  it('hrefForPatientHomeDrilldown passes through when not anonymous', () => {
    expect(hrefForPatientHomeDrilldown('/app/patient/sections/x', false)).toBe(
      '/app/patient/sections/x',
    );
  });
  ...
  it('stripApiMediaForAnonymousGuest clears /api/media/ only for anonymous', () => {
    ...
    expect(stripApiMediaForAnonymousGuest('https://cdn.example/x.jpg', true)).toBe(
