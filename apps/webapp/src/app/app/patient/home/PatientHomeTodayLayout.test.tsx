...
function block(
  code: PatientHomeTodayLayoutBlock['code'],
  label: string,
...
describe('PatientHomeTodayLayout', () => {
  it('renders dashboard grid with stable desktop placement metadata on blocks', () => {
    ...
    const subscription = container.querySelector(
      '[data-patient-home-block="subscription_carousel"]',
    );
  ...
  it('places sos_booking_split full width below next reminder row (order 43)', () => {
    const { container } = render(
      <PatientHomeTodayLayout
        personalizedName={null}
        blocks={[block('sos_booking_split', 'Split')]}
      />,
    );
  ...
  it('does not render full-width carousel wrapper when carousel block is absent', () => {
    const { container } = render(
      <PatientHomeTodayLayout
        personalizedName={null}
        blocks={[block('daily_warmup', 'Warmup'), block('booking', 'Booking')]}
      />,
    );

    expect(
      container.querySelector('[data-patient-home-block="subscription_carousel"]'),
  ...
  it('stretches useful post full-width when warmup block is absent', () => {
    const { container } = render(
      <PatientHomeTodayLayout
        personalizedName={null}
        blocks={[block('useful_post', 'UsefulPost'), block('booking', 'Booking')]}
      />,
    );
  ...
  it('keeps mobile block order in the same order as resolved settings', () => {
    ...
    const layoutGrid = screen.getByTestId('patient-home-layout-grid');
    expect(
      [...layoutGrid.children].map((child) => child.getAttribute('data-patient-home-block')),
    ).toEqual(['daily_warmup', 'situations', 'booking']);
  });
});
