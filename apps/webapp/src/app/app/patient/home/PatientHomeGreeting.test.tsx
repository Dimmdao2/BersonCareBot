...
describe('buildPatientGreetingTitle', () => {
  it('formats time-of-day with name', () => {
    expect(buildPatientGreetingTitle('Demo Client', 'Доброй ночи')).toBe(
      'Доброй ночи, Demo Client!',
    );
  });
});
...
describe('PatientHomeGreetingMobileHeader', () => {
  it('uses greeting styles not shell header title', () => {
    render(
      <PatientHomeGreetingMobileHeader
        personalizedName="Demo Client"
        timeOfDayPrefix="Доброй ночи"
      />,
    );
...
describe('PatientHomeGreeting', () => {
  ...
  it('shows unread chat hint with link under greeting', () => {
    render(
      <PatientHomeGreeting
        personalizedName="Анна"
        timeOfDayPrefix="Доброе утро"
        unreadChatCount={2}
      />,
    );

    expect(screen.getByRole('link', { name: 'в чате' })).toHaveAttribute(
      'href',
