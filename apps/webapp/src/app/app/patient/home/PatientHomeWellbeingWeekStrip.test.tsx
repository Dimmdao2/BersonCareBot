...
describe('PatientHomeWellbeingWeekStrip', () => {
  ...
  it('draws a horizontal dashed path to the first score, not a vertical tick', () => {
    ...
    const { container } = render(
      <PatientHomeWellbeingWeekStrip
        marks={[
          {
            recordedAt: DateTime.fromISO('2026-05-18T10:00:00', { zone: 'Europe/Moscow' })
              .toUTC()
              .toISO()!,
            score: 5,
          },
  ...
  it('renders day labels for the rolling window only', () => {
    const wednesday = DateTime.fromObject(
      { year: 2026, month: 5, day: 20, hour: 12 },
      { zone: 'Europe/Moscow' },
    );
    ...
    expect(container.querySelectorAll('[role="listitem"]')).toHaveLength(
