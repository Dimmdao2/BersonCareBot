...
describe('bucketWellbeingStripMarksForChart', () => {
  it('merges marks within the same 2h slot into one averaged point', () => {
    ...
    const bucketed = bucketWellbeingStripMarksForChart(
      [
        {
          recordedAt: DateTime.fromISO('2026-05-20T10:05:00', { zone: tz }).toUTC().toISO()!,
          score: 2,
        },
        {
          recordedAt: DateTime.fromISO('2026-05-20T10:20:00', { zone: tz }).toUTC().toISO()!,
          score: 4,
        },
        {
          recordedAt: DateTime.fromISO('2026-05-20T10:50:00', { zone: tz }).toUTC().toISO()!,
          score: 4,
        },
      ],
      windowStartMs,
  ...
  it('keeps separate points for marks in different 2h slots', () => {
    ...
    const bucketed = bucketWellbeingStripMarksForChart(
      [
        {
          recordedAt: DateTime.fromISO('2026-05-20T09:30:00', { zone: tz }).toUTC().toISO()!,
          score: 2,
        },
        {
          recordedAt: DateTime.fromISO('2026-05-20T13:30:00', { zone: tz }).toUTC().toISO()!,
          score: 5,
        },
      ],
      windowStartMs,
...
describe('buildPatientHomeWellbeingWeekStripChart', () => {
  ...
  it('connects marks in time order with dashed lead when anchor day had no marks', () => {
    ...
    const windowStartIso = thursday
      .minus({ days: HOME_WELLBEING_STRIP_DAY_COUNT - 1 })
      .toISODate()!;
    const { segments } = buildPatientHomeWellbeingWeekStripChart({
      marks: [
        {
          recordedAt: DateTime.fromISO('2026-05-19T09:00:00', { zone: tz }).toUTC().toISO()!,
          score: 3,
        },
        {
          recordedAt: DateTime.fromISO('2026-05-20T10:00:00', { zone: tz }).toUTC().toISO()!,
          score: 5,
        },
      ],
      timeZone: tz,
  ...
  it('clips line at nowX and draws dashed tail when today has no marks', () => {
    ...
    const { segments, nowX } = buildPatientHomeWellbeingWeekStripChart({
      marks: [
        {
          recordedAt: DateTime.fromISO('2026-05-21T10:00:00', { zone: tz }).toUTC().toISO()!,
          score: 4,
        },
      ],
      timeZone: tz,
  ...
  it('extends solid tail to nowX when there is a mark today', () => {
    ...
    const { segments, nowX } = buildPatientHomeWellbeingWeekStripChart({
      marks: [
        {
          recordedAt: DateTime.fromISO('2026-05-22T08:00:00', { zone: tz }).toUTC().toISO()!,
          score: 3,
        },
      ],
      timeZone: tz,
  ...
  it('uses solid bridge from anchor day before window when it had marks', () => {
    ...
    const { segments } = buildPatientHomeWellbeingWeekStripChart({
      marks: [
        {
          recordedAt: DateTime.fromISO('2026-05-18T11:00:00', { zone: tz }).toUTC().toISO()!,
          score: 5,
        },
      ],
      timeZone: tz,
  ...
  it('keeps the first in-window connection after a solid anchor bridge', () => {
    const wednesday = DateTime.fromObject(
      { year: 2026, month: 5, day: 20, hour: 15 },
      { zone: tz },
    );
    vi.setSystemTime(wednesday.toMillis());
    const windowStartIso = wednesday
      .minus({ days: HOME_WELLBEING_STRIP_DAY_COUNT - 1 })
      .toISODate()!;
    const { segments } = buildPatientHomeWellbeingWeekStripChart({
      marks: [
        {
          recordedAt: DateTime.fromISO('2026-05-19T10:00:00', { zone: tz }).toUTC().toISO()!,
          score: 3,
        },
        {
          recordedAt: DateTime.fromISO('2026-05-20T14:00:00', { zone: tz }).toUTC().toISO()!,
          score: 5,
        },
      ],
      timeZone: tz,
  ...
  it('uses fewer solid segments when many marks fall in the same 2h bucket', () => {
    ...
    const { segments } = buildPatientHomeWellbeingWeekStripChart({
      marks: [
        {
          recordedAt: DateTime.fromISO('2026-05-22T08:10:00', { zone: tz }).toUTC().toISO()!,
          score: 1,
        },
        {
          recordedAt: DateTime.fromISO('2026-05-22T08:25:00', { zone: tz }).toUTC().toISO()!,
          score: 5,
        },
        {
          recordedAt: DateTime.fromISO('2026-05-22T08:40:00', { zone: tz }).toUTC().toISO()!,
          score: 3,
        },
        {
          recordedAt: DateTime.fromISO('2026-05-22T09:05:00', { zone: tz }).toUTC().toISO()!,
          score: 4,
        },
      ],
      timeZone: tz,
