...
const patientPrincipalContexts = vi.hoisted(
  () => [] as Array<{ organizationId: string; platformUserId: string; source: string }>,
);
...
const getPatientDefaultPromoTreatmentProgramTemplateId = vi.hoisted(() =>
...
describe('PatientHomeToday', () => {
  beforeEach(() => {
    ...
    listBlocksWithItems.mockResolvedValue([
      homeBlock('daily_warmup', 10, [
        homeItem('i-w', 'daily_warmup', 'content_page', 'fixture-warmup-page', 0),
      ]),
      homeBlock('useful_post', 15, []),
      homeBlock('booking', 20, []),
      homeBlock('situations', 30, [
        homeItem('i-s', 'situations', 'content_section', 'fixture-section-a', 0),
      ]),
  ...
  it('anonymous guest: no personal API, login drilldown on warmup, shows personal blocks with guest CTAs', async () => {
    ...
    expect(start.getAttribute('href')).toContain(
      encodeURIComponent(routePaths.patientGoDailyWarmup),
    );
    ...
    const loginReminder = screen
      ...
      .find((link) =>
        link.getAttribute('href')?.includes(encodeURIComponent(routePaths.patientReminders)),
      );
    expect(loginReminder?.getAttribute('href')).toContain(
      encodeURIComponent(routePaths.patientReminders),
    );
  });
  ...
  it('patient tier: calls personal loaders and shows progress', async () => {
    ...
    expect(getRecentDaysSparkline).toHaveBeenCalledWith(
    ...
    expect(screen.getByRole('heading', { name: /Сегодня выполнено/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Самочувствие 4 из 5/i })).toHaveAttribute(
      'aria-pressed',
      'true',
    );
  });
  ...
  it('patient tier: warmup hero shows «Разминка выполнена» and cooldown caption when cooldown active and n===1', async () => {
    getDailyWarmupHeroCooldownMeta.mockResolvedValueOnce({
      active: true,
      minutesAgo: 3,
      minutesRemaining: 17,
    });
    ...
    expect(
      screen.getByRole('status', { name: /Разминка дня уже отмечена выполненной/i }),
    ).toBeInTheDocument();
  ...
  it('patient tier: keeps warmup CTA when cooldown active but dailyWarmupCount >= 2', async () => {
    ...
    expect(
      screen.queryByRole('status', { name: /Разминка дня уже отмечена выполненной/i }),
    ).toBeNull();
  ...
  it('patient tier: hides plan block for promo assignment', async () => {
    ...
    expect(
      screen.queryByRole('heading', { name: /Мой план реабилитации/i }),
  ...
  it('patient tier: renders useful post link when block has a content_page item', async () => {
    listBlocksWithItems.mockResolvedValue([
      ...
      homeBlock('useful_post', 15, [
        homeItem('i-up', 'useful_post', 'content_page', 'fixture-useful-post', 0),
      ]),
      homeBlock('booking', 20, []),
      homeBlock('situations', 30, [
        homeItem('i-s', 'situations', 'content_section', 'fixture-section-a', 0),
