...
describe('PatientProgramStageItemPageClient', () => {
  ...
  it('shows instruction label and discussion CTA variants with feature-gated discussion UI', async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith(`/api/patient/treatment-program-instances/${instanceId}`)) {
        return new Response(
          JSON.stringify({ ok: true, item: completed ? completedDetail() : makeDetail() }),
          { status: 200 },
        );
      }
    ...
    await waitFor(() => {
      expect(
        fetchMock.mock.calls.some((call) => String(call[0]).endsWith('/progress/complete')),
      ).toBe(true);
    });
  ...
  it('opens discussion dialog from comments CTA and marks it read', async () => {
    ...
    await waitFor(() => {
      expect(
        fetchMock.mock.calls.some((call) => String(call[0]).includes('/discussion/read')),
      ).toBe(true);
    });
  });
  ...
  it('hides discussion controls when comments interaction is not visible', async () => {
    ...
    expect(screen.queryByRole('button', { name: 'Камера' })).not.toBeInTheDocument();
    expect(
      screen.queryByRole('button', { name: /Оставить комментарий к выполнению/i }),
    ).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /Открыть комментарии/i })).not.toBeInTheDocument();
    expect(fetchMock.mock.calls.some((call) => String(call[0]).includes('/discussion'))).toBe(
      false,
    );
  });
  ...
  it.each(['promo', 'course'] as const)(
    ...
    async (assignmentSource) => {
      ...
      expect(screen.queryByRole('button', { name: 'Камера' })).not.toBeInTheDocument();
      expect(
        screen.queryByRole('button', { name: /Отметить выполнение/i }),
      ).not.toBeInTheDocument();
      ...
      expect(
        screen.queryByRole('button', { name: /Открыть комментарии/i }),
      ).not.toBeInTheDocument();
      expect(fetchMock.mock.calls.some((call) => String(call[0]).includes('/discussion'))).toBe(
        false,
      );
