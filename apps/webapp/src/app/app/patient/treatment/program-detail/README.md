...
| `PatientPlanPersonalProgramCtaCard.tsx` | **Активная** программа: CTA «персональная программа» под панелями вкладок — **hero-геометрия главной** (`patientHomeHeroCardGeometryClass`, `patientHomeHeroTextColumnClass`, `patientHeroPrimaryActionClass`, слот изображения справа), **без** верхней строки бейджей. Статика фото: `public/patient/personal-program-consultation.png`. Кнопка «Отправить заявку» → **`routePaths.intakeLfk`** (`/app/patient/intake/lfk`, онлайн-реабилитация). |
