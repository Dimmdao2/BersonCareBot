...
- Врач: **`GET /api/doctor/treatment-program-instances/{instanceId}/items/{itemId}/discussion`** + read-only **`DoctorProgramItemDiscussionDialog`** из журнала программы (кнопка «Обсуждение»); ответ — отдельная модалка «Ответить».
