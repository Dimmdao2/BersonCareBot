...
describe('FormatStepClient', () => {
  ...
  it('adds the configured Online service entry without replacing rehab/nutrition intake', () => {
    ...
    expect(screen.getByRole('link', { name: 'Реабилитация онлайн' })).toHaveAttribute(
      'href',
      routePaths.intakeLfk,
    );
    expect(screen.getByRole('link', { name: 'Нутрициология онлайн' })).toHaveAttribute(
      'href',
      routePaths.intakeNutrition,
    );
  });
});
