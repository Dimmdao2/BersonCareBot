...
): PersonalReminderRowVM['iconKind'] {
  switch (linked) {
    case 'lfk_complex':
      return 'lfk';
    case 'rehab_program':
...
export async function RemindersPageBody({ session }: { session: AppSession }) {
  ...
  const rehabMatches = rehabProgramForBlock
    ? rules.filter(
        (r) =>
          r.linkedObjectType === 'rehab_program' && r.linkedObjectId === rehabProgramForBlock.id,
      )
    : [];
  rehabMatches.sort((a, b) => (a.updatedAt < b.updatedAt ? 1 : -1));
  const rehabRuleForBlock = rehabMatches[0] ?? null;

  const warmMatches = rules.filter((r) =>
    isWarmupsContentSectionReminderRule(r, warmupsSectionSlug),
  );
  ...
  const muteUntilLabel =
    muteActive && mutedUntil?.trim()
      ? formatBookingDateTimeMediumRu(mutedUntil.trim(), appTz)
      : null;
