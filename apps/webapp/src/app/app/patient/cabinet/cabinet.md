...
- блок полезных ссылок (адрес/подготовка/стоимость) — компонент [`CabinetInfoLinks.tsx`](CabinetInfoLinks.tsx) (RSC): плитки «Как подготовиться» / «Стоимость» только при опубликованных статьях `help` со slug `preparation` / `cost`;
