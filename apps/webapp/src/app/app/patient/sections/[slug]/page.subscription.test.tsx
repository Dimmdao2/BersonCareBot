...
describe('PatientSectionPage / subscription (Phase 7)', () => {
  ...
  it('shows Открыть курс for linked published course', async () => {
    ...
    getCourseForDoctorMock.mockImplementation(async (id: string) =>
      id === courseId
        ? { id: courseId, title: 'Курс', description: null, status: 'published' }
        : null,
    );
    getOptionalPatientSessionMock.mockResolvedValue({
      user: { userId: 'patient-a', role: 'client' },
    });
  ...
  it.each([
    {
      label: 'the courses mechanic is OFF',
      organization: { ok: true, organizationId: 'org-a' },
      entitled: false,
    },
