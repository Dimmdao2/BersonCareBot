...
function compareMsgs(
  a: Pick<DiscussionMsg, 'createdAt' | 'id'>,
  b: Pick<DiscussionMsg, 'createdAt' | 'id'>,
) {
...
describe('patient item discussion route', () => {
    ...
    listMessagesPageMock.mockImplementation(
      async (input: {
        ...
        if (input.direction === 'forward') {
          let start = 0;
          if (input.cursor) {
            while (start < sorted.length && compareMsgs(sorted[start]!, input.cursor) <= 0)
              start += 1;
          }
        ...
        if (input.cursor) {
          ...
          while (
            ...
            compareMsgs(sorted[endExclusive]!, input.cursor) < 0
          ) {
            endExclusive += 1;
          }
        }
  ...
  it('POST proxies comment through observation dual-write path', async () => {
    ...
    listMessagesForStageItemMock.mockResolvedValue([posted]);
