...
describe('GET /api/patient/diary/lfk-stats', () => {
  ...
  it('returns 401 without session', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/patient/diary/lfk-stats?period=week&offset=0'),
    );
  ...
  it('returns 404 for foreign complexId', async () => {
    ...
    const res = await GET(
      new Request(
        'http://localhost/api/patient/diary/lfk-stats?complexId=c-other&period=week&offset=0',
      ),
    );
  ...
  it('returns detail with sessions when complexId owned', async () => {
    ...
    expect(data.detail?.chartPoints.some((p) => p.date === '2025-03-01' && p.value >= 1)).toBe(
      true,
    );
  });
});
