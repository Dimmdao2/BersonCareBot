...
describe('GET /api/patient/diary/symptom-stats', () => {
  ...
  it('returns 401 without session', async () => {
    ...
    const res = await GET(
      new Request(
        'http://localhost/api/patient/diary/symptom-stats?trackingId=t1&period=week&offset=0',
      ),
    );
  ...
  it('returns 401 when logged in as non-patient', async () => {
    ...
    const res = await GET(
      new Request(
        'http://localhost/api/patient/diary/symptom-stats?trackingId=tr-1&period=week&offset=0',
      ),
    );
    expect(res.status).toBe(401);
  });
  ...
  it('returns 404 when tracking is not owned by user', async () => {
    ...
    const res = await GET(
      new Request(
        'http://localhost/api/patient/diary/symptom-stats?trackingId=other-tracking&period=week&offset=0',
      ),
    );
  ...
  it('returns 200 with points for owned tracking', async () => {
    ...
    const res = await GET(
      new Request(
        'http://localhost/api/patient/diary/symptom-stats?trackingId=tr-1&period=month&offset=0',
      ),
    );
    ...
    expect(
      data.points.some((p) => p.date === '2025-03-01' && p.instant === 4 && p.daily === null),
    ).toBe(true);
  });
  ...
  it('returns 400 for invalid period', async () => {
    ...
    const res = await GET(
      new Request(
        'http://localhost/api/patient/diary/symptom-stats?trackingId=tr-1&period=invalid',
      ),
    );
