...
const { gateMock, buildAppDepsMock, bootstrapMock, pollNewMock, sendTextMock, unreadMock } =
  vi.hoisted(() => {
...
describe('GET /api/patient/messages', () => {
  ...
  it('surfaces readOnly from bootstrap and polling for a closed own conversation', async () => {
    ...
    expect((await boot.json()) as { readOnly?: boolean }).toMatchObject({
      ok: true,
      readOnly: true,
    });
    ...
    expect((await polled.json()) as { readOnly?: boolean }).toMatchObject({
      ok: true,
      readOnly: true,
    });
  });
  ...
  it('returns 400 for invalid conversationId uuid', async () => {
    gateMock.mockResolvedValue(okClient('u1'));
    const res = await GET(
      new Request('http://localhost/api/patient/messages?conversationId=not-a-uuid'),
    );
