...
export async function GET(request: Request) {
  ...
  const resolved =
    parsed.data.kind === 'organization_go'
      ...
      : await service.resolveTreatmentProgramOrganizationForPatient(
          session.user.userId,
  ...
  const contextChanged = Boolean(
    rememberedOrganizationId && rememberedOrganizationId !== resolved.organizationId,
  );
  let targetPath: string;
  if (parsed.data.kind === 'organization_go') {
    const goPath =
      parsed.data.goKind === 'daily-warmup'
