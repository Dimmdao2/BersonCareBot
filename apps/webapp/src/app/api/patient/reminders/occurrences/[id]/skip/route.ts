...
/**
 * POST /api/patient/reminders/occurrences/[id]/skip
 *
 * Called from the service worker (sw.js) when the patient taps "Пропустить" on a
 * web-push notification action button. Records the skip with reason "web_push".
 */
export async function POST(
  ...
  const res = await deps.reminders.skipOccurrence(gate.session.user.userId, occurrenceId, "web_push");
  if (!res.ok) {
  ...
  return NextResponse.json({ ok: true, skippedAt: res.data.skippedAt });
}
