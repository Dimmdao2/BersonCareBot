...
export async function POST(req: Request) {
  ...
  if (
    typeof linkedObjectType !== 'string' ||
    !LINKED_TYPES.has(linkedObjectType as ReminderLinkedObjectType)
  ) {
    return NextResponse.json({ ok: false, error: 'validation_error' }, { status: 400 });
  }
  ...
  if (
    typeof linkedObjectIdRaw !== 'string' ||
    linkedObjectIdRaw.trim().length === 0 ||
    linkedObjectIdRaw.length > 200
  ) {
    return NextResponse.json({ ok: false, error: 'validation_error' }, { status: 400 });
  }
