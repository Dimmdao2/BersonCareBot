...
export async function POST(request: Request) {
  ...
  let userId = await deps.userPasswordCredentials.findUserIdByEmailChallengeId(
  ...
  if (intent.organizationSlug === null) {
    ...
    enterStaffSecuritySelfPrincipal(
      userId,
      'api/auth/specialist-signup/confirm:slug-recovery-self',
    );
    try {
  ...
  if (!establishedSession) {
    ...
    enterStaffSecuritySelfPrincipal(
      verifiedUserId,
      'api/auth/specialist-signup/confirm:verified-self',
    );
    try {
  ...
  let provisioned: Awaited<
    ReturnType<typeof deps.organizationProvisioning.provisionSpecialistOwner>
  >;
  try {
