...
vi.mock('@/modules/auth/emailOtpPublic', () => ({
  startPublicEmailOtpRegistration: (...args: unknown[]) =>
    startPublicEmailOtpRegistrationMock(...args),
}));
...
describe('POST /api/auth/email-otp/register', () => {
  beforeEach(() => {
    ...
    startPublicEmailOtpRegistrationMock.mockResolvedValue({
      ok: true,
  ...
  it('rejects missing or blank required FIO before the domain path', async () => {
    const missing = await POST(
      request({ email: 'patient@example.com', firstName: 'Иван' }, '10.2.0.2'),
    );
    expect(missing.status).toBe(400);
    const blank = await POST(
      request({ email: 'patient@example.com', lastName: ' ', firstName: 'Иван' }, '10.2.0.3'),
    );
  ...
  it('rejects registration before account lookup when email auth is disabled', async () => {
    ...
    const response = await POST(
      request({ email: 'patient@example.com', lastName: 'Иванов', firstName: 'Иван' }, '10.2.0.8'),
    );
  ...
  it('accepts optional patronymic and starts a structured patient registration', async () => {
    const response = await POST(
      request({ email: 'patient@example.com', lastName: 'Иванов', firstName: 'Иван' }, '10.2.0.4'),
    );
  ...
  it('does not allow registration to overwrite a verified duplicate', async () => {
    startPublicEmailOtpRegistrationMock.mockResolvedValueOnce({
      ok: false,
      code: 'duplicate_email',
    });
    const response = await POST(
      request({ email: 'patient@example.com', lastName: 'Иванов', firstName: 'Иван' }, '10.2.0.5'),
    );
