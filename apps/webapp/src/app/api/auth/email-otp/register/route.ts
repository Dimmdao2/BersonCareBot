...
export async function POST(request: Request) {
  ...
  if (await isEmailOtpStartRateLimitedByKey(identity.key)) {
    return NextResponse.json(
      { ok: false, error: "rate_limited", retryAfterSeconds: 60, message: formatOtpRetryAfterMessage(60) },
  ...
  if (!parsed.success) {
    return NextResponse.json({ ok: false, error: "invalid_fio", message: "Укажите фамилию и имя" }, { status: 400 });
  }

  const result = await startPublicEmailOtpRegistration(parsed.data, buildAppDeps().emailOtpPublicDb);
  if (result.ok) {
    return NextResponse.json({
      ok: true,
  ...
  if (result.code === "duplicate_email") {
    return NextResponse.json({ ok: false, error: "duplicate_email", message: "Аккаунт с этой почтой уже существует." }, { status: 409 });
  }
  if (result.code === "invalid_fio") {
    return NextResponse.json({ ok: false, error: "invalid_fio", message: "Укажите фамилию и имя" }, { status: 400 });
  }
  if (result.code === "invalid_email") {
    return NextResponse.json({ ok: false, error: "invalid_email", message: "Неверный формат email" }, { status: 400 });
  }
  if (result.code === "rate_limited") {
    const retryAfterSeconds = result.retryAfterSeconds ?? 60;
    return NextResponse.json(
      { ok: false, error: "rate_limited", retryAfterSeconds, message: formatOtpRetryAfterMessage(retryAfterSeconds) },
