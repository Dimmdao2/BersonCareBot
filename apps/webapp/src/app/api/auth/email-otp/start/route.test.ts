...
import { BC_CORRELATION_ID_HEADER, getCurrentCorrelationIdHeader } from '@bersoncare/db-principal';
...
describe('POST /api/auth/email-otp/start', () => {
  ...
  it('keeps the inbound UUID for the downstream email OTP operation', async () => {
    ...
    const request = makeStartRequest({ email: 'correlation@example.com' }, '10.0.0.31');
