...
/**
 * Локальный приёмник пульса «подтверждённая доставка» (design D-d).
 *
 * Это НЕ замена внешнему сервису. Это приёмник, который мы контролируем, чтобы механизм
 * работал целиком уже сейчас и чтобы `OPERATOR_HEARTBEAT_PIPELINE_URL` можно было позже
...
export async function POST(request: Request) {
  ...
  return recordHeartbeatPing('pipeline_delivery');
}

export async function GET(request: Request) {
  const denied = authorize(request);
  if (denied) return denied;
  return readHeartbeatVerdict('pipeline_delivery');
}
