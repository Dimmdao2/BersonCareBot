...
const { uploadMock, folderExistsMock, sessionMock, requireDoctorWorkspaceApiContextMock } =
  vi.hoisted(() => ({
    uploadMock: vi.fn(),
...
describe('POST /api/media/upload', () => {
  ...
  it('uploads and returns url for allowed image', async () => {
    ...
    fd.set(
      'file',
      new File([new Uint8Array([0xff, 0xd8, 0xff, 0xdb, 0x00])], 'a.jpg', { type: 'image/jpeg' }),
    );
    const res = await POST(
      new Request('http://localhost/api/media/upload', { method: 'POST', body: fd }),
    );
  ...
  it('returns 415 when MIME does not match magic bytes', async () => {
    ...
    fd.set(
      'file',
      new File([new Uint8Array([0x25, 0x50, 0x44, 0x46, 0x2d])], 'a.jpg', { type: 'image/jpeg' }),
    );
    const res = await POST(
      new Request('http://localhost/api/media/upload', { method: 'POST', body: fd }),
    );
  ...
  it('uploads multiple files from files[]', async () => {
    ...
    fd.append(
      'files[]',
      new File([new Uint8Array([0x25, 0x50, 0x44, 0x46, 0x2d])], 'b.pdf', {
  ...
  it('returns detailed error for mixed batch', async () => {
    ...
    fd.append(
      'files[]',
      new File([new Uint8Array([0xff, 0xd8, 0xff, 0xdb, 0x00])], 'ok.jpg', { type: 'image/jpeg' }),
    );
    fd.append(
      'files[]',
      new File([new Uint8Array([1, 2, 3])], 'bad.exe', { type: 'application/octet-stream' }),
    );
