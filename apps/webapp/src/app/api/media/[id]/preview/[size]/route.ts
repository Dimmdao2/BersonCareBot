...
export async function GET(
  ...
  const serve = async (session: AppSession): Promise<Response> => {
    ...
    if (
      ...
      logger.debug(
        { mediaId: id, size, validatorSource, cacheHit: true },
        '[preview GET] not modified (If-Modified-Since)',
      );
    ...
    if (ifNoneMatch && ifNoneMatch === etag) {
      logger.debug(
        { mediaId: id, size, validatorSource, cacheHit: true },
        '[preview GET] not modified (ETag, after body read)',
      );
      return new NextResponse(null, {
        status: 304,
        headers: {
    ...
    logger.debug(
      { mediaId: id, size, validatorSource, cacheHit: false },
      '[preview GET] served body',
    );
