...
export async function GET() {
  ...
  const enabled = parseStoredBoolean(settings.find((row) => row.key === "error_tracking_enabled")?.valueJson);
  const dsn = parseStoredString(settings.find((row) => row.key === "error_tracking_dsn")?.valueJson);
