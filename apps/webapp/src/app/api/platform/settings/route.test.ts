...
vi.mock('@/app-layer/di/buildAppDeps', () => ({
  buildAppDeps: () => ({
    systemSettings: { listSettingsByScope: listMock, updateSetting: updateMock },
  }),
}));
...
describe('/api/platform/settings', () => {
  beforeEach(() => {
    ...
    updateMock
      .mockReset()
      .mockImplementation(
        async (key: string, scope: string, valueJson: unknown, updatedBy: string) => ({
          key,
          scope,
          organizationId: null,
          valueJson,
  ...
  it('normalizes and writes the structured location palette globally', async () => {
    ...
    expect(updateMock).toHaveBeenCalledWith(
      ...
      'admin',
      {
        value: {
          ...value,
          physicalPalette: value.physicalPalette.map((color) => color.toUpperCase()),
          online: '#ABCDEF',
        },
      },
      platformSession.user.userId,
