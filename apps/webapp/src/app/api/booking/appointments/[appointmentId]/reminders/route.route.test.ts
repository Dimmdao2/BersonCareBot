...
function patchReminder(presetId: string | null) {
  return PATCH(
...
describe('patient appointment reminder preference', () => {
  it('does not deduplicate a later return to an earlier choice', async () => {
    expect((await patchReminder('day_before')).status).toBe(200);
    expect((await patchReminder('two_hours_before')).status).toBe(200);
    expect((await patchReminder('day_before')).status).toBe(200);
