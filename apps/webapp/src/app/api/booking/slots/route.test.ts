...
describe('GET /api/booking/slots', () => {
  ...
  it('uses canonical branchId+serviceId', async () => {
    ...
    const response = await GET(
      new Request(
        `http://localhost/api/booking/slots?type=in_person&branchId=${BRANCH_ID}&serviceId=${SERVICE_ID}`,
      ),
    );
    ...
    expect(getSlotsMock).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'in_person', branchId: BRANCH_ID, serviceId: SERVICE_ID }),
    );
  });
