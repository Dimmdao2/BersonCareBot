...
describe('patient booking history routes', () => {
  ...
  beforeEach(() => {
    ...
    listPatientPaymentHistoryMock.mockImplementation(
      async (organizationId: string, userId: string) => {
        expectExactPatientPrincipal(organizationId, userId);
        return [{ id: `payment:${organizationId}:${userId}` }];
      },
    );
    listPatientVisitHistoryMock.mockImplementation(
      async (organizationId: string, userId: string) => {
        expectExactPatientPrincipal(organizationId, userId);
  ...
  it('never crosses patient A/B organization walls', async () => {
    ...
    expect(listPatientPaymentHistoryMock).not.toHaveBeenCalledWith(
      ORG_A,
      USER_B,
      expect.anything(),
    );
