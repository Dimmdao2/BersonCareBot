...
requirePatientBookingTrustedPhoneAccessMock.mockImplementation(
...
describe('GET /api/booking/actions', () => {
  ...
  it('returns cancel and reschedule previews', async () => {
    getCurrentSessionMock.mockResolvedValue(patientClientSession);
    previewCancelMock.mockResolvedValue({
      ok: true,
      allowed: true,
      isFree: true,
      messageKey: 'cancel_free',
    });
    ...
    const response = await GET(
      new Request(
        'http://localhost/api/booking/actions?bookingId=2f14566f-a4de-4ab4-9336-5ddf806cd6ce',
      ),
    );
