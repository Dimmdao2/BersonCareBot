...
export async function POST(request: Request) {
  ...
  if (
    !detail?.purchase.paymentIntentId ||
    detail.purchase.paymentIntentId !== parsed.data.intentId
  ) {
    return NextResponse.json({ ok: false, error: 'purchase_not_found' }, { status: 404 });
  }
  ...
  const resolved = await withExplicitOrganizationPrincipal(
    ...
    () => resolveOrCreateUserByPhone(parsed.data.contactPhone, parsed.data.contactPhone, true),
  );
  if (!resolved.ok) {
  ...
  try {
    await withExplicitOrganizationPrincipal(
      ...
      () =>
        deps.products!.captureProductPayment(parsed.data.intentId, organizationId, resolved.userId),
    );
