...
const validUrl =
  'http://localhost/api/doctor/schedule-kpis?from=2026-06-01T00:00:00&to=2026-06-04T00:00:00';
const ORGANIZATION_A = '00000000-0000-4000-8000-0000000000aa';
const ORGANIZATION_B = '00000000-0000-4000-8000-0000000000bb';

function workspaceGate(organizationId = ORGANIZATION_A, role: 'doctor' | 'admin' = 'doctor') {
  return {
    ok: true as const,
    ctx: {
...
describe('GET /api/doctor/schedule-kpis', () => {
  ...
  it('returns 400 when from is missing', async () => {
    const res = await GET(
      new Request('http://localhost/api/doctor/schedule-kpis?to=2026-06-04T00:00:00'),
    );
  ...
  it('returns 400 when to is missing', async () => {
    const res = await GET(
      new Request('http://localhost/api/doctor/schedule-kpis?from=2026-06-01T00:00:00'),
    );
    expect(res.status).toBe(400);
  });
  ...
  it('passes from/to/branchId/serviceId to getScheduleKpis', async () => {
    const url =
      'http://localhost/api/doctor/schedule-kpis?from=2026-06-01T00:00:00&to=2026-06-08T00:00:00&branchId=branch-1&serviceId=svc-2';
  ...
  it.each([ORGANIZATION_A, ORGANIZATION_B])(
    ...
    async (organizationId) => {
      ...
      expect(getScheduleKpisMock).toHaveBeenCalledWith(expect.any(Object), {
        excludedUserIds: [],
        organizationId,
      });
    },
