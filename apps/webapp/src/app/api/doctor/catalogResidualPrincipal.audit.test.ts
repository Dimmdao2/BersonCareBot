...
const principalOnlyRepoWriteFiles = [
  "src/infra/repos/pgClinicalTestMeasureKinds.ts",
];

describe("doctor catalog residual principal coverage", () => {
  it.each(actionCoreFiles)("%s uses selected workspace principal for catalog write cores", (file) => {
  ...
  it.each(repoWriteFiles)("%s runs catalog writes through principal-aware mutation transactions", (file) => {
    const src = readSource(file);
