...
vi.mock('@/app-layer/di/buildAppDeps', () => ({
  buildAppDeps: () => ({
    ...
    messaging: {
      doctorSupport: { unreadFromPatient: unreadFromPatientMock, unreadFromUsers: vi.fn() },
    },
  }),
}));
...
describe('GET /api/doctor/messages/unread-count organization boundary', () => {
  ...
  it("does not count the second organization's patient", async () => {
    ...
    const response = await GET(
      new Request(`http://localhost/api/doctor/messages/unread-count?patientUserId=${PATIENT_B}`),
    );
  ...
  it("counts only the resolved organization's patient", async () => {
    ...
    const response = await GET(
      new Request(`http://localhost/api/doctor/messages/unread-count?patientUserId=${PATIENT_A}`),
    );
