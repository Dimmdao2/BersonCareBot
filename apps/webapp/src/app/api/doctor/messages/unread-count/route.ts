...
export async function GET(request: Request) {
  ...
  if (patientUserId) {
    ...
    unreadCount = await deps.messaging.doctorSupport.unreadFromPatient(patientUserId, auth.ctx.organizationId);
  } else {
    unreadCount = await deps.messaging.doctorSupport.unreadFromUsers({ organizationId: auth.ctx.organizationId });
  }
