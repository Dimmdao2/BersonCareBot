...
describe('GET /api/doctor/exercise-comments', () => {
  ...
  it('returns enriched items with patientDisplayName and href', async () => {
    ...
    const data = (await res.json()) as {
      ok: boolean;
      items: Array<{ patientDisplayName: string; href: string }>;
    };
  ...
  it('search dobor: filters items by patient name and message body (q param)', async () => {
    ...
    const res = await GET(new Request('http://localhost/api/doctor/exercise-comments?q=Иванов'));
    const data = (await res.json()) as {
      ok: boolean;
      items: Array<{ patientDisplayName: string }>;
    };
    expect(data.ok).toBe(true);
    expect(data.items).toHaveLength(1);
  ...
  it('search dobor: hasMore=false even if more rows exist (search is not paginated)', async () => {
    ...
    const res = await GET(new Request('http://localhost/api/doctor/exercise-comments?q=Текст'));
