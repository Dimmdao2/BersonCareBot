...
describe('/api/doctor/booking-engine/working-schedule-templates', () => {
  ...
  it('applies a template inside doctor workspace principal and forces own specialist', async () => {
    ...
    const res = await POST(
      new Request(
        'http://localhost/api/doctor/booking-engine/working-schedule-templates?action=apply',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  it('deletes a template inside doctor workspace principal', async () => {
    ...
    const res = await DELETE(
      new Request(
        `http://localhost/api/doctor/booking-engine/working-schedule-templates?id=${TMPL}`,
      ),
    );
    const json = (await res.json()) as { ok: boolean };
