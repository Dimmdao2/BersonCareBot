...
const { requireDoctorWorkspaceApiContextMock, buildAppDepsMock } = vi.hoisted(() => ({
...
function makeDeps(overrides: {
  ...
  return {
    ...
    programActionLog: {
      listDoneItemsByLocalDateInWindowForPatient: vi
        .fn()
        .mockResolvedValue(overrides.programDoneItems ?? []),
    },
...
describe('GET /api/doctor/patients/[userId]/exercise-calendar', () => {
  ...
  it('returns 401 when not authenticated', async () => {
    ...
    const res = await GET(makeRequest(userId, '2026-06-01', '2026-06-30'), {
      params: makeParams(userId),
    });
  ...
  it('returns 400 on invalid date params', async () => {
    ...
    const res = await GET(makeRequest(userId, 'not-a-date', '2026-06-30'), {
      params: makeParams(userId),
    });
  ...
  it('returns 404 before loading calendar sources outside selected workspace', async () => {
    ...
    expect(deps.doctorClientsPort.getClientIdentityForOrganization).toHaveBeenCalledWith(
      userId,
      ORG_ID,
    );
  ...
  it('merges counts from lfk_sessions, practiceCompletions (non-warmup), and programActionLog', async () => {
    const deps = makeDeps({
      ...
      programDoneItems: [{ localDate: '2026-06-10', itemId: 'item-1', instanceId: 'inst-1' }],
    });
    buildAppDepsMock.mockReturnValue(deps);
    const res = await GET(makeRequest(userId, '2026-06-01', '2026-06-30'), {
      params: makeParams(userId),
