...
async function resolvePatient(userId: string, organizationId: string) {
  ...
  const identity = await deps.doctorClientsPort.getClientIdentityForOrganization(userId, organizationId);
  return identity ? { deps, patientUserId: identity.userId, invitedEmail: identity.email ?? null } : null;
}
...
export async function GET(_request: Request, { params }: { params: Promise<{ userId: string }> }) {
  ...
  if (!parsed.success) return NextResponse.json({ ok: false, error: "invalid_user_id" }, { status: 400 });
...
export async function POST(_request: Request, { params }: { params: Promise<{ userId: string }> }) {
  ...
  const patient = await resolvePatient(parsed.data.userId, gate.ctx.organizationId);
  if (!patient) return NextResponse.json({ ok: false, error: "not_found" }, { status: 404 });
  const result = await withDoctorWorkspacePrincipal(gate.ctx, "doctor.patient-portal-invite.issue", () =>
...
export async function DELETE(request: Request, { params }: { params: Promise<{ userId: string }> }) {
  ...
  const patient = await resolvePatient(parsed.data.userId, gate.ctx.organizationId);
  if (!patient) return NextResponse.json({ ok: false, error: "not_found" }, { status: 404 });
  const revoked = await withDoctorWorkspacePrincipal(gate.ctx, "doctor.patient-portal-invite.revoke", () =>
