...
describe('doctor support/task workspace principal cutover', () => {
  ...
  it.each(principalBackedRepoFiles)(
    '%s uses principal-aware transactions and organization stamping',
    (file) => {
      const src = readSource(file);
  ...
  it.each(patientBoundReadRouteFiles)(
    '%s resolves patient-bound access through the selected workspace',
    (file) => {
      const src = readSource(file);
      expect(src).toContain('requireDoctorWorkspaceApiContext');
      expect(src).toContain('getClientIdentityForOrganization');
  ...
  it.each(doctorCommentsReadRouteFiles)(
    '%s scopes comment reads to the selected workspace',
    (file) => {
      const src = readSource(file);
      expect(src).toContain('requireDoctorWorkspaceApiContext');
      expect(src).toContain('withDoctorWorkspacePrincipal');
  ...
  it('doctor client-card schedule and booking writes use selected workspace organization', () => {
    const bookingProfileRoute = readSource(
      'src/app/api/doctor/clients/[userId]/booking-profile/route.ts',
    );
  ...
  it('doctor patient payment routes, repo, and SSR preload use selected workspace principal', () => {
    ...
    const timelineRoute = readSource(
      'src/app/api/doctor/patients/[userId]/payment-timeline/route.ts',
    );
    ...
    expect(timelineRoute).toContain(
      'listPaymentHistoryForUser(identity.userId, gate.ctx.organizationId)',
    );

    const acquiringRoute = readSource(
      'src/app/api/doctor/patients/[userId]/acquiring-charge/route.ts',
    );
  ...
  it('doctor patient adjunct read routes use selected workspace membership and canonical patient id', () => {
    ...
    const unlinkedRoute = readSource(
      'src/app/api/doctor/patients/[userId]/appointments/unlinked/route.ts',
    );
    ...
    expect(doctorClientsRepo).toContain(
      'listPatientAppointments(userId: string, organizationId?: string)',
    );
    ...
    expect(patientPracticeRepo).toContain(
    ...
    expect(programItemDiscussionRepo).toContain(
  ...
  it('doctor patient list route scopes list filters to the selected workspace organization', () => {
    ...
    expect(page.indexOf('deps.doctorClientsPort.getDashboardPatientMetrics({')).toBeLessThan(
      page.indexOf(
        ...
        page.indexOf('deps.doctorClientsPort.getDashboardPatientMetrics({'),
      ),
    );
    expect(page).not.toContain('requireDoctorAccess');
  });
