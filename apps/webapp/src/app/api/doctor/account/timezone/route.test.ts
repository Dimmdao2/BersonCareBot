...
const { requireDoctorApiSessionMock, getDoctorAccountTimezoneMock, setDoctorAccountTimezoneMock } =
  vi.hoisted(() => ({
...
describe('doctor account timezone route', () => {
  ...
  it('PATCH stores valid timezone', async () => {
    ...
    expect(setDoctorAccountTimezoneMock).toHaveBeenCalledWith(
      sessionOk.session.user.userId,
      'Europe/Samara',
    );
  });
