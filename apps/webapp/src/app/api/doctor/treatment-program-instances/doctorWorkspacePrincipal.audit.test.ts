...
describe("doctor treatment program instance principal cutover", () => {
  it.each(mutatingRouteFiles)("%s resolves workspace and wraps mutations in DB principal", (file) => {
    const src = readSource(file);
    expect(src).toContain("requireDoctorWorkspaceApiContext");
    expect(src).toContain("withDoctorWorkspacePrincipal");
  });

  it.each(transactionBackedRepoFiles)("%s has no direct db writes outside a transaction wrapper", (file) => {
    const src = readSource(file);
    expect(src).not.toMatch(/await\s+db\s*\.\s*(insert|update|delete)\b/);
    expect(src).not.toMatch(/const\s+\[[^\]]+\]\s*=\s*await\s+db\s*\.\s*(insert|update|delete)\b/);
  ...
  it("create route resolves the patient inside the selected workspace organization", () => {
    const src = readSource("src/app/api/doctor/clients/[userId]/treatment-program-instances/route.ts");
    expect(src).toContain("getClientIdentityForOrganization");
    expect(src).toContain("gate.ctx.organizationId");
  });

  it.each(
    mutatingRouteFiles.filter((file) => file.includes("/treatment-program-instances/[instanceId]/")),
