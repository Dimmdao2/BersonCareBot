/** POST /api/doctor/tasks/:taskId/reactivate */
...
export async function POST(_request: Request, context: { params: Promise<{ taskId: string }> }) {
  ...
  if (!entitlement.ok) {
    return entitlementMutationRefusalResponse('specialist_tasks', 'активировать задачу');
  }
  ...
  const task = await withDoctorWorkspacePrincipal(gate.ctx, () =>
    deps.specialistTasks.reactivate(taskId, session.user.userId),
  );
