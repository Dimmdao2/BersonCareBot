...
const { verifySignatureMock, executeMessengerPhoneHttpBindMock, isAuthChannelEnabledMock } =
  vi.hoisted(() => ({
...
vi.mock('@/app-layer/integrator/messengerPhoneHttpBindExecute', () => ({
  executeMessengerPhoneHttpBindWithDefaultPool: (...args: unknown[]) =>
    executeMessengerPhoneHttpBindMock(...args),
}));
...
describe('POST /api/integrator/messenger-phone/bind', () => {
  beforeEach(async () => {
    process.env.IDEMPOTENCY_STORE_PATH =
      '/tmp/bersoncare-webapp-idempotency-messenger-bind-test.json';
