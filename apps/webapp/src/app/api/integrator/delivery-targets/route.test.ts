...
describe('GET /api/integrator/delivery-targets', () => {
  ...
  it('returns 403 when target identity is outside the signed organization', async () => {
    const { DeliveryTargetsTenantDeniedError } =
      await import('@/modules/integrator/deliveryTargetsApi');
    getTargetsMock.mockRejectedValue(new DeliveryTargetsTenantDeniedError());
    const res = await GET(
      new Request(
        'http://localhost/api/integrator/delivery-targets?telegramId=tg1&integratorUserId=42&organizationId=11111111-1111-4111-8111-111111111111',
        {
  ...
  it('returns 404 instead of a legacy fallback when target identity is not found', async () => {
    ...
    const res = await GET(
      new Request(
        'http://localhost/api/integrator/delivery-targets?telegramId=missing&integratorUserId=42&organizationId=11111111-1111-4111-8111-111111111111',
        {
          headers: integratorGetSignedHeadersOk,
        },
      ),
  ...
  it('returns 401 when signature invalid', async () => {
    const res = await GET(
      new Request(
        'http://localhost/api/integrator/delivery-targets?telegramId=tg1&organizationId=11111111-1111-4111-8111-111111111111',
        {
