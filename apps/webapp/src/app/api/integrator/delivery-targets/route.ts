...
export async function GET(request: Request) {
  ...
  if (!organizationId || !enterVerifiedIntegratorOrganizationPrincipal(organizationId, "integrator-delivery-targets")) {
  ...
  try {
    ...
    if (error instanceof DeliveryTargetsTenantDeniedError) {
      return NextResponse.json({ ok: false, error: "delivery target is outside organization" }, { status: 403 });
    }
    throw error;
  }
  if (!result) {
