...
export async function POST(request: Request) {
  ...
  if (!enterVerifiedIntegratorOrganizationPrincipal(parsed.data.organizationId, 'integrator-payment-captured')) {
  ...
  const handler = buildAppDeps().appointmentPaymentConfirmed;
  if (!handler) return NextResponse.json({ ok: false, error: 'payment_lifecycle_unavailable' }, { status: 503 });
  try {
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : 'payment_lifecycle_failed' },
      { status: 503 },
    );
  }
}
