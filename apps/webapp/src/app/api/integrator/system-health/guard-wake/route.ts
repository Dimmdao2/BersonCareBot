...
export async function POST(request: Request) {
  ...
  if (!parsed.success || idempotencyKey !== `system-health-guard-wake:${parsed.data.wakeId}`) {
    return NextResponse.json({ ok: false, error: 'invalid payload' }, { status: 400 });
  }
  enterWithDbInfraPrincipal({ source: 'api/integrator/system-health/guard-wake:POST' });
