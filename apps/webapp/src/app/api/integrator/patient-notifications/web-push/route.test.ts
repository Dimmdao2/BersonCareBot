...
const mocks = vi.hoisted(() => ({
  ...
  findByIntegratorId: vi.fn(async () => ({
...
vi.mock('@/modules/patient-notifications/patientWebPushNotify', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('@/modules/patient-notifications/patientWebPushNotify')>();
...
describe('patient web-push signed M2M organization boundary', () => {
  ...
  it('enters signed organization before idempotency and verifies enrollment', async () => {
    ...
    expect(mocks.enterOrg.mock.invocationCallOrder[0]).toBeLessThan(
      mocks.getCached.mock.invocationCallOrder[0]!,
    );
    expect(mocks.hasActiveEnrollment).toHaveBeenCalledWith(
