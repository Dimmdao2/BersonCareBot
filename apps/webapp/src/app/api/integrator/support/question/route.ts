...
import { integratorSupportQuestionWriteSchema } from '@/modules/messaging/integratorSupportHttp';

/** POST /api/integrator/support/question — webapp-owned canonical question write. */
export async function POST(request: Request) {
  ...
  const parsed = integratorSupportQuestionWriteSchema.safeParse(parsedJson);
  if (!parsed.success) {
  ...
  const result = await bridge.syncQuestionWrite(parsed.data);
  const status = result.ok ? 200 : result.error === 'organization_mismatch' ? 403 : 400;
