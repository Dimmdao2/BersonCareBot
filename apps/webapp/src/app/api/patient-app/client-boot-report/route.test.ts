...
const { ensurePortsMock, fallbackEnabledMock, rateLimitMock, infoMock, warnMock, errorMock } =
  vi.hoisted(() => ({
...
vi.mock('@/app-layer/di/bindAuthModulePorts', () => ({
  ensureAuthModulePortsBound: ensurePortsMock,
}));
...
describe('POST /api/patient-app/client-boot-report', () => {
  ...
  it('accepts a valid chunked Web Stream without Content-Length', async () => {
    ...
    const streamed = streamingRequest([serialized.slice(0, 25), serialized.slice(25)], {
  ...
  it('streams and validates the body when Content-Length is invalid', async () => {
    const streamed = streamingRequest([JSON.stringify(validPayload)], {
      'content-length': '9999-invalid',
    });
    const response = await POST(streamed.request);
    expect(response.status).toBe(202);
    expect(infoMock).toHaveBeenCalledOnce();
  ...
  it.each([
    ...
    'stack',
    'body',
  ])('rejects forbidden or unknown raw field %s without logging its value', async (field) => {
