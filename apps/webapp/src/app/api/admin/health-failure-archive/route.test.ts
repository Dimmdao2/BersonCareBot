...
describe('GET /api/admin/health-failure-archive', () => {
  ...
  it('returns 400 on invalid probe query', async () => {
    const res = await GET(
      new Request('http://localhost/api/admin/health-failure-archive?probe=not-a-probe'),
    );
  ...
  it('returns 200 and delegates listForAdmin', async () => {
    ...
    const res = await GET(
      new Request(
        'http://localhost/api/admin/health-failure-archive?probe=outgoing_delivery&limit=10',
      ),
    );
  ...
  it('passes cursor to listForAdmin', async () => {
    ...
    expect(res.status).toBe(200);
    expect(listForAdminMock).toHaveBeenCalledWith(expect.objectContaining({ cursor: 'curs123' }));
  });
