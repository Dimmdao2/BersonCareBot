...
export async function POST() {
  ...
  const { acknowledged } = await buildAppDeps().operatorHealthWrite.acknowledgeOpenOutboundProviderIncidents();
  logger.info({ acknowledged, actorId: gate.session.user.userId }, "operator_incidents.acknowledge_all");
