...
const mocks = vi.hoisted(() => ({
  admin: vi.fn(),
  workspace: vi.fn(),
  principal: vi.fn((_ctx: unknown, fn: () => unknown) => fn()),
  acknowledge: vi.fn(),
  audit: vi.fn(),
  pool: { tag: 'pool' },
}));
...
vi.mock('@/app-layer/guards/requireRole', () => ({
  requireDoctorWorkspaceApiContext: mocks.workspace,
}));
...
vi.mock('@/app-layer/di/buildAppDeps', () => ({
  buildAppDeps: () => ({
    operatorHealthWrite: { acknowledgeOpenOutboundProviderIncidents: mocks.acknowledge },
  }),
}));
