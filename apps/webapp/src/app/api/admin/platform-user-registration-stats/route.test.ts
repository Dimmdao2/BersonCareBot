...
describe("GET /api/admin/platform-user-registration-stats", () => {
  ...
  it("returns stats payload when authorized", async () => {
    ...
    getRegistrationStatsMock.mockResolvedValue({
      ...
      fromDay: "2026-05-16",
      toDay: "2026-05-16",
      startUtcIso: "2026-05-15T21:00:00.000Z",
    ...
    const res = await GET(new Request("http://localhost/api/admin/platform-user-registration-stats?preset=today"));
    ...
    expect(getRegistrationStatsMock).toHaveBeenCalledWith(
      expect.objectContaining({ preset: "today", iana: "Europe/Moscow" }),
    );
  });
});
