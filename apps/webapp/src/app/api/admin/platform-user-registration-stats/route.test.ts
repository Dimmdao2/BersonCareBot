...
describe('GET /api/admin/platform-user-registration-stats', () => {
  ...
  it('returns 403 before DB-backed stats when the platform guard rejects a foreign audience', async () => {
    ...
    const res = await GET(
      new Request('http://localhost/api/admin/platform-user-registration-stats'),
    );
    expect(res.status).toBe(403);
    expect(loadDoctorAnalyticsAudienceMock).not.toHaveBeenCalled();
    expect(getRegistrationStatsMock).not.toHaveBeenCalled();
  });
  ...
  it('normalizes preset=today query to week', async () => {
    ...
    await GET(
      new Request('http://localhost/api/admin/platform-user-registration-stats?preset=today'),
    );
    expect(getRegistrationStatsMock).toHaveBeenCalledWith(
