...
describe('admin audit log workspace principal cutover', () => {
  it.each(adminAuditRouteFiles)(
    '%s requires selected workspace and runs audit work under principal',
    (file) => {
  ...
  it.each(adminOpsAuditWriteRouteFiles)(
    '%s writes admin audit rows under selected workspace principal',
    (file) => {
      const src = readSource(file);
      expect(src).toContain('requireDoctorWorkspaceApiContext');
      expect(src).toContain('withDoctorWorkspacePrincipal');
