...
export async function PATCH(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const room = await withDoctorWorkspacePrincipal(gate.ctx, "admin.booking-engine.rooms.update", () =>
...
export async function DELETE(_request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const bad = jsonIfInvalidUuid(id);
  if (bad) return bad;
  const ok = await withDoctorWorkspacePrincipal(gate.ctx, "admin.booking-engine.rooms.deactivate", () =>
