...
describe('parseDoctorCatalogRegionQueryParam', () => {
  ...
  it('drops UUID without separate error flag', () => {
    expect(parseDoctorCatalogRegionQueryParam('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa')).toEqual({
  ...
  it('accepts missing-region sentinel', () => {
    expect(parseDoctorCatalogRegionQueryParam('__missing__')).toEqual({
      regionCode: '__missing__',
    });
  });
});
