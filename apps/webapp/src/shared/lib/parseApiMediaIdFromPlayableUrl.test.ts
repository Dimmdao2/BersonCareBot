...
describe('parseApiMediaIdFromPlayableUrl', () => {
  ...
  it('rejects wrong shape', () => {
    ...
    expect(
      parseApiMediaIdFromPlayableUrl(`https://evil.example/api/media/${SAMPLE_ID}`),
    ).toBeNull();
  });
});
...
describe('parseApiMediaIdFromHref', () => {
  ...
  it('matches absolute URL when origin trusted', () => {
    expect(parseApiMediaIdFromHref(`https://app.example/api/media/${SAMPLE_ID}`, trusted)).toBe(
      SAMPLE_ID,
    );
  });
  ...
  it('rejects foreign origin', () => {
    expect(
      parseApiMediaIdFromHref(`https://evil.example/api/media/${SAMPLE_ID}`, trusted),
    ).toBeNull();
  });
...
describe('parseApiMediaIdFromMarkdownHref', () => {
  ...
  it('returns null when no origin matches absolute URL', () => {
    expect(
      parseApiMediaIdFromMarkdownHref(`https://evil.example/api/media/${SAMPLE_ID}`, [
        'https://app.example',
      ]),
    ).toBeNull();
  });
