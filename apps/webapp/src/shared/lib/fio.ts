...
const STRONG_SOURCES = new Set<FioSource>([
  'manual',
  'booking',
  'native_booking',
  'profile_structured',
...
export function normalizeFioPart(value: string | null | undefined): string | null {
  ...
  return normalized.split(' ').map(titleCasePart).join(' ');
}
...
function markToken(token: string, dictionaries: RussianNameDictionaries): TokenMark {
  ...
  const isPatronymic =
    (dictionaries.patronymics?.has(key) ?? false) || likelyPatronymicBySuffix(token);
  return {
...
function confidenceFor(
  ...
  const recognizedPatronymic =
    !value.patronymic || marks.some((mark) => mark.value === value.patronymic && mark.isPatronymic);
  ...
  if (
    value.firstName &&
    value.lastName &&
    STRONG_SOURCES.has(source) &&
    !reasons.includes('latin_or_mixed')
  ) {
    return recognizedFirst ? 'high' : 'medium';
  }
  if (value.firstName && value.patronymic && recognizedFirst && recognizedPatronymic)
    return 'medium';
...
export function parseFioCandidate(
  ...
  const marks = normalizedInput
    .split(' ')
    .slice(0, 4)
    .map((token) => markToken(token, dictionaries));
  const value = parseMarkedTokens(marks, reasons);
  if (
    value.firstName &&
    !marks.some((mark) => mark.value === value.firstName && mark.isFirstName)
  ) {
    reasons.push('unknown_first_name');
  }
  if (
    value.patronymic &&
    !marks.some((mark) => mark.value === value.patronymic && mark.isPatronymic)
  ) {
...
function fioComparisonKey(value: StructuredFio): string {
  return [value.lastName, value.firstName, value.patronymic]
    .map((part) => dictionaryKey(part ?? ''))
    .join('|');
}

export function formatDoctorFio(value: StructuredFio | null | undefined, fallback = ''): string {
  const label = [value?.lastName, value?.firstName, value?.patronymic]
