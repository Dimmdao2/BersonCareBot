...
export function classifySelectSurfaces(source: string): Surface[] {
  ...
  while ((match = re.exec(source)) !== null) {
    ...
    if (/\bitems\s*=/.test(openTag)) {
      out.push({ line, safeBy: 'items= on Select' });
      continue;
    }
    if (/\bitemToStringLabel\s*=/.test(openTag)) {
      out.push({ line, safeBy: 'itemToStringLabel=' });
      continue;
    }
    if (/<SelectTrigger[^>]*\bdisplayLabel\s*=/s.test(body)) {
      out.push({ line, safeBy: 'displayLabel=' });
      continue;
    }
    // `<SelectValue>…</SelectValue>` с непустыми детьми.
    if (/<SelectValue(?:\s[^>]*?)?>\s*[^<\s]/s.test(body)) {
      out.push({ line, safeBy: 'SelectValue children' });
      continue;
    }
    // Триггер без `<SelectValue>` вообще — подпись рисует сам вызов.
    if (!/<SelectValue/.test(body)) {
      out.push({ line, safeBy: 'no SelectValue' });
      continue;
    }
...
describe('детектор классификации select-поверхностей', () => {
  ...
  it('считает безопасными все поддержанные формы подписи', () => {
    ...
    for (const [expected, source] of Object.entries(cases)) {
      expect(
        classifySelectSurfaces(source).map((s) => s.safeBy),
        expected,
      ).toEqual([expected]);
    }
  });
  ...
  it('разбирает открывающий тег с `{}` в пропах (иначе тело съедет)', () => {
    ...
    expect(classifySelectSurfaces(tricky).map((s) => s.safeBy)).toEqual([
      'auto-collected <SelectItem>',
    ]);
  });
});
...
describe('перепись select-поверхностей приложения', () => {
  ...
  const withSelect = files.filter(
    (rel) =>
      /\.tsx$/.test(rel) && readFileSync(new URL(rel, SRC), 'utf8').includes('<SelectTrigger'),
  );
  ...
  it('Base UI select берут только через канонический примитив', () => {
    const direct = files.filter(
      (rel) =>
        rel !== CANONICAL_PRIMITIVE &&
        /from\s+["']@base-ui\/react\/select["']/.test(readFileSync(new URL(rel, SRC), 'utf8')),
    );
  ...
  it('копии примитива реэкспортируют канон, а не дублируют его', () => {
    for (const rel of [
      ...
      'shared/ui/patient/primitives/select.tsx',
      'shared/ui/doctor/primitives/select.tsx',
    ]) {
