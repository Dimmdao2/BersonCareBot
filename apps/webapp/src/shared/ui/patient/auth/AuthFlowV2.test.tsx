      ...
      vi.fn((input: RequestInfo | URL) => {
        ...
        if (url.includes('/api/auth/check-phone')) {
          return jsonRes({
            ok: true,
            exists: true,
            methods: { sms: true, pin: true, telegram: true },
          });
        }
      ...
      vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes('/api/auth/check-phone')) {
          return jsonRes({
            ok: true,
            exists: true,
            methods: { sms: true, pin: false, telegram: true },
          });
        }
        if (url.includes('/api/auth/phone/start')) {
...
describe('AuthFlowV2 — browser', () => {
  ...
  it('restores pending register verify UI from sessionStorage', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/Код отправлен на user@example\.com/i)).toBeInTheDocument(),
    );
  ...
  it('phone login path: check-phone, messenger bind, confirm', async () => {
    ...
    expect(fetchMock.mock.calls.some((c) => String(c[0]).includes('/api/auth/phone/confirm'))).toBe(
      true,
    );
    expect(
      fetchMock.mock.calls.some((c) =>
        String(c[0]).includes('/api/auth/phone/messenger-bind/finish'),
      ),
    ).toBe(false);
  });
    ...
    vi.stubGlobal(
      ...
      vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes('/api/auth/email-otp/register')) {
          ...
          return jsonRes({
            ok: true,
            challengeId: `patient-ch-${payloads.length}`,
            retryAfterSeconds: 0,
          });
        }
        return jsonRes({});
  ...
  it('completes the staff password login through a one-time recovery factor', async () => {
    ...
    expect(
      screen.getByRole('link', { name: 'Нет доступа к приложению и резервным кодам' }),
    ).toHaveAttribute('href', '/app/contact-support?from=staff-factor');
  ...
  it('shows distinct invalid and taken public-address feedback before signup submit', async () => {
    ...
    const fetchMock = vi.fn((input: RequestInfo | URL) => {
      const url = typeof input === 'string' ? input : input.toString();
      if (url.includes('/api/auth/specialist-signup/slug')) {
        return jsonRes({ ok: false, error: 'slug_unavailable' }, { ok: false, status: 409 });
      }
      return jsonRes({});
    });
    vi.stubGlobal('fetch', fetchMock);
    ...
    vi.stubGlobal(
      ...
      vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes('/api/auth/specialist-signup/start')) {
          ...
          return jsonRes({
            ok: true,
            challengeId: `signup-ch-${payloads.length}`,
            retryAfterSeconds: 0,
          });
        }
        return jsonRes({});
  ...
  it('specialist signup shows duplicate email error from start endpoint', async () => {
    ...
    await waitFor(() =>
      expect(toastError).toHaveBeenCalledWith('Аккаунт с этой почтой уже существует.'),
    );
  });
    ...
    vi.stubGlobal(
      ...
      vi.fn((input: RequestInfo | URL) => {
        ...
        if (url.includes('/api/auth/specialist-signup/confirm')) {
          ...
          return jsonRes(
            { ok: false, error: 'too_many_attempts', retryAfterSeconds: 60 },
            { ok: false, status: 429 },
          );
        }
        return jsonRes({});
  ...
  it('specialist signup confirm shows disabled message when rollout is turned off before provisioning', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn((input: RequestInfo | URL) => {
        ...
        if (url.includes('/api/auth/specialist-signup/confirm')) {
          return jsonRes(
            { ok: false, error: 'specialist_signup_disabled' },
            { ok: false, status: 423 },
          );
        }
        return jsonRes({});
    ...
    expect(
      await screen.findByText('Регистрация кабинета специалиста пока недоступна.'),
    ).toBeInTheDocument();
  });
