...
vi.mock('@/modules/messaging/hooks/useSupportUnreadPolling', async (importOriginal) => {
  const actual =
    await importOriginal<typeof import('@/modules/messaging/hooks/useSupportUnreadPolling')>();
  return {
    ...actual,
    usePatientSupportUnreadCount: () => {
      ...
      useEffect(() => {
        ...
        return () =>
          window.removeEventListener('bersoncare:patient-support-unread-refresh', onRefresh);
      }, []);
      return count;
    },
...
describe('PatientTopNav', () => {
  it('renders mobile top nav as moved bottom menu, no warmups or desktop actions', () => {
    ...
    ).toEqual(['Сегодня', 'Упражнения', 'Статистика', 'Запись', 'Чат']);
  ...
  it('sets aria-current=page on active nav link', () => {
    ...
    render(<PatientTopNav />);
    const mobileNav = screen.getByTestId('patient-mobile-top-nav');
    expect(within(mobileNav).getByRole('link', { name: 'Сегодня' })).toHaveAttribute(
      'aria-current',
      'page',
    );
  });
  ...
  it('sets aria-current=page on Упражнения when pathname matches /app/patient/treatment', () => {
    ...
    render(<PatientTopNav />);
    const mobileNav = screen.getByTestId('patient-mobile-top-nav');
    expect(within(mobileNav).getByRole('link', { name: 'Упражнения' })).toHaveAttribute(
      'aria-current',
      'page',
    );
  });
  ...
  it('does not set plan active on legacy treatment-programs pathname', () => {
    ...
    render(<PatientTopNav />);
    const mobileNav = screen.getByTestId('patient-mobile-top-nav');
    expect(within(mobileNav).getByRole('link', { name: 'Упражнения' })).not.toHaveAttribute(
      'aria-current',
      'page',
    );
  });
