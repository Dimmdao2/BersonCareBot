...
/**
 * Patient `Dialog`: на десктопе — общий shadcn-диалог, на мобиле геометрию задаёт
 * канонический patient {@link DrawerContent}. Так у оставшихся legacy-диалогов зоны не может
 * появиться собственная мобильная геометрия, расходящаяся с {@link PatientModal}.
 * Новые feature-модалки открываются через `PatientModal`, а не напрямую этими примитивами.
 */
type PatientDialogContextValue = {
  isMobile: boolean;
  isNestedLayer: boolean;
  showRootOverlay: boolean;
};

const PatientDialogMobileContext = React.createContext<PatientDialogContextValue>({
  isMobile: false,
  isNestedLayer: false,
...
function Dialog(props: DialogPrimitive.Root.Props) {
  ...
  const { isNestedLayer, parentDepth } = usePatientModalLayer();
  ...
  const showRootOverlay = usePatientModalOverlay(currentOpen && !isNestedLayer, isNestedLayer);
  ...
  if (!isMobile) {
    return (
      <PatientDialogMobileContext.Provider
        value={{ isMobile: false, isNestedLayer, showRootOverlay }}
      >
        <PatientModalLayerProvider depth={parentDepth + (currentOpen ? 1 : 0)}>
  ...
  return (
    <PatientDialogMobileContext.Provider value={{ isMobile: true, isNestedLayer, showRootOverlay }}>
      <PatientModalLayerProvider depth={parentDepth + (currentOpen ? 1 : 0)}>
...
function DialogTrigger(props: DialogPrimitive.Trigger.Props) {
  const { isMobile } = React.useContext(PatientDialogMobileContext);
...
function DialogContent({
  ...
  const { isMobile, showRootOverlay } = React.useContext(PatientDialogMobileContext);
  ...
  return (
    <DrawerContent
      ...
      className={cn(
        className,
        fullScreen
          ? '!h-dvh !max-h-dvh !max-w-full rounded-none border-0 bg-black p-0'
