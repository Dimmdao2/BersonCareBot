...
describe('doctor presentation chrome', () => {
  ...
  it('keeps the page header white and exposes a full-width right slot', () => {
    render(<DoctorPageHeader title="Клиенты" tabsClassName="w-full" tabs={<div>Поиск</div>} />);
  ...
  it('uses 18px row alignment with larger, lighter primary list text', () => {
    expect(doctorDnaFlatListInsetClass).toContain('mx-[var(--doctor-block-padding,18px)]');
    expect(doctorDnaFlatListRowClass).toContain('px-[var(--doctor-list-inline-padding,18px)]');
  ...
  it('keeps menu rows minimally rounded and section tabs independently pill-shaped', () => {
    ...
    expect(doctorSectionTabClass(true)).toContain('rounded-[var(--doctor-control-radius,24px)]');
    ...
    expect(doctorSectionTabClass(false)).toContain('hover:bg-[var(--doctor-section-tab-hover)]');
