...
describe('DoctorMenuAccordion', () => {
  ...
  it('renders top-level sidebar links including Каталог ЛФК group trigger', () => {
    render(
      <DoctorMenuAccordion variant="sidebar" pathname="/app/doctor" menuAccess={menuAccess} />,
    );
  ...
  it('sidebar: Каталог ЛФК trigger has aria-haspopup=menu', () => {
    ...
    expect(screen.getByRole('button', { name: /Каталог ЛФК/ })).toHaveAttribute(
      'aria-haspopup',
      'menu',
    );
  });
  ...
  it('shows settings and personal account links for a dual-capability actor (doctor menu kind, default)', () => {
    render(
      <DoctorMenuAccordion variant="sidebar" pathname="/app/doctor" menuAccess={adminAccess} />,
    );
  ...
  it('hides communications badge when counts are zero', () => {
    ...
    expect(
      screen
        .getByRole('link', { name: /Коммуникации/ })
        .querySelector("[aria-label^='Непрочитанных']"),
    ).toBeNull();
  });
