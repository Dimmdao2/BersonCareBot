...
/** Цвета и обводка звёзд (см. `.material-rating-stars` в `patient.css`). */
...
export function MaterialRatingBlock({
  ...
  if (loading && !aggregate) {
    return (
      <div className={cn("text-sm text-muted-foreground", className)} aria-busy>
        …
      </div>
    );
  }
    ...
    <div className={cn("flex flex-col gap-1.5", className)}>
      {showSummaryRow ? (
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs font-normal leading-snug">
          <span className={cn("text-sm text-muted-foreground", "font-normal")}>Ваша оценка:</span>
