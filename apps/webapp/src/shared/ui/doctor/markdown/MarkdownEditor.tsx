...
function ToolbarButton({ active = false, disabled = false, children, onClick }: ToolbarButtonProps) {
  return (
    <Button
      type="button"
...
export function MarkdownEditor({
  ...
  const setLink = useCallback(() => {
    ...
    const href = window.prompt("Адрес ссылки", typeof previous === "string" ? previous : "https://");
    if (href === null) return;
    if (href.trim() === "") {
  ...
  const helper =
    helpText === null ? null : helpText === undefined ? (
      <p className="m-0 text-sm text-muted-foreground">До {maxLength.toLocaleString("ru-RU")} символов.</p>
    ) : (
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap gap-2" role="toolbar" aria-label="Форматирование текста">
        ...
        <ToolbarButton active={editor?.isActive("link")} disabled={!editor || disabled} onClick={setLink}>
          Ссылка
        </ToolbarButton>
        <ToolbarButton
          disabled={!editor || disabled}
          onClick={() => run(() => editor?.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run() ?? false)}
        >
          Таблица
        </ToolbarButton>
