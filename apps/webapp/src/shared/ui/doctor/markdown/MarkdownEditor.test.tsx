...
vi.mock('./MediaLibraryInsertDialog', () => ({
  MediaLibraryInsertDialog: ({
    ...
    <button
      ...
      disabled={disabled}
      onClick={() =>
        onInsert('/api/media/11111111-1111-4111-8111-111111111111', 'photo.png', { kind: 'image' })
      }
    >
...
describe('MarkdownEditor', () => {
  ...
  it('serializes edits into the hidden form field and onChange', async () => {
    ...
    const { container } = render(
      <MarkdownEditor name="body_md" defaultValue="" onChange={onChange} />,
    );
    ...
    await waitFor(() =>
      expect(container.querySelector('input[name="body_md"]')).toHaveValue('Новый текст'),
    );
  ...
  it('inserts media through the existing media-library control', async () => {
    ...
    await waitFor(() => {
      const value =
        (container.querySelector('input[name="body_md"]') as HTMLInputElement | null)?.value ?? '';
  ...
  it('rejects edits whose serialized markdown exceeds maxLength', async () => {
    const user = userEvent.setup();
    const { container } = render(
      <MarkdownEditor name="body_md" defaultValue="12345" maxLength={5} />,
    );
    const textbox = await findEditor(container);
  ...
  it('accepts an external controlled value without emitting a local change', async () => {
    ...
    await waitFor(() =>
      expect(container.querySelector('input[name="body_md"]')).toHaveValue('Второе'),
    );
