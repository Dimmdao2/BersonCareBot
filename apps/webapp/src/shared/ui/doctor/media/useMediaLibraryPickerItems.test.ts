...
import { pickerListTotalForKind } from '@/shared/ui/doctor/media/useMediaPickerFilteredList';
...
describe('filterMediaLibraryPickerItemsByQuery', () => {
  ...
  it('matches filename substring case-insensitively', () => {
    const items = [
      baseItem({ filename: 'Alpha.PNG' }),
      baseItem({ id: '2', filename: 'beta.jpg' }),
    ];
...
describe('narrowMediaLibraryPickerItemsByKind', () => {
  ...
  it('for all returns items unchanged', () => {
    const items = [
      baseItem({ id: 'a' }),
      baseItem({ id: 'b', kind: 'file', mimeType: 'text/plain' }),
    ];
