...
describe('isDoctorNavItemActive', () => {
  ...
  it('matches path prefix for nested routes', () => {
    ...
    expect(
      isDoctorNavItemActive('/app/doctor/analytics/clients', '/app/doctor/analytics/clients'),
    ).toBe(true);
  });
  ...
  it('matches communications paths', () => {
    expect(isDoctorNavItemActive('/app/doctor/communications', '/app/doctor/communications')).toBe(
      true,
    );
    expect(
...
describe('doctor menu structure', () => {
  it('requires both clinical workspace and the trusted courses entitlement', () => {
    expect(
      getDoctorMenuItems({ ...doctorAccess, coursesEnabled: false }).map((item) => item.id),
  ...
  it('isDoctorMenuLinkVisible evaluates clinic-management and global tiers separately', () => {
    ...
    expect(isDoctorMenuLinkVisible(globalItem, { capabilities: ['organization.management'] })).toBe(
      false,
    );
    expect(isDoctorMenuLinkVisible(globalItem, { capabilities: ['clinical.workspace'] })).toBe(
      false,
    );
...
describe('getDoctorShellHomeHref', () => {
  ...
  it('keeps an owner with a specialist binding clinical by default but exposes management nav', () => {
    const access = {
      capabilities: ['organization.management', 'clinical.workspace', 'account.self'] as const,
    };
