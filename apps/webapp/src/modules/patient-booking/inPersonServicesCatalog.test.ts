...
describe('inPersonServicesCatalog', () => {
  ...
  it('keeps the reserved Online branch out of the in-person city list', async () => {
    ...
    await expect(
      listInPersonCitiesForOrganization(onlineDeps, organizationId),
    ).resolves.toMatchObject([{ code: 'msk' }]);
  });
  ...
  it('does not expose a location-only service without an active specialist assignment', async () => {
    ...
    const result = await listInPersonServicesForBranch(locationOnlyDeps, organizationId, branchId);
  ...
  it('uses the selected specialist assignment when one is provided', async () => {
    ...
    await expect(
      listInPersonServicesForBranch(selectedDeps, organizationId, branchId, anotherSpecialistId),
  ...
  it('exposes Online only when active and assigned to an active exact-org specialist', async () => {
    ...
    await expect(
      resolveBookableOnlineLocationForOrganization(onlineDeps, organizationId),
    ).resolves.toEqual({
      id: onlineBranchId,
      cityCode: 'online',
      title: 'Онлайн',
    ...
    await expect(
      resolveBookableOnlineLocationForOrganization(unassigned, organizationId),
    ).resolves.toBeNull();
    ...
    await expect(
      resolveBookableOnlineLocationForOrganization(inactive, organizationId),
    ).resolves.toBeNull();
  });
  ...
  it('fails closed when a branch belongs to another organization', async () => {
    ...
    await expect(
      listInPersonServicesForBranch(foreignDeps, organizationId, branchId),
    ).resolves.toBeNull();
