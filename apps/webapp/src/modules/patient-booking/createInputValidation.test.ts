...
describe('validateCreatePatientBookingInput', () => {
  it('normalizes canonical in-person keys', () => {
    expect(validateCreatePatientBookingInput(base)).toMatchObject({
  ...
  it('rejects malformed canonical keys', () => {
    expect(() => validateCreatePatientBookingInput({ ...base, branchId: 'not-uuid' })).toThrow(
      'invalid_in_person_keys',
    );
  });
  ...
  it('rejects empty cityCode for in_person', () => {
    expect(() => validateCreatePatientBookingInput({ ...base, cityCode: '   ' })).toThrow(
      'invalid_city_code',
    );
  });
});
