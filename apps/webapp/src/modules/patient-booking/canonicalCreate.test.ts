...
function deps() {
  const bookingsPort = {
    ...
    markAwaitingPayment: vi
      .fn()
      .mockResolvedValue({ ...pendingRecord(), status: 'awaiting_payment' }),
  };
  const bookingEngine = {
    catalog: {
      getBranch: vi
    ...
    createAppointmentChain: vi
      .fn()
      .mockImplementation(async (inputs: { startAt: string; endAt: string }[]) =>
    ...
    transitionAppointmentStatus: vi
      .fn()
      .mockImplementation(async (input: { appointmentId: string; toStatus: string }) => ({
  ...
  const bookingScheduling = {
    resolveCanonicalInPersonContext: vi
      .fn()
      .mockResolvedValue({
        organizationId: ORG_ID,
        branchId: BRANCH_ID,
...
describe('createBookingOnCanonicalEngine', () => {
  it('creates a canonical appointment without writing a legacy patient_bookings link', async () => {
    ...
    expect(input.bookingScheduling.resolveCanonicalInPersonContext).toHaveBeenCalledWith({
      organizationId: ORG_ID,
      branchId: BRANCH_ID,
      serviceId: SERVICE_ID,
    });
    expect(input.bookingsPort.createPending).toHaveBeenCalledWith(
      expect.objectContaining({ branchId: null, serviceId: null, branchServiceId: null }),
    );
    expect(input.bookingEngine.createAppointment).toHaveBeenCalledWith(
      expect.objectContaining({ branchId: BRANCH_ID, serviceId: SERVICE_ID }),
    );
  });
  ...
  it('F2: in-person create rejected when resolved context has no specialist (not inserted)', async () => {
    const input = deps();
    (
      input.bookingScheduling.resolveCanonicalInPersonContext as ReturnType<typeof vi.fn>
    ).mockResolvedValue({
      organizationId: ORG_ID,
      branchId: BRANCH_ID,
      serviceId: SERVICE_ID,
    ...
    await expect(createBookingOnCanonicalEngine(input as never, inPersonInput)).rejects.toThrow(
      'specialist_required',
    );
  ...
  it('rejects invalid form answers', async () => {
    const input = deps();
    (input.bookingForm!.validateAnswers as ReturnType<typeof vi.fn>).mockResolvedValue({
      ok: false,
      error: 'required_field_missing',
    });
    await expect(
      createBookingOnCanonicalEngine(input as never, onlineInput, [
  ...
  it('enforces the organization consecutive-slot cap instead of a hard-coded duration', async () => {
    const input = deps();
    (
      input.bookingScheduling.getMaxConsecutiveSlotHours as ReturnType<typeof vi.fn>
    ).mockResolvedValue(1);
    await expect(
      createBookingOnCanonicalEngine(input as never, {
        ...onlineInput,
  ...
  it('creates every chain row through the atomic chain port and rolls pending mirrors back on failure', async () => {
    const input = deps();
    (input.bookingsPort.createPending as ReturnType<typeof vi.fn>).mockImplementation(
      async (row: { slotStart: string }) =>
        pendingRecord({ id: `pb-${row.slotStart.slice(11, 13)}` }),
    );
    (
      input.bookingEngine.createOnlineAppointmentsIfAvailable as ReturnType<typeof vi.fn>
    ).mockRejectedValue(new Error('slot_overlap'));
  ...
  it('reserves one membership visit for every appointment in a chain', async () => {
    ...
    (input.bookingsPort.markConfirmed as ReturnType<typeof vi.fn>).mockImplementation(
      async (id: string, _sourceId: unknown, options: { canonicalAppointmentId?: string }) =>
    ...
    await createBookingOnCanonicalEngine({ ...input, memberships } as never, {
      ...inPersonInput,
      slotEnd: '2026-06-01T12:00:00.000Z',
      slotCount: 2,
  ...
  it('auto FEFO reserves package on in-person create when no explicit package id', async () => {
    ...
    expect(memberships.pickAutoPackageForBooking).toHaveBeenCalledWith(
      'user-1',
      ORG_ID,
      SERVICE_ID,
    );
  ...
  it('canonical create cancels appointment when package reserve fails', async () => {
    ...
    await expect(
      createBookingOnCanonicalEngine({ ...input, memberships } as never, {
        ...inPersonInput,
        patientPackageId: 'pkg-1',
      }),
  ...
  it('canonical create cancels appointment when product consume fails', async () => {
    ...
    await expect(
      createBookingOnCanonicalEngine({ ...input, products } as never, {
        ...inPersonInput,
  ...
  it('canonical create cancels orphan appointment when markConfirmed fails', async () => {
    ...
    await expect(createBookingOnCanonicalEngine(input as never, onlineInput)).rejects.toThrow(
  ...
  it('canonical create creates the native appointment before awaiting payment', async () => {
    const input = deps();
    const payments = {
      resolvePrepayment: vi
        .fn()
        .mockResolvedValue({ required: true, amountMinor: 150000, currency: 'RUB' }),
  ...
  it('does not upsert booking contacts equal to identity', async () => {
    const input = deps();
    const upsert = vi.fn();
    const getPlatformUserIdentityContacts = vi
      .fn()
      .mockResolvedValue({ phone: '+79990001122', email: 'identity@example.com' });
  ...
  it('upserts booking contacts that differ from identity', async () => {
    const input = deps();
    const upsert = vi.fn();
    await createBookingOnCanonicalEngine({ ...input, platformUserContacts: { upsert } } as never, {
      ...onlineInput,
