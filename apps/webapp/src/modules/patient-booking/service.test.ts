...
describe('createPatientBookingService', () => {
  ...
  it('cancelBooking: cancels only the selected chain appointment', async () => {
    ...
    syncPort.emitBookingEvent.mockResolvedValue(undefined);
    const appointmentLifecycle = {
      previewPatientCancel: vi
        .fn()
        .mockResolvedValue({ ok: true, allowed: true, requiresStaffConfirmation: false }),
      patientCancel: vi.fn().mockResolvedValue({
        ok: true,
    ...
    await expect(
      svc.cancelBooking({ userId: row.userId!, bookingId: row.id }),
  ...
  it('rescheduleBooking: complete canonical in-person row uses canonical availability without legacy branch-service resolution', async () => {
    ...
    const event = syncPort.emitBookingEvent.mock.calls[0]?.[0] as {
  ...
  it('rescheduleBooking: canonical path no longer calls the retired mirror', async () => {
    const row = sampleRow({
      status: 'confirmed',
      canonicalAppointmentId: 'appt-1',
    });
  ...
  it('cancelBooking: canonical path no longer calls the retired mirror', async () => {
    const row = sampleRow({
      status: 'confirmed',
      canonicalAppointmentId: 'appt-1',
    });
