...
export type BookingSyncPort = {
  emitBookingEvent(input: {
    ...
    /**
     * Владелец 19.08: «Про событие записи вижу явный косяк — интегратор тут вообще ни при чем.
     * Запись делает вебапп. Напоминания и отправку уведомлений — интегратор с шедулером.»
     *
     * По умолчанию событие уходит ПОСЛЕ ответа человеку: он ждал 3,4 с на каждом создании и каждой
     * отмене, и ждал он лестницу пауз повторов, чей результат вызывающий всё равно выбрасывал.
     *
     * `true` ставят ровно те вызывающие, кто отказ ПОТРЕБЛЯЕТ: маршрут смены пресета напоминаний
     * (человек видит 503 `schedule_sync_failed`) и обработчик подтверждённой оплаты (по броску
     * платёжный провайдер повторяет вебхук). У них поведение остаётся прежним бит в бит.
     */
