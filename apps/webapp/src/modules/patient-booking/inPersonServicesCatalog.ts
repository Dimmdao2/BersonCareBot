 ...
 *
 * Отбора здесь нет: «активен» решает дверь каталога в SQL, и второй копии этого правила в
 * приложении быть не должно. `null` — одинаковый отказ на все три причины (нет такого, чужой,
 * неактивен), из которого экран делает «этот специалист больше не принимает записи» (план §6.3).
 * Публичность его карточки на запись не влияет (§17.Q) — она решает только, читается ли описание.
 */
