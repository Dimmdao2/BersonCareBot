...
describe('reminderScheduleEvaluationInstant', () => {
  it('returns now when mute is absent or expired', () => {
    ...
    expect(reminderScheduleEvaluationInstant(now, '2026-05-09T11:00:00.000Z').getTime()).toBe(
      now.getTime(),
    );
  });
  ...
  it('returns muted-until when it is after now', () => {
    ...
    expect(reminderScheduleEvaluationInstant(now, until.toISOString()).getTime()).toBe(
      until.getTime(),
    );
  });
});
...
describe('countPlannedHomeReminderOccurrencesInUtcRange', () => {
  ...
  it('countPlannedHomeLinkedReminderOccurrencesWithPredicate sums only matching rules', () => {
    ...
    expect(countPlannedHomeReminderOccurrencesInUtcRange([warm, rehab], rangeStart, rangeEnd)).toBe(
      3,
    );
    expect(
      countPlannedHomeLinkedReminderOccurrencesWithPredicate(
