...
function item(
  p: Partial<PatientHomeBlockItem> & Pick<PatientHomeBlockItem, 'id' | 'targetType' | 'targetRef'>,
...
function homeBlock(
  p: Partial<PatientHomeBlock> & Pick<PatientHomeBlock, 'code' | 'items'>,
): PatientHomeBlock {
  return {
    title: '',
    description: '',
...
describe('patientHomeResolvers', () => {
  ...
  it('getSubscriptionCarouselSectionPresentation uses custom badgeLabel', () => {
    ...
    expect(getSubscriptionCarouselSectionPresentation(blocks, 'fixture-beta')?.badgeLabel).toBe(
      'Клуб',
    );
  });
