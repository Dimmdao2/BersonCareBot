...
function item(
  p: Partial<PatientHomeBlockItem> & Pick<PatientHomeBlockItem, 'targetType' | 'targetRef'>,
...
describe('patientHomeBlockItemDisplayTitle', () => {
  it('prefers titleOverride', () => {
    ...
    expect(
      patientHomeBlockItemDisplayTitle(
        item({ targetType: 'content_page', targetRef: 'a', titleOverride: '  X  ' }),
        titles,
      ),
    ).toBe('X');
  });
  ...
  it('resolves content_page slug to page title', () => {
    ...
    expect(
      patientHomeBlockItemDisplayTitle(
        item({ targetType: 'content_page', targetRef: 'warm', titleOverride: null }),
        titles,
      ),
    ).toBe('Разминка утром');
  });
  ...
  it('resolves content_section slug to section title', () => {
    ...
    expect(
      patientHomeBlockItemDisplayTitle(
        item({ targetType: 'content_section', targetRef: 'pain', titleOverride: null }),
        titles,
      ),
    ).toBe('Боль в спине');
  });
  ...
  it('resolves course id to title', () => {
    ...
    expect(
      patientHomeBlockItemDisplayTitle(
        item({ targetType: 'course', targetRef: 'c1', titleOverride: null }),
        titles,
      ),
    ).toBe('Курс ЛФК');
  });
  ...
  it('falls back to ref when missing in maps', () => {
    expect(
      patientHomeBlockItemDisplayTitle(
        item({ targetType: 'content_page', targetRef: 'ghost', titleOverride: null }),
        emptyPatientHomeRefDisplayTitles,
      ),
    ).toBe('ghost');
  });
