...
export type PatientHomeLeadingIconBlockCode =
  (typeof PATIENT_HOME_LEADING_ICON_BLOCK_CODES)[number];
...
export function allowedTargetTypesForBlock(
  code: PatientHomeBlockCode,
): readonly PatientHomeBlockItemTargetType[] {
...
export function isPatientHomeContentPageCandidateForBlock(
  ...
  if (blockCode === 'useful_post') {
    ...
    if (sec.kind === 'system') {
      if (
        sec.systemParentCode != null &&
        USEFUL_POST_EXCLUDED_SYSTEM_PARENTS.has(sec.systemParentCode)
      ) {
        return false;
      }
      return true;
