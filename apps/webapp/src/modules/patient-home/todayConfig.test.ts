...
describe('getPatientHomeTodayConfig', () => {
  ...
  it('guest tier returns first warmup without calling getLatestCompleted', async () => {
    const getLatestCompletedContentPageId = vi.fn(
      async () => 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb',
    );
  ...
  it('patient tier shows next after last completed on home; push targets page after home pick', async () => {
    ...
    const buildSync = (
      ...
      port = createInMemoryPatientDailyWarmupPresentationPort(),
    ) => {
    ...
    const afterPresented = await getPatientHomeTodayConfig(
      deps,
