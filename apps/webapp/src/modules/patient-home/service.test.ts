...
describe('patient-home service', () => {
  ...
  it('requires complete known list for reorder blocks', async () => {
    const { service } = makeService();
    await expect(service.reorderBlocks(['daily_warmup', 'booking'])).rejects.toThrow(
      'invalid_block_count',
    );
  });
  ...
  it('setBlockIcon updates supported blocks and clears with null', async () => {
    ...
    expect(blocks.find((b) => b.code === 'booking')?.iconImageUrl).toBe(
  ...
  it('setBlockIcon rejects unsupported blocks', async () => {
    const { service } = makeService();
    await expect(service.setBlockIcon('daily_warmup', 'https://x')).rejects.toThrow(
      'block_icon_not_supported',
    );
    await expect(service.setBlockIcon('useful_post', 'https://x')).rejects.toThrow(
      'block_icon_not_supported',
    );
  });
  ...
  it('retarget rejects invalid target type for block', async () => {
    ...
    await expect(
      service.updateItem(id, { targetType: 'content_section', targetRef: 's-1' }),
    ).rejects.toThrow('invalid_target_type_for_block');
  });
  ...
  it('retarget rejects missing CMS page', async () => {
    ...
    await expect(service.updateItem(id, { targetRef: 'nope' })).rejects.toThrow(
      'target_content_page_not_found',
    );
  });
  ...
  it('retarget rejects invalid course id format', async () => {
    ...
    await expect(service.updateItem(id, { targetRef: 'not-a-uuid' })).rejects.toThrow(
      'invalid_course_id',
    );
  });
});
