...
- `todayConfig.ts` — `listDailyWarmupPagesForHome(deps)` (упорядоченный список опубликованных `content_page` блока `daily_warmup`), `buildPatientDailyWarmupNav(slug, pages)` для pager на `/app/patient/content/[slug]?from=daily_warmup`; `getPatientHomeTodayConfig(deps, weekdayIndex, warmupPick?)`: ротация «разминки дня» из того же списка + `patient_home_daily_practice_target` (1–10, default 3). `warmupPick`: `cooldownMinutes` / `skipCooldownPages` как раньше. Экран разминки: `PatientDailyWarmupPager` («Разминка дня n/N», циклические prev/next только по блоку главной, не по всему CMS-разделу). **Backlog:** отдельная настройка «K разминок в день» / регионы — не `patient_home_daily_practice_target`.
...
- `nextReminderOccurrence.ts` — ближайшее срабатывание по `interval_window` или `slots_v1`, типы связей включая `rehab_program`; счётчик запланированных слотов за календарный день приложения (`countPlannedHomeReminderOccurrencesInUtcRange`) для блока «Следующее напоминание» и **n/N** на главной (см. [`docs/archive/2026-05-initiatives/PATIENT_REMINDER_UX_INITIATIVE/README.md`](../../../../../docs/archive/2026-05-initiatives/PATIENT_REMINDER_UX_INITIATIVE/README.md)). Для карточки прогресса персонального tier знаменатель «цели на сегодня» совпадает с этим же `plannedTotal`, только если есть **включённое** напоминание на раздел разминок в CMS (`content_section`, slug после `resolvePatientContentSectionSlug` совпадает с каноническим разделом разминок); иначе используется **`patient_home_daily_practice_target`** из `todayConfig`. При паузе push (`reminder_muted_until`) счётчик плана на день обнуляется, цель прогресса — 0, микротекст разбивки не показывается.
...
- Материал разминки дня: [`content/[slug]?from=daily_warmup`](../../app/app/patient/content/[slug]/page.tsx) — back «Меню» на главную, без «Назад к разделу»; CTA с главной и из напоминания (`/app/patient/go/daily-warmup` → тот же URL).
