...
describe('patient-home CMS taxonomy helpers', () => {
  ...
  it('filters daily_warmup pages to warmups cluster', () => {
    ...
    expect(isPatientHomeContentPageCandidateForBlock('daily_warmup', pageOk, sectionMap)).toBe(
      true,
    );
    expect(
      isPatientHomeContentPageCandidateForBlock(
        'daily_warmup',
        { ...pageOk, section: 'articles' },
        sectionMap,
      ),
    ).toBe(false);
  });
  ...
  it('filters subscription_carousel section and page candidates to article taxonomy', () => {
    ...
    const articleOnlyMap = new Map([
      ['articles', { kind: 'article' as const, systemParentCode: null }],
    ]);
    expect(
      isPatientHomeContentPageCandidateForBlock('subscription_carousel', pageOk, articleOnlyMap),
    ).toBe(true);
    expect(
      isPatientHomeContentPageCandidateForBlock(
