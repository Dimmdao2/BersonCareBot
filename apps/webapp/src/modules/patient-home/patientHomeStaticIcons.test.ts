...
describe('patientHomeStaticIcons', () => {
  it('maps known block codes to public PNG paths', () => {
    ...
    expect(PATIENT_HOME_BLOCK_STATIC_ICON_URL.next_reminder).toBe(
      '/patient/home/icons/next-reminder.png',
    );
  ...
  it('prefers bundled block icon over CMS url', () => {
    expect(
      resolvePatientHomeBlockLeadingIconUrl(
        'booking',
        '/api/media/bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb',
      ),
    ).toBe('/patient/home/icons/booking.png');
  });
  ...
  it('falls back to CMS url when block has no bundled icon', () => {
    ...
    ).toBe('/api/media/cccccccc-cccc-4ccc-8ccc-cccccccccccc');
