...
export type CalendarAppointmentEvent = {
  ...
  /**
   * «Человек пришёл впервые»: у этого пациента в организации НЕТ более ранней неотменённой
   * записи, чем эта. Признак самой записи, а не периода — владелец 15.09: «первичные — это
   * человек впервые пришёл, а не первая за период».
   *
   * Живёт здесь, а не только в КПИ, ровно поэтому. КПИ отдают `firstVisitIds` — список за окно
   * `visibleRange`, и фильтр по нему работал только в календарной сетке: в ленте, которая тянет
   * историю месяцами, записи вне окна в список не попадали, и отбор резал ленту до якорного
   * периода вместо отбора первичных. Критерий здесь тот же, что в `getScheduleKpis`: строгий
   * порядок по `(start_at, id)`, отменённые не в счёт.
   *
   * Поле необязательное: его проставляет только календарный репозиторий, из которого идут и
   * сетка, и лента. Отсутствие читается как «не первое посещение» — фильтр по нему тогда просто
   * ничего не отберёт, а не покажет лишнее.
   */
