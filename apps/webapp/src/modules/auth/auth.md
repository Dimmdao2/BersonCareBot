...
- Подсказки ссылок на ботов при `access_denied` (**`loadMiniappAuthHelpLinks`**) запрашивают публичные конфиги с заголовком **`x-bc-auth-correlation-id`**, как остальные публичные auth GET в этом потоке.
...
- Prefetch `GET /api/auth/oauth/providers`, `GET /api/auth/telegram-login/config`, `GET /api/auth/login/alternatives-config` в **`AuthBootstrap`** привязан к тому же **поколению** bootstrap (`authEpochRef`), что и опрос initData: при retry / смене query / unmount устаревший ответ не вызывает `setPrefetchedAuth`.
...
Наблюдаемость и поля успеха/отказа для M2M (`ok === true` в JSON при 200/202), а также логи TX-привязки телефона в интеграторе — в `apps/webapp/INTEGRATOR_CONTRACT.md` (раздел Contract Principles → Observability) и `docs/WEBAPP_FIRST_PHONE_BIND/STAGE_05_OBSERVABILITY_TESTS_DOCS.md`.
