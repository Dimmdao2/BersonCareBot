...
| GET   | `/api/auth/login/alternatives-config` | Публичные альтернативы входа: Max URL, VK URL, SMS fallback, public channel policy и rollout specialist signup; без секретов. Это HTTP-проекция серверного `buildPrefetchedPublicAuthConfig` для внешних клиентов. |
| GET   | `/api/auth/oauth/providers`       | Флаги `yandex` / `google` / `vk` / `apple`, **без** секретов; `Cache-Control: private, no-store`. Это HTTP-проекция серверного `buildPrefetchedPublicAuthConfig` для внешних клиентов. |
...
- Единый серверный снимок `buildPrefetchedPublicAuthConfig` собирает только публичные флаги провайдеров, поля альтернатив входа и rollout-флаг **`specialist_signup_enabled`**. Он передаётся в **`AuthBootstrap`/`AuthFlowV2`** как **`prefetchedAuthConfig`** / **`initialPublicAuthConfig`** через RSC props, поэтому экран `/app` не делает client GET; `login/alternatives-config` и `oauth/providers` — его HTTP-проекции для внешних клиентов. Секреты в тип снимка не входят.
...
`/api/auth/exchange`, `/api/auth/telegram-init`, **`/api/auth/max-init`**, `/api/auth/telegram-login/config`, `/api/auth/check-phone`, `/api/auth/phone/start`, `/api/auth/phone/confirm`, **`/api/auth/phone/messenger-bind/start`**, **`/api/auth/phone/messenger-bind/status`**, **`/api/auth/phone/messenger-bind/finish`**, **`/api/integrator/phone-messenger-bind/complete`**, `/api/auth/channel-link/start`, `/api/auth/login/alternatives-config`, `/api/auth/oauth/start`, `/api/auth/oauth/providers`, `/api/auth/oauth/callback`, `/api/auth/oauth/callback/yandex`, `/api/auth/oauth/callback/google`, `/api/auth/oauth/callback/apple`, `/api/auth/logout` (POST). Пациентский контур: `POST /api/patient/support` (см. выше).
