...
- **`POST /api/auth/phone/messenger-bind/finish`** — `{ setupToken, browserCalendarIana? }`. Только **`login`**, после `otp_ready`: server-side `confirmPhoneAuth` по challenge из secret (поле **`code` в body не принимается**, schema strict) → cookie сессии → `{ ok, redirectTo, role }`. **`already_consumed`** + активная сессия → **200** idempotent redirect; без сессии → **409**. Ошибки resolve: **404** `not_found`, **409** `not_ready` \| `challenge_expired`, **400** прочие коды.
- **`POST /api/integrator/phone-messenger-bind/complete`** (M2M, подпись как channel-link) — контакт из бота. Перед выдачей OTP/consume webapp пробует auto-merge через общий merge-engine: классический кейс «Telegram/Max binding на onboarding/contact-only строке, телефон на другой карточке» проходит без ручного вмешательства; `profile_bind` также сначала пробует merge с владельцем телефона. Hard blockers возвращаются как **409** `{ error: "conflict", mergeReason }` и пишутся в audit. Ответ **200** `{ ok: true, purpose, … }`:
  - **`purpose: login`** — OTP-challenge, secret → `otp_ready`; integrator **`user.phone.link` до success**, затем `*:phoneAuthReturnToApp` + главное меню (без `{{code}}` в TG/Max); PWA — **`messenger-bind/finish`**; **replay** `otp_ready` → меню без повторного текста с кодом.
  - **`purpose: profile_bind`** — OTP не создаётся, secret → `consumed`; integrator `user.phone.link` + `*:phoneAuthPhoneLinked` + главное меню; **`otpCode` нет**.
  - **`used_token`** / secret уже `consumed` → **200** `{ status: "already_used" }`.
- **`POST /api/auth/phone/confirm`** — **`login`** через `phone/start` (уже привязанный мессенджер) и legacy; messenger-bind **`login`** → **`messenger-bind/finish`**. Verify + bind → `markPhoneMessengerBindConsumedByChallenge` → при ошибке post-steps **`server_error`** → `consumePhoneOtpChallenge` при полном успехе.

Клиент: `PhoneMessengerAuthFlow` (`purpose: login` — poll `otp_ready` → finish; `profile_bind` на **`/app/patient/bind-phone`** — poll до `consumed`, без формы кода). Mini App на bind-phone — по-прежнему `PatientBindPhoneClient` (request-contact). Открытие deep link — `finishChannelLinkNavigation` (как channel-link). Логи: `phone_messenger_bind_start`, `phone_messenger_bind_complete_ok|fail` (без `otpCode`). Runbook: `docs/OPERATIONS/PHONE_MESSENGER_AUTH_RUNBOOK.md`. Планы A/B: `.cursor/plans/phone_messenger_bind_pwa_autologin.plan.md`, `.cursor/plans/phone_messenger_bind_bot_ux.plan.md` (`status: completed`); ручной smoke — `docs/LOGIN_REGISTER_NEW_LOGIC/LOG.md` §«Приёмка A+B».
