...
| GET   | `/api/auth/oauth/providers`       | Флаги `yandex` / `google` и постоянный `apple: false`, **без** секретов; `Cache-Control: private, no-store`. Серверный `buildPrefetchedPublicAuthConfig` возвращает тот же `apple: false` для SSR.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
- **`POST /api/auth/phone/messenger-bind/finish`** — legacy/server-side finish для старого PWA-потока; основной login flow больше его не вызывает. Новый flow после `otp_ready` показывает форму кода и подтверждает через **`POST /api/auth/phone/confirm`**.
- **`POST /api/integrator/phone-messenger-bind/complete`** (M2M, подпись как channel-link) — контакт из бота. Перед выдачей OTP/consume webapp пробует auto-merge через общий merge-engine: классический кейс «Telegram/Max binding на onboarding/contact-only строке, телефон на другой карточке» проходит без ручного вмешательства; `profile_bind` также сначала пробует merge с владельцем телефона. Hard blockers возвращаются как **409** `{ error: "conflict", mergeReason }` и пишутся в audit. Ответ **200** `{ ok: true, purpose, … }`:
  - **`purpose: login`** — OTP-challenge, secret → `otp_ready`; webapp уже применил канонический write (`applyMessengerContactPreOtp`) в том же запросе `.../complete` — `result.ok === true` и есть сам write, integrator ничего досинхронизировать не должен (`user.phone.link` выведен из рантайма 2026-08-26); затем `*:phoneAuthAccountCreated` или `*:phoneAuthLoginCode` с `{{code}}` + главное меню; PWA показывает форму кода и завершает вход через **`phone/confirm`**; **replay** `otp_ready` → меню без повторного текста с кодом.
  - **`purpose: profile_bind`** — OTP не создаётся, secret → `consumed`; та же логика — `*:phoneAuthPhoneLinked` + главное меню без отдельного integrator-write; **`otpCode` нет**.
...
Только для **внешнего** M2M-клиента (другой сервис, админка): та же каноническая транзакция привязки `public.platform_users` / `public.user_channel_bindings`, что webapp применяет напрямую в hot-пути (`applyMessengerContactPreOtp` в `modules/auth/phoneMessengerBind.ts` — параллельный write `user.phone.link` в integrator выведен из рантайма 2026-08-26), подпись **`x-bersoncare-timestamp` / `x-bersoncare-signature`**, обязательный **`x-bersoncare-idempotency-key`**. Семантический хеш для кеша успешного ответа — поля **`channelCode`**, **`externalId`**, **`phoneNormalized`** (`apps/webapp/src/infra/idempotency/messengerPhoneBindRequestHash.ts`). При **одной БД** сценарии бота **не** вызывают этот URL — привязка идёт через прямой канонический write webapp внутри `.../complete`. Контракт и коды ответов: `apps/webapp/INTEGRATOR_CONTRACT.md`, этап: `docs/archive/2026-04-initiatives/WEBAPP_FIRST_PHONE_BIND/STAGE_06_OPTIONAL_HTTP_BIND_ROUTE.md`.
