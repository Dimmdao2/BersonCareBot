...
const client = toClientEnvironmentTelemetry(
  parseSupportedClientEnvironment(
...
describe('unsupported client watchdog', () => {
  ...
  it('keeps the SSR fallback hidden until zero-module timeout, then reports a bounded category', () => {
    ...
    const body = JSON.parse(FakeXhr.bodies[0] ?? '{}') as {
      failureSignals?: Record<string, unknown>;
    };
  ...
  it.each([
    [
      ...
      () =>
        window.dispatchEvent(
          new ErrorEvent('error', { message: 'SyntaxError', error: new SyntaxError('synthetic') }),
        ),
    ],
    [
      'runtime_error',
      () =>
        window.dispatchEvent(
          new ErrorEvent('error', { message: 'synthetic', error: new Error('synthetic') }),
        ),
    ],
    [
      'script_load_error',
    ...
    'reports only the bounded %s captured-error category',
    (capturedError, dispatchFailure) => {
      evaluateWatchdog();
      dispatchFailure();
      vi.advanceTimersByTime(1_000);
      const body = JSON.parse(FakeXhr.bodies[0] ?? '{}') as {
  ...
  it('keeps failed service-worker/storage probes bounded to categories', async () => {
    Object.defineProperty(navigator, 'serviceWorker', {
      ...
      value: {
        controller: null,
        getRegistration: vi.fn().mockRejectedValue(new Error('synthetic')),
      },
    });
    Object.defineProperty(navigator, 'storage', {
      configurable: true,
