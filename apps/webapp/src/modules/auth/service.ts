...
async function persistNewAuthSession(
  ...
  // passkey, мессенджеры, приглашения, второй фактор) приходят сюда. Развесить запись по
  // маршрутам значило бы завести двадцать мест, где о ней можно забыть.
  //
  // Импорт динамический сознательно: статический притянул бы сюда операторский канал доставки, а
  // через него — `buildAppDeps`, то есть ровно тот цикл, о котором предупреждает комментарий выше.
  // Запись журнала не роняет вход: помощник сам ловит свой отказ и уводит его в лог.
  //
  // Не-канонический идентификатор сюда доходит (операторские и служебные сессии), и в `actor_id`
