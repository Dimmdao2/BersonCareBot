...
describe('confirmEmailChallenge (in-memory)', () => {
  ...
  it('возвращает email_conflict если email уже занят другим пользователем', async () => {
    ...
    const conflict = await confirmEmailChallenge(
      otherId,
      startOther.challengeId,
      otherCode,
      'email_verify',
    );
...
describe('consumeLatestEmailChallengeCodeForUser', () => {
  ...
  it('принимает код без challengeId (in-memory челлендж)', async () => {
    ...
    const consumed = await consumeLatestEmailChallengeCodeForUser(
      uid,
      sentCode as string,
      'email_verify',
    );
...
describe('purpose binding (C-2 step 4, OWASP ASVS V6.6.2 / NIST SP 800-63B §5.1.3, in-memory)', () => {
  ...
  it('rejects a code minted for one purpose (e.g. password_reset) when confirmed against a different expected purpose, with the exact same shape as a wrong code', async () => {
    ...
    const rightPurpose = await confirmEmailChallenge(
      uid,
      start.challengeId,
      code,
      'password_reset',
  ...
  it('rejects a cross-purpose code via consumeLatestEmailChallengeCodeForUser too', async () => {
    ...
    const start = await startEmailChallenge(
      uid,
      'cross-purpose-consume@example.org',
      'specialist_signup',
    );
    expect(start.ok).toBe(true);
    const code = sendEmailCodeMock.mock.calls[0]?.[1] as string;

    const wrongPurpose = await consumeLatestEmailChallengeCodeForUser(
      uid,
      code,
