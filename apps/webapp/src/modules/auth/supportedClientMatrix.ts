...
export function toClientEnvironmentTelemetry(
  input: ParsedClientEnvironment,
): ClientEnvironmentTelemetry {
  return {
...
export function formatClientEnvironmentFact(input: ParsedClientEnvironment): string | null {
  ...
  const parts = [
    input.deviceName,
    input.osName && input.osVersion ? `${input.osName} ${input.osVersion}` : input.osName,
    input.browserName,
