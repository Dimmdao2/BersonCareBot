...
import { getRedirectPathForRole, isSafeNext, getPostAuthRedirectTarget } from './redirectPolicy';
...
describe('redirectPolicy', () => {
  ...
  describe('getPostAuthRedirectTarget', () => {
    it('returns safe next only for client', () => {
      expect(getPostAuthRedirectTarget('client', '/app/patient/cabinet')).toBe(
        '/app/patient/cabinet',
      );
    ...
    it('returns role path when next is unsafe', () => {
      ...
      expect(getPostAuthRedirectTarget('doctor', null, '/app/account?tab=profile')).toBe(
        '/app/doctor',
      );
    });
  });
