...
describe('pickPrimaryOtpChannelPublic', () => {
  ...
  it('prefers telegram over sms', () => {
    expect(pickPrimaryOtpChannelPublic({ sms: true, telegram: true })).toBe('telegram');
  });
});
...
describe('pickOtpChannelWithPreferencePublic', () => {
  it('uses email preference when available', () => {
    expect(
      pickOtpChannelWithPreferencePublic({ sms: true, email: true, telegram: true }, 'email'),
    ).toBe('email');
  });

  it('ignores sms preference so auto-start never uses SMS', () => {
    expect(pickOtpChannelWithPreferencePublic({ sms: true, telegram: true }, 'sms')).toBe(
      'telegram',
    );
  });

  it('uses non-email preference when available', () => {
    expect(pickOtpChannelWithPreferencePublic({ sms: true, max: true }, 'max')).toBe('max');
  });
});
