...
describe("buildPrefetchedPublicAuthConfig", () => {
  beforeEach(() => {
    ...
    getPublicRuntimeBoolMock.mockImplementation(
      async (key: string) => key === "auth_oauth_google_enabled" || key === "auth_oauth_yandex_enabled",
    );
  ...
  it("uses only derived provider availability and includes the public alternatives snapshot", async () => {
    ...
    expect(toggleKeys).toEqual(["auth_oauth_google_enabled", "auth_oauth_yandex_enabled", "oauth_apple_enabled"]);
