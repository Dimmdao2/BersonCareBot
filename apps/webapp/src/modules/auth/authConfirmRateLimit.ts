/**
 * Single chokepoint for the per-IP rate limit shared by confirm-shaped and password-proof routes.
 * Every route calls `ensureAuthModulePortsBound()` before `checkAuthConfirmRateLimit`, then maps the
 * result onto its OWN existing failure shape (ASVS 6.3.8: a rate-limited response must not differ
