...
import { assertSessionRevocationSchema, SessionRevocationSchemaError } from "./sessionRevocationSchema";
...
describe("assertSessionRevocationSchema", () => {
  it("refuses to start when the database answers and the column is missing", async () => {
    await expect(assertSessionRevocationSchema(async () => false, () => {})).rejects.toBeInstanceOf(
