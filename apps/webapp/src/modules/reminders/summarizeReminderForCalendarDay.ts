 ...
 */
export function isSlotsV1DayActive(
...
export function summarizeReminderForCalendarDay(
  ...
  const noonPatient = DateTime.fromISO(`${calendarDateKey}T12:00:00`, {
    zone: patientCalendarDayIana,
  });
  if (!noonPatient.isValid) return 'См. расписание';
  const dayInRuleTz = noonPatient.setZone(ruleTz);

  if (
    rule.scheduleType === 'slots_v1' &&
    rule.scheduleData &&
    Array.isArray(rule.scheduleData.timesLocal)
  ) {
...
export function formatPlanReminderTodayLine(
  ...
  remainingLabels.sort(
    (a, b) => (parseHhMmToMinuteOfDay(a) ?? 0) - (parseHhMmToMinuteOfDay(b) ?? 0),
  );
  if (remainingLabels.length > 0) {
    return `Сегодня еще ${remainingLabels.length}: в ${remainingLabels.join(', ')}`;
  }
