 ...
 */
import { enqueueCurrentReminderRulePushDefault } from '@/infra/integrator-push/integratorPushOutbox';
...
export async function notifyIntegratorRuleUpdated(rule: ReminderRule): Promise<void> {
  ...
  try {
    ...
    if (msg === 'integrator_m2m_unconfigured') {
      console.warn(
        '[reminders] notifyIntegrator: INTEGRATOR_API_URL or secret not configured, skipping',
      );
      return;
    }
    try {
      await enqueueCurrentReminderRulePushDefault(rule.id);
    } catch (enqueueErr) {
