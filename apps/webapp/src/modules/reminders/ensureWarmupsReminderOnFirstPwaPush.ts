...
export function isPwaPushPlatform(
  platform: string | undefined | null,
): platform is PwaPushPlatform {
...
export async function ensureWarmupsReminderOnFirstPwaPush(params: {
  ...
  const hasWarmupRule = rules.some((rule) =>
    isWarmupsContentSectionReminderRule(rule, warmupsSectionSlug),
  );
  if (hasWarmupRule) {
