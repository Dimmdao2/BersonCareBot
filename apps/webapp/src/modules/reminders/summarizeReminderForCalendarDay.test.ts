...
describe('summarizeReminderForCalendarDay', () => {
  ...
  it('slots_v1 weekdays: Sunday says no reminders today', () => {
    ...
    expect(summarizeReminderForCalendarDay(r, '2026-05-10', 'Europe/Moscow')).toBe(
      'Сегодня без напоминаний',
    );
  });
  ...
  it('interval_window: inactive day', () => {
    ...
    expect(summarizeReminderForCalendarDay(r, '2026-05-10', 'Europe/Moscow')).toBe(
      'Сегодня выходной по расписанию',
    );
  });
  ...
  it('slots_v1 weekly_mask: respects mask for weekday', () => {
    ...
    expect(summarizeReminderForCalendarDay(r, tue, 'Europe/Moscow')).toBe(
      'Сегодня без напоминаний',
    );
...
describe('formatPlanReminderTodayLine', () => {
  it('returns не настроено for null rule', () => {
    expect(
      formatPlanReminderTodayLine(
        ...
        '2026-05-11',
        'Europe/Moscow',
        new Date('2026-05-11T10:00:00+03:00'),
      ),
    ).toBe('не настроено');
  });
  ...
  it('slots_v1: template with count of remaining times', () => {
    ...
    expect(formatPlanReminderTodayLine(r, '2026-05-11', 'Europe/Moscow', now)).toBe(
      'Сегодня еще 1: в 18:00',
    );
  });
...
describe('formatExercisesTodayTrainingStatus', () => {
  ...
  it('maps remaining slots to еще n в …', () => {
    ...
    const now = DateTime.fromISO('2026-05-11T10:00:00', { zone: 'Europe/Moscow' }).toJSDate();
    expect(formatExercisesTodayTrainingStatus(r, '2026-05-11', 'Europe/Moscow', now)).toBe(
      'еще 1 в 18:00',
    );
  });
