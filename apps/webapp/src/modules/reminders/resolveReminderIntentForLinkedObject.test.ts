...
describe('resolveReminderIntentForLinkedObject', () => {
  it('returns warmup for canonical warmups section slug without lookup', async () => {
    await expect(resolveReminderIntentForLinkedObject('content_section', 'warmups')).resolves.toBe(
      'warmup',
    );
  });
  ...
  it('returns exercises for rehab_program', async () => {
    await expect(resolveReminderIntentForLinkedObject('rehab_program', 'inst-1')).resolves.toBe(
      'exercises',
    );
  });

  it('returns generic for lfk_complex', async () => {
    await expect(resolveReminderIntentForLinkedObject('lfk_complex', 'cx-1')).resolves.toBe(
      'generic',
    );
  });
});
