...
describe('staff security keyring crypto', () => {
  it('writes with the active key and reads retained-key envelopes after rotation', () => {
    const beforeRotation = createStaffSecurityCrypto({
      activeKeyId: 'old',
      keys: { old: OLD_KEY },
    });
  ...
  it('fails closed for tampering, missing retained keys, and a wrong key', () => {
    ...
    expect(() => missing.decryptTotpSecret(envelope)).toThrow(
      'staff_security_read_key_missing:old',
    );
  ...
  it('rejects malformed configuration and does not fall back to the session cookie secret', () => {
    expect(() =>
      createStaffSecurityCrypto({ activeKeyId: 'missing', keys: { old: OLD_KEY } }),
    ).toThrow('staff_security_active_key_missing');
    ...
    expect(() => lazy.encryptTotpSecret('secret')).toThrow(
      'STAFF_SECURITY_KEYRING_JSON is required',
    );
  });
  ...
  it('binds login challenge hashes to retained key ids', () => {
    ...
    const rotated = createStaffSecurityCrypto({
      activeKeyId: 'new',
      keys: { old: OLD_KEY, new: NEW_KEY },
    });
