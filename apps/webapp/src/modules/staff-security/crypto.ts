...
function normalizeKeyring(config: StaffSecurityKeyringConfig): { activeKeyId: string; keys: ReadonlyMap<string, Buffer> } {
  if (!KEY_ID_RE.test(config.activeKeyId)) throw new Error("staff_security_active_key_id_invalid");
  const keys = new Map(Object.entries(config.keys).map(([keyId, encoded]) => [keyId, decodeKey(keyId, encoded)]));
...
function parseKeyringJson(raw: string | undefined): StaffSecurityKeyringConfig {
  if (!raw?.trim()) throw new Error("STAFF_SECURITY_KEYRING_JSON is required for staff security operations");
  let parsed: unknown;
  try {
  ...
  if (typeof record.activeKeyId !== "string" || !record.keys || typeof record.keys !== "object" || Array.isArray(record.keys)) {
...
export function createStaffSecurityCrypto(config: StaffSecurityKeyringConfig): StaffSecurityCryptoPort {
