...
describe('runtime config provider', () => {
  ...
  it('fails closed for missing or invalid values', async () => {
    ...
    await expect(
      provider.isFlagEnabled('patient_program_discussion_ui_enabled', {
        ...context,
        organizationId: '',
      }),
  ...
  it('clamps legacy cooldown values and uses the default for a missing row', async () => {
    const getEffective = vi
      .fn<RuntimeConfigPort['getEffective']>()
  ...
  it('preserves auth-channel rollout defaults when the public projection is absent', async () => {
    ...
    await expect(
      provider.getPublicBoolean('patient_unsupported_client_fallback_enabled'),
    ).resolves.toBe(false);
  });
  ...
  it('does not use an authorization fallback for strict server token lists', async () => {
    ...
    await expect(provider.getServerTokenListStrict('admin_emails')).resolves.toBe('');
    await expect(provider.getServerTokenListStrict('admin_emails')).rejects.toThrow(
      'permission denied',
    );
