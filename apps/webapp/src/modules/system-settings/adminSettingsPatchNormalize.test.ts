...
} from './adminSettingsPatchNormalize';
...
describe('normalizeModesFormPatchItem', () => {
  ...
  it('rejects patient_booking_url ftp', () => {
    expect(normalizeModesFormPatchItem('patient_booking_url', { value: 'ftp://x' })).toEqual({
      ok: false,
    });
  });

  it('rejects maintenance message over 500 chars', () => {
    const r = normalizeModesFormPatchItem('patient_app_maintenance_message', {
      value: 'x'.repeat(501),
    });
