...
describe('parseWebPushVapidPatchValue', () => {
  ...
  it('rejects empty public key', () => {
    expect(
      parseWebPushVapidPatchValue(
        { value: { publicKey: '', privateKey: validPriv } },
        { hasExistingPrivate: false },
      ),
    ).toEqual({
      ok: false,
    });
  ...
  it('rejects non-base64url public key', () => {
    expect(
      parseWebPushVapidPatchValue(
        { value: { publicKey: 'has space', privateKey: validPriv } },
        { hasExistingPrivate: false },
      ),
    ).toEqual({ ok: false });
  });
  ...
  it('rejects public key with valid alphabet but wrong decoded length', () => {
    ...
    expect(
      parseWebPushVapidPatchValue(
        { value: { publicKey: shortPub, privateKey: validPriv } },
        { hasExistingPrivate: false },
      ),
    ).toEqual({
      ok: false,
  ...
  it('rejects private key with wrong decoded length', () => {
    ...
    expect(
      parseWebPushVapidPatchValue(
        { value: { publicKey: validPub, privateKey: badPriv } },
        { hasExistingPrivate: false },
      ),
    ).toEqual({
      ok: false,
