...
describe('SystemSettingsService', () => {
  ...
  it('clearSetting removes the override', async () => {
    const port = makePort();
    await expect(
      createSystemSettingsService(port).clearSetting('operator_health_probe_config', 'admin', 'u'),
    ).resolves.toBe(true);
        ...
        return [
          {
            ...
            valueJson: input.legacyRows[0]!.valueJson,
            updatedAt: '',
            updatedBy: 'u1',
          },
        ];
    ...
    await service.updateSetting(
      ...
      'admin',
      {
        value: {
          enabled: true,
          defaultProviderId: 'p',
          providers: [{ id: 'p', label: 'P', enabled: true, apiKey: 'private-token' }],
        },
      },
      'u1',
      { organizationId: 'org-1' },
  ...
  it('reads runtime first, falls back only when absent, and records bounded value-free telemetry', async () => {
    ...
    await expect(
      service.getSetting('patient_program_discussion_ui_enabled', 'admin'),
    ).resolves.toMatchObject({ valueJson: { value: true } });
  ...
  describe('P0.11.3 org-aware write chokepoint (resolveWriteOrganizationId)', () => {
    ...
    it('does not turn the explicit platform fallback option into a generic per-org bypass', async () => {
      const service = createSystemSettingsService(makePort());
      await expect(
        service.updateSetting('patient_label', 'doctor', { value: 'Клиенты' }, 'platform-user', {
          organizationId: null,
    ...
    it('updateSetting — GLOBAL key forces organizationId: null at the port even when caller passes one', async () => {
      ...
      expect(port.upsert).toHaveBeenCalledWith('dev_mode', 'admin', { value: true }, 'user-uuid', {
        organizationId: null,
      });
    });
  ...
  it('shouldDispatchRelayToRecipient — dev_mode false → true для любого recipient', async () => {
    ...
    const service = createSystemSettingsService(port);
    expect(
      await service.shouldDispatchRelayToRecipient({ channel: 'telegram', recipient: '999' }),
    ).toBe(true);
  });
  ...
  it('shouldDispatchRelayToRecipient — dev_mode true, telegram recipient в списке → true', async () => {
    ...
    const service = createSystemSettingsService(port);
    expect(
      await service.shouldDispatchRelayToRecipient({ channel: 'telegram', recipient: '111' }),
    ).toBe(true);
  });
  ...
  it('shouldDispatchRelayToRecipient — dev_mode true, test_account_identifiers отсутствует → false', async () => {
    ...
    const service = createSystemSettingsService(port);
    expect(
      await service.shouldDispatchRelayToRecipient({ channel: 'telegram', recipient: 'any' }),
    ).toBe(false);
  });
  ...
  it('shouldDispatchRelayToRecipient — dev_mode true, recipient не в списке → false', async () => {
    ...
    const service = createSystemSettingsService(port);
    expect(
      await service.shouldDispatchRelayToRecipient({ channel: 'telegram', recipient: '222' }),
    ).toBe(false);
  });
  ...
  it('shouldDispatchRelayToRecipient — dev_mode true, max recipient в списке → true', async () => {
    ...
    const service = createSystemSettingsService(port);
    expect(await service.shouldDispatchRelayToRecipient({ channel: 'max', recipient: 'm1' })).toBe(
      true,
    );
  });
  ...
  it('getRelayDevContext — dev_mode off → devMode false', async () => {
    ...
    const service = createSystemSettingsService(port);
    await expect(service.getRelayDevContext()).resolves.toEqual({
      devMode: false,
      testAccounts: null,
    });
  });
