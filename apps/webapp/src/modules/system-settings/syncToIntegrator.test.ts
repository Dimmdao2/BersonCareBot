...
const {
  enqueueIntegratorPushMock,
  enqueuePlatformSystemSettingsPushMock,
  getCurrentDbPrincipalMock,
} = vi.hoisted(() => ({
...
describe('syncSettingToIntegrator', () => {
  ...
  it('POSTs signed body to integrator settings/sync', async () => {
    ...
    expect(typeof (init.headers as Record<string, string>)['x-bersoncare-timestamp']).toBe(
      'string',
    );
    expect(typeof (init.headers as Record<string, string>)['x-bersoncare-signature']).toBe(
      'string',
    );
  });
