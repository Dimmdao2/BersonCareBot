...
describe('operator health probe settings', () => {
  it('keeps the registry default in code and accepts it', () => {
    expect(SYSTEM_SETTING_REGISTRY.operator_health_probe_config.defaultValue).toBe(
      JSON.stringify(OPERATOR_HEALTH_PROBE_DEFAULT_VALUE),
    );
    expect(() =>
      assertOperatorHealthProbeConfig({ value: OPERATOR_HEALTH_PROBE_DEFAULT_VALUE }),
    ).not.toThrow();
  });
  ...
  it('rejects unsafe timeout and interval with an explanation', () => {
    ...
    expect(() => assertOperatorHealthProbeConfig({ value: tooFast })).toThrow(
      'могут перегрузить провайдера',
    );
  });
