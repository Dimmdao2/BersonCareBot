...
function supportMessage(
  overrides: Partial<SerializedSupportMessage> & Pick<SerializedSupportMessage, 'senderRole'>,
...
describe('ChatView message links and actions', () => {
  ...
  it.each(['doctor', 'patient'] as const)(
    'opens a pasted URL externally in a hosted %s chat',
    async (variant) => {
      Element.prototype.scrollIntoView = vi.fn();
      ...
      render(
        <ChatView
          variant={variant}
          messages={[
            supportMessage({ senderRole: 'user', text: 'Откройте https://example.com/path' }),
          ]}
          composer={null}
        />,
      );
