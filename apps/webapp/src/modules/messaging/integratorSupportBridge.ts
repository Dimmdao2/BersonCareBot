...
import { parseWebappConversationId } from '@/modules/messaging/supportConversationIds';
...
export function createIntegratorSupportBridge(deps: {
  ...
  return {
    async syncUserMessage(
      ...
      const { id: conversationId, organizationId } =
      ...
      await deps.port
        ...
        .catch((err: unknown) => {
          logger.error(
            ...
            '[integratorSupportBridge] merge legacy conversations error',
          );
        });
