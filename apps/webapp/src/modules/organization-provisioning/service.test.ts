...
describe('createOrganizationProvisioningService', () => {
  ...
  it('lets the transactional port own absent-intent failures', async () => {
    const port = createPort();
    vi.mocked(port.provisionSpecialistOwner).mockRejectedValueOnce(
      new Error('specialist_signup_intent_not_found'),
    );
    const service = createOrganizationProvisioningService({ provisioningPort: port });

    await expect(service.provisionSpecialistOwner({ challengeId: 'challenge-1' })).rejects.toThrow(
      'specialist_signup_intent_not_found',
    );
  ...
  it('delegates provisioning and replay idempotency to the transactional port', async () => {
    ...
    await expect(service.provisionSpecialistOwner({ challengeId: 'challenge-1' })).resolves.toEqual(
      {
        organizationId: 'org-1',
        specialistId: 'specialist-1',
        membershipId: 'membership-1',
