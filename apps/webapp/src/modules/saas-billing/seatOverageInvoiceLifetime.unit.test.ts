...
describe('Р-15: место открывается в момент выставления счёта, а не в момент оплаты', () => {
   ...
   *
   * ⚠️ Правка по authority, не подгонка под код: прежняя редакция этого `it` требовала
   * `amountMinor` СТРОГО МЕНЬШЕ прежнего («остатка стало меньше»), то есть закрепляла ровно
   * находку F-B слепого аудита 19.08 — при сроке счёта по умолчанию клиника пользовалась местом
   * месяц и оставалась должна 48,39 ₽ вместо 1500,00 ₽. Третья часть Р-15 («отдельным счётом
   * место оплачивается ОДИН РАЗ, на момент открытия») это запрещает: пересчитывается остаток от
   * момента открытия, а он не меняется, — новым у замены остаётся только срок оплаты.
   */
  it('отменяет просроченный счёт и выставляет новый — с новым сроком, но той же суммой', async () => {
    ...
    // Услуга оказывается с момента ОТКРЫТИЯ места, а не с момента перевыставления, — четверо
    // суток просрочки не удешевляют место.
    expect(replacement?.amountMinor).toBe(purchase.invoice.amountMinor);
    expect(replacement?.servicePeriodStartsAt).toBe(purchase.invoice.servicePeriodStartsAt);
    // Новым становится ровно одно — срок оплаты: он «заданная длительность ОТ ВЫСТАВЛЕНИЯ».
    expect(replacement?.expiresAt).toBe('2026-07-23T12:00:00.000Z');
    expect(replacement?.expiresAt).not.toBe(purchase.invoice.expiresAt);
