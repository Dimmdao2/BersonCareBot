...
describe('onlineIntakeService', () => {
  ...
  describe('submitLfk', () => {
    ...
    it('throws on description too short', async () => {
      await expect(service.submitLfk({ ...base, description: 'short' })).rejects.toMatchObject({
    ...
    it('stamps and filters by organization in the in-memory port', async () => {
      ...
      await expect(
        runWithDbOrganizationPrincipal(orgA, () => svc.getRequestForDoctor(result.id)),
      ).resolves.toMatchObject({
        id: result.id,
      });
      await expect(
        runWithDbOrganizationPrincipal(orgB, () => svc.getRequestForDoctor(result.id)),
      ).resolves.toBeNull();
    });
  });
  ...
  describe('submitNutrition', () => {
    ...
    it('throws on description too short', async () => {
      await expect(
        service.submitNutrition({ ...base, description: 'short' }),
