      ...
      if (options.contentPages) {
        try {
          ...
          if (row) {
            ...
            /*
             * Всё, что не файл медиатеки (`api`), — внешняя ссылка. Дверь показа одна и
             * allowlist держит она — `toHostedVideoEmbedSrc`; ссылка, которую та не признала,
             * просто не превратится в iframe.
             */
