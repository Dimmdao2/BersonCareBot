...
describe('integratorPatientReminderNotifyBodySchema tenant context', () => {
  it('rejects a signed reminder payload without organizationId', () => {
    ...
    expect(integratorPatientReminderNotifyBodySchema.safeParse(withoutOrganization).success).toBe(
      false,
    );
  });
});
...
function buildDeps(
  overrides: Partial<PatientReminderIntegratorNotifyDeps> = {},
): PatientReminderIntegratorNotifyDeps {
  return {
...
describe('runPatientReminderIntegratorNotify', () => {
  ...
  it('sends email when selected and not rate limited (via relayOutbound)', async () => {
    ...
    expect(relayOutboundMock).toHaveBeenCalledWith(
      expect.objectContaining({
        channel: 'email',
        metadata: expect.objectContaining({ subject: expect.any(String) }),
      }),
    );
  });
