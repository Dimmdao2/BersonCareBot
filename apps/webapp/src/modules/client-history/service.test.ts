...
function port(overrides: Partial<ClientHistoryPort> = {}): ClientHistoryPort {
  return {
    ...
    listPatientTimeline: vi.fn().mockResolvedValue([]),
    listPatientPaymentHistory: vi.fn().mockResolvedValue([]),
    listPatientVisitHistory: vi.fn().mockResolvedValue([]),
...
describe('createClientHistoryService', () => {
  it('assertSelfServiceBookingAllowed throws when blocked', async () => {
    ...
    await expect(svc.assertSelfServiceBookingAllowed('org-1', 'user-1')).rejects.toThrow(
      'booking_blocked',
    );
  });
  ...
  it('upsertBookingProfile trims problematic note', async () => {
    ...
    expect(upsert).toHaveBeenCalledWith(expect.objectContaining({ problematicNote: null }));
  });
});
