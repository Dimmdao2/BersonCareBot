...
Timeline merges (tenant-scoped by `organization_id`, patient by `platform_user_id`):
...
Dedupe: detailed reschedule/cancel vs timeline mirror; package usage vs package history; payment timeline mirror vs canonical payment row.

Product catalog (`be_products`/`be_product_purchases`/`be_product_history_events`, incl. the phone-matched
orphan-payment merge) was cut whole — owner decision 01.08, `docs/_TODO/SAAS_FOUNDATION/SAAS_BILLING_PLAN.md` B1.4.
