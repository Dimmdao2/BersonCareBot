...
const receiptMigration = readFileSync(
  resolve(repoRoot, "apps/webapp/db/drizzle-migrations/0183_reference_catalog_snapshot_receipts.sql"),
  "utf8",
);
...
const rlsOverlay = readFileSync(resolve(repoRoot, "deploy/postgres/reference-catalog-rls.sql"), "utf8");
...
describe("reference catalog snapshot-at-provision contract", () => {
  it("stores a versioned canonical manifest independent from every clinic catalog", () => {
    ...
    const parsed = JSON.parse(manifest!) as { categories: Array<{ code: string; items: unknown[] }> };
    ...
    expect(parsed.categories.find((category) => category.code === "body_region")?.items).toHaveLength(20);
  ...
  it("uses per-org uniqueness and an immutable version receipt instead of reconciliation", () => {
    ...
    expect(receiptMigration.indexOf("IF FOUND THEN")).toBeLessThan(
      receiptMigration.indexOf("FROM public.reference_catalog_baselines", receiptMigration.indexOf("IF FOUND THEN")),
    );
  ...
  it("seeds the snapshot inside specialist owner provisioning before it returns", () => {
    ...
    const seed = provisioning.indexOf("PERFORM app.seed_reference_catalog_snapshot(v_organization_id)");
    ...
    expect(provisioning).toContain("REVOKE ALL ON FUNCTION app.seed_reference_catalog_snapshot(uuid) FROM PUBLIC");
    expect(rlsOverlay).toContain(
      'ALTER FUNCTION app.seed_reference_catalog_snapshot(uuid) OWNER TO :"provisioning_owner"',
    );
    expect(rlsOverlay).toContain("GRANT EXECUTE ON FUNCTION app.seed_reference_catalog_snapshot(uuid)");
  });
  ...
  it.each([
    ...
    ["webapp-only", webappProductionDeploy, "specialist-owner-provisioning-rls.sql", "reference-catalog-rls.sql"],
  ])("refreshes both overlays after %s production migrations and before restart", (_name, deploy, provisioningMarker, referenceMarker) => {
  ...
  it("keeps patient access read-only and tied to an active enrollment in the selected org", () => {
    ...
    expect(rlsOverlay).toContain("enrollment.organization_id = reference_categories.organization_id");
    ...
    expect(rlsOverlay).not.toMatch(/FOR (?:ALL|INSERT|UPDATE|DELETE) TO :"reference_catalog_patient_role"/);
