...
function member(
  overrides: Partial<OrganizationMemberDirectoryRecord> = {},
): OrganizationMemberDirectoryRecord {
  return {
    id: 'membership-1',
...
function makeService(params: {
  ...
  const inviteSeatReservations =
    params.inviteSeatReservations ??
    pendingInvites.filter((item) => item.invitedRole === 'doctor').length;
...
describe('getSeatStatus', () => {
  ...
  it('counts a pending doctor invite as a reservation but not a pending admin invite', async () => {
    const service = makeService({
      ...
      pendingInvites: [
        ...
        invite({ id: 'invite-2', invitedRole: 'admin' }),
      ],
      seatLimit: 2,
    });
    const status = await service.getSeatStatus('org-a');
  ...
  it('keeps an accepted doctor invite reserved until its membership receives a specialist binding', async () => {
    ...
    await expect(service.getSeatStatus('org-a')).resolves.toEqual({
      limit: 2,
      used: 1,
      available: 1,
    });
  ...
  it('reports a 0 limit and 0 availability when clinic_team is not enabled', async () => {
    const service = makeService({
      members: [member({ specialistId: 'spec-1' })],
      clinicTeamEnabled: false,
    });
    const status = await service.getSeatStatus('org-a');
