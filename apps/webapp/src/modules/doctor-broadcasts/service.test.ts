...
vi.mock('@/modules/messaging/appendPatientInboundAdminMessage', () => ({
  appendPatientInboundAdminMessage: vi
    .fn()
    .mockResolvedValue({ conversationId: 'c1', messageId: 'm1' }),
  broadcastChatIntegratorMessageId: (auditId: string, userId: string) =>
    `broadcast:${auditId}:${userId}`,
}));
import type {
...
describe('doctor-broadcasts service', () => {
  ...
  it('preview passes segmentSize when resolver returns it', async () => {
    const svc = createDoctorBroadcastsService({
      resolveBroadcastAudience: async (filter, channels, category) => ({
        ...(await makeResolve([client('x', { bindings: { telegramId: '1' } })])(
          filter,
          channels,
          category,
        )),
  ...
  it('throws when delivery job cap exceeded', async () => {
    const many = Array.from({ length: 3000 }, (_, i) =>
      client(`u${i}`, { bindings: { telegramId: String(100000 + i) } }),
    );
    const svc = createDoctorBroadcastsService({
  ...
  it('execute with email channel calls email fan-out', async () => {
    const emailFanOut = vi
      .fn()
      .mockResolvedValue({ attempted: 1, delivered: 1, errors: 0, skipped: 0 });
