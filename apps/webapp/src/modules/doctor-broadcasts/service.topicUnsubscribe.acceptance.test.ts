...
describe('doctor broadcast topic unsubscribe — send-time gate', () => {
  it.each([
    ['marketing', 'patient_news', 'Новости и уведомления'],
    ['important_notice', 'important_broadcasts', 'Важные рассылки'],
  ] as const)(
    'keeps an unsubscribed recipient out of bot/email and names the %s topic in both deliveries',
    async (category, topicCode, topicTitle) => {
      const clients = [
        client(SUBSCRIBED_USER_ID, 'Подписан'),
      ...
      const service = createDoctorBroadcastsService({
        resolveBroadcastAudience: async () => ({
          ...
          recipientsPreview: {
            names: clients.map((row) => row.displayName),
            total: 2,
            truncated: false,
          },
        ...
        getTopicDisplayTitle: async (resolvedTopicCode) =>
          resolvedTopicCode === 'patient_news'
            ...
            : resolvedTopicCode === 'important_broadcasts'
              ? 'Важные рассылки'
              : null,
        broadcastAuditPort: {
          append: async (entry) => ({
