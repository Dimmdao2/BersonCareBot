...
function diagnostics(status: SaasIsolationHealthPayload['status']): SaasIsolationHealthPayload {
  return {
    ...
    active: {
      unexplained: status === 'critical' ? 1 : 0,
      explained: 0,
      occurrences: status === 'critical' ? 1 : 0,
    },
...
describe('tenant isolation critical health runtime', () => {
  ...
  it('alerts on monotonic routing-counter deltas and recovers only after two clean ticks', () => {
    expect(observeTenantIsolationRuntimeCounters({ missingPrincipalSelections: 1 }, 0)).toEqual({
      critical: true,
      missingPrincipalDelta: 1,
    });
    expect(
  ...
  it('primes an empty or zero-row fresh installation without a restart false positive', () => {
    expect(observeTenantIsolationCanary(canary([]), 0)).toEqual({
      status: 'priming',
      affectedOrganizations: 0,
    });
    expect(observeTenantIsolationCanary(canary([]), 300_000)).toEqual({
      status: 'ok',
      affectedOrganizations: 0,
    });
  ...
  it('requires two zero-row samples, reports aggregate count only, and recovers when rows return', () => {
    ...
    expect(observeTenantIsolationCanary(canary(healthy), 900_000)).toEqual({
      status: 'ok',
      affectedOrganizations: 0,
    });
  });
  ...
  it('debounces a failed or truncated bounded read', () => {
    ...
    resetTenantIsolationCriticalHealthForTest();
    const oversized = canary(
      Array.from({ length: TENANT_ISOLATION_CANARY_MAX_ORGANIZATIONS + 1 }, (_, index) => ({
  ...
  it('keeps unsupported-client telemetry outside the tenant-isolation health path', () => {
    const sources = [
      ...
      readFileSync(
        new URL('../../app-layer/health/collectCriticalHealthSignals.ts', import.meta.url),
        'utf8',
      ),
