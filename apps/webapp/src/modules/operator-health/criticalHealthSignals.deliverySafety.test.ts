...
describe('classifyProviderQuotaSignals (D-f)', () => {
  ...
  it('pages on credit exhaustion and on credential rejection alike', () => {
    expect(
      classifyProviderQuotaSignals([incident({ errorClass: 'provider_credit_exhausted' })]),
    ).toHaveLength(1);
    expect(
      classifyProviderQuotaSignals([incident({ errorClass: 'provider_auth_rejected' })]),
    ).toHaveLength(1);
  });

  it('stays out of the way for ordinary send failures', () => {
    expect(
      classifyProviderQuotaSignals([incident({ errorClass: 'provider_send_failed' })]),
    ).toEqual([]);
  });
