...
describe('classifyOutboundProviderErrorClass', () => {
  ...
  it('classifies SMTP credential rejection', () => {
    ...
    expect(classifyOutboundProviderErrorClass('EAUTH: Invalid login')).toBe(
      'provider_auth_rejected',
    );
  });
  ...
  it('pages on the first occurrence for every dead-delivery class', () => {
    for (const message of [
      ...
      expect(
        isPageOnFirstOccurrenceProviderErrorClass(classifyOutboundProviderErrorClass(message)),
      ).toBe(true);
    }
  });
});
