...
function validateServiceOperation(service: SaasIsolationSourceService, operation: SaasIsolationSourceOperation): void {
  if (!OPERATIONS_BY_SERVICE[service].includes(operation)) throw new Error("invalid_saas_isolation_service_operation");
}

export function redactSaasIsolationEventInput(input: ReportSaasIsolationEventInput): ReportSaasIsolationEventInput {
  const object = strictObject(input, ["eventClass", "sourceService", "sourceOperation", "explanationStatus"], "unsafe_saas_isolation_event");
  const eventClass = enumValue(SAAS_ISOLATION_EVENT_CLASSES, object.eventClass, "invalid_saas_isolation_event_class");
  const sourceService = enumValue(SAAS_ISOLATION_SOURCE_SERVICES, object.sourceService, "invalid_saas_isolation_service");
  const sourceOperation = enumValue(SAAS_ISOLATION_SOURCE_OPERATIONS, object.sourceOperation, "invalid_saas_isolation_operation");
  validateServiceOperation(sourceService, sourceOperation);
  const explanationStatus = object.explanationStatus === undefined
    ? "unexplained"
    : enumValue(["explained", "unexplained"] as const, object.explanationStatus, "invalid_saas_isolation_explanation");
  return { eventClass, sourceService, sourceOperation, explanationStatus };
}

export function validateSaasIsolationCoverageInput(value: unknown): RecordSaasIsolationCoverageInput {
  const object = strictObject(value, ["id", "status", "startedAt", "finishedAt", "servicesChecked", "checksCount", "unexpectedErrorsCount"], "unsafe_saas_isolation_coverage");
  if (!Array.isArray(object.servicesChecked)) throw new Error("invalid_saas_isolation_coverage_services");
  const servicesChecked = [...new Set(object.servicesChecked.map((service) => enumValue(
  ...
  const result: RecordSaasIsolationCoverageInput = {
    ...
    status: enumValue(["complete", "incomplete", "failed"] as const, object.status, "invalid_saas_isolation_coverage_status"),
    ...
    unexpectedErrorsCount: nonNegativeInteger(object.unexpectedErrorsCount, "invalid_saas_isolation_unexpected_count"),
  };
  if (result.finishedAt < result.startedAt) throw new Error("invalid_saas_isolation_coverage_time");
  if (result.status === "complete" && (servicesChecked.length !== SAAS_ISOLATION_REQUIRED_SERVICES.length || result.checksCount < SAAS_ISOLATION_REQUIRED_SERVICES.length)) {
...
export function validateSaasIsolationEventAggregate(value: unknown): SaasIsolationEventAggregate {
  const object = strictObject(value, ["eventClass", "sourceService", "sourceOperation", "explanationStatus", "lifecycleStatus", "occurrenceCount", "firstSeenAt", "lastSeenAt"], "unsafe_saas_isolation_event_row");
  const sourceService = enumValue(SAAS_ISOLATION_SOURCE_SERVICES, object.sourceService, "invalid_saas_isolation_service");
  const sourceOperation = enumValue(SAAS_ISOLATION_SOURCE_OPERATIONS, object.sourceOperation, "invalid_saas_isolation_operation");
  validateServiceOperation(sourceService, sourceOperation);
  return {
    eventClass: enumValue(SAAS_ISOLATION_EVENT_CLASSES, object.eventClass, "invalid_saas_isolation_event_class"),
    sourceService,
    sourceOperation,
    explanationStatus: enumValue(["explained", "unexplained"] as const, object.explanationStatus, "invalid_saas_isolation_explanation"),
    lifecycleStatus: enumValue(["active", "resolved"] as const, object.lifecycleStatus, "invalid_saas_isolation_lifecycle"),
    occurrenceCount: positiveInteger(object.occurrenceCount, "invalid_saas_isolation_occurrence_count"),
...
export function validateSaasIsolationTrend(value: unknown): SaasIsolationTrend {
  const object = strictObject(value, ["asOf", "current24Hours", "previous24Hours", "daily7Days"], "unsafe_saas_isolation_trend");
  ...
  const expectedDates = emptySaasIsolationTrend(Date.parse(asOf)).daily7Days.map((point) => point.date);
  ...
  const current24Hours = nonNegativeInteger(object.current24Hours, "invalid_saas_isolation_current_24h");
  const previous24Hours = nonNegativeInteger(object.previous24Hours, "invalid_saas_isolation_previous_24h");
  return {
    asOf,
    current24Hours,
    previous24Hours,
...
export function buildSaasIsolationHealthPayload(
  ...
  const lastCoverage = unsafeLastCoverage === null ? null : validateSaasIsolationCoverageRun(unsafeLastCoverage);
  ...
  const coverageFresh = Number.isFinite(finishedMs) && finishedMs <= nowMs
    && nowMs - finishedMs <= SAAS_ISOLATION_COVERAGE_FRESH_HOURS * 3_600_000;
  const coverageComplete = lastCoverage?.status === "complete" && missingServices.length === 0
  ...
  if ((lastCoverage?.unexpectedErrorsCount ?? 0) > 0) statusReasons.push("coverage_unexpected_error");
  ...
  const status: SaasIsolationHealthStatus = statusReasons.some((reason) => reason === "active_unexplained_event" || reason === "coverage_unexpected_error" || reason === "coverage_failed")
    ? "critical"
    : !coverageComplete || active.explained > 0
      ...
      : !coverageFresh
        ? "stale"
        : "okay";
  return { schemaVersion: SAAS_ISOLATION_DIAGNOSTICS_SCHEMA_VERSION, status, statusReasons, active, resolved, byClass, events, lastEventAt, lastCoverage, coverageFresh, coverageComplete, missingServices, trend };
}

export function createSaasIsolationDiagnosticsService(port: SaasIsolationDiagnosticsPort) {
  return {
    report: (input: ReportSaasIsolationEventInput) => port.recordEvent(redactSaasIsolationEventInput(input)),
    recordCoverage: (input: RecordSaasIsolationCoverageInput) => port.recordCoverageAndResolve(validateSaasIsolationCoverageInput(input)),
...
export type SaasIsolationDiagnosticsService = ReturnType<typeof createSaasIsolationDiagnosticsService>;
