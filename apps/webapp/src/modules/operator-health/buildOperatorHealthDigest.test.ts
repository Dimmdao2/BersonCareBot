...
describe('buildOperatorHealthDigest', () => {
    ...
    const result = buildOperatorHealthDigest(
      baseInput({
        ...
        jobFailures: [
          {
            jobFamily: 'health',
            jobKey: 'health.operator_health_critical.tick',
            lastFailureAt: 'x',
          },
        ],
      }),
  ...
  it('always prints the delivery evidence, including in a green digest', () => {
    ...
    expect(
      result.lines.some((l) =>
        l.includes('Последняя подтверждённая доставка: 2026-07-26T09:00:00.000Z'),
      ),
    ).toBe(true);
    expect(result.lines.some((l) => l.startsWith('Самая старая неотправленная позиция:'))).toBe(
      true,
    );
  });
  ...
  it('is red when nothing was confirmed and the queue is stuck (the July shape)', () => {
    ...
    expect(
      result.lines.some((l) =>
        l.includes('Последняя подтверждённая доставка: 2026-07-25T09:00:00.000Z'),
      ),
    ).toBe(true);
