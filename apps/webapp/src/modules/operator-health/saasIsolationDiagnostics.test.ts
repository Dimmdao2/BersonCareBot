...
function coverage(
  overrides: Partial<RecordSaasIsolationCoverageInput> = {},
...
describe('E1 SaaS isolation diagnostics contract', () => {
  it('exposes distinct okay, stale, incomplete and critical states with reasons', () => {
    ...
    const stale = buildSaasIsolationHealthPayload(
      [],
      coverage({
    ...
    const incomplete = buildSaasIsolationHealthPayload(
      [],
      coverage({ status: 'incomplete', servicesChecked: ['webapp'], checksCount: 1 }),
      NOW,
    );
    ...
    expect(buildSaasIsolationHealthPayload([], coverage({ status: 'failed' }), NOW).status).toBe(
      'critical',
    );
  });
  ...
  it('validates a bounded seven-day trend and computes the rolling 24h delta', () => {
    ...
    expect(
      buildSaasIsolationHealthPayload([], coverage(), NOW, trend()).trend.daily7Days,
    ).toHaveLength(7);
    expect(() => validateSaasIsolationTrend({ ...trend(), rawSql: 'select secret' })).toThrow(
      'unsafe_saas_isolation_trend',
    );
    expect(() => validateSaasIsolationTrend({ ...trend(), daily7Days: [] })).toThrow(
      'invalid_saas_isolation_daily_trend',
    );
    expect(() =>
      validateSaasIsolationTrend({
        ...trend(),
  ...
  it('parses only the closed reversible TEST diagnostics scenarios', () => {
    expect(parseSaasIsolationDiagnosticsCommand(['scenario', '--state', 'critical'])).toEqual({
      kind: 'scenario',
      state: 'critical',
    });
    expect(() =>
      parseSaasIsolationDiagnosticsCommand(['scenario', '--state', 'prod-reset']),
  ...
  it('accepts exactly one conventional pnpm separator for the post-runtime gate', () => {
    ...
    expect(
      parseSaasIsolationDiagnosticsCommand([
        'post-runtime-gate',
        '--started-at',
  ...
  it('keeps diagnostics CLI command and argument validation closed', () => {
    expect(() => parseSaasIsolationDiagnosticsCommand(['--', '--', 'read'])).toThrow('usage:');
    expect(() => parseSaasIsolationDiagnosticsCommand(['read', '--unexpected'])).toThrow('usage:');
    expect(() =>
      parseSaasIsolationDiagnosticsCommand([
        '--',
        'post-runtime-gate',
  ...
  it('refuses empty or partial coverage claiming complete', () => {
    expect(() =>
      validateSaasIsolationCoverageInput(coverage({ servicesChecked: [], checksCount: 0 })),
    ).toThrow('invalid_saas_isolation_complete_coverage');
    expect(() =>
      validateSaasIsolationCoverageInput(
        coverage({ servicesChecked: ['webapp'], checksCount: 20 }),
      ),
    ).toThrow('invalid_saas_isolation_complete_coverage');
  });
  ...
  it('rejects unsafe event input and unsafe persisted rows before UI/API use', async () => {
    const port: SaasIsolationDiagnosticsPort = {
      ...
      listEventAggregates: vi.fn(async () => [
        { ...event(), sourceOperation: 'GET /patients/secret' },
      ]),
    ...
    for (const unsafeKey of [
      ...
      ).toThrow('unsafe_saas_isolation_event');
    }
  ...
  it('records coverage through one atomic port operation', async () => {
    ...
    const port: SaasIsolationDiagnosticsPort = {
      recordEvent: vi.fn(),
      recordCoverageAndResolve,
      listEventAggregates: vi.fn(async () => []),
      getLastCoverageRun: vi.fn(async () => null),
      getTrend: vi.fn(async () => trend()),
    };
  ...
  it('enqueues telemetry without waiting and bounds failure storms', async () => {
    ...
    const input = {
      eventClass: 'cleanup_failure' as const,
  ...
  it('migration and privilege overlay are closed and least-privilege', async () => {
    const migration = await readFile(
      join(process.cwd(), 'db/drizzle-migrations/0185_saas_isolation_diagnostics.sql'),
      'utf8',
    );
    const overlay = await readFile(
      join(process.cwd(), '../../deploy/postgres/saas-isolation-telemetry.sql'),
      'utf8',
    );
