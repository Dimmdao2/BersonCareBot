...
describe("outbound provider incident cadence", () => {
  it("alerts immediately at T0", () => {
    expect(outboundProviderIncidentAlertPhase(incident(), Date.parse("2026-07-22T06:00:00.000Z"))).toBe("initial");
  });

  it("does not repeat before T+1h and repeats at T+1h", () => {
    const row = incident({ alertSentAt: "2026-07-22T06:00:05.000Z" });
    expect(outboundProviderIncidentAlertPhase(row, Date.parse("2026-07-22T06:59:59.999Z"))).toBeNull();
    expect(outboundProviderIncidentAlertPhase(row, Date.parse("2026-07-22T07:00:00.000Z"))).toBe("one_hour_repeat");
  });

  it("stops critical repeats after the one-hour escalation marker", () => {
    const row = incident({ alertSentAt: "2026-07-22T07:00:00.000Z" });
    expect(outboundProviderIncidentAlertPhase(row, Date.parse("2026-07-23T06:00:00.000Z"))).toBeNull();
  });
