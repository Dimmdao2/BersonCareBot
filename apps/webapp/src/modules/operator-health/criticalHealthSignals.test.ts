...
function healthyInput(
  overrides: Partial<CriticalHealthSignalsInput> = {},
...
function healthyBanner(
  overrides: Partial<OperatorHealthBannerInput> = {},
...
describe('classifyCriticalHealthSignals', () => {
  ...
  it('does not treat debounced detector states as critical', () => {
    ...
    expect(candidates.some((candidate) => candidate.topic.startsWith('tenant_isolation'))).toBe(
      false,
    );
  });
  ...
  it('flags projection deadCount but not retries only', () => {
    const dead = classifyCriticalHealthSignals(
      healthyInput({
        projection: { probeStatus: 'degraded', deadCount: 2, retriesOverThreshold: 0 },
      }),
    );
    ...
    const retriesOnly = classifyCriticalHealthSignals(
      healthyInput({
        projection: { probeStatus: 'degraded', deadCount: 0, retriesOverThreshold: 5 },
      }),
    );
  ...
  it('flags projection unreachable and error probe status', () => {
    expect(
      classifyCriticalHealthSignals(
        healthyInput({
          projection: { probeStatus: 'unreachable', deadCount: 0, retriesOverThreshold: 0 },
        }),
    ...
    expect(
      classifyCriticalHealthSignals(
        healthyInput({
          projection: { probeStatus: 'error', deadCount: 0, retriesOverThreshold: 0 },
        }),
      ).some((x) => x.topic === 'projection'),
    ).toBe(true);
  });
...
describe('classifyOperatorHealthBannerSignals', () => {
  ...
  it('shows banner for open operator incidents', () => {
    expect(
      classifyOperatorHealthBannerSignals(healthyBanner({ operatorIncidentsOpenCount: 2 })),
    ).toBe(true);
  });
  ...
  it('shows banner for a threshold-gated probe incident and video transcode error', () => {
    ...
    expect(
      classifyOperatorHealthBannerSignals(healthyBanner({ videoTranscodeStatus: 'error' })),
    ).toBe(true);
    expect(
      classifyOperatorHealthBannerSignals(healthyBanner({ videoTranscodeStatus: 'degraded' })),
    ).toBe(false);
  });
});
