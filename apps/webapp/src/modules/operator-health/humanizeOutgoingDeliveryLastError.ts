 ...
 */
export function humanizeOutgoingDeliveryLastError(
  ...
  if (upper.includes('MISSING_BROADCAST_AUDIT_ID') || upper === 'MISSING_BROADCAST_AUDIT_ID') {
    return {
      reason_code: 'MISSING_BROADCAST_AUDIT_ID',
      reason_ru: 'В задаче нет идентификатора журнала рассылки',
    };
  }
  if (upper.includes('MISSING_INCIDENT_ID') || upper === 'MISSING_INCIDENT_ID') {
    return {
      reason_code: 'MISSING_INCIDENT_ID',
      reason_ru: 'В задаче операторского алерта нет incident_id',
    };
  }
  if (upper.includes('MISSING_REMINDER_FIELDS') || upper === 'MISSING_REMINDER_FIELDS') {
    return {
      reason_code: 'MISSING_REMINDER_FIELDS',
      reason_ru: 'Не хватает полей для доставки напоминания',
    };
  }
...
export function humanizeIntegratorPushOutboxLastError(
  lastError: string | null | undefined,
): HumanizeResult {
  const raw = (lastError ?? '').trim();
  if (!raw) {
  ...
  return {
    ...
    reason_ru: 'Сбой синка в integrator (см. усечённый текст)',
  };
}
