...
export async function buildDiaryPlanReminderStrip(
  ...
  const warmupsSectionAvailable = Boolean(
    warmRes && (!warmRes.section.requiresAuth || canViewAuthOnlyContent),
  );
  ...
  const rehabRuleForStrip = activePlan
    ? pickLinkedReminderRule(rules, 'rehab_program', activePlan.id)
    : null;
  const warmupRuleForStrip = pickLinkedReminderRule(rules, 'content_section', warmupsLinkedId);

  return {
    rehabTodayLine: formatPlanReminderTodayLine(
      rehabRuleForStrip,
      calendarDateKey,
      resolvedIana,
      now,
    ...
    warmupTodayLine: warmupsSectionAvailable
      ...
      : null,
    remindersHref: activePlan
      ? `${routePaths.patientReminders}#patient-reminders-rehab`
