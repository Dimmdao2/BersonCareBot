...
export function PatientDiaryWarmupWeekBars({
  weekDayLabels,
  days,
}: PatientDiaryWarmupWeekBarsProps) {
  return (
    <section
        ...
        {weekDayLabels.map((label, idx) => {
          ...
          return (
            <div key={label} className="flex min-w-0 flex-col items-center gap-1">
              ...
              {d == null ? (
                <div
                  className="mt-auto h-16 w-full max-w-[2.5rem] rounded-sm bg-[var(--patient-border)]/58"
                  aria-hidden
                />
              ) : (
                <div className="relative mt-auto flex h-16 w-full max-w-[2.5rem] flex-col justify-end rounded-sm bg-[var(--patient-border)]/44">
                  {d.allDone ? (
                    <span
                      ...
                      aria-label="Все слоты дня закрыты"
                    >
