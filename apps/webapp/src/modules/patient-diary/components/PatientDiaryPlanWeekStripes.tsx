...
export function PatientDiaryPlanWeekStripes({
  weekDayLabels,
  days,
}: PatientDiaryPlanWeekStripesProps) {
          ...
          return (
            <div key={label} className="flex min-w-0 flex-col items-stretch gap-1">
              ...
              <div className="flex min-h-[48px] flex-1 flex-col justify-end gap-0.5">
                {items.length === 0 ? (
                  <div
                    className="h-0.5 w-full rounded-full bg-[var(--patient-border)]/60"
                    aria-hidden
                  />
                ) : (
                  items.map((it) => (
