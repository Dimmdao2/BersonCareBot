...
export const LEGACY_SKIP_PUSH_CATEGORIES = new Set([
  'water',
  'breathing',
...
export function classifyReminderPushKind(input: {
  ...
  if (lot === 'rehab_program' || lot === 'treatment_program_item' || lot === 'lfk_complex')
    return 'training';
  ...
  if (
    openUrl.includes('/treatment/') ||
    openUrl.includes('/go/plan') ||
    openUrl.includes('/diary/lfk/')
  ) {
    return 'training';
  }
