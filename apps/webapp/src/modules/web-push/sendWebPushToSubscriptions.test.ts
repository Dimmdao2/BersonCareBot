...
describe('sendWebPushToSubscriptions onAttempt', () => {
  ...
  it('calls onAttempt with success', async () => {
    ...
    expect(attempts).toEqual([
      { status: 'success', endpointHash: expect.any(String), providerStatusCode: 201 },
    ]);
    const sentBody = JSON.parse(sendNotificationMock.mock.calls[0]![1] as string) as Record<
      string,
      string
    >;
  ...
  it('keeps provider error as warn regardless of verbose', async () => {
    ...
    const providerErrorWarn = warnCalls.some(
      (args) =>
        ...
        (args[0] as Record<string, unknown>).event === 'web_push_provider_response',
    );
