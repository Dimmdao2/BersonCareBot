...
function makePort(overrides: Partial<PatientClinicalPort> = {}): PatientClinicalPort {
  return {
    ...
    getAnamnesis: vi.fn().mockResolvedValue({ trauma: [], illness: [], lifestyle: [] }),
    appendAnamnesisTrauma: vi.fn().mockImplementation(async (i) => ({ id: 't1', ...i })),
    appendAnamnesisIllness: vi.fn().mockImplementation(async (i) => ({ id: 'i1', ...i })),
...
describe('patient-clinical service — инлайн-правка полей', () => {
  ...
  it('updateVisitFields rejects when no section is provided', async () => {
    const port = makePort();
    const svc = createPatientClinicalService({ patientClinicalPort: port });
    await expect(svc.updateVisitFields({ patientUserId: PATIENT, visitId: 'v1' })).rejects.toThrow(
      /nothing_to_update/,
    );
  });
