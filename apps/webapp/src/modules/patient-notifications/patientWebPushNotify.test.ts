...
vi.mock('@/modules/messaging/appendPatientInboundAdminMessage', () => ({
  ...
  bookingLifecycleChatIntegratorMessageId: (variant: string, bookingId: string) =>
    `booking-${variant}:${bookingId}`,
}));
...
vi.mock('@/app-layer/product-analytics/createTrackedWebPushPayload', () => ({
  createTrackedWebPushPayload: vi
    .fn()
    .mockImplementation(
      (input: {
        title: string;
...
describe('runPatientWebPushNotify — P15 migration (relay-outbound, no direct send)', () => {
  ...
  it('skips with muted when gate is muted', async () => {
    const deps = baseDeps();
    vi.mocked(deps.readReminderNotifyGate).mockResolvedValue({
      muted: true,
