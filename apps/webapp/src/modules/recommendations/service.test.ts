...
describe('recommendations service', () => {
  ...
  it('createRecommendation rejects empty title', async () => {
    ...
    await expect(svc.createRecommendation({ title: '  ', bodyMd: 'x' }, null)).rejects.toThrow(
      /обязательно/,
    );
  });
  ...
  it('listRecommendations filters by domain', async () => {
    await inMemoryRecommendationsPort.create(
      { title: 'N', bodyMd: 'x', domain: 'nutrition' },
      null,
    );
    await inMemoryRecommendationsPort.create(
      { title: 'M', bodyMd: 'y', domain: 'motivation' },
      null,
    );
  ...
  it('unarchiveRecommendation throws when not archived', async () => {
    ...
    await expect(svc.unarchiveRecommendation(rec.id)).rejects.toBeInstanceOf(
      RecommendationUnarchiveNotArchivedError,
    );
  });
  ...
  it('updateRecommendation rejects new unknown domain', async () => {
    const rec = await inMemoryRecommendationsPort.create(
      { title: 'L', bodyMd: 'x', domain: 'nutrition' },
      null,
    );
    const svc = createRecommendationsService(inMemoryRecommendationsPort, inMemoryReferencesPort);
  ...
  it('archiveRecommendation runs only archive write through write options', async () => {
    ...
    await svc.archiveRecommendation('rec-1', undefined, {
