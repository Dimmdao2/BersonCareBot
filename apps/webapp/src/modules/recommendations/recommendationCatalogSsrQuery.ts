...
export type RecommendationCatalogSsrParsed = {
  ...
  /** Непустой `?domain=` не распознан как код типа — фильтр по типу не применяется (как `GET /api/doctor/recommendations`). */
 ...
 */
export function parseRecommendationCatalogSsrQuery(sp: {
  region?: string;
  domain?: string;
  ...
  const domainParsed = domainRaw ? parseRecommendationDomain(domainRaw) : undefined;
