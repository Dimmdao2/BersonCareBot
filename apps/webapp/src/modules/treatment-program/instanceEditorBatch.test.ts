...
describe('doctorApplyInstanceEditorBatch', () => {
  ...
  it('writes single program_changed for combined metadata and structural batch', async () => {
    ...
    expect(programChanged.length).toBe(
      eventsBefore.filter((e) => e.eventType === 'program_changed').length + 1,
    );
  });
        ...
        itemCreates: [
          {
            ...
            snapshot: {
              title: 'Черновик',
              media: [
                { mediaUrl: `/api/media/${exRef}/preview/sm`, mediaType: 'image', sortOrder: 0 },
              ],
            },
          },
        ],
  ...
  it.each([
    ...
    'rejects an individual exercise API payload targeting the %s system group',
    async (systemKind, message) => {
      const tpl = await tplSvc.createTemplate({ title: 'П', status: 'published' }, null);
      const s1 = await tplSvc.createStage(tpl.id, { title: 'Э1' });
      const inst = await instSvc.assignTemplateToPatient({
        templateId: tpl.id,
  ...
  it('ignores legacy itemPatches keyed by draft client id', async () => {
    ...
    const created = next.stages.flatMap((s) => s.items).find((i) => i.localComment === 'из create');
    expect(created).toBeDefined();
    expect(
  ...
  it('creates lfk_complex_expand lines with loadSettings and per-line groupId', async () => {
    ...
    const items = next.stages
      .flatMap((s) => s.items)
      .filter((i) => i.itemRefId === exRef1 || i.itemRefId === exRef2);
