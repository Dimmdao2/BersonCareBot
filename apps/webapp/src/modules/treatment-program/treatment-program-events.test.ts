...
describe('treatment-program events (§8)', () => {
  ...
  it('AUDIT_PHASE_7: status_changed program при завершении экземпляра', async () => {
    ...
    const st = ev.find(
      (e) =>
        e.eventType === 'status_changed' && e.targetType === 'program' && e.targetId === inst.id,
    );
  ...
  it('фаза 9: reorder этапов — status_changed stages_reordered и порядок id', async () => {
    ...
    const sorted = [...detail.stages].sort(
  ...
  it('фаза 9: замена запрещена при completed_at (отключение разрешено)', async () => {
    ...
    const itemId = instStageByTpl(inst, s1.id).items[0]!.id;
    await persistence.instancePort.setStageItemCompletedAt(
      inst.id,
      itemId,
  ...
  it('фаза 9–11: проекция ЛФК из активных программ для интегратора (развёрнутый комплекс)', async () => {
    ...
    const tplPortLocal = createInMemoryTreatmentProgramPort({
      lfkComplexExpandPreview: lfkPreview,
    });
    const persistenceLocal = createInMemoryTreatmentProgramPersistence({
      lfkComplexExpandPreview: lfkPreview,
    });
    ...
    const blocks = await localInstSvc.listTreatmentProgramLfkBlocksForIntegratorPatient(
      inst.patientUserId,
    );
  ...
  it('AUDIT_PHASE_9 FIX 9-M-2: цепочка add → replace → reorder сохраняет id и обновляет snapshot', async () => {
    ...
    detail = await instSvc.getInstanceById(inst.id);
    expect(instStageByTpl(detail, s1.id).items.map((i) => i.id)).toEqual([
      itemSecondId,
      itemFirstId,
    ]);
