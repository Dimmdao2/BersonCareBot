...
export type TreatmentProgramInstanceDetailStageRow =
...
export function patientStageSectionShouldRender(
  stage: Pick<
    TreatmentProgramInstanceStageRow,
    'status' | 'goals' | 'objectives' | 'expectedDurationDays' | 'expectedDurationText'
  > & {
...
export function patientStageItemShowsNewBadge(
  item: Pick<
    TreatmentProgramInstanceStageItemRow,
    'itemType' | 'isActionable' | 'status' | 'lastViewedAt'
  >,
 ...
 */
export function splitPatientProgramStagesForDetailUi(
...
export function computePatientProgramElapsedDayCount(
  ...
  return computeProgressDaysAt0300(
    startIso,
    now,
    patientCalendarIana,
    appDisplayTimeZoneForAnchorIso,
  );
}
...
export function resolvePatientProgramProgressDaysForPatientUi(
  ...
  return computePatientProgramElapsedDayCount(
    detail,
    now,
    patientCalendarIana,
    appDisplayTimeZoneForAnchorIso,
...
export function computeProgressDaysAt0300(
  ...
  const startInstant = Number.isNaN(parsedJs.getTime())
    ? DateTime.fromISO(trimmed, { zone: 'utc' })
    : DateTime.fromMillis(parsedJs.getTime());
