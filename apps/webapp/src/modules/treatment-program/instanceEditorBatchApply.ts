...
function resolveBatchId(id: string, idMap: Map<string, string>, label: string): string {
  ...
  if (!resolved) throw new UserFacingError(notificationTextFactory.unknownDraftId(label));
  return resolved;
}
...
async function assertStageItemAllowsStructuralChange(
  ...
  if (item.completedAt) {
    throw new UserFacingError(notificationText.treatmentProgramElementDeleteReplaceLocked);
  }
...
function mergeLoadSettings(
  ...
  const prev =
    prevRaw != null && typeof prevRaw === 'object' && !Array.isArray(prevRaw) ? { ...prevRaw } : {};
  ...
  const applyInt = (
    ...
    if (!Number.isFinite(n) || n < min || n > max) {
      throw new UserFacingError(notificationTextFactory.integerRangeRequired(label, min, max));
    }
    prev[key] = n;
  };
...
function assertSameIdSet(ordered: string[], expected: Set<string>, label: string): void {
  if (!sameIdSet(ordered, expected)) {
    throw new UserFacingError(notificationTextFactory.invalidOrder(label));
  }
}
...
function validateItemStructuralGroupPatch(
  ...
  if (isStageZero(stage)) {
    ...
    if (patch.groupId != null) {
      throw new UserFacingError(notificationText.treatmentProgramGeneralStageNoGroupBinding);
    }
    return;
  }
  if (!nextGroupId) {
...
async function validateInstanceEditorBatchDraft(
  ...
  for (const [stageIdRaw, orderedGroupIds] of Object.entries(draft.groupReorders)) {
    ...
    const resolved = orderedGroupIds.map((id, i) =>
      resolveBatchId(id, previewIdMap, `Группа ${i + 1}`),
    );
  ...
  for (const [stageIdRaw, orderedItemIds] of Object.entries(draft.itemReorders)) {
    ...
    const resolved = orderedItemIds.map((id, i) =>
      resolveBatchId(id, previewIdMap, `Элемент ${i + 1}`),
    );
  ...
  for (const [itemIdRaw, patch] of Object.entries(draft.itemPatches)) {
    ...
    if (patch.recommendationContent !== undefined) {
      if (item.itemType !== 'recommendation') {
        throw new UserFacingError(
          notificationText.treatmentProgramExecutionModeRecommendationsOnly,
        );
      }
      if (!patch.recommendationContent.title.trim()) {
        throw new UserFacingError(notificationText.recommendationNameRequired);
      }
    }
  }
}
...
export async function applyInstanceEditorBatch(
  ...
  return deps.instances.runInMutationTransaction(async () => {
    ...
    for (
      let stageCreateIndex = 0;
      stageCreateIndex < input.draft.stageCreates.length;
      stageCreateIndex++
    ) {
      ...
      if (stageCreate.objectives !== undefined) {
        meta.objectives =
          stageCreate.objectives === null ? null : stageCreate.objectives.trim() || null;
      }
      if (stageCreate.expectedDurationDays !== undefined)
        meta.expectedDurationDays = stageCreate.expectedDurationDays;
      if (stageCreate.expectedDurationText !== undefined) {
        meta.expectedDurationText =
          stageCreate.expectedDurationText === null
            ? null
            : stageCreate.expectedDurationText.trim() || null;
      }
      ...
      if (create.kind === 'library_item') {
        ...
        if (isStageZero(stage)) {
          ...
          if (create.itemType === 'recommendation' || create.itemType === 'clinical_test') {
            ...
            if (!sg)
              throw new UserFacingError(notificationText.treatmentProgramStageSystemGroupNotFound);
        ...
        const row = await instances.addInstanceStageItem(input.instanceId, stageId, {
          ...
          isActionable:
            create.isActionable ?? (create.itemType === 'recommendation' ? false : null),
        ...
        if (!result)
          throw new UserFacingError(notificationText.treatmentProgramRecommendationAddFailed);
        ...
        if (Object.keys(freeformPatch).length > 0) {
          const row = await instances.patchInstanceStageItem(
            input.instanceId,
            result.item.id,
            freeformPatch,
          );
        ...
        if (!result)
          throw new UserFacingError(notificationText.treatmentProgramPersonalExerciseCreateFailed);
        idMap.set(create.clientId, result.item.id);
        diff.itemsAdded += 1;
      } else if (create.kind === 'test_set_expand') {
        const stageId = resolveBatchId(create.stageId, idMap, 'Элемент');
        ...
        if (!testsGroup)
          throw new UserFacingError(notificationText.treatmentProgramTestingSystemGroupNotFound);
    ...
    for (const [stageIdRaw, patch] of Object.entries(input.draft.stageMetadata)) {
      ...
      if (patch.goals !== undefined)
        norm.goals = patch.goals === null ? null : patch.goals.trim() || null;
      ...
      if (patch.expectedDurationDays !== undefined)
        norm.expectedDurationDays = patch.expectedDurationDays;
    ...
    for (const [itemIdRaw, patch] of Object.entries(input.draft.itemPatches)) {
      ...
      if (patch.recommendationContent !== undefined) {
        const item = detail.stages.flatMap((stage) => stage.items).find((row) => row.id === itemId);
        if (!item || item.itemType !== 'recommendation') {
          throw new UserFacingError(notificationText.treatmentProgramElementNotFound);
        }
        const row = await instances.patchInstanceStageItem(input.instanceId, itemId, {
          snapshot: {
            ...item.snapshot,
            title: patch.recommendationContent.title.trim(),
            bodyMd: patch.recommendationContent.bodyMd.trim(),
