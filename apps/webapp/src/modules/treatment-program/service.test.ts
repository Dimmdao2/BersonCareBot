...
import {
  ...
  type TreatmentProgramStageItem,
} from './types';
...
describe('treatment-program service', () => {
  ...
  it('updateStageItem rejects non-recommendation on template stage zero', async () => {
    ...
    const nextRef = '22222222-2222-4222-8222-222222222222';
    await expect(
      svc.updateStageItem(item.id, { itemType: 'lesson', itemRefId: nextRef }),
  ...
  it('swap stage sortOrder reverses display order among non-zero stages', async () => {
    ...
    const titles = sortByOrderThenId(after.stages.filter((s) => s.sortOrder > 0)).map(
      (s) => s.title,
    );
  ...
  it('deleteTemplate requires acknowledgement when active instances exist', async () => {
    ...
    await expect(svc.deleteTemplate(tpl.id)).rejects.toBeInstanceOf(
      TreatmentProgramTemplateUsageConfirmationRequiredError,
    );
  ...
  describe('expandLfkComplexIntoTemplateStageItems', () => {
    ...
    it('inserts exercise rows ungrouped', async () => {
      ...
      expect(
        out.items.every(
          (i: TreatmentProgramStageItem) => i.itemType === 'exercise' && i.groupId == null,
        ),
      ).toBe(true);
    ...
    it('409 path: existing group with description and copy requested', async () => {
      port = createInMemoryTreatmentProgramPort({
        lfkComplexExpandPreview: {
          [complexId]: {
            exerciseIds: [ex1],
            complexDescription: 'From complex',
            complexTitle: null,
          },
        },
      });
    ...
    it('new_group copies complex description when checkbox true', async () => {
      port = createInMemoryTreatmentProgramPort({
        lfkComplexExpandPreview: {
          [complexId]: {
            exerciseIds: [ex1],
            complexDescription: 'Catalog text',
            complexTitle: null,
          },
        },
      });
  ...
  describe('expandTestSetIntoTemplateStageItems', () => {
    ...
    it('inserts clinical_test rows in set order under system tests group', async () => {
      ...
      expect(
        out.items.every((i) => i.itemType === 'clinical_test' && i.groupId === testsGroup.id),
      ).toBe(true);
    });
    ...
    it('skips tests already present in the system tests group', async () => {
      ...
      const testsGroup = beforeAdd.stages
        .find((s) => s.id === stage.id)!
        .groups.find((g) => g.systemKind === 'tests')!;
      await svc.addStageItem(stage.id, {
    ...
    it('rejects expand on template stage zero', async () => {
      ...
      const s0 = d.stages.find((s) => s.sortOrder === 0)!;
      await expect(
        svc.expandTestSetIntoTemplateStageItems(tpl.id, s0.id, testSetId),
    ...
    it('rejects expand when template is archived', async () => {
      ...
      await svc.updateTemplate(tpl.id, { status: 'archived' });
      await expect(
        svc.expandTestSetIntoTemplateStageItems(tpl.id, stage.id, testSetId),
      ).rejects.toBeInstanceOf(TreatmentProgramTemplateAlreadyArchivedError);
    });
    ...
    it('rejects unknown test set (no lines in catalog fixture)', async () => {
      ...
      await expect(
        svc.expandTestSetIntoTemplateStageItems(tpl.id, stage.id, missingSet),
      ).rejects.toBeInstanceOf(TreatmentProgramExpandNotFoundError);
    });
  });
  ...
  it('reorderTemplateStages rejects moving stage zero from index 0', async () => {
    ...
    const s0 = d.stages.find((s) => s.sortOrder === 0)!;
    const s1 = await svc.createStage(tpl.id, { title: 'S1' });
    await expect(svc.reorderTemplateStages(tpl.id, [s1.id, s0.id])).rejects.toThrow(
      /Общие рекомендации/,
    );
  });
  ...
  it('reorderTemplateStages rejects incomplete orderedStageIds', async () => {
    ...
    await expect(svc.reorderTemplateStages(tpl.id, [s0.id])).rejects.toThrow(
      /Некорректный порядок этапов/,
    );
  });
  ...
  it('reorderTemplateStageItems renumbers all items on stage', async () => {
    ...
    const items = sortByOrderThenId(
      (await svc.getTemplate(tpl.id)).stages.find((s) => s.id === stage.id)!.items,
    );
