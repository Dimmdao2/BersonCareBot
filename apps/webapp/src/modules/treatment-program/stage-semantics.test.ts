...
describe('stage-semantics (1.1a detail split)', () => {
  ...
  it('splitPatientProgramStagesForDetailUi separates zero, pipeline, archive', () => {
    const stages = [mk('z', 0, 'available'), mk('a', 1, 'completed'), mk('b', 2, 'available')];
  ...
  it('patientTreatmentProgramStageScreenVariant: zero stage is always interactive', () => {
    expect(patientTreatmentProgramStageScreenVariant({ sortOrder: 0, status: 'locked' })).toBe(
      'interactive',
    );
    expect(patientTreatmentProgramStageScreenVariant({ sortOrder: 0, status: 'completed' })).toBe(
      'interactive',
    );
  });

  it('patientTreatmentProgramStageScreenVariant maps pipeline statuses', () => {
    expect(patientTreatmentProgramStageScreenVariant({ sortOrder: 1, status: 'completed' })).toBe(
      'pastReadOnly',
    );
    expect(patientTreatmentProgramStageScreenVariant({ sortOrder: 1, status: 'skipped' })).toBe(
      'pastReadOnly',
    );
    expect(patientTreatmentProgramStageScreenVariant({ sortOrder: 1, status: 'locked' })).toBe(
      'futureLocked',
    );
    expect(patientTreatmentProgramStageScreenVariant({ sortOrder: 1, status: 'in_progress' })).toBe(
      'interactive',
    );
    expect(patientTreatmentProgramStageScreenVariant({ sortOrder: 1, status: 'available' })).toBe(
      'interactive',
    );
  });

  it('countBlockingStagesBeforePatientStage counts non-terminal predecessors', () => {
    const stages = [mk('a', 1, 'completed'), mk('b', 2, 'in_progress'), mk('c', 3, 'locked')];
  ...
  it('calendarDaysFromUtcIsoToNowInZone is non-negative', () => {
    expect(
      calendarDaysFromUtcIsoToNowInZone('2026-01-01T00:00:00.000Z', 'Europe/Moscow'),
  ...
  it('formatRelativePatientCalendarDayRu omits clock time (today / yesterday / N days ago)', () => {
    ...
    expect(
      formatRelativePatientCalendarDayRu('2026-05-06T02:00:00.000Z', 'Europe/Moscow', now),
    ).toBe('Сегодня');
    expect(
      formatRelativePatientCalendarDayRu('2026-05-05T12:00:00.000Z', 'Europe/Moscow', now),
    ).toBe('Вчера');
    expect(
      formatRelativePatientCalendarDayRu('2026-05-03T12:00:00.000Z', 'Europe/Moscow', now),
    ).toBe('3 дня назад');
    expect(
      formatRelativePatientCalendarDayRu('2026-04-26T12:00:00.000Z', 'Europe/Moscow', now),
    ).toBe('10 дней назад');
  });
  ...
  it('resolvePatientProgramProgressDaysForPatientUi is null while awaiting first engagement (available)', () => {
    ...
    expect(
      resolvePatientProgramProgressDaysForPatientUi(
        d,
        DateTime.fromISO('2026-01-10T12:00:00Z'),
        'Europe/Moscow',
        'Europe/Moscow',
      ),
    ).toBeNull();
  ...
  it('resolvePatientProgramProgressDaysForPatientUi is null when program completed', () => {
    ...
    expect(
      resolvePatientProgramProgressDaysForPatientUi(
        d,
        DateTime.fromISO('2026-02-01T12:00:00Z'),
        'Europe/Moscow',
        'Europe/Moscow',
      ),
    ).toBeNull();
  ...
  it('patientInstanceSystemGroupHasVisibleItems is false for empty system groups', () => {
    ...
    expect(
      patientInstanceSystemGroupHasVisibleItems({
        group: recGroup,
        items: [
          { groupId: 'g-rec', itemType: 'recommendation', status: 'active', isActionable: true },
        ],
      }),
    ).toBe(true);
    expect(
    ...
    expect(
      patientInstanceSystemGroupHasVisibleItems({
        group: testsGroup,
        items: [
          { groupId: 'g-tests', itemType: 'clinical_test', status: 'active', isActionable: null },
        ],
      }),
    ).toBe(true);
  });
