...
import {
  inferNormalizedDecisionFromScoring,
  scoringAllowsNumericDecisionInference,
  scoringConfigIsQualitative,
...
describe('treatment-program progress-service', () => {
  ...
  it('clinical_test: doctor cannot accept older submitted attempt while newer open attempt exists', async () => {
    ...
    const after = await persistence.instancePort.getInstanceForPatient(patient, inst.id);
    expect(after!.stages[0]!.items[0]!.completedAt).toBeNull();
    const attempts = await persistence.testAttemptsPort.listAttemptsForStageItem(
      itemId,
      patient,
      10,
    );
  ...
  it('list and count pending test evaluations have exact organization parity and fail closed', async () => {
    ...
    const itemPromo = promoInst.stages[0]!.items[0]!.id;
    await progress.patientSubmitTestResult({
      patientUserId: patientB,
      instanceId: promoInst.id,
      stageItemId: itemPromo,
      testId,
      rawValue: { score: 4 },
    });
...
describe('progress-scoring', () => {
  ...
  it('scoringAllowsNumericDecisionInference matches threshold keys used by infer', () => {
    ...
    expect(
      scoringAllowsNumericDecisionInference({ schema_type: 'qualitative', measure_items: [] }),
    ).toBe(false);
  ...
  it('scoringConfigIsQualitative', () => {
    expect(scoringConfigIsQualitative({ schema_type: 'qualitative', measure_items: [] })).toBe(
      true,
    );
