...
- **`ensureWellbeingTracking`** ищет трекинг среди активных и **неактивных** (кроме soft-delete), чтобы не плодить дубликаты `general_wellbeing`.
- Сервис: `createPatientMoodService({ diaries, references })` в `wellbeingMoodService.ts`; при отсутствии трекинга — создание с привязкой к справочнику `general_wellbeing`.
...
- `GET /api/patient/mood/week` — семь локальных дней «сегодня минус 6…сегодня» для полоски на главной (`warmupHint` зарезервировано).
