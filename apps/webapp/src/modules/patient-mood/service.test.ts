...
function makeDeps(overrides?: {
  ...
  const addEntry = vi.fn(
    async (p: { value0_10: number; recordedAt: string; notes?: string | null }) => ({
      id: 'new-entry',
      userId: 'u1',
...
describe('createPatientMoodService (wellbeing / symptom_entries)', () => {
  ...
  it('submitScore rejects non-integer and out-of-range scores', async () => {
    ...
    await expect(svc.submitScore('u1', 'UTC', 0, 'auto')).resolves.toMatchObject({
      ok: false,
      error: 'invalid_score',
    });
    await expect(svc.submitScore('u1', 'UTC', 6, 'auto')).resolves.toMatchObject({
      ok: false,
      error: 'invalid_score',
    });
    await expect(svc.submitScore('u1', 'UTC', 2.5, 'auto')).resolves.toMatchObject({
      ok: false,
      error: 'invalid_score',
    });
