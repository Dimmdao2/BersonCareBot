...
- Загрузка в библиотеку (роль doctor/admin): **легаси** presigned single-PUT (`presign` → PUT → `confirm`) или **multipart** direct-to-S3 при поддержке клиентом и флаге `s3Multipart` из `GET /api/media/s3-status`. The same `uploadMode: 'multipart'` is available only from the already-authorized patient program submission, doctor patient-file and individual-exercise begin doors; each supplies its closed server policy to `beginAuthorizedMultipartUpload`, never a bucket/key/policy from the client. Multipart object metadata records that immutable policy with media id, owner and expected size; generic part/complete/abort derive authorization from the installed doctor or patient DB principal and select the existing policy finalizer. Клиент библиотеки после неуспешного multipart делает **best-effort** `POST /api/media/multipart/abort` (в т.ч. из `libraryMultipartUpload` при ошибке после `init`). Ошибки `part-url` / `complete`: отдельно `session_expired`, `session_state_conflict` и `session_not_found` (см. `api.md`).
- Единый lifecycle upload: proxy сначала создаёт `media_files.pending`, затем пишет S3 и CAS-переводит в `ready`; terminal receive failure переводит pending в `pending_delete`, а штатный purge удаляет S3-first/DB-second с retry. Перед batch purge переводит только sessionless pending старше 24 часов; multipart остаётся в собственном session lifecycle. Связанный незавершённый `patient_files` удаляется в одной транзакции со stage abort.
...
- **Иерархия папок** (`media_folders`): CRUD через admin API; в UI библиотеки — хлебные крошки и дочерние папки с действиями переименовать / переместить / удалить (пустая папка). Перемещение файла — `PATCH /api/admin/media/[id]` с `folderId`. Колонки **`kind`** (`standard` \| `client_files_root` \| `client_patient`) и **`patient_user_id`** (миграции **`0113`**, **`0114`**): системная папка **«Пациенты»** (тип `client_files_root`) и подпапки на каждого пациента (тип `client_patient`); логика в [`clientFilesFolders.ts`](./clientFilesFolders.ts) и [`pgClientMediaFolders.ts`](../../infra/repos/pgClientMediaFolders.ts). Presign submission пациента и (при явном `folderId`) врачебная загрузка валидируют назначаемую папку; корень клиентских файлов для новых объектов запрещён.
