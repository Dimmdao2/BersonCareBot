...
import { PROGRAM_ITEM_DISCUSSION_ORIGINS, PROGRAM_ITEM_DISCUSSION_SENDER_ROLES } from './types';
...
function assertSenderRole(
  raw: string,
): asserts raw is (typeof PROGRAM_ITEM_DISCUSSION_SENDER_ROLES)[number] {
  if (
    !PROGRAM_ITEM_DISCUSSION_SENDER_ROLES.includes(
      raw as (typeof PROGRAM_ITEM_DISCUSSION_SENDER_ROLES)[number],
    )
  ) {
...
function assertOrigin(
  raw: string,
): asserts raw is (typeof PROGRAM_ITEM_DISCUSSION_ORIGINS)[number] {
  if (
    !PROGRAM_ITEM_DISCUSSION_ORIGINS.includes(
      raw as (typeof PROGRAM_ITEM_DISCUSSION_ORIGINS)[number],
    )
  ) {
...
export function createProgramItemDiscussionService(port: ProgramItemDiscussionPort) {
  async function appendMessage(
    ...
    const supportMessageId = input.supportMessageId
      ? assertUuid(input.supportMessageId, 'support_message_id')
      : null;
  ...
  return {
    ...
    async listMessagesForStageItem(
      ...
      return port.listMessagesForStageItem(
        assertUuid(stageItemId, 'stage_item_id'),
        limit,
        safeOffset,
      );
    },

    async listAttentionSummaryForStageItems(
      stageItemIds: string[],
    ...
    async getMaxLastReadAtForViewers(input: {
      ...
      const viewerUserIds = [
        ...new Set(input.viewerUserIds.map((id) => assertUuid(id, 'viewer_user_id'))),
      ];
    ...
    async listUnreadExerciseCommentsForDoctor(input: {
      ...
      const patientUserIds = [
        ...new Set(input.patientUserIds.map((id) => assertUuid(id, 'patient_user_id'))),
      ];
