...
describe('classifyEmptyAudienceSignal', () => {
  ...
  it('is silent on a never-used counter', () => {
    expect(
      classifyEmptyAudienceSignal({ ...EMPTY_EMPTY_AUDIENCE_COUNTER, byTopic: {} }, NOW).active,
    ).toBe(false);
  });
});
...
describe('isDestinationPresent', () => {
  ...
  it('counts a bounce-suppressed destination as ABSENT until the suppression expires', () => {
    ...
    expect(
      isDestinationPresent({ ...base, suppressedUntil: '2026-08-02T00:00:00.000Z' }, NOW),
    ).toBe(false);
    expect(
      isDestinationPresent({ ...base, suppressedUntil: '2026-07-20T00:00:00.000Z' }, NOW),
    ).toBe(true);
  });
  ...
  it('counts a blank address as ABSENT', () => {
    expect(
      isDestinationPresent(
        { channel: 'email', address: '   ', verifiedAt: '2026-07-01T00:00:00.000Z' },
        NOW,
      ),
    ).toBe(false);
  });
