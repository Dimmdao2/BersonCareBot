...
describe("relayOperatorAlert", () => {
  ...
  it("passes a bounded abort signal to the dedicated relay", async () => {
    ...
    await expect(relayOperatorAlert({
      messageId: "incident:1:initial",
      channel: "telegram",
      recipient: "42",
      text: "alert",
    }, { fetchImpl, timeoutMs: 25 })).resolves.toEqual({ ok: true, status: "accepted" });
  });

  it("returns a deterministic failure when the relay aborts", async () => {
    const fetchImpl = vi.fn().mockRejectedValue(new DOMException("timed out", "TimeoutError")) as typeof fetch;
    await expect(relayOperatorAlert({
      messageId: "incident:1:initial",
      channel: "max",
      recipient: "43",
      text: "alert",
    }, { fetchImpl, timeoutMs: 1 })).resolves.toEqual({ ok: false, reason: "timed out" });
  });
});
