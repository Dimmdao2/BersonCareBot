...
describe('notifyDoctorPatientMessageToStaff', () => {
  ...
  it('delivers telegram to staff with default fallback and binding', async () => {
    ...
    const result = await notifyDoctorPatientMessageToStaff(input, {
  ...
  it.each(['patient@example.com', '+79991234567', 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa'])(
    'uses a neutral patient label instead of unsafe sender value %s',
    async (unsafeLabel) => {
    ...
    const result = await notifyDoctorPatientMessageToStaff(
      {
        ...
        replyMarkup: {
          inline_keyboard: [
            [{ text: 'Ответить', callback_data: 'admin_reply:webapp:platform:u1' }],
          ],
        },
      },
      {
