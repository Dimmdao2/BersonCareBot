      ...
      <div className={landingContainer}>
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-2">
          <article className="rounded-[24px] border border-[#DDE5EF] bg-white p-6 sm:p-8">
            ...
            <ul className="mt-5 space-y-3">
              {soloCapabilities.map((item) => (
                <li
                  key={item}
                  className="flex items-start gap-2.5 text-[0.9375rem] leading-6 text-[#334155]"
                >
                  <Check
                    className="mt-1 h-4 w-4 shrink-0 text-[#406CA7]"
                    strokeWidth={2.5}
                    aria-hidden
                  />
                  {item}
          ...
          <article className="rounded-[24px] border border-[#DDE5EF] bg-white p-6 sm:p-8">
            ...
            <p className={cn(landingBodySecondary, 'mt-2 max-w-xl')}>
              Если с клиентами работает команда, расскажите о вашем сценарии. Мы покажем текущие
              возможности и обсудим, что нужно для запуска.
            </p>
            <div className="mt-5 rounded-2xl bg-[#FAF9F4] p-4 text-sm leading-6 text-[#5F574C]">
              Клинический режим для команды пока подключается через демо, без обещания неготовых
              функций.
            </div>
            <Link
