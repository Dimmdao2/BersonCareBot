...
export function detectRequiredBrowserMissing(
  ...
  if (platform === 'ios') {
    const isSafari = /Safari/i.test(userAgent) && !/CriOS|FxiOS|OPiOS|EdgiOS|GSA/i.test(userAgent);
    return !isSafari;
  }
  if (platform === 'android') {
    const isChrome =
      /Chrome\//i.test(userAgent) && !/EdgA|OPR\/|SamsungBrowser|YaBrowser/i.test(userAgent);
    return !isChrome;
  }
  return false;
}
