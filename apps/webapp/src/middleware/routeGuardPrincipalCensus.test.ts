...
const ROUTE_EXCEPTIONS: Readonly<Record<string, string>> = {
  'admin/booking-engine/appointments/[id]/delete/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/appointments/[id]/lifecycle/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/appointments/[id]/manual-cancel/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/appointments/[id]/manual-no-show/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/appointments/[id]/manual-reschedule/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/appointments/[id]/package/detach/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/appointments/[id]/package/refund/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/appointments/[id]/package/unlink/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/appointments/[id]/payment/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/appointments/manual/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/availability/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/branches/[id]/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/branches/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/calendar/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/form-fields/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/merge-candidates/[id]/dismiss/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/merge-candidates/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/online-location/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/organizations/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/overview/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/packages/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/patient-packages/[id]/consume/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/patient-packages/[id]/recalc/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/patient-packages/[id]/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/patient-packages/[id]/sessions/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/patient-packages/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/patient-products/[id]/consume/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/patient-products/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/policies/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/prepayment-policies/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/products/[id]/pay-link/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/products/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/public-appointments/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/rooms/[id]/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/rooms/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/schedule-blocks/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/scheduling-settings/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/services/[id]/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/services/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/slots-probe/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/specialist-rooms/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/specialists/[id]/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/specialists/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/working-days/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/working-hours/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'admin/booking-engine/working-schedule-templates/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'auth/channel-link/start/route.ts':
    'pre-auth: channel proof is established before a session exists',
  ...
  'auth/email-password/forgot/route.ts':
    'pre-auth: password recovery starts before a session exists',
  ...
  'auth/email-password/register/confirm/route.ts':
    'pre-auth: registration proof creates the session',
  ...
  'auth/email-password/setup-code/complete/route.ts':
    'pre-auth: account setup uses a one-time proof',
  ...
  'auth/oauth/callback/apple/route.ts':
    'pre-auth: OAuth callback verifies provider state before creating a session',
  'auth/oauth/callback/google/route.ts':
    'pre-auth: OAuth callback verifies provider state before creating a session',
  'auth/oauth/callback/route.ts':
    'pre-auth: OAuth callback verifies provider state before creating a session',
  'auth/oauth/callback/yandex/route.ts':
    'pre-auth: OAuth callback verifies provider state before creating a session',
  ...
  'auth/phone/messenger-bind/finish/route.ts':
    'pre-auth: messenger binding uses a one-time setup proof',
  'auth/phone/messenger-bind/start/route.ts':
    'pre-auth: messenger binding starts from a setup proof',
  'auth/phone/messenger-bind/status/route.ts':
    'pre-auth: messenger binding status uses a setup proof',
  ...
  'auth/specialist-signup/confirm/route.ts':
    'pre-auth: specialist registration proof creates the session',
  'auth/specialist-signup/retry/route.ts':
    'pre-auth: specialist registration retry precedes the session',
  'auth/specialist-signup/slug/route.ts': 'pre-auth: public registration availability check',
  'auth/specialist-signup/start/route.ts':
    'pre-auth: specialist registration starts before a session exists',
  ...
  'booking/public/create/confirm/route.ts':
    'pre-auth: public booking uses a one-time confirmation proof',
  ...
  'booking/public/payments/mock-complete/route.ts':
    'pre-auth: development payment callback uses booking proof',
  'booking/public/products/link/route.ts': 'pre-auth: public product link uses booking proof',
  'booking/public/products/payment-status/route.ts':
    'pre-auth: public product payment status uses booking proof',
  'booking/public/products/payments/mock-complete/route.ts':
    'pre-auth: development payment callback uses booking proof',
  'booking/public/products/purchase/route.ts':
    'pre-auth: public product purchase uses booking proof',
  ...
  'clinic/invites/accept/start/route.ts':
    'pre-auth: invite acceptance starts before a session exists',
  'doctor/booking-engine/appointments/[id]/delete/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/appointments/[id]/lifecycle/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/appointments/[id]/manual-cancel/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/appointments/[id]/manual-no-show/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/appointments/[id]/manual-reschedule/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/appointments/[id]/package/detach/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/appointments/[id]/package/refund/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/appointments/[id]/package/unlink/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/appointments/[id]/payment/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/appointments/manual-patient-visit/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/appointments/manual/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/calendar/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/overview/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/packages/[id]/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/packages/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/patient-packages/[id]/consume/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/patient-packages/[id]/recalc/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/patient-packages/[id]/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/patient-packages/[id]/sessions/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/patient-packages/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/patient-products/[id]/consume/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/patient-products/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/products/[id]/pay-link/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/products/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/services/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/working-days/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/working-hours/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/booking-engine/working-schedule-templates/route.ts':
    'delegated guard: local booking wrapper is outside app-layer/guards',
  'doctor/clients/merge-preview/route.ts':
    'disabled stub: returns not_available and performs no DB access',
  'doctor/clients/merge-user-search/route.ts':
    'disabled stub: returns not_available and performs no DB access',
  'doctor/clients/merge/route.ts': 'disabled stub: returns not_available and performs no DB access',
  'health/projection/route.ts':
    'public: health probe is intentionally callable without a user session',
  'health/route.ts': 'public: health probe uses an explicit infra principal without a user session',
  'integrator/appointments/active-by-user/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'integrator/appointments/record/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'integrator/channel-link/complete/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/communication/conversations/[id]/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'integrator/communication/conversations/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'integrator/communication/questions/by-conversation/[conversationId]/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'integrator/communication/questions/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'integrator/delivery-targets/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'integrator/diary/lfk-complexes/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'integrator/diary/symptom-trackings/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'integrator/events/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/messenger-phone/bind/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/patient-notifications/web-push/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/patient-reminders/notify-channels/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/phone-messenger-bind/complete/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/program-note/reply-begin/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/reminders/dispatch/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/reminders/history/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'integrator/reminders/messenger-topic/disable/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/reminders/mute/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/reminders/notification-settings/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/reminders/notification-settings/toggle/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/reminders/occurrences/done/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/reminders/occurrences/skip/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/reminders/occurrences/snooze/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/reminders/rules/by-category/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'integrator/reminders/rules/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'integrator/subscriptions/for-user/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'integrator/subscriptions/topics/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'integrator/support/admin-reply/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/support/sync-user-message/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/web-push/subscriptions/delete/route.ts':
    'signed ingress: integrator HMAC verifier is outside app-layer/guards',
  'integrator/web-push/subscriptions/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'integrator/web-push/vapid/route.ts':
    'signed ingress: integrator GET HMAC verifier is outside app-layer/guards',
  'internal/heartbeat/digest/route.ts':
    'internal job: constant-time INTERNAL_JOB_SECRET bearer verification',
  'internal/heartbeat/pipeline_delivery/route.ts':
    'internal job: constant-time INTERNAL_JOB_SECRET bearer verification',
  'internal/media-hls-proxy-errors/retention/route.ts':
    'internal job: constant-time INTERNAL_JOB_SECRET bearer verification',
  'internal/media-multipart/cleanup/route.ts':
    'internal job: constant-time INTERNAL_JOB_SECRET bearer verification',
  'internal/media-pending-delete/purge/route.ts':
    'internal job: constant-time INTERNAL_JOB_SECRET bearer verification',
  'internal/media-playback-stats/retention/route.ts':
    'internal job: constant-time INTERNAL_JOB_SECRET bearer verification',
  'internal/media-preview/process/route.ts':
    'internal job: constant-time INTERNAL_JOB_SECRET bearer verification',
  'internal/media-transcode/enqueue/route.ts':
    'internal job: constant-time INTERNAL_JOB_SECRET bearer verification',
  'internal/media-transcode/reconcile/route.ts':
    'internal job: constant-time INTERNAL_JOB_SECRET bearer verification',
  'internal/operator-health-critical/tick/route.ts':
    'internal job: constant-time INTERNAL_JOB_SECRET bearer verification',
  'internal/operator-health-digest/tick/route.ts':
    'internal job: constant-time INTERNAL_JOB_SECRET bearer verification',
  'internal/product-analytics/retention/route.ts':
    'internal job: constant-time INTERNAL_JOB_SECRET bearer verification',
  'internal/reminders/web-push-only/tick/route.ts':
    'internal job: constant-time INTERNAL_JOB_SECRET bearer verification',
  'internal/specialist-task-reminders/tick/route.ts':
    'internal job: constant-time INTERNAL_JOB_SECRET bearer verification',
  'internal/system-health-guard/tick/route.ts':
    'internal job: constant-time INTERNAL_JOB_SECRET bearer verification',
  ...
  'patient-app/client-boot-report/route.ts':
    'public: bounded telemetry uses bootstrap principal and rate limiting',
  'patient/analytics/push-open/route.ts':
    'public telemetry: service-worker/PWA push clicks are intentionally accepted without a session',
  'patient/material-ratings/route.ts':
    'mixed access: GET intentionally retains optional patient/guest semantics while PUT uses the patient business guard',
  'patient/organization-context/route.ts':
    'legacy gap: raw session access is outside the approved app-layer guard set',
  'patient/support/route.ts':
    'legacy gap: authorization lives outside the approved app-layer guard set',
  'payments/patient-acquiring-webhook/[provider]/route.ts':
    'provider webhook: provider signature is verified before processing',
  'payments/webhook/[provider]/route.ts':
    'provider webhook: provider signature is verified before processing',
  'public/support-contact-url/route.ts':
    'public: anonymous support configuration uses a bootstrap principal',
  'public/support/route.ts':
    'public: anonymous support submission uses bootstrap plus rate limiting',
  'references/[categoryCode]/route.ts':
    'public: read-only baseline uses an explicit bootstrap principal',
...
function bindingForCall(
  ...
  if (ts.isIdentifier(call.expression)) {
    return (
      imports.get(call.expression.text) ?? { module: '<local>', imported: call.expression.text }
    );
  }
...
describe('API route guard/principal AST census (§15.1)', () => {
  it('recognizes only executed, awaited guards in the same exported handler', () => {
    ...
    `)[0]?.issues.some((issue) => issue.startsWith('guard is not before buildAppDeps at line ')),
    ).toBe(true);
    expect(
      analyze(`
      export async function GET() {
    ...
    `)[0]?.issues,
    ).toContain('approved guard is not directly awaited');
    expect(
      analyze(`
  ...
  it('binds every configured guard and principal name to a real source export', () => {
    const guardExports = new Set(
      sourceFiles(GUARDS_ROOT).flatMap((file) => [...exportedValueNames(file)]),
    );
    ...
    expect(
      [...STATEMENT_PRINCIPAL_ESTABLISHERS, ...WRAPPER_PRINCIPAL_ESTABLISHERS].filter(
        (name) => !principalExports.has(name),
      ),
    ).toEqual([]);
