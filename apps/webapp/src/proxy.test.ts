...
describe('proxy (Next convention)', () => {
  ...
  it('allows browser API mutations with exact Origin or Referer', () => {
    ...
    const originResponse = proxy(
      mutationRequest('/api/auth/exchange', {
    ...
    const refererResponse = proxy(
      mutationRequest('/api/booking/public/create', {
  ...
  it('allows same-origin Server Actions and rejects cross-site or headerless actions before dispatch', async () => {
    const sameOrigin = proxy(
      mutationRequest('/app/doctor/content', {
        Origin: 'https://bersoncare.ru',
        'Sec-Fetch-Site': 'same-origin',
  ...
  it('uses exact Host, ignores X-Forwarded-Host, and validates the first forwarded proto', () => {
    const ignoredForwardedHost = proxy(
      mutationRequest('/api/auth/exchange', {
        Origin: 'https://bersoncare.ru',
    ...
    const exactLoopbackHost = proxy(
      new NextRequest('http://127.0.0.1:5200/api/auth/exchange', {
        method: 'POST',
        headers: {
          Host: '127.0.0.1:5200',
