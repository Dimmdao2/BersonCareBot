...
export type ProgramSubmissionTranscodeEnqueueResult = { ok: true } | { ok: false; error: string };
...
export async function enqueueProgramSubmissionTranscodeAfterConfirm(
  ...
  try {
    ...
    logger.warn(
      { mediaId, error: out.error },
      '[media] program_submission_transcode_enqueue_failed',
    );
