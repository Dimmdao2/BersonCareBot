...
const { returningMock, onConflictMock, valuesMock, insertMock, getDrizzleMock, loggerErrorMock } =
  vi.hoisted(() => ({
    returningMock: vi.fn(),
...
describe('recordPlaybackUserVideoFirstResolve', () => {
  ...
  it('returns false when userId is not a UUID', async () => {
    await expect(
      recordPlaybackUserVideoFirstResolve({ userId: 'tg:x', mediaId: mid }),
    ).resolves.toBe(false);
  ...
  it('returns true when insert yields a returning row (first pair)', async () => {
    ...
    await expect(recordPlaybackUserVideoFirstResolve({ userId: uid, mediaId: mid })).resolves.toBe(
      true,
    );
    ...
    expect(onConflictMock).toHaveBeenCalledWith({
      target: [
        mediaPlaybackUserVideoFirstResolveTable.userId,
        mediaPlaybackUserVideoFirstResolveTable.mediaId,
      ],
    });
