...
describe('legacyHlsBackfillCandidateWhereClause', () => {
  it('matches fetchLegacyBackfillBatch includeFailed semantics in SQL shape', () => {
    expect(legacyHlsBackfillCandidateWhereClause('x', false)).not.toContain(
      "x.video_processing_status = 'failed'",
    );
    expect(legacyHlsBackfillCandidateWhereClause('x', true)).toContain(
      "x.video_processing_status = 'failed'",
    );
  });
});
