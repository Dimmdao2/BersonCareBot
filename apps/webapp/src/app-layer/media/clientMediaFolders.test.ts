...
vi.mock('@/infra/repos/pgClientMediaFolders', () => ({
  pgEnsureClientFilesRootFolder: vi.fn(
    async (): Promise<MediaFolderRecord> => ({
      id: STUB_ROOT_ID,
      parentId: null,
      name: 'Пациенты',
  ...
  isSystemManagedMediaFolder: vi.fn(
    (kind: string) => kind === 'client_files_root' || kind === 'client_patient',
  ),
...
import { pgEnsureClientPatientFolder, pgEnsureClientFilesRootFolder } from './clientMediaFolders';
