...
describe('U5A patient organization context wiring', () => {
  it('treats the remembered organization as a server-revalidated hint at the auth chokepoint', () => {
    ...
    expect(auth).toContain(
      'stampDbPrincipalFromSession(normalized, "getCurrentSession", patientOrganizationHint)',
    );
  ...
  it('blocks care rendering on zero/ambiguous context and exposes the authorized context in the shell', () => {
    ...
    const organizationUi = source(
      'src/shared/ui/patient/organization/PatientOrganizationContext.tsx',
    );
  ...
  it('binds both reminder delivery paths to the occurrence organization', () => {
    ...
    const integratorHandler = source(
  ...
  it('maps treatment object links to an enrolled organization before changing visible context', () => {
    ...
    const capability = source(
      'db/drizzle-migrations/0216_current_patient_organization_context.sql',
    );
  ...
  it('keeps locked patient organization reads behind exact capability ACLs', () => {
    ...
    const entitlementCapability = source(
      'db/drizzle-migrations/0219_current_patient_organization_entitlements.sql',
    );
    ...
    expect(entitlementCapability).toContain(
    ...
    expect(commercialCapability).toContain(
    ...
    expect(entitlementRepository).toContain(
