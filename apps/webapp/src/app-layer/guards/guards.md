...
- **requirePatientApiBusinessAccess** — для Route Handlers: тот же критерий, что у `requirePatientAccessWithPhone`; 401 / 403 с `error: patient_activation_required` и `redirectTo` (bind-phone) при onboarding / без tier **patient** у канона. Единственное узкое чтение material-ratings использует `businessAccess: 'optional'`: общий email-gate и patient principal остаются обязательными, а `hasBusinessAccess=false` запрещает персональную часть ответа, сохраняя прежний агрегат в период активации. Метод-level гейт `scripts/check-patient-api-business-access-door.mjs` требует эту дверь для каждого обработчика `api/patient/**` и непубличного `api/booking/**`; исключения перечислены в самом гейте с причинами. **`requirePatientApiSessionWithPhone`** — алиас (устаревшее имя).
