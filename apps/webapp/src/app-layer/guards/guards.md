...
- **Сервер:** `patientRouteApiPolicy` (`patientPathRequiresBoundPhone`, `resolvePatientLayoutPathname`) + редирект в `app/app/patient/layout.tsx` (заголовки `x-bc-pathname` / `x-bc-search` из `middleware.ts`; если `x-bc-pathname` пуст — fallback по `Referer`). RSC с персональными данными из БД — **`patientRscPersonalDataGate`**.
