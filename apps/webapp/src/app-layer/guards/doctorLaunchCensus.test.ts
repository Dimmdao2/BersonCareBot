...
function collectServerActionFiles(dir: URL, result: URL[] = []): URL[] {
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    else if (/\.tsx?$/.test(entry.name) && hasUseServerDirective(readFileSync(child, 'utf8')))
      result.push(child);
  }
  return result;
}
...
const launchManifest: readonly LaunchManifestEntry[] = [
  {
    ...
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-neutral-no-pii',
  },
  {
    route: 'booking-merge/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-neutral-no-pii',
  },
  {
    route: 'analytics/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-config',
  },
  {
    route: 'analytics/clients/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-config',
  },
  {
    route: 'analytics/notifications/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-config',
  },
...
const newPlatformLaunchManifest: readonly LaunchManifestEntry[] = [
  {
    route: 'system-health/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-config',
  },
  {
    route: 'health-archive/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-config',
  },
  {
    route: 'audit-log/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-config',
  },
  {
    route: 'commercial/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-config',
  },
  ...
  {
    route: 'booking/catalog/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-config',
  },
  {
    route: 'booking/form-public/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-config',
  },
  ...
  {
    route: 'booking/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-config',
  },
  {
    route: 'booking/payments/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-config',
  },
  {
    route: 'clinics/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-config',
  },
  {
    route: 'clinics/[organizationId]/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-config',
  },
  {
    route: 'integrations/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-config',
  },
  {
    route: 'technical/page.tsx',
    launchClass: 'platform',
    capability: 'platform-operations',
    objectPolicy: 'platform-config',
  },
...
const mutableMediaManifest: readonly LaunchManifestEntry[] = [
  {
    route: 'multipart/abort/route.ts',
    launchClass: 'clinical',
    capability: 'doctor-workspace',
    objectPolicy: 'workspace-media',
  },
  {
    route: 'multipart/complete/route.ts',
    launchClass: 'clinical',
    capability: 'doctor-workspace',
    objectPolicy: 'workspace-media',
  },
  {
    route: 'multipart/init/route.ts',
    launchClass: 'clinical',
    capability: 'doctor-workspace',
    objectPolicy: 'workspace-media',
  },
  {
    route: 'multipart/part-url/route.ts',
    launchClass: 'clinical',
    capability: 'doctor-workspace',
    objectPolicy: 'workspace-media',
  },
...
const delegatedActionFiles = new Set(
  doctorServerActionManifest.filter((route) => route.endsWith('actionsInline.ts')),
);
...
describe('U1 finite doctor launch manifest', () => {
  ...
  it('recognizes valid Server Action directives despite harmless leading syntax', () => {
    ...
    expect(
      hasUseServerDirective('// action module\n"use server";\nexport async function action() {}'),
    ).toBe(true);
  ...
  it('is exact for platform URLs and marks analytics and repair as PII-free absence', () => {
    const discovered = new Set(
      collectFiles(platformRoot, 'page.tsx').map((file) =>
        fileURLToPath(file).replace(fileURLToPath(platformRoot), ''),
      ),
    );
    expect(discovered).toEqual(new Set(launchManifest.map((entry) => entry.route)));
    expect(
      launchManifest
        .filter((entry) => entry.objectPolicy === 'platform-neutral-no-pii')
        .map((entry) => entry.route),
    ).toEqual(['usage/page.tsx', 'booking-merge/page.tsx']);
  });
  ...
  it('has an exact mutable-media manifest and keeps its workspace guard', () => {
    ...
    expect(new Set(discoveredMutable)).toEqual(
      new Set(mutableMediaManifest.map((entry) => entry.route)),
    );
  });
  ...
  it('has an exact Server Action manifest and forbids raw role/session authorization', () => {
    ...
    for (const route of doctorServerActionManifest) {
      ...
      if (!delegatedActionFiles.has(route)) {
        expect(
          source.includes('requireDoctorWorkspaceContext(') ||
            source.includes('requireDoctorAccess('),
          route,
        ).toBe(true);
      }
    }
