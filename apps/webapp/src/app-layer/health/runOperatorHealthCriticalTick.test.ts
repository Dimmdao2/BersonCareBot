...
describe('runOperatorHealthCriticalTick', () => {
  beforeEach(() => {
    ...
    completeMock.mockImplementation(
      (
        input: Parameters<
          typeof inMemoryOperatorHealthWritePort.completeOutboundProviderAlertClaim
        >[0],
    ...
    releaseMock.mockImplementation(
      (
        input: Parameters<
          typeof inMemoryOperatorHealthWritePort.releaseOutboundProviderAlertClaim
        >[0],
      ) => inMemoryOperatorHealthWritePort.releaseOutboundProviderAlertClaim(input),
    );
    openOrTouchMock.mockImplementation(
      (
        input: Parameters<
          typeof inMemoryOperatorHealthWritePort.openOrTouchCriticalAlertIncident
        >[0],
      ) => inMemoryOperatorHealthWritePort.openOrTouchCriticalAlertIncident(input),
    );
    claimIncidentAlertIfDueMock.mockImplementation(
      (input: Parameters<typeof inMemoryOperatorHealthWritePort.claimIncidentAlertIfDue>[0]) =>
        inMemoryOperatorHealthWritePort.claimIncidentAlertIfDue(input),
    );
    resolveStaleMock.mockImplementation(
      (
        input: Parameters<
          typeof inMemoryOperatorHealthWritePort.resolveStaleCriticalAlertIncidents
        >[0],
    ...
    loadAdminNotificationTargetsMock.mockResolvedValue({
      telegram: ['4242'],
      max: [],
      sms: [],
      email: [],
  ...
  it('uses distinct stable T0/+1h incident-phase delivery ids', async () => {
    ...
    expect(await runOperatorHealthCriticalTick(new Date('2026-07-22T06:00:00.000Z'))).toMatchObject(
      { alerted: 1 },
    );
    expect(await runOperatorHealthCriticalTick(new Date('2026-07-22T07:00:00.000Z'))).toMatchObject(
      { alerted: 1 },
    );
    expect(await runOperatorHealthCriticalTick(new Date('2026-07-23T06:00:00.000Z'))).toMatchObject(
      { alerted: 0 },
    );
    const messageIds = relayOutboundMock.mock.calls.map(
      ([value]) => (value as { messageId: string }).messageId,
    );
    expect(messageIds).toContain(
      `operator-alert:incident:${incident.id}:phase:initial:telegram:4242`,
    );
    expect(messageIds).toContain(
      `operator-alert:incident:${incident.id}:phase:one_hour_repeat:telegram:4242`,
    );
  ...
  describe('escalation cadence (#1038)', () => {
    ...
    it('PROOF 2 — held fault escalates at T0 and again at exactly +1h (owner cadence #950), via the generic incident-cadence path', async () => {
      ...
      const recipients = relayOutboundMock.mock.calls.map(
        ([value]) => (value as { recipient: string }).recipient,
      );
    ...
    it('PROOF 4 — resolution stops the escalation, and does not keep shouting once healthy', async () => {
      ...
      const resolvedTick = await runOperatorHealthCriticalTick(
        new Date('2026-07-27T07:05:00.000Z'),
      );
      expect(resolvedTick.alerted).toBe(0);
      expect(relayOutboundMock).not.toHaveBeenCalled();

      const stillHealthy = await runOperatorHealthCriticalTick(
