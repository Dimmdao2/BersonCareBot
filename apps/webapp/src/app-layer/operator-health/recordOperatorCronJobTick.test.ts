...
vi.mock("@/infra/saasIsolationReporterRuntime", () => ({ reportSaasIsolationEventBestEffort: reportMock }));
vi.mock("@/app-layer/di/buildAppDeps", () => ({
  buildAppDeps: () => ({ operatorHealthWrite: {
...
describe("recordOperatorCronJobTickBestEffort SaaS telemetry", () => {
  ...
  it("does not infer isolation telemetry from a failed cron result", async () => {
    await recordOperatorCronJobTickBestEffort({ ...base, error: "permission denied for table media_files" });
  ...
  it("reports a recognized isolation rejection from the status write port", async () => {
    recordFailureMock.mockRejectedValueOnce(Object.assign(
      new Error("permission denied for table operator_job_status"),
      { code: "42501" },
    ));
    await recordOperatorCronJobTickBestEffort({ ...base, error: "ffmpeg timeout" });
