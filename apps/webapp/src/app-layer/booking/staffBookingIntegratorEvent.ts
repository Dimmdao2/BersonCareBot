...
function contactNameFromAppointment(appt: BeAppointment): string {
...
function serviceTitleFromAppointment(appt: BeAppointment, bookingRow?: PatientBookingRecord | null): string | null {
...
export async function emitStaffCanonicalBookingEvent(opts: {
  ...
  const contactName = bookingRow?.contactName ?? contactNameFromAppointment(opts.appointment);
  ...
  try {
    await opts.syncPort.emitBookingEvent({
      ...
      payload: {
        ...
        serviceTitleSnapshot: serviceTitleFromAppointment(opts.appointment, bookingRow),
