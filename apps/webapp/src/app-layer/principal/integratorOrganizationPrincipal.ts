...
export function isVerifiedIntegratorOrganizationId(organizationId: string): boolean {
  return UUID_RE.test(organizationId.trim());
}
...
export function enterVerifiedIntegratorOrganizationPrincipal(
  ...
  if (!isVerifiedIntegratorOrganizationId(normalized)) return false;
