  ...
  try {
    try {
        ...
        if (err.code !== 'db_transient_failure') {
          ...
          void (async () => {
            ...
            try {
              ...
              logger.warn(
                { enrichErr, reason: err.code },
                'messenger_phone_http_bind: audit enrich failed',
              );
            ...
            const auditDetails: Record<string, unknown> = {
              ...
              phoneSuffix,
              candidateIds: err.candidateIds,
              ...(validated.correlationId?.trim()
                ? { correlationId: validated.correlationId.trim() }
                : {}),
      ...
      logger.error(
        ...
        'messenger_phone_http_bind: unexpected error',
      );
      logger.warn(
        {
          ...bindLogBase,
