...
describe('buildAppDeps', () => {
  it('returns all required keys', () => {
    ...
    expect(deps).toHaveProperty('orgEntitlements');
  ...
  it('does not capture the current organization principal in the cached dependency graph', async () => {
    ...
    await runWithDbOrganizationPrincipal('cccccccc-cccc-4ccc-8ccc-cccccccccccc', async () => {
  ...
  it('doctorAppointments has listAppointmentsForSpecialist and getAppointmentStats', async () => {
    ...
    const list = await deps.doctorAppointments.listAppointmentsForSpecialist({
      kind: 'range',
      range: 'today',
    });
    const stats = await deps.doctorAppointments.getAppointmentStats({
      kind: 'range',
      range: 'today',
    });
    expect(Array.isArray(list)).toBe(true);
  ...
  it('diaries exposes listSymptomEntries, createSymptomTracking, ensureGeneralWellbeingTracking, ensureWarmupFeelingTracking, upsertWarmupFeelingTrackingIdInTx, listSymptomTrackings, addSymptomEntry, createLfkComplex, listLfkComplexes, listLfkSessions, addLfkSession, stats helpers', () => {
    const deps = buildAppDeps();
    ...
    expect(deps.diaries).toHaveProperty('ensureWarmupFeelingTracking');
    expect(deps.diaries).toHaveProperty('upsertWarmupFeelingTrackingIdInTx');
    ...
    expect(typeof deps.diaries.ensureWarmupFeelingTracking).toBe('function');
    expect(typeof deps.diaries.upsertWarmupFeelingTrackingIdInTx).toBe('function');
