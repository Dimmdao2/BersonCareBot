...
function sql(text) {
  return run(
    ...
    ['-h', socket, '-p', port, '-v', 'ON_ERROR_STOP=1', '-At', '-d', db],
    text,
  );
}
...
function sqlAsync(text) {
  return new Promise((resolve, reject) => {
    const child = spawn(path.join(pgBin, 'psql'), [
      '-h',
      socket,
      '-p',
      port,
    ...
    child.on('close', (code) =>
      code === 0 ? resolve() : reject(new Error(`concurrent psql failed: ${stderr}`)),
    );
...
function assertEq(actual, expected, label) {
  if (actual.trim() !== expected)
    throw new Error(`${label}: expected ${expected}, got ${actual.trim()}`);
}
...
function assertRepairAndFreshChain() {
  ...
  assertEq(
    sql(`SELECT count(*) FROM be_organization_members WHERE role='admin';`),
    '0',
    'fresh 0143 chain does not seed global admin',
  );
  ...
  assertEq(
    sql(
      `SELECT count(*) FROM be_organization_members WHERE organization_id='${defaultOrg}' AND role='admin';`,
    ),
    '0',
    'repair removes every old seed shape',
  );
  assertEq(
    sql(`SELECT count(*) FROM platform_users WHERE id IN ('${globalA}','${globalB}');`),
    '2',
    'repair preserves platform users',
  );
  assertEq(
    sql(`SELECT role FROM be_organization_members WHERE platform_user_id='${owner}';`),
    'owner',
    'owner promotion preserved',
  );
  file('apps/webapp/db/drizzle-migrations/0207_remove_seeded_global_admin_membership.sql');
  assertEq(
    sql(`SELECT count(*) FROM be_organization_members WHERE platform_user_id='${owner}';`),
    '1',
    'repair repeat is idempotent',
  );
  const duplicate = spawnSync(
    path.join(pgBin, 'psql'),
    ['-h', socket, '-p', port, '-v', 'ON_ERROR_STOP=1', '-d', db],
    {
      encoding: 'utf8',
      input: `INSERT INTO be_organization_members (organization_id, platform_user_id, role) VALUES ('${defaultOrg}', '${owner}', 'owner');`,
    },
  );
  if (duplicate.status === 0)
    throw new Error('membership unique invariant did not reject duplicate');
}
...
async function assertInviteLifecycleAndConcurrency() {
  ...
  if (newIdentityFirst.length !== 2 || newIdentityFirst[1] !== 'true') {
    throw new Error(
      `new email identity was not created exactly once: ${newIdentityFirst.join('|')}`,
    );
  }
  ...
  assertEq(
    sql(`SELECT count(*) FROM platform_users WHERE email_normalized='${newInviteEmail}';`),
    '1',
    'new email creates one platform user',
  );
  ...
  assertEq(
    sql(`SELECT count(*) FROM platform_users WHERE email_normalized='${existingInviteEmail}';`),
    '1',
    'existing identity remains one platform user',
  );
  assertEq(
    sql(
      `SELECT count(*) FROM be_organization_members WHERE organization_id='${defaultOrg}' AND platform_user_id='${inviteUser}';`,
    ),
    '1',
    'existing identity accept creates one membership',
  );
  assertEq(
    sql(
      `SELECT count(*) FROM be_organization_members WHERE organization_id='${defaultOrg}' AND platform_user_id='${newInviteUser}';`,
    ),
    '1',
    'new identity accept creates one membership',
  );
  assertEq(
    sql(
      `SELECT code FROM app.accept_org_invite('existing-accept-token', '${inviteUser}', '${existingInviteEmail}');`,
    ),
    'reused_token',
    'accept replay rejected',
  );
  sql(`INSERT INTO organization_member_invites (organization_id, invited_email, invited_role, token_hash, expires_at, created_by_platform_user_id)
  ...
  assertEq(
    sql(
      `SELECT code FROM app.accept_org_invite('expired-token', '${inviteUser}', 'expired@example.test');`,
    ),
    'expired_token',
  ...
  assertEq(
    sql(
      `SELECT code FROM app.accept_org_invite('revoked-token', '${inviteUser}', 'revoked@example.test');`,
    ),
    'reused_token',
  ...
  assertEq(
    sql(
      `SELECT count(*) FROM be_specialists s JOIN be_organization_members m ON m.specialist_id=s.id WHERE m.organization_id='${defaultOrg}' AND m.platform_user_id='${inviteUser}';`,
    ),
    '1',
    'concurrent first login creates exactly one specialist',
  );
  ...
  assertEq(
    sql(
      `SELECT count(*) FROM organization_member_invites WHERE organization_id='${defaultOrg}' AND invited_email='replace@example.test' AND status='pending';`,
    ),
    '1',
    'concurrent replacement leaves one pending invite',
  );
}
