...
function runtimeSources(relativeRoot) {
  ...
  const visit = (absolutePath) => {
    ...
    if (!/\.(?:[cm]?js|tsx?)$/.test(absolutePath) || /\.(?:test|spec)\.[^.]+$/.test(absolutePath))
      return;
    found.push({
...
forbidText(
  ...
  'reachable_integrator_clear_phone',
);
forbidText(
  'operations',
  operations,
  'await integratorPurgeUserById(arg1)',
  'reachable_integrator_purge_user',
);
...
for (const item of runtimeSources('src/app')) {
  forbidText(
    ...
    'runtime_strict_purge_entrypoint',
  );
  forbidText(
    item.relativePath,
    item.source,
    'purgePlatformUserByPlatformId',
    'runtime_full_purge_entrypoint',
  );
}
for (const item of runtimeSources('src/app/app/doctor')) {
  ...
  forbidText(
    item.relativePath,
    item.source,
    'doctor-client-permanent-delete-btn',
    'destructive_account_purge_ui',
  );
}
for (const item of runtimeSources('scripts')) {
  ...
  forbidText(
    ...
    item.source,
    'runStrictPurgePlatformUser',
    'operational_strict_purge_entrypoint',
  );
  forbidText(
    item.relativePath,
    item.source,
    'purgePlatformUserByPlatformId',
    'operational_full_purge_entrypoint',
  );
  forbidPattern(
    item.relativePath,
    item.source,
...
console.log(
  'Administrative account purge is fail-closed; strict core and media cleanup remain present.',
);
