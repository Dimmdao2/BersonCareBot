...
function runSelfTest() {
  ...
  assertSelfTestIssue(
    "missing-import",
    (sources) => sources.set(ROUTES[1], sources.get(ROUTES[1]).replace("@/shared/http/apiResponse", "@/shared/http/missing")),
    `missing_helper_import:${ROUTES[1]}`,
  );
  assertSelfTestIssue(
    "raw-message",
    (sources) => sources.set(ROUTES[2], `${sources.get(ROUTES[2])}\njsonError(error.message, {}, { status: 500 });\n`),
    `raw_or_derived_error_message:${ROUTES[2]}`,
  );
  assertSelfTestIssue(
    "boundary-import",
    (sources) => sources.set(ROUTES[3], `${sources.get(ROUTES[3])}\nimport { getPool } from "@/infra/db/client";\n`),
    `new_boundary_import:${ROUTES[3]}:@/infra/db/client`,
  );
  assertSelfTestIssue(
    "direct-response",
    (sources) => sources.set(ROUTES[4], `${sources.get(ROUTES[4])}\nNextResponse.json({ ok: false });\n`),
...
if (issues.length > 0) {
  process.stderr.write(`E2 API response contract: FAIL\n${issues.map((issue) => `- ${issue}`).join("\n")}\n`);
  process.exitCode = 1;
} else {
