...
requireText(onlineIndex, "ARRAY[false, false, false, true]::boolean[]", "online index direction postcheck");
...
for (const [source, label] of [[migration, "migration"], [schema, "Drizzle schema"]]) {
  ...
  const drizzleOrganizationAt = source.indexOf("table.organizationId.asc()", source.indexOf("idx_patient_lfk_assign_active_template"));
...
requireText(migration, "duplicate active patient LFK assignment inside one organization", "assignment duplicate preflight");
requireText(merge, "a.organization_id = b.organization_id", "platform merge exact-org assignment conflict guard");
requireText(mergePreview, "a.organization_id = b.organization_id", "merge preview exact-org assignment conflict guard");
