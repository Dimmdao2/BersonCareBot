...
pnpm --dir apps/webapp exec tsx scripts/integrator-schema-cleanup/02_backfill.ts
...
3. Use `02_backfill.ts` only for explicit compatibility mirror reconciliation, not as a product migration.
4. Use `04_disable_writers.ts` to print the exact owner-gated settings/writers that must be disabled before destructive cleanup.
5. Use `05_drop_deprecated.ts` only to generate blocked/drop-safety output. It refuses to print destructive SQL unless a table is in the locally safe allowlist and source references are clear.
