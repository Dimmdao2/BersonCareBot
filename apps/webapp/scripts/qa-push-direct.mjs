...
try {
  ...
  if (s.rows.length === 0) {
    console.error(
      'No subscriptions for the test user — nothing to send (he must enable notifications in the app).',
    );
    process.exit(0);
  }
  ...
  for (const r of s.rows) {
    try {
      ...
      console.error(
        `web_push FAILED: HTTP ${e?.statusCode ?? '?'} ${String(e?.body ?? e).slice(0, 200)}`,
      );
    }
  }
