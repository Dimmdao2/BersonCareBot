...
/**
 * Proof 9 / §5a item 2.8: the terminal state is a Stripe-shaped choice of exactly three genuinely
 * distinct behaviours — «оставить доступ» (full_access) · «только чтение» (read_only) · «скрыть
 * раздел» (disabled) — not one hardcoded "off". Same days, same anchor, only terminalState varies.
 */
async function proveTerminalStateVariantsAreDistinct(connection) {
  const policyWith = (terminalState) => ({
    graceDays: 1,
    readOnlyDays: 1,
    notifications: [],
    terminalState,
  });
  // Well past graceDays(1) + readOnlyDays(1): every variant below is read at the TERMINAL rung.
  await setDegradationAnchor(connection, { endsAtIntervalFromNow: '-10 days' });

  await setPolicies(connection, {
    systemAccessPolicy: null,
    mechanicAccessPolicies: { courses: policyWith('full_access') },
  });
  const full = await resolveAs(connection, ORG_ID, 'courses');
  if (full.state !== 'full_access' || full.mutation_allowed !== true) {
    fail(`terminalState "full_access" (оставить доступ) must keep the mechanic fully mutable, got ${JSON.stringify(full)}`);
  }

  await setPolicies(connection, {
    systemAccessPolicy: null,
    mechanicAccessPolicies: { courses: policyWith('read_only') },
  });
  const readOnly = await resolveAs(connection, ORG_ID, 'courses');
  if (readOnly.state !== 'read_only' || readOnly.mutation_allowed !== false) {
    fail(`terminalState "read_only" (только чтение) must refuse mutation while staying visible, got ${JSON.stringify(readOnly)}`);
  }

  await setPolicies(connection, {
    systemAccessPolicy: null,
    mechanicAccessPolicies: { courses: policyWith('disabled') },
  });
  const disabled = await resolveAs(connection, ORG_ID, 'courses');
  if (disabled.state !== 'disabled' || disabled.mutation_allowed !== false) {
    fail(`terminalState "disabled" (скрыть раздел) must resolve to disabled, got ${JSON.stringify(disabled)}`);
  }

  if (new Set([full.state, readOnly.state, disabled.state]).size !== 3) {
    fail(
      `the three terminal-state variants must resolve to three distinct states, got ` +
        `${JSON.stringify([full.state, readOnly.state, disabled.state])}`,
    );
  }
}

/**
 * Proof 10 / §5a item 2.9: the ladder's days AND terminal state are DATA end to end. The SAME
 * running function, the SAME open connection, only `UPDATE public.saas_tariffs` in between — no
 * restart, no migration, no code touched — and behaviour still moves.
 */
async function provePolicyEditsApplyLiveNoCodeChange(connection) {
  // Fixed anchor for the whole proof — only the tariff row changes below.
  await setDegradationAnchor(connection, { endsAtIntervalFromNow: '-4 days' });

  await setPolicies(connection, {
    systemAccessPolicy: null,
    mechanicAccessPolicies: {
      courses: { graceDays: 10, readOnlyDays: 10, notifications: [], terminalState: 'read_only' },
    },
  });
  const wideGrace = await resolveAs(connection, ORG_ID, 'courses');
  if (wideGrace.state !== 'grace') {
    fail(`4 days into a 10-day grace expected grace, got ${wideGrace.state}`);
  }

  // Shorten graceDays only — a plain UPDATE, nothing else changes.
  await setPolicies(connection, {
    systemAccessPolicy: null,
    mechanicAccessPolicies: {
      courses: { graceDays: 1, readOnlyDays: 10, notifications: [], terminalState: 'read_only' },
    },
  });
  const shortenedGrace = await resolveAs(connection, ORG_ID, 'courses');
  if (shortenedGrace.state !== 'read_only') {
    fail(
      `editing graceDays alone in the tariff row (10 -> 1) must move behaviour to read_only with ` +
        `zero code change, got ${shortenedGrace.state}`,
    );
  }

  // Shorten readOnlyDays past the whole ladder and flip terminalState — still one UPDATE.
  await setPolicies(connection, {
    systemAccessPolicy: null,
    mechanicAccessPolicies: {
      courses: { graceDays: 1, readOnlyDays: 1, notifications: [], terminalState: 'disabled' },
    },
  });
  const disabledTerminal = await resolveAs(connection, ORG_ID, 'courses');
  if (disabledTerminal.state !== 'disabled') {
    fail(
      `editing terminalState in the tariff row must flip terminal behaviour with zero code change, ` +
        `got ${disabledTerminal.state}`,
    );
  }
  ...
  await setPolicies(connection, {
    systemAccessPolicy: null,
    mechanicAccessPolicies: {
      courses: { graceDays: 1, readOnlyDays: 1, notifications: [], terminalState: 'full_access' },
    },
  });
  const fullAccessTerminal = await resolveAs(connection, ORG_ID, 'courses');
  if (fullAccessTerminal.state !== 'full_access' || fullAccessTerminal.mutation_allowed !== true) {
    fail(
      `editing terminalState to full_access must keep mutation open with zero code change, got ` +
        `${JSON.stringify(fullAccessTerminal)}`,
    );
  }
}

/**
 * Proof 11 / §5a item 2.10: a policy edit acts FORWARD on an organization already on the ladder —
 * lengthening never blocks it, shortening moves it exactly as far as real elapsed time now places
 * it (not straight to terminal), and the accumulated time (the original anchor) is never reset or
 * lost across any of the edits.
 */
async function proveForwardRecomputeNotRetroactive(connection) {
  // Fixed anchor for the whole proof: 3 real days have already elapsed on this organization's clock.
  await setDegradationAnchor(connection, { endsAtIntervalFromNow: '-3 days' });
  const anchorBefore = await connection.query(
    'SELECT ends_at FROM public.saas_organization_trials WHERE organization_id = $1',
    [ORG_ID],
  );
  ...
  await setPolicies(connection, {
    systemAccessPolicy: null,
    mechanicAccessPolicies: {
      courses: { graceDays: 5, readOnlyDays: 5, notifications: [], terminalState: 'disabled' },
    },
  });
  const inGrace = await resolveAs(connection, ORG_ID, 'courses');
  if (inGrace.state !== 'grace') fail(`3 days into a 5-day grace expected grace, got ${inGrace.state}`);

  // (a) Lengthen grace — организация в терпении не блокируется при удлинении.
  await setPolicies(connection, {
    systemAccessPolicy: null,
    mechanicAccessPolicies: {
      courses: { graceDays: 10, readOnlyDays: 5, notifications: [], terminalState: 'disabled' },
    },
  });
  const lengthened = await resolveAs(connection, ORG_ID, 'courses');
  if (lengthened.state !== 'grace' || lengthened.mutation_allowed !== true) {
    fail(`lengthening grace on an organization already in grace must not block it, got ${JSON.stringify(lengthened)}`);
  }

  // (b) Shorten grace below the 3 already-elapsed days, but readOnly still covers them — the
  // organization must land exactly on read_only (what 3 real days now means under this policy),
  // NOT be thrown straight to the terminal state as an instant penalty for editing the policy.
  await setPolicies(connection, {
    systemAccessPolicy: null,
    mechanicAccessPolicies: {
      courses: { graceDays: 1, readOnlyDays: 5, notifications: [], terminalState: 'disabled' },
    },
  });
  const shortened = await resolveAs(connection, ORG_ID, 'courses');
  if (shortened.state !== 'read_only' || shortened.mutation_allowed !== false) {
    fail(
      `shortening grace below the elapsed 3 days must move the organization forward to read_only ` +
        `(not skip it), got ${JSON.stringify(shortened)}`,
    );
  }

  // (c) Shorten past the whole (now tiny) ladder — the organization finally reaches the terminal
  // state, but only because 3 real elapsed days now exceed it, continuing from the SAME anchor.
  await setPolicies(connection, {
    systemAccessPolicy: null,
    mechanicAccessPolicies: {
      courses: { graceDays: 1, readOnlyDays: 1, notifications: [], terminalState: 'disabled' },
  ...
  const atTerminal = await resolveAs(connection, ORG_ID, 'courses');
  if (atTerminal.state !== 'disabled') {
    fail(`3 elapsed days past a 1+1-day ladder must reach the terminal state, got ${atTerminal.state}`);
  }

  // «без потери накопленного» — editing the tariff's ladder never touches the organization's own
  // degradation anchor; the accumulated 3 days are the same 3 days throughout.
  const anchorAfter = await connection.query(
    'SELECT ends_at FROM public.saas_organization_trials WHERE organization_id = $1',
    [ORG_ID],
  );
  if (anchorBefore.rows[0]?.ends_at?.getTime() !== anchorAfter.rows[0]?.ends_at?.getTime()) {
    fail('editing the tariff ladder must not move the organization degradation anchor');
  }
}
...
const REGRESSIONS = [
  ...
  {
    // §5a item 2.8 — collapses the owner's three-way choice back into a single hardcoded value.
    label: 'terminal state variants collapsed to a single hardcoded value',
    proof: proveTerminalStateVariantsAreDistinct,
    breakSource: (source) =>
      source.replace(
        `WHEN degradation_started_at IS NOT NULL THEN policy ->> 'terminalState'`,
        `WHEN degradation_started_at IS NOT NULL THEN 'disabled'`,
      ),
  },
  {
    // §5a item 2.9 — an agent hardcodes graceDays instead of reading the tariff row: exactly the
    // shape 2.6's TS-side `ladderConstants.ts` checker forbids, on the SQL side where it cannot look.
    label: 'graceDays hardcoded in the door function — editing the tariff row stops doing anything',
    proof: provePolicyEditsApplyLiveNoCodeChange,
    breakSource: (source) => source.replaceAll(`(policy ->> 'graceDays')::integer`, `10`),
  },
  {
    // §5a item 2.10 — the anchor gets re-read as "now" instead of preserved: an organization's
    // accumulated ladder time is silently lost on every call.
    label: 'degradation anchor reset to now — accumulated ladder time is lost',
    proof: proveForwardRecomputeNotRetroactive,
    breakSource: (source) =>
      source.replace(
        `COALESCE(trial.ends_at, paid_period.period_ends_at) AS degradation_started_at,`,
        `v_now AS degradation_started_at,`,
      ),
  },
];
...
try {
  ...
  await withDb(primaryDb, async (connection) => {
    ...
    await proveTerminalStateVariantsAreDistinct(connection);
    console.log('  proof OK: full_access/read_only/disabled terminal states are three genuinely distinct behaviours (§5a 2.8)');
    await provePolicyEditsApplyLiveNoCodeChange(connection);
    console.log('  proof OK: editing days and terminal state in the tariff row is live, zero restart, zero code change (§5a 2.9)');
    await proveForwardRecomputeNotRetroactive(connection);
    console.log('  proof OK: policy edits act forward on an organization already on the ladder, accumulated time is never lost (§5a 2.10)');
  });
