...
function runtimeDefinitions() {
  const migration = readFileSync(
    path.join(
      root,
      'apps/webapp/db/drizzle-migrations/0209_s5_runtime_settings_audit_contract.sql',
    ),
    'utf8',
  );
...
function installMinimalPredecessorAndFixture() {
  ...
  apply(
    'apps/webapp/db/drizzle-migrations/0186_app_runtime_settings.sql',
    '0186 minimal runtime root',
  );

  const normalDefinitions = runtimeDefinitions().filter(
    ({ key }) => key !== 'patient_booking_url' && key !== defaultedRuntimeKey,
  );
  const normalRows = normalDefinitions.map(
    ({ key, scope }) =>
...
function assertIdempotenceAndAuditTransaction() {
  sql(
    ...
    'clear backfill audit evidence before idempotence probe',
  );
  apply(
    'apps/webapp/db/drizzle-migrations/0209_s5_runtime_settings_audit_contract.sql',
    'idempotent 0209 reapply',
  );
  assertSqlTrue(
...
function assertS53DualWriteTriggerContract() {
  ...
  sql(
    'TRUNCATE public.app_runtime_settings_audit;',
    'clear audit evidence before dual-write probe',
  );
  sql(
    `
  ...
  sql(
    'TRUNCATE public.app_runtime_settings_audit;',
    'clear audit evidence before payment projection probe',
  );
  sql(
    `
    INSERT INTO public.system_settings (key, scope, organization_id, value_json, updated_at, updated_by)
  ...
  sql(
    'TRUNCATE public.app_runtime_settings_audit;',
    'clear audit evidence before OAuth projection probe',
  );
  sql(
    `
    UPDATE public.system_settings
  ...
  sql(
    'TRUNCATE public.app_runtime_settings_audit;',
    'clear audit evidence before SMS projection probe',
  );
  sql(
    `
    UPDATE public.system_settings
  ...
  sql(
    'TRUNCATE public.app_runtime_settings_audit;',
    'clear audit evidence before explicit dual-write probe',
  );
  sql(
    `
    BEGIN;
  ...
  sql(
    'TRUNCATE public.app_runtime_settings_audit;',
    'clear audit evidence before manual legacy probe',
  );
  sql(
    `
    UPDATE public.system_settings
