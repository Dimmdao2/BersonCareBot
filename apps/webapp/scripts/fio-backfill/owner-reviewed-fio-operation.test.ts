...
describe("owner-reviewed FIO manifest contract", () => {
  it("is hash-bound and rejects mutation", () => {
    ...
    expect(() => parseAndVerifyManifest({ ...manifest, rows: [{ ...manifest.rows[0], id: ID_B }] })).toThrow(
      "SHA-256 mismatch",
    );
  });

  it("rejects duplicate identities and invalid exception bindings", () => {
    expect(() => makeManifest([manifestPayload().rows[0], manifestPayload().rows[0]])).toThrow("duplicate IDs");
    expect(() =>
      buildManifest({
        ...manifestPayload(),
        exceptions: { expectedMissing: [{ id: ID_B, reference: "not-in-manifest" }], preserveCurrent: [] },
      }),
    ...
    expect(() =>
      buildManifest({
        ...
        exceptions: {
          ...
          preserveCurrent: [{ id: ID_A, exactCurrent: REVIEW_TIME_DRIFT, reference: "known-drift" }],
        },
      }),
  ...
  it("requires explicit TEST, exact database, and exact loopback", () => {
    expect(() => assertTestTarget("postgres://u:p@127.0.0.1/bersoncarebot_test", "bersoncarebot_test", false)).toThrow(
      "--test",
    );
    expect(() => assertTestTarget("postgres://u:p@localhost/bersoncarebot_test", "bersoncarebot_test", true)).toThrow(
      "127.0.0.1",
    );
    expect(() => assertTestTarget("postgres://u:p@127.0.0.1/bcb_webapp_prod", "bcb_webapp_prod", true)).toThrow(
      "bersoncarebot_test",
    );
    expect(() =>
...
describe("owner-reviewed FIO apply and rollback", () => {
  ...
  it("rejects symlink JSON inputs and symlinked rollback directory parents", async () => {
    ...
    await expect(writer.writeBeforeMutation(buildRollbackArtifact(manifest, plan.updates, manifest.createdAt))).rejects.toThrow(
      "real directories",
    );
  });
  ...
  it("writes a durable 0600 rollback artifact before the conditional update", async () => {
    ...
    const result = await applyOwnerReviewedFio(manifest, db, createDurableRollbackWriter(directory), () =>
    ...
    const artifact = parseAndVerifyRollbackArtifact(JSON.parse(await readFile(result.artifactPath!, "utf8")));
  ...
  it("keeps distinct durable artifacts for repeated attempts of the same manifest", async () => {
    ...
    const first = await applyOwnerReviewedFio(manifest, memoryDb([current(ID_A, BEFORE)]).db, writer, fixedNow);
    const second = await applyOwnerReviewedFio(manifest, memoryDb([current(ID_A, BEFORE)]).db, writer, fixedNow);
  ...
  it("does not write when artifact creation fails and rolls back a later conditional failure", async () => {
    ...
    await expect(
      applyOwnerReviewedFio(manifest, first.db, { writeBeforeMutation: async () => Promise.reject(new Error("disk")) }),
    ...
    second.db.transaction = (run) =>
      originalTransaction(async (tx) =>
        run({ ...tx, conditionalUpdate: async () => false }),
      );
    await expect(
      applyOwnerReviewedFio(manifest, second.db, { writeBeforeMutation: async () => "/secure/artifact" }),
  ...
  it("aborts an unlisted missing row and allows only its exact hash-bound exception", async () => {
    await expect(
      applyOwnerReviewedFio(makeManifest(), memoryDb([]).db, { writeBeforeMutation: async () => "/unused" }),
  ...
  it("preserves only the exact reviewed drift and aborts a later edit", async () => {
    ...
    const exactExceptionManifest = buildManifest({
      ...
      exceptions: {
        expectedMissing: [],
        preserveCurrent: [
          { id: ID_A, exactCurrent: REVIEW_TIME_DRIFT, reference: "owner-reviewed-changed-row-exception" },
        ],
      },
    });
  ...
  it("rolls back only from the exact recorded post-apply state", async () => {
    ...
    await expect(rollbackOwnerReviewedFio(captured!, changedAfterApply.db)).rejects.toThrow("rollback conflict");
