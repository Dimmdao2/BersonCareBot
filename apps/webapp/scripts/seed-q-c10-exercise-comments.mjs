...
async function main() {
  ...
  try {
    for (const rec of seedRecords) {
      ...
      console.log(
        `  INSERT ${rec.created_at} — reps=${rec.reps}, weightKg=${rec.weightKg}, difficulty=${rec.perceivedDifficulty} — id=${result.rows[0].id}`,
      );
      insertedCount++;
    }
  } finally {
  ...
  console.log(
    `  Metrics present:         reps (green line), weightKg (blue line), difficulty (red line)`,
  );
}
