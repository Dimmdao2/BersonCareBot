...
main().catch((error) => {
  process.stderr.write(
    `patient-invite-disposable-proof: FAIL: ${error instanceof Error ? error.message : String(error)}\n`,
  );
  process.exitCode = 1;
});
...
async function main() {
  try {
    ...
    process.stdout.write(
      'patient-invite-disposable-proof: PASS (0220→0221→legacy-bound fixture→0222, retry-safe no-contact card command, claim, rollback guard, reissue, single-use, cross-org, concurrent redeem/claim/issue/registration)\n',
    );
  } finally {
    cleanup();
  }
...
function run(command, args, input) {
  ...
  if (result.error || result.status !== 0) {
    const detail =
      result.stderr?.trim() || result.stdout?.trim() || result.error?.message || 'unknown';
...
function psql(sql) {
  run(
    ...
    ['-h', socket, '-p', port, '-d', dbName, '-X', '-v', 'ON_ERROR_STOP=1'],
    sql,
  );
}
...
async function proveForwardState(db) {
  ...
  assert(
    result.rows[0]?.portal_activated_at == null,
    'legacy active relationship was guessed as portal-linked',
  );
  assert(
    result.rows[0]?.superseded_by_invite_id === newInvite,
    'replacement FK was not linked after insert',
  );
  ...
  assert(
    legacy.rows[0]?.recipient_binding === 'bound_email' &&
...
async function proveBearerAndProof(db) {
  ...
  const wrongRecipient = await db.query(
    ...
    [
      ...
      'wrong-proof-hash',
      proofExpiresAt,
      wrongRecipientAuth.nonce,
      wrongRecipientAuth.expiresEpoch,
      wrongRecipientAuth.signature,
    ],
  );
  assert(
    wrongRecipient.rows[0]?.ok === false && wrongRecipient.rows[0]?.code === 'wrong_recipient',
    'wrong recipient did not keep the correct recovery state',
  );
  const startAuth = proofAuthorization(
    'start',
    'continuation-a',
  ...
  assert(
    cooldown.rows[0]?.ok === false && cooldown.rows[0]?.code === 'rate_limited',
    'proof resend cooldown was bypassed',
  );
  ...
  const verifyAuth = proofAuthorization(
    'verify',
    'continuation-a',
    'patient-a@example.test',
    'proof-hash',
  ...
  const verified = await db.query(
    ...
    [
      ...
      'proof-hash',
      verifyAuth.nonce,
      verifyAuth.expiresEpoch,
      verifyAuth.signature,
    ],
  );
...
async function proveConcurrentRedeem() {
  ...
  try {
    ...
    try {
      await admin.query(
        'INSERT INTO app.patient_proof_principals(backend_pid,patient_user_id) VALUES($1,$3),($2,$3)',
        [firstPid.rows[0].pid, secondPid.rows[0].pid, patientA],
      );
    } finally {
      await admin.end();
    }
    ...
    assert(
      rows.filter((row) => row?.ok === true).length === 1,
      'concurrent redeem did not have one winner',
    );
    assert(
      rows.filter((row) => row?.ok === false && row?.code === 'already_linked').length === 1,
      'concurrent loser was not terminal',
    );
  } finally {
...
async function claimUnbound(db, continuationHash, email) {
  ...
  return db.query('SELECT * FROM app.claim_unbound_patient_invite_email($1,$2,$3,$4,$5)', [
    continuationHash,
    email,
    authorization.nonce,
    authorization.expiresEpoch,
    authorization.signature,
  ]);
}

async function proveUnboundClaim(db) {
  await prepareUnboundInvite(
    db,
    'unbound-token',
  ...
  try {
    ...
    assert(
      rows.filter(
        (row) =>
          row?.ok === true && row?.organization_id === orgA && row?.patient_user_id === patientC,
      ).length === 2,
  ...
  assert(
    row?.email_normalized === 'new@example.test' && row?.email_verified_at != null,
    'unbound claim did not attach the verified email to the placeholder',
  );
  assert(
    row?.platform_user_id === patientC &&
      row?.invite_patient_user_id === patientC &&
      row?.care_ref_count === 1,
    'unbound claim rebound an existing patient FK',
  );
  assert(
    row?.status === 'active' &&
      row?.portal_activated_at != null &&
  ...
  const retryVerify = await db.query(
    ...
    [
      ...
      'new@example.test',
      'unbound-code',
      retryVerifyAuth.nonce,
      retryVerifyAuth.expiresEpoch,
      retryVerifyAuth.signature,
    ],
  );
  assert(
    retryVerify.rows[0]?.ok === true,
    'accepted unbound invite did not retain the exact valid proof for session retry',
  );
  const retryLookup = await db.query('SELECT * FROM app.lookup_patient_invite_continuation($1)', [
    'continuation-unbound',
  ]);
  assert(
    retryLookup.rows[0]?.ok === true && retryLookup.rows[0]?.recipient_hint == null,
  ...
  const wrongCode = await db.query(
    ...
    [
      ...
      'wrong-code',
      wrongCodeAuth.nonce,
      wrongCodeAuth.expiresEpoch,
      wrongCodeAuth.signature,
    ],
  );
  assert(
    wrongCode.rows[0]?.ok === false && wrongCode.rows[0]?.code === 'invalid_code',
  ...
  assert(
    wrongEmailClaim.rows[0]?.ok === false &&
...
async function proveRepeatedConflict(db) {
  ...
  assert(
    first.rows[0]?.ok === false && first.rows[0]?.code === 'conflicting_identity',
    'occupied email was not rejected',
  );
  assert(
    second.rows[0]?.ok === false && second.rows[0]?.code === 'conflicting_identity',
  ...
  const row = state.rows[0];
  assert(
    row?.email_normalized == null && row?.status === 'invited' && row?.portal_activated_at == null,
    'conflict partially mutated the placeholder or enrollment',
  );
  assert(
    row?.invite_status === 'pending' && row?.candidate_count === 1,
...
async function proveRegistrationRace() {
  ...
  try {
    ...
    if (placeholderWon) {
      assert(
        row?.email_normalized === 'race@example.test' &&
          row?.status === 'active' &&
          row?.portal_activated_at != null &&
          row?.invite_status === 'accepted',
...
async function issueUnboundInvite(db, id, tokenHash) {
  ...
  try {
    ...
    if (previous.rows[0]?.id) {
      await db.query('UPDATE public.patient_invites SET superseded_by_invite_id=$1 WHERE id=$2', [
        id,
        previous.rows[0].id,
      ]);
    }
...
async function proveConcurrentIssue() {
  ...
  try {
    ...
    assert(
      state.rows[0]?.pending_count === 1 && state.rows[0]?.superseded_count === 1,
...
async function executeManualCardCommand(
  db,
  organizationId,
  fingerprint,
  ...
  try {
    await db.query('SELECT pg_advisory_xact_lock(hashtextextended($1::text,0))', [
      `doctor-manual-patient:${manualCardCommand}`,
    ]);
    const existing = await db.query(
      'SELECT organization_id,command_kind,request_fingerprint,platform_user_id FROM public.manual_patient_commands WHERE command_id=$1',
      [manualCardCommand],
    );
    ...
    await db.query(
      "INSERT INTO public.platform_users(id,role,display_name,last_name,first_name) VALUES($1,'client','Иванов Иван','Иванов','Иван')",
      [patientId],
    );
    await db.query(
      "INSERT INTO public.org_enrollments(id,organization_id,platform_user_id,status) VALUES($1,$2,$3,'invited')",
      [randomUUID(), organizationId, patientId],
    );
    await db.query(
      'INSERT INTO public.manual_patient_commands(command_id,organization_id,command_kind,request_fingerprint,platform_user_id) VALUES($1,$2,$3,$4,$5)',
      [manualCardCommand, organizationId, kind, fingerprint, patientId],
    );
    await db.query('COMMIT');
    return patientId;
  } catch (error) {
...
async function proveManualCardCommandReplay() {
  ...
  try {
    ids = await Promise.all([
      executeManualCardCommand(left, orgA, fingerprint),
      executeManualCardCommand(right, orgA, fingerprint),
    ]);
  } finally {
  ...
  try {
    assert(
      (await executeManualCardCommand(db, orgA, fingerprint)) === ids[0],
      'exact replay returned another patient',
    );
    for (const args of [
      [orgA, changed],
      [orgB, fingerprint],
      [orgA, fingerprint, 'walk_in'],
    ]) {
      let rejected = false;
      try {
        await executeManualCardCommand(db, ...args);
      } catch (error) {
        rejected = error instanceof Error && error.message === 'idempotency_conflict';
      }
      assert(rejected, 'payload, organization, or kind collision was accepted');
    }
    const count = await db.query(
      'SELECT count(*)::int commands,count(DISTINCT c.platform_user_id)::int users,count(DISTINCT e.id)::int enrollments FROM public.manual_patient_commands c JOIN public.org_enrollments e ON (e.organization_id,e.platform_user_id)=(c.organization_id,c.platform_user_id) WHERE c.command_id=$1',
      [manualCardCommand],
    );
    assert(
      count.rows[0]?.commands === 1 &&
        count.rows[0]?.users === 1 &&
        count.rows[0]?.enrollments === 1,
      'replay duplicated user or enrollment',
    );
  } finally {
    await db.end();
  }
...
async function proveStaffCrossOrg(db) {
  ...
  try {
    ...
    await db.query("SELECT set_config('app.staff','1',true), set_config('app.org',$1,true)", [
      orgA,
    ]);
    const result = await db.query(
      'SELECT count(*)::int AS count FROM public.patient_invites WHERE organization_id=$1',
      [orgB],
    );
    ...
    const manual = await db.query(
      'SELECT count(*)::int AS count FROM public.manual_patient_commands WHERE organization_id=$1',
      [orgA],
    );
...
async function proveAclAndForce(db) {
  ...
  assert(
    row?.relrowsecurity && row?.relforcerowsecurity && row?.manual_rls_force,
    'RLS/FORCE is not active',
  );
  assert(
    row?.patient_select === false &&
      row?.patient_manual_select === false &&
      row?.patient_redeem === true &&
      row?.patient_claim === true,
    'patient ACL boundary is wrong',
  );
  assert(
    row?.function_owner === 'app_owner' && row?.claim_owner === 'app_owner',
...
async function proveRollback(db) {
  ...
  try {
    await db.query(
      'ALTER TABLE public.patient_invites ALTER COLUMN invited_email_normalized SET NOT NULL',
    );
  } catch (error) {
  ...
  assert(
    durable.rows[0]?.email_normalized === 'new@example.test' &&
  ...
  `);
  assert(
    result.rows[0]?.table_gone &&
      result.rows[0]?.manual_commands_gone &&
      result.rows[0]?.columns_gone,
...
function proofAuthorization(action, continuationHash, email, codeHash, proofExpiresEpoch) {
  ...
  const canonical = [
    ...
    email,
    codeHash,
    proofExpiresEpoch == null ? '' : String(proofExpiresEpoch),
  ].join('|');
  return {
    nonce,
    expiresEpoch,
...
function cleanup() {
  ...
  if (root.startsWith('/tmp/bcb_saas_patient_invite_scratch_'))
