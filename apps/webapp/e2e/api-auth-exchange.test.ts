...
describe("api/auth/exchange e2e", () => {
  ...
  it.skipIf(skip)("DEV WebView-style cross-site or missing origin metadata is rejected", async () => {
    const rejectedHeaderSets: Array<Record<string, string>> = [
      { "Content-Type": "application/json", Origin: "https://cross-site.invalid", "Sec-Fetch-Site": "cross-site" },
      { "Content-Type": "application/json" },
    ];
