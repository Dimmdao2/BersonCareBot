...
describe('doctor clients scope and subscribers redirects', () => {
  ...
  it('/app/doctor/subscribers redirects to /app/doctor/patients', async () => {
    ...
    await expect(SubscribersListPage({ searchParams: Promise.resolve({}) })).rejects.toThrow(
      'redirect',
    );
  ...
  it('/app/doctor/subscribers/[userId] redirects to clients profile with scope=all', async () => {
    ...
    await expect(
      SubscribersProfilePage({ params: Promise.resolve({ userId: uid }) }),
    ).rejects.toThrow('redirect');
    expect(redirectMock).toHaveBeenCalledWith(
      `/app/doctor/clients/${encodeURIComponent(uid)}?scope=all`,
    );
  });
