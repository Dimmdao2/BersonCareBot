...
Workflow [`.github/workflows/ci.yml`](.github/workflows/ci.yml) выполняет проверки (`pnpm run ci`) и не деплоит автоматически. Production workflow технически остаётся ручным [`.github/workflows/deploy-prod.yml`](.github/workflows/deploy-prod.yml) (`workflow_dispatch` + environment approval) и принимает только `DEPLOY_HOST=135.106.162.170`, но сами PROD entrypoint сейчас намеренно закрыты fail-closed до owner-approved миграции прав `A→B0`. PROD находится только на `135.106.162.170` (`adelaide`). Текущий `151.241.228.122` — DEV/RELAY/TEST; PROD-юниты там запускать нельзя. Runbook: [`deploy/HOST_DEPLOY_README.md`](deploy/HOST_DEPLOY_README.md).
