...
- Инфраструктура: host deploy (systemd, nginx) + GitHub Actions; Docker Compose для локальной разработки

Архитектура и потоки: `ARCHITECTURE.md`. Оглавление документации: `docs/README.md`.
...
- Изменился только runtime `.env`: `cp /opt/tgcarebot/.env .env` и `docker compose up -d --force-recreate <service>`
- Изменились ключи интеграций в `src/integrations/*/config.ts`: пересоберите и перезапустите нужные сервисы
