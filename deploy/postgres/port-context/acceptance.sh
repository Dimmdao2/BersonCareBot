...
(cd "$repo_root" && \
  node --experimental-strip-types deploy/postgres/privileges/generate-cli.mjs --shared-role-baseline) \
...
psql_admin -c "ALTER ROLE $staff_login NOINHERIT; ALTER ROLE $patient_login NOINHERIT; ALTER ROLE $global_admin_login NOINHERIT; ALTER ROLE $integrator_login NOINHERIT; GRANT app_pre_session, app_staff, app_clinic_billing, app_worker, app_operational_media_worker, saas_telemetry_operator TO $staff_login WITH INHERIT FALSE, SET TRUE, ADMIN FALSE; GRANT app_pre_session, app_patient TO $patient_login WITH INHERIT FALSE, SET TRUE, ADMIN FALSE; GRANT app_platform_settings, app_platform_admin TO $global_admin_login WITH INHERIT FALSE, SET TRUE, ADMIN FALSE; GRANT app_integrator_request, app_integrator_resolver, app_operational_delivery_worker, app_operational_scheduler, app_tenant_service, app_service TO $integrator_login WITH INHERIT FALSE, SET TRUE, ADMIN FALSE;"
...
assert_eq "$(psql_admin -Atc "SELECT count(*) FROM pg_proc WHERE oid IN ('app.install_port_context(uuid,app.port_context_claims)'::regprocedure,'app.clear_port_context()'::regprocedure,'app.require_accepted_context(name,name,app.port_context_class,text,bytea,regprocedure)'::regprocedure,'app.require_attested_context_for_roles(name,name[])'::regprocedure,'app.current_org_id()'::regprocedure,'app.current_actor_user_id()'::regprocedure,'app.current_patient_user_id()'::regprocedure,'app.current_integrator_user_id()'::regprocedure) AND proowner <> 'app_seam_context_owner'::regrole")" 0
...
WITH expected(signature,owner_name,is_definer,volatility,parallelism,config) AS (VALUES
 ...
 ('app.require_attested_context_for_roles(name,name[])','app_seam_context_owner',true,'v'::"char",'u'::"char",ARRAY['search_path=pg_catalog, app, app_ext, pg_temp']::text[]),
...
WITH expected(signature,grantees) AS (VALUES
 ('app.hash_port_typed_args(app.port_typed_arg[])',ARRAY['app_seam_context_owner','app_seam_password_auth_owner','app_seam_identity_lookup_owner','app_seam_payment_webhook_owner']::name[]),
...
), gate_owner(role_name) AS (VALUES ('app_seam_context_owner'),('app_seam_password_auth_owner'),('app_seam_identity_lookup_owner'),('app_seam_staff_security_owner'),('app_seam_patient_self_actions_owner'),('app_seam_settings_runtime_owner'),('app_seam_org_commerce_owner'),('app_seam_delivery_scope_owner'),('app_seam_phone_binding_owner'),('app_seam_payment_webhook_owner')), hash_owner(role_name) AS (VALUES ('app_seam_context_owner'),('app_seam_password_auth_owner'),('app_seam_identity_lookup_owner'),('app_seam_payment_webhook_owner'))
...
assert_eq "$seam_missing_gate_execute" 32
assert_eq "$seam_missing_hash_execute" 38
