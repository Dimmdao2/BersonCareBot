...
test('all latest active B0-forward definers have exact executable relation-operation surfaces', () => {
  ...
  assert.equal(functions.length, 119);
...
test('all 409 declared functions have the exact source-reconstructed base type and set-returning flag', () => {
  ...
  assert.equal(canonical.size, 410);
  ...
  assert.deepEqual(forms, { SCALAR: 286, TABLE: 120, SETOF: 4 });
  ...
  assert.equal(Object.values(declaration.portContext.functions).filter((fn) => !fn.returnsSet).length, 288);
...
test('legacy census is restored without obsolete context and overlaid by the active B0-forward roots', () => {
  ...
  assert.equal(testFunctions.filter(([, fn]) => fn.security === 'DEFINER').length, 397);
  assert.equal(devFunctions.filter(([, fn]) => fn.security === 'DEFINER').length, 395);
  ...
  assert.equal(testFunctions.length, 412);
  assert.equal(devFunctions.length, 410);
...
test('per-DB function SQL is deterministic and contains the bilateral metadata check', () => {
  for (const database of DATABASES) {
    ...
    const expectedDefiners = database === 'bersoncarebot_test' ? 397 : 395;
