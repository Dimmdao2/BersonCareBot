...
export interface FunctionRelationSurface {
   ...
   * в клинику; глобальная строка платформы; клиника ещё создаётся). Пометки без причины не бывает:
   * пустая строка — это выключенная проверка, и гейт на неё краснеет.
   *
   * ПОЧЕМУ ПОМЕТКА ВООБЩЕ НУЖНА. `SECURITY DEFINER` исполняется правами владельца, RLS его не
   * сужает. Стену, которую снаружи держала политика, тело обязано повторить само — иначе корень
   * ШИРЕ прежнего реляционного чтения и молча отдаёт строки чужой клиники. Гейт
   * `definer-tenant-predicate.test.mjs` требует привязки к принципалу порта у КАЖДОГО такого
   * чтения; эта пометка — единственный способ сказать «здесь её нет по делу», и она НАЗЫВАЕТ дело.
   *
   * Маркер один на репозиторий (AGENTS.md §5). Он НЕ выключает проверку целиком: снимается ровно
   * одна пара «функция → отношение», а сам набор помеченных пар записан переписью имён
   * (`definerRootsCrossingTenantWall`), поэтому вырасти молча он не может.
   */
  crossesTenantWall?: { why: string };
