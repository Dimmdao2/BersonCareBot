...
const EXPECTED_RUNTIME_SOURCES = new Map(
  Object.entries({
    'integrator:delivery': [
      'delivery-handler',
...
function productionSourceFiles(root) {
  ...
  const visit = (path) => {
    for (const entry of readdirSync(path, { withFileTypes: true })) {
      ...
      if (!entry.isFile() || !/\.[cm]?[jt]sx?$/.test(entry.name) || TEST_FILE_RE.test(entry.name))
        continue;
    ...
    for (const path of productionSourceFiles(root)) {
      ...
      function visit(node) {
        if (
          ...
          ['runIntegratorNamedRoot', 'runWebappNamedRoot', 'runWebappPreSessionNamedRoot'].includes(
          ...
          if (ts.isStringLiteralLike(identity)) {
            assert.ok(
              ts.isArrayLiteralExpression(typedArgs),
            ...
            assert.ok(
              ts.isIdentifier(identity) && identity.text === 'functionIdentity',
              `${path}:${line}: unexpected dynamic named-root identity`,
            );
            assert.ok(
              ts.isIdentifier(typedArgs) && typedArgs.text === 'functionArgs',
...
function assertCallsiteCatalog(candidate, discovered = collectNamedRootCallsites()) {
  ...
  assert.equal(
    dynamicWrappers[0].port,
    'integrator',
    'generic named-root wrapper belongs to integrator',
  );
  assert.equal(
    dynamicWrappers[0].path,
  ...
  assert.equal(
    new Set(callsites.map((row) => row.identity)).size,
    callsites.length,
    'each production named root must have one exact callsite',
  );

  const roots = Object.values(candidate.portContext.capabilities).filter(
    (descriptor) => descriptor.functionIdentity,
  );
  const expectedDescriptorCount = [...EXPECTED_ROOTS.values()].reduce(
    (count, descriptor) => count + (descriptor.descriptorCount ?? 1),
    0,
  );
  ...
  for (const callsite of callsites) {
    ...
    if (expected.descriptors) {
      assert.deepEqual(
        ...
        [...expected.descriptors].sort((left, right) =>
    ...
    for (const descriptor of descriptors) {
      const expectedTargetRole =
        expected.targetRolesBySessionRole?.[descriptor.sessionRole] ?? expected.targetRole;
      assert.deepEqual(
        {
...
test('the oracle reds on identity mutation, a missing descriptor, and a wrong descriptor', () => {
  const find = (candidate, identity) =>
    Object.entries(candidate.portContext.capabilities).find(
...
test('the oracle reds on added, moved, removed, extra and cross-port production callsites', () => {
  ...
  assert.throws(() =>
    assertCallsiteCatalog(declaration, [...discovered, discovered[firstLiteralIndex]]),
  );
  assert.throws(() =>
    assertCallsiteCatalog(
      declaration,
  ...
  crossPort[firstLiteralIndex].port =
    crossPort[firstLiteralIndex].port === 'webapp' ? 'integrator' : 'webapp';
...
test('production discovery is path-independent and excludes tests/generated output', () => {
  ...
  assert.equal(
    files.some((path) => TEST_FILE_RE.test(path) || path.includes('/generated/')),
    false,
  );
  const discovered = collectNamedRootCallsites();
