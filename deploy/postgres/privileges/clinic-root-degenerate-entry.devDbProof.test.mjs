 ...
 *
 * Второе утверждение этого файла СНЯТО вместе с тем, что оно сторожило. Оно требовало, чтобы
 * выключенная визитка сужала набор медиа до логотипа и опустошала список людей. Владелец 11.09
 * отверг это прямо: «выключение визитки закрывает заодно страницы специалистов этой клиники и
 * отдачу её фотографий — НЕ ВЕРНО… нам не надо сейчас пытаться что-то от кого-то здесь закрыть».
 * Галка решает только ВИД корневого экрана, поэтому проверять здесь больше нечего, а обратное
 * утверждение («галка ничего не закрывает») пишет независимый аудитор этапа.
 *
