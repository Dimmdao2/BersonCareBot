 ...
 *
 *   2. **Выключатель визитки перестаёт выключать.** Набор медиа, который вернула дверь, И ЕСТЬ
 *      право на анонимную отдачу `/{clinic}/media/{uuid}`: маршрут отдаёт ровно то, что в наборе,
 *      и больше ничего не проверяет — проверять нечем, в этом вся конструкция. Точно так же
 *      страница специалиста ищет человека в списке, который вернула ЭТА дверь. Значит лишняя
 *      строка в наборе или в списке людей — это фотография, видео из материала или целая страница
 *      сотрудника, выданные анониму по прямой ссылке у клиники, которая сняла публикацию одной
 *      галкой. Страница при этом рисует вход в кабинет и выглядит закрытой.
 *
 * Чего здесь НЕТ намеренно. Разметка вырожденного экрана (имя, кнопка входа, отметка платформы)
 * проверяется живым взглядом, а не тестом (§10a: автоматизированные UI-тесты запрещены). Отказ
 * страницы специалиста отдельно не проверяется: `readPublicSpecialist` ищет человека В УЖЕ
 * ПРОЧИТАННОМ ответе этой двери и второго запроса не делает — «его нет в ответе» и «страница
 * отказала» одно утверждение, а не два.
 *
...
test('выключенная визитка закрывает и фотографии, и материалы, и людей — набор двери и есть авторизация', { skip: !ENABLED }, () => {
  const off = card(CLINIC.listedCardOff.slug);
  const ids = off.media.map((asset) => asset.id);

  // Логотип остаётся: его показывает сам вырожденный экран рядом с именем клиники.
  assert.deepEqual(ids, [MEDIA.logoOff], 'в наборе выключенной визитки лежит не только логотип');
  assert.deepEqual(off.specialists, [], 'у выключенной визитки остались люди — их страницы продолжат открываться');
  assert.deepEqual(off.services, [], 'выключенная визитка отдала услуги');
  assert.deepEqual(off.locations, [], 'выключенная визитка отдала адреса филиалов');
  assert.equal(off.description, null, 'выключенная визитка отдала рекламный текст');
  assert.equal(off.fullDescriptionMarkdown, null, 'выключенная визитка отдала полное описание');
  assert.equal(off.publicContactPhone, null, 'выключенная визитка отдала контакт клиники');

  // Зеркальная половина: у клиники с ВКЛЮЧЁННОЙ визиткой ничего из этого не закрылось.
  const on = card(CLINIC.listedCardOn.slug);
  assert.deepEqual(
    on.media.map((asset) => asset.id).sort(),
    [MEDIA.logoOn, MEDIA.photoOn, MEDIA.avatarOn].sort(),
    'у включённой визитки пропали фотография или аватар — на живой странице битые картинки',
  );
