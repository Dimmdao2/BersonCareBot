...
const REV10_CONTEXT = {
  ...
  functions: {
    ...
    // ходил отношением под `app_operational_media_worker` — и не мог пройти НИКОГДА: единственная
    // разрешающая политика этой роли на `public.media_transcode_jobs` состоит из арендаторских
    // веток, обе зовут `app.current_org_id()` подзапросом-InitPlan'ом, а та на роли воркера не
    // возвращает NULL, а поднимает `42501 accepted organization context required`. Замер на TEST
    // 19.08: воркер падал раз в 5 секунд с 18.08 19:26, видео не пересобиралось больше суток.
...
const REV10_SYSTEM_DIRECT_ACCESS: Record<string, DirectAccessSeed> = {
  ...
  // оператор под этой ролью не проходил. Убраны в миграции 0050 вместе с переводом диспетчера на
