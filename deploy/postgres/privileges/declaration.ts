...
export const OWNER_DECISIONS: OwnerDecision[] = [
  {
    ...
    encodedAs:
      'PLATFORM_ROLE_SCOPE: платформенной роли только коммерция, каркас и аудит; медицины — ноль (C1/C2)',
  },
  {
    id: 'D2-patient-visibility',
    provisional: false,
    said:
      'Пациент видит ТОЛЬКО тесты, добавленные в его программу; он НЕ ВИДИТ внутренние комментарии и ' +
      'пометку проблемный и тд.',
    encodedAs:
      'PATIENT_VISIBILITY + revoke/pol на staff-комментариях, booking-профилях, test_attempts (C14)',
  },
  {
    id: 'D3-reference-org-copy',
    provisional: false,
    said:
      'Справочники: глобальный шаблон → копия на организацию при её создании; клиника владеет своей ' +
  ...
  },
  {
    ...
    provisional: false,
    said:
      'Ровно ДВА порта доступа к БД: webapp и integrator. Воркеры, крон и прунер своих подключений НЕ ' +
      'открывают — ходят через один из двух.',
    encodedAs:
      'PORTS + поле `port` на каждом логине; сегодняшний третий порт помечен `mustFold` (C5-C8)',
  },
  {
    id: 'D5-narrow-resolver',
    provisional: false,
    said:
      'Узкая роль резолвера: предмаршрутный поиск интегратора (chat_id/внешний id → организация) получает ' +
      'СВОЮ узкую роль ровно на этот один поиск.',
    encodedAs:
      'роль `app_integrator_resolver` + definer `app.resolve_organization_for_channel_identity` (C3/C4)',
  },
  {
    id: 'D6-acceptance-invariant',
    provisional: false,
    said:
      'Любой запрос к базе данных без контекста и точного совпадения разрешений выдаёт 0 строк и пишет ' +
      'ошибку в журнал.',
  ...
  },
  {
    ...
    encodedAs:
      '`cls` на каждой строке + CLASS_DEFAULT_WALL; отклонение обязано нести причину (проверяет expandTables)',
  },
  {
    id: 'D8-pruner',
    provisional: false,
    said:
      'Прунер работает под своей сервисной ролью через порт webapp (внутренний эндпоинт), никогда под ' +
      'арендной ролью.',
    encodedAs:
      'роль `app_operational_maintenance` + definer `app.prune_context_nonce_ledger` (C12)',
  },
  {
    id: 'D9-superuser',
    provisional: false,
    ...
    encodedAs:
      'роль `postgres`: superuser, GLOBAL, `bypassrls: true` — объявлено, не дефект; §G.5 рендерит отсюда',
  },
];
...
export const OWNER_GATES_OPEN: OwnerGate[] = [
  {
    ...
    safeDefault:
      'backfill первым: включение стены как есть отрезает 83-100% строк на трёх живых таблицах. ' +
  ...
  },
  {
    ...
    safeDefault:
      'оставить + стена (на TEST обратимо); удаление данных — не инженерное решение. Объявлены ' +
      "ACTIVE с `gate: ['O4']`.",
  },
  {
    id: 'O5-user-identity-cutover',
    question:
      'user_identity + user_contacts: 18 политик и 31 грант охраняют КОПИЮ (evidence/18 §1-2: 237/237 ' +
      'и 192/192 совпадений). Направление cutover — снести зеркало или снести колонки-источники в platform_users?',
    safeDefault:
      'до ответа не двигать; обе объявлены ACTIVE со своими стенами (сегодня в них живые ПДн).',
  },
  {
    id: 'O6-webapp-session-logins',
    question:
      'D4 говорит «свой env-секрет и свой пул» на порт. Порт webapp держит ДВА рантайм-логина (staff и ' +
      'nonstaff) — это два подключения ОДНОГО порта. Свести их в один логин с SET ROLE?',
    safeDefault:
      'НЕТ — не сводить. Один логин, состоящий в app_staff, делает `app.is_staff()` истинным ДО ' +
...
export const ACCEPTANCE_INVARIANT: AcceptanceInvariant = {
  owner:
    'любой запрос к базе данных без контекста и точного совпадения разрешений выдает 0 строк и пишет ' +
    'ошибку в журнал.',
  date: '2026-08-08',
  zeroRows:
    'механизм сегодняшний: RLS+FORCE на каждой объявленной таблице + deny-by-default (§D) + стена в ' +
    'точке рождения (§E). Без принципала любой предикат ложен, ответ — 0 строк.',
  andLogs:
    'этой половины сегодня НЕТ, и она меняет поведение: отсутствие принципала даёт ТИХИЙ ноль ' +
  ...
  appliedBy:
    'ТЕЛО функции в её миграции (одна власть, dbt #6238) — генератор не пишет ни тел, ни proconfig; ' +
    '§F только сравнивает.',
  acceptanceTest:
    'по каждой объявленной таблице: сессия логином порта, принципал НЕ ставим, SELECT. Ожидание — ' +
...
export const CODE_MUST_CHANGE: CodeChange[] = [
  {
    ...
    what:
      'политика `operator_health_failure_archive` стоит на `USING true` — платформа читает архив отказов всех ' +
      'клиник с doctor_user_id',
    where: [
      'deploy/postgres (политика на public.operator_health_failure_archive)',
      'evidence/14 часть 3 В2',
    ],
  },
  {
    id: 'C2',
    becauseOf: 'D1-platform-scope',
    what:
      'политика `product_analytics_registration_platform_operations_select` даёт платформе кросс-арендные ' +
      'события регистрации с user_id',
    where: [
      'deploy/postgres (политика на public.product_analytics_events_recent)',
      'evidence/14 часть 3 В2',
    ],
  },
  {
    ...
    what:
      'предмаршрутный резолв идёт сырым join по четырём таблицам (отсюда 4-стороннее членство логина) — обязан ' +
      'звать один definer-аксессор',
    where: [
      'apps/integrator/src/infra/db/repos/channelUsers.ts:65-95',
      'apps/integrator/src/app/routes.ts:44-95',
    ],
  },
  {
    id: 'C4',
    becauseOf: 'D5-narrow-resolver',
    what:
      'снять членство логина интегратора в app_identity_bootstrap/app_patient/app_staff/app_worker; ⚠ побочный ' +
  ...
  },
  {
    ...
    what:
      'интегратор открывает ЧЕТЫРЕ пула (request + DIAGNOSTIC + DELIVERY_WORKER + SCHEDULER); один порт = один ' +
  ...
  },
  {
    ...
    becauseOf: 'D4-two-ports',
    what:
      'SAAS_ISOLATION_OPERATOR_DATABASE_URL / DATABASE_URL_CONFIG_READER — логины вне двух портов, обязаны ' +
      'ходить через webapp',
    where: [
  ...
  },
  {
    ...
    what:
      'контекст-аксессоры возвращают NULL при отсутствии контекста, обязаны RAISE; каждый вызывающий, трактующий ' +
      'NULL как «нет строк», перечитывается',
    where: [
      'deploy/postgres/p2-b-protected-principal-context.sql (тела app.current_org_id / ' +
  ...
  },
  {
    ...
    becauseOf: 'D6-acceptance-invariant',
    what:
      "отказы глотаются приложением (42501 → reason:'user_not_found', catch → false, catch → null) — поэтому " +
  ...
  },
  {
    ...
    becauseOf: 'D8-pruner',
    what:
      'закрытый инфра-крон-шов гоняет ретеншен под SET ROLE app_staff (арендная ORG-роль с DELETE на ' +
  ...
  },
  {
    ...
    what:
      'сырой SQL по таблицам аутентификации минует полный definer-шов; вызов ломается после снятия ' +
  ...
  },
  {
    ...
    what:
      'снять пациентские чтения служебного материала (staff-комментарии, booking-профиль) и перевести ' +
  ...
  },
  {
    ...
    what:
      'у app_staff сегодня INSERT на clinical_test_measure_kinds (пул, который сам код называет глобальным) и ' +
      'полный CRUD на booking_cities — запись арендатора в шаблон запрещена',
    where: [
      'apps/webapp/src/modules/tests/measureKindCode.ts:1',
      'apps/webapp/src/app/api/api.md:100',
      'FINDINGS Д21',
    ],
  },
  {
    ...
    becauseOf: 'D1-platform-scope',
    what:
      'закрыть эскалацию: app_staff — член app_platform_settings и app_clinic_billing, один SET ROLE выводит ' +
  ...
  },
  {
    ...
    becauseOf: 'D6-acceptance-invariant',
    what:
      'app.is_staff() проверяет ЧЛЕНСТВО вместо USAGE — любой логин-член app_staff «персонал» для RLS до всякого ' +
  ...
  },
  {
    ...
    becauseOf: 'D6-acceptance-invariant',
    what:
      "два org-аксессора в одной базе: сырой current_setting('app.org') в политиках c4_web_push_reminder_* " +
      'против app.current_org_id() везде — сырая форма молча переживёт переход на RAISE',
    where: [
      'deploy/postgres (c4_web_push_reminder_catalog на content_pages/content_sections; та же форма на ' +
...
export const PLATFORM_ROLE_SCOPE: PlatformRoleScope = {
  ...
  provisional: true,
  mayTouch: [
    // коммерция + каркас клиники + аудит платформы; всё остальное — вне области
  ...
  mustNotTouch:
    'всё медицинское и клиническое: treatment_program_* (9), support_* (5), tests / test_sets / ' +
  ...
  consequenceRecorded:
    'платформа не может диагностировать и восстановить программу лечения клиники иначе как ' +
...
export const REFERENCE_MODEL: ReferenceModel = {
  ...
  shape:
    'при создании организации платформенный засев КОПИРУЕТСЯ в строки, принадлежащие организации. Клиника ' +
    'владеет копией: правит, переименовывает, удаляет ненужное. Права записи в глобальный шаблон у аренды нет.',
  alreadyImplemented:
    'public.reference_categories + public.reference_items + ' +
  ...
  consequence:
    'большинство справочных таблиц поэтому ORG-скоуплены, а не глобальны: класс остаётся R, стена — ' +
...
export const PORTS: Record<Port, PortSpec> = {
  webapp: {
    ...
    what:
      'всё, что делает человек в кабинете, плюс каждая внутренняя работа: тики крона приходят на ' +
    ...
    reachedThrough:
      'крон хоста → POST /api/internal/<job>/tick (Bearer INTERNAL_JOB_SECRET) → пул webapp с ' +
      'объявленной сервисной ролью.',
  },
  integrator: {
    process: 'apps/integrator (модуль доставки)',
    what:
      'входящие вебхуки, исходящая доставка, тики планировщика и проекция. По формулировке владельца ' +
      '(evidence/15) интегратор — модуль ДОСТАВКИ, а не хранилище пользовательских данных.',
    logins: ['<env>_integrator'],
    reachedThrough:
      'один пул; операционная роль (delivery / scheduler / diagnostic) выбирается SET ROLE на ' +
...
const roles: Record<string, RoleDecl> = {
  ...
  app_staff: {
    kind: 'terminal',
    scope: 'ORG', // evidence/13 §4: своя организация
    login: false,
    superuser: false,
    bypassrls: false,
    inherit: true,
    ...
    why:
      'терминал персонала клиники. ⚠ Сегодня app_staff — ЧЛЕН app_platform_settings и app_clinic_billing: ' +
      'один SET ROLE выводит арендную сессию в GLOBAL-роль на 14 таблицах (Д4). Членство не объявлено — C16.',
  },
  app_patient: {
    kind: 'terminal',
    scope: 'OWN', // FACTS §1.5: стена своих данных; ошибочное ORG-правило даёт 65 тихих нулей
    login: false,
    superuser: false,
    bypassrls: false,
    inherit: true,
  ...
  app_platform_settings: {
    ...
    rolconfig: null,
    why:
      'платформенная (глобал-админ) роль. Поверхность ограничена PLATFORM_ROLE_SCOPE (D1): коммерция + ' +
  ...
  app_worker: {
    ...
    rolconfig: null,
    why:
      'инфра-роль воркеров вебаппа; фильтр — на ENQUEUE, не в RLS (канон владельца; И11 требует объявить ' +
  ...
  app_operational_media_worker: {
    ...
    rolconfig: null,
    why:
      'медиа-конвейер. ⚠ Сегодня media-worker — отдельный процесс со своим подключением = третий порт (C7); ' +
  ...
  app_operational_web_push_reminder: {
    ...
    rolconfig: null,
    why:
      'web-push напоминания; discovery — через app_web_push_reminder_discovery_definer. evidence/13 §1.1: ' +
  ...
  app_operational_maintenance: {
    ...
    why:
      'РЕШЕНИЕ D8: прунер/ретеншен журналов через ВНУТРЕННИЙ эндпоинт порта webapp, никогда арендной ролью ' +
  ...
  app_clinic_billing: {
    ...
    why:
      'биллинг в рамках своей организации. ⚠ Членство app_staff→сюда — часть Д4 (эскалация); оставлено ' +
  ...
  app_identity_bootstrap: {
    ...
    rolconfig: null,
    why:
      'предсессионный резолв идентичности при регистрации; гранты только на public.platform_users и ' +
  ...
  app_integrator_resolver: {
    ...
    isNew: true,
    why:
      'РЕШЕНИЕ D5: узкая роль предмаршрутного резолва (chat_id/внешний id → организация) — ровно один поиск ' +
  ...
  saas_telemetry_owner: {
    ...
    members: [],
    why:
      'владеет saas_isolation_* и 7 definer-функциями телеметрии (evidence/13 §3.2; §C). Эталон формы: ACL ' +
  ...
  saas_telemetry_operator: {
    ...
    rolconfig: null,
    why:
      'читает телеметрию изоляции кросс-орг (saas-isolation-telemetry.sql). Это диагностика ПЛАТФОРМЫ о ' +
  ...
  postgres: {
    ...
    rolconfig: null,
    why:
      'РЕШЕНИЕ D9: полный доступ DBA сохраняется и ОБЪЯВЛЯЕТСЯ (не дефект); на проде — сильный пароль. ' +
...
const envMapping: Record<string, Record<string, LoginRecord>> = {
  test: {
    ...
    bcb_test_integrator_login: {
      ...
      createrole: false,
      inherit: false,
      passwordEnv: 'PGPASSWORD_BCB_TEST_INTEGRATOR',
      rolconfig: [SEARCH_PATH_PUBLIC_INTEGRATOR],
      connect: ['bersoncarebot_test'],
      why:
        'порт integrator. ⚠ ЖИВОЕ состояние — член ЧЕТЫРЁХ ролей сразу (app_identity_bootstrap, app_patient, ' +
    ...
    bcb_test_nonstaff_login: {
      ...
      createrole: false,
      inherit: false,
      passwordEnv: 'PGPASSWORD_BCB_TEST_NONSTAFF',
      rolconfig: [SEARCH_PATH_PUBLIC_INTEGRATOR],
      connect: ['bersoncarebot_test'],
      why:
        'пациентская сессия порта webapp (DATABASE_URL_NONSTAFF). ⚠ Живьём ещё и член app_identity_bootstrap ' +
    ...
    bcb_test_staff_login: {
      ...
      inherit: false,
      inheritDrift:
        'живое rolinherit=t + inherit_option=t членства (evidence/13 §1.2/§1.3): логин несёт права ' +
        'app_staff до всякого SET ROLE — механизм FINDINGS И3',
      passwordEnv: 'PGPASSWORD_BCB_TEST_STAFF',
      rolconfig: [SEARCH_PATH_PUBLIC_INTEGRATOR],
      connect: ['bersoncarebot_test'],
    ...
    bcb_test_worker_login: {
      ...
      passwordEnv: 'PGPASSWORD_BCB_TEST_WORKER',
      rolconfig: [SEARCH_PATH_PUBLIC_INTEGRATOR],
      connect: ['bersoncarebot_test'],
    ...
    bcb_test_maintenance_login: {
      ...
      connect: ['bersoncarebot_test'],
      why:
        'НОВЫЙ (D8): прунер/ретеншен через внутренний эндпоинт порта webapp. Сегодня роли нет и ретеншен ' +
        'бежит под app_staff (C12).',
    },
    bcb_test_operational_delivery_login: {
      port: 'integrator',
      mustFold:
        'C5 — DATABASE_URL_DELIVERY_WORKER сегодня отдельный пул; роль обязана достигаться SET ROLE на ' +
    ...
    bcb_test_operational_diagnostic_login: {
      ...
      createrole: false,
      inherit: false,
      passwordEnv: 'PGPASSWORD_BCB_TEST_OP_DIAGNOSTIC',
      rolconfig: null,
      connect: ['bersoncarebot_test'],
    },
    bcb_test_operational_scheduler_login: {
      ...
      createrole: false,
      inherit: false,
      passwordEnv: 'PGPASSWORD_BCB_TEST_OP_SCHEDULER',
      rolconfig: null,
      connect: ['bersoncarebot_test'],
    },
    bcb_test_operational_web_push_reminder_login: {
      ...
      membership: {
        role: 'app_operational_web_push_reminder',
        admin: false,
        inherit: false,
        set: true,
      },
      ...
      createrole: false,
      inherit: false,
      passwordEnv: 'PGPASSWORD_BCB_TEST_OP_WEBPUSH',
      rolconfig: null,
      connect: ['bersoncarebot_test'],
    },
    bcb_saas_operator_test: {
      ...
      passwordEnv: 'PGPASSWORD_BCB_SAAS_OPERATOR_TEST',
      rolconfig: null,
    ...
    bcb_saas_diag_test: {
      port: null,
      mustFold:
        'C8 — канонического членства в переписи нет (evidence/13 §1.3) и порт не объявлен: либо свернуть ' +
      ...
      passwordEnv: 'PGPASSWORD_BCB_SAAS_DIAG_TEST',
      rolconfig: null,
      connect: ['bersoncarebot_test'],
      why:
        'TODO(owner?): логин без объявленной роли и без порта. Кандидат на удаление (та же категория, что ' +
  ...
  dev: {
    bcb_webapp_dev_user: {
      ...
      inherit: false,
      inheritDrift:
        'живое rolinherit=t и живое членство в app_identity_bootstrap (evidence/13 §1.2/§1.3) — ' +
      ...
      why:
        'dev migrator-login = datdba базы bcb_webapp_dev (evidence/13 §3.5); его search_path — НЕСУЩАЯ строка ' +
    ...
    bcb_dev_runtime_nonstaff_login: {
      ...
      createrole: false,
      inherit: false,
      passwordEnv: 'PGPASSWORD_BCB_DEV_NONSTAFF',
      rolconfig: null,
      connect: ['bcb_webapp_dev'],
      why:
        'пациентская сессия порта webapp на dev. ⚠ Живьём ещё и член app_identity_bootstrap (Д5) — членство ' +
        'не объявлено.',
    },
    bcb_dev_runtime_staff_login: {
      ...
      createrole: false,
      inherit: false,
      passwordEnv: 'PGPASSWORD_BCB_DEV_STAFF',
      rolconfig: null,
      connect: ['bcb_webapp_dev'],
    },
    bcb_saas_operator_dev: {
      ...
      inherit: false,
      inheritDrift: 'живое rolinherit=t (evidence/13 §1.2)',
      passwordEnv: 'PGPASSWORD_BCB_SAAS_OPERATOR_DEV',
      rolconfig: null,
      connect: ['bcb_webapp_dev'],
...
const DEFINER_DEFAULTS: DefinerExceptionsSection['defaults'] = {
  ...
  coveredCount: 235,
  rule:
    'каждая SECURITY DEFINER функция схемы `app`, НЕ перечисленная в proconfigExceptions/ownershipExceptions, ' +
...
const PROCONFIG_EXCEPTIONS: Record<string, DefinerException> = {
  ...
  'app.current_integrator_user_id()': {
    ...
    why:
      'принципал-аксессор. ⚠ РЕШЕНИЕ D6: обязан RAISE при отсутствии контекста, а не возвращать NULL ' +
  ...
  'app.current_org_id()': {
    ...
    searchPath: ['search_path=app, pg_catalog'],
    execute: [
      // evidence/13 §3.3 ЖИВЫЕ грантополучатели + четыре недостающих (дефект ниже)
      'app_owner',
      'app_staff',
      'app_patient',
      'app_worker',
    ...
    why:
      'org-аксессор. ⚠ ДЕФЕКТ (evidence/13 §3.3 / FACTS §1.1): EXECUTE НЕ выдан четырём operational-ролям → ' +
  ...
  'app.close_active_user_phone_history(uuid)': {
    owner: 'app_owner',
    searchPath: ['search_path=app, public, pg_catalog'],
  ...
  'app.read_outbound_provider_incident_health()': {
    ...
    searchPath: ['search_path=pg_catalog, public'],
    why:
      'чтение здоровья исходящих инцидентов. ⚠ ДРЕЙФ владения: живьём владелец — мигратор-логин; цель — ' +
  ...
  'app.resolve_organization_for_channel_identity(text,text)': {
    ...
    isNew: true,
    why:
      'РЕШЕНИЕ D5: единственный вход узкой роли резолвера. Заменяет сырой join по четырём таблицам, из-за ' +
  ...
  'app.prune_context_nonce_ledger(integer,integer)': {
    ...
    isNew: true,
    why:
      'РЕШЕНИЕ D8: app.context_nonce_ledger закрыта от ВСЕХ ролей, поэтому прунер входит только через definer. ' +
...
const OWNERSHIP_EXCEPTIONS: DefinerExceptionsSection['ownershipExceptions'] = {
  intentional: {
    saas_telemetry_owner: {
      count: 7,
      why: 'владеет definer-функциями телеметрии изоляции (§C; evidence/13 §3.2)',
      functions: { todo: 'TODO(census-gap G3): 7 имён не перечислены read-only переписью' },
    },
    saas_system_health_owner: {
      count: 4,
      why: 'владеет definer-функциями health-агрегации; BYPASSRLS-владелец (§I Р9)',
      functions: { todo: 'TODO(census-gap G3): 4 имени не перечислены' },
    },
    app_web_push_reminder_discovery_definer: {
      count: 1,
      why: 'владеет своим discovery-швом (§C; evidence/13 §3.2)',
...
const REV_D1 =
  'Д1: табличный грант арендной роли в обход definer-шва — персонал любой клиники читает и ' +
  'переписывает секреты входа всех пользователей';
const W_AUTH_DEFINER =
  'таблица аутентификации: рантайм-ролям ноль грантов, путь только через ' +
  'definer-аксессоры app.*';
const W_REF_COPY =
  'D3: клинике принадлежит КОПИЯ платформенного шаблона (org-стена), запись в шаблон запрещена';
const W_PLATFORM_OR_CLINIC =
  'две ветки: строки organization_id IS NULL — только платформенная роль, остальные ' +
  'под стеной клиники';
const W_PLATFORM_TELEMETRY =
  'телеметрия платформы о самой себе: стена своей роли, арендной роли грантов нет';
const W_PRINCIPAL_SEAM =
  'шов принципала: ACL только у владельца, вход исключительно через definer-аксессоры';
const RLS_OFF_MIGRATOR_LEDGER =
  "ЯВНО объявленное отсутствие RLS (SCHEME §A.4: 'off' — объявленное " +
...
const TABLE_ROWS: TableRow[] = [
  {
    t: 'app.context_nonce_ledger',
    cls: 'T',
    owner: 'app_owner',
    wall: 'definer-only',
    why: 'защита от повтора ' + 'подписи — без неё подписанный контекст можно проиграть повторно',
    wallWhy: W_PRINCIPAL_SEAM,
    pol:
      'I14: росла неограниченно (12,6 млн просроченных строк снято 08.08) — D8 даёт прунер ' +
  ...
  {
    t: 'app.context_signing_secrets',
    cls: 'T',
    owner: 'app_owner',
    wall: 'definer-only',
    why:
      'HMAC-секрет подписи ' +
      'контекста — утечка = подделка принципала, т.е. обход всех стен разом',
    wallWhy: W_PRINCIPAL_SEAM,
    defect: ['I1-definer-plus-force'],
  },
  {
    t: 'app.principal_context',
    cls: 'T',
    owner: 'app_owner',
    wall: 'definer-only',
    why:
      '«кто сейчас в этой сессии» ' +
      '— несущая деталь: без неё все RLS-предикаты видят NULL и вся база становится пустой',
    wallWhy: W_PRINCIPAL_SEAM,
    pol:
      'пишет только app.install_signed_context, читают только context-аксессоры; без ' +
      'неё все предикаты видят NULL — отсюда требование D6 RAISE вместо NULL',
    defect: ['I1-definer-plus-force'],
  },
  {
    t: 'drizzle.__drizzle_migrations',
    cls: 'T',
    rls: 'off',
    why:
      'журнал применённых миграций webapp — миграции ' + 'применяются повторно или не применяются',
    rlsWhy: RLS_OFF_MIGRATOR_LEDGER,
  },
  {
    t: 'integrator.contacts',
    cls: 'P',
    why: 'контакты пользователя мессенджера — нельзя связать чат с телефоном ' + 'пациента',
    drop: {
      verdict: 'DROP',
      source:
        'evidence/15 §5 — 78/78 телефонов уже в public.platform_users.phone_normalized; ' +
        'легаси-фолбэк не даёт ничего',
    },
  },
  {
    t: 'integrator.content_access_grants',
    cls: 'P',
    why:
      'временные ссылки-доступы к контенту пациента — по ссылке ' +
      'из напоминания не открывается материал',
    drop: {
      verdict: 'DROP',
      source: 'evidence/15 §2 — волна 0, 0 строк, писатель недостижим, читателя нет',
    },
  },
  {
    t: 'integrator.conversation_messages',
    cls: 'P',
    why: 'сообщения диалога — пропадает текст переписки с пациентом',
    defect: ['D25-foundation-identities'],
    drop: {
      verdict: 'MOVE+DROP',
      source: 'evidence/15 §6-9 — волна 2',
      blockedBy: 'зеркало ' + 'public.support_conversation_messages 34/34',
    },
  },
  {
    t: 'integrator.conversations',
    cls: 'P',
    why: 'диалоги поддержки — ломается переписка «пациент ↔ поддержка»',
    defect: ['D25-foundation-identities'],
    drop: {
      verdict: 'MOVE+DROP',
      source: 'evidence/15 §6-9 — волна 2',
  ...
  {
    t: 'integrator.delivery_attempt_logs',
    cls: 'S',
    org: false,
    why: 'журнал попыток отправки — нельзя разобрать, ' + 'почему письмо/СМС не ушло',
    revoke: {
      app_staff:
        'D14/I16: payload_json — тело отправленного сообщения; I16(б) — не добавлять ' +
  ...
  {
    t: 'integrator.idempotency_keys',
    cls: 'S',
    org: false,
    why: 'ключи идемпотентности API — повтор вебхука ' + 'начинает дублировать записи и отправки',
    revoke: { app_staff: 'D14: очередь дедупа вебхуков — не место арендной роли.' },
    pol:
      "приоритет низкий: ПДн нет вовсе — опровергнуто замером (~225 живых строк, response_body='{}' в 261 из " +
      '261). Стены клиники/пациента не требуется',
    defect: ['D14-integrator-no-wall'],
  },
  {
    t: 'integrator.integration_data_quality_incidents',
    cls: 'S',
    org: false,
    why: 'инциденты качества внешней ' + 'интеграции — не видно, что система прислала мусор',
    revoke: {
      app_staff: 'D14: raw_value может содержать исходное значение поля пациента или филиала.',
    },
    pol:
      'по смыслу клиническая стена (инцидент принадлежит интеграции конкретной клиники), но organization_id нет; ' +
      'при 3 строках приоритет низкий (evidence/15 §19).',
    defect: ['D14-integrator-no-wall'],
  },
  {
    t: 'integrator.projection_outbox',
    cls: 'S',
    org: false,
    why: 'очередь проекций событий в webapp — события ' + 'интегратора перестают доезжать в webapp',
    revoke: { app_staff: 'D14: payload несёт события по конкретным пациентам и записям.' },
    pol:
      'приоритет понижен — ставить после переезда поддержки, когда ясен остаточный состав событий (evidence/15 ' +
      '§15).',
    defect: ['D14-integrator-no-wall'],
  },
  {
    t: 'integrator.question_messages',
    cls: 'P',
    why: 'сообщения внутри вопроса — обрывается нитка ответа на вопрос',
    defect: ['D25-foundation-identities'],
    drop: {
      verdict: 'MOVE+DROP',
      source: 'evidence/15 §6-9 — волна 2',
      blockedBy: 'зеркало ' + 'public.support_question_messages 20/20; не читается ниоткуда',
    },
  },
  {
    t: 'integrator.schema_migrations',
    cls: 'T',
    rls: 'off',
    why: 'журнал миграций интегратора — миграции ' + 'применяются повторно или не применяются',
    rlsWhy: RLS_OFF_MIGRATOR_LEDGER,
  },
  {
    t: 'integrator.telegram_users',
    cls: 'P',
    why: 'легаси-хранилище Telegram-аккаунтов — ничего не ломается — ' + 'таблица мёртвая',
    defect: ['D14-integrator-no-wall'],
    drop: {
      verdict: 'DROP',
      source:
  ...
  {
    t: 'integrator.user_questions',
    cls: 'P',
    why: 'вопросы пациента врачу/поддержке — вопрос пациента не доходит до ' + 'персонала',
    defect: ['D25-foundation-identities'],
    drop: {
      verdict: 'MOVE+DROP',
      source: 'evidence/15 §6-9 — волна 2',
      blockedBy: 'зеркало public.support_questions ' + '16/16',
    },
  },
  {
    t: 'integrator.user_reminder_delivery_logs',
    cls: 'P',
    org: true,
    why: 'журнал доставки напоминаний — не видно, ' + 'почему напоминание не дошло',
    pol:
      '⚠ evidence/18 §6: полная проекция в public.reminder_delivery_events ' +
      '(1735/1735 в обе стороны) — одна из двух таблиц уходит; какая, решает evidence/15',
  },
  {
    t: 'integrator.user_reminder_occurrences',
    cls: 'P',
    org: true,
    why:
      'конкретные срабатывания напоминаний — ' + 'напоминания не ставятся в очередь и дублируются',
    pol: 'опирается на reminder_rules; после волны 3 проверить, ' + 'на что смотрит ветка',
  },
  {
    t: 'integrator.user_reminder_rules',
    cls: 'P',
    why: 'правила напоминаний пациента — пациент перестаёт получать ' + 'напоминания',
    drop: { verdict: 'DROP', source: 'evidence/15 §4 — 27/27 уже в public.reminder_rules' },
  },
  {
    t: 'public.admin_audit_log',
    cls: 'S',
    org: true,
    wall: 'platform-role+clinic',
    why:
      'журнал административных ' +
      'действий — пропадает разбор «кто что сделал» и авто-мерджи конфликтов',
    wallWhy: W_PLATFORM_OR_CLINIC,
  },
  {
    t: 'public.app_runtime_settings',
    cls: 'S',
    org: true,
    wall: 'platform-role+clinic',
    why: 'настройки рантайма — ' + 'сервис теряет управляемые из кабинета настройки',
    wallWhy: W_PLATFORM_OR_CLINIC,
  },
  {
    t: 'public.app_runtime_settings_audit',
    cls: 'S',
    org: true,
    wall: 'platform-role+clinic',
    why: 'кто и когда ' + 'менял настройку — нельзя восстановить, кто сломал настройку',
    wallWhy: W_PLATFORM_OR_CLINIC,
  },
  {
    t: 'public.auth_rate_limit_events',
    cls: 'S',
    org: false,
    wall: 'definer-only',
    why:
      'счётчик попыток ' + 'входа/отправки кода — снимается защита от перебора OTP и OAuth-стартов',
  ...
  {
    t: 'public.be_appointment_cancellations',
    cls: 'P',
    why: 'отмены визитов — ломается политика отмен и возвратов ' + 'предоплаты',
  },
  {
    t: 'public.be_appointment_events',
    cls: 'P',
    why: 'системные события записи — пропадает машинная история ' + 'изменения брони',
    drop: {
      verdict: 'DUP-DROP',
      source:
        'evidence/18 §3 — 434/434 совпадений с be_appointment_history_events, ноль ' +
        'SELECT во всём репозитории',
      blockedBy:
  ...
  {
    t: 'public.be_appointment_history_events',
    cls: 'P',
    why: 'человекочитаемая история записи — врач перестаёт ' + 'видеть «кто и когда менял запись»',
  },
  {
    t: 'public.be_appointment_no_shows',
    cls: 'P',
    why: 'неявки — не считается счётчик неявок пациента',
  },
  {
    t: 'public.be_appointment_reschedules',
    cls: 'P',
    why: 'переносы — ломается бесплатный/платный перенос и лимит ' + 'переносов',
  },
  {
    t: 'public.be_appointment_staff_comments',
    cls: 'P',
    wall: 'clinic',
    why: 'внутренние комментарии персонала о ' + 'пациенте — врач теряет заметки по визиту',
    wallWhy:
      'РЕШЕНИЕ D2: пациентская ветка снимается — это внутренние ' + 'комментарии персонала о нём',
    revoke: {
      app_patient:
        'РЕШЕНИЕ D2 дословно: «он НЕ ВИДИТ внутренние комментарии»; body заполняет ' +
        'врач/администратор (pgClientHistory.ts)',
    },
    pol:
      'РЕШЕНИЕ D2: пациентскую ветку политики (platform_user_id = app.current_patient_user_id()) снять. Закрывает ' +
      'О2 в сторону «пациент не видит»',
    code: ['C14'],
  },
  {
    t: 'public.be_appointments',
    cls: 'P',
    org: true,
    why: 'записи на приём — без них нет ни расписания врача, ни ' + 'визита пациента',
  },
  {
    t: 'public.be_availability_rules',
    cls: 'C',
    org: true,
    why: 'правила доступности специалиста — не считаются ' + 'свободные слоты',
  },
  {
    t: 'public.be_booking_form_fields',
    cls: 'C',
    org: true,
    why: 'конструктор полей формы записи — форма записи ' + 'теряет настраиваемые поля',
  },
  {
    t: 'public.be_booking_form_submissions',
    cls: 'P',
    why: 'ответы пациента в форме записи — теряются данные, ' + 'введённые пациентом при записи',
  },
  {
    t: 'public.be_branches',
    cls: 'C',
    org: true,
    why: 'филиалы клиники — расписание некуда привязать, ломаются ' + 'часовые пояса',
  },
  {
    t: 'public.be_cancellation_policies',
    cls: 'C',
    org: true,
    why: 'политика отмен — отмены перестают штрафоваться ' + 'по правилам клиники',
  },
  {
    t: 'public.be_clinic_services',
    cls: 'C',
    org: true,
    why: 'услуги клиники — не на что записываться и нечего ' + 'считать в прайсе',
  },
  {
    t: 'public.be_external_entity_mappings',
    cls: 'C',
    org: true,
    why:
      'сопоставление «наш id ↔ id внешней системы» ' +
      '— рвётся связь с Rubitime/внешними системами, начинаются дубли',
  },
  {
    t: 'public.be_organization_members',
    cls: 'C',
    org: true,
    why:
      'членство человека в клинике — никто не ' +
      'определяется как врач/админ клиники — падает вся авторизация кабинета',
    revoke: {
      app_platform_settings:
        'SCHEME §I Р4 + D1: платформенное чтение членств — через definer-исключение, а ' +
        'не табличный SELECT; одна из 2 живых ячеек утечки FACTS §1.2',
      bcb_test_integrator_login:
        'I2: то же; ' + 'резолвер получает организацию через definer (D5).',
      bcb_test_nonstaff_login: 'I2: грант выдан логин-роли ' + 'напрямую.',
    },
    pol:
      'D16: живьём relrowsecurity=false. На этой таблице стоит определение «кто врач/админ клиники», то есть ' +
      'авторизация кабинета целиком',
    defect: ['D16-org-members-leak', 'I2-grant-to-login'],
  },
  {
    t: 'public.be_organizations',
    cls: 'C',
    org: false,
    why: 'сама клиника — без неё нет арендатора вообще',
  },
  {
    t: 'public.be_package_history_events',
    cls: 'P',
    why: 'история абонемента пациента — не видно, кто ' + 'продлил/заморозил абонемент',
  },
  {
    t: 'public.be_package_items',
    cls: 'C',
    org: false,
    wall: 'parent',
    why: 'состав абонемента-шаблона — нельзя ' + 'описать, что входит в абонемент',
    wallWhy:
      'organization_id нет ПО ЗАМЫСЛУ: org выводится EXISTS по родителю ' +
      'be_subscription_packages',
  },
  {
    t: 'public.be_package_usages',
    cls: 'P',
    why: 'списания сеансов абонемента — сеансы не списываются с абонемента',
  },
  {
    t: 'public.be_patient_booking_profiles',
    cls: 'P',
    wall: 'clinic',
    why: 'профиль пациента у клиники — нельзя ' + 'заблокировать самозапись проблемному пациенту',
    wallWhy:
      'РЕШЕНИЕ D2: пациентская ветка снимается — ' + '«проблемный», блокировка, счётчик неявок',
    revoke: {
      app_patient:
    ...
    pol: 'РЕШЕНИЕ D2: снять пациентскую ветку политики. Закрывает FINDINGS О2',
    code: ['C14'],
  },
  {
    t: 'public.be_patient_package_items',
    cls: 'P',
    org: false,
    wall: 'parent+patient',
    why: 'состав купленного ' + 'абонемента — не известно, сколько сеансов какой услуги куплено',
    wallWhy:
      'organization_id нет ПО ЗАМЫСЛУ: ' + 'org и пациент выводятся EXISTS по be_patient_packages',
  },
  {
    t: 'public.be_patient_packages',
    cls: 'P',
    why: 'купленные пациентом абонементы — абонементы перестают ' + 'списываться и показываться',
  },
  {
    t: 'public.be_patient_timeline_events',
    cls: 'P',
    why: 'лента событий пациента — пропадает единая хронология по ' + 'клиенту',
  },
  {
    t: 'public.be_payment_history_events',
    cls: 'P',
    why: 'история платежей пациента — пропадает платёжная ' + 'хронология в карточке пациента',
  },
  {
    t: 'public.be_payment_intents',
    cls: 'P',
    why: 'намерения оплаты — не создаётся ссылка на оплату/предоплату',
  },
  {
    t: 'public.be_payment_provider_events',
    cls: 'C',
    org: true,
    why: 'сырые вебхуки платёжного провайдера — платёж ' + 'не подтверждается автоматически',
  },
  { t: 'public.be_payments', cls: 'P', why: 'платежи пациента — нет учёта оплат визитов' },
  {
    t: 'public.be_prepayment_policies',
    cls: 'C',
    org: true,
    why: 'политика предоплаты по услуге — не берётся ' + 'предоплата',
  },
  { t: 'public.be_refunds', cls: 'P', why: 'возвраты — нельзя вернуть предоплату' },
  {
    t: 'public.be_reschedule_policies',
    cls: 'C',
    org: true,
    why: 'политика переносов — пациент переносит визит без ' + 'ограничений',
  },
  {
    t: 'public.be_rooms',
    cls: 'C',
    org: true,
    why: 'кабинеты филиала — нельзя развести приёмы по кабинетам',
  },
  {
    t: 'public.be_schedule_blocks',
    cls: 'C',
    org: true,
    why: 'блокировки времени (отпуск, перерыв) — врача ' + 'записывают в занятое/нерабочее время',
  },
  {
    t: 'public.be_schedule_templates',
    cls: 'C',
    org: true,
    why: 'Шаблоны рабочего дня клиники — без неё нельзя ' + 'быстро назначить типовой график',
  },
  {
    t: 'public.be_service_location_availability',
    cls: 'C',
    org: true,
    why: 'Где оказывается услуга — без неё запись ' + 'не знает, в каком филиале доступна услуга',
  },
  {
    t: 'public.be_specialist_locations',
    cls: 'C',
    org: true,
    why: 'Специалист ↔ филиал — без неё специалист не ' + 'привязан к филиалу — слоты не строятся',
  },
  {
    t: 'public.be_specialist_rooms',
    cls: 'C',
    org: true,
    why: 'Специалист ↔ кабинет — распределение по кабинетам ' + 'при записи',
  },
  {
    t: 'public.be_specialist_service_availability',
    cls: 'C',
    org: true,
    why:
      'Какой специалист какую услугу ' +
  ...
  {
    t: 'public.be_specialists',
    cls: 'C',
    org: true,
    why:
      'Карточка специалиста клиники — витрина записи и ' +
  ...
  {
    t: 'public.be_subscription_packages',
    cls: 'C',
    org: true,
    why: 'Абонементы клиники — без неё нельзя ' + 'продать/списать абонемент',
  },
  {
    t: 'public.be_working_days',
    cls: 'C',
    org: true,
    why:
      'График на конкретную дату (перекрывает недельный) — ' +
      'разовые изменения графика (отпуск, дополнительный день)',
  },
  {
    t: 'public.be_working_hours',
    cls: 'C',
    org: true,
    why: 'Недельный график — базовое расписание — без него нет ни ' + 'одного слота',
  },
  {
    t: 'public.booking_calendar_map',
    cls: 'S',
    org: false,
    wall: 'definer-only',
    why:
      'Связь записи с событием ' +
  ...
  {
    t: 'public.booking_cities',
    cls: 'R',
    wall: 'reference-org-copy',
    why:
      'города каталога записи (2 строки) — шов ' +
      'listActiveBookingCities мёртв, в коде остались только code-строки',
    wallWhy: W_REF_COPY,
    revoke: {
      app_staff:
        'D3+D21: клиника A переименовывает/удаляет город, который видит клиника B; запись в ' +
  ...
  {
    t: 'public.broadcast_audit',
    cls: 'C',
    org: true,
    why: 'Журнал рассылок клиники — без неё нет истории рассылок и ' + 'счётчиков доставки',
    pol: 'I4: app_owner=r без политики — тот же класс тихого нуля.',
    defect: ['I4-dead-grant'],
  },
  {
    t: 'public.broadcast_audit_recipients',
    cls: 'P',
    org: true,
    why: 'Кому ушла рассылка — пациент видит ' + 'адресованные ему рассылки; врач — охват',
  },
  {
    t: 'public.broadcast_drafts',
    cls: 'C',
    org: true,
    why: 'Черновики рассылок — врач теряет несохранённый текст ' + 'рассылки',
  },
  {
    t: 'public.channel_link_secrets',
    cls: 'S',
    org: false,
    wall: 'definer-only',
    why: 'Одноразовые секреты привязки ' + 'мессенджера — привязка Telegram/MAX к аккаунту',
    wallWhy: W_AUTH_DEFINER,
    revoke: { app_staff: REV_D1 },
  ...
  {
    t: 'public.clinic_dedicated_bot_bindings',
    cls: 'C',
    org: true,
    why:
      'Привязка собственного бота клиники — без ' +
      'неё вебхук собственного бота клиники не маршрутизируется',
    pol:
      'I4 «мёртвый грант»: app_staff=arwd без ' +
      'staff-политики при FORCE RLS = тихий ноль. Либо отозвать грант (путь идёт через definer), либо дописать ' +
      'политику',
    defect: ['I4-dead-grant'],
  },
  {
    t: 'public.clinic_public_directory_entries',
    cls: 'C',
    org: true,
    why: 'Публичная витрина клиники — без неё ' + 'клиника не находится по публичной ссылке записи',
  },
  {
    t: 'public.clinical_anamnesis_illness',
    cls: 'P',
    org: true,
    why:
      'Анамнез: перенесённые болезни и стрессы — без ' +
      'неё врач теряет историю болезней пациента в карточке',
  },
  {
    t: 'public.clinical_anamnesis_lifestyle',
    cls: 'P',
    org: true,
    why: 'Анамнез: образ жизни — блок «Образ жизни» в ' + 'карточке пациента',
  },
  {
    t: 'public.clinical_anamnesis_trauma',
    cls: 'P',
    org: true,
    why: 'Анамнез: травмы и операции — блок «Травмы и ' + 'операции»',
  },
  {
    t: 'public.clinical_complaint',
    cls: 'P',
    org: true,
    why: 'Жалобы пациента — без неё нет списка жалоб и их ' + 'закрытия',
  },
  {
    t: 'public.clinical_complaint_update',
    cls: 'P',
    org: true,
    why:
      'Динамика жалобы по визитам — без неё жалоба ' + 'статична, нет истории «стало лучше/хуже»',
  },
  {
    t: 'public.clinical_diagnosis',
    cls: 'P',
    org: true,
    why: 'Диагнозы пациента — основной клинический артефакт ' + 'карточки',
  },
  {
    t: 'public.clinical_diagnosis_catalog',
    cls: 'C',
    org: true,
    why: 'Справочник диагнозов клиники — врач выбирает ' + 'диагноз из своего справочника',
  },
  {
    t: 'public.clinical_diagnosis_status_history',
    cls: 'P',
    org: true,
    why: 'Журнал смены статуса диагноза — аудит: ' + 'кто и когда снял/поставил диагноз',
  },
  {
    t: 'public.clinical_diagnosis_update',
    cls: 'P',
    org: true,
    why: 'Уточнения диагноза по визитам — без неё ' + 'диагноз не уточняется от визита к визиту',
  },
  {
    t: 'public.clinical_test_measure_kinds',
    cls: 'R',
    org: false,
    wall: 'reference-org-copy',
    why: 'Виды измерений ' + 'для клинических тестов — единые подписи измерений в тестах',
    wallWhy: W_REF_COPY,
    revoke: {
      app_staff:
        'D3+D21: арендная роль имеет INSERT в пул, который сам код называет глобальным ' +
  ...
  {
    t: 'public.clinical_test_regions',
    cls: 'C',
    org: true,
    why: 'Связка «клинический тест ↔ регион тела» — фильтр ' + 'тестов по региону тела',
    pol:
      'РЕШЕНИЕ D2: пациенту гранта НЕТ и не будет — клинические тесты с приёма ему не ' +
      'показываются. Объявленное КОНЕЧНОЕ состояние (закрывает О2), а не пробел',
  },
  {
    t: 'public.clinical_visit',
    cls: 'P',
    org: true,
    why: 'Клинический визит — приём как таковой: осмотр, ' + 'манипуляции, рекомендации',
  },
  {
    t: 'public.comments',
    cls: 'P',
    org: true,
    why: 'Комментарии к сущностям — диалог врач↔пациент вокруг ' + 'упражнений, тестов, программ',
    pol:
      'D10: дизъюнкт target_type = ' +
      'ANY(exercise,test,test_set,recommendation,lesson) стоит БЕЗ условия и в USING, и в WITH CHECK — сотрудник ' +
      'клиники A правит комментарии клиники B',
    defect: ['D10-comments'],
  },
  {
    t: 'public.content_access_grants_webapp',
    cls: 'P',
    org: true,
    why:
      'Выданные пациенту доступы к контенту — ' + 'пациент теряет доступ к выданным ему материалам',
  },
  {
    t: 'public.content_pages',
    cls: 'C',
    org: true,
    why: 'Страницы CMS — контент, который читает пациент',
    pol:
      'I5/C18: политика c4_web_push_reminder_catalog читает org сырым current_setting, остальные — ' +
      'app.current_org_id(). Свести к одному аксессору, иначе D6 обходится',
    defect: ['I5-two-org-accessors'],
    code: ['C18'],
  },
  {
    t: 'public.content_section_slug_history',
    cls: 'C',
    org: true,
    why:
      'История переименований разделов — старые ' +
      'ссылки пациента не ломаются после переименования',
    pol:
      'плюс пациентское чтение. I6: политика ' +
      'patient_current_org_select выдана роли public вместо app_patient — привести к app_patient',
    defect: ['I6-policy-to-public'],
  },
  {
    t: 'public.content_sections',
    cls: 'C',
    org: true,
    why: 'Разделы CMS — навигация пациентского контента',
    pol: 'то же, что content_pages (I5 / C18)',
    defect: ['I5-two-org-accessors'],
    code: ['C18'],
  },
  {
    ...
    cls: 'C',
    org: true,
    why: 'Курсы клиники — платный/бесплатный курс как продукт клиники',
  },
  {
    t: 'public.doctor_notes',
    cls: 'P',
    org: true,
    why: 'Заметки врача о пациенте — личные пометки врача по клиенту',
  },
  {
    t: 'public.doctor_patient_support',
    cls: 'P',
    org: true,
    why:
      'Флаги сопровождения пациента — определяет, ведёт ' +
      'ли врач клиента и открыты ли ему чат/медиа',
  },
  {
    t: 'public.email_challenges',
    cls: 'S',
    org: false,
    wall: 'definer-only',
    why: 'Коды подтверждения почты — вход ' + 'и подтверждение почты',
    wallWhy: W_AUTH_DEFINER,
    revoke: { app_staff: REV_D1 },
    defect: ['D1-auth-tables'],
    code: ['C13'],
  ...
  {
    t: 'public.email_otp_locks',
    cls: 'S',
    org: false,
    wall: 'definer-only',
    why: 'Блокировка после неудачных ' + 'попыток — защита входа от перебора кода',
    wallWhy: W_AUTH_DEFINER,
    revoke: { app_staff: REV_D1 },
    defect: ['D1-auth-tables'],
    code: ['C13'],
  ...
  {
    t: 'public.email_send_cooldowns',
    cls: 'S',
    org: false,
    wall: 'definer-only',
    why: 'Антиспам отправки писем — ' + 'без неё письма уходят пачками',
    wallWhy: W_AUTH_DEFINER,
    revoke: { app_staff: REV_D1 },
    defect: ['D1-auth-tables'],
    code: ['C13'],
  ...
  {
    t: 'public.idempotency_keys',
    cls: 'S',
    org: false,
    why:
      'кэш ответов межсервисного API (1 251 959 строк по ' +
      'reltuples) — повтор вебхука не выполняет операцию дважды',
    revoke: {
      app_staff:
        'D19: response_body несёт тела ответов по обращениям пациентов и привязке телефонов — ' +
        'арендной роли на межсервисном кэше не место',
    },
    pol:
      '⚠ GAP G11: объём под вопросом (1 251 959 строк по reltuples против 0 строк на TEST) — нужен count(*). На ' +
      'сам дефект стены не влияет',
    defect: ['D19-operator-tables', 'I1-definer-plus-force'],
  },
  {
    t: 'public.integration_webhook_error_events',
    cls: 'S',
    org: false,
    why: 'Ошибки входящих вебхуков — диагностика ' + 'молчащего вебхука',
    revoke: {
      app_staff:
  ...
  {
    t: 'public.integration_webhook_last_status',
    cls: 'S',
    org: false,
    why: 'Последний статус вебхука — панель ' + 'здоровья интеграций',
    revoke: { app_staff: 'D19: платформенная телеметрия входящих вебхуков.' },
    defect: ['D19-operator-tables'],
  },
  {
    t: 'public.integrator_push_outbox',
    cls: 'S',
    org: false,
    why:
      'Очередь исходящих push к integrator — без неё ' +
  ...
  {
    t: 'public.lfk_complex_exercises',
    cls: 'P',
    org: true,
    why: 'Строки комплекса пациента — сам состав назначения ' + '(что и сколько делать)',
  },
  {
    t: 'public.lfk_complex_template_exercises',
    cls: 'C',
    org: true,
    wall: 'reference-org-copy',
    why: 'Строки ' + 'шаблона — состав шаблонного комплекса',
    wallWhy: W_REF_COPY,
  },
  {
    t: 'public.lfk_complex_templates',
    cls: 'C',
    org: true,
    wall: 'reference-org-copy',
    why: 'Шаблоны комплексов — ' + 'библиотека готовых комплексов клиники и платформы',
    wallWhy: W_REF_COPY,
  },
  {
    t: 'public.lfk_complexes',
    cls: 'P',
    org: true,
    why:
      'Назначенные пациенту комплексы ЛФК — без неё пациент не ' +
      'получает назначенных упражнений',
    pol:
      'I12: пациентская ветка смотрит только на platform_user_id, а колонок ' +
      'две (есть legacy user_id text) — строка с NULL пациенту невидима (подкласс D27: стена прячет данные)',
    defect: ['I12-two-patient-keys'],
  },
  {
    t: 'public.lfk_exercise_media',
    cls: 'C',
    org: true,
    wall: 'reference-org-copy',
    why: 'Видео/картинки упражнения ' + '— пациент не видит показ упражнения',
    wallWhy: W_REF_COPY,
  },
  {
    t: 'public.lfk_exercise_regions',
    cls: 'C',
    org: true,
    wall: 'reference-org-copy',
    why: 'Упражнение ↔ регион ' + 'тела — фильтр упражнений по региону',
    wallWhy: W_REF_COPY,
  },
  {
    t: 'public.lfk_exercises',
    cls: 'C',
    org: true,
    wall: 'reference-org-copy',
    why: 'Каталог упражнений — без ' + 'каталога упражнений нет назначений',
    wallWhy: W_REF_COPY,
  },
  {
    t: 'public.lfk_sessions',
    cls: 'P',
    org: true,
    why: 'Дневник выполнения ЛФК — без неё нет дневника и статистики ' + 'выполнения',
  },
  {
    t: 'public.login_tokens',
    cls: 'S',
    org: false,
    wall: 'definer-only',
    why: 'Одноразовые токены входа — вход по ' + 'ссылке/коду',
    wallWhy: W_AUTH_DEFINER,
    revoke: { app_staff: REV_D1 },
    defect: ['D1-auth-tables'],
    code: ['C13'],
  ...
  {
    t: 'public.manual_patient_commands',
    cls: 'P',
    org: true,
    wall: 'clinic',
    why:
      'Идемпотентность ручных команд по ' +
      'пациенту — защита от двойного выполнения ручной команды (приглашение и т.п.)',
    wallWhy:
      'служебная ' + 'идемпотентность действий персонала — пациентской ветки нет по построению',
  },
  {
    t: 'public.material_ratings',
    cls: 'P',
    org: true,
    why: 'Оценки материалов пациентом — обратная связь по ' + 'материалам, отчёты врачу',
  },
  {
    t: 'public.media_files',
    cls: 'C',
    org: true,
    why: 'Файлы медиатеки — хранилище всех медиа: видео упражнений, ' + 'логотипы, файлы пациента',
    pol:
      'D9: в пациентской ветке saas_org_dormant_p0_8_3 нет проверки organization_id ' +
      '— пациент клиники A читает s3_key любого файла клиники B',
    defect: ['D9-media-files'],
  },
  {
    t: 'public.media_folders',
    cls: 'P',
    org: true,
    why:
      'Папки медиатеки, в т.ч. личные папки пациентов — файлы ' +
      'клиента и библиотека клиники раскладываются по папкам',
    pol:
      'D11: дизъюнкт без условий (patient_user_id IS ' +
      'NULL) пропускает всю библиотеку клиники любой сессии с грантом — и в USING, и в WITH CHECK',
    defect: ['D11-media-folders'],
  },
  {
    t: 'public.media_hls_proxy_error_events',
    cls: 'T',
    org: true,
    wall: 'platform-role',
    why: 'Отказы HLS-прокси — ' + 'диагностика «видео не играет» у конкретного пациента',
    wallWhy: W_PLATFORM_TELEMETRY,
    pol:
      'I7: у app_staff ' +
  ...
  {
    t: 'public.media_playback_client_events',
    cls: 'T',
    org: true,
    wall: 'platform-role',
    why: 'Клиентские события ' + 'плеера — понять, почему у пациента не грузится видео',
    wallWhy: W_PLATFORM_TELEMETRY,
    pol: 'I7: тот же ' + 'несогласованный набор (awd без r).',
    defect: ['I7-privilege-mismatch'],
  },
  {
    t: 'public.media_playback_resolution_events',
    cls: 'T',
    org: true,
    wall: 'platform-role',
    why: 'Как отдавалось ' + 'видео — оценка минут просмотра в отчётах',
    wallWhy: W_PLATFORM_TELEMETRY,
  },
  {
    t: 'public.media_playback_stats_hourly',
    cls: 'T',
    org: false,
    wall: 'platform-role',
    why: 'Почасовой агрегат ' + 'воспроизведений — дешёвый график вместо скана событий',
    wallWhy: W_PLATFORM_TELEMETRY,
    revoke: {
      app_staff:
        'D21: строка bucket_hour × delivery суммирует воспроизведения ВСЕХ клиник — ни чтения, ни ' +
        'записи арендной роли',
    },
    defect: ['D21-reference-write'],
  },
  {
    t: 'public.media_playback_user_video_first_resolve',
    cls: 'P',
    org: true,
    why: 'отметка «впервые досмотрел ' + 'видео» — без неё нет метрики первого просмотра',
    pol:
      'I7: вставку делает ПАЦИЕНТСКАЯ сессия, гранта у ' +
      'app_patient нет, ошибка глотается — метрика молча пишется в ноль. Назвать роль пути и привести грант к ней',
    defect: ['I7-privilege-mismatch'],
    code: ['C10'],
  },
  {
    t: 'public.media_transcode_jobs',
    cls: 'T',
    org: true,
    wall: 'clinic',
    why:
      'очередь перекодирования видео — без ' +
      'неё загруженное видео не превращается в проигрываемое',
    wallWhy:
      'очередь принадлежит клинике; ветки воркеров ' +
      'мимо org-фильтра — ИМЕНОВАННОЕ исключение (I11)',
    pol:
      'ИМЕНОВАННОЕ исключение: ветки ' +
      "pg_has_role(...,'app_worker'/'app_operational_media_worker') идут мимо org-фильтра НАМЕРЕННО — «фильтр " +
      'воркера на ENQUEUE, не в RLS» (I11)',
    defect: ['I11-worker-bypass'],
  },
  {
    t: 'public.media_upload_sessions',
    cls: 'P',
    org: true,
    why:
      'сессия многочастной загрузки файла — без неё ' +
      'нельзя загрузить файл/видео кусками (обрывы, докачка)',
  },
  {
    t: 'public.message_log',
    cls: 'P',
    org: true,
    why:
      'журнал отправленных человеку сообщений — без неё врач не ' +
      'видит историю переписки с пациентом и не доказать факт отправки',
  },
  {
    t: 'public.motivational_quotes',
    cls: 'C',
    org: true,
    why: 'мотивационные цитаты клиники — без неё пропадает ' + 'блок цитаты на главной пациента',
    pol:
      'I13: контент пациентский, но у app_patient ни гранта, ни политики — ' +
      'под пациентской ролью 42501. Установить фактом, под какой ролью рендерится экран',
    defect: ['I13-patient-content-no-path'],
  },
  {
    t: 'public.notification_delivery_attempts',
    cls: 'P',
    org: true,
    why:
      'попытки доставки уведомления — без неё не ' +
      'видно, дошло ли напоминание, и не работает диагностика доставки',
    pol:
      'I5/C18: та же форма сырого ' +
  ...
  {
    t: 'public.online_intake_answers',
    cls: 'P',
    org: true,
    why:
      'ответы на анкету первичного обращения — без неё ' +
      'теряется содержимое онлайн-заявки пациента',
    gate: ['O4-dead-tables'],
  },
  {
    t: 'public.online_intake_attachments',
    cls: 'P',
    org: true,
    why:
      'файлы к анкете — без неё не удалить файлы ' +
      'пациента из S3 при purge; без неё не приложить документы к заявке',
    gate: ['O4-dead-tables'],
  },
  {
    t: 'public.online_intake_requests',
    cls: 'P',
    org: true,
    why: 'сама заявка — без неё нет входящего потока ' + 'онлайн-обращений',
    gate: ['O4-dead-tables'],
  },
  {
    t: 'public.online_intake_status_history',
    cls: 'P',
    org: true,
    why: 'смена статуса заявки — без неё нет аудита ' + '«кто перевёл заявку в отказ»',
    gate: ['O4-dead-tables'],
  },
  {
    t: 'public.operator_health_alert_sent',
    cls: 'S',
    org: false,
    why:
      'отметки «алерт с таким ключом уже отправлен» ' +
  ...
  {
    t: 'public.operator_health_failure_archive',
    cls: 'C',
    org: true,
    why:
      'архив разобранных отказов здоровья — без ' +
      'неё админ не может «закрыть» разобранный инцидент и он висит вечно',
    revoke: {
      app_platform_settings:
        'РЕШЕНИЕ D1: USING true отдаёт платформе архив отказов ВСЕХ клиник вместе с ' +
        'doctor_user_id — это медицина, вне коммерции и каркаса',
    },
    code: ['C1'],
  },
  {
    t: 'public.operator_incidents',
    cls: 'S',
    org: false,
    why:
      'реестр инцидентов интеграций — без неё платформа не ' +
      'знает, что интеграция сломалась; на ней стоит вся панель здоровья',
    revoke: {
      app_staff:
        'D19: SET ROLE app_staff даёт 9 строк реестра инцидентов платформы (включая ' +
        'alert_claim_token) с полным CRUD',
    },
    defect: ['D19-operator-tables'],
  },
  {
    t: 'public.operator_job_status',
    cls: 'S',
    org: false,
    why:
      'состояние фоновых задач — без неё не видно, живы ли ' +
      'крон-задачи; это корень 61 050 отказов из FACTS §1.1',
    revoke: {
      app_staff:
        'D12: политика saas_enforce_default_deny_p0_9_1 выдана PUBLIC с USING true; SET ROLE ' +
        'app_staff даёт 20 строк состояния планировщика платформы с полным CRUD',
    },
    pol:
      'реальный предикат стены платформенной роли вместо USING true. Эта же таблица — корень 61 050 отказов FACTS ' +
      '§1.1.',
    defect: ['D12-operator-job-status'],
  },
  {
    t: 'public.org_brand_revisions',
    cls: 'C',
    org: true,
    why:
      'ревизии брендинга клиники — без неё клиника не может ' +
      'менять логотип/название с версионированием',
  },
  {
    t: 'public.org_enrollments',
    cls: 'P',
    org: true,
    why: 'прикрепление человека к клинике — на неё опирается вся ' + 'стена арендатора',
  },
  {
    t: 'public.organization_member_invites',
    cls: 'C',
    org: true,
    why: 'приглашения сотрудников — без неё нельзя ' + 'завести второго врача в клинику',
  },
  {
    t: 'public.organization_slug_claims',
    cls: 'C',
    org: true,
    why: 'занятые адреса клиник — без неё две клиники ' + 'займут один публичный адрес',
    pol:
      'ОБЪЯВЛЕННЫЙ definer-шов проверки занятости (I10): в реестре уникальности ' +
      'чужая занятая строка невидима, выглядит свободной, и UNIQUE падает на вставке',
    defect: ['I10-slug-uniqueness'],
  },
  {
    t: 'public.organization_slug_rename_events',
    cls: 'C',
    org: true,
    why: 'журнал переименований — без неё нет ' + 'аудита смены публичного адреса',
  },
  {
    t: 'public.outgoing_delivery_queue',
    cls: 'S',
    org: true,
    why: 'очередь исходящих сообщений — без неё не уходит ' + 'ни одно сообщение пациенту',
    revoke: {
      app_staff:
        'D18: 812 строк с payload_json (тела сообщений пациентам) читает терминал персонала любой ' +
        'клиники без принципала',
    },
    pol:
      'клиническая стена невозможна ДО backfill: 812 из 812 строк несут organization_id IS NULL (D27/O3) — ' +
      'включение org-стены в лоб отрежет всю доставку',
    defect: ['D18-outgoing-delivery-queue', 'D27-empty-org-discriminator'],
    gate: ['O3-empty-tenant-discriminator'],
  },
  {
    t: 'public.password_altcha_challenges',
    cls: 'S',
    org: false,
    wall: 'definer-only',
    why:
      'одноразовые ' +
      'задачи-«капчи» при входе по паролю — без неё вход по паролю открыт для перебора',
    wallWhy: W_AUTH_DEFINER,
    defect: ['I1-definer-plus-force'],
  },
  {
    t: 'public.password_login_identifier_protection',
    cls: 'S',
    org: false,
    wall: 'definer-only',
    why: 'защита от ' + 'перебора по идентификатору — без неё пароль подбирается без ограничений',
    wallWhy: W_AUTH_DEFINER,
    defect: ['I1-definer-plus-force'],
  },
  {
    t: 'public.patient_bookings',
    cls: 'P',
    org: true,
    why:
      'старые записи на приём — легаси-таблица записей; без ' +
      'неё теряется история бронирований до перехода на `be_appointments`',
    pol:
      'D17: сегодня off/off, и 263 строки ' +
  ...
  {
    t: 'public.patient_comorbidity',
    cls: 'P',
    org: true,
    why: 'сопутствующие заболевания — без неё врач не видит ' + 'фон пациента',
  },
  {
    t: 'public.patient_content_rating_feedback',
    cls: 'P',
    org: true,
    why: 'оценка материала пациентом — без неё нет ' + 'обратной связи по контенту',
  },
  {
    t: 'public.patient_daily_warmup_presentations',
    cls: 'P',
    org: true,
    why:
      'какая «разминка дня» показана ' +
      'пациенту — без неё не ротируется ежедневный контент — пациент видит одно и то же',
  },
  {
    t: 'public.patient_daily_warmup_video_views',
    cls: 'P',
    org: true,
    why: 'просмотры видео-разминки — без неё нет ' + 'отметки «сделал разминку» и админ-статистики',
    defect: ['D27-empty-org-discriminator'],
    gate: ['O3-empty-tenant-discriminator'],
  },
  {
    t: 'public.patient_diary_day_snapshots',
    cls: 'P',
    org: true,
    why:
      'слепок дня пациента — без неё дневник и ' + '«активность по дням» в карточке пациента пусты',
    defect: ['D27-empty-org-discriminator'],
    gate: ['O3-empty-tenant-discriminator'],
  },
  {
    t: 'public.patient_files',
    cls: 'P',
    org: true,
    why:
      'файлы в карте пациента — без неё нет медицинских ' +
      'документов в карте и не считается квота хранилища клиники',
  },
  {
    t: 'public.patient_home_block_items',
    cls: 'C',
    org: true,
    why: 'элементы блоков — без неё блоки пустые',
  },
  {
    t: 'public.patient_home_blocks',
    cls: 'C',
    org: true,
    why: 'блоки главной пациента (настройка клиники) — без неё ' + 'главная пациента пустая',
  },
  {
    t: 'public.patient_invites',
    cls: 'P',
    org: true,
    wall: 'clinic',
    why:
      'приглашение пациента в портал — без неё ' +
      'врач не может пригласить пациента в личный кабинет',
    wallWhy:
      'секрет приглашения выпускает клиника; пациент ' + 'читает его не из таблицы, а по ссылке',
  },
  {
    t: 'public.patient_lfk_assignments',
    cls: 'P',
    org: true,
    why:
      'назначенные пациенту комплексы ЛФК — без неё ' + 'пациент не видит назначенных упражнений',
  },
  {
    t: 'public.patient_merge_candidates',
    cls: 'P',
    org: true,
    wall: 'clinic',
    why:
      'кандидаты на слияние дублей ' +
      'пациента — без неё дубли пациентов не всплывают админу клиники',
    wallWhy: 'разбор дублей — работа админа ' + 'клиники; пациенту эта очередь не показывается',
  },
  {
    t: 'public.patient_payment',
    cls: 'P',
    org: true,
    why: 'платежи пациента — без неё нет финансовой истории по ' + 'пациенту',
  },
  {
    t: 'public.patient_practice_completions',
    cls: 'P',
    org: true,
    why:
      'выполненные практики и самочувствие — без ' +
      'неё нет календаря упражнений и трекинга самочувствия',
    defect: ['D27-empty-org-discriminator'],
    gate: ['O3-empty-tenant-discriminator'],
  },
  {
    t: 'public.patient_specialist_links',
    cls: 'P',
    org: true,
    why:
      'связь «пациент ↔ специалист» — без неё «свой ' +
      'пациент» невыразим (VISIBILITY_MODEL_GAP §1)',
    defect: ['D24-dev-force-off'],
  },
  {
    t: 'public.phone_challenges',
    cls: 'S',
    org: false,
    wall: 'definer-only',
    why:
      'SMS-челленджи входа, код ОТП ' +
      'лежит открытым текстом — без неё нет входа по телефону и публичной записи',
    wallWhy: W_AUTH_DEFINER,
    revoke: { app_staff: REV_D1 },
    defect: ['D1-auth-tables'],
    code: ['C13'],
  ...
  {
    t: 'public.phone_messenger_bind_secrets',
    cls: 'S',
    org: false,
    wall: 'definer-only',
    why:
      'секреты привязки ' +
      'мессенджера к телефону — без неё нельзя привязать Telegram/MAX к аккаунту',
    wallWhy: W_AUTH_DEFINER,
    revoke: { app_staff: REV_D1 },
    defect: ['D1-auth-tables'],
    code: ['C13'],
  ...
  {
    t: 'public.phone_otp_locks',
    cls: 'S',
    org: false,
    wall: 'definer-only',
    why: 'блокировки по телефону после ' + 'неудачных ОТП — без неё ОТП перебирается',
    wallWhy: W_AUTH_DEFINER,
    revoke: { app_staff: REV_D1 },
    defect: ['D1-auth-tables'],
    code: ['C13'],
  ...
  {
    t: 'public.platform_user_contacts',
    cls: 'P',
    org: true,
    why:
      'дополнительные контакты человека — без неё нет ' +
      'запасных телефонов/почт пациента для связи и дедупликации',
    pol:
      'D7: нет ветки current_patient_user_id(), ' +
  ...
  {
    t: 'public.platform_users',
    cls: 'P',
    org: false,
    why: 'единственная таблица ПДн — без неё нет ни одного ' + 'человека в системе',
    grants: {
      app_patient: {
        privs: [
          {
        ...
        why:
          'пациент правит СВОИ настройки напоминаний; колоночный грант — живой механизм (FACTS §1.4, evidence/13 ' +
          '§2.5). Табличная проверка без колоночной здесь врёт.',
      },
      app_web_push_reminder_discovery_definer: {
        privs: [{ kind: 'columns', priv: 'SELECT', columns: ['reminder_muted_until'] }],
        why:
          'discovery-шов web-push ' +
          'читает ровно одну колонку, чтобы не будить замьюченных (evidence/13 §2.5).',
      },
    },
    revoke: {
      app_identity_bootstrap:
        'D5: bootstrap-политики проверяют «кто ты» (pg_has_role … member) и ничего про ' +
      ...
      bcb_test_nonstaff_login:
        'D5/I2: табличный SELECT выдан ЛОГИН-роли — под ним читаются все 278 строк ПДн; ' +
        'гранты живут на рантайм-ролях, логин получает права членством',
    },
    pol:
      'RLS+FORCE (SCHEME §I Р3 — единственная стена на 278 строк ПДн). Предикат обязан фильтровать СТРОКУ, а не ' +
      'роль.',
    defect: ['D5-identity-bootstrap', 'I15-bootstrap-form', 'I2-grant-to-login'],
    code: ['C13'],
  },
  {
    t: 'public.product_analytics_events_recent',
    cls: 'P',
    org: true,
    why: 'сырые события продукта — без неё нет ' + 'продуктовой аналитики и воронки регистрации',
    revoke: {
      app_platform_settings:
        'РЕШЕНИЕ D1: политика product_analytics_registration_platform_operations_select ' +
        'даёт кросс-арендные события регистрации с user_id',
    },
    code: ['C2'],
  },
  {
    t: 'public.product_analytics_hourly',
    cls: 'C',
    org: true,
    why:
      'агрегат событий по часам (без человека) — без ' + 'неё нет агрегированных графиков продукта',
    pol:
      'D23: relrowsecurity=false при pol=1 — политика написана и ' +
  ...
  {
    t: 'public.product_analytics_user_hourly',
    cls: 'P',
    org: true,
    why:
      'почасовая активность человека — без неё ' + 'врач не видит, заходит ли пациент в приложение',
  },
  {
    t: 'public.product_push_notifications',
    cls: 'P',
    org: true,
    why:
      'отправленные push’и — без неё нельзя связать ' + 'открытие приложения с конкретным push’ем',
    pol: 'I5/C18: сырой current_setting в web-push-политике.',
    defect: ['I5-two-org-accessors'],
    code: ['C18'],
  },
  {
    t: 'public.program_action_log',
    cls: 'P',
    org: true,
    why:
      'действия пациента по программе лечения — без неё врач ' +
      'не видит, что пациент делал по программе',
  },
  {
    t: 'public.program_item_discussion_messages',
    cls: 'P',
    org: true,
    why:
      'переписка врач↔пациент по пункту ' +
      'программы — без неё нет комментариев к упражнению — ключевой канал общения',
  },
  {
    t: 'public.program_item_discussion_reads',
    cls: 'P',
    org: true,
    why: 'отметки прочтения обсуждения — без неё ' + 'счётчики непрочитанного врут',
  },
  {
    t: 'public.recommendation_regions',
    cls: 'C',
    org: true,
    why:
      'связь рекомендация↔область тела — без неё не ' + 'работают фильтры каталога по области тела',
  },
  {
    t: 'public.recommendations',
    cls: 'C',
    org: true,
    why: 'справочник рекомендаций клиники — без неё врачу нечего ' + 'назначать',
  },
  {
    t: 'public.reference_catalog_baselines',
    cls: 'R',
    org: false,
    wall: 'definer-only',
    why:
      'версионированные ' +
      'глобальные шаблоны справочников — без них новая клиника создаётся пустой',
    wallWhy:
      'платформенный ' +
      'шаблон-эталон: вход только через засевочный definer-шов, арендной роли грантов нет',
  },
  {
    t: 'public.reference_catalog_snapshot_receipts',
    cls: 'T',
    org: true,
    wall: 'reference-org-copy',
    why:
      'расписка «этой клинике справочник уже засеян» — без неё справочник клиники будет пересеян поверх правок ' +
      'врача',
    wallWhy:
      'расписка о засеве копии ДЛЯ ЭТОЙ организации — истинная org-таблица (G7 закрыт), а не ' +
      'глобальный справочник',
    pol:
      'RLS+FORCE обязателен (D26), но включать осознанно: расписку читают ПОЛИТИКИ ' +
      'reference_categories/_items через EXISTS. Гранты — только app_owner',
    defect: ['D26-receipts-no-rls'],
  },
  {
    t: 'public.reference_categories',
    cls: 'C',
    org: true,
    wall: 'reference-org-copy',
    why: 'категории справочников ' + 'клиники — без неё пусты все выпадающие списки каталогов',
    wallWhy: W_REF_COPY,
    pol:
      'ЭТАЛОН D3: staff по org ' +
  ...
  {
    t: 'public.reference_items',
    cls: 'C',
    org: true,
    wall: 'reference-org-copy',
    why: 'элементы справочников ' + 'клиники — без них выпадающие списки каталогов пусты',
    wallWhy: W_REF_COPY,
    pol: 'та же эталонная форма D3, ' + 'что у reference_categories',
  },
  {
    t: 'public.reminder_delivery_events',
    cls: 'P',
    org: true,
    why:
      'события доставки напоминаний из интегратора — ' +
      'без неё не видно, дошло ли напоминание, и не считается здоровье конвейера',
    pol:
      '⚠ evidence/18 §6: дубль ' +
      'integrator.user_reminder_delivery_logs 1735/1735 — одна из двух уходит',
  },
  {
    t: 'public.reminder_journal',
    cls: 'P',
    org: true,
    why:
      'действия пациента с напоминанием — без неё пациент не ' +
      'видит истории «отложил/пропустил»',
    defect: ['D27-empty-org-discriminator'],
    gate: ['O3-empty-tenant-discriminator'],
  },
  {
    t: 'public.reminder_occurrence_history',
    cls: 'P',
    org: true,
    why:
      'история срабатываний напоминаний — без неё ' +
      'нет истории напоминаний и статистики соблюдения режима',
  },
  {
    t: 'public.reminder_rules',
    cls: 'P',
    org: true,
    why: 'правила напоминаний пациенту — без неё пациент перестаёт ' + 'получать напоминания',
  },
  {
    t: 'public.saas_billing_accounts',
    cls: 'C',
    why: 'платёжный профиль клиники — без неё клиника не выставит счёт',
    defect: ['D4-role-escalation'],
  },
  {
    t: 'public.saas_billing_invoices',
    cls: 'C',
    why: 'счета — оплата подписки',
    defect: ['D4-role-escalation'],
  },
  {
    t: 'public.saas_billing_periods',
    cls: 'R',
    why: 'справочник периодов оплаты — выбор «месяц/год» при оплате',
    pol:
      'I9: сегодня закрыто ГРАНТОМ (только app_platform_settings), а не политикой; у saas_tariffs — RLS+FORCE и ' +
  ...
  {
    t: 'public.saas_billing_provider_events',
    cls: 'C',
    why: 'вебхуки провайдера — идемпотентность оплаты',
    defect: ['D4-role-escalation'],
  },
  {
    t: 'public.saas_billing_refunds',
    cls: 'C',
    why: 'возвраты — возврат денег клинике',
    pol:
      'D13: у клиники ни ' +
  ...
  {
    t: 'public.saas_billing_subscriptions',
    cls: 'C',
    why: 'подписка клиники — доступ клиники к продукту',
    defect: ['D4-role-escalation'],
  },
  {
    t: 'public.saas_isolation_coverage_runs',
    cls: 'S',
    owner: 'saas_telemetry_owner',
    why: 'прогоны покрытия — гейт ' + 'деплоя TEST',
  },
  {
    t: 'public.saas_isolation_event_hourly',
    cls: 'S',
    owner: 'saas_telemetry_owner',
    why: 'почасовая агрегация — ' + 'тренд изоляции на экране здоровья',
  },
  {
    t: 'public.saas_isolation_events',
    cls: 'S',
    owner: 'saas_telemetry_owner',
    why: 'события нарушения изоляции — ' + 'без неё платформа не видит собственные утечки',
  },
  {
    t: 'public.saas_org_entitlement_overrides',
    cls: 'C',
    why: 'ручные включения механик клинике — точечная выдача ' + 'функций клинике',
    defect: ['D4-role-escalation'],
  },
  {
    t: 'public.saas_organization_trials',
    cls: 'C',
    why: 'триал клиники — бесплатный период',
    defect: ['D4-role-escalation'],
  },
  {
    t: 'public.saas_paid_period_policy',
    cls: 'S',
    why:
      'поведение после окончания оплаченного периода — что ' +
      'происходит с клиникой после неоплаты',
    pol:
      'I8: у сестринских ' +
  ...
  {
    t: 'public.saas_registration_tariff_policy',
    cls: 'S',
    why: 'тариф по умолчанию при регистрации — на каком ' + 'тарифе стартует новая клиника',
    defect: ['D4-role-escalation'],
  },
  {
    t: 'public.saas_tariffs',
    cls: 'R',
    why: 'тарифы платформы — без него клиника не понимает, что ей доступно',
    defect: ['D4-role-escalation'],
  },
  {
    t: 'public.saas_trial_policy',
    cls: 'S',
    why: 'глобальная политика триала — правило «сколько длится триал»',
    defect: ['D4-role-escalation'],
  },
  {
    t: 'public.schema_migrations',
    cls: 'T',
    why: 'журнал миграций integrator — без него мигратор перезальёт схему',
    drop: {
      verdict: 'DUP-DROP',
      source:
        'evidence/18 §4 — 73/73 в public.webapp_schema_migrations, журнал замёрз ' +
        '2026-04-13, единственный читатель недостижим',
      blockedBy:
        'снять дамп 73 строк; удалить мёртвый ' +
  ...
  {
    t: 'public.specialist_signup_intents',
    cls: 'S',
    wall: 'definer-only',
    why: 'заявка на создание клиники — ' + 'самостоятельная регистрация специалиста',
    wallWhy:
      'заявка на регистрацию живёт ДО организации — стены ' +
      'клиники нет, вход через definer-шов регистрации',
    defect: ['I1-definer-plus-force'],
  },
  {
    t: 'public.specialist_tasks',
    cls: 'P',
    why: 'задачи врача по пациенту — пропадёт список задач врача и ' + 'напоминания по ним',
  },
  {
    t: 'public.staff_security_profiles',
    cls: 'S',
    wall: 'definer-only',
    why: 'второй фактор персонала — 2FA ' + 'сотрудников',
    wallWhy: W_AUTH_DEFINER,
    defect: ['I1-definer-plus-force'],
  },
  {
    t: 'public.support_conversation_messages',
    cls: 'P',
    why: 'сообщения диалога — тело переписки',
  },
  {
    t: 'public.support_conversations',
    cls: 'P',
    why: 'диалоги поддержки — без неё нет переписки врач↔пациент',
  },
  {
    t: 'public.support_delivery_events',
    cls: 'P',
    why: 'журнал доставки сообщений — без него не видно, дошло ли ' + 'сообщение',
  },
  { t: 'public.support_question_messages', cls: 'P', why: 'реплики внутри вопроса — тело вопроса' },
  {
    t: 'public.support_questions',
    cls: 'P',
    why: 'вопросы пациента из бота — очередь «вопрос из мессенджера → врач»',
  },
  ...
  {
    t: 'public.system_settings',
    cls: 'S',
    wall: 'platform-role+clinic',
    why: 'настройки платформы и клиники — без ' + 'неё не работает ни один внешний канал',
    wallWhy: W_PLATFORM_OR_CLINIC,
    revoke: {
      app_staff:
    ...
    pol:
      'ветка organization_id IS NULL ОБЯЗАНА проверять роль: saas_bootstrap_hybrid_p0_8_6 выдана TO public и её ' +
      'первая ветка безусловна — это и есть механизм дефекта',
    defect: ['D3-system-settings', 'D4-role-escalation'],
  },
  {
    t: 'public.system_settings_audit',
    cls: 'S',
    wall: 'platform-role+clinic',
    why: 'история изменений настроек — ' + 'доказательство «кто менял секрет»',
    wallWhy: W_PLATFORM_OR_CLINIC,
    revoke: {
      app_staff:
        'D3: значения секретов лежат в old_value_json/new_value_json (аудит 28.07 нашёл там ' +
  ...
  {
    t: 'public.test_attempts',
    cls: 'P',
    why: 'попытки прохождения теста — пациент не сможет сдать тест',
    pol:
      'РЕШЕНИЕ D2: пациентская ветка обязана резолвиться ЧЕРЕЗ ПРОГРАММУ (instance_stage_item_id → ' +
      'treatment_program_instance_stage_items → instances.patient_user_id), а не по плоской patient_user_id',
    code: ['C14'],
  },
  {
    t: 'public.test_results',
    cls: 'P',
    why: 'результат попытки — оценка теста',
    pol:
      'РЕШЕНИЕ D2: пациентская ветка ' +
      '— только через test_attempts, привязанную к элементу его программы (см. test_attempts).',
    code: ['C14'],
  },
  ...
  { t: 'public.test_sets', cls: 'C', why: 'наборы тестов — пакетное назначение тестов' },
  {
    t: 'public.tests',
    cls: 'C',
    why: 'каталог клинических тестов клиники — без него врач не назначит тест',
  },
  {
    t: 'public.treatment_program_events',
    cls: 'P',
    why: 'журнал изменений программы — аудит «кто что менял в ' + 'лечении»',
  },
  {
    t: 'public.treatment_program_instance_stage_groups',
    cls: 'P',
    why: 'группы внутри этапа — группировка заданий',
  },
  {
    t: 'public.treatment_program_instance_stage_items',
    cls: 'P',
    why: 'сами задания — что пациент делает каждый день',
  },
  {
    t: 'public.treatment_program_instance_stages',
    cls: 'P',
    why: 'этапы программы — шаги лечения',
  },
  {
    t: 'public.treatment_program_instances',
    cls: 'P',
    why: 'назначенная пациенту программа — ядро лечения — без неё ' + 'нет программы',
  },
  {
    t: 'public.treatment_program_template_stage_groups',
    cls: 'C',
    why: 'группы в этапе шаблона — группировка в ' + 'шаблоне',
  },
  {
    t: 'public.treatment_program_template_stage_items',
    cls: 'C',
    why: 'задания шаблона — содержимое шаблона',
  },
  {
    t: 'public.treatment_program_template_stages',
    cls: 'C',
    why: 'этапы шаблона — структура шаблона',
  },
  {
    t: 'public.treatment_program_templates',
    cls: 'C',
    why: 'шаблоны программ лечения — без них нечего назначать ' + 'пациенту',
  },
  {
    t: 'public.user_channel_bindings',
    cls: 'P',
    org: false,
    why: 'привязка мессенджера — вход через Telegram/MAX и ' + 'рассылки',
    revoke: {
      app_staff:
        'D20: organization_id нет, RLS выключен, политик ноль — сотрудник любой клиники правит ' +
        'external_id мессенджеров всех 131 привязки платформы',
    },
    pol:
      'ветка «свой пациент» по user_id. Сегодня app_patient=r без единой политики — любой пациент читает ' +
      'идентификаторы всех людей в Telegram/MAX',
    defect: ['D20-notification-tables'],
  },
  {
    t: 'public.user_channel_preferences',
    cls: 'P',
    why: 'согласия по каналам — по какому каналу писать пациенту',
    pol:
      'D20: политика c4_web_push_reminder_user в каталоге ЕСТЬ, но relrowsecurity=false — перепись по pol=N ' +
      'показывает «стена есть», а её нет',
    defect: ['D20-notification-tables'],
  },
  {
    t: 'public.user_contacts',
    cls: 'P',
    org: false,
    why: 'сводный индекс контактов — вход по почте/телефону и поиск ' + 'пациента',
    revoke: {
      app_identity_bootstrap:
        'D5: предикат обязан фильтровать строку; bootstrap уезжает в definer (I15).',
    },
    pol:
      'D2-user-contacts-write: staff-политики UPDATE/DELETE/INSERT несут ровно app.is_staff() БЕЗ ' +
      'organization_id, а PERMISSIVE объединяются по OR — подмена value_normalized уводит вход на чужой аккаунт. D5: ' +
      'bootstrap-политики фильтруют роль, а не строку',
    defect: ['D2-user-contacts-write', 'D5-identity-bootstrap'],
    gate: ['O5-user-identity-cutover'],
  },
  {
    t: 'public.user_email_setup_tokens',
    cls: 'S',
    wall: 'pending-removal',
    rls: 'n/a',
    disp: 'REMOVED',
    why: 'УДАЛЕНО миграцией 0388: старые ссылочные токены настройки email заменены живым password_setup OTP flow',
    wallWhy:
      'Физически удалённая legacy-таблица остаётся именованной только для двусторонней проверки каталога',
  },
  {
    t: 'public.user_identity',
    cls: 'P',
    org: false,
    why: 'ФИО и дата рождения — имя пациента во всех экранах',
    revoke: { app_identity_bootstrap: 'D5: тот же дефект — «кто ты» вместо «какая строка».' },
    pol:
      'D6: user_identity_staff_insert несёт WITH CHECK (app.is_staff()) без org — сотрудник любой клиники заводит ' +
  ...
  {
    t: 'public.user_notification_topic_channels',
    cls: 'P',
    why: 'тема × канал — тонкая настройка уведомлений',
    pol: 'D20: инертная политика при выключенном RLS',
    defect: ['D20-notification-tables'],
  },
  {
    t: 'public.user_notification_topics',
    cls: 'P',
    why: 'подписки на темы — пациент перестанет управлять ' + 'уведомлениями',
    pol:
      'плюс ветка своего пациента. D20: app_patient=arw без единой политики — пациент правит ' +
      'чужие подписки на уведомления (349 строк)',
    defect: ['D20-notification-tables'],
  },
  {
    t: 'public.user_oauth_bindings',
    cls: 'S',
    wall: 'definer-only',
    why: 'привязки соцвходов — вход через ' + 'Google/VK/Яндекс',
    wallWhy: W_AUTH_DEFINER,
    revoke: { app_staff: REV_D1 },
    defect: ['D1-auth-tables'],
    code: ['C13'],
  ...
  {
    t: 'public.user_passkey_accounts',
    cls: 'S',
    wall: 'definer-only',
    why: '`user_handle` для WebAuthn — вход по ' + 'passkey',
    wallWhy: W_AUTH_DEFINER,
    defect: ['I1-definer-plus-force'],
  },
  {
    t: 'public.user_passkey_challenges',
    cls: 'S',
    wall: 'definer-only',
    why: 'вызовы WebAuthn — защита от повтора',
    wallWhy: W_AUTH_DEFINER,
    defect: ['I1-definer-plus-force'],
  },
  {
    t: 'public.user_passkey_credentials',
    cls: 'S',
    wall: 'definer-only',
    why: 'ключи — сам вход по passkey',
    wallWhy: W_AUTH_DEFINER,
    defect: ['I1-definer-plus-force'],
  },
  {
    t: 'public.user_password_credentials',
    cls: 'S',
    wall: 'definer-only',
    why: 'хэши паролей — вход по паролю',
    wallWhy: W_AUTH_DEFINER,
    revoke: { app_staff: REV_D1 },
    defect: ['D1-auth-tables'],
    code: ['C13'],
  ...
  {
    t: 'public.user_phone_history',
    cls: 'P',
    why: 'история телефонов — смена номера и поиск по старому номеру',
    revoke: {
      bcb_test_nonstaff_login:
        'I2: табличный грант arw выдан ЛОГИН-роли напрямую, минуя рантайм-роль.',
    },
    pol:
      'D8: единственная политика несёт ТОЛЬКО org-ветку, а app_patient держит SELECT — пациент видит историю ' +
  ...
  {
    t: 'public.user_web_push_subscriptions',
    cls: 'P',
    why: 'push-подписки браузера — без неё нет web-push',
    pol:
      'D20: у app_patient полный arwd (в том числе DELETE) при инертной политике — пациент удаляет чужие ' +
      'push-подписки',
    defect: ['D20-notification-tables'],
  },
  {
    t: 'public.webapp_schema_migrations',
    cls: 'T',
    rls: 'off',
    why:
      'журнал миграций webapp (89 строк) — миграции ' + 'применяются повторно или не применяются',
    rlsWhy: RLS_OFF_MIGRATOR_LEDGER,
  },
];
...
const db_bersoncarebot_test: DatabaseDecl = {
  database: {
    ...
    connect: [
      // ЦЕЛЬ: явный CONNECT после отзыва у PUBLIC (§D.1); рендерится из envMapping
      'bersoncarebot_test',
      'bcb_test_integrator_login',
      'bcb_test_nonstaff_login',
      'bcb_test_staff_login',
    ...
    note:
      'evidence/13 §1.1: живой datacl = PUBLIC=Tc, owner=CTc, app_operational_web_push_reminder=c, ' +
  ...
  schemas: {
    ...
    app_ext: {
      owner: 'postgres',
      present: true, // G5 закрыт: канонический владелец = postgres на ОБЕИХ базах (§C)
      usage: ['app_owner'],
      create: ['postgres'],
      why: 'pgcrypto-шов (app_ext.hmac, p2-b:94). evidence/13 §2.1: postgres=UC, app_owner=U.',
    },
    drizzle: {
      owner: 'bersoncarebot_test',
      present: true,
      usage: ['bersoncarebot_test'],
    ...
    integrator: {
      ...
      present: true,
      usage: [
        // evidence/13 §2.1 — TEST даёт diagnostic/delivery/scheduler (отличие от dev)
        'bersoncarebot_test',
        'app_staff',
        'app_patient',
        'bcb_test_integrator_login',
      ...
      create: ['bersoncarebot_test'],
      why:
        '⚠ USAGE у app_staff/app_patient переживёт волны сноса evidence/15 только там, где останутся живые ' +
    ...
    app_control: {
      owner: 'postgres',
      present: false, // ⚠ evidence/13 §2.5: отсутствует на обеих — стена НЕ установлена
      usage: [],
      create: ['postgres'],
      why:
        'схема стены (org_table_allowlist, privileges_watermark, ddl_wall_log). Строится wall-install каждым ' +
  ...
  sequences: {
    rule:
      'роль с INSERT/UPDATE на таблице получает USAGE(+SELECT) на её *_id_seq (serial DEFAULT требует USAGE; ' +
  ...
  functionsViews: {
    default:
      'дефолтного EXECUTE нет; wall-install §D.5 снимает материализованный PUBLIC EXECUTE. EXECUTE на ' +
  ...
  orgTableAllowlist: {
    ...
    named: [
      // org-таблицы, названные переписью ДЕФЕКТНЫМИ (нет RLS или FORCE) — evidence/13 §2.3
    ...
    todo:
      'Derived set: 116 of the 239 classified tables declare org: true (the classification stated the ' +
...
const DEV_TABLE_DELTAS: Record<string, TableDecl> = {
  'public.patient_specialist_links': {
    ...
    drift:
      'DEV-ONLY ДЕФЕКТ (FINDINGS Д24, evidence/13 §2.3): на dev relrowsecurity=t, relforcerowsecurity=f — ' +
...
const db_bcb_webapp_dev: DatabaseDecl = {
  ...
  schemas: {
    app: {
      ...
      present: true,
      usage: [
        // evidence/13 §2.2 — здесь PUBLIC USAGE НЕТ (отличие от TEST)
        'app_staff',
        'app_patient',
        'bcb_webapp_dev_user',
        'app_platform_settings',
        ...
        'saas_system_health_owner',
        'app_integrator_resolver',
        'app_operational_maintenance', // НОВЫЕ роли (D5/D8)
      ],
      create: ['app_owner'],
    },
    app_ext: {
      ...
      create: ['postgres'],
      ownerDrift:
        'ЖИВОЙ владелец на dev = bcb_webapp_dev_user (evidence/13 §2.2). Приводится к postgres: ' +
        'extension-шов принадлежит суперпользователю (§C), TEST уже так.',
    },
    drizzle: {
      owner: 'bcb_webapp_dev_user',
      present: true,
      usage: ['bcb_webapp_dev_user'],
    ...
    integrator: {
      ...
      present: true,
      usage: [
        // evidence/13 §2.2 — dev даёт из операционных только delivery (на TEST ещё diag/scheduler)
        'bcb_webapp_dev_user',
        'app_staff',
        'app_patient',
        'app_owner',
    ...
    app_control: {
      owner: 'postgres',
      present: false, // ⚠ отсутствует и на dev (evidence/13 §2.5)
      usage: [],
      create: ['postgres'],
  ...
  functionsViews: {
    default:
      'та же политика, что на TEST (§A.5): дефолтного EXECUTE нет; представлениям нужен security_invoker.',
  ...
  orgTableAllowlist: {
    derivedFrom: 'tables[*].org === true',
    named: [
      // дефектные org-таблицы dev (evidence/13 §2.3), минус та, что под снос
      'public.be_organization_members',
      'public.outgoing_delivery_queue',
      'public.patient_bookings',
      'public.product_analytics_hourly',
    ...
    fullCountLive: 172,
    todo:
      'TODO(census-gap G2): the dev org-table total was not counted separately (the census counted ' +
...
const REV10_SEAM_OWNERS = [
  ...
  'app_seam_patient_org_projection_owner',
  'app_seam_patient_booking_owner',
  'app_seam_patient_self_actions_owner',
  'app_seam_reminder_patient_owner',
  'app_seam_reminder_materialization_owner',
  'app_seam_reminder_specialist_owner',
  'app_seam_reminder_appointment_owner',
  'app_seam_reminder_email_cooldown_owner',
  'app_seam_telemetry_patient_owner',
  'app_seam_telemetry_media_owner',
  'app_seam_telemetry_operator_owner',
  'app_seam_catalog_public_owner',
  'app_seam_catalog_admin_owner',
  'app_seam_org_directory_owner',
  'app_seam_telemetry_exclusion_owner',
  'saas_telemetry_owner',
  'saas_system_health_owner',
  'app_seam_login_token_owner',
  'app_seam_oauth_owner',
  'app_seam_phone_otp_owner',
...
const REV10_ROLES: Record<string, RoleDecl> = Object.fromEntries([
  ...REV10_RUNTIME.map((name) => [
    name,
    revision10Role('terminal', 'NONE', 'revision-10 runtime role'),
  ]),
  [
    'app_platform_settings',
    revision10Role('terminal', 'GLOBAL', 'global settings and system-health surface'),
  ],
  [
    'app_platform_admin',
    revision10Role('terminal', 'GLOBAL', 'cross-organization directory/admin surface'),
  ],
  ...REV10_SEAM_OWNERS.map((name) => [
    name,
    revision10Role('owner', 'NONE', 'revision-10 narrow seam owner'),
  ]),
  [
    'app_object_owner',
    revision10Role('owner', 'NONE', 'ordinary application objects only; no definer functions'),
  ],
  [
    'bcb_dev_migrator',
    revision10Role('service', 'NONE', 'local postgres migration wrapper identity'),
  ],
  [
    'bcb_test_migrator',
    revision10Role('service', 'NONE', 'local postgres migration wrapper identity'),
  ...
  [
    'postgres',
    {
      ...
      createrole: true,
      rolconfig: null,
      why: 'local administrative exception only',
    },
  ],
]);
...
const REV10_ENV_MAPPING: Record<string, Record<string, LoginRecord>> = {
  dev: {
    bcb_dev_webapp_staff: {
      ...
      createrole: false,
      inherit: false,
      passwordEnv: 'BCB_DEV_WEBAPP_STAFF_PASSWORD',
      rolconfig: null,
      connect: ['bcb_webapp_dev'],
    },
    bcb_dev_webapp_patient: {
      port: 'webapp',
      canonicalRole: 'app_patient',
      memberships: [rev10Membership('app_pre_session'), rev10Membership('app_patient')],
      login: true,
      superuser: false,
      bypassrls: false,
      createrole: false,
      inherit: false,
      passwordEnv: 'BCB_DEV_WEBAPP_PATIENT_PASSWORD',
      rolconfig: null,
      connect: ['bcb_webapp_dev'],
    },
    bcb_dev_webapp_global_admin: {
      port: 'webapp',
      canonicalRole: 'app_platform_settings',
      memberships: [
        rev10Membership('app_platform_settings'),
        rev10Membership('app_platform_admin'),
      ],
      login: true,
      superuser: false,
      bypassrls: false,
      createrole: false,
      inherit: false,
      passwordEnv: 'BCB_DEV_WEBAPP_GLOBAL_ADMIN_PASSWORD',
      rolconfig: null,
      connect: ['bcb_webapp_dev'],
    },
    bcb_dev_integrator: {
      port: 'integrator',
      canonicalRole: 'app_integrator_request',
      memberships: [
        ...[
          'app_integrator_request',
          'app_integrator_resolver',
      ...
      createrole: false,
      inherit: false,
      passwordEnv: 'BCB_DEV_INTEGRATOR_PASSWORD',
      rolconfig: null,
      connect: ['bcb_webapp_dev'],
    },
  },
  test: {
    bcb_test_webapp_staff: {
      ...
      createrole: false,
      inherit: false,
      passwordEnv: 'BCB_TEST_WEBAPP_STAFF_PASSWORD',
      rolconfig: null,
      connect: ['bersoncarebot_test'],
    },
    bcb_test_webapp_patient: {
      ...
      createrole: false,
      inherit: false,
      passwordEnv: 'BCB_TEST_WEBAPP_PATIENT_PASSWORD',
      rolconfig: null,
      connect: ['bersoncarebot_test'],
    },
    bcb_test_webapp_global_admin: {
      ...
      createrole: false,
      inherit: false,
      passwordEnv: 'BCB_TEST_WEBAPP_GLOBAL_ADMIN_PASSWORD',
      rolconfig: null,
      connect: ['bersoncarebot_test'],
    },
    bcb_test_integrator: {
      ...
      createrole: false,
      inherit: false,
      passwordEnv: 'BCB_TEST_INTEGRATOR_PASSWORD',
      rolconfig: null,
      connect: ['bersoncarebot_test'],
    },
  },
...
const INTEGRATOR_SERVICE_SOURCES = ['integrator-health-check'] as const;
...
const REV10_CONTEXT = {
  ...
  privateRelations: {
    ...
    'app_control.relation_wall_registry': {
      owner: 'postgres',
      columns: ['schema_name', 'table_name', 'data_class', 'wall', 'expected_owner'],
    },
    ...
    'app_ext.variant_a_identity_refs': {
      ...
      columns: ['physical_user_id', 'opaque_ref', 'created_at'],
    },
  },
  capabilities: {
    integrator_request_relation: {
      port: 'integrator',
      runtimeName: 'request',
    ...
    integrator_resolver_relation: {
      port: 'integrator',
      runtimeName: 'resolver',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_integrator_resolver',
      contextClass: 'integrator',
      purpose: 'relation',
    },
    integrator_delivery_relation: {
      port: 'integrator',
      runtimeName: 'delivery',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_operational_delivery_worker',
      contextClass: 'service',
      purpose: 'relation',
      runtimeSources: INTEGRATOR_DELIVERY_SOURCES,
    },
    integrator_scheduler_relation: {
      port: 'integrator',
      ...
      contextClass: 'service',
      purpose: 'relation',
      runtimeSources: INTEGRATOR_SCHEDULER_SOURCES,
    },
    integrator_tenant_service_relation: {
      port: 'integrator',
      runtimeName: 'tenant_service',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_tenant_service',
      contextClass: 'tenant_service',
      purpose: 'relation',
    },
    integrator_service_relation: {
      port: 'integrator',
      runtimeName: 'service',
      sessionRole: 'app_integrator_request',
      ...
      contextClass: 'service',
      purpose: 'relation',
      runtimeSources: INTEGRATOR_SERVICE_SOURCES,
    },
    integrator_migration_ledger_read: {
      port: 'integrator',
      runtimeName: 'migration_ledger',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_service',
      contextClass: 'service',
      purpose: 'migration.ledger.read',
      functionIdentity: 'app.read_integrator_migration_ledger()',
      runtimeSources: INTEGRATOR_MIGRATION_LEDGER_SOURCES,
    },
    integrator_projection_health_read: {
      port: 'integrator',
      runtimeName: 'projection_health',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_service',
      contextClass: 'service',
    ...
    integrator_provider_runtime_setting_read: {
      port: 'integrator',
      runtimeName: 'provider_runtime_setting',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_service',
      contextClass: 'service',
    ...
    integrator_auth_channel_setting_read: {
      port: 'integrator',
      runtimeName: 'auth_channel_setting',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_service',
      contextClass: 'service',
    ...
    integrator_smtp_outbound_setting_read: {
      port: 'integrator',
      runtimeName: 'smtp_outbound_setting',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_service',
      contextClass: 'service',
    ...
    integrator_idempotency_acquire: {
      port: 'integrator',
      runtimeName: 'idempotency_acquire',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_service',
      contextClass: 'service',
      purpose: 'integrator.idempotency.acquire',
      functionIdentity: 'app.try_acquire_integrator_idempotency(text,integer)',
    },
    integrator_idempotency_release: {
      port: 'integrator',
      runtimeName: 'idempotency_release',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_service',
      contextClass: 'service',
      purpose: 'integrator.idempotency.release',
      functionIdentity: 'app.release_integrator_idempotency(text)',
    },
    integrator_data_quality_upsert: {
      port: 'integrator',
      runtimeName: 'data_quality_upsert',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_service',
      contextClass: 'service',
    ...
    count_active_canonical_appointments: {
      port: 'integrator',
      runtimeName: 'count_active_canonical_appointments',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_service',
      contextClass: 'service',
      purpose: 'booking.admin-active.count',
      functionIdentity: 'app.count_active_canonical_appointments()',
    },
    read_patient_telegram_display_handle: {
      port: 'webapp',
      runtimeName: 'read_patient_telegram_display_handle',
      sessionRole: 'app_staff',
    ...
    read_canonical_appointment_by_external_id: {
      port: 'webapp',
      runtimeName: 'read_canonical_appointment_by_external_id',
      sessionRole: 'app_staff',
      targetRole: 'app_worker',
      contextClass: 'service',
      purpose: 'booking.integrator-record.read',
      functionIdentity: 'app.read_canonical_appointment_by_external_id(text)',
    },
    list_active_canonical_appointments_by_phone: {
      port: 'webapp',
      runtimeName: 'list_active_canonical_appointments_by_phone',
      sessionRole: 'app_staff',
      targetRole: 'app_worker',
      contextClass: 'service',
      purpose: 'booking.integrator-active.read',
      functionIdentity: 'app.list_active_canonical_appointments_by_phone(text)',
    },
    webapp_staff_identity_resolve: {
      port: 'webapp',
      runtimeName: 'staff_identity_resolve',
      sessionRole: 'app_staff',
    ...
    webapp_patient_identity_resolve: {
      port: 'webapp',
      runtimeName: 'patient_identity_resolve',
    ...
    webapp_global_admin_identity_resolve: {
      port: 'webapp',
      runtimeName: 'globalAdmin_identity_resolve',
      sessionRole: 'app_platform_settings',
      targetRole: 'app_platform_admin',
      contextClass: 'pre_session',
      purpose: 'identity.variant-a.resolve',
      functionIdentity: 'app.pre_session_resolve_identity(uuid)',
    },
    webapp_staff_relation: {
      port: 'webapp',
      runtimeName: 'staff',
      sessionRole: 'app_staff',
      targetRole: 'app_staff',
    ...
    webapp_clinic_billing_relation: {
      port: 'webapp',
      runtimeName: 'clinicBilling',
      sessionRole: 'app_staff',
      targetRole: 'app_clinic_billing',
      contextClass: 'staff',
      purpose: 'relation',
    },
    webapp_platform_relation: {
      port: 'webapp',
      runtimeName: 'platform',
      sessionRole: 'app_platform_settings',
      targetRole: 'app_platform_settings',
      contextClass: 'platform',
      purpose: 'relation',
    },
    webapp_platform_admin_relation: {
      port: 'webapp',
      runtimeName: 'platform_admin',
      sessionRole: 'app_platform_settings',
      targetRole: 'app_platform_admin',
      contextClass: 'platform',
      purpose: 'relation',
    },
    webapp_worker_relation: {
      port: 'webapp',
      runtimeName: 'worker',
      sessionRole: 'app_staff',
      targetRole: 'app_worker',
    ...
    webapp_media_relation: {
      port: 'webapp',
      runtimeName: 'media_worker',
      sessionRole: 'app_staff',
    ...
    webapp_telemetry_relation: {
      port: 'webapp',
      runtimeName: 'telemetry',
      sessionRole: 'app_staff',
      targetRole: 'saas_telemetry_operator',
      contextClass: 'service',
      purpose: 'relation',
      runtimeSources: WEBAPP_TELEMETRY_SOURCES,
    },
    saas_billing_invoice_webhook_resolve: {
      port: 'webapp',
      runtimeName: 'saas_billing_invoice_webhook_resolve',
      sessionRole: 'app_staff',
      targetRole: 'app_worker',
      contextClass: 'service',
    ...
    saas_billing_refund_webhook_resolve: {
      port: 'webapp',
      runtimeName: 'saas_billing_refund_webhook_resolve',
      sessionRole: 'app_staff',
      targetRole: 'app_worker',
      contextClass: 'service',
    ...
    saas_billing_provider_preauth_read: {
      port: 'webapp',
      sessionRole: 'app_patient',
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
    ...
    saas_billing_provider_clinic_read: {
      port: 'webapp',
      sessionRole: 'app_staff',
      targetRole: 'app_clinic_billing',
      contextClass: 'staff',
    ...
    saas_billing_provider_platform_read: {
      port: 'webapp',
      sessionRole: 'app_platform_settings',
      targetRole: 'app_platform_settings',
      contextClass: 'platform',
    ...
    password_login_complete: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.password.complete',
      functionIdentity: 'app.password_login_complete(uuid,boolean)',
    },
    password_login_read_altcha_secret: {
      port: 'webapp',
      sessionRole: 'app_patient',
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.password.altcha-secret',
    ...
    password_login_issue_altcha_challenge: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.password.altcha-issue',
      functionIdentity:
    ...
    auth_login_token_create: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.login-token.create',
    ...
    auth_login_token_expire_past: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.login-token.expire',
    ...
    auth_login_token_confirm: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.login-token.confirm',
    ...
    auth_login_token_mark_session_issued: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.login-token.session-issued',
    ...
    auth_oauth_find_user: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.oauth.callback.find-binding',
    ...
    auth_oauth_upsert_binding: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.oauth.callback.upsert-binding',
    ...
    email_auth_find_email_otp_lock: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.email-otp.lock.read',
    ...
    email_auth_register_email_otp_lockout: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.email-otp.lock.register',
    ...
    email_auth_reset_email_otp_lockout: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.email-otp.lock.reset',
    ...
    phone_auth_find_latest_challenge_created_at: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.phone-otp.cooldown.read',
    ...
    phone_auth_find_otp_lock: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.phone-otp.lock.read',
    ...
    phone_auth_register_otp_lockout: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.phone-otp.lock.register',
    ...
    phone_auth_reset_otp_lockout: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.phone-otp.lock.reset',
    ...
    phone_challenge_store_upsert: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.phone-challenge.upsert',
    ...
    phone_challenge_store_read: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.phone-challenge.read',
    ...
    phone_challenge_store_delete: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.phone-challenge.delete',
    ...
    phone_challenge_store_delete_by_phone: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.phone-challenge.delete-by-phone',
    ...
    phone_challenge_store_increment_attempts: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.phone-challenge.attempt.increment',
    ...
    phone_otp_public_booking_issue_challenge: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'booking.public-phone-otp.issue',
      functionIdentity:
    ...
    phone_otp_public_booking_consume_challenge: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'booking.public-phone-otp.consume',
    ...
    read_webapp_server_runtime_setting: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'config.runtime.server.read',
    ...
    is_smtp_outbound_configured: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.channel.smtp.configured',
      functionIdentity: 'app.is_smtp_outbound_configured()',
    },
    is_sms_provider_configured: {
      port: 'webapp',
      sessionRole: 'app_patient',
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.channel.sms.configured',
      functionIdentity: 'app.is_sms_provider_configured()',
    },
    is_telegram_login_configured: {
      port: 'webapp',
      sessionRole: 'app_patient',
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.channel.telegram.configured',
      functionIdentity: 'app.is_telegram_login_configured()',
    },
    is_max_bot_configured: {
      port: 'webapp',
      sessionRole: 'app_patient',
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.channel.max.configured',
      functionIdentity: 'app.is_max_bot_configured()',
    },
    passkey_issue_authentication_challenge: {
      port: 'webapp',
      sessionRole: 'app_patient',
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
    ...
    passkey_issue_registration_challenge: {
      port: 'webapp',
      sessionRole: 'app_patient',
      targetRole: 'app_patient',
      contextClass: 'patient',
    ...
    passkey_read_authentication_challenge: {
      port: 'webapp',
      sessionRole: 'app_patient',
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
    ...
    passkey_read_registration_challenge: {
      port: 'webapp',
      sessionRole: 'app_patient',
      targetRole: 'app_patient',
      contextClass: 'patient',
    ...
    passkey_read_credential: {
      port: 'webapp',
      sessionRole: 'app_patient',
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'auth.passkey.credential.read',
      functionIdentity: 'app.passkey_read_credential(text)',
    },
    passkey_complete_authentication: {
      port: 'webapp',
      sessionRole: 'app_patient',
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
    ...
    get_public_reference_baseline: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'catalog.public-reference.read',
    ...
    read_webapp_preauth_provider_setting: {
      port: 'webapp',
      sessionRole: 'app_patient',
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'config.preauth-provider.read',
    ...
    resolve_public_organization_by_slug: {
      ...
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'booking.public-organization.resolve',
      functionIdentity: 'app.resolve_public_organization_by_slug(text)',
    },
    resolve_public_organization_slug: {
      port: 'webapp',
      sessionRole: 'app_patient',
      targetRole: 'app_pre_session',
      contextClass: 'pre_session',
      purpose: 'booking.public-slug.resolve',
    ...
    mark_operator_incident_alert_sent: {
      port: 'integrator',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_operational_delivery_worker',
      contextClass: 'service',
      purpose: 'delivery.incident-alert-mark',
      functionIdentity: 'app.mark_operator_incident_alert_sent(uuid)',
    },
    ...
    get_google_calendar_event_id: {
      port: 'integrator',
      runtimeName: 'get_google_calendar_event_id',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_tenant_service',
      contextClass: 'tenant_service',
      purpose: 'calendar.map.get',
      functionIdentity: 'app.get_google_calendar_event_id(uuid)',
    },
    upsert_google_calendar_event_id: {
      port: 'integrator',
      runtimeName: 'upsert_google_calendar_event_id',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_tenant_service',
      contextClass: 'tenant_service',
      purpose: 'calendar.map.upsert',
      functionIdentity: 'app.upsert_google_calendar_event_id(uuid,text)',
    },
    delete_google_calendar_event_id: {
      port: 'integrator',
      runtimeName: 'delete_google_calendar_event_id',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_tenant_service',
      contextClass: 'tenant_service',
      purpose: 'calendar.map.delete',
      functionIdentity: 'app.delete_google_calendar_event_id(uuid)',
    },
    read_booking_calendar_patient_profile: {
      port: 'integrator',
      runtimeName: 'read_booking_calendar_patient_profile',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_tenant_service',
      contextClass: 'tenant_service',
      purpose: 'calendar.patient-profile.read',
      functionIdentity: 'app.read_booking_calendar_patient_profile(uuid)',
    },
    read_booking_calendar_latest_staff_comment: {
      port: 'integrator',
      runtimeName: 'read_booking_calendar_latest_staff_comment',
      sessionRole: 'app_integrator_request',
      targetRole: 'app_tenant_service',
      contextClass: 'tenant_service',
      purpose: 'calendar.staff-comment.read',
      functionIdentity: 'app.read_booking_calendar_latest_staff_comment(uuid)',
    },
    is_current_patient_self_booking_allowed: {
      port: 'webapp',
      runtimeName: 'is_current_patient_self_booking_allowed',
      sessionRole: 'app_patient',
      targetRole: 'app_patient',
      contextClass: 'patient',
      purpose: 'booking.self.allowed',
      functionIdentity: 'app.is_current_patient_self_booking_allowed()',
    },
  },
  functions: {
    ...
    'app.password_login_issue_altcha_challenge_impl(text,uuid,text,timestamp with time zone)': {
      ...
      invocation: 'internal' as const,
      purpose:
        'private implementation behind exact-gated app.password_login_issue_altcha_challenge',
    },
    ...
    'app.assert_organization_slug_alias_complete()': rev10Function({
      ...
      returns: 'trigger',
      execute: [],
      purpose: 'deferred organization slug alias completeness constraint trigger',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
    ...
    'app.assert_organization_slug_rename_complete()': rev10Function({
      ...
      returns: 'trigger',
      execute: [],
      purpose: 'deferred organization slug rename completeness constraint trigger',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog'],
    ...
    'app.enforce_lfk_child_owner()': rev10Function({
      ...
      returns: 'trigger',
      execute: [],
      purpose: 'LFK child ownership integrity trigger',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog'],
    ...
    'app.guard_clinic_directory_current_slug()': rev10Function({
      ...
      returns: 'trigger',
      execute: [],
      purpose: 'clinic directory current-slug integrity trigger',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog'],
    ...
    'app.guard_org_brand_revision()': rev10Function({
      ...
      returns: 'trigger',
      execute: [],
      purpose: 'organization brand revision monotonicity trigger',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog'],
    ...
    'app.guard_organization_slug_claim_mutation()': rev10Function({
      ...
      returns: 'trigger',
      execute: [],
      purpose: 'organization slug claim mutation guard trigger',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog'],
    ...
    'app.guard_organization_slug_rename_event_mutation()': rev10Function({
      ...
      returns: 'trigger',
      execute: [],
      purpose: 'organization slug rename audit mutation guard trigger',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog'],
    ...
    'app.reject_staff_commercial_organization_update()': rev10Function({
      ...
      returns: 'trigger',
      execute: [],
      purpose: 'staff commercial-organization update rejection trigger',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog'],
    ...
    'public.audit_app_runtime_settings_change()': rev10Function({
      ...
      returns: 'trigger',
      execute: [],
      purpose: 'application runtime-settings audit trigger',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: [],
    ...
    'public.media_folders_enforce_depth()': rev10Function({
      ...
      returns: 'trigger',
      execute: [],
      purpose: 'media-folder maximum-depth integrity trigger',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: [],
    ...
    'public.media_folders_prevent_cycle()': rev10Function({
      ...
      returns: 'trigger',
      execute: [],
      purpose: 'media-folder cycle prevention trigger',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: [],
    ...
    'public.sync_registered_app_runtime_setting()': rev10Function({
      ...
      returns: 'trigger',
      execute: [],
      purpose: 'registered runtime-setting projection trigger',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: [],
    ...
    'public.system_settings_test_lock_guard()': rev10Function({
      ...
      returns: 'trigger',
      execute: [],
      purpose: 'system-setting protected-key lock trigger',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: [],
    ...
    'app.passkey_issue_challenge(uuid,text,uuid,text,text,text,timestamp with time zone)':
      rev10Function({
        ...
        execute: ['app_pre_session', 'app_patient'],
    ...
    'app.passkey_read_challenge(uuid,text)': rev10Function({
      ...
      owner: 'app_seam_passkey_owner',
      execute: ['app_pre_session', 'app_patient'],
      purpose: 'exact authentication or patient-registration challenge read',
      typedArgs: ['uuid', 'text'],
      volatility: 'STABLE',
      parallel: 'RESTRICTED',
      proconfig: ['search_path=pg_catalog, app, public, pg_temp'],
    ...
    'app.passkey_complete_authentication(uuid,text,bigint,bigint,text,boolean)': rev10Function({
      ...
      execute: ['app_pre_session'],
      purpose: 'auth.passkey.authentication.complete',
      typedArgs: ['uuid', 'text', 'bigint', 'bigint', 'text', 'boolean'],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog, app, public, pg_temp'],
    }),
    ...
    'app.read_integrator_migration_ledger()': rev10Function({
      ...
      returns: 'record',
      execute: ['app_service'],
      purpose: 'read the exact integrator startup migration ledger without relation ACL',
      typedArgs: [],
      volatility: 'STABLE',
      parallel: 'RESTRICTED',
      ...
      relationSurfaces: [
        {
          relation: 'integrator.schema_migrations',
    ...
    'app.read_integrator_projection_health(integer)': rev10Function({
      ...
      relationSurfaces: [
        {
          relation: 'integrator.projection_outbox',
          columns: ['status', 'next_try_at', 'attempts_done', 'updated_at'],
          operations: ['SELECT' as const],
          evidence: 'pg16-function-body-lexical-upper-bound' as const,
        },
      ],
    ...
    'app.read_integrator_provider_runtime_setting(text)': rev10Function({
      ...
      execute: ['app_service'],
      purpose: 'return one fixed-allowlist provider setting to the integrator service',
    }),
    'app.read_integrator_auth_channel_setting(text)': rev10Function({
      ...
      execute: ['app_service'],
      purpose: 'return one fixed-allowlist auth-channel flag to the integrator service',
    }),
    'app.read_integrator_smtp_outbound_setting()': rev10Function({
      ...BUSINESS_SEAM_FUNCTIONS['app.read_integrator_smtp_outbound_setting()'],
      execute: ['app_service'],
      purpose: 'return only the global SMTP envelope to the integrator service',
    }),
    'app.try_acquire_integrator_idempotency(text,integer)': rev10Function({
      ...
      returns: 'boolean',
      execute: ['app_service'],
      purpose: 'atomically acquire exactly one attested integrator idempotency key',
      typedArgs: ['text', 'integer'],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog, app, integrator, pg_temp'],
      relationSurfaces: [
        {
          relation: 'integrator.idempotency_keys',
    ...
    'app.release_integrator_idempotency(text)': rev10Function({
      ...
      returns: 'void',
      execute: ['app_service'],
      purpose: 'release exactly one attested integrator idempotency key',
      typedArgs: ['text'],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog, app, integrator, pg_temp'],
    ...
    'app.upsert_integration_data_quality_incident(text,text,text,text,text,text,text)':
      rev10Function({
        ...
        relationSurfaces: [
          {
            relation: 'integrator.integration_data_quality_incidents',
            columns: [
              'integration',
              'entity',
              'external_id',
    ...
    'app.read_patient_telegram_display_handle(uuid)': rev10Function({
      ...
      execute: ['app_staff'],
      purpose:
        'return one Telegram display handle only for an active patient of the current organization',
      typedArgs: ['uuid'],
      volatility: 'STABLE',
      parallel: 'RESTRICTED',
      proconfig: ['search_path=pg_catalog, app, public, pg_temp'],
      relationSurfaces: [
        ...
        },
        {
          ...
          columns: ['user_id', 'channel_code', 'display_handle'],
          operations: ['SELECT' as const],
          evidence: 'pg16-function-body-lexical-upper-bound' as const,
        },
      ],
    ...
    'app.list_active_canonical_appointments_by_phone(text)': rev10Function({
      ...
      execute: ['app_worker'],
      purpose:
        'return only active canonical appointments for an HMAC-authenticated integrator phone lookup',
      typedArgs: ['text'],
      volatility: 'STABLE',
      parallel: 'RESTRICTED',
      proconfig: ['search_path=pg_catalog, app, public, pg_temp'],
    ...
    'app.count_active_canonical_appointments()': rev10Function({
      ...
      relationSurfaces: [
        {
          relation: 'public.be_appointments',
          columns: ['status', 'deleted_at', 'start_at'],
          operations: ['SELECT' as const],
          evidence: 'pg16-function-body-lexical-upper-bound' as const,
        },
      ],
    ...
    'app.get_google_calendar_event_id(uuid)': rev10Function({
      ...
      purpose: 'read one Google event id after proving appointment organization',
      typedArgs: ['uuid'],
      volatility: 'STABLE',
      parallel: 'RESTRICTED',
      proconfig: ['search_path=pg_catalog, app, public, pg_temp'],
      relationSurfaces: [
        ...
        {
          relation: 'public.booking_calendar_map',
          columns: ['appointment_key', 'gcal_event_id'],
          operations: ['SELECT' as const],
          evidence: 'pg16-function-body-lexical-upper-bound' as const,
        },
      ],
    ...
    'app.upsert_google_calendar_event_id(uuid,text)': rev10Function({
      ...
      execute: ['app_tenant_service'],
      purpose:
        'atomically upsert calendar mapping and patient-booking mirror after organization proof',
      typedArgs: ['uuid', 'text'],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog, app, public, pg_temp'],
      relationSurfaces: [
        ...
        {
          relation: 'public.booking_calendar_map',
          columns: ['appointment_key', 'gcal_event_id', 'updated_at'],
          operations: ['SELECT' as const, 'INSERT' as const, 'UPDATE' as const],
          evidence: 'pg16-function-body-lexical-upper-bound' as const,
        },
        {
          relation: 'public.patient_bookings',
          columns: ['canonical_appointment_id', 'gcal_event_id', 'updated_at'],
    ...
    'app.delete_google_calendar_event_id(uuid)': rev10Function({
      ...
      execute: ['app_tenant_service'],
      purpose:
        'atomically delete calendar mapping and clear patient-booking mirror after organization proof',
      typedArgs: ['uuid'],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog, app, public, pg_temp'],
    ...
    'app.read_booking_calendar_patient_profile(uuid)': rev10Function({
      ...
      execute: ['app_tenant_service'],
      purpose:
        'calendar enrichment returns only the problem flag and note for the appointment patient',
      typedArgs: ['uuid'],
      volatility: 'STABLE',
      parallel: 'RESTRICTED',
      proconfig: ['search_path=pg_catalog, app, public, pg_temp'],
      relationSurfaces: [
        {
          relation: 'public.be_appointments',
          columns: ['id', 'organization_id', 'platform_user_id'],
          operations: ['SELECT' as const],
          evidence: 'pg16-function-body-lexical-upper-bound' as const,
        },
        {
          relation: 'public.be_patient_booking_profiles',
          columns: ['organization_id', 'platform_user_id', 'is_problematic', 'problematic_note'],
          operations: ['SELECT' as const],
          evidence: 'pg16-function-body-lexical-upper-bound' as const,
        },
      ],
    ...
    'app.read_booking_calendar_latest_staff_comment(uuid)': rev10Function({
      ...
      returns: 'text',
      execute: ['app_tenant_service'],
      purpose: 'calendar enrichment returns only the latest staff comment body for one appointment',
      typedArgs: ['uuid'],
      volatility: 'STABLE',
      parallel: 'RESTRICTED',
      proconfig: ['search_path=pg_catalog, app, public, pg_temp'],
      relationSurfaces: [
        ...
        {
          relation: 'public.be_appointment_staff_comments',
          columns: ['organization_id', 'appointment_id', 'body', 'created_at'],
          operations: ['SELECT' as const],
          evidence: 'pg16-function-body-lexical-upper-bound' as const,
        },
      ],
    ...
    'app.is_current_patient_self_booking_allowed()': rev10Function({
      ...
      execute: ['app_patient'],
      purpose:
        'return only whether the current patient may self-book, never sensitive profile fields',
      typedArgs: [],
      volatility: 'STABLE',
      parallel: 'RESTRICTED',
      proconfig: ['search_path=pg_catalog, app, public, pg_temp'],
      relationSurfaces: [
        {
          relation: 'public.be_patient_booking_profiles',
          columns: ['organization_id', 'platform_user_id', 'booking_blocked'],
          operations: ['SELECT' as const],
          evidence: 'pg16-function-body-lexical-upper-bound' as const,
        },
      ],
    }),
    'app.install_port_context(uuid,app.port_context_claims)': rev10Function({
      owner: 'app_seam_context_owner',
      security: 'DEFINER',
      returns: 'void',
      loginExecute: true as const,
      execute: [],
      purpose: 'install',
    ...
    'app.clear_port_context()': rev10Function({
      owner: 'app_seam_context_owner',
      security: 'DEFINER',
      returns: 'void',
      loginExecute: true as const,
    ...
    'app.require_accepted_context(name,name,app.port_context_class,text,bytea,regprocedure)':
      rev10Function({
        ...
        security: 'DEFINER',
        returns: 'boolean',
        execute: [...REV10_RUNTIME, ...REV10_SEAM_OWNERS],
        purpose: 'gate',
        typedArgs: ['name', 'name', 'class', 'text', 'bytea', 'regprocedure'],
        volatility: 'VOLATILE',
        parallel: 'UNSAFE',
        proconfig: ['search_path=pg_catalog, app, app_ext, pg_temp'],
      }),
    'app.require_platform_principal()': rev10Function({
      owner: 'app_seam_context_owner',
      security: 'DEFINER',
      returns: 'boolean',
      execute: ['app_platform_settings', 'saas_telemetry_operator', ...REV10_SEAM_OWNERS],
      purpose: 'platform',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
    ...
    'app.current_org_id()': rev10Function({
      owner: 'app_seam_context_owner',
      security: 'DEFINER',
      returns: 'uuid',
      execute: [...REV10_RUNTIME, ...REV10_SEAM_OWNERS],
    ...
    'app.current_actor_user_id()': rev10Function({
      owner: 'app_seam_context_owner',
      security: 'DEFINER',
      returns: 'uuid',
      execute: [...REV10_RUNTIME, ...REV10_SEAM_OWNERS],
      purpose: 'current-actor',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog, app, app_ext, pg_temp'],
    }),
    'app.current_patient_user_id()': rev10Function({
      owner: 'app_seam_context_owner',
      security: 'DEFINER',
      returns: 'uuid',
      execute: [...REV10_RUNTIME, ...REV10_SEAM_OWNERS],
      purpose: 'current-patient',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog, app, app_ext, pg_temp'],
    }),
    'app.current_integrator_user_id()': rev10Function({
      owner: 'app_seam_context_owner',
      security: 'DEFINER',
      returns: 'bigint',
      execute: [...REV10_RUNTIME, ...REV10_SEAM_OWNERS],
      purpose: 'current-integrator',
      typedArgs: [],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog, app, app_ext, pg_temp'],
    }),
    'app.hash_port_typed_args(app.port_typed_arg[])': rev10Function({
      owner: 'app_seam_context_owner',
      security: 'INVOKER',
      returns: 'bytea',
      execute: ['app_seam_context_owner', ...REV10_SEAM_OWNERS],
      purpose: 'typed-args',
      typedArgs: ['app.port_typed_arg[]'],
      volatility: 'IMMUTABLE',
      parallel: 'SAFE',
      proconfig: ['search_path=pg_catalog'],
    }),
    'app.is_staff()': rev10Function({
      owner: 'app_object_owner',
      security: 'INVOKER',
      returns: 'boolean',
      execute: [...REV10_RUNTIME],
      purpose: 'staff-class',
      typedArgs: [],
      volatility: 'STABLE',
      parallel: 'SAFE',
    ...
    'app_ext.resolve_variant_a_identity(uuid)': rev10Function({
      owner: 'app_seam_identity_lookup_owner',
      security: 'DEFINER',
      returns: 'uuid',
      execute: [],
      purpose: 'private variant-a map mutation behind the exact pre-session root',
      typedArgs: ['uuid'],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog, app, app_ext, pg_temp'],
      relationSurfaces: [
        {
          relation: 'app_ext.variant_a_identity_refs',
          columns: ['physical_user_id', 'opaque_ref'],
          operations: ['SELECT' as const, 'INSERT' as const, 'UPDATE' as const],
          evidence: 'pg16-function-body-lexical-upper-bound' as const,
        },
      ],
    ...
    'app_ext.resolve_variant_a_physical(uuid)': rev10Function({
      ...
      purpose: 'resolve an opaque Variant-A context reference only for the context installer',
      typedArgs: ['uuid'],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog, app, app_ext, pg_temp'],
    ...
    'app.pre_session_resolve_identity(uuid)': rev10Function({
      ...
      security: 'DEFINER',
      returns: 'uuid',
      execute: ['app_pre_session', 'app_platform_admin'],
      purpose: 'exact physical-to-opaque handoff before a human transaction',
      typedArgs: ['uuid'],
      volatility: 'VOLATILE',
      parallel: 'UNSAFE',
      proconfig: ['search_path=pg_catalog, app, app_ext, pg_temp'],
...
const REV10_LOCKED_POLICIES = new Map<string, LockedPolicyTarget>(
  (getPhase4LockedPolicyTargets() as LockedPolicyTarget[]).map((target: LockedPolicyTarget) => [
...
const REV10_SYSTEM_DIRECT_ACCESS: Record<string, DirectAccessSeed> = {
  'public.operator_health_failure_archive': {
    kind: 'direct',
    purpose:
      'clinic staff handles only its archive rows; platform health route handles only global archive rows',
    codePaths: [
  ...
  'integrator.projection_outbox': {
    kind: 'direct',
    purpose: 'enqueue projections in request transactions and drain them in the dedicated worker',
    codePaths: [
      'apps/integrator/src/infra/db/repos/projectionOutbox.ts',
  ...
  'public.app_runtime_settings_audit': {
    kind: 'direct',
    purpose: 'platform operators inspect the immutable runtime-setting change ledger',
  ...
  'public.integration_webhook_error_events': {
    kind: 'direct',
    purpose: 'the health worker records and prunes inbound webhook failures',
    codePaths: [
      'apps/webapp/src/app-layer/health/runIntegratorPushOutboxHealthGuardTick.ts',
      'packages/operator-db-schema/src/integrationWebhook.ts',
    ],
    grants: [{ role: 'app_worker', operations: ['SELECT', 'INSERT', 'DELETE'], columns: 'table' }],
  },
  'public.saas_billing_periods': {
    kind: 'direct',
    purpose: 'staff reads the billing-period catalog; platform operations alone maintain it',
    codePaths: [
  ...
  'public.system_settings_audit': {
    kind: 'direct',
    purpose:
      'only the platform settings role may read or append the global secret-bearing settings ledger',
    codePaths: ['apps/webapp/src/infra/repos/pgSystemSettings.ts'],
    grants: [{ role: 'app_platform_settings', operations: ['SELECT', 'INSERT'], columns: 'table' }],
  },
};

const REV10_NO_RUNTIME_ACCESS: Record<
  string,
  Extract<RelationAccess, { kind: 'no-runtime-surface' }>
> = {
  'public.phone_messenger_bind_secrets': {
    kind: 'no-runtime-surface',
    purpose:
      'post-zero direct bearer-secret access is intentionally disabled until its replacement is a single exact pre-session named root; no completion role is permitted',
    evidence: [
  ...
  'app.context_nonce_ledger': {
    kind: 'no-runtime-surface',
    purpose:
      'obsolete custom signed-context nonce ledger replaced by app_ext accepted transaction contexts',
    evidence: [
  ...
  'app.principal_context': {
    kind: 'no-runtime-surface',
    purpose:
      'obsolete session-row context replaced by transaction-bound app_ext.accepted_port_contexts',
    evidence: [
  ...
  'drizzle.__drizzle_migrations': {
    kind: 'no-runtime-surface',
    purpose: 'Drizzle-owned migration ledger is used only by the migration process',
    evidence: [
  ...
  'public.webapp_schema_migrations': {
    kind: 'no-runtime-surface',
    purpose: 'legacy webapp migration ledger is not a runtime product surface',
    evidence: [
...
function revision10RelationSeams(tableKey: string, dbName: string): NamedSeamAccess[] {
  ...
  for (const [regprocedure, fn] of Object.entries(REV10_CONTEXT.functions) as Array<
  ...
  >) {
    ...
    seams.push({
      regprocedure,
      owner: fn.owner,
      callers: invocation === 'runtime' ? [...fn.execute] : [],
      invocation,
...
function revision10RelationAccess(tableKey: string, dbName: string): RelationAccess {
  ...
  if (seams.length > 0)
    return {
      kind: 'named-seams',
      seams,
      purpose: `exact declared function surfaces for ${tableKey}`,
    };
  return {
    kind: 'unresolved',
    reason: 'no exact direct, seam, or exhaustive no-runtime proof',
    codePaths: [],
  };
}
...
function revision10TableGrants(access: RelationAccess): Record<string, GrantDecl> {
  ...
  const add = (
    ...
    for (const operation of operations) {
      const entry =
        columns === 'table' || operation === 'DELETE'
          ? operation
          : { kind: 'columns' as const, priv: operation, columns: [...columns].sort() };
      const serialized = JSON.stringify(entry);
      if (!row.privs.some((candidate) => JSON.stringify(candidate) === serialized))
        row.privs.push(entry);
    }
    byRole.set(role, row);
  };
  if (access.kind === 'direct') {
    for (const grant of access.grants)
      add(grant.role, grant.operations, grant.columns, access.purpose);
  }
...
function revision10ContextGates(
  ...
  if (
    ordinaryDirectRoles.length > 0 ||
    access.kind === 'no-runtime-surface' ||
    access.kind === 'unresolved'
  )
    policies.push({
      ...
      using: ordinaryPredicate,
      withCheck: ordinaryPredicate,
  ...
  if (seamOwners.length > 0)
    policies.push({
      name: `rev10_named_root_owner_gate_${index + 1}`,
      as: 'RESTRICTIVE',
      cmd: 'ALL',
      to: seamOwners,
...
const REV10_TENANT_MEMBERSHIP_BASE: Record<string, readonly TenantMembershipReference[]> = {
  ...
  'public.user_channel_preferences': [
    ...
    { column: 'platform_user_id', type: 'uuid' },
  ],
...
const REV10_TENANT_MEMBERSHIP_WRITE: Record<
  string,
  Partial<Record<'INSERT' | 'UPDATE', readonly TenantMembershipReference[]>>
> = {
  'public.be_appointment_staff_comments': {
    UPDATE: [{ column: 'platform_user_id', type: 'uuid' }],
  },
  'public.be_appointments': { UPDATE: [{ column: 'platform_user_id', type: 'uuid' }] },
  'public.be_patient_booking_profiles': {
    INSERT: [
      { column: 'platform_user_id', type: 'uuid' },
      { column: 'updated_by', type: 'uuid' },
    ],
  ...
  'public.reminder_rules': {
    INSERT: [{ column: 'platform_user_id', type: 'uuid' }],
    UPDATE: [{ column: 'platform_user_id', type: 'uuid' }],
  },
...
function revision10TenantMemberPredicate(tableKey: string, ref: TenantMembershipReference): string {
  ...
  return (
    `(EXISTS (SELECT 1 FROM public.be_organization_members tenant_staff` +
...
function revision10TenantMembershipPredicate(
  tableKey: string,
  refs: readonly TenantMembershipReference[],
  requireReference: boolean,
): string {
  const nonNull = refs
    .map((ref) => `${revision10TenantOuterColumn(tableKey, ref.column)} IS NOT NULL`)
    .join(' OR ');
  const bounded = refs
    .map((ref) => {
      const outerColumn = revision10TenantOuterColumn(tableKey, ref.column);
...
function revision10TenantBasePredicate(tableKey: string): string {
  ...
  if (tableKey === 'public.be_patient_package_items') {
    return (
      `(EXISTS (SELECT 1 FROM public.be_patient_packages tenant_package` +
...
function revision10TenantParentWritePredicate(
  tableKey: string,
  ...
  if (tableKey === 'public.support_conversation_messages') {
    return (
      `EXISTS (SELECT 1 FROM public.support_conversations tenant_conversation` +
  ...
  if (tableKey === 'public.support_delivery_events' && operation === 'INSERT') {
    return nullableParent(
      ...
      `EXISTS (SELECT 1 FROM public.support_conversation_messages tenant_message` +
  ...
  if (tableKey === 'public.support_question_messages' && operation === 'INSERT') {
    return (
      `EXISTS (SELECT 1 FROM public.support_questions tenant_question` +
  ...
  if (tableKey === 'public.treatment_program_events' && operation === 'INSERT') {
    return (
      `EXISTS (SELECT 1 FROM public.treatment_program_instances tenant_instance` +
  ...
  if (tableKey === 'public.symptom_entries' && operation === 'UPDATE') {
    return (
      `EXISTS (SELECT 1 FROM public.symptom_trackings tenant_tracking` +
...
function revision10DirectBusinessPredicate(
  ...
  const rolePredicate =
    ordinaryRoles.length > 0
      ? ordinaryRoles.map((role) => `current_user = '${role}'::name`).join(' OR ')
      : 'false';
  if (tableKey === 'public.clinical_test_regions')
    return (
      `(current_user = 'app_staff'::name AND organization_id = app.current_org_id()` +
  ...
  if (tableKey === 'public.be_appointment_staff_comments')
    return (
      `(current_user = 'app_staff'::name AND organization_id = app.current_org_id()` +
  ...
  if (tableKey === 'public.be_patient_booking_profiles')
    return "(current_user = 'app_staff'::name AND organization_id = app.current_org_id())";
  if (tableKey === 'public.be_organizations')
    return "(current_user = 'app_staff'::name AND id = app.current_org_id())";
  if (tableKey === 'public.operator_health_failure_archive')
    return "((current_user = 'app_staff'::name AND organization_id = app.current_org_id()) OR (current_user = 'app_platform_settings'::name AND organization_id IS NULL))";
  if (tableKey === 'public.system_settings_audit')
    return "(current_user = 'app_platform_settings'::name AND organization_id IS NULL)";
  const platformUserColumn = REV10_PLATFORM_USER_COLUMN[tableKey];
  if (platformUserColumn)
    return (
      `((${rolePredicate}) AND EXISTS (SELECT 1 FROM public.be_organization_members access_member` +
  ...
  if (REV10_EXPLICIT_ORG_COLUMN.has(tableKey))
    return `((${rolePredicate}) AND organization_id = app.current_org_id())`;
  return `(${rolePredicate})`;
}

function revision10SeamOwnerPolicy(
  tableKey: string,
  index: number,
  access: RelationAccess,
): PolicyDecl[] {
  ...
  return [
    {
      name: `rev10_seam_business_${index + 1}`,
      as: 'PERMISSIVE',
      cmd: 'ALL',
      to: owners,
      using: `(${predicate})`,
      withCheck: `(${predicate})`,
      note: `only declared narrow owners may reach ${tableKey}`,
    },
  ];
}
...
function revision10Database(name: 'bersoncarebot_test' | 'bcb_webapp_dev'): DatabaseDecl {
  ...
  const tables = Object.fromEntries(
    Object.entries(legacy.tables).map(([key, table], index) => {
      ...
      const explicitPolicies = (table.policies ?? []).filter(
        (policy): policy is PolicyDecl =>
          !('todo' in policy) && policy.to.every((role) => known.has(role) || role === 'PUBLIC'),
      );
      ...
      const classSafe = (predicate: string) =>
        predicate
          ...
          .replaceAll(
            'app.current_patient_user_id() IS NOT NULL AND "platform_user_id" = app.current_patient_user_id()',
            'app.current_patient_user_id() IS NOT NULL AND "organization_id" = app.current_org_id() AND "platform_user_id" = app.current_patient_user_id()',
          )
          .replaceAll(
            '"b4f_appt"."platform_user_id" = app.current_patient_user_id()',
      ...
      const directRoles =
        access?.kind === 'direct'
          ? [...new Set(access.grants.map((grant) => grant.role))].sort()
          : [];
      const ordinaryDirectRoles = directRoles.filter(
        (role) => !['app_tenant_service'].includes(role),
      );
      const directBusiness: PolicyDecl[] =
        access?.kind === 'direct' && ordinaryDirectRoles.length > 0
          ? [
              {
                name: `rev10_direct_business_${index + 1}`,
                as: 'PERMISSIVE',
                cmd: 'ALL',
                to: ordinaryDirectRoles,
                using: revision10DirectBusinessPredicate(key, access),
                withCheck: revision10DirectBusinessPredicate(key, access),
      ...
      const runtimeBusinessBase: PolicyDecl[] = !active
        ? []
        : access?.kind === 'direct' && specialized
          ? directBusiness
                ...
                : [
                    {
                      ...
                      using: 'false',
                      withCheck: 'false',
                      note: `no direct runtime relation surface for ${key}`,
                    },
                  ];
      const tenantBusiness =
        access?.kind === 'direct' && directRoles.includes('app_tenant_service')
    ...
    [
      ...new Set(
        Object.entries(tables)
          ...
          .flatMap(([, table]) => Object.keys(table.grants ?? {})),
      ),
    ].sort();
  return {
    ...legacy,
    ...
    schemas: {
      ...
      app_ext: {
        owner: 'app_object_owner',
        present: true,
        usage: ['app_seam_context_owner', 'app_seam_identity_lookup_owner'],
        create: ['app_object_owner'],
      },
      public: {
        owner: 'app_object_owner',
        present: true,
        usage: schemaUsage('public'),
        create: ['app_object_owner'],
      },
      integrator: {
        owner: 'app_object_owner',
        present: true,
        usage: schemaUsage('integrator'),
        create: ['app_object_owner'],
      },
      drizzle: {
        owner: 'app_object_owner',
    ...
    functionsViews: {
      default: 'all views are SECURITY INVOKER; no undeclared view ACL',
      views: {},
    },
    definerExceptions: {
      defaults: {
        ...
        securityDefiner: true,
        owner: 'app_object_owner',
        searchPath: ['search_path=pg_catalog, app, app_ext, pg_temp'],
        publicExecute: false,
        coveredCount: 0,
    ...
    orgTableAllowlist: {
      derivedFrom: 'tables[*].org === true',
      named: Object.keys(tables)
        .filter((key) => tables[key].org === true)
        .sort(),
      fullCountLive: Object.keys(tables).filter((key) => tables[key].org === true).length,
      todo: '',
    },
...
export const DECLARATION_STATS = {
  ...
  revokePairs: allTables.reduce(
    (n: number, t: TableDecl) => n + Object.keys(t.revoke ?? {}).length,
    0,
  ),
