...
function buildArtifacts(declaration, dbName, withAllowlist, source, portContextOnly = false, zeroState = false) {
  if (zeroState) {
    return [{ kind: 'zeroState', text: generateZeroStateSql(declaration, dbName, { source }) }];
  }
  if (portContextOnly) {
...
async function main() {
  ...
  if (zeroStateCluster) {
    ...
    const file = args.values.has('out')
      ? path.resolve(args.values.get('out'))
      : path.join(outDir, 'zero-state.cluster.sql');
    if (args.flags.has('stdout') || args.values.get('out') === '-') {
      process.stdout.write(text);
      return;
    }
    if (args.flags.has('check')) {
      if (!fs.existsSync(file)) {
        console.error(`КРАСНЫЙ cluster/zeroState: артефакт ${path.relative(repoRoot, file)} не закоммичен`);
        process.exit(1);
      }
      const committed = fs.readFileSync(file, 'utf8');
      if (committed !== text) {
        console.error(`КРАСНЫЙ cluster/zeroState: ${path.relative(repoRoot, file)} разошёлся с декларацией`);
        console.error(firstDifference(committed, text));
        process.exit(1);
      }
