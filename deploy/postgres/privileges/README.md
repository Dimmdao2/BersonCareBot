...
Кластерные примитивы `--shared-role-baseline` и `--shared-role-verify` принимают необязательный `--db`: он
называет не объекты базы, а ЦЕЛЕВОЙ КЛАСТЕР. `reconcile-access.mjs` всегда передаёт его; без него
раскладываются/сверяются все объявленные роли, как раньше.
