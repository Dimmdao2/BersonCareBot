...
/** `count` РАЗНЫХ живых клиник DEV, у каждой — активный сотрудник со ссылкой актора. */
export async function clinicsWithDoctors(client, count) {
  const rows = await client.query(
    `WITH ranked AS (
  ...
  if (rows.rows.length !== count) {
    throw new Error(`DEV has ${rows.rows.length} clinics with an active member, need ${count}`);
  }
  return rows.rows;
}
