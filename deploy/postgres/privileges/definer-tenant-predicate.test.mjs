...
/*
 * Перепись помеченных пар здесь СНЯТА (владелец, 15.09). Она записывала 57 имён «арендный
 * DEFINER-корень → стенованное отношение» и краснела на любое движение обычной работы — в том числе
 * на УЛУЧШЕНИЕ: Н-5 убрала чтение `be_specialists` из снимка создания записи вместе с пресетами, и
 * набор покраснел на то, что поверхность стала уже. Чинится такое ретипированием списка, то есть
 * проверка заставляла переписывать себя, а не находила дефект. Сам факт пометки живёт в
 * `declaration.ts` и виден в его диффе — вторая копия того же списка нарушала и правило одного
 * источника, записанное в шапке `name-census.mjs`.
 *
 * Свойство, ради которого тест существует, осталось: пометка, которой больше нечего объяснять,
 * краснеет ниже, а снятый из тела предикат ловят инъекции в конце файла. Мелкие реестры исключений
 * (`principalAccessorClosure`, `tenantRuntimeRoles`, `relationWideCapabilities` и прочие) не
 * трогались: там любое движение — событие безопасности, а не побочный эффект обычной работы.
 */
test('пометка crossesTenantWall не живёт там, где предикат на месте', () => {
  const { subjects } = analysis();
  const stale = [];
  for (const { signature, fn, unbound } of subjects) {
