...
zero() {
  ...
  "$pg_bin/psql" -X -v ON_ERROR_STOP=1 -h "$data_dir" -p "$port" -U postgres -d postgres -1 \
    -f "$repo_root/deploy/postgres/generated/zero-state.cluster.sql" >/dev/null
}
install() {
...
assert_eq "$(admin -Atc "SELECT count(*) FROM pg_roles WHERE rolcanlogin AND rolname ~ '^(app_|bcb_|saas_|bersoncarebot_)'")" 3
assert_eq "$(admin -Atc "SELECT count(*) FROM pg_roles WHERE rolname IN ('bcb_test_webapp_staff','bcb_test_webapp_patient','bcb_test_integrator') AND rolcanlogin")" 3
assert_eq "$(admin -Atc "SELECT count(*) FROM pg_auth_members WHERE member IN ('bcb_test_webapp_staff'::regrole,'bcb_test_webapp_patient'::regrole,'bcb_test_integrator'::regrole) AND set_option")" 15
...
SQL
if BCB_TEST_WEBAPP_STAFF_PASSWORD=x BCB_TEST_WEBAPP_PATIENT_PASSWORD=x BCB_TEST_INTEGRATOR_PASSWORD=x \
...
admin -Atc "SELECT count(*) FROM pg_roles WHERE rolname IN ('bcb_test_webapp_staff','bcb_test_webapp_patient','bcb_test_integrator')" | grep -qx 0
