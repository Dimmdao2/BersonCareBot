 ...
 *      ответы двери по форме — «нет такого», «есть, но уволен», «есть в другой клинике» — и
 *      перебор отвечает, кто в клинике работает. ФИО сотрудника это персональные данные. На экране
 *      при этом не меняется НИЧЕГО: человек в любом случае видит «этот специалист больше не
 *      принимает записи». Требование владельца: план §3.3 («одинаковый отказ, иначе анонимный
 *      запрос перечисляет»), §17.G («неактивный или удалённый специалист даёт тот же отказ»).
 *
 *      Случай «клиника его не публикует» из этого перечня ВЫВЕДЕН решением владельца 11.09
...
/**
 * #926 §17.Q, находка 1 слепого аудита 2c и живой дефект: конструктор ссылки в кабинете отбирает
 * специалистов по `is_active`, а дверь сужала по `is_active AND card_is_published`, у которого
 * `DEFAULT false`. По СВЕЖЕЙ ссылке посетитель поэтому читал «этот специалист больше не принимает
 * записи в этой клинике» — неправду о человеке, который записи принимает, — и клиника об этом не
 * узнавала никогда, потому что в кабинете у неё всё цело. Верни предикат обратно, и утверждение
 * ниже покраснеет; снаружи не изменится ни один пиксель.
 */
test('свежий специалист без публикации карточки записи ПРИНИМАЕТ', { skip: !ENABLED }, () => {
  const hidden = parse('unpublished');
  assert.equal(
    hidden.specialist?.id,
    SPECIALIST.unpublished,
    'дверь отказала специалисту, у которого выключена визитка, — ссылка врёт про живого человека',
  );
  assert.deepEqual(
    titles(hidden),
    [SERVICE_OF_UNPUBLISHED],
    'каталог по ссылке на этого специалиста не сузился до его услуг',
  );
  // Галка организации по умолчанию включена, но его собственная визитка выключена — значит читать
  // про него в модуле записи нечего, и дверь обязана сказать это ОДНИМ готовым ответом (§17.Q).
  assert.equal(
    hidden.specialist?.cardIsReadable,
    false,
    'дверь предложила открыть карточку, которой клиника не публиковала, — это ссылка в 404',
  );
  assert.equal(
    parse('published').specialist?.cardIsReadable,
    true,
    'у опубликованного специалиста карточка перестала открываться из модуля записи',
  );
});

test('неактивный, чужой и несуществующий специалист неразличимы по ответу', { skip: !ENABLED }, () => {
  const refusals = ['inactive', 'foreign', 'missing'].map((label) => readDoor().get(label));

  // Против «доказано пустотой» здесь работает самотест шва выше (`3/3`): у каждого отказного
  // специалиста в фикстуре есть строка и ЖИВАЯ активная услуга в том же филиале, значит отказ
  // даёт предикат двери, а не отсутствие данных.

  assert.equal(
    new Set(refusals).size,
    1,
    'ответы трёх отказов различаются — по форме ответа аноним перечисляет людей клиники',
  );
