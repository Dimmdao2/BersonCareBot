...
test('Ш3: обратный резолвер ПРИНИМАЕТ вид и ещё НЕ сравнивает его',
  { skip: !ENABLED }, () => {
    // Ожидание временное и названо шагом: Ш5 переворачивает его в `42501` на несовпадении вида.
    // До Ш5 сравнивать нечего — приложение кладёт одну и ту же ссылку и в `actor_ref`, и в
    // `subject_ref` (`portContextRuntime.ts`), и проверка отвергала бы каждого живого пациента.
    const parts = probe(`
  ...
  END;`);
    assert.equal(parts.length, 2, `обратный резолвер отказал раньше Ш5: ${parts.join('|')}`);
    assert.equal(parts[0], parts[1],
      'обратный резолвер начал проверять вид раньше названного шага Ш5');
  });
