...
-- This overlay is applied after E1 runtime migrations and re-asserts the final closed
-- service/operation vocabulary on the table. It does NOT own any function body: see the note below
-- app.report_saas_isolation_event.
...
-- The BODY of app.report_saas_isolation_event is owned by the drizzle migration ledger
-- (apps/webapp/db/drizzle-migrations/20260828T092521_deliver_cron_isolation_operations.sql), NOT by this
-- overlay. Both files used to carry a full CREATE OR REPLACE of it and agreed only by accident: this
-- overlay is applied after every migrate, so a stale copy here silently reverts a migrated fix, and
-- nothing compared the two. Same rule as app.open_or_touch_operator_incident in
-- deploy/postgres/c4-operational-runtime.sql, and it is enforced mechanically by
-- scripts/check-c4-migration-owned-function-bodies.mjs. This overlay owns only the ownership and the
-- exact EXECUTE grants below.
