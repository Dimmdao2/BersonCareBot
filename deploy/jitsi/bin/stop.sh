...
# compose files + the env file) — not bare `docker compose -p <project> down`, which has no compose
...
cd "$HERE"
# No host gate here on purpose: this is the ExecStop path, and refusing to stop a stack is not a safe
# failure mode. The profile library is still sourced, because which env file and which compose project
# "stop" means is profile-specific.
...
if [[ ! -f "$ENV_FILE" ]]; then
  ...
  echo "$JITSI_LOG_TAG $ENV_FILE not present, nothing to stop"
  exit 0
fi
# shellcheck disable=SC1090
set -a; source "$ENV_FILE"; set +a
TURN_ENV_FILE="$JITSI_TURN_ENV_FILE"
[[ -f "$TURN_ENV_FILE" ]] || { echo "$JITSI_LOG_TAG FATAL: missing $TURN_ENV_FILE" >&2; exit 1; }
# shellcheck disable=SC1090
...
if [[ -d "$VENDOR_DIR" ]]; then
  docker compose \
    ...
    -p "$JITSI_COMPOSE_PROJECT" down
else
  # Vendor dir gone but containers might still exist under the project name — remove by project label only.
  docker compose -p "$JITSI_COMPOSE_PROJECT" down 2>/dev/null || true
fi
echo "$JITSI_LOG_TAG stopped"
