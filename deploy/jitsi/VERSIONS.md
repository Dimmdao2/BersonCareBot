...
3. `deploy/jitsi/env/jitsi-test.env.example` AND `deploy/jitsi/env/jitsi-prod.env.example`
   (`JITSI_RELEASE_TAG`, `ARCHIVE_SHA256`) — both profiles share one pin, and the coturn tag likewise lives
   in both `env/coturn-test.env.example` and `env/coturn-prod.env.example`. `bin/check-latest-jitsi-tag.sh`
   fails if the two templates ever disagree, because the image digests in the shared compose override are
   pinned once for both hosts.
