...
SELECT (
  ...
  AND has_function_privilege(current_user, 'app.resolve_payment_webhook_organization(text,text,text)', 'EXECUTE')
  AND NOT has_table_privilege(current_user, 'public.be_payment_provider_events', 'SELECT')
  AND NOT has_table_privilege(current_user, 'public.be_payment_intents', 'SELECT')
