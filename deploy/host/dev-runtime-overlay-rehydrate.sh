...
usage() {
  cat <<EOF
Usage: bash deploy/host/dev-runtime-overlay-rehydrate.sh --execute
...
runtime_role="$(runtime_overlay_parse_database_identity "DEV runtime DATABASE_URL" "$TARGET_DB" "$runtime_identity")"
if [[ "$runtime_role" != "$TARGET_ROLE" ]]; then
  echo "FATAL: DEV runtime role is '$runtime_role', expected '$TARGET_ROLE'" >&2
  exit 1
fi
...
run_dev_admin_psql -d "$TARGET_DB" -qAt \
  -v expected_runtime_role="$TARGET_ROLE" <<'SQL' >/dev/null
SELECT 1 / (
  pg_get_userbyid((SELECT datdba FROM pg_database WHERE datname = current_database())) = :'expected_runtime_role'
...
SELECT 1 / (
  pg_has_role(:'expected_runtime_role', 'app_staff', 'SET')
  AND pg_has_role(:'expected_runtime_role', 'app_patient', 'SET')
)::int AS dev_runtime_can_set_classified_roles;
SQL
...
runtime_overlay_apply_post_migration_chain "$REPO_ROOT" "$TARGET_DB" "$TARGET_ROLE" 1 >/dev/null
