...
cleanup() {
  ...
  if [[ "$status" -ne 0 && "$WRITERS_STOPPED" == 1 && "$SERVICES_RELEASED" != 1 ]]; then
    ...
    # Отсюда НЕ уходит оповещение владельцу: 06.09 владелец отменил телеграм-сообщения об упавшем
    # TEST-деплое — это операционный шум идущей разработки, а не то, что требует его внимания.
    # Диагностика отказа остаётся здесь и целиком: эта строка в stderr, строка `FATAL: ` и весь ход
    # выкатки в транскрипте ($TRANSCRIPT), плюс неизменённый код возврата ниже. Не возвращать сюда
    # вызов notify-owner.sh / Нео / Telegram без прямого распоряжения владельца.
  fi
  exit "$status"
}
trap cleanup EXIT
...
# покрытие вообще, и «Здоровье системы» показывала монитор изоляции слепым (`coverage_missing`,
# все шесть служб в `missingServices`). Вернули только производитель покрытия, не всю последовательность.
# С 02.09.2026 (#1085) самой последовательности больше нет: её гейты пересоздавали снятые логины и
# ставили владельцем выведенную роль `app_owner`, а права целиком принадлежат декларации
# (`deploy/postgres/privileges/`), которую этот деплой сводит ниже через `reconcile-access.mjs`.
