...
} from "./saas-test-fixture-packet.mjs";
...
function decodeSignedSession(raw, secret) {
  ...
  if (!payload || !signature || !safeEqual(signature, sign(payload, secret))) fail("invalid_session_signature");
  let session;
  try {
...
function boundAdminSession(raw, secret, ttlSeconds, nowSec = Math.floor(Date.now() / 1000)) {
  ...
  if (session?.user?.role !== "admin" || session.adminMode !== true) fail("global_admin_mode_required");
...
function assertTestWebappListenerIdentity() {
  ...
  if (!listeners.some((listener) => listener.localAddress === "127.0.0.1:6300" && expectedPid.test(listener.process))) {
...
function writeJar({ filePath, cookie, expiresAt, uid, gid }) {
  ...
  if (existsSync(directory)) {
    ...
    if (!directoryMetadata.isDirectory() || directoryMetadata.isSymbolicLink()) fail("handoff_directory_must_be_real");
  } else {
  ...
  const metadata = lstatSync(filePath);
  if (!metadata.isFile() || metadata.isSymbolicLink() || metadata.uid !== uid || metadata.gid !== gid || (metadata.mode & 0o777) !== 0o640) {
...
function readJarExpiry(filePath) {
  const line = readFileSync(filePath, "utf8").split(/\r?\n/).find((candidate) =>
    candidate.includes(`\t${sessionCookieName}\t`),
  );
...
function resolveDevIdentity() {
  const passwd = readFileSync("/etc/passwd", "utf8").split(/\r?\n/).find((line) => line.startsWith("dev:"));
...
async function issue(ttlSeconds) {
  ...
  if (!Number.isSafeInteger(ttlSeconds) || ttlSeconds < minTtlSeconds || ttlSeconds > maxTtlSeconds) {
  ...
  writeJar({ filePath: outputPath, cookie: bounded.cookie, expiresAt: bounded.expiresAt, uid: 0, gid: dev.gid });
  process.stdout.write(`TEST visual global-admin handoff issued\npath=${outputPath}\nexpires_at=${new Date(bounded.expiresAt * 1000).toISOString()}\n`);
}
...
function status(filePath = outputPath) {
  ...
  process.stdout.write(`TEST visual global-admin handoff: ${state}\nexpires_at=${new Date(expiresAt * 1000).toISOString()}\n`);
}
...
function selfTest() {
  ...
  try {
    ...
    if (decoded.expiresAt !== now + 600 || decoded.operatorSession?.purpose !== purpose) fail("self_test_bound_failed");
    const jar = path.join(root, "global-admin.cookies");
    writeJar({ filePath: jar, cookie: bounded.cookie, expiresAt: bounded.expiresAt, uid: process.getuid?.() ?? 0, gid: process.getgid?.() ?? 0 });
    ...
    if (parseTestWebappConfigText(validEnv).secret !== envSecret) fail("self_test_env_parse_failed");
    ...
    const matchingListener = listeners.find((listener) =>
      listener.localAddress === "127.0.0.1:6300" && /(?:^|[,(])pid=1234(?:,|\))/.test(listener.process),
    );
    if (!matchingListener) fail("self_test_structured_listener_parse_failed");
    const crossRecordAccepted = listeners.some((listener) =>
      listener.localAddress === "127.0.0.1:6300" && /(?:^|[,(])pid=9999(?:,|\))/.test(listener.process),
    );
...
try {
  if (command === "issue") {
    ...
    fail("usage: test-visual-global-admin-session.mjs issue [--ttl-seconds 300..3600] | status | revoke | --self-test");
  }
} catch (error) {
  process.stderr.write(`TEST visual global-admin handoff failed: ${error instanceof Error ? error.message : "unknown"}\n`);
  process.exitCode = 1;
}
