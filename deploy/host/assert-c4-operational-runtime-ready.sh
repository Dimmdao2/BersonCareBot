...
web_push_reminder_login="$(probe "$web_push_reminder_url" "BEGIN; SET ROLE app_operational_web_push_reminder; SELECT count(*) FROM app.list_web_push_reminder_organization_ids(now()); UPDATE public.webapp_reminder_occurrences SET id=id WHERE false; ROLLBACK; RESET ROLE; SELECT session_user;" | tail -n 1)"
