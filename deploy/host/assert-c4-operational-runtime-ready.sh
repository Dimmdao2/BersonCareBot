...
node "$REPO_ROOT/deploy/host/saas-c2-secret-preflight.mjs" \
  --runtime-phase=pre-cutover-source \
...
diagnostic_login="$(probe "$diagnostic_url" "SELECT app.release_principal_context(); SET ROLE app_operational_diagnostic; RESET ROLE; SELECT session_user;" | tail -n 1)"
delivery_login="$(probe "$delivery_url" "SELECT app.release_principal_context(); BEGIN; SET ROLE app_operational_delivery_worker; UPDATE public.outgoing_delivery_queue SET id=id WHERE false; SELECT resolution FROM app.resolve_outgoing_delivery_scope('00000000-0000-4000-8000-000000000000'::uuid); SELECT app.operator_incident_alert_already_sent('00000000-0000-4000-8000-000000000000'::uuid); SELECT app.mark_operator_incident_alert_sent('00000000-0000-4000-8000-000000000000'::uuid); SELECT 1 / has_function_privilege(current_user, 'app.record_operator_delivery_attempt(text,text,text,uuid,text,text,integer,text,text,timestamp with time zone)', 'EXECUTE')::int; SELECT app.revalidate_specialist_task_reminder_materialization('00000000-0000-4000-8000-000000000000'::uuid); SELECT app.apply_specialist_task_reminder_success_outcome('00000000-0000-4000-8000-000000000000'::uuid); SELECT 1 / has_function_privilege(current_user, 'app.revalidate_patient_reminder_delivery_materialization(uuid)', 'EXECUTE')::int; SELECT app.read_operational_verbose_log_flag(); ROLLBACK; RESET ROLE; SELECT session_user;" | tail -n 1)"
scheduler_login="$(probe "$scheduler_url" "SELECT app.release_principal_context(); BEGIN; SET ROLE app_operational_scheduler; SELECT count(*) FROM app.list_scheduler_reminder_organization_ids(); UPDATE integrator.idempotency_keys SET key=key WHERE false; DELETE FROM integrator.idempotency_keys WHERE false; INSERT INTO integrator.idempotency_keys(key, expires_at, request_hash, status, response_body) SELECT 'c4-readiness', now(), '', 200, '{}'::jsonb WHERE false; SELECT app.read_operator_health_probe_config(); SELECT app.read_operator_health_imap_setting(); SELECT app.read_operator_health_smtp_outbound_setting('patient'); SELECT app.read_operator_health_smtp_outbound_setting('staff'); SELECT app.read_operational_verbose_log_flag(); SELECT app.read_operator_outbound_probe_meta(); SELECT count(*) FROM app.list_google_calendar_probe_organization_ids(); SELECT app.resolve_operator_probe_incidents('outbound:max:'); SELECT app.resolve_operator_probe_incidents('outbound_delivery_provider:email:email_patient_round_trip_failed'); SELECT app.record_operator_outbound_probe_run('success', now(), NULL, '{}'::jsonb); SELECT id FROM app.open_or_touch_operator_probe_incident('max','max_probe_failed','c4-readiness'); SELECT id FROM app.open_or_touch_operator_probe_incident('email','email_patient_round_trip_failed','c4-readiness'); ROLLBACK; RESET ROLE; SELECT session_user;" | tail -n 1)"
...
expect_denied "$scheduler_url" "scheduler cross-contour delivery audit capability" \
  "SET ROLE app_operational_scheduler; SELECT app.record_operator_delivery_attempt('message.send','c4-readiness',NULL,'00000000-0000-4000-8000-000000000000'::uuid,'max','failed',1,NULL,'{}', now());"
