...
function bootstrap({ apiPath, webappPath, mediaPath, ownerUid = 0, deployGid }) {
  ...
  for (const key of ["DB_PRINCIPAL_CONTEXT_MODE", "DB_PRINCIPAL_SIGNING_SECRET"]) {
    if (!api.get(key) || api.get(key) !== webapp.get(key)) fail(`${key} must be present and equal in api.test/webapp.test`);
  }
  ...
  if (parsedMedia.get("DB_PRINCIPAL_CONTEXT_MODE") !== api.get("DB_PRINCIPAL_CONTEXT_MODE") ||
