...
# have one stable password on named DEV/TEST (AGENTS.md §1a). Restore that non-prod contract while writers remain stopped;
# the helper takes the environment by name, refuses every database outside its DEV/TEST allowlist, and never prints the
# password or hashes.
log "restore canonical TEST owner account passwords"
sudo -u postgres env \
  DATABASE_URL="postgresql:///$DB?host=/var/run/postgresql" \
  node "$DEPLOY_REPO/apps/webapp/scripts/ensure-nonprod-owner-account-passwords.mjs" \
    --execute --confirm-test-owner-password-reset --database=test
