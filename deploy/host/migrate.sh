...
assert_canonical_prod_host() {
  ...
  [ "$current_hostname" = "adelaide" ] ||
    fail "refusing PROD migration on host '${current_hostname:-unknown}'; expected adelaide"
  for address in $(hostname -I 2>/dev/null || true); do
    [ "$address" = "135.106.162.170" ] && return 0
  done
  fail "refusing PROD migration without local IPv4 135.106.162.170"
}
