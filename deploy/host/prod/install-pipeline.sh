...
# Канонический скрипт бэкапов ставится тем же деплоем и тем же коммитом, что и его расписание.
# Раньше его не ставил никто: в /opt/backups/scripts было пусто, и потому у боевой базы нового прода
# не существовало НИ ОДНОГО бэкапа (замер 14.09.2026). Ставить руками — это ровно тот механизм,
# которым «поставить забыли, и никто не заметил» и получается.
#
# 0700: скрипт читает env-файлы с боевыми строками подключения; читать его незачем никому, кроме root.
# Каталог самих бэкапов здесь НЕ создаётся — его создаёт сам скрипт с проверкой режима и владельца.
echo "==> устанавливаю канонический скрипт бэкапов"
install -d -m 0700 -o root -g root /opt/backups/scripts
install -m 0700 -o root -g root "$SRC/deploy/postgres/postgres-backup.sh" \
