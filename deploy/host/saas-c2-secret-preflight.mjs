...
function validateLoadedFiles(loadedFiles) {
  ...
  if (mediaWorker?.values.get('DATABASE_URL')?.trim()) fail('media-worker must not receive DATABASE_URL');
  if (mediaWorker?.values.get('DB_PRINCIPAL_CONTEXT_MODE')?.trim() || mediaWorker?.values.get('DB_PRINCIPAL_SIGNING_SECRET')?.trim()) {
    fail('media-worker must not receive DB principal configuration');
  }
  try { new URL(mediaWorker?.values.get('MEDIA_WORKER_CONTROL_URL') ?? ''); } catch { fail('media-worker MEDIA_WORKER_CONTROL_URL must be an HTTP URL'); }
  if (mediaWorker?.values.get('INTERNAL_JOB_SECRET') !== webapp?.values.get('INTERNAL_JOB_SECRET')) {
  ...
  if (new Set(runtimeUrls).size !== runtimeUrls.length) {
    fail(
      'integrator operational DATABASE_URL values must use distinct login credentials',
    );
  }
  ...
  if (new Set(runtimeUsernames).size !== runtimeUsernames.length) {
    fail(
      'integrator operational DATABASE_URL values must use distinct PostgreSQL login roles',
    );
  }
  ...
  return {
    ...
    mediaWorkerControlUrlShape: fingerprintUrlHost(mediaWorker?.values.get('MEDIA_WORKER_CONTROL_URL') ?? ''),
  };
}
...
`),
  ...
  for (const broken of [brokenSecret, brokenCrossProcessUsername, brokenOperationalUsername, brokenMediaSecret]) {
    try {
  ...
  if (detected !== 4) fail('self-test did not detect all secret/login collision regressions');
