...
const screenshotRoot =
  '/home/dev/dev-projects/BersonCareBot/.claude/screenshots/SAAS-S3-TEST-WALKTHROUGH';
  ...
  for (let redirectCount = 0; redirectCount <= 12; redirectCount += 1) {
    ...
    try {
      ...
      return {
        ...
        path: normalizedDiagnosticPath(current),
        status,
        category: 'redirect_invalid_location',
      };
    }
    if (next.origin !== exactBase) {
      return {
        ...
        path: normalizedDiagnosticPath(current),
        status,
        category: 'cross_origin_redirect',
      };
    }
    next.search = '';
    next.hash = '';
...
async function capture() {
  ...
  if (process.getuid?.() !== dev.uid || process.getgid?.() !== dev.gid)
    fail('capture_requires_dev_identity');
  ...
  process.stdout.write(
    `TEST global-admin System Health capture complete\noutput=${outputDirectory}\n`,
  );
}
...
async function selfTest() {
  ...
  try {
    ...
    if (
      env.BASE !== exactBase ||
      'NODE_OPTIONS' in env ||
      Object.values(env).includes('https://evil.example')
    ) {
    ...
    const okProbe = await probeFixedNavigation('self-test-cookie-secret', async () =>
      response(200),
    );
    ...
    const loopProbe = await probeFixedNavigation('self-test-cookie-secret', async () =>
      loopResponses.shift(),
    );
    if (loopProbe.category !== 'redirect_loop' || loopProbe.path !== exactRoute) {
      fail('self_test_navigation_loop_failed');
    }
    const crossOriginProbe = await probeFixedNavigation('self-test-cookie-secret', async () =>
      response(307, 'https://evil.example/path?secret=must-not-appear'),
    );
    if (
      crossOriginProbe.category !== 'cross_origin_redirect' ||
      crossOriginProbe.path !== exactRoute
    ) {
      fail('self_test_navigation_cross_origin_failed');
    }
    if (
      JSON.stringify(crossOriginProbe).includes('secret') ||
      JSON.stringify(crossOriginProbe).includes('evil')
    ) {
    ...
    try {
      ...
      falseSuccessRejected =
        error instanceof Error && error.message === 'capture_png_missing_or_ambiguous';
    }
    ...
    const chromiumStyleRows = Buffer.from([
      0, 255, 255, 255, 255, 0, 0, 0, 255, 4, 0, 0, 0, 0, 0, 0, 0, 0,
    ]);
    validatePngContents(
      Buffer.concat([
        pngSignature,
    ...
    Buffer.from([0, 0, 0, 0, 0x49, 0x45, 0x4e, 0x44, 0xae, 0x42, 0x60, 0x82]).copy(
      crafted36BytePng,
      24,
    );
    ...
    assertInvalidPng(
      'wrong_ihdr_length',
      Buffer.concat([pngSignature, pngChunk('IHDR', Buffer.alloc(12)), validIdat, validIend]),
    );
    assertInvalidPng('missing_idat', Buffer.concat([pngSignature, validIhdr, validIend]));
    assertInvalidPng(
      'empty_idat',
      Buffer.concat([pngSignature, validIhdr, pngChunk('IDAT', Buffer.alloc(0)), validIend]),
    );
    assertInvalidPng(
      'nonempty_iend',
      Buffer.concat([pngSignature, validIhdr, validIdat, pngChunk('IEND', Buffer.from([0]))]),
    );
    assertInvalidPng(
      'ihdr_not_first',
      Buffer.concat([pngSignature, validIdat, validIhdr, validIend]),
    );
    assertInvalidPng(
      'invalid_zlib_stream',
      Buffer.concat([pngSignature, validIhdr, pngChunk('IDAT', Buffer.from([0])), validIend]),
    );
    assertInvalidPng(
    ...
    assertInvalidPng(
      'oversized_dimension',
      Buffer.concat([pngSignature, pngChunk('IHDR', oversizedIhdrData), validIdat, validIend]),
    );
    ...
    bombIhdrData[9] = 6;
    assertInvalidPng(
      'decoded_resource_bomb',
      Buffer.concat([pngSignature, pngChunk('IHDR', bombIhdrData), validIdat, validIend]),
    );
...
try {
  ...
  process.stderr.write(
    `TEST global-admin capture failed: ${error instanceof Error ? error.message : 'unknown'}\n`,
  );
  process.exitCode = 1;
}
