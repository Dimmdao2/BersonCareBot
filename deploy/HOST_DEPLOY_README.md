...
**Напоминания о задачах специалиста (фаза 2C) — временный legacy reconciliation, не целевой producer:** canonical `create`/`update` на `specialist_tasks` уже материализуют готовую строку напоминания в единую `public.outgoing_delivery_queue` write-time, тем же коммитом что и запись `remind_at`; `complete`/`delete` terminalize'ят ещё не отправленные строки. `POST /api/internal/specialist-task-reminders/tick` (после миграции **`0102_specialist_tasks.sql`**) остаётся подстраховочным sweep поверх той же очереди — due-задачи (`remind_at`, не выполнена, без `reminder_sent_at`), доставку выполняет резидентный процесс интегратора, читающий очередь. Роут пока существует и cron на loopback **каждые 5–15 минут** пока нужен: снимаются вместе, тем же коммитом, что и пост-деплойные проверки, только после именованной DEV/TEST read-only сверки старых будущих задач и живого доказательства write-time пути (`docs/_TODO/runs/integrator-cleanup/D30_SCHEDULER_REVERSAL_PLAN.md` Ш3). Параметр **`?limit=`** (по умолчанию 50, cap 100). Ответ JSON: `processed`, `sent`, `errors`.
...
| ----------------------------------------------------------------- | --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| **`bersoncarebot-specialist-task-reminders`** (имя на усмотрение) | **временный, legacy reconciliation** — canonical create/update уже пишут write-time в `outgoing_delivery_queue`; снимается после DEV/TEST сверки (Ш3) | `POST /api/internal/specialist-task-reminders/tick` каждые 5–15 мин — подстраховочный sweep по `specialist_tasks`, не целевой producer         |
