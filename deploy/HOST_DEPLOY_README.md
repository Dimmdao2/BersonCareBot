...
**Превью медиатеки (фон):** превью и стандартный рендишн картинки делает **`media-worker`**, а НЕ вебапп. С 10.09.2026 (М7, `docs/_TODO/STORAGE_PACKAGES_2026-09-10.md`) разбор чужих байт — sharp/libvips, ImageMagick, кадр ffmpeg — вынесен из процесса Next.js, который держит пулы к базе и сессионный секрет, в отдельный процесс без учётных данных БД. Воркер берёт наряд и отчитывается об исходе тем же контрольным швом, что и пересборка видео: `POST /api/internal/media-worker/control` (команды `preview_claim`, `preview_hosted_bytes`, `preview_done_image`, `preview_done_poster`, `preview_failed`, `preview_tick`). Объекты те же: `previews/sm/…`, `previews/md/…` и `media/{id}/standard.webp` в приватном бакете; отдача в браузер — `GET /api/media/:id/preview/sm|md` (сессия врача). **Cron у этого задания больше нет:** дверь `/api/internal/media-preview/process`, скрипт `pnpm run media-preview:tick` и оба шаблона `bersoncarebot*-media-preview.cron.template` сняты вместе с обработчиком. Строка «Превью медиа» в «Здоровье системы» осталась — отметку (`job_family=media`, ключ `media.preview.process`) пишет сам воркер не реже раза в минуту, в том числе в простое, поэтому пустая строка теперь означает «media-worker не работает». **Действие оператора при обновлении:** удалить `/etc/cron.d/bersoncarebot-media-preview` (на TEST — `bersoncarebot-test-media-preview`) и убедиться, что служба media-worker запущена; `node deploy/host/background-jobs-cli.mjs --verify-installed --env prod|test` покажет лишнюю строку расписания.
