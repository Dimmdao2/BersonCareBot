...
**Превью медиатеки (фон):** превью и стандартный рендишн картинки делает **`media-worker`**, а НЕ вебапп. С 10.09.2026 (М7, `docs/_TODO/STORAGE_PACKAGES_2026-09-10.md`) разбор чужих байт — sharp/libvips, ImageMagick, кадр ffmpeg — вынесен из процесса Next.js, который держит пулы к базе и сессионный секрет, в отдельный процесс без учётных данных БД. Воркер берёт наряд и отчитывается об исходе тем же контрольным швом, что и пересборка видео: `POST /api/internal/media-worker/control` (команды `preview_claim`, `preview_hosted_bytes`, `preview_done_image`, `preview_done_poster`, `preview_failed`, `preview_tick`). Исходник читается из СЫРОГО бакета (`S3_RAW_BUCKET`, М7), рендишн и эскизы пишутся в бакет выдачи: `previews/sm/…`, `previews/md/…` и `media/{id}/standard.webp`; оригинал НЕ переписывается и НЕ удаляется, `s3_key` строки остаётся прежним, а единственный факт готовности — `standard_rendition_at`. Отдача в браузер — `GET /api/media/:id/preview/sm|md` (сессия врача). **Cron у этого задания больше нет:** дверь `/api/internal/media-preview/process`, скрипт `pnpm run media-preview:tick` и оба шаблона `*-media-preview.cron.template` сняты вместе с обработчиком. Строка «Превью медиа» в «Здоровье системы» осталась — отметку (`job_family=media`, ключ `media.preview.process`) пишет сам воркер не реже раза в минуту, в том числе в простое, поэтому пустая строка теперь означает «media-worker не работает». **Действие оператора при обновлении:** удалить `/etc/cron.d/therapysto-media-preview` (на TEST — `bersoncarebot-test-media-preview`) и убедиться, что служба media-worker запущена; `node deploy/host/background-jobs-cli.mjs --verify-installed --env prod|test` покажет лишнюю строку расписания.

> ⚠️ **Открыто после слияния 13.09.2026:** разового бэкфилла старых картинок больше нет. Режим > `--reconcile-images` (`pnpm run media-preview:tick -- --reconcile-images --limit N` и > `POST /api/internal/media-preview/process?mode=reconcile-images`) жил в снятых двери и скрипте; очередь воркера > берёт только `pending`, а у загруженного до М7 изображения `preview_status = 'ready'` при пустом > `standard_rendition_at`. Пока замена не выбрана, такие строки остаются на заглушке «Картинка готовится» > (см. `docs/_TODO/STORAGE_PACKAGES_2026-09-10.md`, остаток по четырём строкам). Сам возврат строк в `pending` — > чистый UPDATE и чужих байт не касается, поэтому его новое место не обязано быть внутри воркера.

**Playback telemetry (HOUSEKEEPING):** `POST /api/internal/media-playback-stats/retention` с тем же Bearer за один тик чистит три хранилища: почасовые агрегаты **`media_playback_stats_hourly`** старше **90 дней**, сырые успешные резолвы и клиентские ошибки playback старше **400 дней**. Параметр **`?days=`** переопределяет только 90-дневное окно агрегатов; **`?dryRun=1`** возвращает счётчики по всем трём хранилищам без удаления. Таблица дедупа **`media_playback_user_video_first_resolve`** не затрагивается. Расписание объявлено в manifest (`playback_retention`, еженедельно).
...
**Retention служебных журналов БД (Track D, HOUSEKEEPING):** `POST /api/internal/db-journal-retention/tick` с тем же Bearer: один тик сметает все цели из единого `deploy/postgres/privileges/journal-lifecycle-registry.ts` через существующие чекпоинты `app.prune_retention_target`/`app.prune_context_nonce_ledger` — **`app.context_nonce_ledger`** (grace 1 час), **`public.idempotency_keys`**/**`integrator.idempotency_keys`** (истёкшие + 24ч), **`public.outgoing_delivery_queue`** статус `sent` (30д по `sent_at`) и `dead` (180д по `dead_at`; живые статусы никогда не трогаются), **`public.notification_delivery_attempts`** (180д). **`?dryRun=1`** — только подсчёт без DELETE. Ответ JSON: `{ ok, dryRun, results: [{ target, deleted }] }`. Расписание объявлено в manifest (`db_journal_retention`, ежечасно): nonce ledger и idempotency-таблицы нуждаются в почасовом ритме, для остальных целей это безвредный избыток. `Host`/`Origin`/`X-Forwarded-Proto` строит общий transport, а не строка расписания.

**Known limitations / runtime requirements:** HEIC/HEIF (`image/heic`, `image/heif`) разбираются `ffmpeg`, а при его отказе — запасным `ImageMagick` (`magick`/`convert`). Оба бинаря нужны теперь **процессу media-worker**, а не вебаппу: у воркера есть bundled `@ffmpeg-installer/ffmpeg`, но на хосте предпочтителен системный — `apt install ffmpeg` + `FFMPEG_PATH=/usr/bin/ffmpeg` в `/opt/env/bersoncarebot/media-worker.prod`. Для запасного разбора HEIC поставьте `imagemagick` и при необходимости задайте `MAGICK_PATH=/usr/bin/magick` (или `/usr/bin/convert`) **в том же env-файле воркера**. В образе `deploy/docker/Dockerfile` ImageMagick нет — там HEIC держится на ffmpeg, и это не регресс: до переезда картинки разбирал тот же образ. Исходник перед разбором скачивается во временный файл, и `ffmpeg` получает **локальный путь** с `-protocol_whitelist file`: подписанный HTTPS-URL в `-i` больше не попадает. Потолок одного разбора — `MEDIA_WORKER_PREVIEW_TIMEOUT_MS` (по умолчанию 120 с), срок аренды строки — `MEDIA_WORKER_PREVIEW_LEASE_MINUTES` (15 минут): не уложился воркер — строка возвращается в очередь сама.
...
**S3 / MinIO и фоновые джобы (webapp):** имена ключей (значения не в документ): `S3_ENDPOINT`, `S3_ACCESS_KEY`, `S3_SECRET_KEY`, **`S3_PRIVATE_BUCKET`** (обязателен для CMS-медиа в private-режиме), **`S3_RAW_BUCKET`** (обязателен так же, как `S3_PRIVATE_BUCKET` — сырой бакет загрузок, М7, см. «CMS медиа и S3» выше), опционально `S3_PUBLIC_BUCKET`, `S3_REGION`, `S3_FORCE_PATH_STYLE`; **`INTERNAL_JOB_SECRET`** — Bearer для `POST /api/internal/media-pending-delete/purge`, `POST /api/internal/media-multipart/cleanup`, `POST /api/internal/media-worker/control` (в т.ч. очередь превью), `POST /api/internal/media-playback-stats/retention`, `POST /api/internal/media-hls-proxy-errors/retention`, `POST /api/internal/product-analytics/retention`, `POST /api/internal/media-transcode/reconcile`, `POST /api/internal/saas-billing/renewal/tick`, `POST /api/internal/db-journal-retention/tick`; `FFMPEG_PATH=/usr/bin/ffmpeg` — путь к системному ffmpeg; нужен теперь службе **media-worker** (её env-файл), вебапп ffmpeg не запускает; опционально **`LOG_LEVEL`** — уровень логов pino в webapp (`info`, `warn`, `error`; по умолчанию в приложении `info`). Подробности и CORS: раздел **Nginx → Webapp** выше («CMS медиа и S3», «Очередь удаления медиа»); канон env: `docs/ARCHITECTURE/SERVER CONVENTIONS.md`. **Политика private-бакета (без анонимного чтения):** чеклист в [`docs/REPORTS/S3_PRIVATE_MEDIA_EXECUTION_LOG.md`](../docs/REPORTS/S3_PRIVATE_MEDIA_EXECUTION_LOG.md) § Private bucket policy.
...
`APP_BASE_URL`, `PATIENT_APP_ORIGIN`, `PATIENT_APP_NAME` и app-facing custom-domain targets
`CUSTOM_DOMAIN_EDGE_IP=151.241.228.122`, `CUSTOM_DOMAIN_CNAME_TARGET=test.therapygo.ru` в существующих TEST env
с сохранением metadata и backup. The normal `deploy-test.sh` path invokes the surface installer and the TEST VPN split-DNS installer from the deployed commit; do not run a separate nginx or Certbot procedure for the branded host.
