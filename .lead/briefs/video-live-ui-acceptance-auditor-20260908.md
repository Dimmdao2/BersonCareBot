...
`docs/_TODO/VIDEO_MEETINGS_JITSI_2026-09.md`, дословно VM-06, VM-10 и UI-08..UI-10. Не заменяй требования
пересказом и не добавляй функции из owner-вопросов о PiP/fullscreen/screen share/app-wide floating call.
...
1. Простой render/open doctor live client не делает POST/create/join. После одного явного нажатия делает ровно один
   POST; быстрый double click не создаёт две meeting sessions. Ошибка create даёт retryable состояние без автоповтора.
...
3. Autosave/re-render соседней панели по-прежнему не dispose/recreate существующую конференцию (NOTE-08).
4. Взглядом на код/конфиг зафиксируй текущий результат для вкладок patient card, реальной высоты iframe/нижней панели,
...
5. Проверь, что предполагаемая реализация может параметризовать существующие `DoctorLiveMeetingClient`,
...
Можно менять только применимые `*.test.tsx` в video/doctor-live UI и создать один audit artifact в `docs/audit/`.
Production code/config, план, taskdb, сервер и deploy не менять. Временные production fault-injection правки полностью
