...
function selectorPrimitive(selector) {
  if (typeof selector === 'string') {
    ...
    const data = selector.match(/^\[data-([A-Za-z][\w-]*)\]$/)?.[1];
    if (data) return { kind: 'data', value: `data-${data}` };
    return null;
  }
...
function routeOwnsModule(template, routeModule) {
  ...
  for (let index = 0; index < routeModule.length; index += 1) {
    ...
    // A catch-all page is also the owner when it receives zero trailing
    // segments (for example a tab shell at its base patient-card URL).
...
function importedPaths(record, recordsByPath, rootPaths) {
  const imported = [...record.source.matchAll(/(?:\bfrom\s+|\bimport\s*\(\s*)["']([^"']+)["']/g)]
    .map((match) => match[1]);
  const resolved = [];
  ...
  const check = (selector, label, source) => {
    ...
    const exists = primitive.kind === 'id'
      ...
      : primitive.kind === 'aria'
        ...
        : primitive.kind === 'data'
          ? new RegExp(`\\b${escaped}(?:\\s*=|\\s|>)`).test(source)
