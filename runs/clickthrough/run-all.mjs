...
async function main() {
  ...
  for (const flow of FLOWS) {
    ...
    try {
      const result = await flow.run({ browser, baseUrl: BASE_URL, screenshotDir: SCREENSHOT_DIR, log: console.log });
      results.push(result);
      console.log(`=== ${flow.name} verdict: ${result.verdict} ===`);
    } catch (e) {
      console.error(`=== ${flow.name} threw: ${e.message} ===`);
      results.push({ flow: flow.name, steps: [{ name: "uncaught_exception", ok: false, detail: e.stack ?? e.message }], verdict: "error" });
    }
  }
  await browser.close();
