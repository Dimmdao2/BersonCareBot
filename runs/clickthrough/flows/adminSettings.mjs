...
export async function runAdminSettingsFlow({ browser, baseUrl, screenshotDir }) {
  ...
  await page
    .screenshot({ path: `${screenshotDir}/admin-settings-before.png`, fullPage: true })
    .catch(() => {});
  ...
  steps.push({
    name: 'admin_console_loads',
    ok: visible,
    detail: `support_contact_url input visible=${visible}`,
  });
  if (!visible) {
    await context.close();
  ...
  await page
    .screenshot({ path: `${screenshotDir}/admin-settings-after-click.png`, fullPage: true })
    .catch(() => {});
  ...
  const valueAfterReload = await page
    .getByPlaceholder('/app/patient/support или https://t.me/…')
    .inputValue();
  await page
    .screenshot({ path: `${screenshotDir}/admin-settings-after-reload.png`, fullPage: true })
    .catch(() => {});
  ...
  if (persisted && valueAfterReload !== originalValue) {
    ...
    await page.goto(`${baseUrl}/app/admin/app-settings`, { waitUntil: 'load', timeout: 20000 });
    const valueAfterRestore = await page
      .getByPlaceholder('/app/patient/support или https://t.me/…')
      .inputValue();
    steps.push({
  ...
  return {
    flow: 'global-admin-settings-save',
    steps,
    verdict: allOk ? 'pass' : 'fail',
    verdictDetail: verdict5,
  };
}
