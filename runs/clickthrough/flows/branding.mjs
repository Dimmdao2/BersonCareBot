...
export async function runBrandingFlow({ browser, baseUrl, screenshotDir }) {
  ...
  await adminPage
    .screenshot({ path: `${screenshotDir}/branding-settings-before.png`, fullPage: true })
    .catch(() => {});

  const nameInput = adminPage.locator('#org-brand-name');
  const inputVisible = await nameInput.isVisible().catch(() => false);
  steps.push({
    name: 'org_brand_name_input_visible',
    ok: inputVisible,
    detail: `visible=${inputVisible}`,
  });
  if (!inputVisible) {
  ...
  steps.push({
    name: 'read_original_name',
    ok: true,
    detail: `originalName length=${originalName.length}`,
  });
  ...
  const savedText = await adminPage
    .locator('body')
    .innerText()
    .catch(() => '');
  const sawSavedConfirmation = savedText.includes('Сохранено.');
  await adminPage
    .screenshot({ path: `${screenshotDir}/branding-settings-after-save.png`, fullPage: true })
    .catch(() => {});
  ...
  await adminPage.goto(`${baseUrl}/app/settings`, { waitUntil: 'load', timeout: 20000 });
  const nameAfterReload = await adminPage
    .locator('#org-brand-name')
    .inputValue()
    .catch(() => '');
  steps.push({
  ...
  const sidebarBrandText = await doctorPage
    ...
    .catch(() => '');
  await doctorPage
    .screenshot({ path: `${screenshotDir}/branding-doctor-shell-after.png`, fullPage: true })
    .catch(() => {});
  ...
  await adminPage
    .locator('section', { hasText: 'Бренд клиники' })
    .first()
    .getByRole('button', { name: 'Сохранить' })
    .click();
  await adminPage.waitForTimeout(1000);
  await adminPage.goto(`${baseUrl}/app/settings`, { waitUntil: 'load', timeout: 20000 });
  const nameAfterRestore = await adminPage
    .locator('#org-brand-name')
    .inputValue()
    .catch(() => '');
  steps.push({
