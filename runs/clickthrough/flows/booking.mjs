...
export async function runBookingFlow({ browser, baseUrl, screenshotDir, log }) {
  const steps = [];
  const context = await browser.newContext({ viewport: { width: 1280, height: 900 } });
  await applyProfileCookie(context, "patient", baseUrl);
  const page = await context.newPage();

  const resp = await page.goto(`${baseUrl}/app/patient/booking`, { waitUntil: "load", timeout: 20000 });
  await page.waitForTimeout(500);
  const bodyText = await page.locator("body").innerText().catch(() => "");
  const digestMatch = bodyText.match(/Код:\s*(\d+)/);
  const errorBoundaryShown = bodyText.includes("Что-то пошло не так");
  await page.screenshot({ path: `${screenshotDir}/booking-open-blocked.png`, fullPage: true }).catch(() => {});
  ...
  steps.push({ name: "note", ok: true, detail: "booking page rendered — full wizard walk not yet implemented past this point (unreached in current TEST state)." });
  await context.close();
