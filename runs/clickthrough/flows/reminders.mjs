  ...
  return page.evaluate(
    async ({ baseUrl, occurrenceId, action, body }) => {
      const res = await fetch(
        `${baseUrl}/api/patient/reminders/${encodeURIComponent(occurrenceId)}/${action}`,
        {
          method: 'POST',
...
export async function runReminderFlow({ browser, baseUrl, screenshotDir }) {
  ...
  await page
    .screenshot({ path: `${screenshotDir}/reminders-list-before.png`, fullPage: true })
    .catch(() => {});
  ...
  const skipResult = await postAction(page, baseUrl, OCCURRENCE_FOR_SKIP, 'skip', {
    reason: 'clickthrough-test',
  });
  steps.push({
    name: 'post_skip',
  ...
  const snoozeResult = await postAction(page, baseUrl, OCCURRENCE_FOR_SNOOZE, 'snooze', {
    minutes: 60,
  });
  steps.push({
    name: 'post_snooze',
  ...
  const journalText = await page
    .locator('body')
    .innerText()
    .catch(() => '');
  await page
    .screenshot({ path: `${screenshotDir}/reminders-journal-after.png`, fullPage: true })
    .catch(() => {});
  ...
  const summary = {
    ...
    productUiGap:
      'No React component in apps/webapp/src calls these three endpoints — verified by grep; there is no button to click for any of the three actions in the current webapp.',
  };
