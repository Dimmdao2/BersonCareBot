...
- TherapyGo login says `Войти в личный кабинет`, with no patient/client role chooser. Initial patient methods are email code and verified TherapyGo bot contact; OAuth/passkey remain implemented but default-policy disabled and become visible/authorized through the existing global policy switch without code changes.
- Host selects presentation only; authorization remains account/membership guarded. Cross-product links use canonical origins, do not forward hostile `next`, and unknown/wrong-product hosts fail closed.
