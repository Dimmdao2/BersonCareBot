...
Мигратор применяет по водяному знаку `created_at`, а не по содержимому: pending — это записи журнала с
`when` больше максимума леджера. Поэтому свободен не тот `when`, которого нет в журнале ветки, а тот, который
выше водяного знака КАЖДОЙ цели (DEV и TEST). Перед сведением в `feat` сверь `when` своей миграции с
`max(created_at)` обеих баз; если соседняя ветка уже подняла водяной знак выше — назначь новый `when` (файл ещё
не применён, переименование разрешено). Ниже водяного знака `when` не опускать никогда.

Проверка стоит в самом мигранте (`deploy/postgres/privileges/migrate-local.mjs`) — единственном месте, через
которое идут все прогоны DEV и TEST: запись журнала одновременно ниже водяного знака и без строки в леджере
останавливает прогон с перечислением застрявших записей. Восстановление — тем же wrapper'ом,
`--apply-out-of-order <tag>` с явным перечислением каждого tag, после проверки, что застрявшая миграция не
зависит по порядку от применённого выше неё. Ручной `psql`-накат мимо wrapper'а не восстанавливает леджер и
запрещён.

**Смысл:** поддерживаемый начальный контракт — `B0`, а не восстановление истории разработки. Номер координирует
только новые post-B0 изменения и не должен превращать старую цепочку обратно в активный путь. Водяной знак же
делает пропуск ОКОНЧАТЕЛЬНЫМ: миграция, чей `when` оказался ниже него, не попадёт в pending уже никогда, а
леджер будет честно отвечать «применено всё» поверх отсутствующих объектов.
