...
### Миграции: номер бронируется на доске до создания файла

Номер миграции берётся не по «последнему в каталоге», а из доски параллельных оркестраторов —
[`docs/_TODO/NIGHT_WAVE_AUDIT_QUEUE_2026-07-28.md`](docs/_TODO/NIGHT_WAVE_AUDIT_QUEUE_2026-07-28.md),
раздел «Доска параллельных оркестраторов». Порядок: прочитать доску → вписать строку со своим номером,
веткой и клоном → создать файл. Освободил или перенумеровал — обновить свою строку. Работая в клоне,
номер в имени файла помечать временным (`-- TEMPORARY LOCAL MIGRATION NUMBER NNNN`); окончательный
присваивает тот, кто сводит в `feat`.
