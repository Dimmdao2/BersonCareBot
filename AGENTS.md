...
### Миграции: временный номер в клоне, окончательный — перед сведением

**Правило владельца 03.08 (заменяет прежнюю бронь номера на доске):** «присвой временный номер и назначь нужные
перед сведением». Порядок: в своём клоне взять ЛЮБОЙ свободный временный номер и пометить его в файле
(`-- TEMPORARY LOCAL MIGRATION NUMBER NNNN`) → работать → **окончательный номер присваивает тот, кто сводит в
`feat`**, непосредственно перед `land`, когда уже видно, что реально занято. Перенумерация при сведении — штатный
шаг, а не авария: переименовать файл, дописать journal (`idx`/`when`), поправить ссылки на номер в
`deploy/host/deploy-test-saas.sh` и в комментариях, прогнать `check-drizzle-journal-sync.sh` и
`run-webapp-drizzle-migrate.mjs --self-test`.

⛔ Прежняя редакция требовала вписать номер на доску ДО создания файла. Она снята: при трёх параллельных агентах
все читают доску раньше, чем любой из них успевает в неё записать, и номера всё равно сталкиваются — это
наблюдалось дважды за один вечер 03.08. Доска остаётся для координации веток и вердиктов, но номер миграции она
больше не раздаёт.

⚠️ Перенумеровать можно только ДО применения. После применения на живой базе номер уже в журнале, и
переименование файла ничего не исправит (см. «Смысл» ниже).

**Смысл:** каталог миграций в клоне не видит номеров, занятых в других клонах, поэтому «последний плюс
один» даёт столкновение. Хуже того, столкновение не чинится переименованием после применения: установленный
мигратор решает по отметке времени журнала против максимума `created_at`, а не по хешу файла, — миграция под
уже применённым номером не доедет никогда.
...
### 3a. Как выбирать DEV, TEST и disposable PostgreSQL

По умолчанию быстрая проверка product/DB-поведения выполняется на существующей `bcb_webapp_dev`: её разрешено
сидировать и менять, а pending migrations применяются только через `migrate-dev.sh --preflight` → `--execute`
из пункта выше. Не поднимать одноразовый кластер только ради того, чтобы не использовать предназначенную для
разработки DEV-базу. DEV-проверка не является финальным доказательством RLS/ACL, если результат зависит от
production-like login roles, владельца SECURITY DEFINER-функции или provisioning overlays.

Disposable PostgreSQL нужен только когда проверке необходима хотя бы одна из этих изоляционных гарантий:

- начать с чистой схемы и повторно проиграть migration tail/ledger;
- намеренно испортить schema/data/permissions или проверить rollback/crash/cleanup;
- доказать transaction/concurrency/parallel-test isolation без влияния общего состояния;
- дать каждому повторяемому DB-test file отдельное детерминированное состояние.

Канонический A0 baseline создан с `--no-owner --no-privileges`: он доказывает DDL, ledgers, seed и прикладное
поведение, не зависящее от runtime ACL. Его запрещено выдавать за DEV/TEST parity, SECURITY DEFINER, RLS или ACL
proof и запрещено «доделывать» локальными GRANT-ами ради зелёного product-теста. Для изолированной матрицы ролей
используется существующий A1; финальная проверка runtime login/principal/RLS и пользовательского E2E выполняется
на TEST под точными `app_*_login` и разрешёнными тестовыми учётками. Новый A0/A1/harness рядом не строить.

**Смысл:** выбор среды определяется поломкой, которую надо поймать. Общая DEV лучше показывает обычное текущее
product-поведение; disposable лучше показывает воспроизводимость и изоляцию; TEST — реальные deploy-роли и полный
путь человека. Перенос проверки в более тяжёлую среду без такой причины создаёт машинерию и может сделать proof
менее точным.
