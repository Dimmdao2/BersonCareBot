...
### UI: автоматизировать устойчивый контракт, изменчивую форму принимать live

- Автоматизировать устойчивые контракты: деньги, доступ, данные, API, бизнес-правила и критические действия
  пользователя.
- Часто меняющийся UI проверять глазами и короткой live-приёмкой. Не закреплять UI-тестом текущие тексты,
  количество элементов, расположение или DOM. Если конкретное значение само является устойчивым бизнес-
  контрактом, проверять контракт ниже UI.
- UI-тест писать только тогда, когда важен наблюдаемый результат действия пользователя и эту поломку нельзя
  надёжно поймать на более дешёвом слое. Правило даты проверяет unit-тест, HTTP-контракт — route-тест; UI —
  последний слой.
- Чем тест ближе к GUI, тем меньше таких тестов должно быть: GUI-тесты дороже и хрупче.

Это следует из принципа Testing Library: тест должен быть похож на использование продукта и не мешать
рефакторингу — [Testing Library](https://testing-library.com/docs/). GUI-тесты составляют только небольшую
верхушку набора — [Martin Fowler, Test Pyramid](https://martinfowler.com/bliki/TestPyramid.html).
