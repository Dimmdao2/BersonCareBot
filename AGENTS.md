...
2. **Настоящая работа находится проходом по пути человека, а не перебором плана.** Берётся цепочка целиком — от
...
3. **Разрыв не чинится возвратом вырезанного.** После удаления реализуй новое решение владельца, а не возвращай
   старое под другим именем, умолчанием или временным исключением. Если решения нет — сначала ищи его в репозитории,
   затем смотри, как это устроено у зрелых продуктов и компаний. Мировая практика применяется и к продуктовым, и к
   инженерным решениям. Владельцу остаются направление, деньги, приоритеты, личные предпочтения и выбор между
   равноценными вариантами с разной ценой для него. Пустой поиск доказывается точными запросами, `code-search` и
   обратными ссылками; «не нашёл» без перечня не считается. При расхождении двух owner-текстов о продукте или
   поведении побеждает **более позднее по дате** решение владельца — смотри на актуальность даты, а не на порядок в
   чате или плане. Доказательство owner-решения: явные маркеры или цитаты владельца, отдельные документы
   (`OWNER_DECISIONS*`, `OWNER_RULINGS*`, owner-строки в punchlist) — **не** поздний narrative агента, плановая проза
   «уточнено агентом» или додумки без owner-метки. Старший owner-текст, заменённый поздним, помечай
   **УСТАРЕЛО/ЗАМЕНЕНО →** новое решение (с датой или ссылкой); не удаляй цитату молча и не оставляй оба варианта
   активными.
   ⛔ **Не смягчать решения владельца.** Агент не перефразирует owner-текст в более слабые формы («опционально»,
   «исключения», «compat», «по умолчанию можно», «если не настроено — мягко»), не держит старое и новое как два
   активных варианта и не подменяет исполнение собственной интерпретацией. Owner-текст исполняется как написан; если
   неясно — **OWNER QUESTION**, не более мягкая версия.

   **Смысл:** без даты и маркера владельца агент не отличит позднее решение от собственной интерпретации; два
   «активных» текста заставляют исполнять устаревшее.

4. **Сначала померить, что уже построено.** До определения объёма открой код и фактическое состояние: сколько пути
   уже работает и где именно разрыв. Работа сжимается от чтения, а не от усилий.

5. **Не плодить сущности.** Новый экран, таблица, файл, абстракция или проверка рядом с существующей — по умолчанию
...
6. **Число называется вместе с командой, которой получено.** «Ноль покупок» — результат конкретного запроса вроде
...
7. **Ложная запись о готовности опаснее незакрытого разрыва:** по ней перестают проверять. Исправляй ложную запись
