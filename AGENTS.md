...
- Chrome-типографика — закрытый набор §B.1: page-title `text-[18px] font-medium`; modal title `doctorModalTitleClass` (`text-base font-medium`); modal entity title `doctorModalEntityTitleClass` наследует modal title; на mobile section/body `text-base`, на desktop `text-sm`; **первичная строка списка** `text-base font-normal`; secondary `text-sm`; meta на mobile `text-[13px]`, на desktop `text-xs`; KPI `doctorMetricValueClass` (`text-[18px]`). Micro-роль `text-[10px]`/`text-[11px]` — только бейджи/календарь/оси графиков/mono. Локальные `text-[13px]`, `text-[18px]`, `text-lg`, `text-xl`, `text-2xl`, `text-3xl` запрещены; нестандартные размеры разрешены только через соответствующие общие роли.
