...
**Порядок — это имя файла, и второго места нет.** Мигратор берёт листинг
`apps/webapp/db/drizzle-migrations/*.sql`, сортирует по имени и в этом порядке применяет.

**Новая миграция называется таймштампом, не номером: `YYYYMMDDTHHMMSS_slug.sql` (UTC, точность до
секунды)**, например `20260820T014233_a_new_reminder_channel.sql`. Номер никто больше не выдаёт и не
резервирует: owner 19.08 — «зачем таймштамп и привязка к нему? так давай просто называть миграции
таймштампом — вряд ли два агента создадут в одну долю секунды одинаково; если надо, ты просто
переименуешь в нужной последовательности когда будешь сводить» (после того, как в один вечер две
ветки независимо заняли номер `0050`, и оба аудита упали на этом). Маркер
`-- TEMPORARY LOCAL MIGRATION NUMBER NNNN` и резервирование номера ведущим при сведении — снятая
практика, таймштамп не резервируют. Встать между двумя существующими миграциями — взять таймштамп
между ними; буквенный суффикс (`0043a_…`) был нужен только для целочисленных номеров, для новых
миграций он не нужен. **Уже применённые 50 файлов сохраняют старые `NNNN[suffix]_slug` имена
навсегда** — их не переименовывать и не переводить на новую схему; список этих имён закрыт и взят из
`meta/_journal.json` (§ ниже), он не может вырасти. `check-drizzle-migration-order.sh` (часть
`pnpm run lint`) отказывает новому файлу, чьё имя не входит в этот список и не является таймштампом
(`findMigrationNameViolations` в `migration-order.mjs`) — приземлить миграцию по старой схеме
невозможно.

Сортировка строкой: `'0' < '2'`, поэтому все `NNNN…`-имена всегда идут раньше любого `YYYYMMDD…`-имени
при том же сравнении, которым реально сортирует `readMigrationFolder` — это подтверждено тестом на
реальной папке репозитория, а не только чтением регулярки.

**Применённый файл не переименовывать никогда:** имя — это личность миграции в леджере. Переставить
ещё НЕ применённую миграцию — обычный `git mv` на новый таймштамп, это безопасно и очевидно: ни один
леджер её так не назовёт, пока она не накатана. Переименовать УЖЕ применённую миграцию раньше молча
превращало её обратно в pending; теперь оба прогонщика отказывают заранее и вслух, если pending-файл
побайтно совпадает с содержимым строки леджера, которую эта проверка не может назвать (чужой tag) —
`findRenamedAppliedMigrations` в том же модуле. Гарантия действует по содержимому, не по имени: если
файл и переименовали, и одновременно изменили — это уже не переименование, а новая миграция, и решение
о её безопасности — на исполнителе, как и для любой forward-миграции.

**Применяется всё, чего НЕТ в леджере, а не всё, что выше водяного знака.** Личность миграции —
колонка `tag` в `drizzle.__drizzle_migrations`; старые строки размечены один раз из
`meta/_journal.json`, и это единственная оставшаяся работа журнала для порядка (вторая, новая — быть
закрытым списком легитимных `NNNN`-имён, см. выше). Журнал больше НЕ задаёт порядок и руками не
правится: расхождение имени и `when` при сведении веток — та самая болезнь, из-за которой 19.08 журнал
...
А водяной знак делал пропуск ОКОНЧАТЕЛЬНЫМ: миграция, чьё имя оказалось ниже него, не попадала в
pending уже никогда, и леджер честно отвечал «применено всё» поверх отсутствующих объектов.
...
**⛔ Полный CI не запускается, пока дерево ещё дописывается (владелец, 20.08).** Прогон измеряет ОДНО
состояние репозитория. Если в этот момент ветки ещё сводятся, агенты ещё коммитят, конфликты ещё
разводятся — измеренного состояния к концу работы не будет, и его результат не является доказательством
ни о чём: ни зелёный (он про исчезнувшее дерево), ни красный (падение может принадлежать половине
слияния). Плюс прогон отбирает машину у тех, кто в этот момент работает.

Поэтому «merge/sync/integration checkpoint» в списке ниже означает момент, когда сведение ЗАКОНЧЕНО:
влита последняя ветка, дерево чистое, никто больше не пишет. «Перед merge» — это перед приземлением
готового результата, а НЕ во время серии слияний. Правило действует и когда главное дерево формально
чистое, а писатели сидят в клонах: чистота дерева отвечает на вопрос ГДЕ гонять, а не КОГДА.

Провал, из-за которого это записано: во время сведения одиннадцати веток, при пяти живых воркерах,
лид запустил `pnpm run ci` в главном дереве — потому что дерево было чистым, — не назвав ни одной
непокрытой области, как того требует абзац выше. Владелец: «ДАЖЕ КОД МЕНЯЕТСЯ В ЭТОТ МОМЕНТ».
...
- ОДИН раз на ЗАВЕРШЁННОМ сведении: все ветки влиты, дерево чистое, писателей нет — тогда полный прогон
  измеряет то состояние, которое действительно поедет. ⛔ Формулировка «особенно ПЕРЕД merge» отменена
  владельцем 20.08 как неверная и бессмысленная: когда невлитых веток много, «перед merge» наступает
  столько же раз, и каждый прогон меряет дерево, которого через час не будет. Правки по ходу сведения
  проверяют АУДИТОР и локальные целевые тесты по масштабу правки (раздел
  [Test execution policy](#10-test-execution-and-audit-policy)) — это их работа, и полный CI её не заменяет
  и не дублирует;
