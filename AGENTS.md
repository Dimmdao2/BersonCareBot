...
- Вопрос владельцу — только после поиска в репозитории И изучения мировой практики; продуктовое и инженерное решаешь сам с обоснованием — [«Как решать и задавать вопросы»](#как-решать-и-задавать-вопросы).
...
### Как решать и задавать вопросы

Прежде чем задать вопрос владельцу — три шага, по порядку. Пропуск шага = вопрос не задаётся.

1. **Поищи ответ в репозитории.** Планы, каноны, `runs/owner-questions.md`, `git log`, решения владельца в
   блоках решений. Ответ на вопрос чаще всего уже дан — иногда тем же владельцем месяц назад.
   Пустой результат ДОКАЗЫВАЕТСЯ перечнем попыток: точный поиск, `code-search`, обратные ссылки. «Не нашёл» без
   перечня — не доказательство.
2. **Вопрос продуктовый или инженерный — реши сам, изучив, КАК ЭТО ДЕЛАЕТСЯ В МИРЕ.** Не изобретать и не
   спрашивать владельца о том, что решается практикой взрослых платформ: очереди, ретраи, лимиты, схемы
   доступа, форматы, безопасность. Найди, как делают, выбери и **обоснуй ссылкой на источник**.
3. **Владельцу остаётся только его:** направление продукта, деньги и цены, приоритеты, его собственные
   предпочтения, и выбор там, где практика допускает несколько равноценных путей с разной ценой для него.

Формулировка вопроса — плейн-языком: что за задача человеческими словами, какие варианты, что каждый
означает и чего не будет, какого именно подтверждения ждёшь, почему сам не решаешь. Номер пункта — в скобках,
не заголовком.

**Смысл:** вопрос владельцу стоит дороже любой другой операции — он останавливает работу и тратит его время.
Вопрос, ответ на который лежал в репозитории или в общеизвестной практике, — это переложенная на него работа.
