...
## 🔴 ГЛАВНОЕ ПРАВИЛО: новая область — карта заголовков, свой раздел, потом действие

Перед первым содержательным действием в новой области работы посмотри карту правил одним запросом:

```bash
grep -n "^## \|^### " AGENTS.md
```

Нашёл заголовок по своей теме — прочитай раздел целиком и действуй по нему. Одна печать карты без чтения найденного
раздела не считается. Не нашёл — ищи готовое в репозитории
...
Повтори карту и чтение только когда произошёл хотя бы один переход: контекст был сжат/восстановлен; работа перешла
к другой области путей или категории риска с другим разделом; владелец исправил правило или `AGENTS.md` изменился;
возникло сомнение, какой раздел действует. Не повторяй карту между последовательными действиями одного уже
понятного этапа и при ожидании или опросе уже запущенного CI, деплоя либо субагента. Читать файл целиком не
требуется: карта и «Маршрут» ниже ведут к нужному разделу. Память из прошлого контекста не заменяет чтение при
одном из перечисленных переходов; внутри непрерывного этапа уже прочитанный раздел повторно открывать не нужно.

**Частые входы.** База и SQL — §5. Сервер, пути, порты, env — §1 и `SERVER CONVENTIONS.md`. Миграции — §1
«Миграции schema B». Запуск воркера — §24.2, §24.3. Запуск аудитора — §24.4, §24.5, §10b. Написание и
удаление тестов — §10a. Коммит и текст сообщения — §7 и §1. Полный CI — §9. План и карточка — §12 и §1.

**Смысл:** карта нужна как вход в применимое правило, а не как периодический ритуал. Повтор без смены контекста
создаёт шум и не доказывает чтение; отсутствие чтения при реальном переходе повторяет уже известные провалы.
...
- ПРОДА ДВА. Старый `135.106.162.170` (`bersoncare.ru`) — **не трогать ничем**, даже чтением. Новый `135.106.187.95` (Therapysto/TherapyGo) выкладывается с dev-бокса одной командой `bash tools/deploy-prod-from-dev.sh [ref]`; ничего настраивать руками на нём не требуется — [§1](#1-онбординг-и-server-conventions), [§1b](#1b-безопасность-dev-среды-изоляция-от-прод-и-реальных-каналов).
...
- Перед созданием, изменением, сохранением или приёмкой любого теста ВСЕ роли читают §10a «⛔ ТЕСТ НЕ ДУБЛИРУЕТ КОД, КОНТРАКТ ИЛИ ТЕКСТ»; тест без независимого oracle и дорогой молчаливой поломки удаляется — [§10a](#10a-тест-проверяет-поведение-а-не-текст-исходника-и-не-обстоятельства-запуска).
...
2. **Строгая цель — решения и указания владельца; план — средство.** Работу и готовность судят по соответствию
   owner-решениям и поручениям, а не по полноте агентского плана или числу закрытых чекбоксов. Перед запуском
   **каждого этапа** остановись и проверь все более поздние owner-регистры, датированные owner-цитаты,
   profile plans/audits и их обратные ссылки; техническая проза плана сама по себе не даёт authority на запуск.
   План и чеклист
   ...
   **более позднее по дате** — смотри на дату, а не на порядок в чате или плане. Доказательство owner-решения:
   ...
   или додумки без owner-метки. В действующем owner-регистре, каноне или открытом плане
   новое решение **заменяет** несовместимую старую прозу: её удаляют, а не оставляют рядом
   зачёркнутой или с меткой «устарело». История остаётся в Git и в уже завершённых audit/evidence/log-записях,
   которые не переписывают. **Не смягчать** owner-текст: без «опционально», «compat», «по
...
| Любой UI-текст/копирайтинг; любой `<Select>`/`SelectTrigger`                                    | [§21](#21-ui-тексты-без-избыточных-пояснений) · [§21a](#21a-тексты-ошибок-предупреждений-и-уведомлений--только-из-словаря) · [§22](#22-ui-select--displaylabel)  |
| Вход, пароль, капча, второй фактор, журнал входов, письма о безопасности (`modules/auth/**`, `app/api/auth/**`, двери `app.password_login_*`, `app.append_user_login_event`) | канон целиком: [`docs/ARCHITECTURE/AUTH_AND_IDENTITY_CANON.md`](docs/ARCHITECTURE/AUTH_AND_IDENTITY_CANON.md), часть V — защита от перебора, капча, журнал, предупреждения. Пороги и сроки оттуда, не из соседнего кода |
| **Слияние аккаунтов, выбор выжившей учётки, ФИО при слиянии, блокировка (глобальная и клиничная), архив в клинике** (`platform-merge/**`, `platformUserMergePreview.ts`, `pgEmailAuth.ts`, `is_blocked`, `org_enrollments`) | канон целиком: [`docs/ARCHITECTURE/AUTH_AND_IDENTITY_CANON.md`](docs/ARCHITECTURE/AUTH_AND_IDENTITY_CANON.md) — §18, §18а и §18б (слияние, ФИО, разбор конфликта врачом), Часть VIII §30–34 (три состояния блокировки, где живут, слияние заблокированных, клиники). **Правила ТОЛЬКО оттуда**: планы в `docs/_TODO/**` хранят цитаты и работы, а не решения |
...
### CI / lint / build-предупреждения — модель выбирать по задаче

Как только нужен зелёный CI/lint/build, использовать самого дешёвого исполнителя, который надёжно закрывает
конкретную причину. Провайдер и модель не закреплены за ролью: обычный механический проход по умолчанию идёт через
Codex, а Claude выбирается только по названному до запуска task-specific преимуществу. Выбор делается по актуальной
карте `/home/dev/brain/docs/MODEL_TIERS.md`; отклонение от её дефолта требует явного обоснования. Сложное,
неочевидное или требующее решения владельца возвращается оркестратору, а не маскируется хаком.

**Смысл:** название роли не доказывает, что ей нужен конкретный бренд модели или frontier-tier; механический цикл
не должен по инерции расходовать более дорогой или дефицитный провайдер.

### Deploy / push

Незакоммиченные или staged Markdown-документы (`*.md`) никогда не блокируют deploy, CI или runtime-операцию:
они не входят в исполняемый артефакт ветки. Не ждать их коммита, не снимать их из staging и не трогать чужую
документационную работу ради запуска. Гейты чистоты дерева проверяют только изменения вне `*.md`.
...
**Прод теперь два, и деплой у них разный.** Старый прод `135.106.162.170` (`adelaide`) обслуживает
`bersoncare.ru` до переезда и описанным выше путём через GitHub Actions; **не трогать его ничем** — ни
чтением env, ни probe. Новый прод `135.106.187.95` (Therapysto/TherapyGo, blue/green на docker) забирает код
из собственного репозитория, GitHub в его схеме не участвует, и выкладка запускается с dev-бокса:

```bash
bash tools/deploy-prod-from-dev.sh          # текущая ветка
bash tools/deploy-prod-from-dev.sh <ref>    # конкретная ветка, тег или коммит
```

Конвейер сам собирает образ, применяет миграции и права, поднимает простаивающий цвет, ждёт его health и
только тогда переключает nginx; не поднялся или не прошли миграции — переключения нет, продолжает работать
предыдущая версия. Откат — `rollback-prod` на хосте. **Руками на хосте делать нечего:** ни миграций, ни
reconcile прав, ни правки env под новый коммит — всё это шаги того же скрипта. Топология, бакеты, каталоги, ключ и
устройство базы: `docs/ARCHITECTURE/SERVER CONVENTIONS.md`, раздел «Новый прод `135.106.187.95`».
...
### Миграции schema B: generated snapshot + timestamp forwards

Owner decision 20.08.2026: историческая webapp-цепочка `0000_*`–`0049_*`, её manifests, frozen-пины и
legacy-name allowlist выведены из активного контура. Их не восстанавливать, не проигрывать и не использовать как
bootstrap. Для A→B cutover schema B приезжает generated snapshot
`deploy/postgres/generated/prod-to-target/schema-pre.sql` + `schema-post.sql`; начальный ledger приезжает
`ledgers-and-baseline.sql`. После snapshot применяются только активные forward-миграции.

**Каждый migration-файл без исключений называется `YYYYMMDDTHHMMSS_slug.sql`** (UTC, точность до секунды),
например `20260820T014233_a_new_reminder_channel.sql`. Номер никто не выдаёт и не резервирует. Старое имя
`NNNN[suffix]_slug` запрещено даже при наличии записи в `meta/_journal.json` или при возврате удалённого frozen-
файла: журнала исключений больше нет. `findMigrationNameViolations` в
`deploy/postgres/privileges/migration-order.mjs` применяет timestamp-правило безусловно; тот же модуль используют
lint-gate и оба migration runner.

**Порядок — имя файла, второго места нет.** Мигратор сортирует
`apps/webapp/db/drizzle-migrations/*.sql` по имени. Вставить ещё не применённую миграцию между двумя другими —
дать ей промежуточный timestamp. `meta/_journal.json` сохраняется пустым (`entries: []`) как совместимый input,
но не задаёт порядок, identity или исключения и руками не расширяется.

**Применённый файл не переименовывать:** имя — identity миграции в колонке `tag` таблицы
`drizzle.__drizzle_migrations`. Переставить ещё не применённую миграцию можно `git mv` на другой timestamp.
`findRenamedAppliedMigrations` отказывает pending-файлу, побайтно совпадающему с foreign ledger row.

**Pending — всё, чего нет в ledger по `tag`, а не всё выше водяного знака.** Хеш не заменяет identity. Каждая
...
операцию reconcile привилегий.

Drizzle ORM migrator здесь не используется: `drizzle-kit migrate`/`push` отказывают, бывшие
`db:migrate:drizzle` и `db:seed-drizzle-meta` ведут к санкционированному маршруту; `drizzle-kit
generate`/`introspect`/`check` продолжают работать.

**Смысл:** поддерживаемый bootstrap — generated schema B, не восстановление истории разработки; единственный
порядок forward-миграций — имя файла, а исключений из timestamp-формата нет.
...
### Перед приземлением миграции — разбор её прав, а не только её объектов

Миграция прав не выдаёт (правило выше), но она создаёт объекты, которым права нужны. Поэтому приёмка ветки с
миграциями включает отдельный разбор, письменно, до `land` и до прогона на TEST. По каждой миграции назвать:

1. какие таблицы и функции она создаёт, меняет или удаляет;
2. под какой ролью выполняется каждое новое/изменённое тело (владелец SECURITY DEFINER-функции, роль рантайма);
3. какие права этой роли нужны, чтобы тело **исполнилось**, а не только чтобы объект существовал;
4. чего из этого нет в декларации (`deploy/postgres/privileges/`) — и добавить туда в этой же ветке.

Разбор строится по телу, а не по названию операции. Смотреть отдельно: `SELECT … FOR UPDATE`/`FOR SHARE` (нужна
табличная привилегия модификации — поколоночного `SELECT` не хватает), запись в таблицу, которую функция раньше
только читала, обращение к новой таблице или новой колонке, смена сигнатуры функции (`function_identity` —
`regprocedure`, DROP+CREATE меняет OID и требует reconcile после миграции), появление новой seam-роли.

Удаление колонки разбирается тем же списком в обратную сторону: колонки нет — упоминания о ней в декларации,
переписи функций и relation-access убрать в той же ветке, иначе генератор выдаст грант на несуществующую колонку
и reconcile упадёт уже на выкатке.

**Смысл:** пайплайн сверяет «объявлено == лежит в базе» и молчит о том, достаточно ли объявленного. Функция с
недостающим правом проходит миграцию, reconcile и деплой зелёными и падает только на первом живом вызове —
`42501` в рантайме, а для пользователя это выглядит не отказом прав, а неверно работающей функцией.
...
На DEV-хосте у проекта работает **ровно один** Next dev-server: Turbopack на `127.0.0.1:5200` из основного
дерева. Worker, verifier и auditor не запускают второй `next dev`/`next start`, не занимают другой webapp-порт и
не подменяют общий Turbopack webpack-режимом. До landing кандидат проходит применимые статические/поведенческие
проверки и независимый аудит; живая UI/runtime-приёмка выполняется после landing на общем `:5200` и остаётся
обязательной частью приёмки, а не доказательством самого landing. Integrator `:4200`, worker и scheduler не
являются вторым Next server.

| Команда                                 | Назначение                                      |
| --------------------------------------- | ----------------------------------------------- |
| `pnpm run dev:turbo`                    | единственный webapp DEV, Turbopack (`:5200`)    |
| `pnpm run webapp:dev`                   | тот же единственный Turbopack webapp (`:5200`)  |
| `pnpm run dev`                          | integrator + тот же Turbopack webapp            |
...
DEV-проверки passwordless-входа `DEV_EMAIL_OTP_DEBUG=true` при `NODE_ENV=development` выводит в лог единственного
...
стоит в integrator непосредственно перед провайдером: локальный DEV подавляет внешнюю отправку, оставляя ровно
одно исключение — `email` в петлевой SMTP-приёмник (§1b.2), — а TEST
доставляет неизменённое сообщение только исходным получателям из `TEST_ACCOUNT_*`, остальных подавляет без
редиректа. Ни одну из этих двух стен нельзя снять флагом тест-раннера: названная среда
(`NODE_ENV=production`/`development`) решает первой. TEST-аккаунты задаются в env телефонами, e-mail, Telegram/MAX ID и web-push user ID.
...
- В `development` все каналы остаются no-op/мок (логируем «отправили бы X», реально не шлём; без ошибок), кроме `email`: он может дойти только до петлевого SMTP-приёмника (`127.0.0.1`, `::1` или `localhost`), а непетлевой хост гасится до отправки. Так DEV не может доставить письмо реальному человеку.
...
- На именованных live DEV/TEST запрещено создавать, сидировать, reconcile-ить или требовать persistent fixture
  клиники, учётки и наборы данных; удалённый fixture-механизм не восстанавливают. Проверки используют уже
  зарегистрированные owner-учётки и клиники. Поведенческий probe может менять данные только внутри гарантированно
...
**Серверная приёмка проверяет действия, а не адреса.** Статус `200`, отсутствие SSR-ошибки и обход списка
маршрутов доказывают только доступность страниц и никогда не считаются приёмкой продукта. При заявлении, что
роль или продукт проверены на DEV/TEST, агент входит штатной живой учёткой этой роли и руками проходит доступные
ей пользовательские функции: создаёт, открывает, изменяет, удаляет/архивирует и восстанавливает данные,
выполняет назначения, запись, переписку, загрузку файлов и остальные применимые действия. Результат каждого
изменяющего действия проверяется после повторного открытия через UI; видимый отказ сопоставляется с runtime-
логом. Непройденная функция называется непроверенной, даже если все её страницы вернули `200`.
...
- Не вставлять токены/пароли/строки подключения с кредами в чат, логи, коммиты, доки (в доки — только
  non-secret операционные факты, см. правило онбординга). Единственное явно опубликованное исключение — общий
  пароль owner-учёток DEV/TEST из §1a; он не превращает остальные DEV/TEST-креды в несекретные.
...
Бриф на реализацию, которая может завести новую функцию/обёртку/гейт, обязан цитировать этот раздел и явно
спросить: нельзя ли параметризовать уже существующую точку вместо новой (§24.2). Дубль, заведённый вместо
параметра существующей точки, — обычное нарушение repo-rule и законный audit finding (§24.6), не стиль.
...
- Migrations via `drizzle-kit generate`; applied only through the sanctioned runners (см. «Миграции schema B»), `drizzle-kit migrate` отказывает
...
Само изменение DB-прав, `GRANT`/`REVOKE` или forward-миграции **не является основанием для full CI**: полный CI
не доказывает права живой runtime-роли. Для такого изменения обязательный evidence — проверка декларации прав и
расхождений (`--check` / `--gaps`), owner-aware preflight и reconcile миграций, затем точный живой сценарий под
той runtime-ролью, которая реально выполняет операцию. Full CI нужен только если вместе с правами изменился
отдельный непокрытый repo-level контракт.
...
- Узкий webapp: `pnpm test:webapp:fast` (проект Vitest `fast`, в GitHub Actions шардируется), `pnpm test:webapp:unit`
  (проект `unit`) или `pnpm test:webapp:route` (проект `route`); два последних вместе —
  `pnpm test:webapp:behavior`. Проект `inprocess` не существует — удалён вместе с disposable-PostgreSQL
  инфраструктурой (`docs/archive/2026-08-no-disposable-db-retirement/RETIREMENT.md`); текущая таксономия webapp —
  `fast`/`unit`/`route` (`apps/webapp/vitest.config.ts`).
...
продуктовым fix не являются: выбранный по §24.1 исполнитель доводит их до зелёного, итоговый diff и SHA принимает
оркестратор без обязательного re-audit. Условия настоящего re-audit — в §24.5.
...
### ⛔ ТЕСТ НЕ ДУБЛИРУЕТ КОД, КОНТРАКТ ИЛИ ТЕКСТ

Этот подраздел обязателен для чтения любому агенту, который создаёт, меняет, сохраняет, удаляет или принимает тест:
воркеру, аудитору, фиксеру и оркестратору. Тест допустим только при двух условиях одновременно: есть независимый
oracle и заранее названа дорогая молчаливая поломка наблюдаемого поведения. Сам проверяемый код, соседний тип,
schema/DTO, registry, список, план и expected, написанный тем же агентом по той же реализации, независимым oracle
не являются.

Запрещено создавать и сохранять тест, который фиксирует:

- точное написание, форматирование, пользовательский текст, label, title, description или имя продукта;
- количество, состав или порядок элементов списка, registry, меню, DOM или другого текущего представления;
- промежуточный объект, аргументы вызова или внутренний контракт между двумя нашими функциями, если конечный
  наблюдаемый результат можно проверить через публичную границу;
- внешний контракт, если expected скопирован из нашего же объявления и меняется вместе с ним в одном коммите;
- то, что и так громко обнаруживается компиляцией, сборкой, типами или непосредственным runtime-падением;
- то, что за нас уже держит стена базы: RLS-политика, FORCE RLS, ограничение уникальности, NOT NULL,
  внешний ключ, GRANT. Проверять надо, что стена СТОИТ (снесли политику — тест покраснел), а не что
  СУБД умеет её применять. Разница в предмете: «чужая строка не видна под законным контекстом» —
  тест; «INSERT с дублем отбивается 23505» — пересказ Postgres.

Между нашими компонентами проверяется путь целиком: подали вход на публичную границу — получили наблюдаемый
результат. Если внутренний формат между функциями изменился несовместимо, такой тест должен краснеть от сломанного
результата, а не от формы промежуточного DTO. Точное утверждение об аргументах допустимо только когда вызов внешней
границы сам является конечным side effect и неверные аргументы дают названный дорогой молчаливый отказ.

Для внешней интеграции копия нашего контракта в expected тоже не является доказательством. Допустимы runtime-
валидация либо проверка против независимой стороны: реального consumer, provider sandbox, опубликованной внешней
спецификации или воспроизведённого инцидента. Если API намеренно изменился, переписывать тест вслед за реализацией
можно только после указания изменившегося независимого oracle; «код теперь выдаёт так» основанием не является.

Если до теста нельзя назвать независимый oracle, дорогую молчаливую поломку и конечное наблюдаемое последствие,
тест не пишется; существующий удаляется. Проверка копирайта, количества или внутренней формы не становится полезной
оттого, что когда-то была зелёной.

**Смысл:** expected, переписанный вместе с реализацией, доказывает только совпадение двух текстов одного автора.
Он не защищает пользователя и обесценивает красный CI при безопасном изменении.

**Линейка владельца (15.09), по ней меряется КАЖДЫЙ тест — существующий тоже.** Дословно: «половина
тестов на то что упадет само из-за стен! и еще куча на то что потребуется править в тестах при любой
правке кода. Тесты нужны там где надо видеть что на выходе из цепочки функций результат такой какой
ждем». Отсюда три вопроса к тесту, и отвечать надо на все три:

1. **Упадёт ли это само?** Стена базы, компилятор, сборка, немедленное падение — тест не нужен.
2. **Придётся ли его править при честной правке кода?** Придётся — он держит форму реализации,
   а не поведение; такой тест вреден, даже когда у него есть зубы.
3. **Видно ли по нему, что на ВЫХОДЕ из цепочки результат тот, какой ждём?** Не видно — не тест.

«Есть зубы» и «стоит существовать» — разные оси, и вторая строже. Тест, который краснеет при смене
внутреннего формата между двумя нашими функциями, зубы имеет и по этой линейке всё равно удаляется.

### ⛔ UI проверяется только live; автоматизированные UI-тесты запрещены

Не создавать, не сохранять и не переносить автоматизированные тесты компонентов, DOM или пользовательского
интерфейса. Файлы `*.ui.test.ts` / `*.ui.test.tsx`, jsdom/RTL-тесты UI, проверки текстов, контролов, модалок,
навигации по клику, раскрытия, скролла, адаптивности, CSS и визуальной геометрии запрещены без исключений.

UI принимается живой проверкой в браузере на нужных viewport. Деньги, доступ, данные, API и бизнес-правила
проверяются на более низкой публичной границе: unit, contract, route или integration; отсутствие подходящего
нижнего слоя не разрешает UI-тест.

**Смысл:** интерфейс меняется по решениям владельца; UI-тест превращает каждую правку формы в переписывание
ложного oracle и мешает менять продукт.
...
unit/route/integration/E2E: defense-in-depth допустим, только если каждый слой ловит другой класс поломки.
...
**Права/RLS/владение таблицами объявляются ТОЛЬКО в `deploy/postgres/privileges/declaration.ts` — единственный
файл, который человек правит руками; `relation-access.ts` и `function-census.ts` с #1069 (2026-09-05) — тонкие
`export … from './declaration.ts'`-шимы без своих данных (см. `deploy/postgres/privileges/README.md`).
Генератор (`generate-cli.mjs`) раскладывает декларацию в SQL-артефакты. Миграция никогда не выдаёт и не отзывает
права — см. «⛔ Миграция не выдаёт и не отзывает права. Никогда» выше.**
...
**Воркер продуктового этапа тесты не пишет.** Он реализует продуктовый код и передаёт результат независимому
аудитору. Каждый аудитор до составления kill-set обязан прочитать §10a и §10b целиком. Первый аудитор новой
поверхности — роль `auditor-live` (обычный read-only `auditor` физически не может внести поломку) — составляет
список поломок по authority и решениям владельца, НЕ читая тестов, затем инспектирует реализацию и существующие
тесты. Недостающие поведенческие тесты, прошедшие фильтр устойчивого контракта и самого дешёвого слоя из §10a,
он добавляет **один раз**. Для уже зелёных тестов он лично вносит поломки и проверяет, что названные authority
поломки убиты; падающий на исходной реализации acceptance-test уже доказывает найденный дефект и передаётся
выбранному по §24.1 исполнителю как фиксированный oracle. Процентного порога нет.

После handoff выбранный по §24.1 исполнитель чинит продукт и гоняет тот же набор до зелёного. Повторный аудитор,
повторное написание тестов и повторный слепой fault injection для той же поверхности не требуются: финальный SHA,
зелёный набор и сам diff проверяет оркестратор. Новый слепой проход нужен только для новой или существенно
изменённой поверхности.
...
- В `*.unit.test.ts` и `*.route.test.ts` не импортировать `page.tsx` ради проверки внутреннего модуля. Выбирать
  узкую публичную границу: API модуля или HTTP handler/proxy.
- Редкий настоящий e2e со страницей должен защищать сквозное поведение, которое нельзя дешевле наблюдать через
  unit/route/integration, и не должен дублировать те же сценарии на каждом слое.
...
- В `apps/webapp/vitest.config.ts` проекты `fast`, `unit` и `route` используют одинаковые по умолчанию
...
- **Модалки:** feature-модалки открывать через `DoctorModal`; он выбирает desktop dialog/right-sheet и канонический mobile bottom-drawer. Legacy `doctor/primitives/Dialog` на mobile обязан делегировать тому же `DrawerContent`; локальные mobile-ветки с собственной геометрией, фоном или анимацией запрещены.
...
- Chrome-типографика — закрытый набор §B.1: page-title `text-[18px] font-medium`; modal title `doctorModalTitleClass` (`text-base font-medium`); modal entity title `doctorModalEntityTitleClass` (`text-[17px] font-semibold`, приглушённый тёмный серо-синий); stage entity в модалке — `doctorModalStageEntityTitleClass` (`text-[19px] font-medium`); card entity — `doctorCardEntityTitleClass` (`text-[18px] font-medium`); на mobile section/body `text-base`, на desktop `text-sm`; **первичная строка списка** `text-base font-normal`; secondary `text-sm`; meta на mobile `text-[13px]`, на desktop `text-xs`; KPI `doctorMetricValueClass` (`text-[18px]`). Micro-роль `text-[10px]`/`text-[11px]` — только бейджи/календарь/оси графиков/mono. Локальные `text-[13px]`, `text-[18px]`, `text-lg`, `text-xl`, `text-2xl`, `text-3xl` запрещены; нестандартные размеры разрешены только через соответствующие общие роли.
- Контролы doctor-zone: input/select-триггер — `h-8`/`h-[32px]`; button/select/input/textarea используют радиус `7px`. Стандартная кнопка `size="sm"` — `h-9`/`h-[36px]`; фактическая поверхность input белая. Радиус задают doctor primitives, не локальный caller.
- Радиусы (§A.3): page-block `10px`, KPI `8px`, doctor button/select/input/textarea `7px`; `rounded-2xl` запрещён. Явный радиус caller (`rounded-none`, icon override и т.п.) сохраняется.
...
## 21a. Тексты ошибок, предупреждений и уведомлений — только из словаря

Любой текст, который увидит человек по известному, ожидаемому коду отказа или события, берётся из
`apps/webapp/src/shared/notifications/notificationText.ts`, а соответствие «машинный код → фраза» — из
`apps/webapp/src/shared/notifications/errorCodeText.ts`. Это относится к `toast.error/success`,
`new UserFacingError(...)`, к полю `message` в ответе маршрута (`NextResponse.json`, `Response.json`,
`jsonError`) и к тексту, который экран показывает вместо ответа сервера.

- Новый код отказа — заведи ключ в словаре и сошлись на него; строку на месте вызова не писать, в том числе
  в `??`-фолбэке и в ветках тернарника.
- Одинаковый смысл — ОДИН ключ на все экраны, а не второй ключ с тем же текстом. Не подходит формулировка —
  правится ключ, для всех сразу.
- Два кода, которые обязаны читаться одинаково по безопасности (чужая роль читается как неверный пароль,
  чтобы не выдать существование учётки), — тоже один ключ, а не два с совпадающим текстом.
- Словарь не участвует в классификации и транспорте ошибок: что вообще можно показать человеку, решает
  отдельный слой (`@/shared/errors/userFacingError`, `safeUserError`, `classifyApiError`) — [§5](#5-clean-architecture-изоляция-модулей).
- Машинный код (`invalid_body`, `not_found`) человеку не показывается никогда.

Сторож: `apps/webapp/scripts/check-notification-text-coverage.mjs` (в `pnpm lint`). Он ловит механически
опознаваемые случаи; чего он не ловит — не разрешено, а просто не поймано.

**Смысл:** одна и та же ошибка, написанная в двух местах разными словами, расходится при первой же правке —
человек видит разный текст на одно и то же, и никто не знает, какой из них правильный.
...
- **Сначала гейт полезности делегирования.** Если причина и место исправления уже локализованы, а сама правка с
  точечной проверкой дешевле постановки, передачи контекста, запуска и приёмки агента, оркестратор делает её сам в
  текущей task-ветке/worktree. Нельзя запускать write-worker/fixer только ради известной одной строки, функции,
  файла или другой механической микроправки. Независимая проверка по риску сохраняется; запрет относится к лишнему
  исполнителю, не к аудитору.
- **Port — режим для полезного делегирования:** `tools/orch-launch.sh` даёт isolation, branch, concurrency и
  acceptance-гейты; через него запускаются параллельные workstream, рискованные/stateful действия и цельные этапы,
  объём которых действительно оправдывает отдельного исполнителя.
- **Прямой spawn допустим:** только для advisory/read-only работы, когда отдельная ветка и автоматические гейты
  избыточны. Оркестратор задаёт точный scope; отчёт такого агента не является независимым `PASS`.
...
- Если существует owner checklist, brief цитирует все относящиеся к проходу ID и текст; roadmap-summary его не
  заменяет. Для цельного делегируемого bounded-этапа без plan-файла authority может быть сам brief.
- Если результат цельного делегируемого этапа — привести активный план/чек-лист/документацию в соответствие фактам
  и owner-решениям, достаточно одного документационного **worker**, а не report-only «аудит»: brief требует за один
  ход найти все расхождения в разрешённом scope, исправить их и закоммитить. Механически локализованную
  документную правку оркестратор делает сам по гейту §24.1. Отдельный auditor/fixer для той же механической
  документации запрещён; независимый docs-аудит нужен только по реальному риску или явному требованию владельца.
- Brief на реализацию, которая может завести новую функцию/обёртку/гейт, обязан явно спросить: нельзя ли
  параметризовать уже существующую точку вместо новой (§5 «Один общий проход») — и назвать кандидата на
  консолидацию, если он есть, вместо того чтобы предлагать воркеру развилку «новая функция или расширение
  старой». Аудитор проверяет то же самое как обычное repo-rule нарушение, не отдельную категорию.
...
- Воркеры используют явный `git -C`, staging только явных путей, коммитят в свою ветку и не пушат. `git add -A`,
  force-push и начало новой worker-работы от устаревшей базы запрещены.
- Аудитор проверяет точный committed candidate от его зафиксированной базы. Сдвиг `feat` после создания
  candidate не требует вливать свежий `feat` перед аудитом: это меняет предмет проверки и втягивает чужие
  workstream-изменения. Пересечения с новыми изменениями `feat` разбирает лид на интеграции; конфликт или
  затронутое поведение требуют повторной целевой проверки уже интеграционного результата.
...
- Сколько stateful фоновых агентов держит один оркестратор — **ориентир, а не запрет**: три комфортно, до пяти
  ...
  не число. **Жёсткого отказа по числу агентов НЕТ** (владелец 22.08.2026, дословно: «убери эту хуйню про
  потолок — я не просил очередной гейт. Я каждому оркестратору выдаю кол-во доступных слотов, и это не общее
  ограничение»). Прежняя редакция объявляла порт носителем host-cap, и порт считал ВСЕХ живых агентов на
  боксе: агенты соседнего чата отказывали запуску, то есть личная квота работала как общий гейт. Теперь порт
  печатает предупреждение с числом живых агентов, а решение принимает оркестратор. Свою квоту он соблюдает
  сам. Независимый непересекающийся scope можно параллелить, общий dev-server и полные прогоны сериализуются.
- Worker не занимает dev-порты и не принимает собственную живую проверку. По канону §1a отдельный verifier/auditor
  не поднимает candidate Next: до landing он независимо принимает применимые кодовые, поведенческие и DB
  rollback-only доказательства, затем live UI/runtime проверяет уже приземлённый результат на единственном общем
  Turbopack `:5200`. Landing переносит кандидат на единственный runtime, но не закрывает live acceptance: реальный
  дефект открывает correction того же scope и требует повторной целевой приёмки.

### 24.4. Аудит: сначала «тест или взгляд»

До классификации каждый аудитор обязан прочитать §10a и §10b целиком, включая обязательный подраздел §10a
«⛔ ТЕСТ НЕ ДУБЛИРУЕТ КОД, КОНТРАКТ ИЛИ ТЕКСТ».
...
- Недостающие поведенческие acceptance-тесты аудитор пишет один раз. Если они падают на текущем продукте, это handoff
  выбранному по §24.1 исполнителю; если зелёные — аудитор один раз подтверждает их fault injection по kill-set.
...
- Исправление делает выбранный по §24.1 исполнитель: локализованную микроправку — сам оркестратор, цельный
  делегируемый этап — воркер. Он гоняет **тот же** набор до зелёного. Оркестратор смотрит итоговый diff и принимает
  зелёный SHA без повторного blind-аудита; повторно писать тесты и kill-set той же поверхности не нужно.
...
- Аудитор не принимает собственный продуктовый fix. Написанные им acceptance-тесты — проверка, не fix; исправление
  делает оркестратор или воркер по гейту §24.1, итоговый зелёный SHA принимает оркестратор.
- Если после первичного аудита остались только падающие acceptance-тесты, нового audit/fix-цикла нет: выбранный по
  §24.1 исполнитель доводит их до зелёного, оркестратор проверяет итог. Новый auditor нужен для оставшихся
  нетестовых findings, новой поверхности или когда объём изменений нельзя надёжно принять оркестратору; повтор той
  же причины без сдвига завершается
  `blocked` с доказательствами и одним вопросом владельцу.
- Для другого разового действия достаточно одного audit-pass: найденное исправляет выбранный по §24.1 исполнитель,
  а оркестратор проверяет конкретное исправление. Серийные новые аудиты той же механической работы ради следующей
  порции замечаний запрещены. Для поведения после fix тот же исполнитель повторяет тесты, не слепой аудит; новый
  blind-pass нужен только новой поверхности.

### 24.7. Проверки и готовность

- Уровни тестов и host-lock определяет §9–§10. Уже зелёный gate на том же SHA не повторять.
- Приёмка тестов блокируется, пока оркестратор не проверил каждый новый или изменённый тест по обязательному
  подразделу §10a «⛔ ТЕСТ НЕ ДУБЛИРУЕТ КОД, КОНТРАКТ ИЛИ ТЕКСТ»: независимый oracle, дорогая молчаливая поломка
  и конечное наблюдаемое последствие названы; проверки копирайта, количества и внутренней формы удалены.
- `stage done`: закрыты пункты этапа и его targeted/phase evidence.
- `land-ready`: candidate прошёл все применимые targeted, audit и DB rollback-only gates. Live UI/runtime
  проверяется после landing на единственном общем Turbopack `:5200`; найденный там дефект повторно открывает тот же
  scope и обязан быть исправлен до `milestone done`.
...
- Ветка с миграциями принимается только после разбора её прав по §1 «Перед приземлением миграции».
