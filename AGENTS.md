...
воркеру как фиксированный oracle. Процентного порога нет.
...
- Если результат задачи — привести активный план/чек-лист/документацию в соответствие фактам и owner-решениям,
  запускать одного документационного **worker**, а не report-only «аудит»: brief требует за один ход найти все
  ограниченные расхождения, исправить их и закоммитить. Оркестратор затем **сам**, без нового агента, проверяет
  итоговый diff и принимает его; отдельный auditor/fixer для той же механической документации запрещён. Если по
  реальному риску всё-таки нужен независимый docs-аудит, найденные им ограниченные правки в разрешённом file/scope
  он исправляет и коммитит тем же ходом; третьего агента нет. Исключения: владелец явно заказал report-only либо
  исправление требует нового owner-решения или выходит за разрешённый scope.
...
- Для другого разового действия достаточно одного audit-pass: найденное исправляет исполнитель, а оркестратор
  проверяет конкретное исправление. Серийные новые аудиты той же механической работы ради следующей порции замечаний
  запрещены. Для поведения после fix воркер повторяет тесты, не слепой аудит; новый blind-pass нужен только новой
  поверхности.
