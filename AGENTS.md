...
Этот файл — **единая точка входа и единственный канонический текст правил** для агентов всех моделей. Cursor
автоматически подхватывает `AGENTS.md` в корне репозитория.

**Разметка правил (решение владельца 01.08.2026 — свести правила в одно место, вычистить дубли):** текст
каждого правила живёт **только здесь**, разделы 1–22 ниже. Файлы `.cursor/rules/*.mdc` сохраняют YAML
frontmatter (`description`/`globs`/`alwaysApply` — по нему Cursor подцепляет правило к нужным путям, это
рабочая механика) и содержат только одну строку-ссылку на соответствующий раздел этого файла. Ничего не
дублировать текстом в двух местах: правку вносить **только в `AGENTS.md`**, `.cursor/rules/*.mdc` не
редактировать, кроме самого frontmatter. `.cursor/rules/test-execution-policy.md` — исключение по чужому
scope (не трогать без отдельного решения).
...
- **STOP-GATE: сначала существующие документы и scripts, потом действия.** Для ЛЮБОЙ существенной задачи агент НЕ имеет права изобретать последовательность, писать новый SQL/script, менять код или запускать команды, пока не:
  ...
  2. нашёл существующие scripts/docs через code-search и точечное чтение;
  3. явно зафиксировал, какие существующие источники являются каноном для текущего действия.
     Для server/deploy/prod/test/env/DB/backup/migration/backfill/reconcile/cutover/Rubitime/clean dump это правило абсолютное и блокирующее.
     Если найденный документ противоречит плану агента — документ побеждает, агент перестраивает план.
...
- **Host identity — blocking gate:** current workspace host `151.241.228.122` is DEV/RELAY/TEST only. PROD is only
  `135.106.162.170`. Never infer PROD from `/opt/projects/bersoncarebot`, `*.prod` files,
  `bersoncarebot-*-prod.service`, public domains, or the word «сервер»: those are stale local remnants/references
  on `151.x`. Before any PROD command, prove target-host = `135.106.162.170` and obtain explicit owner permission.
...
_Источник: `.cursor/rules/unified-task-db.mdc` (alwaysApply)_

**Канон гранулярности и единственное правило для `add`: [`docs/TASKDB_RULES.md`](docs/TASKDB_RULES.md).**
Он побеждает общие списки команд ниже и [`docs/SHARED_TASKDB.md`](docs/SHARED_TASKDB.md); те остаются каноном
порта и статусов.

Все задачи по этому репозиторию ведём в ОБЩЕЙ базе задач (проект `bcb`). Цель: владелец одним запросом видит
«что ждёт его ответа» по всем проектам; ничего не теряется; не нужно перечитывать документы.

**Действие по умолчанию — НАЙТИ, а не ЗАВЕСТИ:** `find` существующий workstream → открыть его план → дописать
туда чекбокс, пункт этапа или новый этап. Карточку при этом не дополнять. `add` — исключение после этого
маршрута, только для НОВОГО цельного workstream, который владелец попросил или одобрил. Своя находка карточкой
не становится никогда. Решение владельца — тоже: «Записать решение — это НЕ завести задачу»
(`docs/ORCHESTRATION_BINDINGS.md`, владелец 28.07).

**Замер 29.07, ради которого это правило появилось:** 121 открытая карточка описывала 25 работ — девять были
детьми одного зонтика, девять заведены из находок агентов, три оказались записями решений, две дублями.
...
- Цельные workstream-карточки репозитория ведём в ОБЩЕЙ базе задач (проект `bcb`) **только** через утилиту-порт:
  ...
  ⛔ **исключение, не обычный шаг.** Только новый цельный workstream по просьбе/одобрению владельца и только
  после `find`. Пустое описание запрещено.
...
**Правило (владелец, 2026-07-20):** индекс — это НЕ «потом». Он часть **каждого** PR, который добавляет
таблицу или колонку, по которой будут фильтровать/сортировать. Дёшево на пустой таблице, больно на большой.

**Что требует индекс в том же PR.** Если новая/изменённая колонка попадает в `WHERE`, `JOIN`, `ORDER BY` или
`GROUP BY` на пути, который исполняется под нагрузкой, — индекс добавляется в **этой же** миграции. Горячие
классы в этом репо:

- **Мультитенант:** любые `org_id` / `clinic_id` / `tenant`-подобные колонки (RLS-фильтры на каждом запросе).
- **Владелец строки:** `user_id` / `patient_id` / `doctor_id`, особенно в паре с временем
  (`(user_id, created_at DESC)`), т.к. так строятся списки/ленты.
- **Аналитика и события:** таймстемпы (`created_at` / `occurred_at`) на event/delivery/reminder-таблицах —
  их сканируют дашборды. Без индекса дашборд держит коннект из маленького staff-пула секундами.
- **Идемпотентность/дедуп:** ключи, по которым проверяется «уже есть?» — `UNIQUE INDEX`.

**Как (чтобы не положить прод).**

- На **маленькой/новой** таблице — обычный `CREATE INDEX IF NOT EXISTS ...` прямо в миграции.
- На **уже большой** таблице — только `CREATE INDEX CONCURRENTLY` (вне транзакции миграции), иначе лок на
  запись на время построения. Если раннер миграций оборачивает всё в транзакцию — вынести в отдельный
  online-шаг, а не в общий батч.
- Композитный порядок колонок: сначала колонка равенства (`org_id`, `user_id`), потом диапазон/сортировка
  (`created_at DESC`).

**Гейт ревью / аудита.** Ревьюер и reality-аудит проверяют: **добавлена таблица/горячая колонка без индекса =
замечание, а не «мелочь».** Существующая дисциплина в репо (165+ индексов, композиты `(*_id, created_at)` уже
есть) — держать, не размывать. Канон рантайм-ёмкости и топологии старта:
...
- Webapp runtime reads must go through `apps/webapp/src/infra/repos/pgSystemSettings.ts`; integrator runtime reads must go through `apps/integrator/src/infra/db/publicSystemSettings.ts` unless a documented process-specific accessor exists. CI enforces direct-read regressions via `apps/webapp/scripts/check-system-settings-accessors.mjs`.
...
- Настройки живут в ОДНОЙ таблице `public.system_settings`; интегратор читает её напрямую. Зеркала `integrator.system_settings` больше нет (удалено 29.07.2026, задача #1076) — см. раздел [system_settings](#4-system_settings-одна-таблица-public-зеркала-нет), `docs/ARCHITECTURE/DATABASE_UNIFIED_POSTGRES.md`, `docs/ARCHITECTURE/CONFIGURATION_ENV_VS_DATABASE.md`.
...
- Интегратор читает настройки **напрямую** из `public.system_settings` (`apps/integrator/src/infra/db/publicSystemSettings.ts`). Зеркала `integrator.system_settings` и проталкивания из webapp больше нет — таблица удалена 29.07.2026 (задача #1076). Прод и тест — **одна** PostgreSQL со схемами `integrator` и `public`, см. `docs/ARCHITECTURE/DATABASE_UNIFIED_POSTGRES.md`.
...
**Отменяет прежнее правило `system-settings-integrator-mirror.mdc` (29.07.2026, решение владельца).** Оно
требовало дублировать каждую настройку в `integrator.system_settings` и проталкивать её HTTP-синхронизацией.

Прод и тест — одна PostgreSQL со схемами `public` и `integrator`
(`docs/ARCHITECTURE/DATABASE_UNIFIED_POSTGRES.md`). Настройки живут ТОЛЬКО в `public.system_settings`;
интегратор читает их напрямую (`apps/integrator/src/infra/db/publicSystemSettings.ts`). Таблица
`integrator.system_settings` удалена миграцией
`apps/integrator/src/infra/db/migrations/core/20260729_0001_drop_integrator_system_settings_mirror.sql`,
синхронизации, очереди и зеркала между схемами больше нет.
...
3. **Новые ключи** — сначала в `ALLOWED_KEYS` (`apps/webapp/src/modules/system-settings/types.ts`), один и тот же `key` и `scope` для всех потребителей. Разных имён под один смысл не заводить.
4. `organization_id IS NULL` — глобальное значение по умолчанию; строка с непустым `organization_id` — переопределение клиники, чтение обязано откатываться на глобальную строку.
5. **Миграции и сиды** пишут настройку в `public.system_settings` и всё — дублировать больше некуда.

Почему убрали (чтобы не отстроили заново): переезд на прямое чтение состоялся давно, а зеркало осталось —
**писалось на каждое изменение и не читалось ниоткуда**: на dev перед сносом 71 вставка, 0 обновлений, **0
чтений по индексу**; ни представлений, ни внешних ключей, ни функций, ссылающихся на таблицу. Ради него жили
SECURITY DEFINER функция `app.enqueue_platform_system_settings_sync` с белым списком из 21 ключа, её генератор
SQL, два проверяющих скрипта и вид задания `system_settings_sync` в очереди; список ключей был переписан
руками в трёх местах и сверялся регуляркой по тексту исходников — из-за чего полный CI 29.07 упал от одной
смены кавычек форматтером. Разбор: задача `#1076`.
...
### 🔴 ДОСТУП К БАЗЕ — ОБА ПРИЛОЖЕНИЯ, БЕЗ ИСКЛЮЧЕНИЙ (добавлено 30.07 по разбору владельца)

**Почему это здесь, наверху.** 30.07 агент написал новый вызов SQL-функции сырым запросом в
`apps/integrator/src/infra/db/directPublic/writeDiaryLfkDirect.ts`. Правило «No raw SQL для нового кода» в этом файле
было — но лежало в разделе про модули вебаппа, поэтому исполнитель, работавший в интеграторе, честно прочитал заголовок и
решил, что к нему это не относится, а дальше скопировал стиль соседнего файла. Владелец: «почему ты не увидел этого ОЧЕНЬ
ВАЖНОГО правила? Значит оно лежит не там где надо».

1. **Правило действует в ОБОИХ приложениях:** `apps/webapp/**` и `apps/integrator/**`. Никаких «у интегратора свои
   правила».
2. **К базе — только через порт своего приложения, на drizzle.** У интегратора это `apps/integrator/src/infra/db/**`
   (drizzle-слой), у вебаппа — `infra/repos/*` плюс порты модулей. Из доменного, сценарного и роут-кода к базе не ходят.
3. **Сырой SQL для НОВОГО кода запрещён** — `pool.query(...)`, `db.query(...)`, `txDb.query(...)` с текстом запроса.
   Существующий сырой SQL остаётся техдолгом и правится отдельными работами, а не «заодно».
4. **Стиль соседнего файла — не авторитет.** Если рядом лежит сырой SQL, это не разрешение писать так же: правило выше
   старого кода. Сомневаешься — спроси, а не копируй.
5. **Брифу исполнителя, который трогает базу, обязательно цитировать этот раздел** — иначе он его не увидит, как и вышло
   30.07.
...
_Источник: `.cursor/rules/ci-fix-test-not-rollback-code.mdc` (alwaysApply)_

Симптом (наблюдалось у слабых агентов на Full CI): после правок в коде падает тест — и агент,
вместо того чтобы обновить тест под новое НАМЕРЕННОЕ поведение, ОТКАТЫВАЕТ только что сделанные
изменения кода, лишь бы зелёный устаревший тест. Это даёт двойную-тройную работу и теряет нужные правки.

ПРАВИЛО:

- Падает тест после изменения кода → разберись, что изменилось и КАКОВО намеренное поведение.
- Если новое поведение верное (мы его и хотели) → ОБНОВИ ТЕСТ под новый код. НЕ откатывай рабочие правки.
- Откатывать код к старому поведению можно ТОЛЬКО если установлено, что изменение было ошибочным (регрессия) —
  а не «потому что так проще пройти старый тест».
- Не уверен, что верно — тест или код → СТОП, спроси владельца/ведущего. Никогда не «подгоняй код под тест» молча.
- Full CI запускать под контролем старшего агента (lead), который понимает, ЧТО и ЗАЧЕМ проверяет каждый тест;
  слабому субагенту Full CI поручать только с явной этой инструкцией и пониманием назначения тестов.

Коротко: код — носитель намерения, тест — его проверка. Меняешь намерение — обновляй проверку, не наоборот.
...
Число audit-проходов задаёт risk-sized режим `docs/ORCHESTRATION_BINDINGS.md`: круги worker→аудит→коррекция сами по
себе не запрещены, пока каждый круг закрывает РАЗНОЕ и чек-лист движется. Стоп — по ПОВТОРУ одной проблемы, а не по
счёту кругов (владелец, 28.07): топтание на одной и той же проблеме два круга подряд → сначала эскалация модели у
аудитора и фиксера, и только если два таких усиленных прохода не сдвинули ту же проблему — стоп + вопрос владельцу.
Раздел [Оркестрация субагентов](#24-оркестрация-субагентов) — полный текст правила. Изменивший код auditor/correction
owner не принимает собственный fix — нужен независимый re-audit.
...
_Источник: `.cursor/rules/tests-check-behaviour-not-circumstances.mdc` (alwaysApply)_

Правило родилось из ревизии тестового набора 29.07.2026 (`docs/_TODO/TEST_SUITE_AUDIT_2026-07-29.md`).
Ниже — не теория, а разбор того, на чём мы реально обожглись, с цифрами.

### Зачем: набор врал в обе стороны

**Врал зелёным.** 28.07 построили тарифную квоту, все тесты вокруг неё зелёные. В живой базе у функции
пересчёта не было права на чтение таблицы — при включённой квоте падало бы создание ЛЮБОЙ страницы.
Тесты этого не видели: они работали на подставленных числах. Нашлось запросом к базе, не тестами.

**Врал красным.** Полный прогон трижды падал не потому, что что-то сломалось, а потому что кто-то
поменял кавычки или перенёс аргумент на другую строку. Разбор каждой такой паники — от получаса до часа.
Хуже: когда красное в половине случаев ничего не значит, к красному перестают относиться серьёзно, и
правка теста «чтобы позеленело» становится нормой. Набор деградирует целиком.

**Замер 29.07.** Прогнали по репозиторию форматтер — преобразование с заведомо нулевым изменением
поведения. База до: 1792 файла / 10 815 тестов, ноль падений. После: 23 файла / 53 теста красных.
**52 из 53 падений — утверждения о тексте исходника.** Ни одно не было поломкой кода.

### Критерий годности: назови поломку

Тест имеет право на жизнь, если **одной строкой** называется конкретная поломка, которую он поймает, в
форме «подали такое — получили неправильное такое», и эта поломка правдоподобна. «Проверяет
корректность» — не ответ. Если названная поломка — «переименование», «смена форматирования», «мы не
вызвали свою же заглушку», тест не нужен.

Спор решается **арбитром**, а не мнением: внеси названную поломку в код руками и посмотри, покраснел ли
тест. Не покраснел — он не ловит то, ради чего написан. Так 29.07 был доказан единственный настоящий
гейт среди 23 красных и опровергнуто «этот тест ценный, вот в шапке написано».

### ⛔ Как НЕ надо — с примерами из этого репозитория

**1. Читать исходник и сверять его текст**

```ts
// ⛔ ловит смену кавычек, а не поведение
const src = readFileSync('service.ts', 'utf8');
expect(src).toContain('listActiveItemsByCategoryCode("symptom_type")');
```

Умер от замены `"` на `'`. Ловил кавычки — больше ничего.

**2. Сравнивать позиции символов в файле**

```ts
// ⛔ утверждает ПОРЯДОК СТРОК в исходнике
expect(src.indexOf('if (structured.lastName')).toBeLessThan(src.indexOf('parseFioCandidate('));
```

Упал на «2678 меньше чем −1»: после смены кавычек вторая строка не нашлась вовсе. Порядок строк в файле
ничего не гарантирует о поведении.

**3. Считать вхождения в тексте**

```ts
// ⛔ выглядит проверкой значения, а на деле тот же пиннинг текста
expect(src.match(/runDrizzleMutationTransaction/g)?.length ?? 0).toBeGreaterThanOrEqual(9);
```

Из-за этой формы классификатор ошибочно счёл 16 файлов «смешанными»: они были текстовыми целиком.

**4. Вмораживать `файл:строка`**

```ts
// ⛔ номера строк едут от любой правки файла
expect(offenders).toEqual(['app/patient/booking/page.tsx:55']);
```

Гейт принципалов покраснел, хотя его анализ (по дереву разбора TypeScript) отработал верно и выдал тот
же список файлов. **Путь — можно, номер строки в ожидаемом значении — нельзя.** Номер показывай в тексте
ошибки, вычисляя его в момент падения:

```ts
// ✅ сверяем пути, номера считаем сейчас и печатаем для человека
expect(offenders, `непокрытые чтения: ${withLines.join(', ')}`).toEqual(paths);
```

**5. Сверять текст `.sql` деплоя или миграции**

```ts
// ⛔ дублирует деплой и делает это хуже
const sql = readFileSync('deploy/postgres/u9a-platform-settings-role.sql', 'utf8');
expect(sql).toContain('GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.system_settings TO app_platform_settings');
```

**Решение владельца 29.07: такие тесты удаляем сразу.** Деплой (`deploy/deploy-test-saas.sh`, шаги
`assert_*`) проверяет права ролей, политики RLS, число защищённых функций и владельца таблиц против
ЖИВОЙ базы и роняет выкат при расхождении. Текстовый тест зеленеет, когда SQL написан верно, но не
применён, и краснеет, когда файл просто переформатировали.

**6. Привязываться к обстоятельствам запуска**

Тест не должен зависеть от того, ГДЕ и КАК его запустили:

```ts
// ⛔ путь от текущего каталога — ломается при запуске из другой директории и в песочнице
readFileSync('../../deploy/postgres/role.sql', 'utf8');
// ⛔ время машины без подмены таймеров — зелёный сегодня, красный в полночь
const now = new Date();
// ⛔ часовой пояс, локаль, случайность, абсолютный путь бокса, живой хост
```

Если файл читать всё же надо — считай путь **от самого файла теста** (`import.meta.url`), а не от
текущего каталога. Время подменяй `vi.useFakeTimers()`/`vi.setSystemTime()`, часовой пояс и локаль
задавай явно.

**7. Проверять, что вызвана собственная заглушка**

```ts
// ⛔ проверяет сам факт вызова, ничего не зная о результате
expect(mockedRepo.save).toHaveBeenCalled();
```

Законно только когда предмет проверки — сам факт обращения к границе (например: при ошибке интегратора
отправка в учёт ошибок вызвана с нужными аргументами, при успехе — не вызвана). Если тест на 80%
состоит из таких утверждений — он не проверяет ничего.

**8. Закреплять тестом собственную ошибку**

28.07 исполнитель, не сумев починить код, подогнал под него тест. Поэтому: **фильтр годности применяет
не автор теста.** Автор всегда найдёт оправдание.
...
- **Доступ к данным:** после отдельного аудита ролей/стен, стабилизации схемы БД и owner-go проверять на
  одноразовой PostgreSQL. До этого fake/DEV-smoke не является доказательством DB/RLS, а новый DB-тест не писать.
- **Структуру кода, если она правда важна, проверяй по ДЕРЕВУ РАЗБОРА, а не регуляркой по тексту.**
  `typescript` уже в зависимостях. Разбор по дереву не замечает кавычек и переносов: `f("x")`, `f('x')`
  и вызов с переносом строки — один и тот же узел.
- **Гейт с самотестом.** Механическая проверка обязана иметь тест «сломай специально — убедись, что
  заметил». Действующий пример — негативный самотест gitleaks в `.github/workflows/security.yml`.
...
- **SUPERSEDED — 2026-07-22:** прежнее правило о белом/inherited workspace background. Latest owner authority
  ...
  canvas `#F6F4EF` through `--doctor-page-gap-background: var(--bc-canvas)`; page header и основные поверхности
  остаются белыми. Глубина — тонкие границы/лёгкие поверхности, не тени (§A). `shadow-*` — только floating
  (медиакарточки §11, поповеры, drag), **не** на page-level секциях/KPI.
...
- Navigation exception (owner 2026-07-22): main doctor sidebar/mobile menu items не являются button pills и
  сохраняют минимальный shared near-rectangular menu radius; 24px control radius на menu rows не распространяется.
  Section tabs имеют отдельный rounded contract.
...
**Решение владельца 01.08.2026:** текст каждого правила живёт только в `AGENTS.md`; `.cursor/rules/*.mdc`
сохраняют frontmatter и содержат одну строку-ссылку на раздел ниже. Все **29** файлов `.cursor/rules/`
(28× `.mdc` + `test-execution-policy.md`) учтены. Исключение по смыслу: §1a
([`LOCAL_DEV_AND_AGENT_TESTING.md`](docs/ARCHITECTURE/LOCAL_DEV_AND_AGENT_TESTING.md)) — канон репозитория, не
rule-файл.

| Файл                                           | В AGENTS                     | Примечание                                                                                                                   |
| ----------------------------------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ |
| `000-critical-integration-config-in-db.mdc`    | §2                            | —                                                                                                                             |
| `answer-itog-without-code-unless-asked.mdc`    | §13                           | —                                                                                                                             |
| `ci-fix-test-not-rollback-code.mdc`            | §10, подраздел «CI: тесты…»  | —                                                                                                                             |
| `clean-architecture-module-isolation.mdc`      | §5                            | —                                                                                                                             |
| `cms-unified-media-picker-layout.mdc`          | §20                           | `alwaysApply: false` — только doctor CMS media pickers                                                                       |
| `codex-language-policy.mdc`                    | §14a                          | —                                                                                                                             |
| `db-migrations-hot-column-indexes.mdc`         | §1, подраздел «Миграции…»    | `alwaysApply: false`, scoped на миграции/схему                                                                               |
| `dev-prod-isolation-no-real-creds.mdc`         | §1b                           | —                                                                                                                             |
| `doctor-ui-shared-primitives.mdc`              | §16                           | —                                                                                                                             |
| `git-commit-push-full-worktree.mdc`            | §7                            | —                                                                                                                             |
| `host-psql-database-url.mdc`                   | §6                            | —                                                                                                                             |
| `no-unsolicited-followups.mdc`                 | §14                           | —                                                                                                                             |
...
| `patient-lfk-means-rehab-program.mdc`          | §18                           | —                                                                                                                             |
| `patient-media-playback-video.mdc`             | §19                           | `alwaysApply: false`, scoped: patient routes                                                                                 |
| `patient-ui-shared-primitives.mdc`             | §15                           | —                                                                                                                             |
| `plan-authoring-execution-standard.mdc`        | §12                           | —                                                                                                                             |
| `pre-push-ci.mdc`                              | §9                            | —                                                                                                                             |
| `push-means-ci-commit-push.mdc`                | §8                            | —                                                                                                                             |
| `runtime-config-env-vs-db.mdc`                 | §3                            | —                                                                                                                             |
| `saas-foundation-aware-development.mdc`        | §4a                           | —                                                                                                                             |
| `server-conventions-and-doc-onboarding.mdc`    | §1                            | —                                                                                                                             |
| `system-settings-single-source.mdc`            | §4                            | —                                                                                                                             |
| `test-execution-policy.md`                     | §10                           | Чужой scope (не трогать) — текст остаётся в файле, не в `AGENTS.md`                                                         |
| `tests-check-behaviour-not-circumstances.mdc`  | §10a                          | —                                                                                                                             |
| `ui-copy-no-excess-labels.mdc`                 | §21                           | —                                                                                                                             |
| `ui-select-trigger-display-label.mdc`          | §22                           | —                                                                                                                             |
| `unified-task-db.mdc`                          | §1, подраздел «Задачи…»      | —                                                                                                                             |
| `webapp-tests-lean-no-bloat.mdc`               | §11                           | —                                                                                                                             |
...
Список файлов, раздел `AGENTS.md` каждого и scoping-примечания — таблица «Покрытие `.cursor/rules` →
`AGENTS.md`» в §23 выше. Отдельная таблица здесь удалена 01.08.2026 как дубль (в т.ч. содержала стухшее имя
`system-settings-integrator-mirror.mdc` — файл переименован в `system-settings-single-source.mdc`, см. §4).
...
**Режим ведения плана из многих этапов (обязателен для ЛЮБОГО плана).** Полный канон — раздел «Универсальный режим
исполнения многоэтапного плана» в `docs/ORCHESTRATION_BINDINGS.md`. Кратко:

- **Стоп — по ПОВТОРУ одной проблемы, а не по счёту кругов (владелец, 28.07 — заменяет прежнюю редакцию «presentation
  = worker + ОДИН аудит, >2 раундов = стоп», SUPERSEDED 28.07: «ты перебдел»).** Круги worker→аудит→коррекция сами по
  себе не запрещены и не считаются провалом, пока каждый круг закрывает РАЗНОЕ и чек-лист движется. Сигнал тревоги —
  топтание на одной и той же (или соседней) проблеме два круга подряд: тогда сначала **эскалация** (сильнее модель у
  аудитора И у фиксера, больше контекста и свободы), а не стоп. **Стоп — только если и два таких усиленных прохода не
  решили ту же проблему** — тогда `status blocked` + вопрос владельцу, не следующий круг.
- **Параллель:** независимые слайсы с непересекающимся file-scope гнать одновременно (≤3), не по очереди.
- **Приёмка владельца в СЕРЕДИНЕ плана,** не только в финале. **«audit PASS» сам по себе ≠ «готово»** — готово =
  галочка чек-листа владельца + зелёный CI + живая проверка.
- **Развилки владельца — заранее одним листом** (рекомендация + safe-default по каждой), не гадать в середине этапа.
- **Debt-first:** планы/этапы, ранее объявленные завершёнными без построчной owner-checklist matrix, сначала проходят
  reconciliation полного linked plan. Доказанное не повторяется; каждый фактический residual остаётся `[ ]` и
  получает exact task до downstream work.
