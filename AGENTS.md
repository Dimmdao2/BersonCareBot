...
- **Advisory/read-only:** прямой spawn допустим для поиска, анализа, plan-critique и рекомендации, если агент не
  меняет файлы/состояние, не запускает acceptance и не закрывает checklist. Его результат — совет, не `PASS`.
- **Repo-work:** worker, fixer, acceptance-auditor и любой агент с записью, тестами, env/БД, сервером, commit или
  evidence запускается через `tools/orch-launch.sh`. Операционная работа без plan-файла использует `ORCH_OPS` и
  self-contained brief; создавать фиктивный план или taskdb-карточку ради запуска запрещено.
...
- Если существует owner checklist, brief цитирует все относящиеся к проходу ID и текст; roadmap-summary его не
  заменяет. Для маленькой операционной задачи authority может быть сам brief.
...
- Первый аудитор новой поверхности составляет kill-set по плану и решениям владельца **до чтения тестов**, затем
