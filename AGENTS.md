...
**Постоянный контракт owner-входа на TEST.** На `https://test.bersoncare.ru` три существующие живые
owner-учётки — врач (DimDAO / Яндекс), глобальный администратор (DimDAO / Gmail) и пациент
(Kinesio Space / Gmail) — используют один общий TEST-only пароль `123456testTEST`. Это намеренно
опубликованный владельцем пароль тестовой среды, а не PROD-секрет и не fixture/bypass. Агент обязан входить
этими учётками штатным email/password-путём при проверке соответствующей роли; если вход не проходит — штатно
восстановить этот пароль на TEST до продолжения проверки. На DEV/PROD и для любых других пользователей этот
пароль не применять.
...
- Не вставлять токены/пароли/строки подключения с кредами в чат, логи, коммиты, доки (в доки — только
  non-secret операционные факты, см. правило онбординга). Единственное явно опубликованное TEST-only
  исключение — общий пароль owner-учёток из §1a; он не превращает остальные TEST-креды в несекретные.
