...
- Полный `pnpm run ci` — на содержательной интеграционной границе deploy/merge/repo-level работы; не повторять после каждой локальной правки уже проверенного этапа — [§9](#9-full-ci-gate), [§10](#10-test-execution-and-audit-policy).
...
**Суть:** полный `pnpm run ci` — не формальный допуск к deploy/merge и не ритуал после изменения файла. Это дорогая
интегральная диагностика на тот случай, когда накопленные изменения могут взаимодействовать между приложениями,
общими контрактами, сборкой или инфраструктурой и целевых проверок уже недостаточно. Вопрос для решения один:
**изменилось ли после последнего зелёного прогона что-то, что способно сломать непокрытую целевым gate часть
репозитория?** Если нет — зелёный evidence переиспользуется; локальная правка проверяется локально и рабочий
сценарий продолжается. Само слово `deploy`, новая попытка deploy, commit или push полного прогона не требуют.
...
Перед передачей накопленного multi-app/repo-level результата в deploy, merge или final integration должен быть
актуальный зелёный интегральный evidence. «Актуальный» означает отсутствие после него изменений, способных
затронуть непроверенную целевыми gate область, а не буквальное совпадение SHA. После локальной правки deploy-wrapper
уже зелёный полный прогон переиспользуется вместе с целевой проверкой правки (reuse — в test-execution-policy).
