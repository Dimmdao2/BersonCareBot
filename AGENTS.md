...
- Один оркестратор держит не больше трёх stateful фоновых агентов; host-cap отдельно задаёт launcher. Независимый
  непересекающийся scope можно параллелить, общий dev-server и полные прогоны сериализуются.
