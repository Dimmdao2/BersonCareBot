...
authority и решениям владельца, НЕ читая тестов, вносит их в код и прогоняет лично; смена автора теста эту слепоту
не заменяет, она лишь переносит её на аудитора. Критерий приёмки — убиты ВСЕ названные authority поломки,
...
- Первый аудитор новой поверхности составляет kill-set по authority и решениям владельца **до чтения тестов**, затем
  `auditor-live` лично вносит каждую поломку в продуктовый код и запускает существующий набор.
- Приёмка бинарная: пойманы все названные планом поломки; процент не заменяет список. Тест, не покрасневший ни на
  одной чужой поломке, защитой не считается.
- Fault injection делается один раз на независимый класс поломки, не на каждый `it`; дерево после аудита чистое.
- После исправления тот же зафиксированный kill-set механически повторяет дешёвый агент или сам worker. Это не
  самосертификация: worker не выбирал поломки. Новый дорогой слепой аудит нужен только для новой поверхности.
...
- Аудитор не принимает собственный продуктовый fix. Если он стал fixer, результат проверяет другой агент; простой
  replay заранее чужого kill-set отдельного аудитора не требует.
- Audit/fix-циклы допустимы, пока закрывают разные причины. Повтор одной причины сначала получает более сильный
  auditor+fixer, полный контекст и цельный scope. Если усиленные проходы снова не сдвинули ту же причину — `blocked`
  с доказательствами и одним вопросом владельцу.
- Для разового действия достаточно одного audit-pass: найденное исправляет исполнитель, а оркестратор проверяет
  ...
  Для поведения после fix повторяется ранее составленный kill-set; новый слепой аудит нужен только новой поверхности.
