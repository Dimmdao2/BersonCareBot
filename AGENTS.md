...
`apps/webapp/db/drizzle-migrations/*.sql`, сортирует по имени и в этом порядке применяет. При
параллельной работе файл получает временный номер с маркером
`-- TEMPORARY LOCAL MIGRATION NUMBER NNNN`; свободное имя назначается перед сведением в `feat`, пока
миграция ещё не применена ни к DEV, ни к TEST. Чтобы встать между двумя уже применёнными, берётся
буквенный суффикс (`0043a_…`) — соседей переименовывать не надо. **Применённый файл не
переименовывать никогда:** имя — это личность миграции в леджере, переименование превратит её обратно
в pending.
...
`meta/_journal.json`, и это единственная оставшаяся работа журнала. Журнал больше НЕ задаёт порядок и
руками не правится: расхождение имени и `when` при сведении веток — та самая болезнь, из-за которой
19.08 журнал чинили трижды и один раз уронили миграцию на TEST. Хешем личность быть не может: файл
миграции здесь штатно правят на месте после применения, и для уже накатанных баз правка инертна.
