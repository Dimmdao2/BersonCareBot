...
Конвейер сам собирает образ, поднимает простаивающий цвет, ждёт его health и только тогда переключает nginx;
не поднялся — переключения нет. Откат — `rollback-prod` на хосте. Топология, бакеты, каталоги, ключ и
