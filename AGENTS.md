...
- Делай **необходимый и достаточный** объём: минимум, при котором требуемое работает; лишнее — не работа — [«Необходимо и достаточно»](#необходимо-и-достаточно).
...
Делай минимальный объём, при котором требуемое поведение работает, — и не больше. Не высасывай проблемы из
пальца: задача решена, когда работает то, что заказано, а не когда закрыты все замеченные по дороге
несовершенства. Нашёл по дороге настоящую проблему вне скоупа — назови её строкой в отчёте, не чини.

Это касается всего: объёма правки, числа новых файлов и абстракций, глубины тестов, длины документа, размера
брифа. Дополнительный слой, «на будущее», «заодно причешу», защита от того, чего не случалось, — не работа.

**Смысл:** лишнее стоит дважды — сначала его пишут, потом его сопровождают, и оно же прячет настоящие
изменения в диффе. Проверка простая: удали кусок — сломается ли то, что заказано? Не сломается — он лишний.
