...
section "Сертификат покрывает произвольный поддомен клиники (B7)"
# Именно этого не хватало 11.09: DNS-запись `*.therapygo.ru` на прод уже смотрит, а сертификат —
# именной список, поэтому клиника, которой нет в списке, умирает на TLS раньше приложения. Проверка
# берёт заведомо несуществующее имя: при wildcard-сертификате TLS обязан пройти (а приложение уже
# потом честно ответит 404), без него — `ssl_verify_result` ненулевой.
v=$(curl -s -o /dev/null -w '%{ssl_verify_result}' -m 15 "https://$UNKNOWN/" 2>/dev/null)
if [ "$v" = 0 ]; then
  ok "$UNKNOWN — TLS проходит, сертификат покрывает произвольный поддомен"
else
  bad "$UNKNOWN" "ssl_verify_result=$v — нет wildcard-сертификата, новая клиника не откроется (пункт B7)"
fi
...
for h in "$STAFF" "$PATIENT" "$BRANDED"; do
  ...
  # `-L`: технический поддомен клиники по решению B8 отвечает 308 на её собственный домен, и без
  # перехода манифест читался бы как пустой — проверка «падала» на правильном поведении.
  m=$(body -L "https://$h$path")
...
for h in "$BRANDED" "$CUSTOM"; do
  ...
  case "$c" in
    ...
    # 308 — штатный ответ технического поддомена клиники, у которой подключён свой домен (B8):
    # редирект с сохранением пути. Это не отказ, поэтому и не провал проверки.
    308) ok "$h → 308 на свой домен клиники (B8)";;
