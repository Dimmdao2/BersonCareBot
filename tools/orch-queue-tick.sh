...
#   3. Запускает ТОЛЬКО через tools/orch-launch.sh, то есть все его ролевые гейты остаются в силе.
...
# 4. Только worker начинает от свежей головы feat. Auditor-live проверяет exact candidate без автослияния.
if [ "$ROLE" = worker ]; then
  ...
  git -C "$CLONE" merge -q --no-edit FETCH_HEAD 2>>"$LOG" || {
    ...
    notify_lead "Конвейер $CLONE_NAME остановлен: конфликт при вливании свежего feat в worker-клон."
    exit 0
  }
fi
