...
# Ориентир параллельности ЭТОГО оркестратора (владелец 22.08.2026: «я каждому оркестратору выдаю
# кол-во доступных слотов, и это не общее ограничение»). НЕ общий host-cap: агенты соседних чатов
# в твой ориентир не входят и запуск не отказывают.
...
# Считаем ТОЛЬКО сам исполняемый процесс порта (`node …/agent-run.mjs`), а не любую строку, где
# это имя упомянуто: раньше в счёт попадала командная строка вызывающей оболочки, если в ней
# встречалось это слово, и потолок 4 отказывал при трёх живых агентах — украденный слот.
LIVE=$(ps -eo args | grep -cE '^[^ ]*node[^ ]* +[^ ]*[a]gent-run\.mjs( |$)' || true)
# Потолок — про ПАРАЛЛЕЛЬНЫЕ ВЕТКИ РАЗРАБОТКИ, а не про любой запуск (владелец 20.08, дословно:
# «количество, которое я тебе говорю, это то, что я рекомендую, как основные потоки разработки…
# если тебе надо прогнать одновременно пять-шесть потоков на короткий момент — делай спокойно,
# но какой нахуй порт»). Пишущий воркер занимает ветку, которую потом надо сводить, — он под
# потолком. Аудит и проверка ветку не заводят и сведения не требуют — они проходят сверх него.
case "$ROLE" in
  worker) CAP=$MAX_AGENTS ;;
  *)      CAP=$(( MAX_AGENTS * 3 )) ;;
esac
# Владелец 22.08.2026, дословно: «убери эту хуйню про потолок — я не просил очередной гейт. Я каждому
# оркестратору выдаю кол-во доступных слотов, и это не общее ограничение». Прежняя редакция считала ВСЕХ
# живых агентов на боксе и отказывала запуску, если соседний чат занял свои слоты — то есть превращала
# личную квоту в общий гейт. Теперь это предупреждение: число остаётся ориентиром для самодисциплины
# оркестратора, а решение принимает он, а не порт.
if [ "$LIVE" -ge "$CAP" ]; then
  echo "  ⚠ живых агентов на боксе: $LIVE (твой ориентир для роли $ROLE — $CAP). Это НЕ отказ: чужие агенты
    в твою квоту не входят. Соотнеси с нагрузкой бокса (8 vCPU) и решай сам.
  Живые сейчас:
$(ps -eo args | grep -E '^[^ ]*node[^ ]* +[^ ]*[a]gent-run\.mjs( |$)' | sed 's/.*--cwd /    /; s/ --run-id / · /')"
fi
...
if [ "$ROLE" = worker ]; then
  ...
  for sha in $(git -C "$CLONE" rev-list --no-merges "$HEAD_MAIN".."$HEAD_CLONE" 2>/dev/null); do
    short=${sha:0:9}
    # Коммит из одного Markdown не несёт исполняемого артефакта: аудировать в нём нечего, а прод он
    # сломать не может. Ровно это суждение порт УЖЕ применяет в land (там главное дерево проверяется
    # на грязь мимо `**/*.md`) — здесь оно просто не было применено, и гейт останавливал запуск из-за
    # план-файла собственной ветки. Владелец 14.09, дословно: «если порт будет блокировать работу
    # из-за этого — это плохая реализация порта, исправь его, чтобы он перестал блокировать работу».
    # Приземление это НЕ ослабляет: в land строка вердикта по-прежнему требуется для каждого коммита,
    # включая документный, — там запись в очереди дёшева и нужна как след того, что именно въехало.
    if [ -z "$(git -C "$CLONE" show --pretty=format: --name-only "$sha" | grep -v '^[[:space:]]*$' | grep -vi '\.md$')" ]; then
      continue
    fi
