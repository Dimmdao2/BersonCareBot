...
# Потолки разные по роли. Воркеров 8 — владелец 01.08 поднял с четырёх («даю еще один слот - даже больше.
# до 8 в сумме»). Прежняя четвёрка — владелец 28.07: «воркеров можно в 4 ручья гнать (только тесты
...
MAX_WORKERS=${ORCH_MAX_WORKERS:-8}
...
# 1. Потолок агентов. auditor-live уходит в порт с --role worker (сандбокс пускает к живой базе),
# поэтому по ps процесс auditor-live неотличим от процесса worker — роли `auditor` с собственным
# отпечатком `--role auditor` в порту больше нет. При запуске auditor-live поэтому консервативно
# считаем аудитором ВСЕ работающие --role worker процессы — иначе один и тот же процесс не
# попадёт ни в один потолок.
...
if [ "$ROLE" = worker ]; then
  ...
  [ "$LIVE_W" -lt "$MAX_AUDITORS" ] || die "уже $LIVE_W аудиторов, потолок $MAX_AUDITORS. В очередь."
  LIVE=$LIVE_W; CAP=$MAX_AUDITORS
fi
