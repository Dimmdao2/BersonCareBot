...
# 3c. Работа по безопасности проектируется по признанному канону, а не по здравому смыслу автора брифа.
# Владелец 01.08: «ты же строишь систему безопасности — ты вообще КАЖДЫЙ свой шаг должен проектировать по
# проверенным канонам». Что предотвращает: своё решение выглядит разумным и проваливается там, где отрасль
# уже знает ответ (реестр маршрутов вместо стены у данных — канон Next.js говорит ставить проверку у
# источника данных, а не перечислять маршруты). Достаточной считается названная внешняя опора: раздел
# официальной документации фреймворка, OWASP/NIST/RFC, практика названной зрелой платформы — со ссылкой.
 if [ -z "${ORCH_NO_CANON:-}" ] && printf '%s' "$BRIEF $PLAN_SLICE" | grep -qiE 'безопасн|права доступа|аутентифик|авторизац|принципал|RLS|стен[аы] арендатор|токен|сесси'; then
  grep -qiE 'канон, по которому|канон проектирования|внешняя опора' "$BRIEF" || die "бриф трогает безопасность,
  но не называет канон, по которому решение проектируется. Добавь блок «Канон, по которому это проектируется:»
  со ссылкой на официальную документацию фреймворка, OWASP/NIST/RFC или названную практику зрелой платформы.
  Что это предотвращает: своё решение выглядит разумным и проваливается там, где отрасль знает ответ.
  Если опоры действительно нет и решение принимается впервые — запускай с ORCH_NO_CANON=\"<почему нет опоры>\"."
fi
...
# Провайдер по умолчанию — claude: владелец запретил запускать Codex/Sol, а прежний дефолт codex означал,
# что забытая переменная окружения молча уводила прогон к запрещённому провайдеру. Переопределяется
# ORCH_PROVIDER. Мимо порта всё равно ничего не запускается — гейты выше не зависят от провайдера.
