...
if [ "$ROLE" = worker ] && [ -z "${ORCH_NO_TESTS:-}" ]; then
  ...
  # 3b (область действия, #1081). Требование оракула применимо ТОЛЬКО к брифу, который поручает
  # писать или менять тесты. Ссылку на правила тестов несёт КАЖДЫЙ бриф (её требует проверка выше),
  # поэтому по ней область определять нельзя: 01.08 гейт отказал трём потокам подряд — починке сырого
  # SQL, правкам порта и своду правил, — ни один из которых тестов не пишет. Ложный отказ
  # останавливает чужую работу и заставляет обходить гейт целиком через ORCH_NO_TESTS, то есть
  # снимает и полезную половину. Триггер тот же, что у 3a: бриф говорит о написании тестов/поломках.
  if grep -qiE 'пиш.{0,20}тест|напис.{0,20}тест|тест.{0,10}на |полом|kill.?set|fault.?injection|покрыт.{0,15}тест' "$BRIEF"; then
