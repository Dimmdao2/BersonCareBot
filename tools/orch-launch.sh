...
#   5. По умолчанию агент идёт БЕЗ песочницы (владелец 01.08) — роль уходит в порт как dev-lead
#      (trusted-full, без bwrap-джейла): виден настоящий apps/webapp/.env.dev, значит агент сам
#      поднимает dev-сервер и проверяет живьём. Вернуть изоляцию — ORCH_ISOLATE=1 (роль → worker,
#      bwrap, .env.dev занулён). Дисциплину auditor-live «не менять файлы кроме поломки» держит
#      бриф и проверка чистоты дерева после прогона, не джейл.
#
...
#
# Вместо неё — только `auditor-live`: по умолчанию в порт уходит без bwrap (доступ к живой базе и
# .env.dev), в потолках считается аудитором, а бриф ей ЗАПРЕЩАЕТ менять файлы. Гарантия не в
# песочнице, а в проверке после прогона: дерево клона обязано остаться чистым (напоминание ниже).
case "$ROLE" in
...
# По умолчанию агент идёт БЕЗ песочницы (владелец, 01.08: «сделай чтобы агенты по умолчанию шли без
# песочницы»). Причина: brain-bwrap маской *.env.* зануляет apps/webapp/.env.dev, поэтому агент не
# поднимает dev-сервер и не видит живую базу — живую приёмку доделывал лид руками, а «слепой аудит»
# сдувался до чтения кода. Роль уходит в порт как `dev-lead`: host-agent-run.mjs (TRUSTED_FULL_ROLES)
# запускает её trusted-full — без bwrap-джейла, с настоящим .env.dev. Прод тут не живёт (другой
# сервер), поэтому радиус — чтение локальных секретов агентом, которого ведёт бриф. Вернуть изоляцию
# для конкретного запуска — ORCH_ISOLATE=1 (тогда роль уходит воркером в bwrap, .env.dev занулён).
