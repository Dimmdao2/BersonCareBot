...
const REVIEWED_TARGET_TARIFFS = new Map([
  // «ПОЛНЫЙ ДОСТУП — РАЗРАБОТЧИК». Строку на DEV переписали 10.09.2026 15:54, и разошлись ДВЕ вещи.
  //
  // 1. Механики: появились `clinic_team: false` и `clinic_vk_community: true` — те же ключи реестра,
  //    что и у соседей. Это то же самое расширение реестра, что и у «СТАРТ» ниже, и едет по тому же
  //    решению владельца 20.08 «пусть сейчас и тарифы и остальное едет как и ехало».
  // 2. `included_seats`: было 1000, стало 1. Это НЕ расширение реестра, а сохранение формы: цена,
  //    валюта и период не менялись. Правкой базы не откатывал — база не место, где принимают
  //    продуктовые решения. Едет как есть, ровно по тому же решению 20.08.
  //
  // ⚠ ВЛАДЕЛЬЦУ, ПЕРЕД ВЫКАТКОЙ ПРОДА: тариф разработчика приезжает с ОДНИМ местом. Если на нём
  // сидит клиника, которой нужно больше одного сотрудника, она упрётся в лимит мест. Разбор — в
  // docs/OWNER_DECISIONS.md, раздел «Что A→B имеет право нести в целевую базу», вместе с «СТАРТ».
  ['d1156dc6-e71e-4225-ad94-93c9d423c9e1', {
    price: 0, currency: 'RUB', seats: 1, period: 'year',
    mechanicsSha256: 'fe69eedf012aaf6678c9c7561e381e9ad7d7d2aecdec16a7d93202b3de3276f4',
  }],
...
export function removeRetiredLinkedPhoneSetting(sql) {
  return sql
    .split('\n')
