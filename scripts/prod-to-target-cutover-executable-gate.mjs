...
CREATE TABLE public.support_conversations (id uuid PRIMARY KEY, organization_id uuid NOT NULL, integrator_conversation_id text, platform_user_id uuid, integrator_user_id bigint, source text NOT NULL, admin_scope text, status text, opened_at timestamptz NOT NULL DEFAULT now(), last_message_at timestamptz NOT NULL DEFAULT now(), created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(), pending_message_drafts jsonb NOT NULL DEFAULT '[]');
