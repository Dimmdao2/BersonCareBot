 ...
 * overage sale (owner decision Р-15, 19.08; срок счёта уточнён Р-19, 20.08).
 *
 ...
 * собирается. Срок оплаты (`expiresAt`) с 20.08 (Р-19) — конец того же отрезка услуги, что и
 * `servicePeriodEndsAt`, а не отдельная настройка «длительность от выставления»; гейт этого не
 * навязывает (дверь вправе посчитать поле как угодно), но по-прежнему требует, чтобы оно пришло
 * ОТТУДА ЖЕ, откуда сумма и отрезок услуги, — второй источник срока запрещён так же, как второй
 * источник цены.
 *
