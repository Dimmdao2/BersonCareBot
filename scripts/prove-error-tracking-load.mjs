...
try {
  ...
  if (
    !accessorQueries.every(
      (query) => query.sql === 'SELECT app.read_global_server_runtime_setting($1) AS value_json',
    )
  ) {
    throw new Error('unexpected_runtime_accessor_query');
  }
  const afterLoadPool = poolSnapshot(nativePool);
  if (JSON.stringify(afterLoadPool) !== JSON.stringify(initialPool))
    throw new Error('db_pool_state_changed');
