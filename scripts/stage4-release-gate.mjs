#!/usr/bin/env node
/**
 * Stage 4 release gate: person-domain reconciliation.
...
async function main() {
  ...
  const reconcile = await run(
    'pnpm',
    ['--dir', 'apps/webapp', 'run', 'reconcile-person-domain'],
    rootDir,
    'reconcile-person-domain',
  );
  if (reconcile) failed.push(reconcile);

  if (failed.length > 0) {
    console.error('[stage4-gate] failed:', failed.join(', '));
    process.exit(1);
  }
  console.log('[stage4-gate] ok: reconciliation passed');
  process.exit(0);
}
