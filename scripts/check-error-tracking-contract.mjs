...
if (/^import .*@sentry\/node/m.test(sources.shared)) failures.push("top-level SDK import is forbidden");
requireMatch(sources.shared, /defaultIntegrations:\s*false/, "default integrations must be disabled");
...
forbid(/enableLogs:\s*true|autoSessionTracking:\s*true|sendDefaultPii:\s*true|includeLocalVariables:\s*true/, "forbidden telemetry is enabled");
forbid(/source.?map.{0,40}(upload|send)|upload.{0,40}source.?map/is, "source-map upload wiring is forbidden");

if (failures.length > 0) {
  console.error(failures.map((failure) => `- ${failure}`).join("\n"));
  process.exitCode = 1;
} else {
  console.log("error-tracking static contract: ok (5 process entrypoints, errors-only, no env/browser/upload wiring)");
}
